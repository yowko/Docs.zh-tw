...
.NET Core 2.2 和更早版本取決於預設的 ICU 行為，這會將 "C" 地區設定對應到 en_US_POSIX 地區設定。 En_US_POSIX 地區設定有不想要的定序行為，因為它不支援不區分大小寫的字串比較。 由於某些 Linux 散發套件會將 "C" 地區設定設為預設地區設定，因此使用者會遇到非預期的行為。
