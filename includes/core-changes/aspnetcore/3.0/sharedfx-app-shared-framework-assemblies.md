...
如需詳細資訊，請參閱[.Net Core 3.0 中的 JSON 未來](https://github.com/dotnet/announcements/issues/90)。 另請參閱已從共用架構移除的[完整二進位檔清單](https://github.com/aspnet/AspNetCore/issues/3755)。

#### <a name="reason-for-change"></a>變更的原因

這項變更可簡化 `Microsoft.AspNetCore.App` 的耗用量，並減少 NuGet 套件和共用架構之間的重複。
...
專案不需要使用 `Microsoft.AspNetCore.App` 中的元件做為 NuGet 套件。 為了簡化 ASP.NET Core 共用架構的目標和使用，已不再產生許多自 ASP.NET Core 1.0 所隨附的 NuGet 套件。 這些套件提供的 Api 仍然可供應用程式使用，`<FrameworkReference>` 到 `Microsoft.AspNetCore.App`。 常見的 API 範例包括 Kestrel、MVC 和 Razor。
...
- 不屬於 `Microsoft.AspNetCore.App` 的 ASP.NET Core 小組所產生的 Api。 例如，下列元件會以 NuGet 套件的形式提供：
  ...
  - 提供協力廠商整合的 Api
  - 實驗性功能
  - 相依性無法[滿足共用架構中的需求](https://github.com/aspnet/AspNetCore/blob/4e44e5bcbedd961cc0d4f6b846699c7c494f5597/docs/SharedFramework.md)的 api
