---
title: "屬性化程式設計模型概觀 (MEF) | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
...
### 延遲匯入  
 在某些情況下，匯入端類別可能需要間接參考所匯入的物件，以避免立即具現化該物件。 在此情況下，類別會使用 `Lazy<T>` 的合約類型來宣告*「延遲匯入」*\(Lazy Import\)。 下列匯入端屬性會宣告延遲匯入。  
...
### 選擇性匯入  
 ...
 您可以使用 `AllowDefault` 屬性，將匯入指定為*「選擇性」*\(Optional\) 匯入。 在此情況下，即使匯入不符合任何可用的匯出，組合也會成功，並且匯入端屬性會設定為其屬性類型的預設值 \(如果是物件屬性則為 `null`，如果是布林值屬性則為 `false`，如果是數值屬性則為零\)。 下列類別使用選擇性匯入。  
...
## 請參閱  
 [Channel 9 影片：透過 Managed Extensibility Framework 擴充應用程式](http://channel9.msdn.com/events/TechEd/NorthAmerica/2009/DTL328)   
 [Channel 9 影片：Managed Extensibility Framework \(MEF\) 2.0](http://channel9.msdn.com/posts/NET-45-Oleg-Lvovitch-and-Kevin-Ransom-Managed-Extensibility-Framework-MEF-20)