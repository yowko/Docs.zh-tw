...
電腦設定檔 machine.config 會儲存在%windir%\Microsoft.NET\Framework 資料夾中。 針對電腦上安裝的每個 .NET Framework 版本，%windir%\Microsoft.NET\Framework 下的資料夾中有個別的 machine.config 檔案，例如：
