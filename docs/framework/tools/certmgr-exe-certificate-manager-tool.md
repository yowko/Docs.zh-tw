...
|**\/CRL**|搭配 **\/add** 使用時會新增 CRL。  搭配 **\/del** 使用時會刪除 CRL。  搭配 **\/put** 使用時會儲存 CRL。  未與 **\/add**、**\/del** 或 **\/put** 選項一起使用時，會顯示 CRL。|  
|**\/CTL**|搭配 **\/add** 使用時會新增 CTL。  搭配 **\/del** 使用時會刪除 CTL。  搭配 **\/put** 使用時會儲存 CTL。  未與 **\/add**、**\/del** 或 **\/put** 選項一起使用時，會顯示 CTL。|  
|**\/del**|從憑證存放區刪除憑證、CTL 和 CRL。|  
|**\/e** *encodingType*|指定憑證的編碼類型。  預設為 `X509_ASN_ENCODING`。|  
|**\/f** *dwFlags*|指定存放區的開放旗標。  這是傳遞到 **CertOpenStore** 的 *dwFlags* 參數。  預設值是 CERT\_SYSTEM\_STORE\_CURRENT\_USER。  只有在使用 **\/y** 選項時，才會將這個選項列入考量。|  
|**\/h**\[**elp**\]|顯示工具的命令語法和選項。|  
...
|**\/r** *location*|識別系統存放區的登錄位置。  只有在指定 **\/s** 選項時，才會將這個選項列入考量。  *location* 必須是下列其中一項：<br /><br /> -   `currentUser` 表示憑證存放區是在 HKEY\_CURRENT\_USER 機碼下方。  這是預設值。<br />-   `localMachine` 表示憑證存放區是在 HKEY\_LOCAL\_MACHINE 機碼下方。|  
...
```  
 ...
 下列命令會將 `my` 系統存放區中通用名稱為 `myCert` 的憑證儲存到名為 `newCert.cer` 的檔案。  
