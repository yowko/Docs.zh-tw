...
<span data-ttu-id="bc0d6-103">原生映像產生器 (Ngen.exe) 是一種可以增進 Managed 應用程式效能的工具。</span><span class="sxs-lookup"><span data-stu-id="bc0d6-103">The Native Image Generator (Ngen.exe) is a tool that improves the performance of managed applications.</span></span> <span data-ttu-id="bc0d6-104">Ngen.exe 會建立原生映像，也就是包含已編譯之處理器特定機器碼的檔案，然後將原生映像安裝到本機電腦上的原生映像快取中。</span><span class="sxs-lookup"><span data-stu-id="bc0d6-104">Ngen.exe creates native images, which are files containing compiled processor-specific machine code, and installs them into the native image cache on the local computer.</span></span> <span data-ttu-id="bc0d6-105">執行階段就可以從快取中使用原生映像，而不是使用 Just-In-Time (JIT) 編譯器來編譯原始組件。</span><span class="sxs-lookup"><span data-stu-id="bc0d6-105">The runtime can use native images from the cache instead of using the just-in-time (JIT) compiler to compile the original assembly.</span></span>  
 ...
 <span data-ttu-id="bc0d6-106">Ngen.exe 在 [!INCLUDE[net_v40_long](../../../includes/net-v40-long-md.md)] 中的變更：</span><span class="sxs-lookup"><span data-stu-id="bc0d6-106">Changes to Ngen.exe in the [!INCLUDE[net_v40_long](../../../includes/net-v40-long-md.md)]:</span></span>  
...
>  <span data-ttu-id="bc0d6-117">您可以在[原生映像產生器 (Ngen.exe) 舊版語法](http://msdn.microsoft.com/library/5a69fc7a-103f-4afc-8ab4-606adcb46324)中找到 .NET Framework 1.0 和 1.1 版的 Ngen.exe 語法。</span><span class="sxs-lookup"><span data-stu-id="bc0d6-117">Ngen.exe syntax for versions 1.0 and 1.1 of the .NET Framework can be found in [Native Image Generator (Ngen.exe) Legacy Syntax](http://msdn.microsoft.com/library/5a69fc7a-103f-4afc-8ab4-606adcb46324).</span></span>  
 ...
 <span data-ttu-id="bc0d6-121">在命令提示字元下輸入下列命令：</span><span class="sxs-lookup"><span data-stu-id="bc0d6-121">At the command prompt, type the following:</span></span>  
...
|`/Profile`|<span data-ttu-id="bc0d6-175">產生可以在分析工具下使用的原生映像。</span><span class="sxs-lookup"><span data-stu-id="bc0d6-175">Generate native images that can be used under a profiler.</span></span>|  
...
-   [<span data-ttu-id="bc0d6-234">疑難排解</span><span class="sxs-lookup"><span data-stu-id="bc0d6-234">Troubleshooting</span></span>](#Troubleshooting)  
    ...
    -   [<span data-ttu-id="bc0d6-237">選擇不產生原生映像</span><span class="sxs-lookup"><span data-stu-id="bc0d6-237">Opting out of native image generation</span></span>](#OptOut)  
...
### <a name="improved-memory-use"></a><span data-ttu-id="bc0d6-249">改進記憶體使用</span><span class="sxs-lookup"><span data-stu-id="bc0d6-249">Improved memory use</span></span>  
 ...
 <span data-ttu-id="bc0d6-252">在終端機服務下執行的應用程式也可以從共用字碼頁獲益。</span><span class="sxs-lookup"><span data-stu-id="bc0d6-252">Applications that are run under terminal services can also benefit from shared code pages.</span></span>  
...
-   <span data-ttu-id="bc0d6-264">由於不需要 JIT 編譯器，原生映像只需要較小的初始工作集。</span><span class="sxs-lookup"><span data-stu-id="bc0d6-264">Native images require a smaller initial working set because there is no need for the JIT compiler.</span></span>  
...
```  
 ...
 <span data-ttu-id="bc0d6-305">組件名稱不包含副檔名。</span><span class="sxs-lookup"><span data-stu-id="bc0d6-305">The assembly name does not include the file name extension.</span></span> <span data-ttu-id="bc0d6-306">可以使用顯示名稱。</span><span class="sxs-lookup"><span data-stu-id="bc0d6-306">Display names can be used.</span></span>  
...
-   <span data-ttu-id="bc0d6-334">.NET Framework 的版本。</span><span class="sxs-lookup"><span data-stu-id="bc0d6-334">The version of the .NET Framework.</span></span>  
...
### <a name="displaying-the-native-image-cache"></a><span data-ttu-id="bc0d6-388">顯示原生映像快取</span><span class="sxs-lookup"><span data-stu-id="bc0d6-388">Displaying the Native Image Cache</span></span>  
...
> [!NOTE]
>  <span data-ttu-id="bc0d6-409">解除安裝 `/debug` 情節不會解除安裝同時包含 `/profile` 和 `/debug.` 的情節。</span><span class="sxs-lookup"><span data-stu-id="bc0d6-409">Uninstalling `/debug` scenarios does not uninstall a scenario that includes both `/profile` and `/debug.`</span></span>  
  
 <span data-ttu-id="bc0d6-410">下列命令會解除安裝 `ClientApp.exe` 特定版本的所有情節：</span><span class="sxs-lookup"><span data-stu-id="bc0d6-410">The following command uninstalls all scenarios for a specific version of `ClientApp.exe`:</span></span>  
  
```  
ngen uninstall "ClientApp, Version=1.0.0.0"  
```  
  
 <span data-ttu-id="bc0d6-411">下列命令會解除安裝 `"ClientApp, Version=1.0.0.0, Culture=neutral, PublicKeyToken=3c7ba247adcd2081, processorArchitecture=MSIL",` 的所有情節，或僅解除安裝該組件的偵錯情節：</span><span class="sxs-lookup"><span data-stu-id="bc0d6-411">The following commands uninstall all scenarios for `"ClientApp, Version=1.0.0.0, Culture=neutral, PublicKeyToken=3c7ba247adcd2081, processorArchitecture=MSIL",` or just the debug scenario for that assembly:</span></span>  
...
```  
 ...
 <span data-ttu-id="bc0d6-412">搭配使用 `install` 動作時，提供副檔名需要從含有組件的目錄執行 Ngen.exe 或指定完整路徑。</span><span class="sxs-lookup"><span data-stu-id="bc0d6-412">As with the `install` action, supplying an extension requires either executing Ngen.exe from the directory containing the assembly or specifying a full path.</span></span>  
  
 <span data-ttu-id="bc0d6-413">如需與原生映像服務相關的範例，請參閱[原生映像服務][Native Image Service]。</span><span class="sxs-lookup"><span data-stu-id="bc0d6-413">For examples relating to the native image service, see [Native Image Service][Native Image Service].</span></span>  
  
## <a name="native-image-task"></a><span data-ttu-id="bc0d6-414">原生映像工作</span><span class="sxs-lookup"><span data-stu-id="bc0d6-414">Native Image Task</span></span>  
 <span data-ttu-id="bc0d6-415">原生映像工作是產生及維護原生映像的 Windows 工作。</span><span class="sxs-lookup"><span data-stu-id="bc0d6-415">The native image task is a Windows task that generates and maintains native images.</span></span> <span data-ttu-id="bc0d6-416">原生映像工作會自動為受支援的案例產生及回收原生映像。</span><span class="sxs-lookup"><span data-stu-id="bc0d6-416">The native image task generates and reclaims native images automatically for supported scenarios.</span></span> <span data-ttu-id="bc0d6-417">(請參閱[建立原生映像](http://msdn.microsoft.com/library/2bc8b678-dd8d-4742-ad82-319e9bf52418))。它也可以讓安裝程式使用 [Ngen.exe (原生映像產生器)](../../../docs/framework/tools/ngen-exe-native-image-generator.md)，在延後的時間建立及更新原生映像。</span><span class="sxs-lookup"><span data-stu-id="bc0d6-417">(See [Creating Native Images](http://msdn.microsoft.com/library/2bc8b678-dd8d-4742-ad82-319e9bf52418).) It also enables installers to use [Ngen.exe (Native Image Generator)](../../../docs/framework/tools/ngen-exe-native-image-generator.md) to create and update native images at a deferred time.</span></span>  
  
 <span data-ttu-id="bc0d6-418">針對電腦上支援的每個 CPU 架構，會各註冊一次原生映像工作，以允許針對以各架構為目標的應用程式進行編譯：</span><span class="sxs-lookup"><span data-stu-id="bc0d6-418">The native image task is registered once for each CPU architecture supported on a computer, to allow compilation for applications that target each architecture:</span></span>  
  
|<span data-ttu-id="bc0d6-419">工作名稱</span><span class="sxs-lookup"><span data-stu-id="bc0d6-419">Task name</span></span>|<span data-ttu-id="bc0d6-420">32 位元電腦</span><span class="sxs-lookup"><span data-stu-id="bc0d6-420">32-bit computer</span></span>|<span data-ttu-id="bc0d6-421">64 位元電腦</span><span class="sxs-lookup"><span data-stu-id="bc0d6-421">64-bit computer</span></span>|  
|---------------|----------------------|----------------------|  
|<span data-ttu-id="bc0d6-422">NET Framework NGEN v4.0.30319</span><span class="sxs-lookup"><span data-stu-id="bc0d6-422">NET Framework NGEN v4.0.30319</span></span>|<span data-ttu-id="bc0d6-423">是</span><span class="sxs-lookup"><span data-stu-id="bc0d6-423">Yes</span></span>|<span data-ttu-id="bc0d6-424">是</span><span class="sxs-lookup"><span data-stu-id="bc0d6-424">Yes</span></span>|  
|<span data-ttu-id="bc0d6-425">NET Framework NGEN v4.0.30319 64</span><span class="sxs-lookup"><span data-stu-id="bc0d6-425">NET Framework NGEN v4.0.30319 64</span></span>|<span data-ttu-id="bc0d6-426">否</span><span class="sxs-lookup"><span data-stu-id="bc0d6-426">No</span></span>|<span data-ttu-id="bc0d6-427">是</span><span class="sxs-lookup"><span data-stu-id="bc0d6-427">Yes</span></span>|  
  
 <span data-ttu-id="bc0d6-428">在 Windows 8 或更新版本上執行時，可在 .NET Framework 4.5 和更新版本中使用原生映像工作。</span><span class="sxs-lookup"><span data-stu-id="bc0d6-428">The native image task is is available in the .NET Framework 4.5 and later versions, when running on Windows 8 or later.</span></span> <span data-ttu-id="bc0d6-429">在舊版 Windows 中，.NET Framework 會使用 [原生映像服務][Native Image Service]。</span><span class="sxs-lookup"><span data-stu-id="bc0d6-429">On earlier versions of Windows, the .NET Framework uses the [Native Image Service][Native Image Service].</span></span>  
  
### <a name="task-lifetime"></a><span data-ttu-id="bc0d6-430">工作存留期</span><span class="sxs-lookup"><span data-stu-id="bc0d6-430">Task Lifetime</span></span>  
 <span data-ttu-id="bc0d6-431">一般而言，Windows 工作排程器會在每天晚上電腦閒置時，開始原生映像工作。</span><span class="sxs-lookup"><span data-stu-id="bc0d6-431">In general, the Windows Task Scheduler starts the native image task every night when the computer is idle.</span></span> <span data-ttu-id="bc0d6-432">此工作會檢查應用程式安裝程式佇列的任何延後工作、任何延後的原生映像更新要求，以及任何自動建立映像的工作。</span><span class="sxs-lookup"><span data-stu-id="bc0d6-432">The task checks for any deferred work that is queued by application installers, any deferred native image update requests, and any automatic image creation.</span></span> <span data-ttu-id="bc0d6-433">此工作會完成未完成的工作項目，然後才關閉。</span><span class="sxs-lookup"><span data-stu-id="bc0d6-433">The task completes outstanding work items and then shuts down.</span></span> <span data-ttu-id="bc0d6-434">如果電腦在工作執行時停止閒置，該工作就會停止。</span><span class="sxs-lookup"><span data-stu-id="bc0d6-434">If the computer stops being idle while the task is running, the task stops.</span></span>  
  
 <span data-ttu-id="bc0d6-435">您也可以透過工作排程器 UI，或是透過手動呼叫 NGen.exe，以手動啟動原生映像工作。</span><span class="sxs-lookup"><span data-stu-id="bc0d6-435">You can also start the native image task manually through the Task Scheduler UI or through manual calls to NGen.exe.</span></span> <span data-ttu-id="bc0d6-436">如果是透過上述其中一種方法來啟動工作，即使電腦不再閒置，還是會繼續執行該工作。</span><span class="sxs-lookup"><span data-stu-id="bc0d6-436">If the task is started through either of these methods, it will continue running when the computer is no longer idle.</span></span> <span data-ttu-id="bc0d6-437">使用 NGen.exe 手動建立的映像會列為優先順序，以為應用程式安裝程式啟用可預期的行為。</span><span class="sxs-lookup"><span data-stu-id="bc0d6-437">Images created manually by using NGen.exe are prioritized to enable predictable behavior for application installers.</span></span>  
  
## <a name="native-image-service"></a><span data-ttu-id="bc0d6-438">原生映像服務</span><span class="sxs-lookup"><span data-stu-id="bc0d6-438">Native Image Service</span></span>  
 <span data-ttu-id="bc0d6-439">原生映像服務是產生及維護原生映像的 Windows 服務。</span><span class="sxs-lookup"><span data-stu-id="bc0d6-439">The native image service is a Windows service that generates and maintains native images.</span></span> <span data-ttu-id="bc0d6-440">原生映像服務可讓開發人員將原生映像的安裝和更新延後到電腦閒置時的時段。</span><span class="sxs-lookup"><span data-stu-id="bc0d6-440">The native image service allows the developer to defer the installation and update of native images to periods when the computer is idle.</span></span>  
  
 <span data-ttu-id="bc0d6-441">一般來說，會由安裝程式 (installer) 為應用程式或更新啟始原生映像服務。</span><span class="sxs-lookup"><span data-stu-id="bc0d6-441">Normally, the native image service is initiated by the installation program (installer) for an application or update.</span></span> <span data-ttu-id="bc0d6-442">針對優先順序 3 的動作，此服務會在電腦閒置期間執行。</span><span class="sxs-lookup"><span data-stu-id="bc0d6-442">For priority 3 actions, the service executes during idle time on the computer.</span></span> <span data-ttu-id="bc0d6-443">此服務會儲存其狀態，而且必要時，可以在多次重新開機後再繼續。</span><span class="sxs-lookup"><span data-stu-id="bc0d6-443">The service saves its state and is capable of continuing through multiple reboots if necessary.</span></span> <span data-ttu-id="bc0d6-444">多個影像編譯可排入佇列。</span><span class="sxs-lookup"><span data-stu-id="bc0d6-444">Multiple image compilations can be queued.</span></span>  
  
 <span data-ttu-id="bc0d6-445">此服務也會與手動 Ngen.exe 命令互動。</span><span class="sxs-lookup"><span data-stu-id="bc0d6-445">The service also interacts with the manual Ngen.exe command.</span></span> <span data-ttu-id="bc0d6-446">手動命令優先於背景活動。</span><span class="sxs-lookup"><span data-stu-id="bc0d6-446">Manual commands take precedence over background activity.</span></span>  
  
> [!NOTE]
>  <span data-ttu-id="bc0d6-447">在 Windows Vista 上，為原生映像服務顯示的名稱是 "Microsoft.NET Framework NGEN v2.0.50727_X86" 或 "Microsoft.NET Framework NGEN v2.0.50727_X64"。</span><span class="sxs-lookup"><span data-stu-id="bc0d6-447">On Windows Vista, the name displayed for the native image service is "Microsoft.NET Framework NGEN v2.0.50727_X86" or "Microsoft.NET Framework NGEN v2.0.50727_X64".</span></span> <span data-ttu-id="bc0d6-448">在所有舊版 Microsoft Windows 上，名稱為「.NET 執行階段最佳化服務  v2.0.50727_X86」或「.NET 執行階段最佳化服務  v2.0.50727_X64」。</span><span class="sxs-lookup"><span data-stu-id="bc0d6-448">On all earlier versions of Microsoft Windows, the name is ".NET Runtime Optimization Service v2.0.50727_X86" or ".NET Runtime Optimization Service v2.0.50727_X64".</span></span>  
  
### <a name="launching-deferred-operations"></a><span data-ttu-id="bc0d6-449">啟動延後的作業</span><span class="sxs-lookup"><span data-stu-id="bc0d6-449">Launching Deferred Operations</span></span>  
 <span data-ttu-id="bc0d6-450">在開始安裝或升級之前，建議先暫停服務。</span><span class="sxs-lookup"><span data-stu-id="bc0d6-450">Before beginning an installation or upgrade, pausing the service is recommended.</span></span> <span data-ttu-id="bc0d6-451">這樣可以確保當安裝程式正在複製檔案或是在全域組件快取中放置組件時，該服務不會執行。</span><span class="sxs-lookup"><span data-stu-id="bc0d6-451">This ensures that the service does not execute while the installer is copying files or putting assemblies in the global assembly cache.</span></span> <span data-ttu-id="bc0d6-452">下列 Ngen.exe 命令列會暫停服務：</span><span class="sxs-lookup"><span data-stu-id="bc0d6-452">The following Ngen.exe command line pauses the service:</span></span>  
  
```  
ngen queue pause  
```  
  
 <span data-ttu-id="bc0d6-453">當所有延後的作業都排入佇列時，下列命令可讓服務繼續：</span><span class="sxs-lookup"><span data-stu-id="bc0d6-453">When all deferred operations have been queued, the following command allows the service to resume:</span></span>  
  
```  
ngen queue continue  
```  
  
 <span data-ttu-id="bc0d6-454">若要在安裝新應用程式或是更新共用元件時，延後產生原生映像，請使用 `/queue` 選項搭配 `install` 或 `update` 動作。</span><span class="sxs-lookup"><span data-stu-id="bc0d6-454">To defer native image generation when installing a new application or when updating a shared component, use the `/queue` option with the `install` or `update` actions.</span></span> <span data-ttu-id="bc0d6-455">下列 Ngen.exe 命令列會為共用元件安裝原生映像，並針對可能受影響的所有根目錄執行更新：</span><span class="sxs-lookup"><span data-stu-id="bc0d6-455">The following Ngen.exe command lines install a native image for a shared component and perform an update of all roots that may have been affected:</span></span>  
...
```  
 ...
 <span data-ttu-id="bc0d6-456">`update` 動作會重新產生已經失效的所有原生映像，而不只是使用 `MyComponent` 的那些原生映像。</span><span class="sxs-lookup"><span data-stu-id="bc0d6-456">The `update` action regenerates all native images that have been invalidated, not just those that use `MyComponent`.</span></span>  
  
 <span data-ttu-id="bc0d6-457">如果您的應用程式包含許多根目錄，您可以控制延後動作的優先順序。</span><span class="sxs-lookup"><span data-stu-id="bc0d6-457">If your application consists of many roots, you can control the priority of the deferred actions.</span></span> <span data-ttu-id="bc0d6-458">下列命令會將三個根目錄的安裝排入佇列。</span><span class="sxs-lookup"><span data-stu-id="bc0d6-458">The following commands queue the installation of three roots.</span></span> <span data-ttu-id="bc0d6-459">會先安裝 `Assembly1`，而不會等到閒置時間。</span><span class="sxs-lookup"><span data-stu-id="bc0d6-459">`Assembly1` is installed first, without waiting for idle time.</span></span> <span data-ttu-id="bc0d6-460">`Assembly2` 也是不會等到閒置時間，但要等到所有優先權 1 動作都完成之後才安裝。</span><span class="sxs-lookup"><span data-stu-id="bc0d6-460">`Assembly2` is also installed without waiting for idle time, but after all priority 1 actions have completed.</span></span> <span data-ttu-id="bc0d6-461">`Assembly3` 則會在服務偵測到電腦閒置時安裝。</span><span class="sxs-lookup"><span data-stu-id="bc0d6-461">`Assembly3` is installed when the service detects that the computer is idle.</span></span>  
...
```  
 ...
 <span data-ttu-id="bc0d6-462">您可以使用 `executeQueuedItems` 動作，強制佇列的動作同步執行。</span><span class="sxs-lookup"><span data-stu-id="bc0d6-462">You can force queued actions to occur synchronously by using the `executeQueuedItems` action.</span></span> <span data-ttu-id="bc0d6-463">如果您提供選擇性的優先權，這個動作只會影響具有相等或較低優先權的佇列動作。</span><span class="sxs-lookup"><span data-stu-id="bc0d6-463">If you supply the optional priority, this action affects only the queued actions that have equal or lower priority.</span></span> <span data-ttu-id="bc0d6-464">預設優先權為 3，因此下列 Ngen.exe 命令會立即處理所有佇列的動作，並且在完成之前都不會傳回：</span><span class="sxs-lookup"><span data-stu-id="bc0d6-464">The default priority is 3, so the following Ngen.exe command processes all queued actions immediately, and does not return until they are finished:</span></span>  
  
```  
ngen executeQueuedItems  
```  
  
 <span data-ttu-id="bc0d6-465">同步命令是由 Ngen.exe 執行，而且不會使用原生映像服務。</span><span class="sxs-lookup"><span data-stu-id="bc0d6-465">Synchronous commands are executed by Ngen.exe and do not use the native image service.</span></span> <span data-ttu-id="bc0d6-466">您可以在原生映像服務執行時，使用 Ngen.exe 來執行動作。</span><span class="sxs-lookup"><span data-stu-id="bc0d6-466">You can execute actions using Ngen.exe while the native image service is running.</span></span>  
  
### <a name="service-shutdown"></a><span data-ttu-id="bc0d6-467">服務關閉</span><span class="sxs-lookup"><span data-stu-id="bc0d6-467">Service Shutdown</span></span>  
 <span data-ttu-id="bc0d6-468">在執行包含 `/queue` 選項的 Ngen.exe 命令來啟始服務之後，服務會在背景執行，直到所有動作完成為止。</span><span class="sxs-lookup"><span data-stu-id="bc0d6-468">After being initiated by the execution of an Ngen.exe command that includes the `/queue` option, the service runs in the background until all actions have been completed.</span></span> <span data-ttu-id="bc0d6-469">此服務會儲存其狀態，因此必要時，可以在多次重新開機後再繼續。</span><span class="sxs-lookup"><span data-stu-id="bc0d6-469">The service saves its state so that it can continue through multiple reboots if necessary.</span></span> <span data-ttu-id="bc0d6-470">當服務偵測到已經沒有動作佇列時，會重設其狀態，使其不會在下次電腦開機時重新啟動，然後將服務本身關閉。</span><span class="sxs-lookup"><span data-stu-id="bc0d6-470">When the service detects that there are no more actions queued, it resets its status so that it will not restart the next time the computer is booted, and then it shuts itself down.</span></span>  
  
### <a name="service-interaction-with-clients"></a><span data-ttu-id="bc0d6-471">服務與用戶端互動</span><span class="sxs-lookup"><span data-stu-id="bc0d6-471">Service Interaction with Clients</span></span>  
 <span data-ttu-id="bc0d6-472">在 .NET Framework 2.0 版中，與原生映像服務的唯一互動，是透過命令列工具 Ngen.exe。</span><span class="sxs-lookup"><span data-stu-id="bc0d6-472">In the .NET Framework version 2.0, the only interaction with the native image service is through the command-line tool Ngen.exe.</span></span> <span data-ttu-id="bc0d6-473">在安裝指令碼中使用命令列工具，將原生映像服務的動作加入佇列中，並與服務互動。</span><span class="sxs-lookup"><span data-stu-id="bc0d6-473">Use the command-line tool in installation scripts to queue actions for the native image service and to interact with the service.</span></span>  
  
## <a name="see-also"></a><span data-ttu-id="bc0d6-474">另請參閱</span><span class="sxs-lookup"><span data-stu-id="bc0d6-474">See Also</span></span>  
 [<span data-ttu-id="bc0d6-475">工具</span><span class="sxs-lookup"><span data-stu-id="bc0d6-475">Tools</span></span>](../../../docs/framework/tools/index.md)  
 [<span data-ttu-id="bc0d6-476">Managed 執行程序</span><span class="sxs-lookup"><span data-stu-id="bc0d6-476">Managed Execution Process</span></span>](../../../docs/standard/managed-execution-process.md)  
 [<span data-ttu-id="bc0d6-477">執行階段如何找出組件</span><span class="sxs-lookup"><span data-stu-id="bc0d6-477">How the Runtime Locates Assemblies</span></span>](../../../docs/framework/deployment/how-the-runtime-locates-assemblies.md)  
 [<span data-ttu-id="bc0d6-478">命令提示字元</span><span class="sxs-lookup"><span data-stu-id="bc0d6-478">Command Prompts</span></span>](../../../docs/framework/tools/developer-command-prompt-for-vs.md)
