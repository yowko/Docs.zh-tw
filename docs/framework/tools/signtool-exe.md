...
> [!Note]  
> Windows 10 SDK、Windows 10 的 HLK、Windows 10 WDK 和 Windows 10 ADK **組建20236和更新版本** 都需要指定摘要演算法。 SignTool `sign` 命令需要分別在 `/fd` 簽署和時間戳記期間指定檔案 **摘要演算法** 和 `/td` **時間戳記摘要演算法** 選項。 警告 (錯誤碼0，如果在簽署期間未指定，則會擲回) ， `/fd` 而且 `/td` 在時間戳記期間未指定時，會擲回。 在 SignTool 的較新版本中，警告會變成錯誤。 建議您將 SHA256 視為比產業的 SHA1 更安全。  
...
|`/fd`|指定要用於建立檔案簽章的檔案摘要演算法。 </br> **注意：** 如果在 `/fd` 簽署時未提供參數，則會產生警告。 預設演算法為 SHA1，但建議使用 SHA256。|
|`/fd`  *certHash*|指定字串 *certHash* 會預設為用於簽署憑證的演算法。 </br> **注意：** 僅適用于 Windows 10 套件組建20236和更新版本。|  
...
|`/td`  *alg*|與 `/tr` 選項一起使用以要求 RFC 3161 時間戳記伺服器使用的摘要演算法。 </br> **注意：** 如果在時間 `/td` 戳時未提供參數，則會產生警告。 預設演算法是 SHA1，但建議使用 SHA256。 <br/> `/td`參數必須在切換之後宣告 `/tr` ，而不是在之前。 如果在 `/td` 參數之前宣告參數 `/tr` ，則傳回的時間戳記是來自 SHA1 演算法，而不是預期的 SHA256 演算法。 |
