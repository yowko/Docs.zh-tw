...
>  使用 Visual Studio 的開發人員也可以使用 [!INCLUDE[vs_ordesigner_long](../../../includes/vs-ordesigner-long-md.md)] 來產生實體類別。 命令列方法會針對大型資料庫做適當調整。 由於 SqlMetal 是命令列工具，因此您可以在建置處理序中使用它。  
 ...
 若要執行此工具，請使用 \[開發人員命令提示字元\] \(或 Windows 7 中的 \[Visual Studio 命令提示字元\]\)。 如需詳細資訊，請參閱 [命令提示字元](../../../docs/framework/tools/developer-command-prompt-for-vs.md)。在命令提示字元中，輸入下列命令：  
...
-   產生程式碼輸出檔。  
     ...
     使用適當的命令列選項，您就可以產生 [!INCLUDE[vbprvb](../../../includes/vbprvb-md.md)] 或 C\# 原始程式碼，或是產生 XML 對應檔。  
