...
> [!NOTE]
> *Ilasm.exe* 的所有選項都不區分大小寫，並以前三個字母來辨識。 例如， **/lis**相當於 **/listing** ，而 **/res**： myresfile.res 相當於 **/resource**： myresfile.res. .res。指定引數的選項會接受冒號（:)或等號（=）做為選項和引數之間的分隔符號。 例如， **/output**:*file.ext* 等同於 **/output=** =*file.ext*。
