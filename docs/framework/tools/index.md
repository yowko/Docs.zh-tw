...
## 在本節中  
 ...
 可讓您檢視和設定電腦原則層級、使用者原則層級和企業原則層級的安全性原則。 在 [!INCLUDE[net_v40_long](../../../includes/net-v40-long-md.md)] \(含\) 以後的版本中，除非將 [\<legacyCasPolicy\> 項目](../../../docs/framework/configure-apps/file-schema/runtime/netfx40-legacysecuritypolicy-element.md) 設為 `true`，否則此工具並不會影響程式碼存取安全性 \(CAS\) 原則。 如需詳細資訊，請參閱[安全性變更](../../../docs/framework/security/security-changes.md)。  
...
## 相關章節  
 ...
 包含 isXPS 一致性工具 \(isXPS.exe\)、效能分析工具以及 XamlPad 之類的工具，後者是可延伸應用程式標記語言 \(XAML\) 的基本視覺編輯器。  
