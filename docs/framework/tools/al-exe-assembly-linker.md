...
|al1019|建立元件時中繼資料失敗-原因<br /><br /> 產生組件時因指定原因而中斷。 例如，如果找不到使用 **/win32res** 選項指定的檔案，便會發生這個錯誤。|
...
|al1022|讀取內嵌資源 ' file' ' 時發生錯誤—原因<br /><br /> *Al.exe* 因指定的原因而無法讀取傳遞至 **/embedresource** 選項的檔案。|
|al1023|內嵌資源 ' file' ' 時發生錯誤—原因<br /><br /> 作業系統因指定原因而無法將資源檔嵌入組件中。|
...
|al1030|未妥善安裝密碼編譯服務或缺乏適當的金鑰提供者<br /><br /> 您可能必須重新安裝作業系統，或安裝用來建立金鑰的一些密碼編譯公用程式。|
...
|al1039|全域組件快取管理員的初始化失敗-原因<br /><br /> 重新安裝 Visual Studio 或 Windows SDK。|
|al1040|無法將元件安裝到快取中—原因<br /><br /> 只有簽署的組件才能安裝至快取。 如需詳細資訊，請參閱[全域組件快取](../app-domains/gac.md)。|
...
|al1048|匯入元件的模組 ' module ' assembly' 時發生錯誤—原因<br /><br /> 開啟多檔案組件的非資訊清單檔時發生錯誤。 這個錯誤並不是由 *Al.exe* 直接發出，但是可以利用程式設計方式傳遞至使用 *Al.exe* 的處理序。|
...
|al1056|參考的組件 'filename' 是當地語系化的附屬組件<br /><br /> 在建立目前的組件時參考到使用 <xref:System.Reflection.AssemblyCultureAttribute> 屬性建立的組件。 <xref:System.Reflection.AssemblyCultureAttribute> 屬性表示檔案是當地語系化的附屬組件，不適合參考附屬組件。 請改為參考主要的父元件。|
...
|al1066|命令列或回應檔中不允許字元 ' character '<br /><br /> 在命令列或檔案中找到無效的字元。|
...
|al1070|不可知的元件不能有處理器特定模組的模組名稱<br /><br /> 如果您使用 **/platform：中立**的（或未指定 **/platform**）進行建立，則會在您嘗試新增不是中立的模組（使用 **/addmodule**）時產生錯誤。 這類似嘗試將 i386 obj 檔連結至 ia64 obj。<br /><br /> 非無從驗證模組的主要來源為 C++。 如果您搭配 C++ 模組使用 **/addmodule**，可能必須修改建置指令碼，以指定適當的 **/platform** 設定。|
