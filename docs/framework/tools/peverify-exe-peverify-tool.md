...
|**\/il**|針對 *filename* 所指定組態中實作的方法，執行 MSIL 類型安全驗證檢查。  除非您指定 **\/quiet** 選項，否則此工具會針對每個找到的問題傳回詳細描述。|  
|**\/md**|在 *filename* 所指定的組件上執行中繼資料驗證檢查。  這會逐步檢查檔案內的所有中繼資料結構，並報告遇到的所有驗證問題。|  
...
## 備註  
 通用語言執行平台透過以類型安全的方式執行應用程式程式碼，協助強制執行安全性和隔離機制。  正常情況下，不是[可驗證類型安全](http://msdn.microsoft.com/zh-tw/095cd1f6-d8db-4c0e-bce2-83ccb34dd5dc)的程式碼無法執行，但是您可以設定安全性原則，讓受信任但無法驗證的程式碼執行。  
...
## 請參閱  
 ...
 [NIB: Writing Verifiably Type\-Safe Code](http://msdn.microsoft.com/zh-tw/d18f10ef-3b48-4f47-8726-96714021547b)   
 [Type Safety and Security](http://msdn.microsoft.com/zh-tw/095cd1f6-d8db-4c0e-bce2-83ccb34dd5dc)   
