...
|**culture**|選擇性。 此應用程式執行的兩部分 ISO 國家及區域代碼。 預設為 `neutral`。|  
...
|**culture**|選擇性。 此應用程式執行的兩部分 ISO 國家/區域代碼。 預設為 `neutral`。|  
...
|**選擇應用程式應該於何時檢查更新檔**|提供兩個更新檢查選項：<br /><br /> -   [應用程式啟動之前]。 執行應用程式之前，先執行更新檢查。<br />-   [應用程式啟動後]。 應用程式的主要表單一完成初始化便會開始檢查更新，並將在下一次應用程式啟動之後執行。|  
|**更新檢查頻率**|決定多久 [!INCLUDE[ndptecclick](../../../includes/ndptecclick-md.md)] 應該檢查更新：<br /><br /> -   [每次執行應用程式時檢查]。 [!INCLUDE[ndptecclick](../../../includes/ndptecclick-md.md)] 將在每次使用者開啟應用程式時執行更新檢查。<br />-   **檢查間隔**：選取檢查更新前，必須經過多久的時間間隔和單位 (小時、天或週)。|  
