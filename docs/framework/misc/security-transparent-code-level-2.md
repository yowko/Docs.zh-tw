...
<a name="top"></a> 層級 2 透明度是 [!INCLUDE[net_v40_long](../../../includes/net-v40-long-md.md)] 引入的。 此模型的三個原則是透明程式碼、安全性安全關鍵程式碼和安全性關鍵程式碼。  
...
### LinkDemand 支援  
 層級 2 透明度模型會將 <xref:System.Security.Permissions.SecurityAction> 取代成 <xref:System.Security.SecurityCriticalAttribute> 屬性。 在舊版 \(層級 1\) 程式碼中，<xref:System.Security.Permissions.SecurityAction> 可自動視為 <xref:System.Security.Permissions.SecurityAction>。  
...
> [!NOTE]
>  `SafeCritical` 方法會傳回 <xref:System.Type.IsSecurityCritical%2A> 和 `` 的 `true`，以及傳回 <xref:System.Reflection.MethodBase.IsSecuritySafeCritical%2A>，因為 `SafeCritical` 確實相當重要 \(其功能與關鍵程式碼相同，不過可從透明程式碼呼叫\)。  
...
### 在完全信任中略過驗證  
 ...
 <xref:System.Security.SecurityRulesAttribute.SkipVerificationInFullTrust%2A> 屬性預設為 `false`，因此這個屬性必須設定為 `true`，才能略過驗證。 您應該僅針對最佳化目的進行此作業。 您應該使用 [PEVerify 工具](../../../docs/framework/tools/peverify-exe-peverify-tool.md)中的 `transparent` 選項，確認組件中的透明程式碼是否可驗證。  
