...
### <a name="in-version-20-and-later-versions-of-the-net-framework"></a>在 2.0 版和更新版本的.NET Framework  
 與舊版中，不同的是 2.0 版和更新版本的.NET Framework 時，執行對委派建立者的安全性動作是在建立和呼叫委派。  
...
## <a name="link-demands-and-wrappers"></a>連結要求和包裝函式  
 ...
 如果完全信任程式碼呼叫屬性、 事件或保護方法[LinkDemand](../../../docs/framework/misc/link-demands.md)，如果成功呼叫**LinkDemand**呼叫端的權限檢查已通過。 此外，如果完全信任的程式碼會公開，採用屬性名稱和呼叫其**取得**存取子使用反映呼叫**取得**存取子會成功，即使使用者程式碼的功能沒有存取這個屬性的權限。 這是因為**LinkDemand**檢查只有立即呼叫者，也就是完全受信任的程式碼。 本質上，完全受信任的程式碼是代表使用者程式碼進行有權限的呼叫，因此不需要確定使用者程式碼是否具有進行該呼叫的權限。  
  
 若要避免這類的安全性漏洞，通用語言執行平台擴充方法、 建構函式、 屬性或事件所保護的任何間接呼叫的完整堆疊查核檢查**LinkDemand**。 這項保護措施會稍微降低效能，也會變更安全性檢查的語意；較快的單一層級檢查可以通過的地方，完整堆疊查核行程要求卻有可能會失敗。  
...
## <a name="assembly-loading-wrappers"></a>組件載入包裝函式  
 ...
 擁有完全信任、或是比潛在呼叫端 (包括網際網路權限層級呼叫端) 更受信任的任何程式碼，都有可能因而降低安全性。 如果您的程式碼具有公用的方法採用位元組陣列，並將其傳遞給**Assembly.Load**，藉此代表呼叫端建立組件，它可能會破壞安全性。  
...
-   <xref:System.Security.Permissions.SecurityAction.Demand> 指定程式碼存取安全性堆疊查核行程。 堆疊上的所有呼叫端都必須具有指定的權限或識別，才能通過。 **視需要**就會發生在每次呼叫堆疊可能包含不同的呼叫端。 如果您重複呼叫一個方法，則每呼叫一次就會執行這個安全性檢查一次。 **視需要**是很好防止引誘攻擊; 系統偵測到嘗試通過它未經授權的程式碼。  
  
-   [LinkDemand](../../../docs/framework/misc/link-demands.md)會在 just-in-time (JIT) 編譯時期發生，並檢查立即呼叫者。 這項安全性檢查不會檢查呼叫端的呼叫端。 一旦通過這項檢查，不論呼叫端可能呼叫的次數為何，都不會再增加額外的安全性負荷。 不過，這也無法防止引誘攻擊。 與**LinkDemand**，通過測試並會參考您的程式碼的任何程式碼可以藉由允許使用授權的程式碼呼叫惡意程式碼可能會破壞安全性。 因此，請勿使用**LinkDemand**除非可以徹底避免所有可能的弱點。  
  
    > [!NOTE]
    >  在[!INCLUDE[net_v40_long](../../../includes/net-v40-long-md.md)]，連結要求已由取代<xref:System.Security.SecurityCriticalAttribute>屬性<xref:System.Security.SecurityRuleSet.Level2>組件。 <xref:System.Security.SecurityCriticalAttribute>相當於連結要求完全信任。 不過，它也會影響繼承規則。 如需有關這項變更的詳細資訊，請參閱[安全性透明程式碼，層級 2](../../../docs/framework/misc/security-transparent-code-level-2.md)。  
  
 使用時所需的額外預防措施**LinkDemand**必須個別進行程式編寫; 安全性系統可協助增強。 任何錯誤都可能會導致出現安全性弱點。 使用您程式碼的所有授權程式碼都必須負責執行下列作業，以實作額外的安全性：  
  
-   限制呼叫的程式碼對類別或組件的存取。  
  
-   對呼叫的程式碼，套用出現在所呼叫之程式碼上的相同安全性檢查，並強制其呼叫端執行這項作業。 例如，如果您撰寫程式碼呼叫的方法受到**LinkDemand**如<xref:System.Security.Permissions.SecurityPermission>與<xref:System.Security.Permissions.SecurityPermissionFlag.UnmanagedCode>指定旗標，您的方法也應該要**LinkDemand** （或**需求**，取較強者)，此權限。 例外狀況是如果您的程式碼會使用**LinkDemand**-受保護的方法，您決定的限制方式安全無虞，您的程式碼中指定 （例如 demand) 的其他安全性保護機制。 在這種例外狀況下，呼叫端會導致基礎程式碼的安全性保護變弱。  
  
-   確保您程式碼的呼叫端無法引誘您的程式碼代表呼叫端呼叫受保護的程式碼。 換句話說，呼叫端無法強制授權程式碼將特定參數傳遞給受保護的程式碼，或從中傳回結果。  
  
### <a name="interfaces-and-link-demands"></a>介面和連結要求  
 如果虛擬方法、 屬性或事件**LinkDemand**基底類別方法，會覆寫基底類別方法也必須具有相同**LinkDemand**覆寫方法才能生效。 惡意程式碼可能會轉換回基底類型，並呼叫基底類別方法。 另請注意，連結要求可以隱含加入沒有 <xref:System.Security.AllowPartiallyTrustedCallersAttribute> 組件層級屬性的組件。  
  
 最佳做法是在介面方法也有連結要求時，使用連結要求來保護方法實作。 請注意下列有關搭配介面使用連結要求的資訊：  
  
-   如果您將放入**LinkDemand**公用方法的實作介面方法，在類別上**LinkDemand**則不會強制執行然後轉換成介面並呼叫方法。 在此情況下，由於您連結的介面，僅**LinkDemand**會接受介面上。  
