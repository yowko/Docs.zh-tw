...
---
# <a name="how-to-create-windows-services"></a>HOW TO：建立 Windows 服務
...
3.  設定下列任何屬性，以決定服務的運作方式。  
    ...
    |<xref:System.ServiceProcess.ServiceBase.AutoLog%2A>|如果為 `True`，則會在服務執行動作時，將資訊項目寫入應用程式事件記錄檔；如果為 `false`，則會停用這項功能。 如需詳細資訊，請參閱[＜How to：記錄關於服務的資訊](../../../docs/framework/windows-services/how-to-log-information-about-services.md)。 **注意：** 依預設，<xref:System.ServiceProcess.ServiceBase.AutoLog%2A> 會設為 `true`。|  
...
6.  為服務應用程式加入必要的安裝程式。 如需詳細資訊，請參閱[＜How to：將安裝程式新增至服務應用程式](../../../docs/framework/windows-services/how-to-add-installers-to-your-service-application.md)。  
...
8.  安裝服務。 如需詳細資訊，請參閱[＜How to：安裝和解除安裝服務](../../../docs/framework/windows-services/how-to-install-and-uninstall-services.md)。  
