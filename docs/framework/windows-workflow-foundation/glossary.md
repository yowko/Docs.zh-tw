...
|Type Converter - 型別轉換子|CLR 型別可以與一個或多個 System.ComponentModel.TypeConverter 衍生型別相關聯，這些型別可將 CLR 型別的執行個體和其他型別的執行個體來回轉換。 型別轉換子透過 System.ComponentModel.TypeConverterAttribute 屬性的 CLR 型別是相關聯。  TypeConverterAttribute 可直接在 CLR 型別或在屬性上指定。 屬性上指定之型別轉換子的優先順序永遠高於屬性之 CLR 型別上指定之型別轉換子。|
