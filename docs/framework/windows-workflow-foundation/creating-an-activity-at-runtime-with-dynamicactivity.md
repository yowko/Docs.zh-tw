...
2. 選取 [檔案]、[**新增** **]、[****專案**]。 在 [**專案類型**] 視窗中，選取 [ **Visual c #** ] 底下的**Workflow 4.0** ，然後選取 [ **v2010]** ] 節點。 在 [**範本**] 視窗中選取 [**連續工作流程主控台應用程式**]。 將新專案命名為 DynamicActivitySample。  
...
2. 選取 [檔案]、[**新增** **]、[****專案**]。 在 [**專案類型**] 視窗中，選取 [ **Visual c #** ] 底下的**Workflow 4.0** ，然後選取 [ **v2010]** ] 節點。 在 [**範本**] 視窗中選取 [**工作流程主控台應用程式**]。 將新專案命名為 DynamicActivitySample。  
  
3. 開啟 HelloActivity 專案中的 Workflow1.xaml。 按一下設計工具底部的 [**引數**] 選項。 建立新的 `In` 引數，其名稱為 `TextToWrite`、型別為 `String`。  
  
4. 將 [ **WriteLine** ] 活動從 [工具箱] 的 [**基本**] 區段拖曳到設計工具介面上。 將值指派 `TextToWrite` 給活動的**Text**屬性。  
...
7. 執行應用程式。 含有 "Hello World！" 文字的主控台視窗  隨即出現。  
