...
## <a name="creating-and-configuring-states"></a>建立和設定狀態  
 ...
 在狀態機器中代表終止狀態的狀態稱為最終狀態。 最終狀態就是其 <xref:System.Activities.Statements.State.IsFinal%2A> 屬性設為 `true` 的狀態，沒有任何 <xref:System.Activities.Statements.State.Exit%2A> 活動，而且沒有源自於此的轉換。 若要新增至工作流程的最終狀態，請拖曳**FinalState**活動設計工具，從**狀態機器**一節**工具箱**拖曳至<xref:System.Activities.Statements.StateMachine>上的活動[!INCLUDE[wfd1](../../../includes/wfd1-md.md)]介面。 狀態機器工作流程至少要有一個最終狀態。  
...
## <a name="creating-and-configuring-transitions"></a>建立和設定轉換  
 ...
 若要新增<xref:System.Activities.Statements.State>並建立轉換，在一個步驟中，拖曳**狀態**活動從**狀態機器**區段**工具箱**並停留在另一個狀態工作流程設計工具中。 當被拖曳的 <xref:System.Activities.Statements.State> 位在另一個 <xref:System.Activities.Statements.State> 上方時，另一個 <xref:System.Activities.Statements.State> 周圍會出現四個三角形。 如果將 <xref:System.Activities.Statements.State> 拖放到四個三角形的其中一個，就會將它加入到該狀態機器，並建立從來源 <xref:System.Activities.Statements.State> 到所放置之目的地 <xref:System.Activities.Statements.State> 的轉換。 如需詳細資訊，請參閱 < [Transition 活動設計工具](/visualstudio/workflow-designer/transition-activity-designer)。  
