...
工作流程可以從內建活動建構，也可以從自訂活動建構。 本主題將逐步說明如何建立使用內建活動（例如 <xref:System.Activities.Statements.Sequence> 活動）的工作流程，以及上一個 [的自訂活動如何：建立活動](how-to-create-an-activity.md) 主題。 此工作流程會以數字猜測遊戲為模型。
...
3. 在 [**名稱**] 方塊中輸入 `SequentialNumberGuessWorkflow`，然後按一下 [**新增**]。
...
4. 在 [**名稱**] 方塊中輸入 `MaxNumber`，從 [**方向**] 下拉式清單中選取 [In]，選取 [自**變數型**別] 下拉式清單**中**的 [ **Int32** ]，然後按下 enter 以儲存引數。

5. 按一下 [**建立引數**]。

6. 在新增的 `MaxNumber` 引數下方的 [**名稱**] 方塊中輸入 `Turns`，從 [**方向**] 下拉式清單中選取 [ **Out** ]，選取 [自**變數型**別] 下拉式清單中的 [ **Int32** ]，然後按 enter。
...
10. 在 [**名稱**] 方塊中輸入 `Guess`，從 [**變數類型**] 下拉式清單中選取 [ **INT32** ]，然後按 enter 儲存變數。

11. 按一下 [**建立變數**]。

12. 在 [**名稱**] 方塊中輸入 `Target`，從 [**變數類型**] 下拉式清單中選取 [ **INT32** ]，然後按 enter 儲存變數。

13. 在活動設計工具的左下方按一下 [**變數**]，以關閉 [**變數**] 窗格。

## <a name="to-add-the-workflow-activities"></a>若要加入工作流程活動

1. 將 [ **Assign** ] 活動從 [**工具箱**] 的 [**基本**] 區段拖放到 [ **Sequence** ] 活動上。 在 [**到**] 方塊中輸入 `Target`，並在 [**輸入C#運算式**] 或 [**輸入 VB 運算式**] 方塊中輸入下列運算式。
...
5. 在 [**屬性] 視窗**中，于 [**提示**] 活動的 [ **BookmarkName** ] 屬性值方塊中輸入 `"EnterGuess"` 包含引號。 在 [**結果**] 屬性值方塊中輸入 `Guess`，然後在 [**文字**] 屬性方塊中輸入下列運算式。
...
7. 在 [**到**] 方塊中輸入 `Turns`，然後 `Turns + 1` 到 [**輸入C#運算式**] 或 [**輸入 VB 運算式**] 方塊中。
