...
description: 本文說明 Windows Workflow Foundation 主要修訂版的效能特性，這是 .NET Framework 4 的一部分。
ms.date: 03/30/2017
...
ms.locfileid: "83421562"
---
# <a name="windows-workflow-foundation-4-performance"></a>Windows Workflow Foundation 4 效能

 .NET Framework 4 包括 Windows Workflow Foundation （WF）的主要修訂版，其效能有大量的投資。 這個新的修訂引進了舊版的重大設計變更 [!INCLUDE[wf1](../../../includes/wf1-md.md)] ，這些是隨附于 .NET Framework 3.0 和 .NET Framework 3.5 的一部分。 在此版本中，程式設計模型、執行階段及工具的核心已重新架構，以大幅改善效能和可用性。 本主題說明這些修訂的重要效能特性，並且與其舊版進行比較。
...
## <a name="terminology"></a>詞彙
 ...
 [!INCLUDE[wf1](../../../includes/wf1-md.md)]本主題的其餘部分將 .NET Framework 4 中引進的版本稱為「WF4」。 [!INCLUDE[wf1](../../../includes/wf1-md.md)]已在 .NET Framework 3.0 中引進，並透過 .NET Framework 3.5 SP1 進行一些次要修訂。 本主題的其餘部分將 .NET Framework 3.5 版的 Workflow Foundation 稱為 WF3。 WF3 隨附于 .NET Framework 4 與 WF4 並存。 如需將 WF3 構件遷移至 WF4 的詳細資訊，請參閱： [Windows Workflow Foundation 4 遷移指南](migration-guidance.md)。
...
### <a name="messaging"></a>Messaging (傳訊)
 WF3 一開始僅透過外部事件或叫用 Web 服務提供相當有限的傳訊支援。 在 .NET 3.5 中，工作流程可以實作為 WCF 用戶端，或透過和公開為 WCF 服務 <xref:System.Workflow.Activities.SendActivity> <xref:System.Workflow.Activities.ReceiveActivity> 。 在 WF4 中，以工作流程為基礎的訊息程式設計概念，透過將 WCF 訊息邏輯緊密整合到 WF 中，進一步加強。
...
## <a name="service-composition-scenario"></a>服務撰寫案例
 如上一節「元件層級的效能比較」中所示，WF3 和 WF4 之間已大幅減少額外負荷。  WCF 工作流程服務現在幾乎可以符合手動編碼 WCF 服務的效能，但仍然具有執行時間的所有優點 [!INCLUDE[wf1](../../../includes/wf1-md.md)] 。  此測試案例會比較 WCF 服務與 WF4 中的 WCF 工作流程服務。
...
##### <a name="test-results"></a>測試結果
 ...
 在上圖中，「冷」指 <xref:System.ServiceModel.WorkflowServiceHost> 的是指定的工作流程沒有現有的案例。  換句話說，冷延遲意謂第一次使用此工作流程，XOML 或 XAML 需要進行編譯。  暖延遲則意謂所建立的新工作流程執行個體，其工作流程類型已完成編譯。  工作流程的複雜度在 WF4 案例中只有相當些微的差異，但是在 WF3 案例中則呈現直線進展。
...
![相互關聯輸送量工作流程測試](./media/performance/correlation-throughput-workflow.gif "相互關聯輸送量工作流程測試設定")
 ...
 先前的工作流程與 [[持續](#persistence)性] 區段中使用的相同。 針對沒有持續性的相互關聯測試，執行時間中不會安裝持續性提供者。 相互關聯會在兩處發生：CreateOrder 和 CompleteOrder。

#### <a name="test-results"></a>測試結果
![相互關聯輸送量](./media/performance/correlation-throughput-graph.gif "相互關聯輸送量圖表")
