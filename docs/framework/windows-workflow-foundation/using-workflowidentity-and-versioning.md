...
## <a name="UsingWorkflowIdentity"></a> 使用 WorkflowIdentity  
 若要使用 <xref:System.Activities.WorkflowIdentity>，請建立執行個體、加以設定，並將其與 <xref:System.Activities.WorkflowApplication> 執行個體建立關聯。 <xref:System.Activities.WorkflowIdentity> 執行個體包含三項識別資訊。 <xref:System.Activities.WorkflowIdentity.Name%2A> 並<xref:System.Activities.WorkflowIdentity.Version%2A>包含名稱和<xref:System.Version>是必要的並和<xref:System.Activities.WorkflowIdentity.Package%2A>是選擇性的而且可用來指定其他的字串，包含資訊，例如組件名稱或其他所需的資訊。 如果 <xref:System.Activities.WorkflowIdentity> 的三個屬性中有任何屬性與另一個 <xref:System.Activities.WorkflowIdentity> 的不同，則其為唯一識別。  
...
### <a name="SxS"></a> 使用 WorkflowIdentity 的並排顯示執行  
 <xref:System.Activities.WorkflowIdentity> 可用來加速工作流程-並存的多個版本的執行。 常見的案例之一是變更長時間執行工作流程的業務需求。 部署更新版本時，可以執行同一個工作流程的多個執行個體。 主應用程式可以設定為在啟動新執行個體時使用更新的工作流程定義，而且主應用程式必須在繼續執行個體時，負責提供正確的工作流程定義。 <xref:System.Activities.WorkflowIdentity> 可用來識別，並繼續工作流程執行個體時，提供相符的工作流程定義。  
