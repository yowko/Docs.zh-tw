---
title: "建立自訂流程控制活動 | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
...
---
# 建立自訂流程控制活動
.Net Framework 包含各種流程控制活動，這些活動的運作方式與抽象程式設計結構 \(例如 <xref:System.Activities.Statements.Flowchart>\) 或標準程式設計陳述式 \(例如 <xref:System.Activities.Statements.If>\) 很相似。本主題將討論其中一個範例專案的架構：[非泛型 ForEach](../../../docs/framework/windows-workflow-foundation/samples/non-generic-foreach.md)。  
  
## 建立自訂類別  
 因為非泛型 ForEach 類別必須排程子活動，而且衍生自 <xref:System.Workflow.Activities.CodeActivity> 的活動沒有這項功能，所以它必須衍生自 <xref:System.Activities.NativeActivity>。  
...
-   `valueEnumerator`：屬於用來反覆查看項目集合之 <xref:System.Collections.IEnumerator> 型別的非公用 <xref:System.Activities.Variable%601>。這個成員屬於 <xref:System.Activities.Variable%601> 型別，因為它是在活動內部使用，而非引數或公用屬性。如果此物件的來源位於活動外部，就會使用引數或公用屬性。  
