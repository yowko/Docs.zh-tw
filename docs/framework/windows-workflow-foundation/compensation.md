...
Windows 工作流基礎 （WF） 中的補償是發生後續故障時可以撤銷或補償以前完成的工作的機制（遵循應用程式定義的邏輯）。 本節描述如何在工作流程中使用補償。  
...
> [!NOTE]
> 本主題介紹工作流程中的補償。 有關工作流中的事務的詳細資訊，請參閱[事務](workflow-transactions.md)和<xref:System.Activities.Statements.TransactionScope>。 有關交易記錄的詳細資訊，請參閱<xref:System.Transactions?displayProperty=nameWithType>和<xref:System.Transactions.Transaction?displayProperty=nameWithType>。  
...
> [!NOTE]
> 確認<xref:System.Activities.Statements.CompensableActivity>後 *，無法*再調用活動的補償。 本節稍後會描述補償程序。  
...
```  
 ...
 當叫用工作流程時，<xref:System.Activities.WorkflowApplication.OnUnhandledException%2A> 中的主應用程式會處理模擬的錯誤條件例外狀況、取消工作流程，然後叫用 <xref:System.Activities.Statements.CompensableActivity> 的取消邏輯。 在此範例中，補償邏輯和取消邏輯有不同的目標。 如果 <xref:System.Activities.Statements.CompensableActivity.Body%2A> 成功完成，表示會透過信用卡扣款並預訂航班，因此補償應該會復原這兩個步驟。 （在此示例中，取消航班會自動取消信用卡費用。但是，如果取消<xref:System.Activities.Statements.CompensableActivity>， 這意味著<xref:System.Activities.Statements.CompensableActivity.Body%2A>未完成， 所以邏輯<xref:System.Activities.Statements.CompensableActivity.CancellationHandler%2A>需要能夠確定如何最好地處理取消. 在此範例中，<xref:System.Activities.Statements.CompensableActivity.CancellationHandler%2A> 會取消信用卡收費，但由於 `ReserveFlight` 是 <xref:System.Activities.Statements.CompensableActivity.Body%2A> 中的最後一個活動，因此不會嘗試取消航班。 因為 `ReserveFlight` 是 <xref:System.Activities.Statements.CompensableActivity.Body%2A> 中的最後一個活動，所以如果其順利完成，則 <xref:System.Activities.Statements.CompensableActivity.Body%2A> 已完成，而且無法取消。  
...
**系統.應用程式異常：工作流中的類比錯誤條件。**
**取消信用卡：取消信用卡費用。**
**已成功完成工作流，狀態為：已取消。**  有關取消的詳細資訊，請參閱[取消](modeling-cancellation-behavior-in-workflows.md)。  
