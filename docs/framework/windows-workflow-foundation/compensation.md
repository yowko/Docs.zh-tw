...
Windows Workflow Foundation （WF）中的補償，這是先前完成的工作可以在後續失敗發生時復原或補償的機制（遵循應用程式所定義的邏輯）。 本節描述如何在工作流程中使用補償。  
...
> [!NOTE]
> 本主題介紹工作流程中的補償。 如需工作流程中交易的詳細資訊，請參閱[交易](workflow-transactions.md)和 <xref:System.Activities.Statements.TransactionScope>。 如需交易的詳細資訊， <xref:System.Transactions?displayProperty=nameWithType>請<xref:System.Transactions.Transaction?displayProperty=nameWithType>參閱和。  
...
> [!NOTE]
> 確認之後<xref:System.Activities.Statements.CompensableActivity> ，就無法再叫用活動的補償。 本節稍後會描述補償程序。  
...
**工作流程未處理的例外狀況：**    
**ApplicationException：工作流程中的模擬錯誤狀況。**    
**CancelFlight:票證已取消。**    
**工作流程已順利完成，狀態為：#A1.**    
 ...
 **ChargeCreditCard:向信用卡收取費用。**  
**SimulatedErrorCondition:擲回 ApplicationException。**    
**工作流程未處理的例外狀況：**    
**ApplicationException：工作流程中的模擬錯誤狀況。**    
**CancelCreditCard:取消信用卡費用。**    
**工作流程已順利完成，狀態為：#A1.**  如需取消的詳細資訊，請參閱[取消](modeling-cancellation-behavior-in-workflows.md)。  
