...
> [!NOTE]
> 「快速入門」教學課程中的每個主題都與之前的主題息息相關。 若要完成本主題, 您必須[先完成如何:建立活動](how-to-create-an-activity.md)和[如何:建立工作流程](how-to-create-a-workflow.md)。
...
1. 開啟先前[的 how to:使用 Visual Studio 2012](how-to-create-an-activity.md)建立活動主題。  
...
1. 按兩下 [ **方案總管** ] 中的 [ **Program.cs** ] 或 [ **Module1.vb** ]，顯示其程式碼。
    ...
    > [!NOTE]
    > 請將`Workflow1`這些範例中的`FlowchartNumberGuessWorkflow`取代`SequentialNumberGuessWorkflow`為、 `StateMachineNumberGuessWorkflow`或, 視您在先前[的 how to:建立工作流程](how-to-create-a-workflow.md)步驟。 如果您不替換 `Workflow1` ，那麼在您嘗試建置或執行工作流程時，會發生建置錯誤。
...
2. 按 CTRL+F5 建置並執行應用程式。 盡量以最少的次數嘗試猜測數字。
     ...
     如需如何將持續性加入工作流應用程式的相關指示, 請參閱[下一個主題: 如何:建立並執行長時間執行的](how-to-create-and-run-a-long-running-workflow.md)工作流程。
...
- [如何：建立並執行長時間執行的工作流程](how-to-create-and-run-a-long-running-workflow.md)
