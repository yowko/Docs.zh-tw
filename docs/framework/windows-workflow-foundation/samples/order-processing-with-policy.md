...
> [!NOTE]
>  此範例需安裝 [!INCLUDE[netfx35_long](../../../../includes/netfx35-long-md.md)] 才能建置及執行。 需要 [!INCLUDE[vs_current_long](../../../../includes/vs-current-long-md.md)] 才能開啟專案和方案檔。  
...
-   `OrderErrorLibrary`  
     ...
     `OrderErrorLibrary` 是定義 `OrderError` 和 `OrderErrorCollection` 類別的類別程式庫。 `OrderError` 執行個體是在輸入無效輸入時所建立。 此類別程式庫也會提供 `OrderErrorCollection` 類別上的擴充方法，此方法會輸出 `ErrorText` 中所有 `OrderError` 物件上的 `OrderErrorCollection` 屬性。  
...
-   `OrderProcessingPolicy`  
    ...
    -   `invalidZip`  
         ...
         這個規則會驗證郵遞區號有 5 個數字，而且範圍在 600 到 99998。 如果郵遞區號在有效範圍之內，此規則就不會執行任何動作 (除了列印至主控台)。 如果郵遞區號少於 5 碼，或者不是在 00600 到 99998 之間，`invalidZip` 規則便會執行下列動作：  
 ...
 在 `+` 專案中，`OrderErrorCollection` 上的多載運算子 `OrderErrorCollection` 是定義在 `OrderErrorLibrary` 類別中。 它會接受兩個 `OrderErrorCollection` 物件，並將它們結合成為一個 `OrderErrorCollection` 物件。  
