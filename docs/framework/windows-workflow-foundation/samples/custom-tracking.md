...
## 範例詳細資料  
 [!INCLUDE[wf](../../../../includes/wf-md.md)] 提供追蹤基礎結構，可追蹤工作流程執行個體的執行。追蹤執行階段會實作工作流程執行個體，以發出與工作流程生命週期相關的事件、工作流程活動的事件，以及自訂追蹤事件。下表詳細說明追蹤基礎結構的主要元件。  
...
|追蹤記錄|描述|  
|----------|--------|  
|工作流程執行個體追蹤記錄。|描述在工作流程執行個體的生命週期。例如，當工作流程啟動或完成時，就會發出執行個體記錄。|  
...
-   藉由呼叫 <xref:System.Activities.ActivityContext> 的追蹤方法，即可發出 <xref:System.Activities.Tracking.CustomTrackingRecord>。  
