...
|自訂活動 (宣告式和以程式碼為主)|此範例中有幾個自訂活動：<br /><br /> -   `SaveActionTracking`：此活動會發出自<xref:System.Activities.Tracking.TrackingRecord>定義 ( <xref:System.Activities.NativeActivityContext.Track%2A>使用)。 這個活動已經使用命令式程式碼擴充 <xref:System.Activities.NativeActivity> 來撰寫。<br />-   `GetEmployeesByPositionTypes`：此活動會接收位置類型識別碼的清單, 並傳回 Contoso 中具有該位置的人員清單。 此活動已經以宣告方式撰寫 (使用活動設計工具)。<br />-   `SaveHiringRequestInfo`：這個活動會儲存的資訊`HiringRequest` (使用`HiringRequestRepository.Save`)。 這個活動已經使用命令式程式碼擴充 <xref:System.Activities.CodeActivity> 來撰寫。|HiringRequestService|  
...
|平行活動|-   <xref:System.Activities.Statements.ParallelForEach%601>用來以平行方式在 CEO 和 HR Manager 的收件匣中註冊 (等候兩個 HR Manager 的核准步驟)。<br />-   <xref:System.Activities.Statements.Parallel>是用來在已完成和已拒絕的步驟中執行一些清除工作|HiringRequestService|  
...
|在[!INCLUDE[wf1](../../../../includes/wf1-md.md)] ASP.NET 應用程式中使用。|工作流程是從兩個 ASP.NET 應用程式進行存取。|InternalClient / CareersWebSite|  
...
7. 以滑鼠右鍵按一下`InternalClient`方案中的  **internalclient** , 然後**在瀏覽器中**選取 View, 以流覽回到。  
...
2. 建立之後, 要求會出現在 Michael 的收件匣中 (如果您沒有看到要求, 請按一下 [重新整理]), 等待 Peter Brehm 的核准, 他是 Michael 的經理。  
...
2. 如果此方案建置失敗，請確認以下事項：  
    ...
    - 或`InternalClient` `ContosoHR` 專案中的`CareersWebSite`參考不存在。  
