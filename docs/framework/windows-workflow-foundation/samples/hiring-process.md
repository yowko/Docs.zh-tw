...
此範例示範如何使用傳訊活動和裝載為工作流程服務的兩個工作流程來實作商務程序。 這些工作流程是虛擬公司 Contoso, Inc. 的 IT 基礎結構的一部分。  
 ...
 `HiringRequest` 工作流程 (實作為 <xref:System.Activities.Statements.Flowchart>) 會要求組織中的幾位經理提供授權。 為了達到此目標，工作流程會使用組織中的其他現有服務（在我們的案例中，收件匣服務和實作為一般 Windows Communication Foundation （WCF）服務的組織資料服務）。  
...
|OrgService|這是公開組織資訊 (員工、職位、職位類型和部門) 的服務。 您可以將這項服務視為企業資源規劃 (ERP) 的公司組織模組。<br /><br /> 這個專案會實作為可公開 Windows Communication Foundation （WCF）服務的主控台應用程式。|  
...
|訊息活動|此流程圖會以兩種方式使用訊息活動：<br /><br /> -取得使用者的資訊（以在每個核准步驟中接收決策和相關資訊）。<br />-若要與其他現有的服務（InboxService 和 OrgDataService）互動，請透過服務參考來使用。|HiringRequestService|  
|以內容為主的相互關聯|核准訊息會依招聘申請的 ID 屬性建立相互關聯性：<br /><br /> -啟動進程時，會使用要求的識別碼來初始化相互關聯控制碼。<br />-內送核准訊息的識別碼相互關聯（每個核准訊息的第一個參數是要求的識別碼）。|HiringRequestService / ResumeRequestService|  
|自訂活動 (宣告式和以程式碼為主)|此範例中有幾個自訂活動：<br /><br /> -   `SaveActionTracking`：此活動會發出自訂 <xref:System.Activities.Tracking.TrackingRecord> （使用 <xref:System.Activities.NativeActivityContext.Track%2A>）。 這個活動已經使用命令式程式碼擴充 <xref:System.Activities.NativeActivity> 來撰寫。<br />-   `GetEmployeesByPositionTypes`：此活動會接收位置類型識別碼的清單，並傳回 Contoso 中具有該位置的人員清單。 此活動已經以宣告方式撰寫 (使用活動設計工具)。<br />-   `SaveHiringRequestInfo`：此活動會儲存 `HiringRequest` 的資訊（使用 `HiringRequestRepository.Save`）。 這個活動已經使用命令式程式碼擴充 <xref:System.Activities.CodeActivity> 來撰寫。|HiringRequestService|  
...
|並行活動|-   <xref:System.Activities.Statements.ParallelForEach%601> 用來以平行方式在 CEO 和 HR Manager 的收件匣中註冊（等候兩個 HR Manager 的核准步驟）。<br />-   <xref:System.Activities.Statements.Parallel> 是用來在已完成和已拒絕的步驟中執行一些清除工作|HiringRequestService|  
...
|長期計時器|`ResumeRequestService` 會使用永久性計時器來定義刊登工作的期間 (一旦過了期限之後，刊登就會結束)。|ResumeRequestService|  
...
|在 ASP.NET 應用程式中使用 [!INCLUDE[wf1](../../../../includes/wf1-md.md)]。|工作流程是從兩個 ASP.NET 應用程式進行存取。|InternalClient / CareersWebSite|  
...
## <a name="data-storage"></a>資料儲存  
 ...
 這兩個資料庫都是藉由從 Visual Studio 的開發人員命令提示字元執行 cmd.exe 腳本所建立。  
...
3. 選取 **多個啟始專案** 選項，並將**careerswebsite**、 **internalclient**、 **HiringRequestService**和**ResumeRequestService**設定為 **啟動**。 將**ContosoHR**、 **InboxService**和**OrgService**保留為 [無]。  
...
2. 以滑鼠右鍵按一下方案中的  **internalclient** ，然後選取 **在瀏覽器中查看**。 隨即顯示 `InternalClient` 的預設頁面。 確定服務正在執行，然後按一下連結。  
  
3. 隨即顯示**HiringRequest**模組。 您可以遵循這裡所詳述的案例。  
...
6. 以滑鼠右鍵按一下方案中的  **careerswebsite** ，然後**在瀏覽器中**選取 View。  
  
7. 以滑鼠右鍵按一下方案中的  **internalclient** ，然後**在瀏覽器中**選取 View，以流覽回 `InternalClient`。  
  
8. 在 [收件匣] 上方功能表中，按一下 [**工作張貼**] 連結，以移至 [ **JobPostings** ] 區段。 您可以遵循這裡所詳述的案例。  
...
2. 建立之後，要求會出現在 Michael 的收件匣中 **（如果您**沒有看到要求，請按一下 [重新整理]），等待 Peter Brehm 的核准，他是 Michael 的經理。  
  
3. Peter 想要針對 Michael 的申請採取行動。 他認為這個職位需要五年的 C# 經驗而不是三年，所以傳回他的意見以供審查。  
  
4. Michael 在他的經理的收件匣中看到一則訊息，想要採取行動。Michael 會看到位置要求的歷程記錄，並同意 Peter。 Michael 於是將職位描述修改為需要五年的 C# 經驗，並接受修改。  
...
1. 現在，工作位置正在等候張貼到外部網站，讓使用者可以套用（您可以看到它按一下 [**工作張貼**] 連結）。 目前這個職位的內容在人力資源部門代表手中，該名代表負責完成此職位的內容，並將它刊登在網站上。  
  
2. HR 想要藉由設定60分鐘的時間（在現實生活中，這可能是數天或數周）來編輯此作業位置（藉由按一下 [**編輯**] 連結）。 此逾時期間可允許根據指定的時間從外部網站取下該職位。  
  
3. 儲存已編輯的工作位置之後，它會出現在 [**接收繼續**] 索引標籤（重新整理網頁以查看新的作業位置）。  
  
### <a name="collecting-resumes"></a>收集履歷表  
  
1. 此職位應該會出現在外部網站上。 當某個人對這個工作有興趣時，他可以申請這個職位並提交履歷表。  
  
2. 如果您回到「工作張貼清單」服務，就可以「view 簡歷」到目前為止已收集。  
  
3. 人力資源部門可以停止收集履歷表 (例如，在找到適當的人選之後)。  
  
## <a name="troubleshooting"></a>疑難排解  
  
1. 請確定您是以系統管理員許可權執行 Visual Studio。  
  
2. 如果此方案建置失敗，請確認以下事項：  
  
    - `InternalClient` 或 `CareersWebSite` 專案中不缺少 `ContosoHR` 的參考。  
...
3. 如果此方案執行失敗，請確認以下事項：  
    ...
    2. 服務參考已更新。  
        ...
        3. 在 Visual Studio 中按 CTRL + SHIFT + B 以重建方案。  
