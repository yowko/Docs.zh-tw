...
|RfpRepository|提案徵求書的儲存機制。  這個實作包含使用 Linq to XML 查詢結構描述化的持續性所產生的提案徵求書 XML 檔案。  這個類別實作 <xref:System.Runtime.Persistence.IDataViewMapper>。|  
...
|表單|描述|  
|NewRfp|建立及提交新的提案徵求書。|  
|ShowProposals|顯示所有作用中和已完成的提案徵求書。 **Note:**  在建立或修改提案徵求書之後，可能需要在 UI 按一下 \[**Refresh**\] 按鈕，才能看到該畫面的變更。|  
...
2.  若要執行 Web 用戶端專案，請開啟 \[**方案總管**\]，以滑鼠右鍵按一下 \[**網頁用戶端**\] 專案。  選取 \[**設定為啟始專案**\]。  
  
3.  若要執行 WinForms 用戶端專案，請開啟 \[**方案總管**\]，以滑鼠右鍵按一下 \[**WinForms 用戶端**\] 專案。  選取 \[**設定為啟始專案**\]。  
...
-   **Create a new RFP**：建立新的提案徵求書 \(RFP\) 並且啟動採購程序工作流程。  
  
-   **Refresh**：在主視窗中重新整理作用中和已完成的 RFP 清單。  
  
-   **View**：顯示現有 RFP 的內容。  供應商可以提交提案 \(如果受邀或 RFP 未完成\)。  
  
-   View As：使用者可透過不同的識別來存取 RFP，方法是在作用中 RFP 方格中選取 \[**View as**\] 下拉式方塊中的所需參與者。  
  
### WinForms 用戶端選項  
  
-   **Create RFP**：建立新的提案徵求書 \(RFP\) 並且啟動採購程序工作流程。  
  
-   **Refresh**：在主視窗中重新整理作用中和已完成的 RFP 清單。  
  
-   **View RFP**：顯示現有 RFP 的內容。  供應商可以提交提案 \(如果受邀或 RFP 未完成\)。  
  
-   **Connect As**：使用者可透過不同的識別來存取 RFP，方法是在作用中 RFP 方格中選取 \[**View as**\] 下拉式方塊中的所需參與者。  
