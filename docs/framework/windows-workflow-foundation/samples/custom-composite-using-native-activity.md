...
## <a name="sample-details"></a>範例詳細資料
 從 `MySequence` 開始，要注意的第一件事就是它會衍生自 <xref:System.Activities.NativeActivity>。 <xref:System.Activities.NativeActivity> 已<xref:System.Activities.Activity>物件所公開的工作流程執行階段，透過全面性<xref:System.Activities.NativeActivityContext>傳遞至`Execute`方法。

 `MySequence` 公開 （expose) 的公用集合<xref:System.Activities.Activity>取得工作流程作者所填入的物件。 在執行工作流程之前，工作流程執行階段會在工作流程中的每一個活動上呼叫 <xref:System.Activities.Activity.CacheMetadata%2A> 方法。 在這個程序期間，執行階段會建立父-子關係來設定資料範圍及進行存留期管理。 預設實作<xref:System.Activities.Activity.CacheMetadata%2A>方法會使用<xref:System.ComponentModel.TypeDescriptor>執行個體類別用於`MySequence`活動，將任何公用屬性的型別<xref:System.Activities.Activity>或是<xref:System.Collections.IEnumerable> \< <xref:System.Activities.Activity>> 為的子系`MySequence`活動。
