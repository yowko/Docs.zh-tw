---
title: "移除檢視狀態，設計工具會新增至 XAML 檔案 | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
...
## 討論  
 ...
 若要建置自訂的 XAML 寫入器，請建立繼承自 <xref:System.Windows.Markup.XamlWriter> 的類別。因為 XAML 寫入器通常為巢狀，一般會追蹤「內部」XAML 寫入器。您可以將這些「內部」寫入器想像成其餘 XAML 寫入器堆疊的參考，讓多個進入點執行工作，然後將處理委派至堆疊的其餘部分。  
...
```  
 ...
 這也會建立周遊節點資料流時所用的 XAML 成員堆疊。這個範例的其餘部分大多是包含在 <xref:System.Windows.Markup.XamlWriter.WriteStartMember%2A> 方法中。  
...
1.  使用 [!INCLUDE[vs2010](../../../../includes/vs2010-md.md)] 開啟 ViewStateCleaningWriter.sln 方案檔案。  
 ...
 **ViewStateCleaningWriter.exe \[input file\] \[output file\]**     這會將 XAML 檔案輸出至 \[outfile\]，其所有檢視狀態資訊已移除。  
