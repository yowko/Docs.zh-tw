...
## 範例詳細資料  
 在這個範例中，設定兩個透過 [!INCLUDE[wf2](../../../../includes/wf2-md.md)] 實作的 [!INCLUDE[indigo1](../../../../includes/indigo1-md.md)] 服務，來進行永久性的雙工訊息交換。此永久性的雙工訊息交換是由兩個透過 MSMQ 傳送並使用 [.NET Context Exchange](http://go.microsoft.com/fwlink/?LinkID=166059) 相互關聯的單向訊息所組成。訊息是使用 <xref:System.ServiceModel.Activities.Send> 和 <xref:System.ServiceModel.Activities.Receive> 訊息活動來傳送。.NET Context Exchange 用來在傳送的訊息上指定回呼位址。兩個服務是透過使用 Windows Process Activation Services \(WAS\) 所主控，並設定為啟用服務執行個體的持續性。  
...
> [!NOTE]
>  這個範例所使用的佇列不是交易式。如需示範如何設定使用交易式佇列的 [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] 訊息交換範例，請參閱 [MSMQ 啟用](../../../../docs/framework/wcf/samples/msmq-activation.md)範例。  
...
2.  設定虛擬應用程式。  
    ...
    2.  以滑鼠右鍵按一下 \[[!INCLUDE[vs2010](../../../../includes/vs2010-md.md)]\]，並選取 \[**以系統管理員身分執行**\]，以系統管理員的權限執行 [!INCLUDE[vs2010](../../../../includes/vs2010-md.md)]。  
...
3.  設定服務佇列。  
    ...
    2.  從命令提示字元執行 `Compmgmt.msc`，開啟 \[**電腦管理**\] 主控台。  
  
    3.  依序展開 \[**服務及應用程式**\]、\[**訊息佇列**\]、\[**私用佇列**\]。  
  
    4.  以滑鼠右鍵按一下 durableduplex\/service1.xamlx 和 durableduplex\/service2.xamlx 佇列，然後選取 \[**屬性**\]。  
  
    5.  選取 \[**安全性**\] 索引標籤，並允許兩個佇列有下列權限：\[**Everyone 接收訊息**\]、\[**查看訊息**\] 和 \[**傳送訊息**\]。  
  
    6.  開啟 Internet Information Services \(IIS\) 管理員。  
  
    7.  瀏覽至 \[**伺服器**\]、\[**站台**\]、\[**預設的網站**\]、\[**私用**\]、\[**永久性雙工**\]，然後選取 \[**進階選項**\]。  
  
    8.  將 \[**已啟用的通訊協定**\] 變更為 **http,net.msmq**。  
...
2.  移除服務的虛擬應用程式。  
    ...
    1.  從命令提示字元執行 `Inetmgr.exe`，以開啟 Internet Information Services \(IIS\) 管理員。  
  
    2.  瀏覽至 \[預設的網站\]，然後移除 **private** 虛擬目錄。  
  
3.  移除此範例中設定的佇列。  
  
    1.  從命令提示字元執行 `Compmgmt.msc`，開啟 \[電腦管理\] 主控台。  
