...
1.  使用系統管理員權限開啟 [!INCLUDE[vs2010](../../../../includes/vs2010-md.md)] 命令提示字元視窗，然後瀏覽至此 DocumentApprovalProcess 資料夾並執行 Setup.cmd。  
...
3.  若要執行方案，請啟動核准管理員應用程式，方法是以滑鼠右鍵按一下 \[**方案總管**\] 中的 ApprovalManager 專案，然後從右鍵功能表依序按一下 \[**偵錯**\] \-\> \[**開始新執行個體**\]。  
...
4.  按一下 \[**discover**\]，等候 \[**subscribe**\] 按鈕啟用。  
  
5.  輸入任何使用者名稱，然後按一下 \[**subscribe**\]。對其中一個用戶端，使用 `UserType1`，對另一個輸入 `UserType2`。  
  
6.  在 `UserType1` 用戶端中，從下拉式功能表中選取單一核准類型，然後輸入文件名稱和內容。按一下 \[**Request Approval**\]。  
  
7.  在 `UserType2` 用戶端中，待核准的文件隨即出現。選取它，然後按 \[**approve**\] 或 \[**reject**\]。`UserType1` 用戶端中應該會顯示結果。  
...
5.  輸入任何使用者名稱，然後按一下 \[**subscribe**\]。對其中一個用戶端，使用 `UserType1`，對另外兩個輸入 `UserType2`。  
  
6.  在 `UserType1` 用戶端中，從下拉式功能表中選取仲裁核准類型，然後輸入文件名稱和內容。按一下 \[**Request Approval**\]。這會要求兩個 `UserType2` 用戶端核准或拒絕文件。雖然兩個 `UserType2` 用戶端都必須回應，但是只要一個用戶端核准文件，文件就會獲得已核准狀態。  
...
5.  輸入任何使用者名稱，然後按一下 \[**subscribe**\]。對其中一個用戶端，使用 `UserType1`，在兩個用戶端中輸入 `UserType2`，並在最後一個用戶端中使用 `UserType3`。  
  
6.  在 `UserType1` 用戶端中，從下拉式功能表中選取單一核准類型，然後輸入文件名稱和內容。按一下 \[**Request Approval**\]。  
  
7.  在 `UserType2` 用戶端中，待核准的文件隨即出現。選取它，然後按 \[**approve**\]，文件會傳遞至 `UserType3` 用戶端。  
