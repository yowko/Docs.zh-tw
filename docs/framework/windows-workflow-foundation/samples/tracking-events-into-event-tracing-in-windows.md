...
4. 使用 [檔案瀏覽器] 開啟 WCF 測試用戶端。
    ...
    WCF 測試用戶端（WcfTestClient）位於 \<Visual Studio 2010 installation folder> \Common7\IDE\ 資料夾中。
...
6. 開啟 [事件檢視器] 應用程式。
    ...
    叫用服務之前，請先從 [**開始**] 功能表啟動事件檢視器，選取 [**執行**]，然後輸入 `eventvwr.exe` 。 確認事件記錄檔正在接聽從工作流程服務發出的追蹤事件。
...
>
> \<id>找不到來源 Microsoft-Windows-應用程式伺服器-應用程式中的事件識別碼描述。 可能是引發此事件的元件未安裝在您的本機電腦上，或安裝已損毀。 您可以在本機電腦上安裝或修復該元件。
>
