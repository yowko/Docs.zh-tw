---
title: "在 Windows 事件追蹤中追蹤事件 | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
...
1.  使用 [!INCLUDE[vs2010](../../../../includes/vs2010-md.md)] 開啟 EtwTrackingParticipantSample.sln 方案檔案。  
...
5.  在 WCF 測試用戶端中，選取 \[**檔案**\] 功能表上的 \[**新增服務**\]。  
  
     在輸入方塊中加入端點位址。預設為 http:\/\/localhost:53797\/SampleWorkflowService.xamlx。  
  
6.  開啟 \[事件檢視器\] 應用程式。  
  
     叫用服務之前，從 \[**開始**\] 功能表啟動 \[事件檢視器\]，其方式是選取 \[**執行**\]，然後輸入 `eventvwr.exe`。確認事件記錄檔正在接聽從工作流程服務發出的追蹤事件。  
  
7.  在 \[事件檢視器\] 的樹狀檢視中，巡覽至 \[**事件檢視器**\]、\[**應用程式及服務記錄檔**\] 和 \[**Microsoft**\]。以滑鼠右鍵按一下 \[**Microsoft**\]，並選取 \[**檢視**\]，然後選取 \[**顯示分析與偵錯記錄檔**\] 來啟用分析和偵錯記錄檔。  
  
     確認已核取 \[**顯示分析與偵錯記錄檔**\] 選項。  
  
8.  在 \[事件檢視器\] 的樹狀檢視中，巡覽至 \[**事件檢視器**\]、\[**應用程式及服務記錄檔**\]、\[**Microsoft**\]、\[**Windows**\]、\[**應用程式伺服器\-應用程式**\]。以滑鼠右鍵按一下 \[**分析**\] 並選取 \[**啟用記錄**\]，啟用 \[**分析**\] 記錄檔。  
...
10. 請查看工作流程所發出的事件。  
     ...
     切換回 \[事件檢視器\]，並巡覽至 \[**事件檢視器**\]、\[**應用程式及服務記錄檔**\]、\[**Microsoft**\]、\[**Windows**\]、\[**應用程式伺服器\-應用程式**\]。以滑鼠右鍵按一下 \[**分析**\]，並選取 \[**重新整理**\]。  
