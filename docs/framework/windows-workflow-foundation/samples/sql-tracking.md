...
此範例示範如何撰寫自訂的 SQL 追蹤參與者，將追蹤記錄寫入至 SQL 資料庫。 Windows Workflow Foundation (WF) 提供工作流程追蹤, 以深入瞭解工作流程實例的執行。 追蹤執行階段會在工作流程執行期間發出工作流程追蹤記錄。 如需工作流程追蹤的詳細資訊, 請參閱[工作流程追蹤和追蹤](../workflow-tracking-and-tracing.md)。
...
10. 在 WCF 測試用戶端中, 按兩下 [ **GetStockPrice**]。 這會開啟`GetStockPrice`接受一個參數的作業, 在值 `Contoso`中輸入, 然後按一下 [叫用]。  
