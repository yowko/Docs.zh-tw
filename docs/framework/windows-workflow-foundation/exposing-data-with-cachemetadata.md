...
- <xref:System.Activities.InArgument%601><xref:System.Activities.OutArgument%601>，或<xref:System.Activities.InOutArgument%601>（泛型引數）：這些引數會公開至執行階段當做引數的名稱和型別等於公開的屬性名稱和類型、 適當的引數方向和一些驗證資料。

- <xref:System.Activities.Variable> 或其任何子類別：這些成員會公開至執行階段中，當做公用變數。

- <xref:System.Activities.Activity> 或其任何子類別：這些成員會公開至執行階段中，當做公用子活動。 可以明確實作的預設行為，藉由呼叫<xref:System.Activities.ActivityMetadata.AddImportedChild%2A>，並傳入子活動。

- <xref:System.Activities.ActivityDelegate> 或其任何子類別：這些成員會公開至執行階段中，當做公用委派。

- <xref:System.Collections.ICollection> 型別的<xref:System.Activities.Variable>:集合中的所有項目公開至執行階段當做公用變數。

- <xref:System.Collections.ICollection> 型別的<xref:System.Activities.Activity>:集合中的所有項目公開至執行階段當做公用子系。

- <xref:System.Collections.ICollection> 型別的<xref:System.Activities.ActivityDelegate>:集合中的所有項目公開至執行階段當做公用委派。
