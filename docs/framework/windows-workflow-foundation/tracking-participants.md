...
## 追蹤參與者  
 ...
 [!INCLUDE[wf](../../../includes/wf-md.md)] 中的 [!INCLUDE[netfx_current_short](../../../includes/netfx-current-short-md.md)] 提供將追蹤記錄寫入至 ETW 工作階段的追蹤參與者。  透過在設定檔中加入特定追蹤的行為，您可以設定工作流程服務上的參與者。  啟用 ETW 追蹤參與者可在事件檢視器中檢視追蹤記錄。  ETW 式追蹤的 SDK 範例是熟悉使用 ETW 式追蹤參與者之 WF 追蹤的理想方式。  
...
```  
 ...
 ![追蹤流程與 ETW 追蹤提供者](../../../docs/framework/windows-workflow-foundation//media/trackingdatathroughetwparticipant.gif "TrackingDatathroughETWParticipant")  
  
## 追蹤參與者事件資料  
 追蹤參與者會以每個追蹤記錄包含一個事件的形式，將已追蹤的事件資料序列化至 ETW 工作階段。  事件會使用從 100 到 199 範圍的 ID 來識別。  如需追蹤參與者所發出之追蹤事件記錄的定義，請參閱[追蹤事件參考](../../../docs/framework/windows-workflow-foundation//tracking-events-reference.md)主題。  
 ...
 變數、引數和自訂資料項目的複雜型別會使用 [NetDataContractSerializer 類別](http://go.microsoft.com/fwlink/?LinkId=177537)序列化至 ETW 事件記錄。  此類別包含序列化 XML 資料流中的 CLR 型別資訊。  
...
2.  選取 \[**事件檢視器、應用程式與服務記錄檔、Microsoft、Windows、應用程式伺服器\-應用程式**\]。  
  
3.  以滑鼠右鍵按一下，以確定選取 \[**檢視、顯示分析與偵錯記錄檔**\]。  如果未選取該選項，請加以選取，使其旁邊出現核取記號。  如此會顯示 \[**分析**\]、\[**效能**\] 和 \[**偵錯**\] 記錄檔。  
  
4.  以滑鼠右鍵按一下 \[**分析**\] 記錄檔，並選取 \[**啟用記錄檔**\]。  記錄檔將位於 %SystemRoot%\\System32\\Winevt\\Logs\\Microsoft\-Windows\-Application Server\-Applications%4Analytic.etl 檔案。  
