...
dev_langs: 
  ...
  - "jsharp"
helpviewer_keywords: 
  - ".NET Framework 應用程式部署"
  - ".NET Framework, 部署"
  - "部署應用程式 [.NET Framework]"
  - "部署應用程式 [.NET Framework], 散發"
  - "部署應用程式 [.NET Framework], 功能"
  - "部署應用程式 [.NET Framework], 封裝"
...
本文將協助您開始在應用程式上部署 .NET Framework。  大部分資訊的目標對象是開發人員、OEM 和企業系統管理員。  想要在電腦上安裝 .NET Framework 的使用者應閱讀[安裝 .NET Framework 4.5](../../../docs/framework/install/guide-for-developers.md)。  
...
-   一般安裝程式和部署資訊：  
    ...
    -   安裝程式選項：  
        ...
        -   [Web 安裝程式](../../../docs/framework/install/guide-for-developers.md#choices)  
  
        -   [離線安裝程式](../../../docs/framework/install/guide-for-developers.md#choices)  
    ...
    -   [.NET Framework 安裝遭到封鎖的疑難排解](../../../docs/framework/install/troubleshoot-blocked-installations-and-uninstallations.md)  
...
-   部署 .NET Framework \(適用於 OEM 和系統管理員\)：  
    ...
    -   [管理員手冊](../../../docs/framework/deployment/guide-for-administrators.md)  
  
 **服務**  
  
-   如需一般資訊，請參閱 [.NET Framework 部落格](http://go.microsoft.com/fwlink/p/?LinkId=254977)  
...
## 封裝和散發 .NET Framework 應用程式  
 文件的其他章節將說明 .NET Framework 的一些封裝和部署資訊。  這些章節提供自我描述單位的相關資訊，這些單位稱為[組件](../../../docs/framework/app-domains/assemblies-in-the-common-language-runtime.md) \(不需要登錄項目\)、[強式名稱的組件](../../../docs/framework/app-domains/strong-named-assemblies.md) \(確保名稱唯一性並防止名稱冒用\) 和[組件版本控制](../../../docs/framework/app-domains/assembly-versioning.md) \(處理許多與 DLL 衝突相關的問題\)。  下列章節提供封裝和散發 .NET Framework 應用程式的相關資訊。  
...
### 安裝位置  
 若要決定應用程式的組件應該部署在哪裡，執行階段才能找到它們，請參閱[執行階段如何找出組件](../../../docs/framework/deployment/how-the-runtime-locates-assemblies.md)。  
...
|[部署應用程式、服務和元件](../Topic/Deploying%20Applications,%20Services,%20and%20Components.md)|討論 Visual Studio 中的部署選項，包括使用 ClickOnce 和 Windows Installer 技術發行應用程式的指示。|  
|[發行 ClickOnce 應用程式](../Topic/Publishing%20ClickOnce%20Applications.md)|描述如何封裝 Windows Form 應用程式，並使用 ClickOnce 將它部署到網路上的用戶端電腦。|  
