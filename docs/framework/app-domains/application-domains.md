---
title: 應用程式網域
description: 閱讀關於應用程式域的資訊，以提供應用程式之間的隔離界限，以提供安全性、可靠性、版本控制、& 在 .NET 中卸載元件。
ms.date: 03/30/2017
helpviewer_keywords:
...
## <a name="application-domains-and-assemblies"></a>應用程式定義域和組件
 ...
 本節描述應用程式定義域和組件之間的關聯性。 您必須先將組件載入到應用程式定義域之後，才能執行它所包含的程式碼。 執行一個典型的應用程式會讓好幾個組件載入應用程式定義域。  
