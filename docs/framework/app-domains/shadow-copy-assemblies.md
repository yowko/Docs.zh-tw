...
-   [已淘汰的方法](#ObsoleteMethods)描述對於 [!INCLUDE[dnprdnlong](../../../includes/dnprdnlong-md.md)] 中控制陰影複製的屬性和方法之變更。  
...
## 啟動效能  
 ...
 對於組件不常變更，且變更通常發生在組件一小部分的應用程式而言，會產生最大的效能改進。  如果應用程式中大部分的組件經常變更，則新的預設行為可能會導致效能變差。  您可以藉由將 [\<shadowCopyVerifyByTimestamp\> 項目](../../../docs/framework/configure-apps/file-schema/runtime/shadowcopyverifybytimestamp-element.md) \(並設定 `enabled="false"`\) 加入至組態檔，還原舊版 .NET Framework 的啟動行為。  
