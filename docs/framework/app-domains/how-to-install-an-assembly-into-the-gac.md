...
> [!IMPORTANT]
>  只有強式名稱組件才能安裝至 GAC。  如需如何建立強式名稱組件的詳細資訊，請參閱 [如何：使用強式名稱簽署組件](../../../docs/framework/app-domains/how-to-sign-an-assembly-with-a-strong-name.md)。  
  
-   使用 [Windows Installer](http://msdn.microsoft.com/library/windows/desktop/cc185688.aspx) \(英文\)。  
...
-   [使用全域組件快取工具 \(Gacutil.exe\)](../../../docs/framework/tools/gacutil-exe-gac-tool.md)。  
...
1.  開啟方案的捷徑功能表，然後選擇 \[**加入**\]、\[**新專案**\]，將安裝和部署套件加入至您的方案。  
  
2.  在 \[**加入新的專案**\] 對話方塊中，於 \[**已安裝**\] 資料夾中選擇 \[**其他專案類型**\]、\[**安裝和部署**\]、\[**InstallShield Limited Edition**\]，然後為您的專案命名。  \(出現提示時，下載、安裝並啟動 InstallShield。\)  
  
3.  使用 \[**方案總管**\] 中的專案助理，或是選擇 \[**方案總管**\] 中編號步驟的子步驟，執行安裝和部署專案的一般組態。  設定您的安裝程式，就像沒有將組件加入 GAC 一樣。  
  
4.  若要開始將組件加入至 GAC 的程序，請選擇位於 \[**方案總管**\] 的 \[**指定應用程式資料**\] 步驟下的 \[**檔案**\]。  
  
5.  在 \[**目的電腦的資料夾**\] 窗格中，開啟 \[**目的電腦**\] 的捷徑功能表，然後選擇 \[**顯示預先定義的資料夾**\]、\[**\[GlobalAssemblyCache\]**\]。  
...
6.  對於方案中包含您要在全域組件快取中安裝之組件的每個專案：  
    ...
    3.  在 \[來源電腦的檔案\] 窗格中，選擇 \[主要輸出來自 *\<project\_name\>*\]。  
