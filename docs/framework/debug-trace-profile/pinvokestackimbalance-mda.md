...
> [!NOTE]
>  在 .NET Framework 3.5 版中，預設會停用 `pInvokeStackImbalance` MDA。  當您搭配 Visual Studio 2005 使用 .NET Framework 3.5 版時，`pInvokeStackImbalance` MDA 會出現在 \[例外狀況\] 對話方塊的 \[Managed 偵錯助理\] 清單中 \(當您按一下 \[偵錯\] 功能表上的 \[例外狀況\] 時會顯示 \[例外狀況\] 對話方塊\)。  不過，選取或清除 `pInvokeStackImbalance` 的 \[擲回\] 核取方塊並不會啟用或停用 MDA，而只會控制 Visual Studio 是否在啟動 MDA 時擲回例外狀況。  
