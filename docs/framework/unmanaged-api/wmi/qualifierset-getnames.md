...
|---------|---------|---------|
|  | 0 | 傳回所有限定詞的名稱。 |
| `WBEM_FLAG_LOCAL_ONLY` | 0x10 | 傳回目前的屬性或物件特定限定詞的名稱。 <br/> 屬性：傳回只特有的屬性 （包括覆寫） 限定詞，並不會傳播從類別定義的限定詞。 <br/> 執行個體：傳回只有特定執行個體的限定詞名稱。 <br/> 類別：傳回特定只限定詞所衍生的類別。
...
`pstrNames`\
[out]新`SAFEARRAY`包含要求的名稱。 陣列可以有 0 的項目。 如果發生錯誤時，新`SAFEARRAY`就不會傳回。
