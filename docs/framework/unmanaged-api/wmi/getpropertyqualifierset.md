...
`wszMethod`\
在屬性名稱。 `wszProperty` 必須指向有效的 `LPCWSTR`。

`ppQualSet`\
脫銷接收介面指標，允許存取屬性的限定詞。 `ppQualSet` 不可以是 `null`。 如果發生錯誤，則不會傳回新的物件，而且指標會設定為指向 `null`。
...
由於系統屬性沒有限定詞，因此如果您嘗試取得系統屬性的[IWbemQualifierSet](/windows/desktop/api/wbemcli/nn-wbemcli-iwbemqualifierset)指標，此函式會傳回 `WBEM_E_SYSTEM_PROPERTY`。
