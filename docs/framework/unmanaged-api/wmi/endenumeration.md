...
結束呼叫啟動列舉順序[BeginEnumeration 函式](beginenumeration.md)。  
...
此函式會包裝呼叫[IWbemClassObject::EndEnumeration](https://msdn.microsoft.com/library/aa391433%28v=vs.85%29.aspx)方法。

呼叫`EndEnumeration`函式不是必要項目，但建議因為列舉型別相關聯的資源釋出。 不過，resoruces 會自動解除配置時啟動下一次列舉或釋放物件。
