...
| `iCodedToken`..`iCodedTokenMax`<br> (64. 95)  | 編碼的權杖 | **IsCodedTokenType** <br>**IsRidOrToken** |
...
儲存在 *堆積* 中的值 (也就是 `IsHeapType == true`) 可以使用下列方式讀取：
...
> [!IMPORTANT]
> 若要使用上表中定義的常數，請包含 `#define _DEFINE_META_DATA_META_CONSTANTS` *cor .h* 標頭檔所提供的指示詞。
