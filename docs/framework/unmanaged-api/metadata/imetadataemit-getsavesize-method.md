...
## <a name="remarks"></a>備註  
 ...
 如果呼叫端實[IMapToken](imaptoken-interface.md)介面（透過[IMetaDataEmit：： SetHandler](imetadataemit-sethandler-method.md)或[IMetaDataEmit：： Merge](imetadataemit-merge-method.md)）， `GetSaveSize` 將會對中繼資料執行兩次傳遞，以將它優化並加以壓縮。 否則，不會執行任何優化。  
