...
> [!NOTE]
> `FunctionTailcall`函數在 .NET Framework 版本2.0 中已淘汰。 它會繼續運作，但會導致效能下降。 請改用 [FunctionTailcall2](functiontailcall2-function.md) 函數。  
...
## <a name="remarks"></a>備註  
 ...
 Tail 呼叫的目標函式會使用目前的堆疊框架，並且會直接傳回給發出 tail 呼叫之函式的呼叫端。 這表示將不會對做為 tail 呼叫目標的函式發出 [FunctionLeave](functionleave-function.md) 回呼。  
  
 `FunctionTailcall`函數是回呼; 您必須加以執行。 實作為必須使用 `__declspec` (`naked`) 儲存類別屬性。  
...
- 在結束時，您必須藉由關閉呼叫端所推送的所有參數來還原堆疊。  
 ...
 的執行 `FunctionTailcall` 不應封鎖，因為它會延遲垃圾收集。 執行不應嘗試垃圾收集，因為堆疊可能不是處於垃圾收集的易記狀態。 如果嘗試垃圾收集，則執行時間會封鎖直到傳回為止 `FunctionTailcall` 。  
  
 此外，此函式 `FunctionTailcall` 不能呼叫 managed 程式碼，或以任何方式造成受控記憶體配置。  
