...
## <a name="remarks"></a>備註  
 呼叫 `Shutdown` 方法之後，分析工具程式碼無法安全地呼叫[ICorProfilerInfo](icorprofilerinfo-interface.md)介面的方法。 `ICorProfilerInfo` 方法的任何呼叫都會在 `Shutdown` 方法傳回之後導致未定義的行為。 某些不可變事件在關機後可能仍會發生;分析工具應該小心在發生這種情況時立即傳回。  
