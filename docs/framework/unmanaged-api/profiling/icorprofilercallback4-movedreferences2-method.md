...
呼叫以報告壓縮記憶體回收造成的堆積中物件的新配置。 如果分析工具已執行 [ICorProfilerCallback4](icorprofilercallback4-interface.md) 介面，則會呼叫這個方法。 此回呼會取代 [ICorProfilerCallback：： MovedReferences](icorprofilercallback-movedreferences-method.md) 方法，因為它可以報告其長度超過 ULONG 中可表示之物件的較大範圍。  
