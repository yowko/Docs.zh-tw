...
```  
 ...
 我們也假設所`Fancy.AddTwice`內嵌呼叫至`SimpleAdd`。 分析工具可以使用這個列舉值，來尋找模組 B 中定義的所有方法的內嵌`Simple.Add`，並會在列舉結果`AddTwice`。  `inlineeModuleId` 是模組的識別項`A`，並`inlineeeMethodId`是識別碼`Simple.Add(int a, int b)`。  
