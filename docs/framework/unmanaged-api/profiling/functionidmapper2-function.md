...
- `pbHookFunction`
  ...
  \[out）當分析工具 `true` 想要接收、、和、、和回呼時，所設定的值指標 `FunctionEnter3` `FunctionLeave3` `FunctionTailcall3` `FunctionEnter3WithInfo` `FunctionLeave3WithInfo` `FunctionTailcall3WithInfo` ; 否則，它會將此值設定為 `false` 。
