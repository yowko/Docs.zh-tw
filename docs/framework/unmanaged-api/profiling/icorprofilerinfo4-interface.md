...
提供程式碼分析工具用來與 common language runtime （CLR）通訊的方法，以控制事件監視和要求資訊。 。 `ICorProfilerInfo4` 介面是其他 `ICorProfilerInfo` 介面的延伸。 它提供新的方法來支援及時（JIT）重新編譯，並在 .NET Framework 4.5 中新增。  
