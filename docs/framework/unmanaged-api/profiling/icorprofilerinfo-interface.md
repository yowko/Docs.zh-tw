...
|[GetClassFromObject 方法](../../../../docs/framework/unmanaged-api/profiling/icorprofilerinfo-getclassfromobject-method.md)|取得的 `ClassID`<br /><br /> 物件，並指定其 `ObjectID`。|  
...
|[SetFunctionReJIT 方法](../../../../docs/framework/unmanaged-api/profiling/icorprofilerinfo-setfunctionrejit-method.md)|未實作。 不要使用。|  
...
## <a name="remarks"></a>備註  
 ...
 CLR 會透過分析工具的[ICorProfilerCallback：： Initialize](../../../../docs/framework/unmanaged-api/profiling/icorprofilercallback-initialize-method.md)執行，在初始化期間，將 `ICorProfilerInfo` 介面傳遞至每個程式碼分析工具。 然後，程式碼分析工具可以呼叫 `ICorProfilerInfo` 介面的方法，以取得在 CLR 控制下執行之 managed 程式碼的相關資訊。  
