...
#### <a name="parameters"></a>參數  
 ...
 `typeDef`  
 [in]`mdTypeDef`參考該類型的中繼資料語彙基元。  
  
 `cTypeArgs`  
 [in]給定類型的型別參數數目。 此值必須是零的非泛型型別。  
...
## <a name="remarks"></a>備註  
 呼叫`GetClassFromTokenAndTypeArgs`方法`mdTypeRef`而不是`mdTypeDef`中繼資料語彙基元可以有無法預期的結果，呼叫端應該解決`mdTypeRef`至`mdTypeDef`時將傳遞給它。  
  
 如果尚未載入的型別，則呼叫`GetClassFromTokenAndTypeArgs`將會觸發載入，這可能會有危險的作業，許多內容中。 例如，呼叫這個方法期間載入模組或其他類型可能會導致無限迴圈因為執行階段嘗試循環載入。  
