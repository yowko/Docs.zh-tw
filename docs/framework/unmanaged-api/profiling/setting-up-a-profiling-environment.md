...
- 如果您在[ICorDebug：： CreateProcess](../debugging/icordebug-createprocess-method.md)呼叫中設定變數，它們只會套用到您當時正在執行的應用程式。 (它們也會套用到繼承此環境之應用程式所啟動的其他應用程式。)  
...
```  
 ...
 分析工具必須查詢 `pICorProfilerInfoUnk` [ICorProfilerInfo](icorprofilerinfo-interface.md)或[ICorProfilerInfo2](icorprofilerinfo2-interface.md)介面指標並加以儲存，以便稍後可以在程式碼剖析期間要求更多的資訊。  
