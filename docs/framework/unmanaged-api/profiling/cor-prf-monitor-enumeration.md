...
|`COR_PRF_MONITOR_APPDOMAIN_LOADS`|控制[ICorProfilerCallback](icorprofilercallback-interface.md)介面中的 `AppDomainCreation*` 和 `AppDomainShutdown*` 回呼。|  
|`COR_PRF_MONITOR_ASSEMBLY_LOADS`|控制[ICorProfilerCallback](icorprofilercallback-interface.md)介面中的 `AssemblyLoad*` 和 `AssemblyUnload*` 回呼。|  
|`COR_PRF_MONITOR_CACHE_SEARCHES`|控制[ICorProfilerCallback](icorprofilercallback-interface.md)介面中的 `JITCachedFunctionSearch*` 回呼。<br /><br /> 此旗標的行為已在 .NET Framework 2.0 版中變更。|  
|`COR_PRF_MONITOR_CCW`|控制[ICorProfilerCallback](icorprofilercallback-interface.md)介面中的 `COMClassicVTable*` 回呼。|  
|`COR_PRF_MONITOR_CLASS_LOADS`|控制[ICorProfilerCallback](icorprofilercallback-interface.md)介面中的 `ClassLoad*` 和 `ClassUnload*` 回呼。|  
|`COR_PRF_MONITOR_CLR_EXCEPTIONS`|控制[ICorProfilerCallback](icorprofilercallback-interface.md)介面中的 `ExceptionCLRCatcher*` 回呼。|  
...
|`COR_PRF_MONITOR_EXCEPTIONS`|控制[ICorProfilerCallback](icorprofilercallback-interface.md)介面中的[ExceptionThrown](icorprofilercallback-exceptionthrown-method.md)回呼和 `ExceptionSearch*`、`ExceptionOSHandler*`、`ExceptionUnwind*`和 `ExceptionCatcher*` 回呼。|  
...
|`COR_PRF_MONITOR_GC`|控制 `ICorProfilerCallback*` 介面中的[GarbageCollectionStarted](icorprofilercallback2-garbagecollectionstarted-method.md)、 [GarbageCollectionFinished](icorprofilercallback2-garbagecollectionfinished-method.md)、 [MovedReferences](icorprofilercallback-movedreferences-method.md)、 [MovedReferences2](icorprofilercallback4-movedreferences2-method.md)、 [SurvivingReferences](icorprofilercallback2-survivingreferences-method.md)、 [SurvivingReferences2](icorprofilercallback4-survivingreferences2-method.md)、 [ObjectReferences](icorprofilercallback-objectreferences-method.md)、 [ObjectsAllocatedByClass](icorprofilercallback-objectsallocatedbyclass-method.md)、 [RootReferences](icorprofilercallback-rootreferences-method.md)、 [RootReferences2](icorprofilercallback2-rootreferences2-method.md) [、HandleCreated、](icorprofilercallback2-handlecreated-method.md) [HandleDestroyed](icorprofilercallback2-handledestroyed-method.md)和[FinalizeableObjectQueued](icorprofilercallback2-finalizeableobjectqueued-method.md)回呼。 配置 `COR_PRF_MONITOR_GC` 時，會關閉並行垃圾收集。|  
|`COR_PRF_MONITOR_JIT_COMPILATION`|控制[ICorProfilerCallback](icorprofilercallback-interface.md)介面中的 `JITCompilation*`、 [JITFunctionPitched](icorprofilercallback-jitfunctionpitched-method.md)和[JITInlining](icorprofilercallback-jitinlining-method.md)回呼。|  
|`COR_PRF_MONITOR_MODULE_LOADS`|控制[ICorProfilerCallback](icorprofilercallback-interface.md)介面中的 `ModuleLoad*`、`ModuleUnload*`和[ModuleAttachedToAssembly](icorprofilercallback-moduleattachedtoassembly-method.md)回呼。|  
...
|`COR_PRF_MONITOR_REMOTING`|控制[ICorProfilerCallback](icorprofilercallback-interface.md)介面中的 `Remoting*` 回呼。|  
|`COR_PRF_MONITOR_REMOTING_ASYNC`|控制 `Remoting*` 回呼是否會監視非同步事件。|  
|`COR_PRF_MONITOR_REMOTING_COOKIE`|控制 Cookie 是否傳遞至 `Remoting*` 回呼。|  
|`COR_PRF_MONITOR_SUSPENDS`|控制[ICorProfilerCallback](icorprofilercallback-interface.md)介面中的 `RuntimeSuspend*`、`RuntimeResume*`、 [RuntimeThreadSuspended](icorprofilercallback-runtimethreadsuspended-method.md)和[RuntimeThreadResumed](icorprofilercallback-runtimethreadresumed-method.md)回呼。|  
...
|成員|描述|  
|------------|-----------------|  
|`COR_PRF_ENABLE_FRAME_INFO`|藉由使用[FunctionEnter2](functionenter2-function.md)回呼所傳回的 `COR_PRF_FRAME_INFO` 值來呼叫[GetFunctionInfo2](icorprofilerinfo2-getfunctioninfo2-method.md)方法，以啟用泛型函式的精確 `ClassID`。|  
...
|`COR_PRF_ENABLE_JIT_MAPS`|已取代。<br /><br /> 可讓分析工具使用[GetILToNativeMapping](icorprofilerinfo-getiltonativemapping-method.md)來取得 IL 對原生對應。 從 .NET Framework 2.0 開始，執行階段會一律追蹤 IL 與原生的對應；所以會一律將此旗標視為必須設定。|  
