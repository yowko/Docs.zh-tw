...
## <a name="remarks"></a>備註  
 ...
 所有執行緒都結束分析工具的程式碼後，就會核發 `ProfilerDetachSucceeded` 回呼。 呼叫這個方法時，分析工具應該執行任何不適合其解構函式的最後一刻工作，例如通知其 UI 或記錄元件。 不過，分析工具在此回呼 (（例如 [ICorProfilerInfo](icorprofilerinfo-interface.md) 或介面) ）期間，不能針對 CLR 所提供的介面呼叫函數 `IMetaData*` 。  
