...
## <a name="remarks"></a>備註  
 應用程式識別碼不是有效的任何資訊要求到`AppDomainCreationFinished`方法呼叫。  
  
 載入應用程式定義域的某些部分可能會繼續之後`AppDomainCreationFinished`回呼。 失敗的 HRESULT 中`hrStatus`表示失敗。 不過，成功 HRESULT 中`hrStatus`只會指出已成功建立應用程式定義域的第一個部分。  
