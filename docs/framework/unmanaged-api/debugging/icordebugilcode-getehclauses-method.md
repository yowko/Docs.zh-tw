...
## <a name="remarks"></a>備註  
 如果`cClauses`為 0 和`pcClauses`是非**null**，`pcClauses`設為 可用的例外狀況處理子句數。 若 `cClauses` 為非零，其代表 `clauses` 陣列的儲存容量。 傳回方法時，`clauses` 會包含 `cClauses` 項目的最大值，且 `pcClauses` 會設為實際寫入至 `clauses` 陣列的子句數。  
