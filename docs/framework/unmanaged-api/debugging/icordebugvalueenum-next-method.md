...
從列舉中取得指定數目的 "ICorDebugValue" 實例，從目前位置開始。  
...
## <a name="parameters"></a>參數  
 `celt`  
 在要抓取的 `ICorDebugValue` 實例數目。  
  
 `values`  
 脫銷指標陣列，其中每一個都會指向一個 `ICorDebugValue` 物件。  
  
 `pceltFetched`  
 脫銷實際傳回之實例數目的指標 `ICorDebugValue` 。 如果是一個，這個值可能會是 null `celt` 。  
