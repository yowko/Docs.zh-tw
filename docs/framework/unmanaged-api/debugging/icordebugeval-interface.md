...
|[GetResult 方法](../../../../docs/framework/unmanaged-api/debugging/icordebugeval-getresult-method.md)|取得包含評估結果之 `ICorDebugValue` 的介面指標。|  
...
## <a name="remarks"></a>備註  
 ...
 因為使用者程式碼正在進行評估，所以可能會發生任何 debug 事件，包括類別載入和中斷點。 偵錯工具會如往常般接收這些事件的回呼。 評估的狀態將會視為一般程式狀態檢查的一部分。 堆疊鏈會是 `CHAIN_FUNC_EVAL` 鏈（請參閱 "CorDebugStepReason" 列舉和[ICorDebugChain：： GetReason](../../../../docs/framework/unmanaged-api/debugging/icordebugchain-getreason-method.md)方法）。 完整的偵錯工具 API 會繼續正常運作。  
