...
## <a name="parameters"></a>參數  
 ...
 `bInitialState`  
 [in] `true` ，如果物件收到信號，則為，否則為 `false` 。  
  
 `ppEvent`  
 擴展 [IHostManualEvent](ihostmanualevent-interface.md) 實例位址的指標，如果無法建立事件，則為 null。  
  
## <a name="return-value"></a>傳回值  
  
|HRESULT|描述|  
|-------------|-----------------|  
|S_OK|`CreateManualEvent` 傳回成功。|  
...
## <a name="remarks"></a>備註  
 ...
 `CreateManualEvent` 建立一個 `IHostManualEvent` 手動重設的事件物件，該物件需要呼叫 [IHostManualEvent：： reset](ihostmanualevent-reset-method.md) 方法，才能將它設定為非信號狀態。 `CreateManualEvent``CreateEvent`使用 `true` 針對參數指定的值，鏡像 Win32 函數 `bManualReset` 。  
