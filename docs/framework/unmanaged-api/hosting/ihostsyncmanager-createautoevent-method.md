...
## <a name="parameters"></a>參數  
 `ppEvent`  
 脫銷主機所實[IHostAutoEvent](../../../../docs/framework/unmanaged-api/hosting/ihostautoevent-interface.md)實例的位址指標，如果無法建立事件物件，則為 null。  
  
## <a name="return-value"></a>傳回值  
  
|HRESULT|描述|  
|-------------|-----------------|  
|S_OK|已成功傳回 `CreateAutoEvent`。|  
...
## <a name="remarks"></a>備註  
 `CreateAutoEvent` 會建立自動事件物件，其狀態會在等候的執行緒釋放之後自動變更為非信號。 這個方法會使用針對 `bManualReset` 參數所指定的 `false` 值，鏡像 Win32 `CreateEvent` 函式  
