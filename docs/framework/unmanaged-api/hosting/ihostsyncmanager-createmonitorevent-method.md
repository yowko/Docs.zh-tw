...
## <a name="remarks"></a>備註  
 ...
 `CreateMonitorEvent` 傳回 `IHostAutoEvent` CLR 在其 managed 類型的實中使用的 <xref:System.Threading.Monitor?displayProperty=nameWithType> 。 這個方法會鏡像 Win32 `CreateEvent` 函數，並 `false` 針對參數指定值 `bManualReset` 。  
  
 主機可以使用 cookie，藉由呼叫 [ICLRSyncManager：： GetMonitorOwner](iclrsyncmanager-getmonitorowner-method.md) 方法來判斷要在監視器上等候的工作。  
