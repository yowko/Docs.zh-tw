...
作為對應 Win32 函式的邏輯包裝函式。 的 Win32 執行會 `VirtualAlloc` 保留或認可呼叫進程之虛擬位址空間中的頁面區域。  
...
## <a name="remarks"></a>備註  
 藉由呼叫，您可以在處理常式的位址空間中保留一個區域 `VirtualAlloc` 。 `pAddress`參數包含您想要的記憶體區塊的開頭位址。 這個參數通常會設定為 null。 作業系統會保留您的進程可用的免費位址範圍記錄。 `pAddress`Null 值會指示系統保留區域，不論其是否適合。 或者，您可以為記憶體區塊提供特定的起始位址。 在這兩種情況下，輸出參數 `ppMem` 都會以指標的形式傳回給配置的記憶體。 函數本身會傳回 HRESULT 值。  
  
 Win32 函式沒有 `VirtualAlloc` `ppMem` 參數，而是改為傳回已配置記憶體的指標。 如需詳細資訊，請參閱 Windows 平臺檔。  
