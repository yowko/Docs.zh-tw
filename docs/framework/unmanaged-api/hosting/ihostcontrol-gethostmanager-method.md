...
## <a name="parameters"></a>參數  
 `riid`  
 在Common language runtime （CLR）所查詢之介面的 `IID`。  
  
 `ppObject`  
 脫銷主機實介面的指標，如果主機不支援這個介面，則為 null。  
  
## <a name="return-value"></a>傳回值  
  
|HRESULT|描述|  
|-------------|-----------------|  
|S_OK|已成功傳回 `GetHostManager`。|  
...
- [IHostSecurityManager](../../../../docs/framework/unmanaged-api/hosting/ihostsecuritymanager-interface.md)  
 ...
 如果主機支援指定的介面，它會將 `ppObject` 設定為該介面的執行。 否則，它會將 `ppObject` 設為 null。  
  
 CLR 不會在主機管理員上呼叫 `Release`，即使您將它關閉也是一樣。  
