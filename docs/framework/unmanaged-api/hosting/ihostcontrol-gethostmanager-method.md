...
## <a name="parameters"></a>參數  
 `riid`  
 在`IID`Common language runtime （CLR）所查詢之介面的。  
...
- [IHostSecurityManager](ihostsecuritymanager-interface.md)  
 ...
 如果主機支援指定的介面，它會將設定 `ppObject` 為該介面的執行。 否則，它會將設定 `ppObject` 為 null。  
  
 CLR 不會呼叫 `Release` 主機管理員，即使您將它關閉也是一樣。  
