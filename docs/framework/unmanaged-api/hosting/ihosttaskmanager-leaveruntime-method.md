...
## <a name="remarks"></a>備註  
 不受管理程式碼的呼叫序列可以進行嵌套。 例如，下列清單描述一種假設的情況，其中的呼叫順序為 `LeaveRuntime` [IHostTaskManager：： ReverseEnterRuntime](../../../../docs/framework/unmanaged-api/hosting/ihosttaskmanager-reverseenterruntime-method.md)， [IHostTaskManager：： ReverseLeaveRuntime](ihosttaskmanager-reverseleaveruntime-method.md)，並 `IHostTaskManager::EnterRuntime` 允許主機識別嵌套層。  
