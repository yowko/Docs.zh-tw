...
取得類型的介面指標<xref:System._AppDomain?displayProperty=nameWithType>，代表目前的處理序的預設網域。  
...
#### <a name="parameters"></a>參數  
 `pAppDomain`  
 [out]類型的介面指標<xref:System._AppDomain?displayProperty=nameWithType>至<xref:System.AppDomain>代表處理程序的預設應用程式定義域的執行個體。  
  
 此指標為型別`IUnknown`，因此呼叫端通常應該呼叫`QueryInterface`取得類型的介面指標<xref:System._AppDomain?displayProperty=nameWithType>。  
