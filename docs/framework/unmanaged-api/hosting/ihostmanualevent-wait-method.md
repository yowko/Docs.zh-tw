...
導致目前的[IHostManualEvent](ihostmanualevent-interface.md)實例等到其擁有，或指定的時間量過後才等候。  
...
## <a name="parameters"></a>參數  
 `dwMilliseconds`  
 在如果目前的 `IHostManualEvent` 實例不是擁有的，則傳回之前要等候的毫秒數。  
  
 `option`  
 在其中一個[WAIT_OPTION](wait-option-enumeration.md)值，指出當此作業封鎖時，主機應採取的動作。  
...
|HOST_E_DEADLOCK|主機在等候間隔期間偵測到鎖死，並選擇目前的 `IHostManualEvent` 實例作為鎖死犧牲者。|  
