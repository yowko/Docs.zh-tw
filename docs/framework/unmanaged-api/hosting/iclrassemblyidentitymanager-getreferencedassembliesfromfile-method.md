...
取得[ICLRReferenceAssemblyEnum](../../../../docs/framework/unmanaged-api/hosting/iclrreferenceassemblyenum-interface.md)實例，其中包含元件在指定檔案路徑中參考的元件清單。  
...
## <a name="parameters"></a>參數  
 ...
 在[ICLRAssemblyReferenceList](../../../../docs/framework/unmanaged-api/hosting/iclrassemblyreferencelist-interface.md)物件的指標，表示應該從 `ppReferenceEnum`排除的元件。  
  
 `ppReferenceEnum`  
 脫銷`ICLRReferenceAssemblyEnum` 物件位址的指標，其中包含元件在 `pwzFilePath`的元件識別資料，但不包括 `pExcludeAssembliesList`所代表的元件。  
...
## <a name="remarks"></a>備註  
 呼叫端可以選擇從傳回的清單中排除一組已知的元件參考。 這個集合是由 `pExcludeAssembliesList` 參數所定義。  
