...
將衍生自的型別設定 <xref:System.AppDomainManager> 為應用程式域管理員的型別。  
...
## <a name="parameters"></a>參數  
 ...
 在實作為衍生自之要求型別的元件名稱 <xref:System.AppDomainManager> 。  
  
 `pwzAppDomainManagerType`  
 在在參數中實作為的型別名稱 `pwzAppDomainManagerAssembly` ，這個參數會執行的功能 <xref:System.AppDomainManager> 。  
