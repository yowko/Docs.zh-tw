...
依指定的數量增加目前 [IHostSemaphore](ihostsemaphore-interface.md) 實例的計數。  
...
## <a name="parameters"></a>參數  
 ...
 `lReleaseCount`  
 在要增加目前實例之計數的量 `IHostSemaphore` 。 此數量必須大於零。  
  
 `lpPreviousCount`  
 擴展先前計數的指標，如果呼叫端不需要先前的計數，則為 null。  
  
## <a name="return-value"></a>傳回值  
  
|HRESULT|描述|  
|-------------|-----------------|  
|S_OK|`ReleaseSemaphore` 傳回成功。|  
...
## <a name="remarks"></a>備註  
 ...
 CLR 通常會呼叫， `ReleaseSemaphore` 以通知主機它已完成使用資源，並傳遞值1給 `lReleaseCount` 參數。  
