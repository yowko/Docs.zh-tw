...
|HRESULT|描述|  
|-------------|-----------------|  
|S_OK|已成功傳回 `CreateRWLockWriterEvent`。|  
...
## <a name="remarks"></a>備註  
 CLR 會呼叫 `CreateRWLockWriterEvent` 方法，以取得 `IHostAutoEvent` 實例的參考，以在其寫入器鎖定的執行中使用。 主機可以使用指定的 cookie，藉由呼叫[ICLRSyncManager](../../../../docs/framework/unmanaged-api/hosting/iclrsyncmanager-interface.md)介面的反復專案方法，來判斷哪些工作正在等候鎖定。  
