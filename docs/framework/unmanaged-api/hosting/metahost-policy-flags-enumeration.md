...
|`METAHOST_POLICY_SHOW_ERROR_DIALOG`|如果顯示錯誤對話方塊`GetRequestedRuntime`是找不到適用於輸入參數的執行階段。 從.NET Framework 4.5 開始，此錯誤對話方塊中，可能會需要 Windows 功能 對話方塊，詢問使用者是否想要啟用適當的功能的形式。|  
