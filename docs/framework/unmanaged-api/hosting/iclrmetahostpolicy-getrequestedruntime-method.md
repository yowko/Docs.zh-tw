...
|`pwzImageVersion`|[out] 選用。 當`GetRequestedRuntime`傳回、 包含 CLR 版本對應至[ICLRRuntimeInfo](../../../../docs/framework/unmanaged-api/hosting/iclrruntimeinfo-interface.md)傳回的介面。|
...
|`pdwConfigFlags`|[out] 選用。 如果`GetRequestedRuntime`繫結過程中，使用組態檔，當它傳回時，`pdwConfigFlags`包含[METAHOST_CONFIG_FLAGS](../../../../docs/framework/unmanaged-api/hosting/metahost-config-flags-enumeration.md)值，指出是否[\<啟動 >](../../../../docs/framework/configure-apps/file-schema/startup/startup-element.md)項目具有`useLegacyV2RuntimeActivationPolicy`屬性集和屬性的值。 適用於[METAHOST_CONFIG_FLAGS_LEGACY_V2_ACTIVATION_POLICY_MASK](../../../../docs/framework/unmanaged-api/hosting/metahost-config-flags-enumeration.md)遮罩至`pdwConfigFlags`若要取得這些值`useLegacyV2RuntimeActivationPolicy`。|
|`riid`|[in]指定要求的介面識別碼 IID_ICLRRuntimeInfo [ICLRRuntimeInfo](../../../../docs/framework/unmanaged-api/hosting/iclrruntimeinfo-interface.md)介面。|
|`ppRuntime`|[out]當`GetRequestedRuntime`會傳回，包含對應的指標[ICLRRuntimeInfo](../../../../docs/framework/unmanaged-api/hosting/iclrruntimeinfo-interface.md)介面。|
