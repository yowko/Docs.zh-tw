...
|HRESULT|描述|  
|-------------|-----------------|  
|S_OK|`DeleteRWLockOwnerIterator` 傳回成功。|  
|HOST_E_CLRNOTAVAILABLE|CLR 尚未載入至進程，或處於無法執行 managed 程式碼或成功處理呼叫的狀態。|  
...
## <a name="remarks"></a>備註  
 ...
 主機可以呼叫這個方法，並 `CreateRWLockOwnerIterator` 確保其執行緒執行保持同步。  
