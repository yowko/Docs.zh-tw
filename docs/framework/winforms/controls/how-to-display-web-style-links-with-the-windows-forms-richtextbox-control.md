...
---
# <a name="how-to-display-web-style-links-with-the-windows-forms-richtextbox-control"></a>HOW TO：顯示 Web 樣式連結，使用 Windows Forms RichTextBox 控制項
...
4.  撰寫事件處理常式<xref:System.Windows.Forms.RichTextBox.LinkClicked>傳送瀏覽器所需的文字的事件。  
    ...
    > [!IMPORTANT]
    >  在呼叫<xref:System.Diagnostics.Process.Start%2A?displayProperty=nameWithType>方法中，您將會遇到<xref:System.Security.SecurityException>例外狀況，如果您在部分信任內容中執行的程式碼，因為沒有足夠的權限。 如需詳細資訊，請參閱[程式碼存取安全性基本概念](../../../../docs/framework/misc/code-access-security-basics.md)。  
