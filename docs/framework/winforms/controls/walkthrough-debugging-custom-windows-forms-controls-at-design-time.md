...
當您建立自訂控制項時，您通常會發現有必要將它的設計階段行為進行調試。 如果您要撰寫自訂控制項的自訂設計工具，更是如此。 如需詳細資訊，請參閱[逐步解說：建立利用 Visual Studio 設計階段功能的 Windows Forms 控制項](creating-a-wf-control-design-time-features.md)。
...
3. 在 [**屬性**] 視窗中尋找 `DemoString` 的自訂屬性。 請注意，您可以變更它的值，就像任何其他屬性一樣。 另請注意，當選取 [`DemoString`] 屬性時，屬性的描述字串會顯示在 [**屬性**] 視窗的底部。
...
1. 在程式**代碼編輯器**中開啟**DebugControl**原始程式檔，並將中斷點放在 `DemoString` 屬性的 `Set` 存取子上。
...
3. 在 Visual Studio 的新實例中，開啟 "DebuggingExample" 解決方案。 您可以從 [檔案] 功能表選取 [**最近使用的專案** **]，輕鬆**地找到解決方案。 "DebuggingExample" 方案檔會列為最近使用的檔案。
