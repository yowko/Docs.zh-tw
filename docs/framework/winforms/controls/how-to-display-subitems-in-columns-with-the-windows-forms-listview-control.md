...
1.  加入所需的任何資料行。 因為第一欄會顯示項目的<xref:System.Windows.Forms.ListView.Text%2A>屬性，您需要一個多個資料行有子項目。 如需有關如何加入資料行的詳細資訊，請參閱[How to： 將資料行加入 Windows Form ListView 控制項](../../../../docs/framework/winforms/controls/how-to-add-columns-to-the-windows-forms-listview-control.md)。  
  
2.  呼叫<xref:System.Windows.Forms.ListViewItem.ListViewSubItemCollection.Add%2A>方法所傳回的集合<xref:System.Windows.Forms.ListViewItem.SubItems%2A>項目的屬性。 下列程式碼範例會設定 employee name 和部門以取得的清單項目。  
