---
title: CheckedListBox 控制項概觀
ms.date: 03/30/2017
f1_keywords:
- CheckedListBox
helpviewer_keywords:
- CheckedListBox control [Windows Forms], about CheckedListBox control
- check marks
- list boxes [Windows Forms], about list boxes
ms.assetid: e6cff3ce-e2ba-458e-851f-299e8e9c8abf
ms.openlocfilehash: f3825fccbd881a3081d0f9954c81d18df5183325
ms.sourcegitcommit: de17a7a0a37042f0d4406f5ae5393531caeb25ba
ms.translationtype: MT
ms.contentlocale: zh-TW
ms.lasthandoff: 01/24/2020
ms.locfileid: "76736984"
---
# <a name="checkedlistbox-control-overview-windows-forms"></a>CheckedListBox 控制項概觀 (Windows Form)
Windows Forms <xref:System.Windows.Forms.CheckedListBox> 控制項會擴充 <xref:System.Windows.Forms.ListBox> 控制項。 它幾乎會執行清單方塊所執行的所有工作，也可以在清單中的專案旁顯示核取記號。 這兩個控制項之間的其他差異在於，核取清單方塊僅支援 <xref:System.Windows.Forms.DrawMode.Normal?displayProperty=nameWithType>;而且核取的清單方塊只能有一個專案，或選取 [無]。 請注意，選取的專案會在表單上反白顯示，而且與核取的專案不同。  
  
 已選取的清單方塊可以在設計階段使用 [**字串集合編輯器**] 加入專案，或是在執行時間使用 [<xref:System.Windows.Forms.CheckedListBox.Items%2A>] 屬性，以動態方式新增其專案。 如需詳細資訊，請參閱[如何：在 Windows Forms ComboBox、ListBox 或 CheckedListBox 控制項中新增和移除專案](add-and-remove-items-from-a-wf-combobox.md)。  
  
## <a name="see-also"></a>另請參閱

- <xref:System.Windows.Forms.CheckedListBox>
- <xref:System.Windows.Forms.CheckedListBox.Items%2A?displayProperty=nameWithType>
- <xref:System.Windows.Forms.ListControl.DataSource%2A?displayProperty=nameWithType>
- [ListBox 控制項概觀](listbox-control-overview-windows-forms.md)
- [用來列出選項的 Windows Forms 控制項](windows-forms-controls-used-to-list-options.md)
- [操作說明：判斷 Windows Forms CheckedListBox 控制項中的已核取項目](how-to-determine-checked-items-in-the-windows-forms-checkedlistbox-control.md)
