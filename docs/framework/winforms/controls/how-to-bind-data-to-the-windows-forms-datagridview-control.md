...
Visual Studio 的資料繫結至 DataGridView 控制項中的廣泛支援。 如需詳細資訊，請參閱[如何：資料繫結至 Windows Form DataGridView 控制項中使用設計工具](https://msdn.microsoft.com/library/33w255ac\(v=vs.110\))。  
...
若要建置並執行此範例中，貼上程式碼插入*Form1*中新的 Windows Form 專案的程式碼檔案。  如需詳細資訊，請參閱[如何：編譯並執行完整的 Windows Form 程式碼範例，使用 Visual Studio](https://msdn.microsoft.com/library/Bb129228\(v=vs.110\))。 如需從建置C#或 Visual Basic 命令列，請參閱[命令列使用 csc.exe 建置](/csharp/language-reference/compiler-options/command-line-building-with-csc-exe)或是[從命令列建置](/visual-basic/reference/command-line-compiler/building-from-the-command-line)。  
