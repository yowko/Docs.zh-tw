...
<xref:System.Windows.Forms.TableLayoutPanel> 會在方格中排列其內容，提供類似 HTML \<table > 元素的功能。 <xref:System.Windows.Forms.TableLayoutPanel> 控制項可讓您將控制項放在方格版面配置中，而不需要明確指定每個個別控制項的位置。 其儲存格依資料列和資料行排列，大小可以各不相同。 資料格可以合併到資料列和資料行。 資料格可以包含表單所能包含的任何內容，而且在大部分其他方面也可以做為容器的行為。
...
2. 將 [<xref:System.Windows.Forms.Button>] 控制項從 [**工具箱**] 拖曳至 [<xref:System.Windows.Forms.TableLayoutPanel>] 控制項，並將它放入其中一個儲存格。 請注意，<xref:System.Windows.Forms.Button> 控制項是建立在您選取的資料格中。

3. 將三個 <xref:System.Windows.Forms.Button> 控制項從 [**工具箱**] 拖曳至 [<xref:System.Windows.Forms.TableLayoutPanel>] 控制項，讓每個資料格都包含一個按鈕。

4. 抓取兩個數據行之間的垂直調整大小控點，並將其向左移。 請注意，第一個資料行中的 <xref:System.Windows.Forms.Button> 控制項會調整成較小的寬度，而第二個數據行中 <xref:System.Windows.Forms.Button> 控制項的大小則不會變更。

5. 抓取兩個數據行之間的垂直調整大小控點，並將它向右移動。 請注意，第一個資料行中的 <xref:System.Windows.Forms.Button> 控制項會回到其原始大小，而第二個數據行中的 <xref:System.Windows.Forms.Button> 控制項則會移至右方。

6. 向上和向下移動水準調整大小控點，以查看面板中的控制項效果。

## <a name="positioning-controls-within-cells-using-docking-and-anchoring"></a>使用停駐和錨定將控制項放在資料格中

<xref:System.Windows.Forms.TableLayoutPanel> 中子控制項的錨定行為，與其他容器控制項中的行為不同。 子控制項的銜接行為與其他容器控制項相同。
...
2. 選取其中一個其他 <xref:System.Windows.Forms.Button> 控制項。 將其 <xref:System.Windows.Forms.Control.Anchor%2A> 屬性的值變更為 <xref:System.Windows.Forms.AnchorStyles.Right>。 請注意，它會移動，使其右框線接近資料格的右框線。 框線之間的距離是 <xref:System.Windows.Forms.Button> 控制項的 <xref:System.Windows.Forms.Control.Margin%2A> 屬性和麵板 <xref:System.Windows.Forms.Control.Padding%2A> 屬性的總和。

3. 將 <xref:System.Windows.Forms.Button> 控制項的 <xref:System.Windows.Forms.Control.Anchor%2A> 屬性值變更為 <xref:System.Windows.Forms.AnchorStyles.Right> 和 <xref:System.Windows.Forms.AnchorStyles.Left>。 請注意，控制項的大小會調整為儲存格的寬度，並將 <xref:System.Windows.Forms.Control.Margin%2A> 和 <xref:System.Windows.Forms.Control.Padding%2A> 值納入考慮。

4. 使用 <xref:System.Windows.Forms.AnchorStyles.Top> 和 <xref:System.Windows.Forms.AnchorStyles.Bottom> 樣式，重複步驟2和3。
...
<xref:System.Windows.Forms.TableLayoutPanel> 控制項會在設計階段，將數個新的屬性加入控制項。 其中兩個屬性 `RowSpan` 和 `ColumnSpan`。 您可以使用這些屬性，讓控制項跨越一個以上的資料列或資料行。
...
3. 在數個 [工具箱]的控制項上按兩下。 請注意，新的控制項會連續出現在 <xref:System.Windows.Forms.TableLayoutPanel> 控制項的未佔用資料格中。 另請注意，如果沒有可用的開啟儲存格，<xref:System.Windows.Forms.TableLayoutPanel> 控制項會展開以容納新的控制項。

## <a name="automatic-handling-of-overflows"></a>自動處理溢位

當您將控制項插入 <xref:System.Windows.Forms.TableLayoutPanel> 控制項時，您可能會用盡新控制項的空白儲存格。 <xref:System.Windows.Forms.TableLayoutPanel> 控制項會藉由增加資料格的數目來自動處理這種情況。

#### <a name="to-observe-automatic-handling-of-overflows"></a>觀察溢出的自動處理

1. 如果 <xref:System.Windows.Forms.TableLayoutPanel> 控制項中仍然有空白的資料格，請繼續插入新的 <xref:System.Windows.Forms.Button> 控制項，直到 <xref:System.Windows.Forms.TableLayoutPanel> 控制項已滿為止。

2. 一旦 <xref:System.Windows.Forms.TableLayoutPanel> 控制項已滿，請按兩下 [**工具箱**] 中的 <xref:System.Windows.Forms.Button> 圖示，以插入另一個 <xref:System.Windows.Forms.Button> 控制項。 請注意，<xref:System.Windows.Forms.TableLayoutPanel> 控制項會建立新的儲存格，以容納新的控制項。 再插入一些控制項，並觀察調整大小行為。

3. 變更 <xref:System.Windows.Forms.TableLayoutPanel> 控制項的 <xref:System.Windows.Forms.TableLayoutPanel.GrowStyle%2A> 屬性值為 <xref:System.Windows.Forms.TableLayoutPanelGrowStyle.FixedSize>。 按兩下 [**工具箱**] 中的 [<xref:System.Windows.Forms.Button>] 圖示，以插入 <xref:System.Windows.Forms.Button> 控制項，直到 <xref:System.Windows.Forms.TableLayoutPanel> 控制項填滿為止。 再次按兩下 [**工具箱**] 中的 [<xref:System.Windows.Forms.Button>] 圖示。 請注意，您會收到來自**Windows Form 設計工具**的錯誤訊息，通知您無法建立其他資料列和資料行。
...
5. 拖曳滑鼠指標以繪製 <xref:System.Windows.Forms.Button> 控制項的外框。 如滿意大小，請放開滑鼠按鈕。 請注意，<xref:System.Windows.Forms.Button> 控制項會建立在您繪製控制項外框的儲存格中。

## <a name="multiple-controls-within-cells-are-not-permitted"></a>不允許在資料格中有多個控制項

<xref:System.Windows.Forms.TableLayoutPanel> 控制項在每個資料格只能包含一個子控制項。
...
- 將多個控制項的選取專案貼入 <xref:System.Windows.Forms.TableLayoutPanel> 控制項，並記下控制項的插入方式。
