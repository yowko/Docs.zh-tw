...
---
# <a name="how-to-lock-controls-to-windows-forms"></a>作法：鎖定 Windows Forms 的控制項
...
1. 在 [**屬性**] 視窗中,按一下 [已鎖定`true`] 屬性, 然後選取。 (按兩下名稱即可切換屬性設定)。
...
1. 在 [**屬性**] 視窗中,按一下 [已鎖定`false`] 屬性, 然後選取。 (按兩下名稱即可切換屬性設定)。
