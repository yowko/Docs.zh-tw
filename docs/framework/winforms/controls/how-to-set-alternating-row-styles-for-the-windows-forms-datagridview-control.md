...
---
# <a name="how-to-set-alternating-row-styles-for-the-windows-forms-datagridview-control"></a>作法：設定 Windows Forms DataGridView 控制項的替代資料列樣式
表格式資料通常會以類似 Ledger 的格式呈現給使用者，在這種格式中，交替資料列會有不同的背景色彩。 這種格式可讓使用者輕鬆地指出每個資料列中有哪些儲存格，特別是具有許多資料行的寬資料表。  
 ...
 在 Visual Studio 中會支援這項工作。  另請[參閱如何:使用設計](set-alternating-row-styles-for-the-datagrid-using-the-designer.md)工具設定 Windows Forms DataGridView 控制項的替代資料列樣式。  
