...
### <a name="avoid-visual-inheritance"></a>避免視覺繼承
 <xref:System.Windows.Forms.TableLayoutPanel>控制項不支援在 Windows Form 設計工具，在 Visual Studio 中的視覺化繼承。 A <xref:System.Windows.Forms.TableLayoutPanel> 「 已鎖定 」 在設計階段，會出現在衍生類別中的控制項。
