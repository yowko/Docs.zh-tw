...
本主題介紹 Windows 表單應用程式的基本元素，並提供一個簡單的示例，用於使用控制項並處理 Windows 表單應用程式中的事件。  
  
## <a name="simple-windows-forms-applications"></a>簡單視窗表單應用程式  
 至少，Windows 表單應用程式包含以下元素：  
  
- 派生自<xref:System.Windows.Forms.Form?displayProperty=nameWithType>的一個或多個類。  
  
- 調用`Main` `static` （`shared`在 Visual Basic 中）<xref:System.Windows.Forms.Application.Run%2A>方法並將<xref:System.Windows.Forms.Form>實例傳遞給它的方法。 該方法<xref:System.Windows.Forms.Application.Run%2A>處理從作業系統到應用程式的消息。  
  
 以下代碼示例顯示了 Windows 表單應用程式的基本元素。  
...
## <a name="using-controls-in-a-windows-forms-application"></a>在 Windows 表單應用程式中使用控制項  
 下面的代碼示例顯示了一個簡單的應用程式，它說明了 Windows 表單應用程式如何使用控制項和處理事件。 該示例由表單上的三個按鈕組成;按一下時，每個按鈕都會更改背景顏色。  
