...
## <a name="test-the-run-time-behavior-of-a-usercontrol"></a>測試 UserControl 的執行階段行為

1. 在 Visual Studio 中，建立 Windows 控制項程式庫專案，稱為**TestContainerExample**。 如需詳細資訊，請參閱 < [Windows 控制項程式庫範本](https://docs.microsoft.com/previous-versions/kxczf775(v=vs.100))。

2. 在  **Windows Form 設計工具**，拖曳<xref:System.Windows.Forms.Label>控制項從**工具箱**拖曳至控制項的設計介面。

3. 按下**F5**以建置專案並執行**UserControl 測試容器**。 測試容器會顯示您<xref:System.Windows.Forms.UserControl>中**預覽**窗格。
...
## <a name="test-user-controls-from-another-project"></a>從另一個專案的測試使用者控制項
