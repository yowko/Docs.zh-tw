...
---
# <a name="how-to-use-a-background-thread-to-search-for-files"></a><span data-ttu-id="d368c-102">如何：使用背景執行緒搜尋檔案</span><span class="sxs-lookup"><span data-stu-id="d368c-102">How to: Use a Background Thread to Search for Files</span></span>
<span data-ttu-id="d368c-103">元件<xref:System.ComponentModel.BackgroundWorker>將替換<xref:System.Threading>命名空間並添加功能;但是，如果<xref:System.Threading>願意，命名空間將保留，以便向後相容性和未來使用。</span><span class="sxs-lookup"><span data-stu-id="d368c-103">The <xref:System.ComponentModel.BackgroundWorker> component replaces and adds functionality to the <xref:System.Threading> namespace; however, the <xref:System.Threading> namespace is retained for both backward compatibility and future use, if you choose.</span></span> <span data-ttu-id="d368c-104">有關詳細資訊，請參閱[後臺輔助角色元件概述](backgroundworker-component-overview.md)。</span><span class="sxs-lookup"><span data-stu-id="d368c-104">For more information, see [BackgroundWorker Component Overview](backgroundworker-component-overview.md).</span></span>

 <span data-ttu-id="d368c-105">Windows 表單使用單線程單元 （STA） 模型，因為 Windows 表單基於本機 Win32 視窗，這些視窗本質上是單元執行緒的。</span><span class="sxs-lookup"><span data-stu-id="d368c-105">Windows Forms uses the single-threaded apartment (STA) model because Windows Forms is based on native Win32 windows that are inherently apartment-threaded.</span></span> <span data-ttu-id="d368c-106">STA 模型意味著可以在任何執行緒上創建視窗，但它在創建執行緒後無法切換執行緒，並且對其的所有函式呼叫都必須在其創建執行緒上發生。</span><span class="sxs-lookup"><span data-stu-id="d368c-106">The STA model implies that a window can be created on any thread, but it cannot switch threads once created, and all function calls to it must occur on its creation thread.</span></span> <span data-ttu-id="d368c-107">在 Windows 表單之外，.NET 框架中的類使用免費執行緒模型。</span><span class="sxs-lookup"><span data-stu-id="d368c-107">Outside Windows Forms, classes in the .NET Framework use the free threading model.</span></span> <span data-ttu-id="d368c-108">有關 .NET 框架中線程化的資訊，請參閱[執行緒](../../../standard/threading/index.md)。</span><span class="sxs-lookup"><span data-stu-id="d368c-108">For information about threading in the .NET Framework, see [Threading](../../../standard/threading/index.md).</span></span>

 <span data-ttu-id="d368c-109">STA 模型要求需要從控制項的創建執行緒外部調用控制項上的任何方法都必須封送到（在）控制項的創建執行緒上執行。</span><span class="sxs-lookup"><span data-stu-id="d368c-109">The STA model requires that any methods on a control that need to be called from outside the control's creation thread must be marshaled to (executed on) the control's creation thread.</span></span> <span data-ttu-id="d368c-110">基類<xref:System.Windows.Forms.Control>為此提供了多種方法<xref:System.Windows.Forms.Control.Invoke%2A>（、<xref:System.Windows.Forms.Control.BeginInvoke%2A>和<xref:System.Windows.Forms.Control.EndInvoke%2A>）。</span><span class="sxs-lookup"><span data-stu-id="d368c-110">The base class <xref:System.Windows.Forms.Control> provides several methods (<xref:System.Windows.Forms.Control.Invoke%2A>, <xref:System.Windows.Forms.Control.BeginInvoke%2A>, and <xref:System.Windows.Forms.Control.EndInvoke%2A>) for this purpose.</span></span> <span data-ttu-id="d368c-111"><xref:System.Windows.Forms.Control.Invoke%2A>進行同步方法調用;<xref:System.Windows.Forms.Control.BeginInvoke%2A>進行非同步方法調用。</span><span class="sxs-lookup"><span data-stu-id="d368c-111"><xref:System.Windows.Forms.Control.Invoke%2A> makes synchronous method calls; <xref:System.Windows.Forms.Control.BeginInvoke%2A> makes asynchronous method calls.</span></span>

 <span data-ttu-id="d368c-112">如果在控制項中使用多執行緒處理資源密集型任務，使用者介面可以在資源密集型計算在後臺執行緒上執行時保持回應。</span><span class="sxs-lookup"><span data-stu-id="d368c-112">If you use multithreading in your control for resource-intensive tasks, the user interface can remain responsive while a resource-intensive computation executes on a background thread.</span></span>

 <span data-ttu-id="d368c-113">下面的示例 （`DirectorySearcher`） 顯示了一個多執行緒 Windows 表單控制項，該控制項使用後臺執行緒遞迴地搜索與指定搜索字串匹配的檔，然後使用搜尋結果填充清單方塊。</span><span class="sxs-lookup"><span data-stu-id="d368c-113">The following sample (`DirectorySearcher`) shows a multithreaded Windows Forms control that uses a background thread to recursively search a directory for files matching a specified search string and then populates a list box with the search result.</span></span> <span data-ttu-id="d368c-114">示例說明的關鍵概念如下：</span><span class="sxs-lookup"><span data-stu-id="d368c-114">The key concepts illustrated by the sample are as follows:</span></span>

- <span data-ttu-id="d368c-115">`DirectorySearcher`啟動新執行緒以執行搜索。</span><span class="sxs-lookup"><span data-stu-id="d368c-115">`DirectorySearcher` starts a new thread to perform the search.</span></span> <span data-ttu-id="d368c-116">執行緒執行的方法`ThreadProcedure`，該方法反過來調用説明器`RecurseDirectory`方法執行實際搜索並填充清單方塊。</span><span class="sxs-lookup"><span data-stu-id="d368c-116">The thread executes the `ThreadProcedure` method that in turn calls the helper `RecurseDirectory` method to do the actual search and to populate the list box.</span></span> <span data-ttu-id="d368c-117">但是，填充清單方塊需要交叉執行緒調用，如以下兩個專案中所述。</span><span class="sxs-lookup"><span data-stu-id="d368c-117">However, populating the list box requires a cross-thread call, as explained in the next two bulleted items.</span></span>

- <span data-ttu-id="d368c-118">`DirectorySearcher`定義將`AddFiles`檔添加到清單方塊的方法;但是，`RecurseDirectory`不能直接調用`AddFiles`，`AddFiles`因為只能在創建的`DirectorySearcher`STA 執行緒中執行。</span><span class="sxs-lookup"><span data-stu-id="d368c-118">`DirectorySearcher` defines the `AddFiles` method to add files to a list box; however, `RecurseDirectory` cannot directly invoke `AddFiles` because `AddFiles` can execute only in the STA thread that created `DirectorySearcher`.</span></span>

- <span data-ttu-id="d368c-119">調用`RecurseDirectory``AddFiles`的唯一方法是通過跨執行緒調用，即通過調用<xref:System.Windows.Forms.Control.Invoke%2A><xref:System.Windows.Forms.Control.BeginInvoke%2A>或封送`AddFiles`到 的`DirectorySearcher`創建執行緒。</span><span class="sxs-lookup"><span data-stu-id="d368c-119">The only way `RecurseDirectory` can call `AddFiles` is through a cross-thread call — that is, by calling <xref:System.Windows.Forms.Control.Invoke%2A> or <xref:System.Windows.Forms.Control.BeginInvoke%2A> to marshal `AddFiles` to the creation thread of `DirectorySearcher`.</span></span> <span data-ttu-id="d368c-120">`RecurseDirectory`使用<xref:System.Windows.Forms.Control.BeginInvoke%2A>以便可以非同步進行調用。</span><span class="sxs-lookup"><span data-stu-id="d368c-120">`RecurseDirectory` uses <xref:System.Windows.Forms.Control.BeginInvoke%2A> so that the call can be made asynchronously.</span></span>

- <span data-ttu-id="d368c-121">封送方法需要等效于函數指標或回檔。</span><span class="sxs-lookup"><span data-stu-id="d368c-121">Marshaling a method requires the equivalent of a function pointer or callback.</span></span> <span data-ttu-id="d368c-122">這是使用 .NET 框架中的委託實現的。</span><span class="sxs-lookup"><span data-stu-id="d368c-122">This is accomplished using delegates in the .NET Framework.</span></span> <span data-ttu-id="d368c-123"><xref:System.Windows.Forms.Control.BeginInvoke%2A>將委託視為參數。</span><span class="sxs-lookup"><span data-stu-id="d368c-123"><xref:System.Windows.Forms.Control.BeginInvoke%2A> takes a delegate as an argument.</span></span> <span data-ttu-id="d368c-124">`DirectorySearcher`因此定義`FileListDelegate`委託 （ ），`AddFiles`綁定到其建構函式中的`FileListDelegate`實例，並將此委託實例傳遞給<xref:System.Windows.Forms.Control.BeginInvoke%2A>。</span><span class="sxs-lookup"><span data-stu-id="d368c-124">`DirectorySearcher` therefore defines a delegate (`FileListDelegate`), binds `AddFiles` to an instance of `FileListDelegate` in its constructor, and passes this delegate instance to <xref:System.Windows.Forms.Control.BeginInvoke%2A>.</span></span> <span data-ttu-id="d368c-125">`DirectorySearcher`還定義在搜索完成時封送的事件委託。</span><span class="sxs-lookup"><span data-stu-id="d368c-125">`DirectorySearcher` also defines an event delegate that is marshaled when the search is completed.</span></span>
...
## <a name="using-the-multithreaded-control-on-a-form"></a><span data-ttu-id="d368c-126">在表單上使用多執行緒控制項</span><span class="sxs-lookup"><span data-stu-id="d368c-126">Using the Multithreaded Control on a Form</span></span>
 <span data-ttu-id="d368c-127">下面的示例演示如何在表單上使用多執行緒`DirectorySearcher`控制項。</span><span class="sxs-lookup"><span data-stu-id="d368c-127">The following example shows how the multithreaded `DirectorySearcher` control can be used on a form.</span></span>
...
## <a name="see-also"></a><span data-ttu-id="d368c-128">另請參閱</span><span class="sxs-lookup"><span data-stu-id="d368c-128">See also</span></span>

- <xref:System.ComponentModel.BackgroundWorker>
- [<span data-ttu-id="d368c-129">使用 .NET Framework 開發自訂的 Windows Forms 控制項</span><span class="sxs-lookup"><span data-stu-id="d368c-129">Developing Custom Windows Forms Controls with the .NET Framework</span></span>](developing-custom-windows-forms-controls.md)
- [<span data-ttu-id="d368c-130">事件架構非同步模式概觀</span><span class="sxs-lookup"><span data-stu-id="d368c-130">Event-based Asynchronous Pattern Overview</span></span>](../../../standard/asynchronous-programming-patterns/event-based-asynchronous-pattern-overview.md)
