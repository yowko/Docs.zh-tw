...
## <a name="working-with-the-label-control"></a>使用標籤控制項  
 因為<xref:System.Windows.Forms.Label>控制項無法接收焦點，它也可用來建立其他控制項的便捷鍵。 便捷鍵可讓使用者選取另一個控制項，藉由按下 ALT 鍵以存取金鑰。 如需詳細資訊，請參閱 <<c0> [ 建立存取金鑰的 Windows Forms 控制項](how-to-create-access-keys-for-windows-forms-controls.md)和[How to:使用 Windows Forms Label 控制項建立便捷鍵](how-to-create-access-keys-with-windows-forms-label-controls.md)。  
