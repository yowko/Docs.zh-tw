---
title: "如何：使用 Windows Form BindingNavigator 控制項巡覽資料 | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
...
dev_langs: 
  - "jsharp"
helpviewer_keywords: 
  - "BindingNavigator 控制項 [Windows Form], 巡覽資料"
  - "資料 [Windows Form], 巡覽"
  - "資料巡覽"
...
在 Windows Form 中 <xref:System.Windows.Forms.BindingNavigator> 控制項的問世，可讓開發人員在他們建立的表單上提供使用者簡單資料巡覽和管理使用者介面。  
 ...
 <xref:System.Windows.Forms.BindingNavigator> 控制項是一種 <xref:System.Windows.Forms.ToolStrip> 控制項，具有預先設定的按鈕，用來巡覽資料集中的第一筆、最後一筆、下一筆和上一筆記錄，以及新增和刪除記錄。  新增按鈕到 <xref:System.Windows.Forms.BindingNavigator> 控制項很容易，因為它是 <xref:System.Windows.Forms.ToolStrip> 控制項。  另請參閱[如何：將載入、儲存和取消按鈕加入 Windows Form BindingNavigator 控制項](http://msdn.microsoft.com/library/safa4957\(v=vs.110\))。  
...
2.  繫結 `bindingSource1`  至資料，並繫結文字方塊控制項至 `bindingSource1`。  若要執行此工作，請將下列程式碼貼到您的表單並從表單的建構函式或 <xref:System.Windows.Forms.Form.Load> 事件處理方法呼叫 `LoadData`。  
