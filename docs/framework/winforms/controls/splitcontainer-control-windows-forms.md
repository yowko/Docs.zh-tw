---
title: "SplitContainer 控制項 (Windows Form) | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
...
> [!NOTE]
>  在 \[工具箱\] 中，這個控制項取代了舊版 Visual Studio 中的 <xref:System.Windows.Forms.Splitter> 控制項。  `SplitContainer` 控制項會比 <xref:System.Windows.Forms.Splitter> 控制項合用。  .NET Framework 中仍會包含 <xref:System.Windows.Forms.Splitter> 類別，以與現有的應用程式相容，但強烈建議您在新專案中使用 `SplitContainer` 控制項。  
...
## 在本節中  
 ...
 另請參閱[如何：使用設計工具水平分隔視窗](http://msdn.microsoft.com/library/ms233667\(v=vs.110\))、[如何：在 Windows Form 建立 Windows 檔案總管樣式的介面](http://msdn.microsoft.com/library/zh2fe5a5\(v=vs.110\))、[如何：使用設計工具，以 Windows Form 建立多窗格使用者介面](http://msdn.microsoft.com/library/ms233661\(v=vs.110\))。  
