...
Windows Form<xref:System.Windows.Forms.MonthCalendar>控制項提供直覺式圖形介面，以便讓使用者檢視並設定日期資訊。 控制項顯示的行事曆： 方格，其中包含月、 星期、 天，其中反白顯示的日期選取範圍下方的資料行中排列已編號的天數。 您可以在月份標題的任一邊的箭號按鈕，即可選取不同的月份。 不同於類似<xref:System.Windows.Forms.DateTimePicker>控制項，您可以選取多個與這個控制項的日期。 如需詳細資訊<xref:System.Windows.Forms.DateTimePicker>控制項，請參閱[DateTimePicker 控制項](../../../../docs/framework/winforms/controls/datetimepicker-control-windows-forms.md)。  
