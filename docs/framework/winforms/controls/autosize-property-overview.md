...
> [!NOTE]
> 若要充分發揮產能，Windows Form 設計工具，在 Visual Studio shadows<xref:System.Windows.Forms.Control.AutoSize%2A>屬性<xref:System.Windows.Forms.Form>類別。 在設計階段表單行為看<xref:System.Windows.Forms.Control.AutoSize%2A>屬性設定為`false`，而不論其實際的設定。 在執行階段，進行任何特殊的住宿，和<xref:System.Windows.Forms.Control.AutoSize%2A>屬性會套用所指定屬性的設定值。
