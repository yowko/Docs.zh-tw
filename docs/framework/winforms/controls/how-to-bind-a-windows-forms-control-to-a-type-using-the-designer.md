...
---
# <a name="how-to-bind-a-windows-forms-control-to-a-type-using-the-designer"></a>作法：使用設計工具將 Windows Forms 控制項繫結至類型

當您在建立與資料互動的控制項時，有時需要將控制項繫結至類型，而非物件。 您在設計階段通常需要將控制項繫結至類型，當時資料可能無法使用，但您仍希望資料繫結的控制項顯示類型公用介面中的資料。 下列程式示範如何建立系結至類型<xref:System.Windows.Forms.BindingSource>的新, 以及如何將其中一個類型的屬性系結<xref:System.Windows.Forms.TextBox.Text%2A>至的屬性<xref:System.Windows.Forms.TextBox>。

### <a name="to-bind-the-bindingsource-to-a-type"></a>將 BindingSource 繫結至類型

1. 建立 Windows Forms 專案 ([  > 檔案] [**新增** > **專案** > ] [**視覺效果C#**  ] 或 [ **Visual Basic**  > **傳統桌面** > ] **Windows Forms 應用程式**)。
...
4. 在 [ **DataSource UI 類型編輯器**] 中, 展開先前加入<xref:System.Windows.Forms.BindingSource>的節點, 然後選取您想要系結至<xref:System.Windows.Forms.TextBox.Text%2A>之<xref:System.Windows.Forms.TextBox>屬性的系結型別屬性。
