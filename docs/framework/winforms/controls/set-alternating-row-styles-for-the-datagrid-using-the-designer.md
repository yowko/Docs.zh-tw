...
2. 在 [**屬性**] 視窗中，按一下 [<xref:System.Windows.Forms.DataGridView.AlternatingRowsDefaultCellStyle%2A>] 屬性旁邊的省略號按鈕（![Visual Studio.](./media/visual-studio-ellipsis-button.png)）屬性視窗中的省略號按鈕（...）。

3. 在 [ **CellStyle**產生器] 對話方塊中，藉由設定屬性來定義樣式，並使用 [**預覽**] 窗格來確認您的選擇。 您指定的樣式會用於控制項中顯示的每個其他資料列，從第二個數據列開始。

4. 若要定義其餘資料列的樣式，請使用 <xref:System.Windows.Forms.DataGridView.RowsDefaultCellStyle%2A> 屬性來重複步驟2和3。
