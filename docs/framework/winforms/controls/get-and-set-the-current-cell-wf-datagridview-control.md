...
-   <xref:System.Windows.Forms.Button>控制項名為`getCurrentCellButton`和`setCurrentCellButton`。 在[!INCLUDE[csprcs](../../../../includes/csprcs-md.md)]，您必須將附加<xref:System.Windows.Forms.Control.Click>至相關聯的事件處理常式，範例程式碼中的每個按鈕的事件。  
