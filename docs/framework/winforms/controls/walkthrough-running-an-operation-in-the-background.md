...
如需在此範例中使用的程式碼的完整清單，請參閱[How to:在背景執行作業](how-to-run-an-operation-in-the-background.md)。

## <a name="run-an-operation-in-the-background"></a>在背景執行作業

1. 與您在 Visual Studio 中的 Windows Form 設計工具中的表單，將兩個<xref:System.Windows.Forms.Button>控制項從**工具箱**到表單，並再把`Name`和<xref:System.Windows.Forms.Control.Text%2A>根據按鈕的屬性下表。
...
15. 按下**F6**以建置方案，然後按下**Ctrl**+**F5**執行偵錯工具外部應用程式。

    > [!NOTE]
    > 如果您按下**F5**若要執行偵錯工具中的應用程式，在都會引發例外狀況`TimeConsumingOperation`方法會攔截到並顯示偵錯工具。 當您執行偵錯工具中，外部應用程式時<xref:System.ComponentModel.BackgroundWorker>處理例外狀況，並快取中<xref:System.ComponentModel.AsyncCompletedEventArgs.Error%2A>屬性<xref:System.ComponentModel.RunWorkerCompletedEventArgs>。
