...
|在編輯儲存格時，刪除插入點之前的字元。|退格鍵|  
|編輯資料格時，刪除插入點之後的字元。|DELETE|  
|移至目前資料列中的第一個資料格。|HOME|  
...
|針對目前的資料列呼叫 <xref:System.Windows.Forms.DataGrid.EndEdit%2A> 方法。|CTRL+ENTER|  
|當處於編輯模式時，請在資料格中輸入 <xref:System.DBNull.Value?displayProperty=nameWithType> 值。|CTRL+0|  
