...
---
# <a name="walkthrough-author-a-composite-control-with-c"></a>演練：使用 C 編寫複合控制項\#
...
1. 在視覺化工作室中，創建新的**Windows 表單控制項庫**專案，並將其命名為**ctlClockLib**。

     專案名稱，`ctlClockLib`，預設也會指派給根命名空間。 根命名空間是用來限定組件中的元件名稱。 例如，如果兩個組件提供元件，名為 `ctlClock`，您可以使用 `ctlClockLib.ctlClock.` 指定您的 `ctlClock` 元件

2. 在**解決方案資源管理器**中，按右鍵**UserControl1.cs，** 然後按一下 **"重命名**"。 將檔案名稱變更為 `ctlClock.cs`。 當您被問到是否要重命名對代碼元素"UserControl1"的所有引用時，請按一下"**是**"按鈕。

    > [!NOTE]
    > 預設情況下，複合控制項從系統提供的<xref:System.Windows.Forms.UserControl>類繼承。 該<xref:System.Windows.Forms.UserControl>類提供所有複合控制項所需的功能，並實現標準方法和屬性。

3. 在 [檔案]**** 功能表上按一下 [全部儲存]**** 以儲存專案。

## <a name="add-windows-controls-and-components-to-the-composite-control"></a>將 Windows 控制項和元件添加到複合控制項
...
1. 在 **"解決方案資源管理器"** 中，按右鍵**ctlClock.cs，** 然後按一下"**查看設計器**"。

2. 在**工具箱**中，展開 **"常見控制項"** 節點，然後按兩下 **"標籤**"。

     名為<xref:System.Windows.Forms.Label>的`label1`控制項將添加到設計器表面上的控制項中。

3. 在設計器中，按一下**標籤 1**。 在 [屬性] 視窗中設定下列屬性。
...
4. 在 [工具箱]**** 中展開 [元件]**** 節點，然後再按兩下 [計時器]****。
     ...
     由於<xref:System.Windows.Forms.Timer>是元件，因此它在運行時沒有可視表示形式。 因此，它不與設計器表面上的控制項一起顯示，而是出現在**元件設計器**（設計器表面底部的託盤）中。

5. 在**元件設計器**中，**按一下計時器1，** 然後將<xref:System.Windows.Forms.Timer.Interval%2A>屬性`1000`設置為 和<xref:System.Windows.Forms.Timer.Enabled%2A>屬性。 `true`

     屬性<xref:System.Windows.Forms.Timer.Interval%2A>控制<xref:System.Windows.Forms.Timer>元件滴答的頻率。 每次 `timer1` 走動時，它會執行 `timer1_Tick` 事件中的程式碼。 間隔代表刻度之間的毫秒數。

6. 在**元件設計器**中，按兩下**計時器1**轉到 的事件`timer1_Tick``ctlClock`。
...
## <a name="add-properties-to-the-composite-control"></a>將屬性添加到複合控制項

時鐘控制項現在封裝了控制項<xref:System.Windows.Forms.Label>和<xref:System.Windows.Forms.Timer>元件，每個控制項和元件都有自己的固有屬性集。 雖然這些控制項的個別屬性無法供控制項的後續使用者存取，但是您可以建立並公開自訂屬性，方法是撰寫適當的程式碼區塊。 在下列程序中，您會將屬性新增至控制項，讓使用者變更背景與文字的色彩。

### <a name="to-add-a-property-to-your-composite-control"></a>若要將屬性新增至複合控制項

1. 在**解決方案資源管理器**中，按右鍵**ctlClock.cs，** 然後按一下"**查看代碼**"。
...
1. 按**F5**生成專案並在**UserControl 測試容器**中運行控制項。
...
## <a name="inherit-from-a-composite-control"></a>從複合控制項繼承
...
1. 在**解決方案資源管理器**中，按右鍵**ctlClockLib，** 指向**添加**，然後按一下 **"使用者控制**"。
...
5. 在**解決方案資源管理器中**，流覽當前專案。

    > [!NOTE]
    > 名為 **ctlAlarmClock.cs** 的檔案已新增至目前的專案。

### <a name="add-the-alarm-properties"></a>添加報警屬性
...
1. 在**解決方案資源管理器**中，按右鍵**ctlAlarmClock，** 然後按一下"**查看代碼**"。
...
### <a name="add-to-the-graphical-interface-of-the-control"></a>添加到控制項的圖形介面
...
1. 在**解決方案資源管理器**中，按右鍵**ctlAlarmClock，** 然後按一下 **"查看設計器**"。
...
3. 向<xref:System.Windows.Forms.Label>複合控制項添加控制項。

4. 使用滑鼠，立即在<xref:System.Windows.Forms.Label>顯示框下方拖動控制項。 在 [屬性] 視窗中設定下列屬性。
...
### <a name="add-the-alarm-functionality"></a>添加報警功能
...
1. 在 **"解決方案資源管理器"** 中，按右鍵**ctlAlarmClock.cs，** 然後按一下"**查看設計器**"。
...
### <a name="use-the-inherited-control-on-a-form"></a>在表單上使用繼承控制項

您可以測試繼承的控制項與測試基類控制項的方式相同：`ctlClock`按**F5**生成專案並在**UserControl 測試容器**中運行控制項。 如需詳細資訊，請參閱[如何：測試 UserControl 的執行階段行為](how-to-test-the-run-time-behavior-of-a-usercontrol.md)。
...
1. 在**解決方案資源管理器**中，按右鍵**ctlClockLib，** 然後按一下 **"生成**"。

2. 向解決方案添加新**的 Windows 表單應用程式**專案，並命名為 **"測試**"。

3. 在**解決方案資源管理器中**，按右鍵測試專案的**參考**節點。 按一下 [加入參考]**** 以顯示 [加入參考]**** 對話方塊。 按一下標籤為 [專案]**** 的索引標籤。 您的 `ctlClockLib` 專案會列在 [專案名稱]**** 底下。 按兩下專案以將參考新增至測試專案。

4. 在**解決方案資源管理器**中，按右鍵 **"測試**"，然後按一下"**生成**"。

5. 在 [工具箱]**** 中，展開 [ctlClockLib 元件]**** 節點。

6. 按兩下 [ctlAlarmClock]**** 以將 `ctlAlarmClock` 的複本新增至表單。

7. 在**工具箱**中，找到並按兩下**DateTimePicker**以向<xref:System.Windows.Forms.DateTimePicker>表單添加控制項，然後通過按兩下<xref:System.Windows.Forms.Label>**"標籤**"添加控制項。
...
12. 在**解決方案資源管理器**中，按右鍵 **"測試**"，然後按一下"**設置為啟動專案**"。

13. 在**調試**功能表上，按一下"**開始調試**"。

     測試程式隨即啟動。 請注意，目前時間在`ctlAlarmClock`控制項中更新，並且開始時間顯示在控制項中。 <xref:System.Windows.Forms.DateTimePicker>

14. 按一下<xref:System.Windows.Forms.DateTimePicker>顯示小時分鐘的位置。
...
本文介紹了一些關鍵概念。 您已經了解藉由將控制項和元件合併成複合控制項容器，來建立複合控制項。 您已經了解將屬性新增至您的控制項，以及撰寫程式碼來實作自訂功能。 在最後一節中，您會了解透過繼承擴充指定複合控制項的功能，並且藉由覆寫這些方法來變更主方法的功能。
