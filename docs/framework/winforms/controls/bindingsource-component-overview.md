---
title: "BindingSource 元件概觀 | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
...
## 做為媒介的 BindingSource 元件  
 <xref:System.Windows.Forms.BindingSource> 元件可做為表單上部分或全部控制項的資料來源。  在 Visual Studio 中，您可以從 \[屬性\] 視窗存取 `DataBindings` 屬性，藉此將 <xref:System.Windows.Forms.BindingSource> 繫結至控制項。  另請參閱[如何：使用設計工具將 Windows Form 控制項和 BindingSource 元件加以繫結](../../../../docs/framework/winforms/controls/bind-wf-controls-with-the-bindingsource.md)。  
 ...
 ![繫結來源和資料繫結架構](../../../../docs/framework/winforms/controls/media/net-bindsrcdatabindarch.gif "NET\_BindSrcDataBindArch")  
  
> [!NOTE]
>  在設計階段，有些動作 \(例如將資料庫資料表從資料視窗拖曳到空白表單上\) 可在一個作業之內同時完成建立 <xref:System.Windows.Forms.BindingSource> 元件、將之繫結至基礎資料來源及加入資料感知控制項。  另請參閱[將 Windows Form 控制項繫結至 Visual Studio 中的資料](../Topic/Bind%20Windows%20Forms%20controls%20to%20data%20in%20Visual%20Studio.md)。  
