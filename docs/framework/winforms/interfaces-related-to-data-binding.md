...
## <a name="consumers-of-data-binding-interfaces"></a>資料繫結介面的取用者  
 下列幾節會說明兩個群組的介面物件。 第一個群組所列出的介面會由資料來源作者實作到資料來源上。 這些介面是設計來供資料來源取用者取用，而在大部分情況下，這些取用者就是 Windows Forms 控制項或元件。 第二個群組所列出的介面則是設計來供元件作者使用。 元件作者在建立可支援資料繫結的元件以供 Windows Forms 資料繫結引擎取用時，就會使用這些介面。 您可以在與表單相關聯的類別內實作這些介面，以啟用資料繫結；下列每個案例都會提出一個類別，該類別會實作可與資料互動的介面。 Visual Studio 快速應用程式開發 (RAD) 資料設計經驗工具已充分利用這項功能。  
