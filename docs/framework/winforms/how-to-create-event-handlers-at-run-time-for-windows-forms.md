...
除了建立使用 Windows Form 設計工具在 Visual Studio 中的事件，您也可以在執行階段建立事件處理常式。 這個動作可讓您在執行階段根據程式碼中的條件來連接事件處理常式，而不需要程式一開始啟動時進行連接。

## <a name="create-an-event-handler-at-run-time"></a>在執行階段建立事件處理常式
