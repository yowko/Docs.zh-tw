...
## <a name="database-access"></a>資料庫存取  
 存取資料庫所需的權限因資料庫提供者而異；不過，只有以適當權限來執行的應用程式可以透過資料連接存取資料庫。 如需存取資料庫所需許可權的詳細資訊, 請參閱代碼啟用[安全性和 ADO.NET](../data/adonet/code-access-security.md)。  
