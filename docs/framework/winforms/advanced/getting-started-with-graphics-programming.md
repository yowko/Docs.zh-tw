---
title: 圖形程式設計入門
ms.date: 03/30/2017
helpviewer_keywords:
- graphics [Windows Forms], getting started
- getting started [Windows Forms], graphics programming
- examples [Windows Forms], graphics
- drawing
ms.assetid: eb0f6d6f-9e52-4167-9592-ff4b82fb5869
ms.openlocfilehash: f19a0e87c140a3fde8c43acc546ae11ea3b62d33
ms.sourcegitcommit: 160a88c8087b0e63606e6e35f9bd57fa5f69c168
ms.translationtype: MT
ms.contentlocale: zh-TW
ms.lasthandoff: 03/09/2019
ms.locfileid: "57720354"
---
# <a name="getting-started-with-graphics-programming"></a>圖形程式設計入門
本節說明如何開始使用[!INCLUDE[ndptecgdiplus](../../../../includes/ndptecgdiplus-md.md)]Windows Forms 應用程式中。 下列主題示範如何完成數個[!INCLUDE[ndptecgdiplus](../../../../includes/ndptecgdiplus-md.md)]工作，例如繪製和填滿圖形和文字。  
  
## <a name="in-this-section"></a>本節內容  
 [如何：建立繪圖的圖形物件](how-to-create-graphics-objects-for-drawing.md)  
 示範如何建立<xref:System.Drawing.Graphics>繪圖的物件。  
  
 [如何：建立畫筆](how-to-create-a-pen.md)  
 示範如何建立畫筆。  
  
 [如何：設定畫筆顏色](how-to-set-the-color-of-a-pen.md)  
 示範如何設定畫筆顏色。  
  
 [如何：建立實心筆刷](how-to-create-a-solid-brush.md)  
 描述如何建立實心筆刷。  
  
 [如何：在 Windows Form 上繪製線條](how-to-draw-a-line-on-a-windows-form.md)  
 示範如何將繪製一條線。  
  
 [如何：繪製外框的形狀](how-to-draw-an-outlined-shape.md)  
 描述如何繪製圖形。  
  
 [如何：Windows Form 上繪製實心的矩形](how-to-draw-a-filled-rectangle-on-a-windows-form.md)  
 說明如何繪製矩形。  
  
 [如何：Windows Form 上繪製實心的橢圓形](how-to-draw-a-filled-ellipse-on-a-windows-form.md)  
 示範如何繪製實心的橢圓形。  
  
 [如何：Windows Form 上繪製文字](how-to-draw-text-on-a-windows-form.md)  
 描述如何繪製文字。  
  
 [如何：Windows Form 上繪製垂直文字](how-to-draw-vertical-text-on-a-windows-form.md)  
 示範如何繪製垂直文字。  
  
 [如何：使用 GDI + 呈現影像](how-to-render-images-with-gdi.md)  
 示範如何將繪製影像。  
  
 [如何：建立圖形化的 Windows Form](how-to-create-a-shaped-windows-form.md)  
 說明如何變更表單的圖形。  
  
 [如何：複製像素以降低 Windows Forms 的閃動](how-to-copy-pixels-for-reducing-flicker-in-windows-forms.md)  
 說明如何以像素為單位複製到另一個區域。  
  
## <a name="reference"></a>參考資料  
 <xref:System.Drawing>  
 描述此命名空間，並連結到其所有成員。  
  
 <xref:System.Windows.Forms>  
 描述此命名空間，並連結到其所有成員。
