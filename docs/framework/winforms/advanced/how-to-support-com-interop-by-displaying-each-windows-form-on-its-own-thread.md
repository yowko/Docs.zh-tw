---
title: "如何：在自己的執行緒上顯示每個 Windows Form 以支援 COM Interop | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
...
您可以藉由在 [!INCLUDE[dnprdnshort](../../../../includes/dnprdnshort-md.md)] 訊息迴圈上顯示表單來解決 COM 互通性問題，這可使用 <xref:System.Windows.Forms.Application.Run%2A?displayProperty=fullName> 方法來建立。  
...
## 範例  
 下列程式碼範例示範如何在個別的執行緒上顯示表單，並呼叫 <xref:System.Windows.Forms.Application.Run%2A?displayProperty=fullName> 方法在該執行緒上啟動 Windows Form 訊息幫浦。 若要使用此方法，您必須使用 <xref:System.Windows.Forms.Control.Invoke%2A> 方法封送處理任何呼叫至來自 Unmanaged 應用程式的表單。  
...
-   請將 `COMForm`、`Form1` 和 `FormManager` 類型編譯至稱為 `COMWinform.dll` 的組件中。 請使用 [封裝 COM 的組件](../../../../docs/framework/interop/packaging-an-assembly-for-com.md) 所述的其中一種方法來登錄 COM Interop 組件。 您現在可以在 Unmanaged 應用程式中使用該組件和對應的類型程式庫 \(.tlb\) 檔案。 例如，您可以使用類型程式庫做為 Visual Basic 6.0 可執行檔專案中的參考。  
