...
您可以建立使用.NET Framework 訊息迴圈上顯示 Windows 表單來解決元件物件模型 (COM) 互通性問題<xref:System.Windows.Forms.Application.Run%2A?displayProperty=nameWithType>方法。  
...
## <a name="procedure"></a>程序  
 使用<xref:System.Windows.Forms.Form.ShowDialog%2A?displayProperty=nameWithType>方法可以是最簡單的方式在.NET Framework 訊息迴圈上顯示表單，因為它在所有方法中，需要最少的程式碼來實作。  
  
 <xref:System.Windows.Forms.Form.ShowDialog%2A?displayProperty=nameWithType> 方法會暫止 Unmanaged 應用程式的訊息迴圈，並將表單顯示成對話方塊。 因為主應用程式的訊息迴圈已經暫止，<xref:System.Windows.Forms.Form.ShowDialog%2A?displayProperty=nameWithType>方法會建立新的.NET Framework 訊息迴圈來處理表單的訊息。  
  
 使用 <xref:System.Windows.Forms.Form.ShowDialog%2A?displayProperty=nameWithType> 方法的缺點是表單會被開啟為強制回應對話方塊。 當 Windows Forms 開啟時，這個行為會封鎖呼叫應用程式中的任何使用者介面 (UI)。 當使用者結束表單時，.NET Framework 訊息迴圈會關閉，並且舊版的應用程式訊息迴圈會再次開始執行。  
...
- 取代所有對<xref:System.Windows.Forms.Form.Show%2A?displayProperty=nameWithType>方法呼叫<xref:System.Windows.Forms.Form.ShowDialog%2A?displayProperty=nameWithType>.NET Framework 元件中的方法。  
