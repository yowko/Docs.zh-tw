...
---
# <a name="how-to-join-lines"></a>作法：聯結線條
線條聯結是指的常見區域由其結束符合或重疊的兩行所構成。 GDI + 提供三個線條聯結樣式： 儀表、 斜面，和捨入。 線條聯結樣式是屬性<xref:System.Drawing.Pen>類別。 當您指定的線條聯結樣式<xref:System.Drawing.Pen>物件，將會聯結樣式套用到所有連接的直線，任何<xref:System.Drawing.Drawing2D.GraphicsPath>使用該畫筆繪製的物件。  
