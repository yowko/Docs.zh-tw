...
5.  在**加入新項目**對話方塊中，選取**Windows Form** (在[!INCLUDE[vbprvb](../../../../includes/vbprvb-md.md)]或[!INCLUDE[csprcs](../../../../includes/csprcs-md.md)]) 或**Windows Forms 應用程式 (.NET)** (在[!INCLUDE[vcprvc](../../../../includes/vcprvc-md.md)])從**範本**窗格。 在**名稱**方塊中，將表單命名為**Form2**。 按一下**開啟**按鈕，將表單加入至專案。  
