...
2. 在包裝函式類別上為每一個所需的應用程式設定定義屬性，並且依據設定的範圍使用 <xref:System.Configuration.ApplicationScopedSettingAttribute> 或 <xref:System.Configuration.UserScopedSettingAttribute> 來套用該屬性。 如需有關設定範圍的詳細資訊，請參閱 <<c0> [ 應用程式設定概觀](application-settings-overview.md)。 現在，您的程式碼應該如下所示：  
