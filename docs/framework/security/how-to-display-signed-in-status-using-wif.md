...
1. 在 Visual Studio 中，以滑鼠右鍵按一下方案總管  底下的 [TestService]  ，然後選取 [身分識別與存取]  。  
...
2. 您可能會注意到瀏覽器以無訊息模式將您的要求重新導向至 STS，然後開啟 Default.aspx 網頁。 如果 WIF 已正確設定，您應該會看到網站顯示下列文字： **"are signed in"** 。
