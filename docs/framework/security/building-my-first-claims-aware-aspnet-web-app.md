---
title: "建置第一個宣告感知 ASP.NET Web 應用程式 | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
ms.reviewer: ""
ms.suite: ""
...
-   ASP.NET  
 ...
 ![WIF 基本 Web 應用程式](../../../docs/framework/security/media/wifbasicwebapp.png "WIFBasicWebApp")  
...
4.  宣告感知應用程式設定為信任 STS 和它發行的權杖。  宣告感知應用程式會使用 WIF 驗證和剖析權杖。  開發人員可以使用適當的 WIF API 和類型 \(例如 **ClaimsPrincipal**\) 來滿足應用程式的需求 \(例如為它實作授權\)。  
 ...
 從 .NET 4.5 開始，WIF 就已經是 .NET Framework 套件的一部分。  在架構本身直接提供 WIF 類別可讓 .NET 平台中的宣告型識別的整合更加深入，讓您更容易使用宣告。  使用 WIF 4.5，您不必安裝任何超出範圍的元件就能開始開發宣告感知 Web 應用程式。  WIF 類別現在已廣泛存在於各種組件，主要的類別是 System.Security.Claims、System.IdentityModel 和 System.IdentityModel.Services。  
...
-   [Microsoft Azure Access Control Service \(ACS\)](http://go.microsoft.com/fwlink/?LinkID=247517) \(http:\/\/go.microsoft.com\/fwlink\/?LinkID\=247517\)  
 ...
 AD FS 2.0 是 Windows Server R2 的一部分，可以當做供內部部署案例使用的 STS；  ACS 則是當做 Microsoft Azure 平台的一部分而提供的雲端服務。  此外，基於測試或教育目的，您也可以使用其他 STS 建立專屬宣告感知應用程式。  例如，您可以使用包含在[適用於 Visual Studio 的識別和存取工具](http://go.microsoft.com/fwlink/?LinkID=245849) \(可在線上免費取得，位址是 http:\/\/go.microsoft.com\/fwlink\/?LinkID\=245849\) 中的本機開發 STS。  
...
-   [如何：使用表單型驗證建置宣告感知的 ASP.NET 應用程式](../../../docs/framework/security/claims-aware-aspnet-app-forms-authentication.md)  
