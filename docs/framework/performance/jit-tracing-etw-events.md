---
title: "JIT 追蹤 ETW 事件 | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
...
|InlinerName|win:UnicodeString|編譯器正在嘗試產生其程式碼之方法的名稱。 如果編譯器嘗試內嵌程式碼到 `MethodBeingCompiledName`，而非產生 `InlinerName` 的呼叫，則這可能與 `MethodBeingCompiledName` 不同。|  
...
|FailReason|win:UnicodeString|INLINE\_NEVER 表示先前的內嵌嘗試決定了內嵌因為某些原因永遠不會成功，否則則為自由形式文字。|  
