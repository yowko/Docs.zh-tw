...
## <a name="why-the-new-compiler-performance-applies-to-your-app"></a>新編譯器效能適用於您的應用程式的原因  
 ...
 有關羅斯林編譯器的詳細資訊，請參閱[.NET 編譯器平臺 SDK](../../csharp/roslyn-sdk/index.md)。
...
### <a name="fact-3-good-tools-make-all-the-difference"></a>事實 3：使用良好工具的成果大不相同  
 良好工具可讓您快速鑽研最大的效能問題 (CPU、記憶體或磁碟)，並協助您找出導致這些瓶頸的程式碼。 微軟提供各種性能工具，如[視覺工作室探測器](/visualstudio/profiling/beginners-guide-to-performance-profiling)和[PerfView。](https://www.microsoft.com/download/details.aspx?id=28567)
...
```  
 ...
 `WriteFormattedDocComment()` 的第一個版本已配置一個陣列、數個子字串、經修剪的子字串，以及空的 `params` 陣列。 它還檢查為"///"。 修訂過的程式碼只會使用索引，而不會進行任何配置。 它查找第一個不是空格的字元，然後按字元檢查字元以查看字串是否以"///"開頭。 新代碼使用`IndexOfFirstNonWhiteSpaceChar`而不是<xref:System.String.TrimStart%2A>返回第一個索引（在指定的開始索引之後），其中發生了非空白字元。 此修正並不完整，不過您可以查看如何套用類似的修正以取得完整的解決方法。 您可以在整個程式碼中應用此方法，來移除 `WriteFormattedDocComment()` 中的所有配置。
...
語言集成查詢 （LINQ） 與 lambda 運算式結合使用，是生產力功能的示例。 但是，隨著時間的推移，它的使用可能會對性能產生重大影響，您可能會發現需要重寫代碼。
...
下一個範例顯示當您嘗試在[非同步](../../csharp/programming-guide/concepts/async/index.md)方法中使用快取的結果時，所發生的常見問題。
 ...
 **範例 6 的修正**  
 ...
 要刪除已完成<xref:System.Threading.Tasks.Task>的分配，可以使用已完成的結果緩存任務物件：  
...
- [性能分析入門指南](/visualstudio/profiling/beginners-guide-to-performance-profiling)
- [效能](index.md)
...
- [點網/羅斯林存儲庫在 GitHub 上](https://github.com/dotnet/roslyn)
