...
在 Windows Communication Foundation （WCF）中，服務會規定用戶端的安全性需求。 也就是說，服務會指定使用哪一個安全性模式，以及用戶端是否必須提供認證。 因此，保護用戶端安全的程序便十分簡單，只要使用從服務 (如果已發行) 取得的中繼資料並建立用戶端即可。 中繼資料指定如何設定用戶端。 如果服務要求用戶端提供認證，則您必須取得符合要求的認證。 本主題將進一步探討此程序。 如需建立安全服務的詳細資訊，請參閱[保護服務](securing-services.md)。  
...
### <a name="determining-the-client-credential-type"></a>判斷用戶端認證類型  
 如果您已產生 Svcutil .exe 工具的設定檔，請檢查[\<bindings >](../configure-apps/file-schema/wcf/bindings.md)區段，以判斷需要何種用戶端認證類型。 在該部分內有指定安全性需要的繫結項目。 具體而言，請檢查每個系結的 \<security > 元素。 該項目包括 `mode` 屬性，您可以將該屬性設定為三個可能值 (`Message`、`Transport` 或 `TransportWithMessageCredential`) 的其中之一。 屬性的值決定模式，而模式決定哪一個子項目是重要的。  
  
 `<security>` 元素可以包含 `<transport>` 或 `<message>` 元素，或兩者。 重要的項目就是符合安全性模式的項目。 例如，下列程式碼指定安全性模式為 `"Message"`，而且 `<message>` 項目的用戶端認證類型是 `"Certificate"`。 在這個情況中，可以忽略 `<transport>` 項目。 但 `<message>` 項目指定必須提供 X.509 憑證。  
...
- 以程式設計方式將它編寫在您的用戶端程式碼中 (使用 `SetCertificate` 方法)。  
 ...
 藉由在用戶端的設定檔中新增[\<behaviors >](../configure-apps/file-schema/wcf/behaviors.md)區段，並使用 `clientCredentials` 元素（如下所示）。  
  
#### <a name="setting-a-clientcredentials-value-in-code"></a>在程式碼中設定 \<clientCredentials > 值  
 若要在程式碼中設定[\<clientCredentials >](../configure-apps/file-schema/wcf/clientcredentials.md)值，您必須存取 <xref:System.ServiceModel.ClientBase%601> 類別的 <xref:System.ServiceModel.ClientBase%601.ClientCredentials%2A> 屬性。 該屬性傳回允許存取各種認證類型的 <xref:System.ServiceModel.Description.ClientCredentials> 物件，如下表所示。  
  
|ClientCredential 屬性|描述|備註|  
|-------------------------------|-----------------|-----------|  
...
#### <a name="setting-a-clientcredentials-value-in-configuration"></a>在 Configuration 中設定 \<clientCredentials > 值  
 認證值的指定方式是使用端點行為做為[\<clientCredentials >](../configure-apps/file-schema/wcf/clientcredentials.md)元素的子項目。 使用的項目視用戶端認證類型而定。 例如，下列範例顯示使用 <[ \<clientCertificate >](../configure-apps/file-schema/wcf/clientcertificate-of-clientcredentials-element.md)設定 x.509 憑證的設定。  
...
```  
 ...
 若要在 configuration 中設定用戶端認證，請將[\<endpointBehaviors >](../configure-apps/file-schema/wcf/endpointbehaviors.md)元素新增至設定檔。 此外，新增的行為元素必須使用[\<client > 專案之 \<endpoint >](../configure-apps/file-schema/wcf/endpoint-of-client.md)的 `behaviorConfiguration` 屬性連結至服務的端點，如下列範例所示。 `behaviorConfiguration` 屬性的值必須與 `name` 屬性的值相符。  
