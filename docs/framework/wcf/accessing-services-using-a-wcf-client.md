...
## <a name="add-service-reference-in-visual-studio"></a>在 Visual Studio 中加入服務參考  
 服務正在執行中，以滑鼠右鍵按一下專案，將包含 WCF 用戶端 proxy，並且選取**加入服務參考**。 在**加入服務參考對話方塊**至您想要呼叫，並按一下 [服務 URL 中的型別**移**] 按鈕。 對話將會顯示您所指定之位址上的可用服務清單。 按兩下服務查看可用合約和作業中，指定產生的程式碼的命名空間，然後按一下**確定** 按鈕。  
