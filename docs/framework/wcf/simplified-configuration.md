...
> [!IMPORTANT]
>  此功能只涉及 WCF 服務組態，不是用戶端組態。  大多數時候，WCF 用戶端組態會以 svcutil.exe 之類的工具產生，或從 Visual Studio 新增服務參考。  如果您手動設定 WCF 用戶端，需要將 \<client\> 項目新增到組態中並指定您要呼叫的任何端點。  
