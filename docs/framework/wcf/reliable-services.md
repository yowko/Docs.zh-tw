...
佇列和可靠會話是執行可靠訊息的 WCF) 功能 Windows Communication Foundation (。 本主題說明 WCF 的可靠訊息功能。  
  
 *可靠的訊息處理* 是可靠的訊息來源 (呼叫 *來源*) 如何可靠地將訊息傳送到可靠的訊息目的地， (稱為 *目的地*) 。  
...
- 區隔來源和目的地。 這樣可以讓來源和目的地所發生的失敗與復原不具相關性，並且提供可靠的訊息傳輸和傳遞，即使在來源或目的地無法使用的情況下也是如此。  
 ...
 可信賴傳訊通常會伴隨長延遲時間的發生。 *延遲* 是指訊息到達來源的目的地所需的時間。 因此，WCF 提供下列類型的可靠訊息：  
...
### <a name="queues"></a>佇列  
 ...
 WCF 中的佇列提供可靠的訊息傳輸，並以高延遲的成本在來源與目的地之間分開。 WCF 佇列通訊是以訊息佇列 (MSMQ) 為基礎。  
...
- [可靠的工作階段概觀](./feature-details/reliable-sessions-overview.md)
