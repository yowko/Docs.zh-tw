...
> [!NOTE]
>  用戶端和服務在相同電腦上執行，且用戶端在系統帳戶下執行時 (也就是 `Local System` 或 `Network Service`)，以可設定狀態的安全性內容權杖建立安全的工作階段時無法模擬用戶端。 WinForms 或主控台應用程式通常在目前登入的帳戶下執行，因此根據預設值可以模擬該帳戶。 不過，用戶端是 ASP.NET 網頁，而該頁面裝載於[!INCLUDE[iis601](../../../includes/iis601-md.md)]IIS 7.0 中，則用戶端未執行或`Network Service`預設帳戶。 所有支援安全工作階段的系統提供繫結預設為使用沒有狀態的安全性內容權杖。 不過，如果用戶端是 ASP.NET 網頁，並使用以可設定狀態的安全性內容權杖的安全工作階段，就無法模擬該用戶端。 如需使用安全工作階段的可設定狀態的安全性內容權杖的詳細資訊，請參閱[How to:建立安全性內容權杖的安全工作階段](../../../docs/framework/wcf/feature-details/how-to-create-a-security-context-token-for-a-secure-session.md)。  
