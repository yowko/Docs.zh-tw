...
ms.locfileid: "79143235"
---
# <a name="specifying-client-run-time-behavior"></a>指定用戶端執行階段行為
Windows 通信基礎 （WCF） 用戶端（如 Windows 通信基礎 （WCF） 服務）可以配置為修改運行時行為以適應用戶端應用程式。 指定用戶端執行階段行為時有三個屬性可供使用。 雙工用戶端回呼物件可以使用 <xref:System.ServiceModel.CallbackBehaviorAttribute> 和 <xref:System.ServiceModel.Description.CallbackDebugBehavior> 屬性來修改其執行階段行為。 而另一個屬性 <xref:System.ServiceModel.Description.ClientViaBehavior> 則可用來區隔邏輯目的和立即網路目的。 此外，雙工用戶端回呼類型也可使用一些服務端的行為。 有關詳細資訊，請參閱[指定服務運行時行為](specifying-service-run-time-behavior.md)。  
...
- 您只能在受控制的偵錯狀況下這樣做。  
 ...
 以下代碼示例顯示了一個用戶端設定檔，該檔指示 WCF 從 SOAP 訊息中的用戶端回檔物件返回託管異常資訊。  
