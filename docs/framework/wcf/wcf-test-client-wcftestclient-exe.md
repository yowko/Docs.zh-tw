...
您通常可以在下列位置找到 WCF 測試用戶端（WcfTestClient）： `C:\Program Files (x86)\Microsoft Visual Studio\2017\Community\Common7\IDE`-社區可以是「企業」、「專業」或「社區」之一，視安裝的 Visual Studio 層級而定。
...
按一下檔案 ->**新增服務**，將服務新增至 WCF 測試用戶端。 接著會要求您輸入要新增之服務的 URI (端點位址)。 服務位址可以是 Mex 位址或 WSDL 位址。
...
您也可以在命令列中啟動 WCF 測試用戶端，使用 `/ProjectPath` 參數指定新的所需路徑來儲存產生的檔案，或使用 `/RestoreProjectPath` 參數來還原預設位置。 語法如下：
...
- **在 [檔案**] 功能表上 **，按一下 [** 結束]。 或者，在 [WCF 測試用戶端] 主視窗中，按一下 [**關閉**]。 這兩個動作也會關閉 WCF 服務自動裝載，並在 Visual Studio 啟動 WCF 測試用戶端時，停止 Visual Studio 的偵錯工具。
