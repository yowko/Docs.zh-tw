...
這個範例會示範如何存取 Windows Communication Foundation （WCF）效能計數器，以及如何建立使用者定義的效能計數器。 這個範例是以[消費者入門](getting-started-sample.md)為基礎。  
...
```  
 ...
 這項工作也可以使用設定[編輯器工具（svcconfigeditor.exe）](../configuration-editor-tool-svcconfigeditor-exe.md)來完成。  
...
1. 依序按一下 [**開始**]、[**執行 ...**]、輸入 `perfmon` 並按一下 **[確定]，** 或從 [控制台] 選取 [系統**管理工具**]，然後按兩下 [**效能**]，以啟動 [效能監視器] 工具。  
