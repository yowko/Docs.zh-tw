...
2. 在 Visual Studio 中開啟 FederationSample，並從 [**組建**] 功能表選取 [**建立方案**]。 這會建置通用專案檔、Bookstore 服務、Bookstore STS、HomeRealm STS，然後將它們部署在 IIS 中。 還會建置 Bookstore 用戶端應用程式，並將可執行檔 BookStoreClient.exe 放置在 FederationSample\BookStoreClient\bin\Debug 資料夾中。  
  
3. 按兩下 BookStoreClient.exe。 BookStoreClient 視窗隨即顯示。  
  
4. 您可以按一下 [ **流覽書籍**]，流覽書店中提供的書籍。  
  
5. 若要購買特定書籍，請選取清單中的書籍，然後按一下 [ **購買書籍**]。 應用程式隨即啟動，然後會使用 Windows 驗證向 HomeRealm 安全性權杖服務進行驗證。  
