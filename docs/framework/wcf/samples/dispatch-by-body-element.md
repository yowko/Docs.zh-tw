...
## <a name="adding-an-operation-selector-to-a-service"></a>將作業選取器新增至服務  
 ...
 服務分派作業選取器是 Windows Communication Foundation (WCF) 發送器的擴充功能。 若要選取雙工合約之回呼通道上的方法，也會使用用戶端作業選取器，其運作相當類似於此處描述的分派作業選取器，不過並沒有明確涵蓋在本範例中。  
  
 和大多數服務模型延伸項目一樣，分派作業選取器也會使用行為新增至發送器。 *行為* 是一種設定物件，可將一或多個延伸模組新增至分派執行時間 (或用戶端執行時間) 或變更其設定。  
...
## <a name="implementing-the-service"></a>實作服務  
 ...
 範例專案服務會將 `DispatchByBodyElementBehaviorAttribute` 合約行為套用至 `IDispatchedByBody` 服務合約，並將這兩個作業和作業行為標記為標籤 `OperationForBodyA()` `OperationForBodyB()` `DispatchBodyElementAttribute` 。 已開啟實作此合約之服務的服務主機時，發送器產生器 (Builder) 會如前所述挑選此中繼資料。  
  
 由於作業選取器只會根據訊息本文項目進行分派，而忽略 "Action"，因此需要告知執行階段不要在傳回的回覆上檢查 "Action" 標頭，方法是將萬用字元 "*" 指派給 `ReplyAction` 的 <xref:System.ServiceModel.OperationContractAttribute> 屬性即可。 此外，必須具有將 "Action" 屬性設定為萬用字元 "" 的預設作業 \* 。 預設作業會接收無法分派也沒有 `DispatchBodyElementAttribute` 的所有訊息：  
