...
這個範例會示範如何實作將傳入訊息指派至作業的替代演算法。  
 ...
 若要變更訊息分派至方法的方法，範例會在 `DispatchByBodyElementOperationSelector` 上實作 <xref:System.ServiceModel.Dispatcher.IDispatchOperationSelector> 擴充性介面。  這個類別會根據訊息本文的第一個項目選取作業。  
...
```  
 ...
 在此範例中，作業選取器會使用 <xref:System.ServiceModel.Channels.Message.GetReaderAtBodyContents%2A>，取得傳入訊息本文的 <xref:System.Xml.XmlDictionaryReader>。  這個方法已經將讀取器放置在訊息本文的第一個子系上，因此便可取得目前項目的名稱和命名空間 URI，並將這些項目結合至 `XmlQualifiedName`，然後使用結合項目查閱作業選取器持有之字典的對應作業。  
...
## 將作業選取器新增至服務  
 ...
 和大多數服務模型延伸項目一樣，分派作業選取器也會使用行為新增至發送器。  「*行為*」\(Behavior\) 是組態物件，可將一或多個延伸項目新增至分派執行階段 \(或新增至用戶端執行階段\)，或者變更其設定。  
...
```  
 ...
 首先，<xref:System.ServiceModel.Description.IContractBehavior.ApplyDispatchBehavior%2A> 實作會藉由反覆查看服務端點之 <xref:System.ServiceModel.Description.ContractDescription> 的 <xref:System.ServiceModel.Description.OperationDescription> 項目，而設定作業選取器的查閱字典。  接著，檢查每個作業描述以確定是否有 `DispatchBodyElementAttribute` 行為，在這個範例中也會定義 <xref:System.ServiceModel.Description.IOperationBehavior> 實作。  當這個類別也是行為時，則為被動類別，而且不會主動對分派執行階段提供組態變更。  所有方法會傳回呼叫者，而不會採取任何動作。  只會有作業行為，因此新分派機制需要的中繼資料 \(也就是選取其作業項目上之本文項目的限定名稱\) 可以和相對的作業產生關聯。  
...
## 實作服務  
 ...
 範例專案服務會將 `DispatchByBodyElementBehaviorAttribute` 合約行為套用至 `IDispatchedByBody` 服務合約，並以 `DispatchBodyElementAttribute` 作業行為對這兩個作業 `OperationForBodyA()` 和 `OperationForBodyB()` 標上標籤。  已開啟實作此合約之服務的服務主機時，發送器產生器 \(Builder\) 會如前所述挑選此中繼資料。  
  
 由於作業選取器只會根據訊息本文項目進行分派，而忽略 "Action"，因此需要告知執行階段不要在傳回的回覆上檢查 "Action" 標頭，方法是將萬用字元 "\*" 指派給 <xref:System.ServiceModel.OperationContractAttribute> 的 `ReplyAction` 屬性即可。  接著，需要讓預設作業的 "Action" 屬性設定為萬用字元 "\*"。  預設作業會接收無法分派也沒有 `DispatchBodyElementAttribute` 的所有訊息：  
