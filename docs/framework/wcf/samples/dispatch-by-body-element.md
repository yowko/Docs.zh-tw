...
## <a name="implementing-the-service"></a>實作服務  
 ...
 範例專案服務會將 `DispatchByBodyElementBehaviorAttribute` 合約行為套用至 `IDispatchedByBody` 服務合約，並將這兩項作業和標籤標示為 `OperationForBodyA()` `OperationForBodyB()` `DispatchBodyElementAttribute` 操作行為。 已開啟實作此合約之服務的服務主機時，發送器產生器 (Builder) 會如前所述挑選此中繼資料。  
