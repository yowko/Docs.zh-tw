...
此示例演示如何使用<xref:System.ServiceModel.Dispatcher.IErrorHandler>介面擴展對 Windows 通信基礎 （WCF） 服務中錯誤處理和錯誤報表的控制。 該示例基於"[入門"，](../../../../docs/framework/wcf/samples/getting-started-sample.md)並添加了一些附加代碼來處理錯誤。 用戶端會強制產生數個錯誤狀況。 服務則攔截這些錯誤並記錄在檔案中。  
...
```  
 ...
 c:\logs\errors.txt 檔案包含服務所記錄關於錯誤的資訊。 請注意，要使服務寫入目錄，必須確保運行服務的進程（通常是ASP.NET或網路服務）具有寫入目錄的許可權。  
