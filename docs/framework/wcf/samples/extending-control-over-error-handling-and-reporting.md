...
>  此範例的安裝程序與建置指示位於本主題的結尾。  
 ...
 <xref:System.ServiceModel.Dispatcher.IErrorHandler.HandleError%2A> 方法，`CalculatorErrorHandler`的錯誤記錄寫入 c:\logs 中 Error.txt 文字檔。 請注意，此範例會記錄錯誤，但不加以隱藏，目的是為了將該錯誤回報到用戶端。  
