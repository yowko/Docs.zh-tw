...
此示例演示如何使用[\<服務授權>](../../../../docs/framework/configure-apps/file-schema/wcf/serviceauthorization-element.md)啟用使用屬性<xref:System.Security.Permissions.PrincipalPermissionAttribute>來授權訪問服務操作。 此示例基於["入門"](../../../../docs/framework/wcf/samples/getting-started-sample.md)示例。 服務和用戶端使用[\<wsHttpBinding>](../../../../docs/framework/configure-apps/file-schema/wcf/wshttpbinding.md)配置。 `mode` `Message` `clientCredentialType`[安全\<>](../../../../docs/framework/configure-apps/file-schema/wcf/security-of-custombinding.md)的屬性已設置為 ，並已設置為`Windows`。 <xref:System.Security.Permissions.PrincipalPermissionAttribute> 會套用至每個服務方法，並且用來限制每個作業的存取。 呼叫者必須是 Windows 系統管理員才能存取每個作業。  
...
> 此範例的安裝程序與建置指示位於本主題的結尾。  
 ...
 服務設定檔使用[\<服務授權>](../../../../docs/framework/configure-apps/file-schema/wcf/serviceauthorization-element.md)來設置屬性`principalPermissionMode`：  
...
```  
 ...
 藉由實作 <xref:System.ServiceModel.Dispatcher.IErrorHandler>，即可向服務通知授權失敗。 有關實現`IErrorHandler`的資訊[，請參閱擴展對錯誤處理和報告的控制](../../../../docs/framework/wcf/samples/extending-control-over-error-handling-and-reporting.md)。  
