...
## <a name="custom-servicehostbase"></a>自訂 ServiceHostBase  
 這個範例會實作基底型別<xref:System.ServiceModel.ServiceHostBase>而不是<xref:System.ServiceModel.ServiceHost>來示範如何使用的自訂訊息處理通道堆疊頂端的圖層取代 Windows Communication Foundation (WCF) 堆疊實作。 您要覆寫虛擬方法 <xref:System.ServiceModel.ServiceHostBase.InitializeRuntime%2A> 才能建立通道接聽項和通道發送器。  
...
## <a name="creating-a-response-message"></a>建立回應訊息  
 訊息處理是以 `MyServiceManager` 類型實作。 在 `HandleRequest` 方法中，會先檢查訊息的 `Action` 標頭，以查看是否支援要求。 預先定義的 SOAP 動作"http://tempuri.org/HelloWorld/Hello」 定義來提供訊息篩選。 這是類似於 WCF 實作的服務合約概念<xref:System.ServiceModel.ServiceHost>。  
 ...
 此範例的用戶端是正常的 WCF 用戶端，也不會採用從服務的任何項目。 因此，服務特別設計來符合您對一般 WCF 的獲得<xref:System.ServiceModel.ServiceHost>實作。 所以，在用戶端上只需要一個服務合約。  
