...
## <a name="custom-servicehostbase"></a>自訂 ServiceHostBase  
 此示例實現基本類型<xref:System.ServiceModel.ServiceHostBase>，而不是<xref:System.ServiceModel.ServiceHost>演示如何將 Windows 通信基礎 （WCF） 堆疊實現替換為通道堆疊頂部的自訂消息處理層。 您要覆寫虛擬方法 <xref:System.ServiceModel.ServiceHostBase.InitializeRuntime%2A> 才能建立通道接聽項和通道發送器。  
...
## <a name="creating-a-response-message"></a>建立回應訊息  
 訊息處理是以 `MyServiceManager` 類型實作。 在 `HandleRequest` 方法中，會先檢查訊息的 `Action` 標頭，以查看是否支援要求。 定義預定義的 SOAPhttp://tempuri.org/HelloWorld/Hello操作""以提供消息篩選。 這與 WCF 實現中的服務協定概念類似<xref:System.ServiceModel.ServiceHost>。  
 ...
 此示例的用戶端是一個正常的 WCF 用戶端，不假定服務的任何內容。 因此，該服務是專門設計的，以匹配您從正常的 WCF<xref:System.ServiceModel.ServiceHost>實現中獲得的內容。 所以，在用戶端上只需要一個服務合約。  
