...
## <a name="custom-servicehostbase"></a>自訂 ServiceHostBase  
 這個範例會將基底類型 <xref:System.ServiceModel.ServiceHostBase>，而不是 <xref:System.ServiceModel.ServiceHost>，以示範如何使用通道堆疊頂端的自訂訊息處理層來取代 Windows Communication Foundation （WCF）堆疊的執行。 您要覆寫虛擬方法 <xref:System.ServiceModel.ServiceHostBase.InitializeRuntime%2A> 才能建立通道接聽項和通道發送器。  
...
## <a name="creating-a-response-message"></a>建立回應訊息  
 訊息處理是以 `MyServiceManager` 類型實作。 在 `HandleRequest` 方法中，會先檢查訊息的 `Action` 標頭，以查看是否支援要求。 定義了預先定義的"http://tempuri.org/HelloWorld/Hello"SOAP 動作, 以提供訊息篩選。 這類似于 <xref:System.ServiceModel.ServiceHost> 的 WCF 執行中的服務合約概念。  
 ...
 這個範例的用戶端是不會從服務採用任何專案的一般 WCF 用戶端。 因此，這項服務是特別設計來比對您從一般 WCF @ no__t-0 實作為取得的內容。 所以，在用戶端上只需要一個服務合約。  
...
>   
> 如果此目錄不存在，請移至[.NET Framework 4 的 Windows Communication Foundation （wcf）和 Windows Workflow Foundation （WF）範例](https://go.microsoft.com/fwlink/?LinkId=150780)，以下載所有 WINDOWS COMMUNICATION FOUNDATION （wcf）和 @no__t 1 範例。 此範例位於下列目錄。  
>   
