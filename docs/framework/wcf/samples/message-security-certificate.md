...
```  
 ...
 服務會公開一個與服務進行通訊的端點，以及一個使用組態檔 (Web.config) 定義並透過 WS-MetadataExchange 通訊協定公開服務之 WSDL 文件的端點。 端點是由位址、繫結及合約所組成。 系結是以標準[ \<的 wsHttpBinding >](../../../../docs/framework/configure-apps/file-schema/wcf/wshttpbinding.md)元素來設定, 預設為使用訊息安全性。 這個範例會將 `clientCredentialType` 屬性設定為 Certificate 以要求用戶端驗證。  
...
```  
 ...
 此行為會指定當用戶端驗證服務時所使用的服務認證。 伺服器憑證主體名稱是在[ \<serviceCredentials >](../../../../docs/framework/configure-apps/file-schema/wcf/servicecredentials.md)元素`findValue`的屬性中指定。  
