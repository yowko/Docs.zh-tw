...
```  
 ...
 為了示範已知型別功能，服務會以 `ComplexNumberWithMagnitude` 只傳回加法和減法的方式來執行。 (雖然合約會指定 `ComplexNumber`，但是因為 `KnownTypeAttribute` 屬性的關係，所以仍會允許這項行為)。 乘法和除法仍會傳回基底 `ComplexNumber` 類型。  
...
```  
 ...
 在用戶端上，服務合約和資料合約都是在來源檔案 generatedClient.cs 中定義，這是由來自服務中繼資料的[System.servicemodel 中繼資料公用程式工具（Svcutil）](../servicemodel-metadata-utility-tool-svcutil-exe.md)所產生。 因為服務資料合約中是指定 <xref:System.Runtime.Serialization.KnownTypeAttribute> 屬性，所以用戶端在使用服務時便能夠同時接收 `ComplexNumber` 和 `ComplexNumberWithMagnitude` 類別。 用戶端會偵測其是否收到 `ComplexNumberWithMagnitude`，然後產生適當的輸出：  
