...
```  
 ...
 為了示範已知型別功能, 服務會以`ComplexNumberWithMagnitude`只傳回加法和減法的方式來執行。 (雖然合約會指定 `ComplexNumber`，但是因為 `KnownTypeAttribute` 屬性的關係，所以仍會允許這項行為)。 乘法和除法仍會傳回基`ComplexNumber`底類型。  
