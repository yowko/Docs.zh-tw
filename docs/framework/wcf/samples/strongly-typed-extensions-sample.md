...
```  
 ...
 類別 `ThreadedItem` 繼承自 `SyndicationItem` ，並做 `InReplyToElement` 為強型別屬性。 這可提供方便、程式設計的方式來存取 `InReplyTo` 延伸資料。 它也會實作 `TryParseElement` 和 `WriteElementExtensions` 以便讀取和寫入其延伸資料，如下列程式碼所示。  
