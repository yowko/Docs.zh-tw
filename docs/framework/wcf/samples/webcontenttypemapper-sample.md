---
title: "WebContentTypeMapper 範例 | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
...
```  
 ...
 此型別必須覆寫 <xref:System.ServiceModel.Channels.WebContentTypeMapper.GetMessageFormatForContentType%28System.String%29> 方法。此方法必須評估 `contentType` 引數，並傳回下列其中一個值：<xref:System.ServiceModel.Channels.WebContentFormat>、<xref:System.ServiceModel.Channels.WebContentFormat>、<xref:System.ServiceModel.Channels.WebContentFormat> 或 <xref:System.ServiceModel.Channels.WebContentFormat>。傳回的 <xref:System.ServiceModel.Channels.WebContentFormat> 會延後至預設的 Web 訊息編碼器對應。在先前的範例程式碼中，`text/javascript` 內容型別會對應至 JSON，而所有其他對應都維持不變。  
...
2.  使用[建置 Windows Communication Foundation 範例](../../../../docs/framework/wcf/samples/building-the-samples.md)中描述的方式建置方案 WebContentTypeMapperSample.sln。  
