...
```  
 ...
 端點包含參考名為 "Binding1." 之繫結組態的 `bindingConfiguration` 屬性。 系結設定會藉由將[\<reliableSession >](../../../../docs/framework/configure-apps/file-schema/wcf/reliablesession.md)的 `enabled` 屬性設為 `true`來啟用可靠會話。 已排序工作階段之傳遞保證的控制方式，是將已排序的屬性設定為 `true` 或 `false`。 預設為 `true`。  
