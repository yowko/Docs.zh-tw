...
WSE 3.0 TCP 互通性傳輸範例示範如何將 TCP 雙工會話實作為自訂 Windows Communication Foundation (WCF) 傳輸。 也會示範如何使用通道層的擴充性，透過網路與現有的已部署系統相連結。 下列步驟示範如何建立這個自訂 WCF 傳輸：  
...
## <a name="creating-iduplexsessionchannel"></a>建立 IDuplexSessionChannel  
 ...
 一旦將 <xref:System.ServiceModel.Channels.Message> 編碼為位元組，就必須在網路上傳輸。 此時需要系統來定義訊息界限。 WSE 3.0 使用某個版本的 [DIME](/archive/msdn-magazine/2002/december/sending-files-attachments-and-soap-messages-via-dime) 作為其框架通訊協定。 `WriteData` 會封裝框架邏輯，以將 byte[] 包裝在一組 DIME 記錄中。  
  
 接收訊息的邏輯很類似。 主要的複雜性是處理通訊端讀取所傳回的位元組數少於所要求的數目。 若要接收訊息，`WseTcpDuplexSessionChannel` 會讀取網路上的位元組、解碼 DIME 框架，然後使用 <xref:System.ServiceModel.Channels.MessageEncoder> 將 byte[] 轉換為 <xref:System.ServiceModel.Channels.Message>。  
...
- 會話。CloseOutputSession--將輸出資料流程關閉 (半近) 。  
...
- `ClientWseTcpDuplexSessionChannel` 將邏輯新增至基底 `WseTcpDuplexSessionChannel` ，以在一段時間連接到 TCP 伺服器 `channel.Open` 。 首先會將主機名稱解析為 IP 位址，如下列程式碼所示。  
...
- `WseTcpChannelListener`衍生自 <xref:System.ServiceModel.Channels.ChannelListenerBase> \<IDuplexSessionChannel> 和覆寫于 [開始] 開啟且在 [開始] 上，關閉以控制其接聽通訊端的存留期。 在 OnOpen 中，您會建立通訊端以接聽 IP_ANY。 更進階的實作則可以建立第二個通訊端，就也可以階聽 IPv6。 這些實作也允許在主機名稱中指定 IP 位址。  
...
## <a name="the-wse-tcp-test-console"></a>WSE TCP 測試主控台  
 ...
 其中包含兩個測試：第一個測試會使用 WSE 3.0 WSDL 產生的程式碼來設定具型別的用戶端。 第二項測試會使用 WCF 做為用戶端和伺服器，方法是直接在通道 Api 上傳送訊息。  
...
1. 若要執行此範例，您必須有 [Web 服務增強功能 (wse) 3.0 （適用于 Microsoft .net](https://www.microsoft.com/download/details.aspx?id=14089) ）和 wse `TcpSyncStockService` 範例安裝。
  
> [!NOTE]
> 因為 Windows Server 2008 不支援 WSE 3.0，所以您無法 `TcpSyncStockService` 在該作業系統上安裝或執行此範例。  
  
1. 在安裝 `TcpSyncStockService` 範例之後，請執行下列步驟：  
  
    1. `TcpSyncStockService`在 Visual Studio 中開啟。  (TcpSyncStockService 範例是與 WSE 3.0 一起安裝。 它不是此範例程式碼的一部分。 )   
  
    2. 將 StockService 專案設定為啟動專案。  
  
    3. 開啟 StockService 專案中的 StockService.cs，並將 `StockService` 類別中的 [Policy] 屬性標記為註解。 這樣就會停用範例的安全性。 雖然 WCF 可以與 WSE 3.0 安全端點交互操作，但會停用安全性，讓此範例著重于自訂 TCP 傳輸。  
