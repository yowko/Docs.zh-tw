...
- 如何使用伺服器的 X.509 憑證來驗證伺服器。
 ...
 此範例也會示範如何在自訂權杖驗證程式之後，從 WCF 存取呼叫端的身分識別。
...
3. 您伺服器憑證的主體名稱必須包含電腦的完整網域名稱。 必須更新服務 App.config 檔以反映這個新憑證名稱。 只要將 `%SERVER_NAME%` 變數設定為服務執行所在之電腦的完整主機名稱，就可以使用 Setup.bat 建立憑證。 請注意，setup.bat 檔案必須從開發人員命令提示字元中執行，才能使用系統管理員許可權開啟 Visual Studio。  
