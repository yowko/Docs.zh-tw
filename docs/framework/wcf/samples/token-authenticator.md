...
2.  提供由自訂權杖驗證器傳回的授權原則。  
     ...
     這個範例自行提供名為 `UnconditionalPolicy` 的 <xref:System.IdentityModel.Policy.IAuthorizationPolicy> 實作，此實作會傳回一組在已於建構函式中傳遞給它的宣告和身分識別。  
...
3.  撰寫自訂安全性權杖管理員。  
     ...
     您可以使用 <xref:System.IdentityModel.Selectors.SecurityTokenManager> 來為特定 <xref:System.IdentityModel.Selectors.SecurityTokenRequirement> 物件 \(這會在 `CreateSecurityTokenAuthenticator` 方法中傳遞給前者\) 建立 <xref:System.IdentityModel.Selectors.SecurityTokenAuthenticator>。安全性權杖管理員也會用來建立權杖提供者與權杖序列化程式，但是這些不在本範例的討論範圍。在這個範例中，自訂安全性權杖管理員繼承自 <xref:System.ServiceModel.Security.ServiceCredentialsSecurityTokenManager> 類別，並且會覆寫 `CreateSecurityTokenAuthenticator` 方法，以便在傳遞的權杖需求表示要求使用者名稱驗證器時傳回自訂使用者名稱權杖驗證器。  
