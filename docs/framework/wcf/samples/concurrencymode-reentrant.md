...
## <a name="demonstrates"></a>示範  
 若要執行範例，請建置用戶端和伺服器專案， 然後開啟兩個命令視窗，並將目錄變更為\<範例 > \CS\Service\bin\debug 和\<範例 > \CS\Client\bin\debug 目錄。 輸入，以啟動服務`service.exe`然後叫用 Client.exe 使用當做輸入引數傳遞的滴答計數初始值。 10 次滴答計數的範例輸出如下所示。  
