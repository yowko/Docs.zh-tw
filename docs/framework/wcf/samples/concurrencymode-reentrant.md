...
## <a name="demonstrates"></a>示範  
 若要執行範例，請建置用戶端和伺服器專案， 然後開啟兩個命令視窗, 並將目錄變更\<為範例 > \CS\Service\bin\debug \<和範例 > \CS\Client\bin\debug 目錄。 然後藉由輸入`service.exe`來啟動服務, 然後使用當做輸入引數傳遞的初始值來叫用 Client .exe。 10 次滴答計數的範例輸出如下所示。  
