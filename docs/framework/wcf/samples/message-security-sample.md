...
---
# 訊息安全性範例
這個範例會示範如何實作一個使用 `basicHttpBinding` 和訊息安全性的應用程式。這個範例是以實作計算機服務的[使用者入門](../../../../docs/framework/wcf/samples/getting-started-sample.md)為基礎。  
...
```  
 ...
 繫結組態會將 [\<安全性\>](../../../../docs/framework/configure-apps/file-schema/wcf/security-of-basichttpbinding.md)的 `mode` 屬性設定為 `Message`，並且將 [\<訊息\>](../../../../docs/framework/configure-apps/file-schema/wcf/message-of-basichttpbinding.md)的 `clientCredentialType` 屬性設定為 `Certificate`，如下列範例組態所示：  
...
6.  編輯 Service.exe.config 以反映新的憑證名稱 \(在 [\<serviceCertificate\>](../../../../docs/framework/configure-apps/file-schema/wcf/servicecertificate-of-servicecredentials.md) 項目的 `findValue` 屬性中\)，這個名稱與機器的完整網域名稱相同。同時將基底位址的值變更成指定完整機器名稱，而不要指定 localhost`.`。  
  
7.  從服務目錄中將 Service.cer 檔案複製至用戶端機器上的用戶端目錄。  
  
8.  在用戶端上執行 `setup.bat client`。使用 `client` 引數執行 `setup.bat` ，就會建立名稱為 client.com 的用戶端憑證，並且會將用戶端憑證匯出為名為 Client.cer 的檔案。  
  
9. 在用戶端機器上的 Client.exe.config 檔案中，變更端點的位址值以符合服務的新位址。您可以藉由使用伺服器的完整網域名稱取代 localhost，完成這個動作。同時將 [\<defaultCertificate\>](../../../../docs/framework/configure-apps/file-schema/wcf/defaultcertificate-element.md)的 `findValue` 屬性變更為新服務憑證的名稱，也就是伺服器的完整網域名稱。  
