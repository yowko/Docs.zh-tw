...
```
 ...
 在中，會指定要用於服務驗證的認證 [\<behavior>](../../configure-apps/file-schema/wcf/behavior-of-endpointbehaviors.md) 。 伺服器憑證中包含的 `SubjectName` 值必須與 `findValue` 屬性的指定值相同，如下列範例程式碼所示。
...
2. 從開發人員命令提示字元中的範例安裝資料夾執行 Setup.bat，以 Visual Studio 以系統管理員許可權執行。 這會安裝執行範例所需的所有憑證。

    > [!NOTE]
    > 安裝批次檔是設計來從 Visual Studio 的開發人員命令提示字元執行。 它要求 path 環境變數指向安裝 SDK 的目錄。 此環境變數會在 Visual Studio 的開發人員命令提示字元內自動設定。  
