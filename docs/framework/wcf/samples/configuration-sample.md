...
> [!NOTE]
>  這個範例會在組態中實作探索。如需在程式碼中實作探索的範例，請參閱[基本](../../../../docs/framework/wcf/samples/basic-sample.md)。  
...
```  
 ...
 [!INCLUDE[crabout](../../../../includes/crabout-md.md)]範圍的詳細資訊，請參閱[探索尋找與尋找準則](../../../../docs/framework/wcf/feature-details/discovery-find-and-findcriteria.md)。  
...
## 用戶端組態  
 在用戶端的應用程式組態檔中，`dynamicEndpoint` 型別的 `standardEndpoint` 會用來使用探索，如下列組態程式碼片段所示。  
