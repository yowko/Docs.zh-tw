...
>  此範例的安裝程序與建置指示位於本主題的結尾。  
 ...
 這個範例會使用 [\<wsHttpBinding\>](../../../../docs/framework/configure-apps/file-schema/wcf/wshttpbinding.md) 公開 `ICalculator` 合約。這個繫結的組態已展開於 Web.config 檔中。  
...
```  
 ...
 在 [\<\<reliableSession\>\>](../../../../docs/framework/configure-apps/file-schema/wcf/reliablesession.md) 項目上，啟用的布林值會設定是否啟用可靠工作階段。`ordered` 值會設定是否保留訊息順序。`inactivityTimeout` 值會設定工作階段在發生錯誤前能夠閒置的時間長度。  
  
 在 [\<安全性\>](../../../../docs/framework/configure-apps/file-schema/wcf/security-of-wshttpbinding.md) 上，`mode` 值會設定應該使用的安全性模式。本範例會使用訊息安全性，因此 [\<安全性\>](../../../../docs/framework/configure-apps/file-schema/wcf/security-of-wshttpbinding.md)中會指定 [\<訊息\>](../../../../docs/framework/configure-apps/file-schema/wcf/message-of-wshttpbinding.md)。  
...
3.  若要建置方案的 C\# 或 Visual Basic .NET 版本，請遵循[建置 Windows Communication Foundation 範例](../../../../docs/framework/wcf/samples/building-the-samples.md)中的指示進行。  
