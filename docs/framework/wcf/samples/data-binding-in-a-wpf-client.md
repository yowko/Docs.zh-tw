...
這個範例示範使用 Windows Presentation Foundation (WPF) 用戶端中的資料繫結。 該示例使用 Windows 通信基礎 （WCF） 服務，該服務隨機生成相冊陣列以返回到用戶端。 每張專輯各有名稱、價格和曲目表。 曲目有名稱和時間長度。 服務返回的資訊將自動綁定到 Windows 演示基礎 （WPF） 用戶端提供的使用者介面 （UI）。  
