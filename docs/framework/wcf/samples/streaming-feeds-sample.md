...
```  
 ...
 這樣一來，項目資料流就永遠不會經過記憶體緩衝處理了。 在`yield return` `ItemGenerator.GenerateItems()`方法內的語句上設定中斷點, 並注意在服務`StreamedFeed()`傳回方法的結果之後, 第一次遇到此中斷點時, 您可以觀察這個行為。  
...
```  
 ...
 這樣一來，除非周遊 `ReadItems()` 結果的用戶端應用程式準備好要使用每個項目，否則無法從網路讀取每個項目。 您可以在`yield return` `StreamedAtom10FeedFormatter.DelayReadItems()`內的語句上設定中斷點, 並注意在呼叫`ReadFrom()`完成後第一次遇到此中斷點, 即可觀察這個行為。  
