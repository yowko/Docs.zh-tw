...
```  
 ...
 這樣一來，除非周遊 `ReadItems()` 結果的用戶端應用程式準備好要使用每個項目，否則無法從網路讀取每個項目。 您可以在 `StreamedAtom10FeedFormatter.DelayReadItems()` 內的 `yield return` 語句上設定中斷點，並注意在完成 `ReadFrom()` 的呼叫之後，第一次遇到此中斷點，即可觀察這個行為。  
