...
> [!IMPORTANT]
>  FindPrivateKey 是必須先編譯才能使用的範例。如需如何建置 FindPrivateKey 工具的相關指示，請參閱下面＜若要建置 FindPrivateKey 專案＞一節。  
