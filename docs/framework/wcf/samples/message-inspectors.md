...
```  
 ...
 每次發生結構描述驗證錯誤或警告時，驗證的 <xref:System.Xml.XmlReader> 就會呼叫 `InspectionValidationHandler` 方法。下列實作只能處理錯誤，而忽略所有警告。  
...
## 行為  
 訊息偵測器為用戶端執行階段或分派執行階段的延伸項目。將會使用「*行為*」\(Behavior\) 來設定這類延伸項目。行為就是類別，而這個類別會透過變更預設組態或新增延伸項目 \(例如訊息偵測器\)，來變更服務模型執行階段的行為。  
...
## 透過組態新增訊息偵測器  
 若要在應用程式組態檔中設定端點上的自訂行為，服務模型會需要實作器建立組態「*延伸項目*」\(Extension Element\)，而這是由衍生自 <xref:System.ServiceModel.Configuration.BehaviorExtensionElement> 的類別所表示。針對如本節所討論之下列延伸項目所示的延伸項目，此延伸項目必須新增至服務模型的組態區段。  
