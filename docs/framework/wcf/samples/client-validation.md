...
1.  在伺服器上，以系統管理員的權限執行 Visual Studio 命令提示字元中輸入`setup.bat service`。 執行`setup.bat`與`service`引數會建立具有電腦完整網域名稱的服務憑證，並將服務憑證匯出為名為 Service.cer 的檔案。  
