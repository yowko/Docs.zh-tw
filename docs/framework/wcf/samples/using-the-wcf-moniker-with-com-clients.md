...
- 端點所支援的合約型別。 這是上面所產生及註冊的型別。 因為 Visual Basic 腳本不提供強型別 COM 環境，所以必須指定合約的識別碼。 此 GUID 是取自 CalcProxy.tlb 的 `interfaceID`，您可以使用 OLE/COM 物件檢視器 (OleView.exe) 這類 COM 工作來檢視 CalcProxy.tlb。 針對強型別環境（例如 Office VBA 或 Visual Basic 6.0），加入類型程式庫的明確參考，然後宣告 proxy 物件的型別，就可以用來取代合約參數。 此動作在應用程式開發期間也會提供 IntelliSense 支援。  
...
- 合約的名稱與命名空間。 這是必要的識別，因為 WSDL 可能包含多個合約。  
    ...
    > [!NOTE]
    > 根據預設，WCF 服務會為使用的每個命名空間產生個別的 WSDL 檔案。 而這些檔案則與使用 WSDL 匯入建構相連結。 由於 Moniker 預期使用單一 WSDL 定義，服務必須如同本範例所示範使用單一命名空間，或必須以手動方式合併不同的檔案。  
...
```  
 ...
 當您執行範例時，作業回應會顯示在 Windows Script Host 訊息方塊視窗中。 這會示範使用具有 MEX 合約的標記進行 COM 呼叫，以與 WCF 服務通訊的 COM 用戶端。  
...
3. 從 Visual Studio 的開發人員命令提示字元中，開啟語言特定資料夾下的 \client\bin 資料夾。  
  
    > [!NOTE]
    > 如果您使用的是 Windows Vista、Windows Server 2008、Windows 7 或 Windows Server 2008 R2，請確定您以系統管理員許可權執行命令提示字元。  
  
4. 輸入 `tlbexp.exe client.dll /out:CalcProxy.tlb` 以將 dll 匯出至 tlb 檔案。 預期會出現「型別程式庫匯出工具警告」，但這不是很重要，因為不需要使用泛型型別。  
  
5. 輸入 `regasm.exe /tlb:CalcProxy.tlb client.dll` 以向 COM 註冊類型。 預期會出現「型別程式庫匯出工具警告」，但這不是很重要，因為不需要使用泛型型別。  
  
6. 輸入 `gacutil.exe /i client.dll` 以將元件新增至全域組件快取。  
  
#### <a name="to-run-the-sample-on-the-same-computer"></a>若要在同一部電腦上執行範例  
  
1. 測試您是否可以使用瀏覽器來存取服務，方法是輸入下列位址： `http://localhost/servicemodelsamples/service.svc` 。 確認頁面應該會顯示在回應中。  
...
8. 輸入 `tlbexp.exe client.dll /out:CalcProxy.tlb` 以將 dll 匯出至 tlb 檔案。 預期會出現「型別程式庫匯出工具警告」，但這不是很重要，因為不需要使用泛型型別。  
  
9. 輸入 `regasm.exe /tlb:CalcProxy.tlb client.dll` 以向 COM 註冊類型。 執行命令之前，請確定已將路徑設定為包含的資料夾 `regasm.exe` 。  
  
10. 輸入 `gacutil.exe /i client.dll` 以將元件新增至全域組件快取。 執行命令之前，請確定已將路徑設定為包含的資料夾 `gacutil.exe` 。  
