...
```  
 ...
 您可以使用 [服務追蹤檢視器工具 ( # A0) ](../service-trace-viewer-tool-svctraceviewer-exe.md)，來查看產生的追蹤記錄 (c:\logs\Message.log) 。 若要查看訊息內容，請選取服務追蹤檢視器工具左窗格和右窗格中的 **訊息** 。 此範例中的追蹤記錄是設定為產生至 C:\LOGS 資料夾中。 請先建立這個資料夾，然後再執行範例並給予使用者這個目錄的網路服務寫入權限。  
