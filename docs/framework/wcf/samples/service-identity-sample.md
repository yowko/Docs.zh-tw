...
1. 在 Windows XP 或 Windows Vista 中，使用 MMC 嵌入式管理單元工具，將身分識別解決方案資料夾中的身分識別 .pfx 憑證檔案匯入到 LocalMachine/My (Personal) 憑證存放區中。 這個檔案有密碼保護。 它會在匯入時要求您提供密碼。 `xyz`在 [密碼] 方塊中輸入。 如需詳細資訊，請參閱 [如何：使用 MMC 嵌入式管理單元查看憑證](../feature-details/how-to-view-certificates-with-the-mmc-snap-in.md) 主題。 完成之後，請以系統管理員許可權在 Visual Studio 開發人員命令提示字元中執行 Setup.bat，這會將此憑證複製到 CurrentUser/受信任的人存放區，以便在用戶端上使用。

2. 在 Windows Server 2003 上，以系統管理員許可權從 Visual Studio 2012 命令提示字元內的範例安裝資料夾中執行 Setup.bat。 這會安裝執行範例所需的所有憑證。
...
3. 從 \service\bin 目錄啟動 Service.exe。 確定服務指出它已就緒，並顯示按下結束服務的提示 \<Enter> 。  
...
6. 在服務上，以 `setup.bat service` 系統管理員許可權開啟 Visual Studio 的開發人員命令提示字元。 `setup.bat`使用 `service` 引數執行時，會建立具有電腦完整功能變數名稱的服務憑證，並將服務憑證匯出至名為 service .cer 的檔案。  
...
11. 在用戶端電腦上，從命令提示字元啟動 Client.exe。 如果用戶端和服務無法通訊，請參閱 [WCF 範例的疑難排解提示](/previous-versions/dotnet/netframework-3.5/ms751511(v=vs.90))。  
