...
>  此範例的安裝程序與建置指示位於本主題的結尾。  
 ...
 中所示[多個端點](../../../../docs/framework/wcf/samples/multiple-endpoints.md)範例，服務可以裝載多個端點，各有不同的位址也可能是不同的繫結。 這個範例會示範如何在相同位址裝載多個端點。 此外，這個範例還會示範服務端點所具有兩種位址的差異：`EndpointAddress` 和 `ListenUri`。  
