...
這段程式碼會建立以原始 `EndpointAddressBuilder` 做為初始值的 `EndpointAddress`。 接著它會加入新建的位址標頭中;若要呼叫`CreateAddressHeader`建立具有特定名稱、 命名空間和值的標頭。 此時的值為 "John"。 一旦標頭新增至產生器後，`ToEndpointAddress()` 方法便會將產生器 (可變的) 轉換回端點位址 (不變的)，此位址已指派回該用戶端端點的 [位址] 欄位。
