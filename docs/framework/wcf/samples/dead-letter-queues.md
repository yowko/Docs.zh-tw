...
---
# 寄不出的信件佇列
這個範例示範如何處理已傳遞失敗的訊息。  它是以[交易 MSMQ 繫結](../../../../docs/framework/wcf/samples/transacted-msmq-binding.md)範例為基礎。  這個範例會使用 `netMsmqBinding` 繫結。  這個服務是自我裝載的主控台應用程式，可讓您觀察接收佇列訊息的服務。  
...
```  
 ...
 範例中的服務程式碼即是[交易 MSMQ 繫結](../../../../docs/framework/wcf/samples/transacted-msmq-binding.md)的服務程式碼。  
...
>  寄不出的信件佇列是用戶端佇列，而且是用戶端佇列管理員本機上的佇列。  
 ...
 寄不出的信件訊息服務實作會檢查訊息傳遞失敗的原因，然後採取更正措施。  訊息失敗的原因可以擷取自兩個列舉：<xref:System.ServiceModel.Channels.MsmqMessageProperty.DeliveryFailure%2A> 和 <xref:System.ServiceModel.Channels.MsmqMessageProperty.DeliveryStatus%2A>。  您可以從 <xref:System.ServiceModel.OperationContext> 擷取 <xref:System.ServiceModel.Channels.MsmqMessageProperty>，如下列範例程式碼所示：  
...
4.  若要在單一或跨電腦組態中執行本範例，請適當變更佇列名稱，並以完整的電腦名稱取代 localhost，然後遵循[執行 Windows Communication Foundation 範例](../../../../docs/framework/wcf/samples/running-the-samples.md)中的指示。  
