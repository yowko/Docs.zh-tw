---
title: "永久性發行權杖提供者 | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
...
## 討論  
 [!INCLUDE[indigo1](../../../../includes/indigo1-md.md)] 中的權杖提供者是用來提供認證給安全性基礎結構。一般而言，權杖提供者會檢查目標並發行適當的認證，讓安全性基礎結構能夠保護訊息的安全。[!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] 發行時會隨附 [!INCLUDE[infocard](../../../../includes/infocard-md.md)] 權杖提供者。自訂權杖提供者適用於下列情況：  
...
-   如果您要建立自訂權杖。  
 ...
 這個範例示範如何建立自訂權杖提供者，以便快取安全性權杖服務 \(STS\) 發行的權杖。  
...
```  
 ...
 在 `wsFederationHttpBinding` 的 `security` 項目上，`mode` 值會設定應該使用的安全性模式。本範例中會使用訊息安全性，因此 `wsFederationHttpBinding` 的 `message` 項目是在 `security` element of `wsFederationHttpBinding` 中指定的。`wsFederationHttpBinding` 之 `message` 項目內 `wsFederationHttpBinding` 的 `issuer` 項目會指定將安全性權杖發行給用戶端之安全性權杖服務的位址與繫結，讓用戶端可以驗證至計算機服務。  
...
```  
 ...
 在 `wsFederationHttpBinding` 的 `security` 項目上，`mode` 值會設定應該使用的安全性模式。本範例中會使用訊息安全性，因此 `wsFederationHttpBinding` 的 `message` 項目是在 `security` element of `wsFederationHttpBinding` 中指定的。`wsFederationHttpBinding` 之 `message` 項目內 `wsFederationHttpBinding` 的 `issuerMetadata` 項目會指定端點的位址與身分識別，這個端點可用於擷取安全性權杖服務的中繼資料。  
...
```  
 ...
 `serviceCredentials` 項目內的 `issuedTokenAuthentication` 項目允許服務在權杖上指定條件約束，而此權杖是服務允許用戶端於驗證期間提出的權杖。這個組態會指定服務接受主體名稱為 CN\=STS 之憑證所簽署的權杖。  
...
2.  撰寫自訂安全性權杖管理員。  
     ...
     此 <xref:System.IdentityModel.Selectors.SecurityTokenManager> 會用來建立特定 <xref:System.IdentityModel.Selectors.SecurityTokenRequirement> \(以 `CreateSecurityTokenProvider` 方法傳遞至該管理員\) 的 <xref:System.IdentityModel.Selectors.SecurityTokenProvider>。安全性權杖管理員也會用來建立權杖驗證器與權杖序列化程式，但是這些不在本範例的討論範圍。在這個範例中，自訂安全性權杖管理員繼承自 <xref:System.ServiceModel.ClientCredentialsSecurityTokenManager> 類別，並且會覆寫 `CreateSecurityTokenProvider` 方法，以便在傳遞的權杖需求表示要求發行的權杖時傳回自訂權杖提供者。  
...
2.  若要建置方案，請遵循[建置 Windows Communication Foundation 範例](../../../../docs/framework/wcf/samples/building-the-samples.md)中的指示。確認已建置方案中的所有專案 \(Shared、RSTRSTR、Service、SecurityTokenService 和 Client\)。  
