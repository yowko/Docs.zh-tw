...
## <a name="step-1-creating-certificates"></a>步驟 1： 建立憑證  
 ...
 若要為您的電腦建立憑證，請以系統管理員許可權開啟 Visual Studio 的開發人員命令提示字元，然後執行每個範例中包含的 Setup.bat，這些範例會使用與 IIS 的安全通訊。 請先確定路徑有將 Makecert.exe 上一層的資料夾包含在內，再執行此批次 (Batch) 檔。 下列命令是用來建立 Setup.bat 中的憑證。  
...
6. 使用 HTTPS 位址在瀏覽器中測試對服務的存取 `https://localhost/servicemodelsamples/service.svc` 。  
