...
-   選項 3：事先啟用連接埠範圍。開啟 \[**Windows 防火牆** **控制台**\] Applet，並啟用範例所使用的連接埠 80、443、8000\-8003 和 9000。接下來的程序中有更詳細的指示。這個選項的安全性最低，因為除了範例以外，它也允許其他程式使用這些連接埠。  
...
4.  按一下 \[**例外狀況**\] 索引標籤上的 \[**新增程式**\]。  
  
5.  按一下 \[**瀏覽**\] 按鈕，選取您計劃執行的範例可執行檔。  
  
6.  重複步驟 4 和 5，直到已新增所有您計劃執行的範例可執行檔為止。  
...
2.  在 Windows 7 或 Windows Server 2008 R2 上，按照以下步驟進行。  
    ...
    5.  選取 \[**TCP**\]，然後在 \[**特定本機連接埠**\] 欄位中輸入 `8000, 8001, 8002, 8003, 9000, 80, 443`。  
  
    6.  接著，請按 \[**下一步**\]。  
  
    7.  選取 **\[允許連線\]**，然後按 **\[下一步\]**。  
  
    8.  選取 \[**網域**\] 和 \[**私用**\]，然後按 \[**下一步**\]。  
  
    9. 將此規則命名為 `WCF-WF 4.0 Samples`，然後按一下 \[**完成**\]。  
...
3.  在 [!INCLUDE[wv](../../../../includes/wv-md.md)] 或 [!INCLUDE[lserver](../../../../includes/lserver-md.md)] 上，按照以下步驟進行。  
    ...
    4.  按一下 \[**變更領域**\] 按鈕，選取 \[**只有我的網路 \(子網路\)**\] 選項，然後按一下 \[**確定**\]。  
...
> [!NOTE]
>  當您使用完範例時，請移除任何防火牆例外狀況。若要這麼做，請開啟 \[**Windows 防火牆控制台**\] Applet，並移除先前程序所新增的任何程式或連接埠項目。