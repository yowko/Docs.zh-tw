---
title: "SOAP 及 HTTP 端點 | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
...
---
# SOAP 及 HTTP 端點
這個範例示範如何使用 [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] Web 程式設計模型實作 RPC 服務，並且在 SOAP 格式和 "Plain Old XML" \(POX\) 格式中公開該服務。  如需服務之 HTTP 繫結的詳細資訊，請參閱[基本 HTTP 服務](../../../../docs/framework/wcf/samples/basic-http-service.md)。  這個範例的重點在於，使用不同繫結透過 SOAP 和 HTTP 公開相同服務的相關詳細資料。  
...
-   ~\/service.svc\/soap 端點，這個端點可讓用戶端使用 SOAP over HTTP 繫結存取服務。  
 ...
 用戶端專案會示範使用 SOAP Proxy 存取服務 \(透過 \[**加入服務參考**\] 產生\)，以及使用 <xref:System.Net.WebClient> 存取服務。  
...
3.  如果尚未開啟，請按下 CTRL\+W、S 開啟 \[**方案總管**\] 視窗。  
  
4.  以滑鼠右鍵按一下 \[**方案總管**\] 視窗中的 \[**服務**\] 專案，並將游標放在 \[**偵錯**\] 內容功能表選項上，如此便會出現 \[**開始新執行個體**\] 內容功能表。  按一下 \[**開始新執行個體**\]。  這樣會啟動裝載服務的 ASP.NET 程式開發伺服器。  
  
5.  以滑鼠右鍵按一下 \[方案總管\] 視窗中的 \[用戶端\] 專案，並將游標放在 \[**偵錯**\] 內容功能表選項上，如此便會出現 \[**開始新執行個體**\] 內容功能表。  按一下 \[**開始新執行個體**\]。  
...
10. 以滑鼠右鍵按一下 Windows 通知區域中的 ASP.NET 程式開發伺服器圖示，然後從內容功能表選取 \[**停止**\]。  
