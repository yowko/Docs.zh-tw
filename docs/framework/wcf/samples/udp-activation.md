...
這個範例是以 [Transport： UDP](transport-udp.md) 範例為基礎。 它會擴充 [Transport： UDP](transport-udp.md) 範例，以支援使用 Windows Process activation SERVICE (WAS) 的進程啟用。  
...
- `WebhostCloseAllListenerChannelInstances`  
 ...
 接聽程式介面卡已在 [<>] 區段的% SystemRoot% \System32\inetsrv\ApplicationHost.config 中註冊，如下所示 `listenerAdapters` ：  
...
### <a name="control-service"></a>控制服務  
 ...
 在此範例中，我們使用 WCF 在啟動程式和 WAS 工作者進程之間進行通訊。 常駐在啟動程式中的服務稱為「控制服務」。  
  
## <a name="protocol-handlers"></a>通訊協定處理常式  

 在接聽程式配接器呼叫 `WebhostOpenListenerChannelInstance` 之後，WAS 處理序管理員就會啟動背景工作處理序 (如果尚未啟動)。 背景工作處理序內的應用程式管理員接著會載入 UDP 處理序通訊協定處理常式 (PPH)，以及針對該 `ListenerChannelId` 所發出的要求。 中的 PPH 會輪流呼叫 `IAdphManager` 。`StartAppDomainProtocolListenerChannel` 若要啟動 UDP AppDomain 通訊協定處理常式 (ADPH) 。  
...
3. 啟動使用者介面應用程式 "WasNetActivator.exe"。 按一下 [ **設定** ] 索引標籤，勾選下列核取方塊，然後按一下 [ **安裝** ] 以安裝它們：  
  
    - UDP 接聽程式配接器  
  
    - UDP 通訊協定處理常式  
  
4. 按一下使用者介面應用程式 "WasNetActivator.exe" 的 [ **啟用** ] 索引標籤。 按一下 [ **開始** ] 按鈕以啟動接聽程式介面卡。 您現在可以準備執行程式。  
...
- Service.dll：服務程式碼。 這個檔案會複製至虛擬應用程式 ServiceModelSamples 的 \bin 目錄。 服務檔是 Service .svc，而設定檔是 Web.config。編譯之後，會將它們複製到下列位置：%Systemdrive%\inetpub\wwwroot\servicemodelsamples。  
...
2. 接著啟動啟動程式：WasNetActivator.exe。 在 [ **啟用** ] 索引標籤下，在下拉式清單中選取唯一的通訊協定 [ **UDP**]。 按一下 [ **開始** ] 按鈕以啟動啟動項。  
