...
---
# UDP 啟用
這個範例是以[傳輸：UDP](../../../../docs/framework/wcf/samples/transport-udp.md) 範例為基礎。將會擴充[傳輸：UDP](../../../../docs/framework/wcf/samples/transport-udp.md) 範例，以支援使用 Windows Process Activation Service \(WAS\) 的處理序啟動。  
...
-   `WebhostCloseAllListenerChannelInstances`  
 ...
 接聽程式配接器會在 \<`listenerAdapters`\> 區段的 %SystemRoot%\\System32\\inetsrv\\ApplicationHost.config 中註冊，如下所示：  
...
## 通訊協定處理常式  
 在接聽程式配接器呼叫 `WebhostOpenListenerChannelInstance` 之後，WAS 處理序管理員就會啟動背景工作處理序 \(如果尚未啟動\)。背景工作處理序內的應用程式管理員接著會載入 UDP 處理序通訊協定處理常式 \(PPH\)，以及針對該 `ListenerChannelId` 所發出的要求。PPH 接著會呼叫 `IAdphManager`.`StartAppDomainProtocolListenerChannel` 以啟動 UDP AppDomain 通訊協定處理常式 \(ADPH\)。  
...
2.  在 Windows Vista 上建立專案。進行編譯之後，也會在建立後階段中執行下列作業：  
...
3.  啟動使用者介面應用程式 "WasNetActivator.exe"。按一下 \[**安裝程式**\] 索引標籤，勾選下列核取方塊之後按一下 \[**安裝**\] 以安裝下列各項：  
  
    -   UDP 接聽程式配接器  
  
    -   UDP 通訊協定處理常式  
  
4.  按一下使用者介面應用程式 "WasNetActivator.exe" 的 \[**啟動**\] 索引標籤。按一下 \[**啟動**\] 按鈕以啟動接聽程式配接器。您現在可以準備執行程式。  
...
2.  接著啟動啟動程式：WasNetActivator.exe。在 \[**啟動**\] 索引標籤下，已在下拉式清單中選取唯一的通訊協定 \[**UDP**\]。按一下 \[**啟動**\] 按鈕以開始啟動程式。  
