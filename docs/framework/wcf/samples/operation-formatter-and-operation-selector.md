...
這個範例會示範如何使用 Windows Communication Foundation （WCF）擴充性點，以允許來自 WCF 所預期的不同格式的訊息資料。 根據預設，WCF 格式器會預期方法參數會包含在 `soap:body` 元素底下。 此範例會示範如何實作自訂作業格式器，而這個作業格式器會剖析 HTTP GET 查詢字串中的參數資料，然後使用該資料叫用方法。  
