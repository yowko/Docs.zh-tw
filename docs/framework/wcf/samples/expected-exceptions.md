...
此範例示範如何在使用型別用戶端時，攔截預期的例外狀況。 這個範例是以實作為計算機服務的 [消費者入門](getting-started-sample.md) 為基礎。 在這個範例中，用戶端是主控台應用程式 (.exe)，而服務則是由網際網路資訊服務 (IIS) 所裝載。  
...
> 此範例的安裝程序與建置指示位於本主題的結尾。  
 ...
 在 Windows Communication Foundation (WCF) 用戶端的通訊方法擲回的例外狀況，可能是預期或非預期的例外狀況。 未預期的例外狀況包含 `OutOfMemoryException` 之類的災難性失敗，以及像 `ArgumentNullException` 或 `InvalidOperationException` 之類的程式設計錯誤。 通常沒有任何實用的方法可處理未預期的錯誤，因此通常不應該在呼叫 WCF 用戶端通訊方法時攔截這些錯誤。  
  
 WCF 用戶端上通訊方法的預期例外狀況包括 `TimeoutException` 、 `CommunicationException` 以及的任何衍生類別 `CommunicationException` 。 這些表示通訊期間的問題，可透過中止 WCF 用戶端並回報通訊失敗，安全地處理。 因為外部因素可能導致在任何應用程式中發生這些錯誤，正確的應用程式必須捕捉這些例外狀況，並在發生時加以復原。  
