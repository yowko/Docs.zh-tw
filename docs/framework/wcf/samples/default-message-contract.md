...
預設訊息合約範例會示範一個服務，在這個服務中可以對服務作業來回傳遞自訂的使用者定義訊息。 這個範例是以將計算機介面實作為具型別服務的 [消費者入門](getting-started-sample.md) 為基礎。 此範例不會針對 [消費者入門](getting-started-sample.md)中使用的加法、減法、乘法和除法進行個別的服務作業，而是傳遞包含運算元和運算子的自訂訊息，並傳回算術計算的結果。  
...
```  
 ...
 針對用戶端產生的用戶端程式代碼是使用 [System.servicemodel Metadata Utility 工具 ( # A0) ](../servicemodel-metadata-utility-tool-svcutil-exe.md) Tool 所建立。 必要時，這個工具會自動在產生的用戶端程式碼中建立訊息合約類型。 您也可以指定 `/messageContract` 命令選項來強制產生訊息合約。  
