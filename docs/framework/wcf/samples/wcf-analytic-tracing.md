 ...
 **事件檢視器**中有一個已知的問題，可能無法解碼 ETW 事件。 您可能會看到一則錯誤訊息，指出：「 \<id> 找不到來自來源 Microsoft Windows-應用程式的事件識別碼描述。 可能是引發此事件的元件未安裝在您的本機電腦上，或安裝已損毀。 您可以在本機電腦上安裝或修復元件。」 如果您遇到這個錯誤，請 **從 [** **動作** ] 功能表中選取 [重新整理]。 接著，事件應該就會正確解碼。  
