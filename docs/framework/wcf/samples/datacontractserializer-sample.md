---
title: "DataContractSerializer 範例 | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
...
DataContractSerializer 範例會示範執行資料合約類別之一般序列化與還原序列化服務的 <xref:System.Runtime.Serialization.DataContractSerializer>。  此範例會建立 `Record` 物件，將其序列化為記憶體資料流，然後將該記憶體資料流還原序列化為另一個 `Record` 物件，以示範 <xref:System.Runtime.Serialization.DataContractSerializer> 的使用方式。  此範例會接著會序列化使用二進位寫入器的 `Record` 物件，以便示範該寫入器會如何影響序列化。  
...
```  
 ...
 此段範例程式碼會建立名為 `record1` 的 `Record` 物件，並接著顯示該物件。  
