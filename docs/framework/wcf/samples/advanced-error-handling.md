...
1.  > [!IMPORTANT]
    >  請先安裝 MSMQ，再執行此範例。  如果未安裝 MSMQ，則會在執行範例時傳回例外狀況訊息。  您可以在[安裝訊息佇列 \(MSMQ\)](http://go.microsoft.com/fwlink/?LinkId=166437) 中找到安裝 MSMQ 的指示。  
  
     使用 [!INCLUDE[vs_current_long](../../../../includes/vs-current-long-md.md)] 開啟 AdvancedErrorHandling.sln。  
  
2.  在 [!INCLUDE[vs_current_short](../../../../includes/vs-current-short-md.md)] 中，按下 **F5** 或 **CTRL\+SHIFT\+B**。  
