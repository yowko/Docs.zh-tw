...
這個範例會示範透過基本繫結來使用 SSL 傳輸安全性。 這個範例是以執行計算機服務的[消費者入門](getting-started-sample.md)為基礎。
...
範例中的程式碼與[消費者入門](getting-started-sample.md)服務相同。 組態檔設定中的端點定義與繫結定義會修改成啟用安全通訊，如下列範例組態所示。
