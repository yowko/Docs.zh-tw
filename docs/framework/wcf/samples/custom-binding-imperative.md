...
該示例演示如何編寫命令性代碼來定義和使用自訂綁定，而無需使用設定檔或生成的 Windows 通信基礎 （WCF） 用戶端。 這個範例會結合 HTTP 傳輸和可靠工作階段通道所提供的功能來建立可靠的 HTTP 架構繫結。 此示例基於實現計算機服務的[入門](../../../../docs/framework/wcf/samples/getting-started-sample.md)。  
