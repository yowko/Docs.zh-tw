...
2. 建置此方案會產生三個檔案：CustomChannelsTester.exe、TestSpec.xml 和 SampleRun.cmd。 檔 SampleRun.cmd 具有一個示例命令列，其中演示如何使用此工具來測試[傳輸：UDP](../../../../docs/framework/wcf/samples/transport-udp.md)示例。  
...
- 在命令提示字元中輸入下列命令：  
    ...
    ```  
     ...
     `/dll`如果"綁定"不是 Windows 通信基金會 （WCF） 提供的系統提供的綁定，則需要綁定。  
