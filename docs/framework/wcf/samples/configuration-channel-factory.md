...
此範例涵蓋 <xref:System.ServiceModel.Configuration.ConfigurationChannelFactory%601> 的使用方法。 允許<xref:System.ServiceModel.Configuration.ConfigurationChannelFactory%601>對 WCF 用戶端配置進行集中管理。 如果在應用程式網域載入時間之後選取或變更組態，這可能也相當實用。
...
1. 使用管理員許可權打開視覺工作室 2012。

2. 按右鍵配置通道工廠解決方案（2 個專案），然後選擇**屬性**。

3. 在 **"通用屬性**"中，選擇**啟動專案**，然後按一下**多個啟動專案**。

4. 將**服務**專案移動到清單的開頭，使用**操作"開始"，** 然後在**服務**專案之後移動**用戶端**專案，以及使用**操作"開始"，** 以便在**服務**專案之後執行**用戶端**專案。

5. 按一下 **"確定**"，然後按 F5（或 CTRL+F5）以運行示例。
