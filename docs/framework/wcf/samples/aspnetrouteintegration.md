...
ms.locfileid: "43475877"
---
# <a name="aspnetrouteintegration"></a>AspNetRouteIntegration
此範例示範如何裝載 Windows Communication Foundation (WCF) REST 服務，使用 ASP.NET 路由。 [基本資源服務](../../../../docs/framework/wcf/samples/basic-resource-service.md)範例示範此案例的自我裝載的版本，並討論服務實作，深入了解。 本主題著重在 ASP.NET 整合功能。 如需有關 ASP.NET 路由的詳細資訊，請參閱<xref:System.Web.Routing>。  
...
## <a name="sample-details"></a>範例詳細資料  
 ...
 除此之外，此範例還包含加入 <xref:System.Web.Routing.UrlRoutingModule> (以開啟 ASP.NET 路由) 的 Web.config 以及服務的組態。 特別是，設定會設定 WCF 服務，預設值<xref:System.ServiceModel.Description.WebHttpEndpoint>具有<xref:System.ServiceModel.Description.WebHttpEndpoint.HelpEnabled%2A>設為  `true`。 如此一來，WCF 基礎結構會建立自動 HTML 說明頁`http://localhost/Customers/help`會提供如何建構 HTTP 的相關資訊的範例要求服務以及如何存取服務之 HTTP 回應 – 比方說，客戶詳細資料會以 XML 和 JSON 表示。  
...
4.  從**方案總管**windows 中，以滑鼠右鍵按一下**服務**專案，然後將游標置於**偵錯**快顯功能表選項，讓**開始新的執行個體**操作功能表隨即出現，並選取**開始新執行個體**。  這樣會啟動裝載服務的 ASP.NET 程式開發伺服器。  
  
5.  從**方案總管**windows 中，以滑鼠右鍵按一下**用戶端**專案，然後將游標置於**偵錯**快顯功能表選項，讓**啟動新執行個體**操作功能表隨即出現，並選取**開始新執行個體**。  
