...
1. 設定服務機器：  
    ...
    1. 在服務機器上，建立一個名稱為 ServiceModelSamples 的虛擬目錄。 [Windows Communication Foundation 範例的一次性安裝程式](one-time-setup-procedure-for-the-wcf-samples.md)中的批次檔 setupvroot.bat 可以用來建立磁碟目錄和虛擬目錄。  
