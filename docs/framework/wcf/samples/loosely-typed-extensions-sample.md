...
```  
 ...
 這個專案的 XML 命名空間是空的命名空間 ( "" ) 而且其值是包含 "hello，world！" 字串的文位元組點。  
  
 建立由許多巢狀項目所組成之複雜項目延伸的其中一種方法，便是使用 .NET Framework API 進行序列化 (同時支援 和)，如下列範例所示。  
...
- [強型別擴充功能](strongly-typed-extensions-sample.md)
