...
ms.topic: "article"
helpviewer_keywords: 
  - "服務交易行為範例 [Windows Communication Foundation]"
...
---
# 服務交易行為
這個範例會示範如何使用用戶端協調的交易，以及設定 ServiceBehaviorAttribute 和 OperationBehaviorAttribute，以控制服務交易行為。這個範例以實作計算機服務的[使用者入門](../../../../docs/framework/wcf/samples/getting-started-sample.md)為基礎，但經過擴充，可以維護資料庫資料表中所執行作業的伺服器記錄檔，以及計算機作業的可設定狀態執行總數。對伺服器記錄資料表的持續性寫入會依用戶端協調的交易結果而定，如果用戶端交易沒有完成，Web 服務交易就會確保資料庫的更新並未經過認可。  
...
-   在 `ServiceBehaviorAttribute` 上：  
    ...
    -   `TransactionAutoCompleteOnSessionClose` 屬性會指定是否要在工作階段關閉時完成未完成的交易。設定為 `false` 時，就需要個別作業，才能將 `OperationBehaviorAttribute``TransactionAutoComplete` 屬性設定為 `true`，或明確呼叫 `SetTransactionComplete` 方法來完成交易。這個範例會示範這兩種方法。  
...
3.  若要在單一或跨電腦的組態中執行本範例，請遵循[執行 Windows Communication Foundation 範例](../../../../docs/framework/wcf/samples/running-the-samples.md)中的指示。  
 ...
 如果要跨電腦執行範例，您必須設定 Microsoft Distributed Transaction Coordinator \(MSDTC\) 以啟用網路交易流程，並使用 WsatConfig.exe 工具來啟用 [!INCLUDE[indigo1](../../../../includes/indigo1-md.md)] 交易網路支援。  
