...
> [!NOTE]
> 當使用 <xref:System.Messaging> 來建立佇列時，佇列名稱使用點 (.) 來代表本機電腦，並在其路徑中使用反斜線分隔符號。 Windows Communication Foundation （WCF）端點位址會指定一個 net.tcp：配置，使用 "localhost" 代表本機電腦，並在其路徑中使用正斜線。
