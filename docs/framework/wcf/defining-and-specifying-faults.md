...
SOAP 錯誤會將錯誤狀況資訊從服務傳送到用戶端，而在雙工案例中，則是以互通的方式從用戶端傳送到服務。 本主題討論何時及如何定義自訂錯誤內容，並指定可以傳回它們的作業。 有關服務或雙工用戶端如何發送這些故障以及用戶端或服務應用程式如何處理這些故障的詳細資訊，請參閱[發送和接收故障](sending-and-receiving-faults.md)。 有關 Windows 通信基礎 （WCF） 應用程式中的錯誤處理的概述，請參閱[在合同和服務中指定和處理錯誤](specifying-and-handling-faults-in-contracts-and-services.md)。  
...
### <a name="defining-error-conditions-that-clients-should-know-about"></a>定義用戶端應知道的錯誤狀況  
 SOAP 錯誤是公開描述的訊息，具有特定作業的錯誤資訊。 由於它們是和其他作業訊息一起以 WSDL 描述的，因此用戶端知道並預期在叫用作業時處理這類錯誤。 但是，由於 WCF 服務是在託管代碼中編寫的，因此確定託管代碼中的哪些錯誤條件要轉換為錯誤並返回到用戶端，因此您有機會將服務中的錯誤條件和錯誤與正式錯誤分開您與客戶的對話。  
...
### <a name="define-the-content-of-error-conditions"></a>定義錯誤狀況的內容  
 ...
 有關如何確保資料可序列化的詳細資訊，請參閱[在服務協定中指定資料傳輸](./feature-details/specifying-data-transfer-in-service-contracts.md)。 有關提供的序列化支援的清單，<xref:System.Runtime.Serialization.DataContractSerializer?displayProperty=nameWithType>請參閱[資料協定序列化器支援的類型](./feature-details/types-supported-by-the-data-contract-serializer.md)。  
...
### <a name="mark-operations-to-establish-the-fault-contract"></a>標示作業以建立錯誤合約  
 ...
 下一步，為了在操作實現中實現故障協定，在主題["發送和接收故障](sending-and-receiving-faults.md)"仲介紹了這一步驟。  
...
#### <a name="soap-wsdl-and-interoperability-considerations"></a>SOAP、WSDL 和互通性考量  
 ...
 當存取產生錯誤的非服務時，會有特定限制。 WCF 僅支援架構描述且與資料協定相容的詳細資訊類型的故障。 例如，如上所述，WCF 不支援在其詳細資訊類型中使用 XML 屬性的故障，也不支援詳細資訊部分中具有多個頂級元素的錯誤。  
