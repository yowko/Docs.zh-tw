...
##### <a name="message-view"></a>訊息檢視
 此檢視可讓您檢視所有訊息記錄追蹤，包括動作、 日期/時間、 處理序活動和從/，並瀏覽至相關聯的訊息記錄追蹤的詳細資料。 您可以將活動界限、 處理序/執行緒，或傳送和接收的訊息記錄追蹤，以方便瀏覽訊息流程。
 
##### <a name="graph-view"></a>圖形檢視  
 此檢視會顯示以圖表形式指定活動的追蹤資料。 圖表格式可讓您看見事件以階段的方式執行，以及當資料在多個活動之間移動時，這些活動之間的關係。  
