...
可以傳送兩種類型的 SOAP*錯誤：宣告*和*未*宣告。 已宣告的 SOAP 錯誤是其中作業具有指定自訂 SOAP 錯誤類型之 <xref:System.ServiceModel.FaultContractAttribute?displayProperty=nameWithType> 屬性的 SOAP 錯誤。 未宣告作業的合約中未指定 SOAP 錯誤。
...
如需有關開發服務或用戶端的這個步驟的詳細資訊，請參閱[定義和指定錯誤](defining-and-specifying-faults.md)。
...
如需詳細資訊，請參閱傳送[和接收錯誤](sending-and-receiving-faults.md)。

## <a name="undeclared-soap-faults-and-debugging"></a>未宣告的 SOAP 錯誤與偵錯

已宣告的 SOAP 錯誤非常適合用來建置穩固、可互通，與分散式應用程式。 但是，在某些情況下則適合透過服務 (或雙工用戶端) 來傳送未宣告的 SOAP 錯誤，此錯誤並未在該作業的 Web 服務描述語言 (WSDL) 中提及。 例如，當您開發服務時，一旦碰到未預期的情況，就可以將資訊傳回用戶端以便進行偵錯。 此外，您可以將 <xref:System.ServiceModel.ServiceBehaviorAttribute.IncludeExceptionDetailInFaults%2A?displayProperty=nameWithType> 屬性或 <xref:System.ServiceModel.Description.ServiceDebugBehavior.IncludeExceptionDetailInFaults%2A?displayProperty=nameWithType> 屬性設定為 `true`，以允許 WCF 用戶端取得內部服務作業例外狀況的相關資訊。 傳送[和接收錯誤](sending-and-receiving-faults.md)中會描述傳送個別錯誤和設定調試行為屬性。
