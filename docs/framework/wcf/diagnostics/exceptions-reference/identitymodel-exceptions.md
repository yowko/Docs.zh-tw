---
title: "IdentityModel 例外狀況 | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
...
|SAMLUnableToLoadAssertion|無法載入 \<saml:assertion\> 項目。|  
|X509FindValueMismatchMulti|特定的 X509FindType 要求 findValue 引數型別必須是兩個值的其中一個值。findValue 引數是另一個型別。|  
...
|SAMLUnableToLoadCondtions|無法載入 \<saml:conditions\> 項目。|  
...
|SAMLUnableToLoadCondtion|無法載入 \<saml:conditions\> 項目。|  
...
|SigningTokenHasNoKeys|指定的簽署權杖沒有金鑰。|  
...
|TraceCodeStreamSecurityUpgradeAccepted|已順利接受資料流安全性升級。|  
...
|IncorrectUserNameFormat|使用者名稱的格式無效。使用者名稱格式必須是 'username' 或 'domain\\\\username' 的形式。|  
...
|TraceCodeSecurityIdentityHostNameNormalizationFailure|端點位址的 HostName 部分無法正常化。|  
...
|TraceCodeSecurityServerSessionRenewalFaultSent|伺服器安全性工作階段已將金鑰更新的錯誤傳送給用戶端。|  
