...
|CannotDetermineSPNBasedOnAddress|用戶端無法根據指定之目標位址中的身分識別判斷服務主要名稱，以用於 SspiNegotiation/Kerberos。 目標位址身分識別必須是 UPN 身分識別（例如 acmedomain\\\alice）或 SPN 身分識別（例如 host/bobs-machine）。|
...
|CannotPerformS4UImpersonationOnPlatform|指定之合約的指定系結僅支援在 Windows Server 2003 和更新版本的 Windows 上模擬。 請在啟用取消的安全交談中，使用 SspiNegotiated 驗證與繫結。|
...
|DnsIdentityCheckFailedForOutgoingMessage|傳出訊息的身分識別檢查失敗。遠端端點應具有指定的網域名稱系統識別。 遠端端點已提供網域名稱系統 (DNS) 宣告。 如果這是合法的遠端端點，只要在建立通道 Proxy 時，明確指定 DNS 身分識別做為 EndpointAddress 的 Identity 屬性，即可解決此問題。|
|DuplicateIdInMessageToBeVerified|指定的識別碼在提供驗證的訊息中出現兩次。|
...
|RequiredSecurityHeaderElementNotSigned|具有指定識別碼的指定安全性標頭元素必須經過簽署。|
...
|SecuritySessionNotPending|沒有具有所指定識別碼的安全性會話正在擱置中。|
