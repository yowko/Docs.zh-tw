...
```  
 ...
 電腦的系統管理員和應用程式部署人員在使用這兩個參數時，應該特別小心謹慎。 如果啟用 PII 記錄，則會記錄安全性金鑰和 PII。 如果停用，敏感資料和應用程式特定資料仍然會記錄在訊息標頭和本文中。 如需隱私權和保護 PII 免于公開的更詳盡討論，請參閱 [使用者隱私權](/previous-versions/dotnet/articles/aa480490(v=msdn.10))。  
