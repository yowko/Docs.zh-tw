---
title: "WCF 效能計數器 | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
...
-   Off：ServiceModel\* 效能計數器會停用。  
 ...
 如果您要啟用所有 [!INCLUDE[indigo2](../../../../../includes/indigo2-md.md)] 應用程式的效能計數器，可以將組態設定放在 Machine.config 檔案中。  如需設定足夠記憶體供您電腦之效能計數器使用的詳細資訊，請參閱以下的＜**為效能計數器增加記憶體大小**＞一節。  
...
## 檢視效能資料  
 若要檢視效能計數器所擷取的資料，您可以使用隨附於 Windows 的效能監視器 \(Perfmon.exe\)。  若要啟動此工具，請至 \[**開始**\] 並按一下 \[**執行**\]，然後在對話方塊中輸入 `perfmon.exe`。  
...
-   服務計數器執行個體名稱可透過 WMI [服務](../../../../../docs/framework/wcf/diagnostics/wmi/service.md) 執行個體的 "CounterInstanceName" 屬性取得。  
  
-   端點計數器執行個體名稱可透過 WMI [端點](../../../../../docs/framework/wcf/diagnostics/wmi/endpoint.md) 執行個體的 "CounterInstanceName" 屬性取得。  
  
-   作業計數器執行個體名稱可透過 WMI [端點](../../../../../docs/framework/wcf/diagnostics/wmi/endpoint.md) 執行個體的 "GetOperationCounterInstanceName" 方法取得。  
...
-   \_TransactionBridgePerfCounters.vrg  
 ...
 如需以程式設計的方式存取計數器的詳細資訊，請參閱[效能計數器程式設計架構](http://go.microsoft.com/fwlink/?LinkId=95179)。  
