...
## 設定活動追蹤  
 [!INCLUDE[indigo2](../../../../../includes/indigo2-md.md)] 提供處理應用程式之預先定義的活動 \(請參閱[活動清單](../../../../../docs/framework/wcf/diagnostics/tracing/activity-list.md)\)。您也可以使用程式設計方式來定義活動，以便將使用者追蹤加以群組。如需詳細資訊，請參閱[發出使用者程式碼追蹤](../../../../../docs/framework/wcf/diagnostics/tracing/emitting-user-code-traces.md)。  
  
 若要在執行階段發出活動追蹤，則針對 `System.ServiceModel` 追蹤來源，或其他 [!INCLUDE[indigo2](../../../../../includes/indigo2-md.md)] 或自訂追蹤來源，請使用 `ActivityTracing` 設定，如下列組態程式碼所示。  
  
```  
<source name="System.ServiceModel" switchValue="Verbose,ActivityTracing">  
```  
  
 如需了解組態項目與所使用的屬性之詳細資訊，請參閱[設定追蹤](../../../../../docs/framework/wcf/diagnostics/tracing/configuring-tracing.md)主題。  
  
## 檢視活動  
 您可以在[服務追蹤檢視器工具 \(SvcTraceViewer.exe\)](../../../../../docs/framework/wcf/service-trace-viewer-tool-svctraceviewer-exe.md)中檢視活動與其功用。啟用 ActivityTracing 時，此工具會取得追蹤並依據活動加以排序。您也可以看見追蹤傳輸的情況。追蹤傳輸表示不同活動之間彼此的關係。您可以看到特定活動導致另一個活動啟動。例如，訊息要求啟動了安全性交換來取得安全對話權杖。  
  
### 透過服務追蹤檢視器來關聯活動  
 服務追蹤檢視器工具提供兩種活動檢視：  
  
-   「**清單**」檢視，使用活動識別碼來直接相互關聯處理序之間的追蹤。來自不同處理序 \(例如用戶端與服務\) 但具有相同活動識別碼的追蹤，會群組成相同的活動。所以，服務上所發生的錯誤，以及因此而造成的用戶端錯誤，都會顯示在工具的相同活動檢視中。  
  
-   \[**圖形**\] 檢視，將所有活動依據處理序分組。在此檢視中，具有相同活動識別碼的用戶端與服務在不同的活動中都有自己的追蹤。若要在不同的處理序當中以相同的活動識別碼來關聯活動，工具會顯示相關活動之間的訊息流程。  
  
 如需詳細資訊，以及查看服務追蹤檢視器工具的圖形檢視，請參閱[服務追蹤檢視器工具 \(SvcTraceViewer.exe\)](../../../../../docs/framework/wcf/service-trace-viewer-tool-svctraceviewer-exe.md)和[使用服務追蹤檢視器檢視相關追蹤並進行疑難排解](../../../../../docs/framework/wcf/diagnostics/tracing/using-service-trace-viewer-for-viewing-correlated-traces-and-troubleshooting.md)。  
...
## 追蹤結構描述  
 ...
 開發人員可以使用執行緒區域儲存區 \(TLS\) 上的 GUID 來設定 <xref:System.Diagnostics.CorrelationManager.ActivityId%2A> 屬性，藉此設定使用追蹤來發出的 AID。以下範例就是示範這項作業。  
...
-   停止：表示活動的結尾。「停止」追蹤會提供結束現有處理里程碑的記錄。它包含特定處理序中所指定之追蹤來源的現有活動識別碼，除非活動識別碼已傳播至各端點時，我們才會在每個端點看到一個「停止」。停止活動的範例包括結束處理中的執行緒，或是結束一開始標示為「開始」追蹤的方法。  
  
-   暫停：表示活動處理暫停。「暫停」追蹤包含現有的活動識別碼 \(預期將在稍後繼續處理\)。在「暫停」與「繼續」事件之間，不會從目前的追蹤來源使用此識別碼發出任何追蹤。範例包括呼叫外部程式庫函式，或等候 I\/O 完成連接埠等資源時暫停活動。  
  
-   繼續：表示繼續處理活動。「繼續」追蹤包含現有的活動識別碼 \(最後一次由目前追蹤來源所發出的追蹤是「暫停」追蹤\)。範例包括從呼叫外部程式庫函式返回，或是 I\/O 完成連接埠等資源表示繼續處理時返回。  
  
-   傳輸：由於某些活動是由其他活動所造成，或是與其他活動相關，因此活動之間將透過「傳輸」追蹤來產生關聯。傳輸會記錄活動之間的導向關係  
...
-   活動就是代表活動，但不一定代表物件。活動應該解讀為「它的發生時機為 ..\(已發出有意義的追蹤\)」。  
