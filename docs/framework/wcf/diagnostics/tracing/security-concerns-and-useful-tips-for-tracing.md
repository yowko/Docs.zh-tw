...
```  
 ...
 如需 PII 記錄的詳細資訊，請參閱 [Pii 安全性鎖定](../../samples/pii-security-lockdown.md) 範例。  
  
 電腦的系統管理員和應用程式部署人員在使用這兩個參數時，應該特別小心謹慎。 如果啟用 PII 記錄，則會記錄安全性金鑰和 PII。 如果停用，敏感資料和應用程式特定資料仍然會記錄在訊息標頭和本文中。 如需隱私權的詳細討論，並保護 PII 免于公開，請參閱 [使用者隱私權](/previous-versions/dotnet/articles/aa480490(v=msdn.10))。  
