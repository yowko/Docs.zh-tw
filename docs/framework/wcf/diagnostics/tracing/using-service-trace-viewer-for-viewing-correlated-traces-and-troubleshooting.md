...
## <a name="servicemodel-e2e-tracing"></a>ServiceModel E2E 追蹤  
 ...
 在用戶端上，每個物件模型呼叫都會建立一個 [!INCLUDE[indigo2](../../../../../includes/indigo2-md.md)] 活動 (例如，開啟 ChannelFactory、新增、分割等等)。每個作業呼叫會處理 「 處理動作 」 活動。  
...
4.  對於每一組已接收位元組的訊息對應，我們處理在 「 處理訊息 」 活動中，這些位元組並建立[!INCLUDE[indigo2](../../../../../includes/indigo2-md.md)]訊息物件。 在此活動中，我們可以看到因為信封不良或格式錯誤的訊息而發生的錯誤  
...
6.  跨處理序的動作，我們建立一個 「 執行使用者程式碼 」 活動來隔離使用者程式碼從發出中的項目中所發出的追蹤[!INCLUDE[indigo2](../../../../../includes/indigo2-md.md)]。 在上述範例中，「 服務會傳送新增回應 」 追蹤，就會發出 「 執行使用者程式碼 」 活動不在用戶端傳播的活動中的話。  
