...
- 跨應用程式所有元件(如操作調用、代碼異常、警告和其他重要處理事件)的進程里程碑的跟蹤。  
...
- 追蹤功能故障時出現的 Windows 錯誤事件。 請參考[事件紀錄紀錄](../../../../../docs/framework/wcf/diagnostics/event-logging/index.md)。  
 ...
 您可以透過編輯應用程式的設定檔來配置追蹤— Web 託管應用程式的 Web.config 或用於自託管應用程式的 Appname.exe.config。 下列是這類編輯的範例。 有關這些設置的詳細資訊,請參閱"配置跟蹤偵聽器以使用跟蹤"部分。  
...
|「詳細資訊」|"積極"事件:標誌著成功里程碑的事件。|將發出用戶代碼和服務的低級事件。|一般來說，您可以使用此層級來進行偵錯或應用程式最佳化。<br /><br /> - 理解的消息頭。|系統管理員<br /><br /> 應用程式開發人員<br /><br /> 產品開發人員。|  
