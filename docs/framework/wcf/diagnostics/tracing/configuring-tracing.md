...
本主題將說明如何啟用追蹤、設定追蹤來源以發出追蹤並設定追蹤層級、設定活動追蹤與傳播以支援端對端追蹤相互關聯，以及設定追蹤接聽項來存取追蹤。  
 ...
 有關生產或調試環境中的跟蹤設置建議，請參閱[跟蹤和消息日誌記錄的建議設置](../../../../../docs/framework/wcf/diagnostics/tracing/recommended-settings-for-tracing-and-message-logging.md)。  
  
> [!IMPORTANT]
> 在 Windows 8 上，您必須提高權限來執行應用程式 (以系統管理員身分執行)，您的應用程式才能產生追蹤記錄。  
  
## <a name="enabling-tracing"></a>啟用追蹤  
 Windows 通信基礎 （WCF） 輸出以下資料進行診斷跟蹤：  
  
- 所有應用程式元件的處理過程里程碑追蹤，例如作業呼叫、程式碼例外狀況、警告和其他重大處理事件。  
  
- 追蹤功能故障時出現的 Windows 錯誤事件。 請參閱[事件日誌記錄](../../../../../docs/framework/wcf/diagnostics/event-logging/index.md)。  
  
 WCF 跟蹤構建在 的<xref:System.Diagnostics>上。 若要使用追蹤，您應該透過組態檔或程式碼來定義追蹤來源。 WCF 為每個 WCF 程式集定義跟蹤源。 跟蹤`System.ServiceModel`源是最通用的 WCF 跟蹤源，並記錄處理整個 WCF 通信堆疊的里程碑，從輸入/離開傳輸到輸入/離開使用者代碼。 `System.ServiceModel.MessageLogging` 追蹤來源會記錄在系統之間來回傳送的所有訊息。  
  
 預設不會啟用追蹤。 要啟動跟蹤，必須創建跟蹤攔截器，並為配置中的選定跟蹤源設置"Off"以外的跟蹤級別;否則，WCF 不會生成任何跟蹤。 如果您沒有指定接聽項，就會自動停用追蹤。 如果已定義接聽項，但是未指定層級，則預設會將層級設定為「關閉」，這表示將不會發出任何追蹤。  
  
 如果使用 WCF 擴充點（如自訂操作調用器），則應發出您自己的跟蹤。 這是因為，如果實現擴充點，WCF 將不再在預設路徑中發出標準跟蹤。 如果您沒有透過發出追蹤來實作手動追蹤支援，可能無法看見預期的追蹤。  
  
 您可以透過編輯應用程式的組態檔 (Web 主控應用程式的 Web.config，或自我裝載應用程式的 Appname.exe.config) 來設定追蹤。 下列是這類編輯的範例。 有關這些設置的詳細資訊，請參閱"配置跟蹤攔截器以使用跟蹤"部分。  
...
> [!NOTE]
> 要在 Visual Studio 中編輯 WCF 服務專案的設定檔，請按右鍵應用程式的設定檔 — Web 託管應用程式的 Web.config，或用於**解決方案資源管理器**中自託管應用程式的 Appname.exe.config。 然後選擇 **"編輯 WCF 配置**上下文"功能表項目。 這將啟動[配置編輯器工具 （SvcConfigEditor.exe），](../../../../../docs/framework/wcf/configuration-editor-tool-svcconfigeditor-exe.md)它使您能夠使用圖形化使用者介面修改 WCF 服務的配置設置。  
  
## <a name="configuring-trace-sources-to-emit-traces"></a>設定追蹤來源來發出追蹤  
 WCF 為每個程式集定義跟蹤源。 組件中產生的追蹤是由針對該來源而定義的接聽項所存取。 下列為定義的追蹤來源：  
  
- System.ServiceModel：記錄 WCF 處理的所有階段，每當讀取配置、在傳輸中處理消息、安全處理、在使用者代碼中調度消息等時。  
...
```  
 ...
 有關創建使用者定義的跟蹤源的詳細資訊，請參閱[擴展跟蹤](../../../../../docs/framework/wcf/samples/extending-tracing.md)。  
  
## <a name="configuring-trace-listeners-to-consume-traces"></a>設定追蹤接聽項來使用追蹤  
 在運行時，WCF 向處理資料的攔截器發送跟蹤資料。 WCF 為<xref:System.Diagnostics>提供了多個預定義的攔截器，它們用於輸出的格式不同。 您也可以新增自訂接聽項型別。  
 ...
 您也可以使用程式設計方式來設定追蹤接聽項。 有關詳細資訊，請參閱[如何：創建和初始化跟蹤攔截器](../../../debug-trace-profile/how-to-create-and-initialize-trace-listeners.md)並[創建自訂跟蹤攔截器](https://docs.microsoft.com/archive/msdn-magazine/2006/april/clr-inside-out-extending-system-diagnostics)。  
...
|重大|"負"事件：指示意外處理或錯誤條件的事件。||已記錄包含下列的未處理例外狀況：<br /><br /> - 記憶體異常<br />- 執行緒中止Exception（CLR 調用任何執行緒中止例外處理常式）<br />- 堆疊溢位異常（無法捕獲）<br />- 配置錯誤例外<br />- SEH例外<br />- 應用程式啟動錯誤<br />- 故障快點事件<br />- 系統掛起<br />- 毒消息：導致應用程式失敗的消息跟蹤。|系統管理員<br /><br /> 應用程式開發人員|  
...
|資訊|"積極"事件：標誌著成功里程碑的事件|不管應用程式是否正常運作，都屬於應用程式執行的重要與成功里程碑。|一般來說，會產生對監控與診斷系統狀態、衡量效能，或描述分析有幫助的訊息。 您可以使用這類資訊來進行容量規劃與效能管理：<br /><br /> - 創建通道。<br />- 創建終結點攔截器。<br />- 消息輸入/離開傳輸。<br />- 檢索安全權杖。<br />- 讀取配置設置。|系統管理員<br /><br /> 應用程式開發人員<br /><br /> 產品開發人員。|  
|「詳細資訊」|"積極"事件：標誌著成功里程碑的事件。|已針對使用者程式碼與服務發出低層級事件。|一般來說，您可以使用此層級來進行偵錯或應用程式最佳化。<br /><br /> - 理解的消息頭。|系統管理員<br /><br /> 應用程式開發人員<br /><br /> 產品開發人員。|  
|ActivityTracing||介於處理活動與元件之間的流程事件。|這個層級可以讓系統管理員與開發人員相互關聯相同應用程式定義域中的多個應用程式：<br /><br /> - 活動邊界（如開始/停止）的跟蹤。<br />- 傳輸跟蹤。|全部|  
...
> [!NOTE]
> 在 WCF 中使用某些擴充性功能時，在啟用活動跟蹤<xref:System.NullReferenceException>時可能會獲取 。 若要修正這個問題，請檢查您應用程式的組態檔，並確定追蹤來源的 `switchValue` 屬性不是設定為 `activityTracing`。  
  
 `propagateActivity` 屬性會指出是否應該將活動傳播至其他參與訊息交換的端點。 將這個值設定為 `true` 之後，您就可以使用任意兩個端點所產生的追蹤檔，來觀察一個端點上的一組追蹤如何流動至另一個端點的一組追蹤。  
  
 有關活動跟蹤和傳播的詳細資訊，請參閱[傳播](../../../../../docs/framework/wcf/diagnostics/tracing/propagation.md)。  
  
 和`propagateActivity``ActivityTracing`布林值都應用於系統.服務模型跟蹤源。 該`ActivityTracing`值也適用于任何跟蹤源，包括 WCF 或使用者定義的源。  
