...
## 啟用訊息記錄  
 根據預設，[!INCLUDE[indigo1](../../../../includes/indigo1-md.md)] 不會記錄訊息。若要啟動訊息記錄，您必須將追蹤接聽項加入至 `System.ServiceModel.MessageLogging` 追蹤來源，並在組態檔中設定 `<messagelogging>` 項目的屬性。  
...
```  
 ...
 [!INCLUDE[crabout](../../../../includes/crabout-md.md)]訊息記錄設定的詳細資訊，請參閱[追蹤與訊息記錄的建議設定](../../../../docs/framework/wcf/diagnostics/tracing/recommended-settings-for-tracing-and-message-logging.md)。  
...
>  由於訊息記錄會存取磁碟空間，因此您應該要限制為特定服務寫入磁碟的訊息數目。當抵達訊息限制時，便會在資訊層上產生追蹤，並停止所有的訊息記錄。  
 ...
 `source` 的 `switchValue` 屬性只對追蹤有效。如果您依下面所示指定 `System.ServiceModel.MessageLogging`的 `switchValue` 屬性，該屬性並不會產生作用。  
  
```  
<source name="System.ServiceModel.MessageLogging" switchValue="Verbose">  
```  
  
 如果想要停用追蹤來源，您就應該改成使用 `messageLogging` 項目的 `logMessagesAtServiceLevel`、`logMalformedMessages` 及 `logMessagesAtTransportLevel` 屬性。您應該將所有這些屬性都設定為 `false`。使用先前程式碼範例中的組態檔、透過組態編輯器 UI 介面，或是使用 WMI，都可以做到這點。[!INCLUDE[crabout](../../../../includes/crabout-md.md)]組態編輯器工具的詳細資訊，請參閱[組態編輯器工具 \(SvcConfigEditor.exe\)](../../../../docs/framework/wcf/configuration-editor-tool-svcconfigeditor-exe.md)。[!INCLUDE[crabout](../../../../includes/crabout-md.md)] WMI 的詳細資訊，請參閱[使用 Windows Management Instrumentation 進行診斷](../../../../docs/framework/wcf/diagnostics/wmi/index.md)。  
...
## 記錄層級和選項  
 ...
 您可以設定 `messageLogging` 項目的 `logMessagesAtServiceLevel`、`logMalformedMessages` 及 `logMessagesAtTransportLevel` 屬性以控制記錄層級。  
...
### 格式錯誤層級  
 格式錯誤訊息是指 [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] 堆疊在任何處理階段所拒絕的訊息。格式錯誤訊息會依現狀加以記錄：也就是若有加密，便會以加密形式記錄，而不會包含不正確的 XML 和其他格式。`maxSizeOfMessageToLog` 定義了要以 CDATA 形式記錄之訊息的大小。根據預設，`maxSizeOfMessageToLog` 會等於 256 K。[!INCLUDE[crabout](../../../../includes/crabout-md.md)]這個屬性的詳細資訊，請參閱＜其他選項＞一節。  
...
-   要記錄訊息的最大大小 \(`maxSizeOfMessageToLog` 屬性\)：這個值會指定所要記錄訊息的最大大小，以位元組為單位。不記錄超過大小限制的訊息，而且也不針對該訊息執行任何其他活動。這個設定會影響所有的追蹤層級。如果有開啟 ServiceModel 追蹤，便會在第一個記錄點 \(ServiceModelSend\* 或 TransportReceive\) 發出警告層級追蹤以通知使用者。服務層級和傳輸層級等訊息的預設值是 256 K，而格式錯誤訊息的預設值是 4 K。  
 ...
 包含本節所描述屬性的訊息記錄選項，可以在執行階段使用 Windows Management Instrumentation \(WMI\) 來加以變更。存取 [AppDomainInfo](../../../../docs/framework/wcf/diagnostics/wmi/appdomaininfo.md) 執行個體即可做到這點，而該執行個體會公開下列布林值屬性：`LogMessagesAtServiceLevel`、`LogMessagesAtTransportLevel` 和 `LogMalformedMessages`。因此，如果您為訊息記錄設定一個追蹤接聽項，但在組態中將這些選項設定為 `false`，那麼您可以在稍後應用程式執行時，將選項變更為 `true`。這會在執行階段有效地啟用訊息記錄。同樣地，如果您在組態檔中啟用訊息記錄，則您可以在執行階段使用 WMI 來停用訊息記錄。[!INCLUDE[crdefault](../../../../includes/crdefault-md.md)][使用 Windows Management Instrumentation 進行診斷](../../../../docs/framework/wcf/diagnostics/wmi/index.md).  
  
 訊息記錄中的 `source` 欄位會指定要用哪種內容來記錄訊息：何時傳送\/接收要求訊息、針對要求\-回覆或單向要求、在服務模型或傳輸層，或是在發生格式錯誤訊息的情況下。  
  
 對於格式錯誤訊息，`source` 等於 `Malformed`。否則，便依據內容將來源指定為下列值。  
  
 針對要求\/回覆  
  
||傳送要求|接收要求|傳送回覆|接收回覆|  
|-|----------|----------|----------|----------|  
|服務模型層|服務<br /><br /> 層級<br /><br /> 請傳送<br /><br /> 要求|服務<br /><br /> 層級<br /><br /> Receive<br /><br /> 要求|服務<br /><br /> 層級<br /><br /> 請傳送<br /><br /> Reply|服務<br /><br /> 層級<br /><br /> Receive<br /><br /> Reply|  
|傳輸層|傳輸<br /><br /> 請傳送|傳輸<br /><br /> Receive|傳輸<br /><br /> 請傳送|傳輸<br /><br /> Receive|  
  
 針對單向要求  
  
||傳送要求|接收要求|  
|-|----------|----------|  
|服務模型層|服務<br /><br /> 層級<br /><br /> 請傳送<br /><br /> 資料包|服務<br /><br /> 層級<br /><br /> Receive<br /><br /> 資料包|  
|傳輸層|傳輸<br /><br /> Send|傳輸<br /><br /> Receive|  
  
## 訊息篩選條件  
 訊息篩選條件會定義於 `diagnostics` 組態區段的 `messageLogging` 組態項目中。訊息篩選條件會套用在服務和傳輸層級上。如果有定義一或多個篩選條件，則只記錄至少符合其中一個篩選條件的訊息。如果沒有定義篩選條件，所有訊息都會通過。  
