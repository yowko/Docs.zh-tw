...
## ASP.NET 相容模式預設值已變更  
 WCF 會在寫入 WCF 服務時提供 ASP.NET 相容模式，將 ASP.NET HTTP 管線功能的完整存取權限授與開發人員。若要使用這個模式，您必須在 web.config 的 [\<serviceHostingEnvironment\>](../../../docs/framework/configure-apps/file-schema/wcf/servicehostingenvironment.md) 區段中將 `aspNetCompatibilityEnabled` 屬性設定為 true。此外，這個 appDomain 中的任何服務還必須將其 <xref:System.ServiceModel.Activation.AspNetCompatibilityRequirementsAttribute> 上的 `RequirementsMode` 屬性設定為 <xref:System.ServiceModel.Activation.AspNetCompatibilityRequirementsMode> 或 <xref:System.ServiceModel.Activation.AspNetCompatibilityRequirementsMode>。根據預設，<xref:System.ServiceModel.Activation.AspNetCompatibilityRequirementsAttribute> 現在會設定為 <xref:System.ServiceModel.Activation.AspNetCompatibilityRequirementsMode>，而預設 WCF 服務應用程式範本會將 `aspNetCompatibilityEnabled` 屬性設定為 `true`。[!INCLUDE[crdefault](../../../includes/crdefault-md.md)][Windows Communication Foundation 4.5 的新功能](../../../docs/framework/wcf/whats-new.md)和[WCF 服務與 ASP.NET](../../../docs/framework/wcf/feature-details/wcf-services-and-aspnet.md)。  
...
|receiveTimeout|SMSvcHost.exe|30 秒|這個屬性會指定讀取 TCP 框架資料以及從基礎連線執行連線分派的逾時。這個屬性的用途是設定 SMSvcHost.exe 服務持續投入讀取傳入連接之前序編碼資料的時間上限。[!INCLUDE[crdefault](../../../includes/crdefault-md.md)][Configuring the Net.TCP Port Sharing Service](http://msdn.microsoft.com/zh-tw/b6dd81fa-68b7-4e1b-868e-88e5901b7ea0).|  
...
> [!IMPORTANT]
>  如需保護資料安全的詳細資訊，請參閱 [資料的安全性考量](../../../docs/framework/wcf/feature-details/security-considerations-for-data.md) 下的「安全使用 XML」。  
