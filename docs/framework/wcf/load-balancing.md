...
增加 Windows Communication Foundation （WCF）應用程式容量的其中一種方式，就是將它們部署到負載平衡的伺服器陣列來相應放大。 WCF 應用程式可以使用標準負載平衡技術來進行負載平衡，包括軟體負載平衡器，例如 Windows 網路負載平衡，以及硬體型負載平衡設備。  
  
 下列各節將討論使用各種系統提供系結建立之 WCF 應用程式的負載平衡考慮。  
  
## <a name="load-balancing-with-the-basic-http-binding"></a>使用基本 HTTP 繫結的負載平衡  
 從負載平衡的觀點來看，使用 <xref:System.ServiceModel.BasicHttpBinding> 進行通訊的 WCF 應用程式與其他常見的 HTTP 網路流量類型（靜態 HTML 內容、ASP.NET 網頁或 .ASMX Web 服務）並無不同。 使用此系結的 WCF 通道原本就是無狀態的，並且會在通道關閉時終止其連接。 因此，<xref:System.ServiceModel.BasicHttpBinding> 可搭配現有的 HTTP 負載平衡技術正常運作。  
...
```  
 ...
 使用 .NET Framework 4 中引進的簡化設定，可以使用下列簡化的設定來完成相同的行為。  
