...
> [!NOTE]
> 使用[system.servicemodel 中繼資料公用程式工具 (Svcutil)](../../../docs/framework/wcf/servicemodel-metadata-utility-tool-svcutil-exe.md) `/config`搭配參數來快速建立設定檔。  
...
### <a name="servicemodel-elements"></a>ServiceModel 項目  
 您可以使用`system.ServiceModel`元素所系結的區段來設定具有一或多個端點的服務類型, 以及服務的設定。 然後可以使用位址、合約與繫結來設定每個端點。 如需有關端點的詳細資訊, 請參閱[端點建立總覽](../../../docs/framework/wcf/endpoint-creation-overview.md)。 如果沒有指定任何端點，則執行階段會加入預設端點。 如需預設端點、繫結和行為的詳細資訊，請參閱[簡化的組態](../../../docs/framework/wcf/simplified-configuration.md)和 [WCF 服務的簡化組態](../../../docs/framework/wcf/samples/simplified-configuration-for-wcf-services.md)。  
  
 繫結程序指定傳輸 (HTTP、TCP、管道、訊息佇列) 與通訊協定 (安全性、可靠性、異動流) 並且包含繫結程序項目，每個皆指定端點如何與外界通訊。  
  
 例如, 指定[ \<basicHttpBinding >](../../../docs/framework/configure-apps/file-schema/wcf/basichttpbinding.md)元素會指出要使用 HTTP 做為端點的傳輸。 這是在執行階段，當使用此端點的服務開啟時，用來連接端點。  
  
 有兩類的繫結：預先定義和自訂。 預先定義的繫結包含有用的項目組合，在一般狀況下使用。 如需 WCF 提供的預先定義系結類型清單, 請參閱[系統提供](../../../docs/framework/wcf/system-provided-bindings.md)的系結。 若無預先定義的繫結程序集合擁有服務應用程式所需的正確功能組合，您可以建構自訂繫結程序來滿足應用程式的要求。 如需自訂系結的詳細資訊, 請參閱[ \<customBinding >](../../../docs/framework/configure-apps/file-schema/wcf/custombinding.md)。  
...
```  
 ...
 在此範例中，`name` 屬性表示組態是針對哪種服務類型。 當您在編碼中以 `HelloWorld` 合約建立服務，它將以範例組態定義的所有端點進行初始化。 如果元件只會執行一個服務合約, 則`name`可以省略屬性, 因為服務會使用唯一可用的類型。 該屬性取用一個字串，格式必須為 `Namespace.Class, AssemblyName, Version=0.0.0.0, Culture=neutral, PublicKeyToken=null`  
...
## <a name="configuring-a-behavior-to-apply-to-a-service"></a>設定行為以套用至服務  
 下列範例會針對服務類型來設定特定行為。 元素是用來啟用[system.servicemodel 中繼資料公用程式工具 (Svcutil)](../../../docs/framework/wcf/servicemodel-metadata-utility-tool-svcutil-exe.md)來查詢服務, 並從中繼資料產生 Web 服務描述語言 (WSDL) 檔。 `ServiceMetadataBehavior`  
...
## <a name="specifying-a-service-with-two-endpoints-using-different-binding-values"></a>指定具有使用不同繫結值之兩端點的服務  
 在此最後的範例，兩個端點設定為 `HelloWorld` 服務類型。 每個端點都會使用相同`bindingConfiguration`系結類型的不同自訂屬性 (每`basicHttpBinding`個都修改)。  
