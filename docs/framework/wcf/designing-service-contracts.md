...
## <a name="creating-a-service-contract"></a>建立服務合約  
 服務會公開作業的數量。 在 Windows Communication Foundation （WCF）應用程式中，藉由建立方法並使用 <xref:System.ServiceModel.OperationContractAttribute> 屬性加以標記來定義作業。 接著，若要建立服務合約，請在以 <xref:System.ServiceModel.ServiceContractAttribute> 屬性標記的介面內宣告作業，或是在以相同屬性標記的類別中定義作業，藉此將作業分組。 （如需基本範例，請參閱[如何：定義服務合約](how-to-define-a-wcf-service-contract.md)）。  
  
 沒有 <xref:System.ServiceModel.OperationContractAttribute> 屬性的任何方法都不是服務作業，而且不會由 WCF 服務公開。  
...
```  
 ...
 請注意，除非您指定不同的基礎訊息模式，否則即使是傳回 `void` 的服務作業（在 Visual Basic 中 `Nothing`）都是要求/回復訊息交換。 作業的結果會是：除非用戶端未同步叫用作業，否則用戶端會停止處理，直到收到傳回訊息為止 (即使一般情況下該訊息會是空的)。 下列 C# 程式碼範例示範一個作業，此作業會在用戶端收到空的訊息回應之後才傳回。  
...
##### <a name="out-and-ref-parameters"></a>Out 和 Ref 參數  
 在大部分的情況下，您可以使用 `in` 參數（在 Visual Basic 中 `ByVal`）和 `out` 和 `ref` 參數（`ByRef`）。 由於 `out` 和 `ref` 這兩個參數都表示資料是從作業傳回，因此即使作業簽章 (如下所示) 傳回 `void`，仍會指定必須要求/回覆作業。  
...
- [如何：建立雙面合約](./feature-details/how-to-create-a-duplex-contract.md)
