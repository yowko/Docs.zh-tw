...
|Windows Process Activation Service (WAS)|Windows Vista、Windows Server 2008|HTTP、<br /><br /> net.tcp、<br /><br /> net.pipe、<br /><br /> net.msmq|是|
 ...
 從未受信任的主機執行服務或任何延伸項目都會破壞安全性，這點請您務必注意。 此外，在模擬下開啟 <xref:System.ServiceModel.ServiceHost> 時，應用程式必須確保使用者不會登出，例如快取使用者的 <xref:System.Security.Principal.WindowsIdentity>。
