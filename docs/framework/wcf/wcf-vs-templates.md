...
Windows Communication Foundation (WCF) Visual Studio 範本為預先定義的專案和項目範本，您可以使用 Visual Studio 中，快速建立 WCF 服務和相關的應用程式。  
...
### <a name="wcf-service-library-project-template"></a>WCF 服務程式庫專案範本  
 WCF 服務程式庫專案範本位在 新增專案對話方塊之下**Visual C# \WCF**和**Visual Basic\WCF**。  
  
 當您建立新專案使用**WCF 服務**範本，新的專案會自動包含下列三個檔案：  
  
-   服務合約檔案 (IService1.cs 或 IService1.vb)。 服務合約檔案是已套用的 WCF 服務屬性的介面。 這個檔案提供簡單服務的定義以示範如何定義服務，而其中也包含以參數為基礎的作業和簡單的資料合約範例。 這是建立 WCF 服務專案後，程式碼編輯器中顯示的預設檔案。  
...
> [!NOTE]
>  Visual Studio 將會使用在執行時，辨識 App.config 檔案為專案的組態檔設定[WCF 服務主機 (WcfSvcHost.exe)](../../../docs/framework/wcf/wcf-service-host-wcfsvchost-exe.md)，這是預設組態。 如果是在可執行檔中裝載服務程式庫，您必須將組態程式碼移至該可執行檔的組態檔，因為 DLL 的組態檔是無效的。  
  
### <a name="wcf-service-application-template"></a>WCF 服務應用程式範本  
 在 新增專案 對話方塊中都提供 WCF 服務應用程式範本**Visual C# \WCF**和**Visual Basic\WCF**。  
...
### <a name="wcf-web-site-template"></a>WCF 網站範本  
 在 [新增專案] 對話方塊中並提供 WCF 網站範本**Visual C# \Web Site\WCF 服務**和**Visual Basic\Web Site\WCF Service**。 這會建立與 WCF 服務應用程式範本相同的檔案，但是其組織方式猶如 ASP.NET 網站。 會建立 App_Code 和 App_Data 資料夾。  
  
### <a name="wcf-service-item-template"></a>WCF 服務項目範本  
 WCF 服務項目範本是提供快速的方法，將 WCF 服務加入至現有的 Visual Studio 專案的自訂範本。  
...
### <a name="wcf-syndication-service-library-template"></a>WCF 新聞訂閱服務程式庫範本  
 此範本可讓您將公開為 WCF 服務的 RSS 或 ATOM 格式的摘要。 如需詳細資訊，請參閱[WCF 新聞訂閱](../../../docs/framework/wcf/feature-details/wcf-syndication.md)。  
  
#### <a name="changing-the-address-of-the-feed"></a>變更摘要的位址  
 新聞訂閱範本在執行期間使用 Internet Explorer。 當您以滑鼠右鍵按一下您的專案中**方案總管 中**在 Visual Studio 中，選取**屬性**，然後選取**偵錯** 索引標籤，您可以看到的預設位址範本。 Internet Explorer 會嘗試開啟這個位址上的摘要。  
...
### <a name="ajax-enabled-wcf-service-item-template"></a>具備 AJAX 能力的 WCF 服務項目範本  
 此範本會將 AJAX 控制項公開為 WCF 服務。 如需 AJAX 控制項的詳細資訊，請參閱[AJAX 控制項文件](http://go.microsoft.com/fwlink/?LinkId=96717)。  
  
### <a name="silverlight-enabled-wcf-service-item-template"></a>啟用 Silverlight 的 WCF 服務項目範本  
 這個範本會建立提供資料給 Silverlight 用戶端或前端的 Web 服務。 範本可以加入至網站或 Web 應用程式專案來建立 WCF 服務，其中包含服務程式碼和組態的支援與 Silverlight 用戶端進行通訊。 然後您可以使用**加入服務參考**服務的用戶端 proxy 加入至用戶端，並在 Silverlight 用戶端和啟用 Silverlight 的 WCF 服務之間交換資料。  
