...
> [!NOTE]
> Visual Studio 設定為使用[WCF 服務主機（WcfSvcHost）（](wcf-service-host-wcfsvchost-exe.md)這是預設設定）執行時，將 app.config 檔案識別為專案的設定檔。 如果是在可執行檔中裝載服務程式庫，您必須將組態程式碼移至該可執行檔的組態檔，因為 DLL 的組態檔是無效的。  
