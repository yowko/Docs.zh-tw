...
- 如果您不熟悉使用 WCF 進行程式設計，而且只想要查看可以使用的範例應用程式，請參閱[Windows Communication Foundation 範例](./samples/index.md)中列出的主題。  
  
- 如需逐步解說建立 WCF 服務和用戶端之基本步驟的教學課程，請參閱[消費者入門教學](getting-started-tutorial.md)課程。  
  
- 如果您對 WCF 背後的概念感興趣，請參閱[概念總覽](conceptual-overview.md)一節中的主題。  
...
- 如果您要尋找 WCF 特定功能或功能的相關指引，請參閱[Wcf 功能詳細資料](./feature-details/index.md)底下的主題。  
  
- 如果您想要擴充或自訂 WCF 以符合您的需求，請參閱[擴充 wcf](./extending/index.md)。  
  
- 如需有助於建立和偵測 WCF 應用程式之工具的相關資訊，請參閱[Windows Communication Foundation 工具](tools.md)。  
  
- 使用 XML 檔案進行設定是 WCF 服務和用戶端程式設計的主要方式。 如需設定檔案中使用之 XML 元素的參考檔，請參閱[WCF 設定架構](../configure-apps/file-schema/wcf/index.md)。  
  
## <a name="troubleshooting"></a>疑難排解  
 如需疑難排解常見 WCF 問題的相關資訊，請參閱[WCF 疑難排解快速入門](wcf-troubleshooting-quickstart.md)。  
  
## <a name="using-windows-communication-foundation-with-other-technologies"></a>搭配其他技術使用 Windows Communication Foundation  
  
- 若要建立與 ASP.NET 用戶端通訊的服務，請參閱[如何：將 WCF 服務設定為與 ASP.NET Web 服務用戶端交互操作](./feature-details/config-wcf-service-with-aspnet-web-service.md)。  
  
- 將[.Net 遠端處理應用程式遷移至 WCF](./feature-details/migrating-net-remoting-applications-to-wcf.md)中會說明與 .NET Framework 遠端處理的整合。  
  
- 若要將現有的 COM + 應用程式與 WCF 服務或用戶端整合，請參閱[與 COM + 應用程式整合總覽](./feature-details/integrating-with-com-plus-applications-overview.md)。  
  
- 若要整合現有的 COM 應用程式與 WCF 服務或用戶端，請參閱[與 COM 應用程式整合](./feature-details/integrating-with-com-applications.md)。  
...
- 若要使用 WCF 來取用 Web 服務延伸模組（WSE）3.0 服務，請參閱[如何：存取 WSE 3.0 服務](./feature-details/how-to-access-a-wse-3-0-service-with-a-wcf-client.md)。  
