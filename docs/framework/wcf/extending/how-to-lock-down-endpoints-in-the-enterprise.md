...
> [!IMPORTANT]
> 當應用程式在部分信任環境中執行時，不會執行加入至設定檔之[\<s >](../../configure-apps/file-schema/wcf/commonbehaviors.md)區段的 <xref:System.Security.AllowPartiallyTrustedCallersAttribute> 屬性（APTCA）標記的服務或端點行為，而且在發生這種情況時，不會擲回任何例外狀況。 若要強制執行通用行為 (例如驗證器)，您必須執行下列其中一項：
>
...
>
> - 請確定應用程式是否部署為完全信任的應用程式，使用者無法修改代碼啟用安全性設定以在部分信任環境中執行應用程式。 如果可以執行這項操作，自訂驗證器就不會執行，也不會擲回例外狀況。 如需確保這項工作的其中一種方式，請參閱使用[代碼啟用安全性原則工具（caspol.exe）](../../tools/caspol-exe-code-access-security-policy-tool.md)的 `levelfinal` 選項。
>
...
3. 請務必以強式名稱簽署編譯的組件。 如需詳細資訊，請參閱[強式名稱工具（sn.exe）。EXE）](../../tools/sn-exe-strong-name-tool.md)和適用于您語言的編譯器命令。
...
3. 使用 <xref:System.Configuration?displayProperty=nameWithType> 命名空間型別以：
    ...
    2. 將行為專案新增至[\<s >](../../configure-apps/file-schema/wcf/commonbehaviors.md)區段的 `EndpointBehaviors` 屬性，並鎖定元素。 （若要在服務上安裝驗證程式，驗證器必須是 <xref:System.ServiceModel.Description.IServiceBehavior> 並加入 `ServiceBehaviors` 屬性中）。下列程式碼範例將示範如何在步驟 a 之後進行適當的設定。 和 b. 後進行適當的設定，唯一的例外是沒有強式名稱時。
...
- [使用 DPAPI 加密設定檔元素](https://docs.microsoft.com/previous-versions/msp-n-p/ff647398(v=pandp.10))
- [使用 RSA 加密設定檔元素](https://docs.microsoft.com/previous-versions/msp-n-p/ff650304(v=pandp.10))
