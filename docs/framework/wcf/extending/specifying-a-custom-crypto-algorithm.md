...
```  
 ...
 若要註冊自訂演算法，在程式碼使用<xref:System.Security.Cryptography.CryptoConfig.AddAlgorithm%2A>System.String[])?qualifyHint=False & autoUpgrade = True 的方法。 此方法會建立兩個對應。 下列範例會示範如何呼叫這個方法：  
