...
通道處理站會建立通道。  
 ...
 WCF 會提供此程序中的基底類別協助程式。 (如本主題中討論的通道協助程式類別的圖表，請參閱 <<c0> [ 通道模型概觀](../../../../docs/framework/wcf/extending/channel-model-overview.md)。)  
...
```  
 ...
 實作`Send()`並`BeginSend()` / `EndSend()`。 這分成兩個主要區段。 首先，將訊息序列化為位元組陣列：  
