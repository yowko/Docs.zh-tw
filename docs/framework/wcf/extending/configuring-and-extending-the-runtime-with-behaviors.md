...
行為使您能夠修改預設行為並添加自訂擴展，以檢查和驗證服務配置或修改 Windows 通信基礎 （WCF） 用戶端和服務應用程式中的運行時行為。 本主題會說明行為介面、如何實作這些介面，以及如何透過程式設計方式或組態檔來將它們新增到服務描述 (在服務應用程式中) 或端點 (在用戶端應用程式中)。 有關使用系統提供的行為的詳細資訊，請參閱[指定服務運行時行為](../specifying-service-run-time-behavior.md)和[指定用戶端運行時行為](../specifying-client-run-time-behavior.md)。  
  
## <a name="behaviors"></a>行為  
 行為類型將添加到服務或服務終結點描述物件（分別在服務或用戶端上），然後 Windows 通信基礎 （WCF） 使用這些物件來創建執行 WCF 服務或 WCF 用戶端的運行時。 當在執行階段建構程序期間呼叫這些行為之後，這些行為即可存取可修改由合約、繫結及位址所構成之執行階段的執行階段屬性和方法。  
...
> [!NOTE]
> 有關可用於修改用戶端執行行為的運行時屬性和擴展類型的討論，請參閱[擴展用戶端](extending-clients.md)。 有關可用於修改服務調度程式執行行為的運行時屬性和擴展類型的討論，請參閱[擴展調度程式](extending-dispatchers.md)。  
  
 大多數 WCF 使用者不直接與運行時交互;因此，這些使用者不會直接與運行時交互。相反，它們使用核心程式設計模型構造，如終結點、協定、綁定、位址和行為屬性，對設定檔中的類或行為。 這些構造構成*描述樹*，它是構造運行時以支援描述樹描述的服務或用戶端的完整規範。  
...
3. 實作會擴充組態的自訂 <xref:System.ServiceModel.Configuration.BehaviorExtensionElement>。 這樣便可從應用程式組態檔使用服務行為。  
 ...
 WCF 中的服務行為示例包括<xref:System.ServiceModel.ServiceBehaviorAttribute>屬性、和<xref:System.ServiceModel.Description.ServiceThrottlingBehavior><xref:System.ServiceModel.Description.ServiceMetadataBehavior>行為。  
  
#### <a name="contract-behaviors"></a>合約行為  
 實作 <xref:System.ServiceModel.Description.IContractBehavior> 介面的合約行為，可用於擴充整個合約的用戶端與服務執行階段。  
  
 將合約行為新增到合約時可以使用兩種機制。  第一種機制是建立要在合約介面上使用的自訂屬性。 當協定介面傳遞給 或<xref:System.ServiceModel.ServiceHost>時<xref:System.ServiceModel.ChannelFactory%601>，WCF 會檢查介面上的屬性。 如果其中任何一個屬性是 <xref:System.ServiceModel.Description.IContractBehavior> 的實作，這些屬性便會新增到為該介面建立之 <xref:System.ServiceModel.Description.ContractDescription?displayProperty=nameWithType> 上的行為集合中。  
 ...
 •合約介面。 在這種情況下，該行為應用於任何終結點中該類型的所有協定，WCF 忽略<xref:System.ServiceModel.Description.IContractBehaviorAttribute.TargetContract%2A?displayProperty=nameWithType>屬性的值。  
  
 •服務類別。 在此情況下，行為只會套用至其中合約為 <xref:System.ServiceModel.Description.IContractBehaviorAttribute.TargetContract%2A> 屬性值的端點。  
  
 •回呼類別。 在這種情況下，該行為將應用於雙工用戶端的終結點，而 WCF 忽略<xref:System.ServiceModel.Description.IContractBehaviorAttribute.TargetContract%2A>屬性的值。  
  
 第二種機制是將行為新增至 <xref:System.ServiceModel.Description.ContractDescription> 上的行為集合。  
  
 WCF 中協定行為的示例包括該<xref:System.ServiceModel.DeliveryRequirementsAttribute?displayProperty=nameWithType>屬性。 如需詳細資訊和範例，請參閱參考主題。  
...
#### <a name="operation-behaviors"></a>作業行為  
 ...
 將作業行為新增到作業時可以使用兩種機制。 第一種機制是建立要用於建立作業模型之方法的自訂屬性。 <xref:System.ServiceModel.ServiceHost>當操作添加到 或 或<xref:System.ServiceModel.ChannelFactory>時，WCF 會向<xref:System.ServiceModel.Description.IOperationBehavior>為該操作<xref:System.ServiceModel.Description.OperationDescription>創建的行為集合添加任何屬性。  
  
 第二種機制是將行為直接新增至已建構 <xref:System.ServiceModel.Description.OperationDescription> 上的行為集合中。  
  
 WCF 中的操作行為示例包括<xref:System.ServiceModel.OperationBehaviorAttribute>和<xref:System.ServiceModel.TransactionFlowAttribute>。  
...
> [!NOTE]
> 您也可以使用支援組態強制公司應用程式原則的行為，使用方法是將這些行為插入 machine.config 組態檔，並鎖定這些項目。 有關說明和示例，請參閱[如何：鎖定企業中的終結點](how-to-lock-down-endpoints-in-the-enterprise.md)。  
...
```  
 ...
 行為`Microsoft.WCF.Documentation.EndpointBehaviorMessageInspector`擴展類型在哪裡，`HostApplication`是已編譯該類的程式集的名稱。  
