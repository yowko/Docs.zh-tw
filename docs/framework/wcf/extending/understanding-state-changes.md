...
圖 1. ICommunicationObject 狀態機器。  
 ...
 Windows Communication Foundation (WCF) 會提供名為的抽象基類 <xref:System.ServiceModel.Channels.CommunicationObject> ，該類別會實作為 <xref:System.ServiceModel.ICommunicationObject> 和通道狀態機器。 下圖是 <xref:System.ServiceModel.Channels.CommunicationObject> 特定的狀態修訂圖。 除了 <xref:System.ServiceModel.ICommunicationObject> 狀態機器之外，它還說明了叫用其他 <xref:System.ServiceModel.Channels.CommunicationObject> 方法的時機。  
  
 ![CommunicationObject 執行狀態變更的資料流程圖。](./media/understanding-state-changes/communicationobject-implementation-state-machine.gif)
...
|開啟|N/A|<xref:System.InvalidOperationException?displayProperty=nameWithType>|  
|已開啟|N/A|<xref:System.InvalidOperationException?displayProperty=nameWithType>|  
|關閉|Yes|<xref:System.ServiceModel.CommunicationObjectAbortedException?displayProperty=nameWithType>|  
|關閉|No|<xref:System.ObjectDisposedException?displayProperty=nameWithType>|  
|封閉式|Yes|在先前明確呼叫 Abort 而關閉物件的情況下，為 <xref:System.ServiceModel.CommunicationObjectAbortedException?displayProperty=nameWithType>。 如果在此物件上呼叫 Close，則會擲回 <xref:System.ObjectDisposedException?displayProperty=nameWithType>。|  
|封閉式|No|<xref:System.ObjectDisposedException?displayProperty=nameWithType>|  
|發生錯誤|N/A|<xref:System.ServiceModel.CommunicationObjectFaultedException?displayProperty=nameWithType>|  
