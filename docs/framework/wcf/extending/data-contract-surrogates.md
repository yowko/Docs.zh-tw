...
1. 為服務建立 <xref:System.ServiceModel.ServiceHost> 的執行個體。 如需完整的指示，請參閱 [基本 WCF 程式設計](../basic-wcf-programming.md)。  
