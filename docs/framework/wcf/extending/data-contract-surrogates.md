...
---
# 資料合約代理
資料合約「*代理*」\(Surrogate\) 是在資料合約模型上建置的進階功能。這項功能是專為在使用者想要變更型別序列化、還原序列化或投射至中繼資料的方式時，用來自訂和替換型別所設計。某些可能使用代理的情況包括：尚未指定型別的資料合約、欄位和屬性 \(Property\) 尚未以 <xref:System.Runtime.Serialization.DataMemberAttribute> 屬性 \(Attribute\) 標記，或是使用者希望動態建立結構描述變形時。  
...
### GetObjectToSerialize 方法  
 ...
 `targetType` 參數會參考宣告的成員型別。這個參數是由 <xref:System.Runtime.Serialization.IDataContractSurrogate.GetDataContractType%2A> 方法傳回的代理型別。序列化程式不會強制傳回的物件必須能夠指派給這個型別。`obj` 參數是要序列化的物件，而且會在必要時轉換成其代理。如果代理的物件不會處理該物件的話，這個方法就必須傳回輸入物件。否則將傳回新的代理物件。如果物件為 null，則不會呼叫代理。這個物件內可定義許多不同執行個體的代理對應。  
...
#### GetCustomDataToExport 方法 \(Type, Type\)  
 <xref:System.Runtime.Serialization.IDataContractSurrogate.GetCustomDataToExport%28System.Type%2CSystem.Type%29?displayProperty=fullName> 方法會在針對每一個型別定義匯出結構描述的期間呼叫。這個方法會在匯出時，將資訊加入至結構描述內的型別。定義的每個型別都會傳送至這個方法，以判斷是否有任何額外的資訊需要加入結構描述中。  
  
#### GetCustomDataToExport 方法 \(MemberInfo, Type\)  
 <xref:System.Runtime.Serialization.IDataContractSurrogate.GetCustomDataToExport%28System.Reflection.MemberInfo%2CSystem.Type%29?displayProperty=fullName> 會在匯出已匯出的型別中每一個成員時呼叫。這個函式可讓您自訂匯出時，將加入結構描述中之成員的任何註解。類別內每一個成員的資訊都會傳送至這個方法，以檢查是否有任何額外的資料需要加入結構描述中。  
...
### ProcessImportedType 方法  
 ...
 如果在中繼資料匯出期間插入自訂資料，則該資料必須在匯入期間提供給使用者才能使用。這項自訂資料可以用於程式設計模型提示，或其他註解。每個 `CodeTypeDeclaration` 和包含如 <xref:System.CodeDom.CodeObject.UserData%2A> 屬性之自訂資料的 <xref:System.CodeDom.CodeTypeMember> 執行個體，都會轉換 \(Cast\) 成 `IDataContractSurrogate` 型別。  
...
1.  為服務建立 <xref:System.ServiceModel.ServiceHost> 的執行個體。如需完整的指示，請參閱[基本 WCF 程式設計](../../../../docs/framework/wcf/basic-wcf-programming.md)。  
...
5.  將 <xref:System.Runtime.Serialization.XsdDataContractImporter> 之 <xref:System.Runtime.Serialization.ImportOptions> 的 <xref:System.Runtime.Serialization.ImportOptions.DataContractSurrogate%2A> 屬性設為代理的新執行個體。  
  
6.  將 <xref:System.Runtime.Serialization.XsdDataContractImporter> 加入至 <xref:System.ServiceModel.Description.WsdlImporter> \(繼承自 <xref:System.ServiceModel.Description.MetadataExporter> 類別\) 的 <xref:System.ServiceModel.Description.MetadataExporter.State%2A> 屬性傳回的集合。  
  
7.  使用 <xref:System.ServiceModel.Description.WsdlImporter> 的 <xref:System.ServiceModel.Description.WsdlImporter.ImportAllContracts%2A> 方法匯入結構描述內的所有資料合約。在進行最後的步驟時，程式碼會從呼叫至代理而載入的結構描述產生。  
...
1.  建立新的 <xref:System.ServiceModel.Description.WsdlExporter>，或是使用傳遞至 <xref:System.ServiceModel.Description.IWsdlExportExtension.ExportContract%2A> 方法的 `wsdlExporter` 參數。  
...
3.  如果 <xref:System.Collections.Generic.Dictionary%602.TryGetValue%2A> 傳回 `false`，則從 <xref:System.ServiceModel.Description.WsdlExporter> 建立含有所產生 XML 結構描述的新 <xref:System.Runtime.Serialization.XsdDataContractExporter>，並且將它加入至 <xref:System.ServiceModel.Description.WsdlExporter> 的 <xref:System.ServiceModel.Description.MetadataExporter.State%2A> 屬性傳回的集合。否則使用 <xref:System.Collections.Generic.Dictionary%602.TryGetValue%2A> 方法的 `out` 參數傳回的匯出工具。  
...
5.  將 <xref:System.Runtime.Serialization.XsdDataContractExporter> 之 <xref:System.Runtime.Serialization.ExportOptions> 的 <xref:System.Runtime.Serialization.ExportOptions.DataContractSurrogate%2A> 屬性設為代理的新執行個體。後續匯出中繼資料的步驟不需要任何改變。  
