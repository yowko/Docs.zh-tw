...
```  
 ...
 如果您必須只在本機電腦上設定 WS-AtomicTransaction 支援，可使用這個工具的命令列版本。 如需命令列工具的詳細資訊，請參閱[ws-atomictransaction 設定公用程式（wsatconfig.exe .exe）](ws-atomictransaction-configuration-utility-wsatconfig-exe.md)主題。  
