...
WCF 現可支援合約優先開發。 Svcutil.exe 會有 /serviceContract 參數可讓您從 WSDL 文件產生服務與資料合約。
...
WebSockets 是透過連接埠 80 和 443 提供真正雙向通訊的技術，其效能特性與 TCP 類似。 為了支援透過 WebSocket 傳輸進行通訊，我們已加入兩個新的繫結。 <xref:System.ServiceModel.NetHttpBinding> 和 <xref:System.ServiceModel.NetHttpsBinding>。 如需詳細資訊，請參閱：[系統提供繫結](../../../docs/framework/wcf/system-provided-bindings.md)。
...
在 Visual Studio 2010 中，不論您是否明確指定端點，WebServiceHost 都會自動建立預設端點。 在 Visual Studio 2012 和更新版本，WebServiceHost 只會建立預設端點明確加入任何端點。 如果您的用戶端所預期的預設端點，您可以明確地新增端點，並將用戶端指向。 或者，您可以將下列設定加入您應用程式的組態檔，告知 WCF 還原成之前的行為
