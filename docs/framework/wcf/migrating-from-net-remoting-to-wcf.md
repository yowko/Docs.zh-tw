...
## <a name="comparing-net-remoting-to-wcf"></a>比較 .NET 遠端處理與 WCF  
 ...
 本節將 .NET 遠端處理的基本建置組塊與其 WCF 對應項進行比較。 我們稍後將使用這些組建區塊，在 WCF 中建立一些常見的用戶端伺服器案例。 下圖摘要說明 .NET 遠端處理與 WCF 之間的主要相似之處和差異。  
...
|用戶端 Proxy 物件|強型別 Transparent Proxy 是從 MarshalByRefObjects 自動建立|強型別 proxy 是依需要使用 ChannelFactory 產生的\<TChannel>|  
...
```  
 ...
 有許多方法可將遠端處理類型當做伺服器來提供，包括使用組態檔。 這只是範例其一。  
...
```  
 ...
 此範例顯示如何在通道層級進行程式設計，因為它最類似遠端處理範例。 此外，也可以在產生程式碼以簡化用戶端程式設計的 Visual Studio 中使用 **加入服務參考** 方法。 如需詳細資訊，請參閱下列主題：  
...
- *依值* –物件的值會跨階層界限序列化，並在另一個階層上建立該物件的新實例。 對新執行個體的方法或屬性所做的任何呼叫只會在本機執行，並不會影響原始物件或階層。  
  
- *依參考* –特殊的「物件參考」會跨階層界限進行序列化。 當某個階層與該物件的方法或屬性互動時，它會回到原始階層與原始物件通訊。 傳址物件流向可為任一方向 (從伺服器到用戶端，或從用戶端到伺服器)。  
...
- **.NET 遠端處理是舊版產品。** 如 [.Net 遠端處理](/previous-versions/dotnet/netframework-4.0/72x4h507(v=vs.100))所述，它被視為舊版產品，不建議用於新的開發。 建議針對新的和現有的應用程式使用 WCF 或 ASP.NET Web API。  
  
- **WCF 使用跨平台標準。** WCF 的設計將跨平台互通性納入考量，並支援許多業界標準 (SOAP、WS-Security、WS-Trust 等)。 WCF 服務可以與在非 Windows 作業系統上執行的用戶端互通。 遠端處理的設計主要是針對伺服器和用戶端應用程式都使用 .NET Framework 在 Windows 作業系統上執行的環境。
  
- **WCF 具有內建安全性。** WCF 是以安全性為考慮而設計，提供許多驗證、傳輸層級安全性、訊息層級安全性等的選項。遠端處理的設計目的是要讓應用程式更容易交互操作，但不是設計成在不信任的環境中受到保護。 WCF 的設計是為了在受信任和未受信任的環境中都能夠正常運作。  
...
5. 接著會使用下列程式碼來啟動 WCF 服務：  
   ...
   ```  
     ...
     當啟動這個 ServiceHost 時，它會使用 web.config 檔案來建立適當的合約、繫結與端點。 如需設定檔的詳細資訊，請參閱 [使用設定檔](./configuring-services-using-configuration-files.md)來設定服務。 這種啟動伺服器的方式稱為自我裝載。 若要深入瞭解裝載 WCF 服務的其他選項，請參閱 [裝載服務](./hosting-services.md)。  
  
6. 用戶端專案的 app.config 必須宣告服務端點的相符繫結資訊。 在 Visual Studio 中最簡單的方式是使用 **加入服務參考**，它會自動更新 app.config 檔。 您也可以手動加入這些相同的變更。  
...
4. 我們需要執行下列兩個動作，來修改伺服器的組態檔，如下列範例所示：  
    ...
    1. 宣告 \<client> 描述會話物件端點的區段。 由於在這種情況下伺服器也會當做用戶端，因此必須執行這個動作。  
...
2. 使用用戶端來建立新的傳值物件 (客戶)，建立要與 ICustomerService 服務通訊的通道，然後將物件傳送至通道。  
 ...
 一般 WCF 交換資料的方式是傳值。 如此可確保在其中一個資料物件上叫用方法的作業只會在本機執行，而不會在另一個階層上叫用程式碼。 雖然可以達成 *從* 伺服器傳回的參考物件之類的內容，但用戶端並不可能將傳址物件傳遞 *給* 伺服器。 如果需要在用戶端與伺服器之間來回溝通，可透過雙工服務在 WCF 中達成。 如需詳細資訊，請參閱 [雙工服務](./feature-details/duplex-services.md)。  
