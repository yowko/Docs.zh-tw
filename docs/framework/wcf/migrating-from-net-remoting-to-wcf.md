...
## <a name="comparing-net-remoting-to-wcf"></a>比較 .NET 遠端處理與 WCF  
 本節將 .NET 遠端處理的基本建置組塊與其 WCF 對應項進行比較。 稍後我們將使用這些構建基塊在 WCF 中創建一些常見的用戶端-伺服器方案。 下圖總結了 .NET 遠端處理和 WCF 之間的主要相似點和差異。  
...
|已傳遞的物件|傳值或傳址|僅限傳值|  
|錯誤/例外狀況|任何可序列化的例外狀況|故障契約\<T 細節>|  
|用戶端 Proxy 物件|強型別 Transparent Proxy 是從 MarshalByRefObjects 自動建立|使用管道工廠\<TChannel 按需生成強型別代理>|  
...
```  
 ...
 此範例顯示如何在通道層級進行程式設計，因為它最類似遠端處理範例。 Visual Studio 中**的附加服務參考**方法也可用，該方法生成代碼以簡化用戶端程式設計。 如需詳細資訊，請參閱下列主題：  
...
- *按值*– 物件的值跨層邊界序列化，並在另一層上創建該物件的新實例。 對新執行個體的方法或屬性所做的任何呼叫只會在本機執行，並不會影響原始物件或階層。  
  
- *通過引用*– 跨層邊界序列化了特殊的"物件引用"。 當某個階層與該物件的方法或屬性互動時，它會回到原始階層與原始物件通訊。 傳址物件流向可為任一方向 (從伺服器到用戶端，或從用戶端到伺服器)。  
...
```  
 ...
 當 WCF 跨階層傳送物件時，它只會序列化值，並在另一個階層上建立新的物件執行個體。 與物件值的任何互動只會在本機進行，而不會和 .NET 遠端處理傳址物件一樣與另一個階層通訊。 有關詳細資訊，請參閱[序列化和反序列化](./feature-details/serialization-and-deserialization.md)。  
...
- **.NET 遠端處理是舊版產品。** 如[.NET 遠端處理](https://docs.microsoft.com/previous-versions/dotnet/netframework-4.0/72x4h507%28v=vs.100%29)中所述，它被視為遺留產品，不建議用於新開發。 建議針對新的和現有的應用程式使用 WCF 或 ASP.NET Web API。  
  
- **WCF 使用跨平台標準。** WCF 的設計將跨平台互通性納入考量，並支援許多業界標準 (SOAP、WS-Security、WS-Trust 等)。 WCF 服務可以與在非 Windows 作業系統上執行的用戶端互通。 遠端處理主要用於伺服器和用戶端應用程式在 Windows 作業系統上使用 .NET 框架運行的環境。
  
- **WCF 具有內建安全性。** WCF 的設計考慮到了安全性，為身份驗證、傳輸級別安全性、消息級安全性等提供了許多選項。遠端處理旨在使應用程式易於交互操作，但設計起來在非受信任的環境中是安全的。 WCF 的設計是為了在受信任和未受信任的環境中都能夠正常運作。  
...
- **創建資料協定。** 定義要在伺服器與用戶端之間交換的資料類型，並以 [DataContract] 屬性標記這些類型。 將用戶端可搭配 [DataMember] 使用之所有欄位和屬性都加以標記。  
...
- **設定及裝載服務。** 建立服務合約之後，下一個步驟是設定繫結，以公開端點上的服務。 有關詳細資訊，請參閱[終結點：位址、綁定和協定](./feature-details/endpoints-addresses-bindings-and-contracts.md)。  
...
5. 接著會使用下列程式碼來啟動 WCF 服務：  
   ...
   ```  
     ...
     當啟動這個 ServiceHost 時，它會使用 web.config 檔案來建立適當的合約、繫結與端點。 有關設定檔的詳細資訊，請參閱[使用設定檔佈建服務](./configuring-services-using-configuration-files.md)。 這種啟動伺服器的方式稱為自我裝載。 要瞭解有關託管 WCF 服務的其他選擇，請參閱[託管服務](./hosting-services.md)。  
  
6. 用戶端專案的 app.config 必須宣告服務端點的相符繫結資訊。 在 Visual Studio 中執行此操作的最簡單方法是使用**添加服務參考**，這將自動更新 app.config 檔。 您也可以手動加入這些相同的變更。  
    ...
    ```  
     ...
     有關使用**添加服務參考**的詳細資訊，請參閱[：添加、更新或刪除服務參考](/visualstudio/data-tools/how-to-add-update-or-remove-a-wcf-data-service-reference)。  
...
4. 我們需要執行下列兩個動作，來修改伺服器的組態檔，如下列範例所示：  
    ...
    1. 聲明一\<個用戶端>部分，用於描述會話物件的終結點。 由於在這種情況下伺服器也會當做用戶端，因此必須執行這個動作。  
...
2. 使用用戶端來建立新的傳值物件 (客戶)，建立要與 ICustomerService 服務通訊的通道，然後將物件傳送至通道。  
 ...
 一般 WCF 交換資料的方式是傳值。 如此可確保在其中一個資料物件上叫用方法的作業只會在本機執行，而不會在另一個階層上叫用程式碼。 雖然可以實現*從*伺服器返回的引用物件，但用戶端不可能將引用物件*傳遞給*伺服器。 如果需要在用戶端與伺服器之間來回溝通，可透過雙工服務在 WCF 中達成。 有關詳細資訊，請參閱[雙工服務](./feature-details/duplex-services.md)。  
