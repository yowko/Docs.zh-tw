---
title: "從 .NET 遠端處理移轉到 WCF | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
...
-   [比較 .NET 遠端處理與 WCF](../../../docs/framework/wcf/migrating-from-net-remoting-to-wcf.md#Compare_Top)  
  
    -   [伺服器實作比較](../../../docs/framework/wcf/migrating-from-net-remoting-to-wcf.md#Server_Comp)  
  
    -   [用戶端實作比較](../../../docs/framework/wcf/migrating-from-net-remoting-to-wcf.md#Client_Comp)  
  
    -   [序列化使用方式](../../../docs/framework/wcf/migrating-from-net-remoting-to-wcf.md#Serialization_Usage)  
  
    -   [例外狀況處理功能](../../../docs/framework/wcf/migrating-from-net-remoting-to-wcf.md#Exception_Handling)  
  
    -   [安全性考量](../../../docs/framework/wcf/migrating-from-net-remoting-to-wcf.md#Security_Considerations)  
  
-   [移轉至 WCF](../../../docs/framework/wcf/migrating-from-net-remoting-to-wcf.md#Migrating_Top)  
  
    -   [為什麼要從遠端處理移轉至 WCF？](../../../docs/framework/wcf/migrating-from-net-remoting-to-wcf.md#Why_Migrate)  
  
    -   [移轉建議](../../../docs/framework/wcf/migrating-from-net-remoting-to-wcf.md#Migration_Recommendations)  
  
    -   [移轉案例](../../../docs/framework/wcf/migrating-from-net-remoting-to-wcf.md#Migration_Scenarios)  
...
|用戶端 Proxy 物件|強型別 Transparent Proxy 是從 MarshalByRefObjects 自動建立|強型別 Proxy 是視需要使用 ChannelFactory\<TChannel\> 所產生|  
...
```  
 ...
 此範例顯示如何在通道層級進行程式設計，因為它最類似遠端處理範例。  您也可以使用 Visual Studio 中的 \[加入服務參考\] 方法，來產生可簡化用戶端程式設計的程式碼。  如需詳細資訊，請參閱下列主題：  
  
-   [用戶端通道層級的程式設計](../../../docs/framework/wcf/extending/client-channel-level-programming.md)  
...
-   傳值 \- 系統會跨階層界限序列化物件的值，並在另一個階層上建立該物件的新執行個體。  對新執行個體的方法或屬性所做的任何呼叫只會在本機執行，並不會影響原始物件或階層。  
  
-   傳址 \- 系統會跨階層界限序列化特殊「物件參考」。  當某個階層與該物件的方法或屬性互動時，它會回到原始階層與原始物件通訊。  傳址物件流向可為任一方向 \(從伺服器到用戶端，或從用戶端到伺服器\)。  
...
-   [WCF 安全性指導方針](http://wcfsecurity.codeplex.com/)  
...
-   **.NET 遠端處理是舊版產品。** 如 [.NET 遠端處理](http://msdn.microsoft.com/library/vstudio/72x4h507\(v=vs.100\).aspx)中所述，.NET 遠端處理會視為舊版產品，不建議用於新的開發作業。  建議針對新的和現有的應用程式使用 WCF 或 ASP.NET Web 應用程式開發介面。  
...
-   **設定及裝載服務。** 建立服務合約之後，下一個步驟是設定繫結，以公開端點上的服務。  如需詳細資訊，請參閱[端點：位址、繫結和合約](../../../docs/framework/wcf/feature-details/endpoints-addresses-bindings-and-contracts.md)。  
...
5.  接著會使用下列程式碼來啟動 WCF 服務：  
    ...
    ```  
     ...
     當啟動這個 ServiceHost 時，它會使用 web.config 檔案來建立適當的合約、繫結與端點。  如需組態檔的詳細資訊，請參閱[使用組態檔設定服務](../../../docs/framework/wcf/configuring-services-using-configuration-files.md)。  這種啟動伺服器的方式稱為自我裝載。  若要進一步了解裝載 WCF 服務的其他選項，請參閱[裝載服務](../../../docs/framework/wcf/hosting-services.md)。  
  
6.  用戶端專案的 app.config 必須宣告服務端點的相符繫結資訊。  在 Visual Studio 中執行此作業的最簡單方式是使用 \[加入服務參考\]，這會自動更新 app.config 檔案。  您也可以手動加入這些相同的變更。  
    ...
    ```  
     ...
     如需使用 \[加入服務參考\] 的詳細資訊，請參閱[How to: Add, Update, or Remove a Service Reference](../Topic/How%20to:%20Add,%20Update,%20or%20Remove%20a%20Service%20Reference.md)。  
...
4.  我們需要執行下列兩個動作，來修改伺服器的組態檔，如下列範例所示：  
    ...
    1.  宣告描述工作階段物件端點的 \<client\> 區段。  由於在這種情況下伺服器也會當做用戶端，因此必須執行這個動作。  
...
2.  使用用戶端來建立新的傳值物件 \(客戶\)，建立要與 ICustomerService 服務通訊的通道，然後將物件傳送至通道。  
 ...
 一般 WCF 交換資料的方式是傳值。  如此可確保在其中一個資料物件上叫用方法的作業只會在本機執行，而不會在另一個階層上叫用程式碼。  這樣做雖然可以取得與從伺服器傳回之傳址物件類似的結果，但用戶端不可能將傳址物件傳遞至伺服器。  如果需要在用戶端與伺服器之間來回溝通，可透過雙工服務在 WCF 中達成。  如需詳細資訊，請參閱[雙工服務](../../../docs/framework/wcf/feature-details/duplex-services.md)。  
