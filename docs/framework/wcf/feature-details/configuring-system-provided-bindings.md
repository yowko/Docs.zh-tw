...
在與端點對話時，繫結會指定要使用的通訊機制，並指出如何連接至端點。 系結是由定義 Windows Communication Foundation (WCF) 通道如何分層，以提供所需通訊功能的元素組成。 繫結包含三種項目型別：  
...
|<xref:System.ServiceModel.MsmqIntegration.MsmqIntegrationBinding>|[\<msmqIntegrationBinding>](../../configure-apps/file-schema/wcf/msmqintegrationbinding.md)|系結，適用于 WCF 應用程式與現有訊息佇列之間的跨電腦通訊 (也稱為 MSMQ) 應用程式。|  
...
|<xref:System.ServiceModel.WSDualHttpBinding>|WS|無、(訊息)|(可靠工作階段)|(無)、是|Yes|  
|<xref:System.ServiceModel.WSFederationHttpBinding>|WS-同盟|無、(訊息)、混合|(無)、可靠工作階段|(無)、是|No|  
|<xref:System.ServiceModel.WS2007FederationHttpBinding>|WS-同盟|無、(訊息)、混合|(無)、可靠工作階段|(無)、是|No|  
|<xref:System.ServiceModel.NetTcpBinding>|.NET|無、(傳輸)、訊息、<br /><br /> Mixed|可靠工作階段、(傳輸)|(無)、是|Yes|  
|<xref:System.ServiceModel.NetNamedPipeBinding>|.NET|無、<br /><br /> (傳輸)|無、(傳輸)|(無)、是|Yes|  
|<xref:System.ServiceModel.NetMsmqBinding>|.NET|無、訊息、(傳輸)、兩者|(無)|(無)、是|No|  
|<xref:System.ServiceModel.NetPeerTcpBinding>|對等|無、訊息、(傳輸)、混合|(無)|(無)|Yes|  
...
|安全性|指定保護通道的方式：<br /><br /> -無： SOAP 訊息未受保護，且用戶端未經過驗證。<br />-Transport：已滿足傳輸層的安全性需求。<br />-Message：已滿足訊息層的安全性需求。<br />-Mixed：此安全性模式稱為 `TransportWithMessageCredentials` 。 它負責處理訊息層級的認證，而完整性和機密性需求則交由傳輸層負責。<br />-兩者：使用訊息層級和傳輸層級安全性。 這項能力是 <xref:System.ServiceModel.NetMsmqBinding> 所獨有的。|  
