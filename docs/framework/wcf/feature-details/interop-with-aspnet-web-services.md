...
ASP.NET Web 服務和 Windows Communication Foundation （WCF） Web 服務之間的互通性可透過確保使用這兩種技術所執行的服務都符合 WS-I Basic Profile 1.1 規格來達成。 符合 WS-I 基本設定檔1.1 的 ASP.NET Web 服務可以使用 WCF 系統提供的系結，與 WCF 用戶端互通 <xref:System.ServiceModel.BasicHttpBinding> 。  
  
 使用 ASP.NET 2.0 選項 <xref:System.Web.Services.WebService> ，將和 <xref:System.Web.Services.WebMethodAttribute> 屬性新增至介面，而不是類別，以及撰寫類別來執行介面，如下列範例程式碼所示。  
