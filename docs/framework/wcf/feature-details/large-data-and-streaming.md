...
---
# 大型資料與資料流
[!INCLUDE[indigo1](../../../../includes/indigo1-md.md)] 是 XML 通訊基礎結構。由於 XML 資料常以在 [XML 1.0 規格](http://go.microsoft.com/fwlink/?LinkId=94838) \(本頁面可能為英文\) 中定義的標準文字格式進行編碼，因此已連線系統開發人員和架構設計人員通常會關心在網路上傳送之訊息的網路使用量 \(或大小\)，以及 XML 文字編碼為二進位資料的傳輸效率所帶來的特殊挑戰。  
...
### 大型資料內容  
 除了網路使用量以外，先前提到的 500 MB 承載也會為服務和用戶端帶來很大的本機挑戰。根據預設，[!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] 會在「*緩衝模式*」\(Buffered Mode\) 中處理訊息。這表示在傳送之前或接收之後，訊息的整個內容都會存在於記憶體中。雖然這對大部分的案例都是一個很好的策略，而且對於訊息功能 \(例如數位簽章和可靠的傳遞\) 是必要的，但是大型訊息可能會很容易耗盡系統的資源。  
...
-   當初始化傳輸時，無法完整提供使用。  
 ...
 傳送大量資料時，您必須設定 maxAllowedContentLength IIS 設定 \(如需詳細資訊，請參閱[設定 IIS 要求限制](http://go.microsoft.com/fwlink/?LinkId=253165)\) 和 maxReceivedMessageSize 繫結設定 \(例如 <xref:System.ServiceModel.BasicHttpBinding.MaxReceivedMessageSize%2A> 或 <xref:System.ServiceModel.NetTcpBinding.MaxReceivedMessageSize%2A>\)。MaxAllowedContentLength 屬性預設為 28.6 M，而 maxReceivedMessageSize 屬性預設為 64 KB。  
  
## 編碼方式  
 「*編碼方式*」\(Encoding\) 會定義一組有關如何在網路上呈現訊息的規則。「*編碼器*」\(Encoder\) 會實作此類編碼，並負責在寄件者端將 <xref:System.ServiceModel.Channels.Message> 記憶體中訊息變成可以在網路上傳送的位元組資料流或位元組緩衝區。在接收者端，編碼器會將位元組序列變成記憶體中的訊息。  
...
### 啟用 MTOM  
 當互通性成為一項需求且必須傳送大型二進位資料時，MTOM 訊息編碼是可以在標準 <xref:System.ServiceModel.BasicHttpBinding> 或 <xref:System.ServiceModel.WSHttpBinding> 繫結上啟用的替代編碼策略，方法是將個別 `MessageEncoding` 屬性設定為 <xref:System.ServiceModel.WSMessageEncoding> 或將 <xref:System.ServiceModel.Channels.MtomMessageEncodingBindingElement> 撰寫為 <xref:System.ServiceModel.Channels.CustomBinding>。下列擷取自 [MTOM 編碼方式](../../../../docs/framework/wcf/samples/mtom-encoding.md)範例的範例程式碼將示範如何在組態中啟用 MTOM。  
...
> [!NOTE]
>  您不應在資料合約內使用 <xref:System.IO.Stream?displayProperty=fullName> 衍生型別。資料流資料應使用資料流模型進行通訊，下面的「資料流資料」一節中將有說明。  
...
-   在資料流模式中傳送及接受要求，而在緩衝模式中接受及傳回回應 \(<xref:System.ServiceModel.TransferMode>\)。  
  
-   在緩衝模式中傳送及接受要求，而在資料流模式中接受及傳回回應 \(<xref:System.ServiceModel.TransferMode>\)。  
  
-   在資料流模式中雙向傳送和接收要求與回應。\(<xref:System.ServiceModel.TransferMode>\).  
  
 您可以將傳輸模式設定為 <xref:System.ServiceModel.TransferMode> \(這是所有繫結上的預設設定\)，以停用資料流。下列程式碼顯示如何在組態中設定傳輸模式。  
...
```  
 ...
 前例中的作業 `Echo` 接收並傳回資料流，因此應該用於使用 <xref:System.ServiceModel.TransferMode> 的繫結上。對於作業 `RequestInfo`，<xref:System.ServiceModel.TransferMode> 是最適合的，因為它只傳回 <xref:System.IO.Stream>。單向作業最適合 <xref:System.ServiceModel.TransferMode>。  
...
### 大型資料的特殊安全性考量  
 所有的繫結都可讓您限制傳入訊息的大小，以防止拒絕服務的攻擊。例如，<xref:System.ServiceModel.BasicHttpBinding> 會公開限定傳入訊息大小的 <xref:System.ServiceModel.BasicHttpBinding.MaxReceivedMessageSize%2A> 屬性，因此也會限定處理訊息時存取的最大記憶體上限。這個單位的設定是位元組，預設值是 65,536 位元組。  
 ...
 另外很重要的是，要了解這個配額只限制 [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] 所完成的緩衝，而無法防止您在自己的服務或用戶端實作中執行的任何緩衝。[!INCLUDE[crabout](../../../../includes/crabout-md.md)]其他安全性考量的詳細資訊，請參閱[資料的安全性考量](../../../../docs/framework/wcf/feature-details/security-considerations-for-data.md)。  
