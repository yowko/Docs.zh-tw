...
ms.locfileid: "79184671"
---
# <a name="large-data-and-streaming"></a>大型資料與資料流

Windows 通信基礎 （WCF） 是基於 XML 的通信基礎結構。 由於XML資料通常以[XML 1.0規範](https://www.w3.org/TR/REC-xml/)中定義的標準文本格式編碼，因此連接的系統開發人員和架構師通常關注通過網路發送的消息的線佔用空間（或大小），而基於文本的XML編碼對二進位資料的有效傳輸提出了特殊挑戰。  
  
## <a name="basic-considerations"></a>基本考量  
 為了提供有關 WCF 以下資訊的背景資訊，本節重點介紹了通常適用于連接的系統基礎結構的編碼、二進位資料和流式處理的一些一般問題和注意事項。  
...
### <a name="encoding-data-text-vs-binary"></a>資料編碼：文字與二進位的比較  
 ...
 XML 文字訊息有一項無庸置疑的優點，就是它們是以標準為基礎，並提供最廣泛的互通性選項和平台支援。 有關詳細資訊，請參閱本主題後面的"編碼"部分。  
...
### <a name="binary-content"></a>二進位內容  
 ...
 為避免這項編碼額外負荷，訊息傳輸最佳化機制 (MTOM) 標準允許外顯化 (Externalizing) 包含在訊息中的大型資料項目，並在不使用任何特殊編碼的情況下以二進位資料包含在訊息中。 使用 MTOM，郵件以類似方式交換，類似于帶有附件或嵌入內容的電子郵件（圖片和其他嵌入內容）;MTOM 消息打包為多部分/相關的 MIME 序列，根部分是實際的 SOAP 訊息。  
 ...
 同樣的，如同 Base64，MTOM 也有一些 MIME 格式的必要額外負荷，因此只有在二進位資料項目的大小超過約 1 KB 時，才會看到使用 MTOM 的優點。 由於額外負荷之故，如果二進位承載保持在該臨界值之下，以 MTOM 編碼的訊息可能會大於使用 Base64 編碼二進位資料的訊息。 有關詳細資訊，請參閱本主題後面的"編碼"部分。  
  
### <a name="large-data-content"></a>大型資料內容  
 除了網路使用量以外，先前提到的 500 MB 承載也會為服務和用戶端帶來很大的本機挑戰。 預設情況下，WCF 在*緩衝模式下*處理消息。 這表示在傳送之前或接收之後，訊息的整個內容都會存在於記憶體中。 雖然這對大部分的案例都是一個很好的策略，而且對於訊息功能 (例如數位簽章和可靠的傳遞) 是必要的，但是大型訊息可能會很容易耗盡系統的資源。  
...
- 當初始化傳輸時，無法完整提供使用。  
 ...
 對於沒有這些條件約束的資料，通常比一個大型訊息更能夠在一個工作階段的範圍內傳送訊息的序列。 有關詳細資訊，請參閱本主題後面的"流資料"部分。  
  
 發送大量資料時，您需要設置`maxAllowedContentLength`IIS 設置（有關詳細資訊，請參閱配置[IIS 請求限制](https://docs.microsoft.com/iis/configuration/system.webServer/security/requestFiltering/requestLimits/)）和`maxReceivedMessageSize`綁定設置（例如[System.ServiceModel.BasicHttpBinding.Max接收消息大小](xref:System.ServiceModel.HttpBindingBase.MaxReceivedMessageSize%2A)或<xref:System.ServiceModel.NetTcpBinding.MaxReceivedMessageSize%2A>）。 該`maxAllowedContentLength`屬性預設為 28.6 MB，`maxReceivedMessageSize`該屬性預設為 64KB。  
  
## <a name="encodings"></a>編碼方式  
 *編碼*定義了一組有關如何線上路上顯示消息的規則。 *編碼器*實現這樣的編碼，並在發送方負責將記憶體<xref:System.ServiceModel.Channels.Message>中轉換為可通過網路發送的位元組流或位元組緩衝區。 在接收者端，編碼器會將位元組序列變成記憶體中的訊息。  
  
 WCF 包括三個編碼器，並允許您編寫和插入您自己的編碼器（如有必要）。  
...
|編碼器繫結項目|描述|  
|-----------------------------|-----------------|  
|<xref:System.ServiceModel.Channels.TextMessageEncodingBindingElement>|文字訊息編碼器是所有 HTTP 繫結的預設編碼器，以及所有優先考量互通性之自訂繫結的適當選擇。 此編碼器不需特別處理二進位資料，即可讀取及撰寫標準 SOAP 1.1/SOAP 1.2 文字訊息。 如果消息<xref:System.ServiceModel.Channels.MessageVersion?displayProperty=nameWithType>的屬性設置為<xref:System.ServiceModel.Channels.MessageVersion.None?displayProperty=nameWithType>，則輸出中省略 SOAP 包絡包裝器，並且僅序列化郵件內文內容。|  
...
|<xref:System.ServiceModel.Channels.BinaryMessageEncodingBindingElement>|二進位消息編碼器是 Net® 綁定的預設編碼器，並且每當通信雙方都基於 WCF 時，都是適當的選擇。 二進位訊息編碼器是使用 .NET 二進位 XML 格式，這是 XML 資訊設定 (Infoset) 的 Microsoft 特定二進位表示法，通常會產生比同等的 XML 1.0 表示法更小的使用量，並將二進位資料編碼為位元組資料流。|  
...
### <a name="enabling-mtom"></a>啟用 MTOM  
 當互通性成為一項需求且必須傳送大型二進位資料時，MTOM 訊息編碼是可以在標準 <xref:System.ServiceModel.BasicHttpBinding> 或 <xref:System.ServiceModel.WSHttpBinding> 繫結程序上啟用的替代編碼策略，方法是將個別 `MessageEncoding` 屬性設定為 <xref:System.ServiceModel.WSMessageEncoding.Mtom> 或將 <xref:System.ServiceModel.Channels.MtomMessageEncodingBindingElement> 撰寫為 <xref:System.ServiceModel.Channels.CustomBinding>。 以下示例代碼（從[MTOM 編碼](../../../../docs/framework/wcf/samples/mtom-encoding.md)示例中提取）演示如何在配置中啟用 MTOM。  
...
### <a name="programming-model"></a>程式設計模型  
 不論您在應用程式中使用這三種內建編碼器的哪一種，程式設計的操作和傳輸二進位資料時完全相同。 區別在於 WCF 如何根據資料的資料類型處理資料。  
...
#### <a name="streaming-and-sessions"></a>資料流與工作階段  
 使用工作階段架構繫結對呼叫進行資料流處理時，可能會遇到未預期的行為。 即使使用的繫結已設定為使用工作階段，所有資料流處理呼叫還是會透過不支援工作階段的單一通道 (資料包通道) 來進行。 如果多個用戶端透過工作階段架構繫結對同一個服務物件進行資料流處理呼叫，而且服務物件的並行模式已設為單一且其執行個體內容模式已設為 PerSession，則所有呼叫必須經過資料包通道，所以一次只能處理一個呼叫。 然後，一個或多個用戶端可能會超時。通過將服務物件的實例上下文模式設置為 PerCall 或將併發設置為"多個"來解決此問題。  
...
```  
 ...
 您可以單獨在通訊方的任一端為要求和回覆或雙向開啟資料流，而不影響功能。 然而，您應該永遠假設傳輸資料大小太過龐大，因此在通訊連結的兩個端點上要調整啟用資料流。 對於未使用 WCF 實現其中一個端點的跨平臺通信，使用流式處理功能取決於平臺的流式處理功能。 另一個少見的例外狀況可能是記憶體消耗導向的案例，其中用戶端或服務必須將工作集減至最小，且只能負荷小量的緩衝區大小。  
...
```  
 ...
 當資料流到達檔案結尾 (EOF) 時，資料流傳輸會結束且訊息會關閉。 發送消息（傳回值或叫用作業）時，可以傳遞<xref:System.IO.FileStream>和 WCF 基礎結構隨後從該流提取所有資料，直到流完全讀取並到達 EOF。 如果要傳輸來源中沒有此類預先建立之 <xref:System.IO.Stream> 衍生類別的資料流資料，請建構此類別，將該類別覆疊在您的資料流來源上，然後用來做為引數或傳回值。  
  
 接收消息時，WCF 會通過 Base64 編碼的郵件內文內容（如果使用 MTOM 則構建相應的 MIME 部分）的流，並且在讀取內容時，流到達 EOF。  
...
### <a name="special-security-considerations-for-large-data"></a>大型資料的特殊安全性考量  
 所有的繫結都可讓您限制傳入訊息的大小，以防止阻絕服務攻擊。 例如<xref:System.ServiceModel.BasicHttpBinding>，公開[一個系統.ServiceModel.BasicHttpBinding.MaxGetGetMessageSize](xref:System.ServiceModel.HttpBindingBase.MaxReceivedMessageSize%2A)屬性，這些屬性對傳入消息的大小進行了限制，因此也限制在處理消息時訪問的最大記憶體量。 這個單位的設定是位元組，預設值是 65,536 位元組。  
  
 大型資料流案例特定的安全性威脅會造成資料在接收者預期進行資料流處理時，進行緩衝處理，引起阻絕服務。 例如，WCF 始終緩衝消息的 SOAP 標頭，因此攻擊者可能會構造一個完全由標頭組成的大型惡意消息，以強制對資料進行緩衝。 當啟用資料流時，`MaxReceivedMessageSize` 可能會設定為極大值，因為接收者絕對不會預期在記憶體中一次緩衝處理整個訊息。 如果 WCF 被迫緩衝消息，則會發生記憶體溢出。  
  
 因此，在這種情況中，限制傳入訊息大小上限是不夠的。 該`MaxBufferSize`屬性需要約束 WCF 緩衝的記憶體。 重要的是在資料流處理時，將它設定為安全值 (或保留為預設值)。 例如，假設您的服務必須接收大小高達 4 GB 的檔案，然後儲存在本機磁碟上。 也請假設您的記憶體受到一次只能緩衝處理 64 KB 資料的限制。 然後您會將 `MaxReceivedMessageSize` 設定為 4 GB，而將 `MaxBufferSize` 設定為 64 KB。 同時，在您的服務實作中，必須確保您只從 64 KB 區塊的傳入資料流讀取，而且在前一個區塊寫入磁碟並從記憶體捨棄之前，不會讀取下一個區塊。  
  
 還必須瞭解，此配額僅限制 WCF 完成的緩衝，無法保護您免受在您自己的服務或用戶端實現中執行的任何緩衝。 有關其他安全注意事項的詳細資訊，請參閱[資料的安全注意事項](../../../../docs/framework/wcf/feature-details/security-considerations-for-data.md)。  
  
> [!NOTE]
> 使用緩衝或資料流傳輸是由端點處決定。 如果是 HTTP 傳輸，傳輸模式不會在連線上傳播，或是在 Proxy 伺服器與其他媒介之間進行傳播。 設定傳輸模式不會反映在服務介面的描述中。 將 WCF 用戶端生成到服務後，必須編輯用於與資料流一起使用的服務的設定檔以設置模式。 如果是 TCP 和具名管道傳輸，會傳播傳輸模式做為原則判斷提示。  
