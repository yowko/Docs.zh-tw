...
-   如果用戶端流動交易，則作業 (包括 Enterprise Services 元件的呼叫) 會在該交易的範圍內執行。 使用 <xref:System.Transactions.EnterpriseServicesInteropOption.Full> 會確保異動與 <xref:System.EnterpriseServices> 內容為同步處理，也就是說 <xref:System.Transactions> 的環境異動和 <xref:System.EnterpriseServices> 會是相同的。  
  
-   如果用戶端沒有流動交易，此時將 <xref:System.ServiceModel.OperationBehaviorAttribute.TransactionScopeRequired%2A> 設定為 `true` 便會建立供作業使用的新交易範圍。 同樣地，使用 <xref:System.Transactions.EnterpriseServicesInteropOption.Full> 可確保作業的異動與 <xref:System.EnterpriseServices> 元件內容中所使用的異動是相同的。  
...
```  
 ...
 如果作業的目前異動和 Enterprise Services 異動元件的呼叫之間不需要同步處理，那麼請在初始化 <xref:System.Transactions.EnterpriseServicesInteropOption.None> 執行個體時使用 <xref:System.Transactions.TransactionScope> 選項。  
