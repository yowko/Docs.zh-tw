...
ms.locfileid: "33507109"
---
# <a name="wcf-web-http-programming-model-overview"></a>WCF Web HTTP 程式設計模型概觀
Windows Communication Foundation (WCF) WEB HTTP 程式設計模型提供建置使用 WCF WEB HTTP 服務所需的基本項目。 WCF WEB HTTP 服務的設計可供最廣泛用戶端，包括 Web 瀏覽器存取，並具有下列獨特需求：  
  
-   **Uri 和 URI 處理**Uri 在 WEB HTTP 服務的設計中扮演重要角色。 WCF WEB HTTP 程式設計模型運用<xref:System.UriTemplate>和<xref:System.UriTemplateTable>類別，以提供 URI 處理能力。  
  
-   **對 GET 和 POST 作業支援**WEB HTTP 服務運用了 GET 動詞來擷取資料，除了各種叫用用於資料修改與遠端叫用的動詞。 WCF WEB HTTP 程式設計模型運用<xref:System.ServiceModel.Web.WebGetAttribute>和<xref:System.ServiceModel.Web.WebInvokeAttribute>來將服務作業同時與 GET 及其他 HTTP 動詞命令，例如 PUT、 POST 和 DELETE。  
  
-   **多種資料格式**Web 樣式服務能夠處理許多種類的資料，包括 SOAP 訊息。 WCF WEB HTTP 程式設計模型運用<xref:System.ServiceModel.WebHttpBinding>和<xref:System.ServiceModel.Description.WebHttpBehavior>以支援多種不同的資料格式包括 XML 文件、 JSON 資料物件，以及資料流的二進位內容，例如影像、 視訊檔案或純文字。  
  
 WCF WEB HTTP 程式設計模型會擴充 WCF Web 樣式案例之包含 WEB HTTP 服務、 AJAX 和 JSON 服務，以及新聞訂閱 (ATOM/RSS) 摘要的觸達。 如需 AJAX 和 JSON 服務的詳細資訊，請參閱[AJAX 整合與 JSON 支援](../../../../docs/framework/wcf/feature-details/ajax-integration-and-json-support.md)。 如需新聞訂閱的詳細資訊，請參閱[WCF 新聞訂閱概觀](../../../../docs/framework/wcf/feature-details/wcf-syndication-overview.md)。  
...
> [!CAUTION]
>  WCF WEB HTTP 程式設計模型以撰寫的服務不會使用 SOAP 訊息。 因為未使用 SOAP，即無法使用由 WCF 所提供的安全性功能。 不過，您可透過以 HTTPS 裝載服務的方式來使用傳輸型安全性。 如需有關 WCF 安全性的詳細資訊，請參閱[安全性概觀](../../../../docs/framework/wcf/feature-details/security-overview.md)  
...
-   `Bind`() 和 `Match`() 都是反向值，因此您可以呼叫 `Match`( `Bind`( x ) ) 並從一開始的相同環境重新開始。  
 ...
 在許多情況下 (特別是在伺服器上，需要根據 URI 將要求分派到服務作業時) 您都想要追蹤資料結構中可以獨立處理每一個包含的範本的 <xref:System.UriTemplate> 物件集合。 <xref:System.UriTemplateTable> 表示一組 URI 範本，並且依據一組指定的範本及候選 URI 選取最佳對象。 這不會關聯到任何特定的網路堆疊 （包含 WCF） 讓您可以在必要時使用它。  
  
 WCF 服務模型會透過 <xref:System.UriTemplate> 和 <xref:System.UriTemplateTable> 將服務作業與 <xref:System.UriTemplate> 所描述的 URI 集合關聯在一起。 服務作業會透過 <xref:System.UriTemplate> 或 <xref:System.ServiceModel.Web.WebGetAttribute>，與 <xref:System.ServiceModel.Web.WebInvokeAttribute> 產生關聯。 如需有關<xref:System.UriTemplate>和<xref:System.UriTemplateTable>，請參閱[UriTemplate 與 UriTemplateTable](../../../../docs/framework/wcf/feature-details/uritemplate-and-uritemplatetable.md)  
  
## <a name="webget-and-webinvoke-attributes"></a>WebGet 和 WebInvoke 屬性  
 WCF WEB HTTP 服務使用擷取動詞 (例如 HTTP GET) 除了各種叫用動詞 （例如 HTTP POST、 PUT 和 DELETE）。 WCF WEB HTTP 程式設計模型可讓服務開發人員控制的兩個 URI 範本及與其服務作業，與相關聯的動詞命令<xref:System.ServiceModel.Web.WebGetAttribute>和<xref:System.ServiceModel.Web.WebInvokeAttribute>。 <xref:System.ServiceModel.Web.WebGetAttribute> 和 <xref:System.ServiceModel.Web.WebInvokeAttribute> 可讓您控制個別作業如何繫結至 URI 及與這些 URI 相關聯的 HTTP 方法。 例如，在下列程式碼中新增 <xref:System.ServiceModel.Web.WebGetAttribute> 和 <xref:System.ServiceModel.Web.WebInvokeAttribute>。  
...
```  
 ...
 若要查看完整的範例會使用 WCF WEB HTTP 程式設計模型的 WCF 服務，請參閱[How to： 建立基本的 WCF Web HTTP 服務](../../../../docs/framework/wcf/feature-details/how-to-create-a-basic-wcf-web-http-service.md)  
