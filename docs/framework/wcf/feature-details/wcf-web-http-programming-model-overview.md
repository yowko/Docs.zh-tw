...
## <a name="webget-and-webinvoke-attributes"></a>WebGet 和 WebInvoke 屬性  
 除了各種調用動詞（例如 HTTP POST、PUT 和 DELETE）之外，WCF WEB HTTP 服務也會使用抓取動詞（例如 HTTP GET）。 WCF WEB HTTP 程式設計模型可讓服務開發人員使用 <xref:System.ServiceModel.Web.WebGetAttribute> 和 <xref:System.ServiceModel.Web.WebInvokeAttribute>，控制與其服務作業相關聯的 URI 範本和動詞。 <xref:System.ServiceModel.Web.WebGetAttribute> 和 <xref:System.ServiceModel.Web.WebInvokeAttribute> 可讓您控制個別作業如何繫結至 URI 及與這些 URI 相關聯的 HTTP 方法。 例如，在下列程式碼中新增 <xref:System.ServiceModel.Web.WebGetAttribute> 和 <xref:System.ServiceModel.Web.WebInvokeAttribute>。  
