...
3. 這  
 ...
 當 IDN 設定設為 "None" 時, Uri. Host 或 Uri.dnssafehost 不會執行任何轉換。 當 IDN 設定為 "All" 時, uri 為。主機保持 Unicode 和 uri。Uri.dnssafehost 會轉換成 Punycode。 當 IDN 設定為 "AllExceptIntranet" 時, uri 為。Uri.dnssafehost 會轉換成 Punycode 的網際網路位址, 並針對內部網路位址保留 Unicode。 這個設定對正確解析 DNS 名稱很重要。 請注意，Windows 8 和較新的版本不需要進行這個設定。  
