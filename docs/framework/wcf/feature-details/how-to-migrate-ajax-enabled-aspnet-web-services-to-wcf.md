---
title: "HOW TO：將啟用 AJAX 的 ASP.NET Web 服務移轉至 WCF | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
...
-   您可以從 [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] 功能獲益，例如追蹤等等。  
 ...
 [!INCLUDE[crabout](../../../../includes/crabout-md.md)]透過啟用 AJAX 的端點公開 [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] 服務的詳細資訊，請參閱[HOW TO：使用組態新增 ASP.NET AJAX 端點](../../../../docs/framework/wcf/feature-details/how-to-use-configuration-to-add-an-aspnet-ajax-endpoint.md)主題。  
...
5.  請從 **\[建置\]** 功能表上，選取 **\[建置方案\]**。  
  
6.  從 \[**偵錯**\] 功能表中，選取 \[**啟動但不偵錯**\]。  
  
7.  在產生的網頁中，選取 `HelloWorld` 作業。  
  
8.  在 測試頁上按一下 \[`HelloWorld`**叫用**\] 按鈕。  您應該會收到下列 XML 回應。  
...
6.  將 `Namespace` 的 <xref:System.ServiceModel.ServiceContractAttribute> 屬性指定為 `WCFHello`。  
...
7.  將 <xref:System.ServiceModel.Web.WebInvokeAttribute> 新增至 `HelloWorld` 作業，並設定 <xref:System.ServiceModel.Web.WebInvokeAttribute.ResponseFormat%2A> 屬性以傳回 <xref:System.ServiceModel.Web.WebMessageFormat>。  請注意，如果沒有設定，預設傳回型別為 <xref:System.ServiceModel.Web.WebMessageFormat>。  
...
11. `WCFHello.svc/HelloWorld` 和 `Service1.aspx/HelloWorld` 端點現在的功能是相等的。  
...
-   {“one”:1,”two”:2}，透過 ASP.NET AJAX <xref:System.Web.Script.Serialization.JavascriptSerializer>  
...
|<xref:System.DBNull.Value> 的序列化|{} \(or {"\_\_type":"\#System"}\)|Null|  
