...
本主題概述將基本 ASP.NET AJAX 服務遷移至對等 Windows Communication Foundation (WCF) 服務之啟用 AJAX 的程式。 它會顯示如何建立 ASP.NET AJAX 服務的功能對等 WCF 版本。 然後，您可以並存使用這兩個服務，也可以使用 WCF 服務來取代 ASP.NET AJAX 服務。
...
- 您可以從 WCF 功能獲益，例如追蹤等等。
 ...
 如需透過啟用 AJAX 的端點公開 WCF 服務的詳細資訊，請參閱 how [to：使用設定新增 ASP.NET AJAX 端點](how-to-use-configuration-to-add-an-aspnet-ajax-endpoint.md) 主題。

### <a name="to-create-and-test-the-aspnet-web-service-application"></a>若要建立並測試 ASP.NET Web 服務應用程式

1. 開啟 Visual Studio 2012。

2. **在 [檔案**] 功能表中，依序選取 [**新增**]、[**專案**]、[ **Web**]，然後選取 [ **ASP.NET Web 服務應用程式**]。

3. 為專案命名 `ASPHello` ，然後按一下 **[確定]**。
...
8. 在 [測試] 頁面上，按一下 [叫用 **] 按鈕** `HelloWorld` 。 您應該會收到下列 XML 回應。
...
5. 在 WCFHello.svc.cs 檔案中貼上已複製的作業執行，以 `HelloWorld` 取代下列程式碼。
...
6. 將的 `Namespace` 屬性指定 <xref:System.ServiceModel.ServiceContractAttribute> 為 `WCFHello` 。
...
7. 將加入 <xref:System.ServiceModel.Web.WebInvokeAttribute> 至作業 `HelloWorld` ，並將 <xref:System.ServiceModel.Web.WebInvokeAttribute.ResponseFormat%2A> 屬性設定為 return <xref:System.ServiceModel.Web.WebMessageFormat.Xml> 。 請注意，如果沒有設定，預設傳回型別為 <xref:System.ServiceModel.Web.WebMessageFormat.Json>。
...
9. 開啟 Wcfhello.svc .svc 檔案，並在 [ **調試** ] 功能表中選取 [ **啟動但不進行調試**]。

10. 服務現在會公開中的端點 `WCFHello.svc/HelloWorld` ，以回應 HTTP POST 要求。 HTTP POST 要求無法從瀏覽器來測試，但是端點會傳回下列 XML。
...
```
 ...
 如果將 .ASMX Web 服務升級並並行遷移至 WCF 服務，請避免將兩個類型對應至用戶端上的相同名稱。 如果在 <xref:System.Web.Services.WebMethodAttribute> 和 <xref:System.ServiceModel.ServiceContractAttribute> 中使用相同的型別，會造成序列化程式中的例外狀況：

- 如果先加入 WCF 服務，在 .ASMX Web 服務上叫用方法會造成中的例外狀況， <xref:System.Web.UI.ObjectConverter.ConvertValue%28System.Object%2CSystem.Type%2CSystem.String%29> 因為 proxy 中順序的 WCF 樣式定義優先。

- 如果先加入 .ASMX Web 服務，則在 WCF 服務上叫用方法會造成中的例外狀況， <xref:System.Runtime.Serialization.Json.DataContractJsonSerializer> 因為 proxy 中順序的 Web 服務樣式定義優先。
...
- {"one"：1，"二"： 2} ASP.NET AJAX <xref:System.Web.Script.Serialization.JavaScriptSerializer>
...
|<xref:System.DateTime> 以 Coordinated Universal Time (UTC) 時間計算|不支援格式 " \\ /Date (123456789U) \\ /" 或 " \\ /Date \\ ( \d + (U&#124; (\\ + \\ -[\d {4} ] ) # A6？ \\) \\ \\ /) 」。|支援格式 " \\ /Date (123456789U) \\ /" 和 " \\ /Date \\ ( \d + (U&#124; (\\ + \\ -[\d {4} ] ) # A6？ \\) \\ \\ /) "作為日期時間值。|
|字典表示法|KeyValuePair 的陣列 \<K,V> ，可處理非字串的索引鍵類型。|做為實際的 JSON 物件 - 但只能處理屬於字串的金鑰型別。|
|逸出字元|一律使用逸出正斜線 (/)；絕不允許使用未逸出的無效 JSON 字元，例如 "\n"。|使用逸出斜線 (/) 做為 DateTime 值。|

## <a name="see-also"></a>另請參閱

- [作法：使用組態新增 ASP.NET AJAX 端點](how-to-use-configuration-to-add-an-aspnet-ajax-endpoint.md)
