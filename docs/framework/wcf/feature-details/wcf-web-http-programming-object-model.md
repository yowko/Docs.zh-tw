...
ms.locfileid: "96266879"
---
# <a name="wcf-web-http-programming-object-model"></a>WCF Web HTTP 程式設計物件模型

WCF WEB HTTP 程式設計模型可讓開發人員透過基本 HTTP 要求公開 Windows Communication Foundation (WCF) Web 服務，而不需要 SOAP。 WCF WEB HTTP 程式設計模型是以現有的 WCF 擴充性模型為基礎。 這個模型會定義下列類別：  
...
## <a name="uritemplate"></a>UriTemplate  
 ...
 <xref:System.UriTemplate> 類別可讓您定義一組結構類似的 URI。 樣板中則包含兩個部分：路徑和查詢。 路徑則包含一系列以斜線 (/) 分隔的區段。 每個區段都可以有常值、在大括弧 [{}] 內 (寫入的變數值、限制為只符合一個區段) 的內容，或以星號 [] 表示的萬用字元 (以星號 [) ] 表示，它 \* 必須出現在路徑結尾。 可以完全省略查詢運算式。 如果出現，會指定一系列非循序的名稱/值組。 查詢運算式的專案可以是常值組 (？ x = 2) 或變數配對 (？ x = {*value*} ) 。 不允許使用未成對的值。 <xref:System.UriTemplate> WCF WEB HTTP 程式設計模型會在內部使用，以將特定 Uri 或 Uri 群組對應至服務作業。  
...
### <a name="extending-webhttpbehavior"></a>擴充 WebHttpBehavior  
 ...
 <xref:System.ServiceModel.Description.WebScriptEnablingBehavior> 是擴充 <xref:System.ServiceModel.Description.WebHttpBehavior> 的範例。 <xref:System.ServiceModel.Description.WebScriptEnablingBehavior> 讓 Windows Communication Foundation (WCF) 端點從瀏覽器型 ASP.NET AJAX 用戶端接收 HTTP 要求。 [使用 HTTP POST 的 AJAX 服務](../samples/ajax-service-using-http-post.md)是使用此擴充點的範例。  
...
因為 WCF WEB HTTP 程式設計模型不支援 WS-* 通訊協定，保護以 WCF WEB HTTP 程式設計模型為基礎之 Web 服務的唯一方法，就是使用 SSL 公開您的服務。 如需使用 IIS 7.0 設定 SSL 的詳細資訊，請參閱 [如何在 iis 中執行 ssl](https://support.microsoft.com/help/299875/how-to-implement-ssl-in-iis)。
