...
一般來說，當您建立 Windows Communication Foundation (WCF) 應用程式時，開發人員會密切注意資料結構和序列化問題，而不需要擔心資料的訊息結構。 針對這類應用程式，建立參數的資料合約或傳回值是很明確的。  (需詳細資訊，請參閱 [指定服務合約中的資料傳輸](specifying-data-transfer-in-service-contracts.md)) 。  
  
 但是，有時候完整控制 SOAP 訊息的結構與控制其內容一樣重要。 當互通性很重要，或是要具體控制在訊息層級或訊息部分的安全性問題時，這就顯得特別重要。 在這些情況下，您可以建立 *訊息合約* ，讓您指定所需的精確 SOAP 訊息的結構。  
  
 此主題會討論如何使用各種訊息合約屬性，以建立作業的特定訊息合約。  
  
## <a name="using-message-contracts-in-operations"></a>在作業中使用訊息合約  

 WCF 支援在遠端程序呼叫上建立模型的作業， *(RPC) 樣式* 或 *訊息樣式*。 在 RPC 樣式作業中，您可以使用任何可序列化類型，並且有存取本機呼叫的功能，例如多個參數以及 `ref` 與 `out` 參數。 在這個樣式中，選擇的序列化形式會控制基礎訊息中的資料結構，而 WCF 執行時間會建立訊息以支援作業。 這可以讓不熟悉 SOAP 和 SOAP 訊息的程式開發人員，快速而輕鬆地建立與使用服務應用程式。  
...
```  
 ...
 通常資料合約已足夠定義訊息的結構描述。 例如，在之前的範例中，如果 `BankingTransaction` 和 `BankingTransactionResponse` 都有定義基礎 SOAP 訊息內容的資料合約，對大部分的應用程式來說就已足夠。 如需資料合約的詳細資訊，請參閱 [使用資料合約](using-data-contracts.md)。  
...
## <a name="using-custom-types-inside-message-contracts"></a>在訊息合約中使用自訂類型  
 ...
 每個個別訊息標頭與訊息本文部分，會針對使用訊息的服務合約，使用選擇的序列化引擎進行序列化 (轉變成 XML)。 預設的序列化引擎 `XmlFormatter` 能夠處理擁有資料合約的任何類型，不論是使用明確的方式 (藉由擁有 <xref:System.Runtime.Serialization.DataContractAttribute?displayProperty=nameWithType>) 或隱含的方式 (藉由成為基本型別、擁有 <xref:System.SerializableAttribute?displayProperty=nameWithType> 等等)。 如需詳細資訊，請參閱 [使用資料合約](using-data-contracts.md)。  
...
- `Relay`  
 ...
 `Actor` 或 `Role` 屬性會定義指定標頭想要之節點的統一資源識別元 (URI)。 `MustUnderstand` 屬性會指定處理標頭的節點是否必須識別它。 `Relay` 屬性會指定標頭是否要轉送至下游節點。 除了 `MustUnderstand` 本主題稍後的「訊息合約版本設定」一節中所指定的屬性以外，WCF 不會對傳入訊息執行這些屬性的任何處理。 但是，它允許您依需要讀取和寫入這些屬性，如同下列所描述。  
...
- WCF 不會將物件指向遺漏的標頭，而對應的成員會保留為其預設值。  
  
- WCF 也會忽略非預期的額外標頭。 這個規則的其中一個例外狀況是，如果在傳入 SOAP 訊息中的額外標頭將 `MustUnderstand` 屬性設定為 `true`，在這樣的情況下，因為無法處理必須解讀的標頭而會發生例外狀況。  
...
## <a name="wsdl-considerations"></a>WSDL 考慮事項  
 ...
 當從使用訊息合約的服務產生 Web 服務描述語言 (WSDL) 合約時，重要的是並非所有訊息合約功能都會反應在產生的 WSDL 中。 請考慮下列幾點：  
...
## <a name="soap-encoding-considerations"></a>SOAP 編碼考量  
 ...
 WCF 可讓您使用舊版 SOAP 編碼樣式的 XML，但不建議使用它。 當使用這個樣式時 (藉由將套用至服務合約之 `Use` 的 `Encoded` 屬性設定為 <xref:System.ServiceModel.XmlSerializerFormatAttribute?displayProperty=nameWithType>)，則適用下列其他考量：  
