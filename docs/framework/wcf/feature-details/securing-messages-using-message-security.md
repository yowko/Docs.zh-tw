---
title: "使用訊息安全性來保護訊息的安全 | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
...
---
# 使用訊息安全性來保護訊息的安全
本章節會討論使用 <xref:System.ServiceModel.NetMsmqBinding> 時的 [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] 訊息安全性。  
...
### 憑證  
 ...
 在執行 Windows 的電腦上，憑證會保存在幾種存放區中。[!INCLUDE[crabout](../../../../includes/crabout-md.md)]不同存放區的詳細資訊，請參閱[憑證存放區](http://go.microsoft.com/fwlink/?LinkId=87787)。  
...
### Windows  
 ...
 針對佇列通訊使用 Kerberos 通訊協定時所存在的一個問題，就是包含金鑰發佈中心 \(KDC\) 所發佈之用戶端身分識別的票證相對存留較短。「*存留時間*」\(Lifetime\) 與 Kerberos 票證 \(表示票證的有效性\) 有關聯性。因此，若為高度幕後性，您就無法確定該權杖對負責驗證用戶端的服務是否仍然有效。  
  
 請注意，當使用這個認證類型時，服務必須在 SERVICE 帳戶下執行。  
  
 選擇訊息認證時會預設使用 Kerberos 通訊協定。[!INCLUDE[crdefault](../../../../includes/crdefault-md.md)][探索 Kerberos，Windows 2000 中分散式安全性的通訊協定](http://go.microsoft.com/fwlink/?LinkId=87790) \(本頁面可能為英文\)。  
