...
此主題提供聯合安全性概念的簡短概觀。 它還介紹了 Windows 通信基礎 (WCF) 對部署聯合安全體系結構的支援。 有關示範以展示的範例應用程式,請參閱[聯合範例](../../../../docs/framework/wcf/samples/federation-sample.md)。  
...
## <a name="support-for-federated-security-in-wcf"></a>在 WCF 中支援聯合安全性  
 ...
 ws-FiHttpBinding>元素提供了一個安全、可靠、可互操作的綁定,需要使用 HTTP 作為請求-答覆通信樣式的基礎傳輸機制,使用文本和 XML 作為編碼[\<](../../../../docs/framework/configure-apps/file-schema/wcf/wsfederationhttpbinding.md)的線格式。  
...
### <a name="phase-1-design-phase"></a>階段 1：設計階段  
 在設計階段,用戶端使用[ServiceModel 中數據實用程式工具 (Svcutil.exe)](../../../../docs/framework/wcf/servicemodel-metadata-utility-tool-svcutil-exe.md)讀取服務終結點公開的策略並收集服務的身份驗證和授權要求。 建構適當的 Proxy 以便在用戶端建立下列聯合安全性通訊模式：  
...
### <a name="phase-2-run-time-phase"></a>階段 2：執行階段  
 在運行時階段,用戶端實例化 WCF 用戶端類的物件並使用 WCF 用戶端進行調用。 WCF 的基礎框架處理聯合安全通信模式中前面提到的步驟,並使客戶端能夠無縫地使用服務。  
  
## <a name="sample-implementation-using-wcf"></a>使用 WCF 的範例實作  
 下圖顯示了使用 WCF 的本機支援的聯合安全體系結構的範例實現。  
...
### <a name="example-myservice"></a>範例 MyService  
 ...
 服務終結點`MyServiceEndpoint`使用[\<ws-IaHBinding>,](../../../../docs/framework/configure-apps/file-schema/wcf/wsfederationhttpbinding.md)並且需要使用`accessAuthorized`STS B 發出 的聲明的有效安全斷言標記語言 (SAML) 權杖。這在服務配置中聲明性地指定。  
