...
此主題提供聯合安全性概念的簡短概觀。 其中也說明部署同盟安全性架構的 Windows Communication Foundation （WCF）支援。 如需示範同盟的範例應用程式，請參閱[同盟範例](../samples/federation-sample.md)。  
...
## <a name="support-for-federated-security-in-wcf"></a>在 WCF 中支援聯合安全性  
 WCF 提供透過來部署聯合安全性架構的全包式支援 [\<wsFederationHttpBinding>](../../configure-apps/file-schema/wcf/wsfederationhttpbinding.md) 。  
  
 [\<wsFederationHttpBinding>](../../configure-apps/file-schema/wcf/wsfederationhttpbinding.md)元素提供安全、可靠且互通的系結，此系結需要使用 HTTP 做為要求-回復通訊樣式的基礎傳輸機制，並採用文字和 XML 作為編碼的電傳格式。  
...
### <a name="phase-1-design-phase"></a>階段 1：設計階段  
 在設計階段期間，用戶端會使用[System.servicemodel 中繼資料公用程式工具（Svcutil）](../servicemodel-metadata-utility-tool-svcutil-exe.md)來讀取服務端點所公開的原則，並收集服務的驗證和授權需求。 建構適當的 Proxy 以便在用戶端建立下列聯合安全性通訊模式：  
...
### <a name="example-myservice"></a>範例 MyService  
 ...
 服務端點會 `MyServiceEndpoint` 使用 [\<wsFederationHttpBinding>](../../configure-apps/file-schema/wcf/wsfederationhttpbinding.md) ，並且需要有效的安全性判斷提示標記語言（SAML）權杖搭配 `accessAuthorized` STS B 所發行的宣告。這會在服務設定中以宣告方式指定。  
...
> [!NOTE]
> 同樣地，宣告 `userAuthenticated` 是 STS B 所需的宣告類型。此宣告類型的完整名稱是 `http://tempuri.org:userAuthenticated` （連同相關聯的命名空間），用於 STS 設定檔中。 這個宣告的值表示這個宣告存在，並且假設 STS A 會將它設定為 `true`。  
...
#### <a name="sts-a"></a>STS A  
 ...
 與 STS B 類似，STS A 也是發行安全性權杖，並針對此目的公開單一端點的 Web 服務。 不過，它會使用不同的系結（ `wsHttpBinding` ），並要求使用者出示具有宣告的有效 CardSpace `emailAddress` 。 它會使用 `userAuthenticated` 宣告發行 SAML 權杖當做回應。 在服務組態中會以宣告方式指定。  
