...
- 若要從自訂的寄不出的信件佇列讀取訊息，URI 的格式必須是： net. msmq：//localhost/private/ \<*custom-dlq-name*> ，其中*dlq*是自訂的寄不出的信件佇列的名稱。  
  
 如需如何定址佇列的詳細資訊，請參閱[服務端點和佇列定址](service-endpoints-and-queue-addressing.md)。  
...
## <a name="example"></a>範例  
 下列範例會示範如何建立寄不出的信件佇列，以及如何使用該佇列來處理過期的訊息。 此範例是以[如何：與 WCF 端點交換佇列訊息](how-to-exchange-queued-messages-with-wcf-endpoints.md)中的範例為基礎。 下列範例會示範如何撰寫訂單處理服務的用戶端程式碼，該服務會分別針對每個應用程式使用寄不出的信件佇列。 這個範例也會示範如何處理寄不出的信件佇列中的訊息。  
