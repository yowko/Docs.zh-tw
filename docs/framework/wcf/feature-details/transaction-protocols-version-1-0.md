---
title: "交易通訊協定 1.0 版 | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
...
---
# 交易通訊協定 1.0 版
[!INCLUDE[indigo1](../../../../includes/indigo1-md.md)] 第 1 版實作了 WS\-Atomic 交易與 WS\-Coordination 通訊協定 1.0 版。[!INCLUDE[crabout](../../../../includes/crabout-md.md)] 1.1 版的詳細資訊，請參閱[交易通訊協定](../../../../docs/framework/wcf/feature-details/transaction-protocols.md)。  
...
|WS\-AtomicTransaction|http:\/\/msdn.microsoft.com\/ws\/2005\/08\/ws\-atomictransaction\/|  
 ...
 下圖描述兩個交易管理員之間：交易管理員 1 和交易管理員 2，以及兩個應用程式之間：應用程式 1 和應用程式 2 的互通性。  
  
 ![傳輸通訊協定](../../../../docs/framework/wcf/feature-details/media/transactionmanagers.gif "TransactionManagers")  
...
#### 2PC 通訊協定繫結組態  
 ...
 B2131：實作必須支援 `wsa:ReferenceParameters`，如同 WS\-Addressing 中所述以達到 [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] 的 2PC 訊息相互關聯。  
  
### 交易管理員混合安全性繫結  
 這是個替代 \(混合模式\) 繫結，會針對身分識別建立目的同時使用傳輸安全性和 WS\-Coordination 發行權杖模型。啟動與登錄是兩個繫結之間唯一不同的項目。  
  
#### HTTPS 傳輸組態  
 X.509 憑證會用來建立交易管理員身分識別。需要用戶端\/伺服器驗證，而用戶端\/伺服器授權則留待實作詳細資料中說明。  
  
#### 啟動訊息繫結組態  
 啟動訊息通常不會參與互通性，因為啟動訊息一般會發生在應用程式與其本機交易管理員之間。  
  
 B1221：[!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] 會在啟動訊息使用雙向 HTTPS 繫結 \(在[訊息通訊協定](../../../../docs/framework/wcf/feature-details/messaging-protocols.md)中有描述\)。要求與回覆訊息會使用 WS\-Addressing 2004\/08 來相互關聯。  
  
 第 8 節的 WS\-Atomic 交易規格進一步描述有關相互關聯與訊息交換模式的詳細資料。  
  
-   R1222：在接收到 `CreateCoordinationContext` 後，協調器必須使用關聯的密碼 `STx` 發行 `SecurityContextToken`。在符合 WS\-Trust 規格的 `t:IssuedTokens` 標頭中會傳回這個權杖。  
  
-   R1223：如果啟動發生在現有的協調內容中，則使用與現有內容關聯之 `SecurityContextToken` 的 `t:IssuedTokens` 標頭就必須在 `CreateCoordinationContext` 訊息上流通。  
  
 應該產生新的 `t:IssuedTokens` 標頭以便附加至傳出的 `wscoor:CreateCoordinationContextResponse`  訊息。  
  
#### 登錄訊息繫結組態  
 B1231：[!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] 使用雙向 HTTPS 繫結 \(在[訊息通訊協定](../../../../docs/framework/wcf/feature-details/messaging-protocols.md)中有描述\)。要求與回覆訊息會使用 WS\-Addressing 2004\/08 來相互關聯。  
  
 第 8 節的 WS\-AtomicTransaction 進一步描述有關相互關聯與訊息交換模式描述的詳細資料。  
  
 R1232：傳出的 `wscoor:Register` 訊息必須使用[安全性通訊協定](../../../../docs/framework/wcf/feature-details/security-protocols.md)中描述的 `IssuedTokenOverTransport` 驗證模式。  
  
 必須使用發行的 `SecurityContextToken``STx` 簽署 `wsse:Timestamp` 項目。這個簽章是證明與特定交易關聯之權杖的所有權，並且用來驗證交易中登錄的參與者。RegistrationResponse 訊息會透過 HTTPS 傳回。  
...
-   R2002：必須提供 `t:IssuedToken` 的完整性與機密性。  
 ...
 如果 `wscoor:CoordinationContext` 的 `wscoor:Identifier` 是相對 URI，交易式 [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] 服務將傳回錯誤。  
  
## 訊息範例  
  
### CreateCoordinationContext 要求\/回應訊息  
 下列訊息會遵循要求\/回應模式。  
