...
- 使用命令列中的 `certutil` 命令來查詢憑證。 如需詳細資訊，請參閱[疑難排解憑證的 Certutil](https://go.microsoft.com/fwlink/?LinkId=120056)工作。  
...
- 將[\<serviceHostingEnvironment >](../../../../docs/framework/configure-apps/file-schema/wcf/servicehostingenvironment.md)的 `aspNetCompatibilityEnabled` 屬性設定為 `true`，即可啟用 ASP.NET 相容性模式。  
