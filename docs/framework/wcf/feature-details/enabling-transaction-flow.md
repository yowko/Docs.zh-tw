...
|---------------------------------|--------------------------------------|----------------------------------------------|------------------------------|  
|強制性|true|WS-AT|交易必須以可互通的 WS-AT 格式來流動。|  
|強制性|true|OleTransactions|交易必須以 [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] OleTransactions 格式來流動。|  
...
|交易不符合預期的通訊協定格式|強制|`MustUnderstand`等於 `false`。|由於需要異動而被拒絕|  
...
## <a name="enabling-transaction-flow-at-the-method-level"></a>啟用方法層級上的交易流程  
