...
|強制性|true|OleTransactions|交易必須以 WCF OleTransactions 格式流動。|  
|強制性|false|不適用|由於不是有效的組態而不適用。|  
|允許|true|WS-AT|交易可以用可互通的 WS-AT 格式來流動。|  
|允許|true|OleTransactions|交易可能會以 WCF OleTransactions 格式流動。|  
|允許|false|任何值|未流動交易。|  
...
|交易不符合預期的通訊協定格式|強制性|`MustUnderstand`等於 `false`。|由於需要交易而被拒絕|  
|交易不符合預期的通訊協定格式|允許|`MustUnderstand`等於 `false`。|由於不瞭解標頭而被拒絕|  
|使用任何通訊協定格式的異動|NotAllowed|`MustUnderstand`等於 `false`。|由於不瞭解標頭而被拒絕|  
|無交易|強制性|N/A|由於需要交易而被拒絕|  
|無交易|允許|N/A|處理序|  
...
## <a name="enabling-transaction-flow-at-the-endpoint-level"></a>啟用端點層級上的交易流程  
 ...
 WCF 提供的大部分持續系 `transactionFlow` 結都包含和 `transactionProtocol` 屬性，可讓您將特定的系結設定為接受傳入交易。 如需設定設定元素的詳細資訊，請參閱 [\<binding>](../../configure-apps/file-schema/wcf/bindings.md) 。  
...
## <a name="security"></a>安全性  
 ...
 當透過使用中繼資料交換產生 WCF 用戶端至未知或不受信任的 Web 服務時，如果可能的話，對這些 Web 服務的作業呼叫應該會隱藏目前的交易。 下列範例示範如何進行這項操作。  
...
## <a name="policy-assertions"></a>原則判斷提示  
 ...
 WCF 會使用原則判斷提示來控制交易流程。 原則判斷提示可以在服務的原則文件中找到，該文件是由合約、組態及屬性彙總而成。 用戶端可以使用 HTTP GET 或 WS-MetadataExchange 要求-回覆來取得服務的原則文件。 用戶端可以接著處理該原則文件，判斷在服務合約中的哪些作業可支援或需要異動流程。  
  
 異動流程原則判斷提示會指定用戶端應該傳送到服務以代表異動的 SOAP 標頭，藉此變動異動流程。 所有異動標頭都必須標記為 `MustUnderstand` 等於 `true`。 任何不是這樣標記的訊息都會被拒絕，並且產生 SOAP 錯誤。  
  
 單一作業中只能存在一個與交易相關的原則判斷提示。 在作業上具有一個以上交易判斷提示的原則檔會被視為無效，而且會被 WCF 拒絕。 此外，每個連接埠類型內部只能存在單一異動通訊協定。 具有在單一端口類型內參照多個交易通訊協定之作業的原則檔會被視為無效，而且會被 [System.servicemodel 中繼資料公用程式工具 ( # A0) ](../servicemodel-metadata-utility-tool-svcutil-exe.md)所拒絕。 輸出訊息或是單向輸入訊息如果存在異動判斷提示，該原則文件就會被視為無效。
