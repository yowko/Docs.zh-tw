...
ms.locfileid: "80635768"
---
# <a name="choose-a-message-encoder"></a>選擇訊息編碼器

本文討論在 Windows 通信基礎 (WCF) 中包含的消息編碼器中選擇的條件:二進位、文本和消息傳輸優化機制 (MTOM)。  
  
 在 WCF 中,您可以指定如何透過*綁定*在端點之間透過網路傳輸數據,綁定元素由一系列*綁定元素*組成。 訊息編碼器是由繫結堆疊中的訊息編碼繫結項目所表示。 繫結包含選擇性通訊協定繫結項目 (例如安全性繫結項目或可靠的訊息繫結項目)、必要的訊息編碼繫結項目和必要的傳輸繫結項目。  
...
- <xref:System.ServiceModel.Channels.TextMessageEncodingBindingElement> 即文字訊息編碼器，支援純 XML 編碼方式和 SOAP 編碼方式。 文字訊息編碼器的純 XML 編碼模式稱為 "plain old XML" (POX)，與文字為主的 SOAP 編碼方式有所區別。 若要啟用 POX，請將 <xref:System.ServiceModel.Channels.TextMessageEncodingBindingElement.MessageVersion%2A> 屬性設定為 <xref:System.ServiceModel.Channels.MessageVersion.None%2A>。 使用文本消息編碼器與非 WCF 終結點互操作。  
  
- <xref:System.ServiceModel.Channels.BinaryMessageEncodingBindingElement>,二進位消息編碼器,使用緊湊的二進位格式,並針對 WCF 到 WCF 通信進行了優化,因此不可互操作。 這也是 WCF 提供的所有編碼器中性能最大的編碼器。  
  
- <xref:System.ServiceModel.Channels.MtomMessageEncodingBindingElement> 即繫結項目，會指定以 MTOM 編碼訊息的字元編碼和訊息版本處理。 MTOM 是在 WCF 訊息中傳輸二進位資料的有效技術。 MTOM 編碼器會嘗試在效率和互通性之間建立平衡。 MTOM 編碼方式會以文字格式傳輸大部分的 XML，但是在傳輸大型區塊的二進位資料時，會依照原狀來傳送 (不轉換成文字)，好讓這些資料最佳化。 在效率方面,在 WCF 提供的編碼器中,MTOM 是文本(最慢)和二進制(速度最快的)之間的中間。  
...
|因數|描述|支援這個因素的編碼器|  
|------------|-----------------|---------------------------------------|  
|支援的字元集。|<xref:System.ServiceModel.Channels.TextMessageEncodingBindingElement>並<xref:System.ServiceModel.Channels.MtomMessageEncodingBindingElement>支援 UTF8 和 UTF16 Unicode(*大末點*和小*端*代碼)編碼。 如果不需要如 UTF7 或 ASCII 等其他編碼，則必須使用自訂編碼器。 有關範例自訂編碼器,請參考[自訂訊息編碼器](https://docs.microsoft.com/dotnet/framework/wcf/samples/custom-message-encoder-custom-text-encoder)。|Text|  
|檢閱|檢查是傳輸期間檢查訊息的能力。 不論是否使用 SOAP，文字編碼都會讓許多應用程式檢查及分析訊息，而不需要使用特別工具。 在消息或傳輸級別使用傳輸安全性會影響您檢查郵件的能力。 機密性會保護訊息不受檢查，而完整性則會保護訊息不受修改。|Text|  
...
|第三方工具支援|編碼方式的支援區域包含開發和診斷。 協力廠商開發人員已經在處理 POX 格式編碼訊息的程式庫和工具組投入大量投資。|文字 (POX)|  
|互通性|此因素是指 WCF 編碼器與非 WCF 服務互操作的能力。|Text<br /><br /> MTOM (部分)|  
...
由於此屬性僅在二進位訊息編碼綁定元素上公開,因此您需要建立如下所示的自定義綁定才能使用此功能:
...
用戶端和服務都需要同意發送和接收壓縮消息,因此必須在用戶端和服務上的二進位MessageEncode元素上配置compressFormat屬性。 如果服務或用戶端未配置為壓縮,但另一端為壓縮,則引發協定異常。 應仔細考慮啟用壓縮。 如果網路頻寬形成瓶頸，壓縮多半是有用的。 在 CPU 成為瓶頸所在的情況下，壓縮會減少輸送量。 必須在模擬環境中進行適當測試，以確認這項功能對應用程式有益。  
