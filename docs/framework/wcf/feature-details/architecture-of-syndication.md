...
- <xref:System.ServiceModel.Syndication.SyndicationPerson>  
 ...
 新聞訂閱支援建置於 WCF REST 程式設計模型之上，此模型會定義系結 <xref:System.ServiceModel.WebHttpBinding> ，以搭配使用以將摘要 <xref:System.ServiceModel.Description.WebHttpBehavior> 提供為服務。 如需 WCF REST 程式設計模型的詳細資訊，請參閱 [Wcf WEB HTTP 程式設計模型總覽](wcf-web-http-programming-model-overview.md)。  
  
> [!NOTE]
> Atom 1.0 規格允許在其任何日期建構中指定小數秒數。 序列化和還原序列化 WCF 實作為時，會忽略小數秒數。  
...
- 擴充性是新聞訂閱通訊協定的一項重要功能。 Atom 1.0 和 RSS 2.0 都可讓您將屬性與項目新增至規格中未定義的新聞訂閱摘要。 WCF 新聞訂閱程式設計模型提供兩種方式來使用自訂屬性和延伸模組：衍生新的類別和鬆散型別存取。 如需詳細資訊， [請參閱新聞](syndication-extensibility.md)訂閱擴充性。  
