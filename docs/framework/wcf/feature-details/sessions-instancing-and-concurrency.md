---
title: 工作階段、執行個體與並行
description: 深入瞭解會話、實例和並行、如何使用它們，以及它們在 WFC 之間的互動。
ms.date: 03/30/2017
...
### <a name="well-known-singleton-services"></a>已知的單一服務  
 ...
 請注意，當物件提供給此函式時，與 Windows Communication Foundation (WCF) 實例行為相關的某些功能會以不同的方式運作。 例如，提供單一物件執行個體時，呼叫 <xref:System.ServiceModel.InstanceContext.ReleaseServiceInstance%2A?displayProperty=nameWithType> 將沒有任何作用。 同樣的，也會忽略任何其他執行個體的釋放機制。 <xref:System.ServiceModel.ServiceHost> 的行為就像是所有作業都已將 <xref:System.ServiceModel.OperationBehaviorAttribute.ReleaseInstanceMode%2A?displayProperty=nameWithType> 屬性設定為 <xref:System.ServiceModel.ReleaseInstanceMode.None?displayProperty=nameWithType> 。  
...
> 您應該了解，開發能夠安全地使用一個以上之執行緒的程式碼，可能會很難順利地撰寫。 在使用 <xref:System.ServiceModel.ConcurrencyMode.Multiple> 或 <xref:System.ServiceModel.ConcurrencyMode.Reentrant> 值之前，請確定已適當地設計您的服務以使用這些模式 如需詳細資訊，請參閱<xref:System.ServiceModel.ServiceBehaviorAttribute.ConcurrencyMode%2A>。  
 ...
 並存的使用與執行個體模式有關。 在 <xref:System.ServiceModel.InstanceContextMode.PerCall> 實例中，並行與不相關，因為每個訊息都是由新的所處理，因此在中永遠不會有 <xref:System.ServiceModel.InstanceContext> 一個以上的執行緒在作用中 <xref:System.ServiceModel.InstanceContext> 。  
...
|Single|-會話通道的行為：會話，以及一個 <xref:System.ServiceModel.InstanceContext> 用於所有呼叫。<br />-無會話通道的行為：擲回例外狀況。|-會話通道的行為：會話和 <xref:System.ServiceModel.InstanceContext> 針對已建立或使用者指定的 singleton。<br />-無會話通道的行為： <xref:System.ServiceModel.InstanceContext> 適用于所建立或使用者指定之 singleton 的。|-會話通道的行為：擲回例外狀況。<br />-無會話通道的行為： <xref:System.ServiceModel.InstanceContext> 每個建立的 singleton 或使用者指定的 singleton 的。|  
  
## <a name="see-also"></a>另請參閱

- [使用工作階段](../using-sessions.md)
- [作法：建立需要工作階段的服務](how-to-create-a-service-that-requires-sessions.md)
- [作法：控制服務執行個體](how-to-control-service-instancing.md)
