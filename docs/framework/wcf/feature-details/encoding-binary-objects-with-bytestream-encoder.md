...
## <a name="byte-stream-message-encoder-architecture"></a>位元組資料流訊息編碼器架構  
 二進位訊息編碼器，會由 WCF 有沒有能力處理、 驗證或識別訊息中的基礎二進位資料。 資料封裝會編碼為 XML，傳送、接收及解碼。 編碼器處理資料的時間，是在資料傳遞到傳輸之後，且在訊息傳送至訊息佇列之前。 在功能上，二進位編碼器會將訊息資料包裝在 `<binary>` 項目中進行傳送，而在訊息接收到之後會移除這些項目。  
...
-   從訊息佇列中讀取資訊並加以處理。 系統將從訊息佇列管理員讀取訊息，並且在即將處理的訊息佇列通道中向上傳遞。 訊息佇列通道就會成為 WCF 通道堆疊中的佇列管理員。  
