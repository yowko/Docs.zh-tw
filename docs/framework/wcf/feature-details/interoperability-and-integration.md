---
title: 互通性與整合
ms.date: 03/30/2017
ms.assetid: 0d925119-01a1-45c2-9ea0-23061323ad33
ms.openlocfilehash: 5a50a8ebece72a0f2f8dc95fd87e73bef5db3195
ms.sourcegitcommit: 3d5d33f384eeba41b2dff79d096f47ccc8d8f03d
ms.translationtype: MT
ms.contentlocale: zh-TW
ms.lasthandoff: 05/04/2018
ms.locfileid: "33492703"
---
# <a name="interoperability-and-integration"></a>互通性與整合
本章節涵蓋互通性與整合與 Windows Communication Foundation (WCF)。  
  
## <a name="in-this-section"></a>本節內容  
 [Web 服務通訊協定互通性手冊](../../../../docs/framework/wcf/feature-details/web-services-protocols-interoperability-guide.md)  
 WCF 是建置與支援一組稱為 Web 服務規格的規格的 Web 服務交互操作。 本主題會列出支援的規格以及支援這些規格的繫結。  
  
 [整合 COM 應用程式](../../../../docs/framework/wcf/feature-details/integrating-with-com-plus-applications.md)  
 討論如何 COM + 應用程式可以公開為 WCF 服務。  
  
 [整合 COM 應用程式](../../../../docs/framework/wcf/feature-details/integrating-with-com-applications.md)  
 討論 COM 應用程式呼叫 WCF 服務的各種方法。  
  
 [將 .NET 遠端處理應用程式移轉至 WCF](../../../../docs/framework/wcf/feature-details/migrating-net-remoting-applications-to-wcf.md)  
 描述如何使用 WCF，與[!INCLUDE[dnprdnshort](../../../../includes/dnprdnshort-md.md)]遠端執行功能。  
  
 [與 Web Services Enhancements 3.0 的互通性](../../../../docs/framework/wcf/feature-details/interoperability-with-web-services-enhancements-3-0.md)  
 描述如何使用 WCF，與實作 Web Services Enhancements (WSE) 3.0 與 Web 服務。  
  
 [從 WSE 3.0 Web 服務移轉至 WCF](../../../../docs/framework/wcf/feature-details/migrating-wse-3-0-web-services-to-wcf.md)  
 說明如何將 WSE 3.0 Web 服務移轉至 WCF 服務。  
  
 [與 ASP.NET Web 服務的互通性](../../../../docs/framework/wcf/feature-details/interop-with-aspnet-web-services.md)  
 描述 WCF 可以相互作用[!INCLUDE[vstecasp](../../../../includes/vstecasp-md.md)]Web 服務。  
  
 [將 ASP.NET Web 服務移轉至 WCF](../../../../docs/framework/wcf/feature-details/migrating-aspnet-web-services-to-wcf.md)  
 描述如何移轉[!INCLUDE[vstecasp](../../../../includes/vstecasp-md.md)]Web 服務以 WCF 服務。  
  
 [將 ASP.NET Web 服務移轉至 WCF](../../../../docs/framework/wcf/feature-details/migrating-aspnet-web-services-to-wcf.md)  
 描述如何移轉[!INCLUDE[vstecasp](../../../../includes/vstecasp-md.md)]Web 服務以 WCF 服務。  
  
 [將 ASP.NET Web 服務移轉至 WCF](../../../../docs/framework/wcf/feature-details/migrating-aspnet-web-services-to-wcf.md)  
 描述如何移轉[!INCLUDE[vstecasp](../../../../includes/vstecasp-md.md)]Web 服務以 WCF 服務。  
  
 [與 POX 應用程式的互通性](../../../../docs/framework/wcf/feature-details/interoperability-with-pox-applications.md)  
 描述 WCF 提供服務和用戶端會使用"Plain Old XML"(POX) 訊息的方式。  
  
## <a name="reference"></a>參考資料  
 <xref:System.ServiceModel>  
  
## <a name="related-sections"></a>相關章節  
 [擴充 WCF](../../../../docs/framework/wcf/extending/index.md)
