...
> [!NOTE]
>  在實際的實作中，Match 方法包含的邏輯會檢查訊息，以判斷是否應該傳回此訊息篩選條件 **，則為 true**或是**false**。  
...
## <a name="usage"></a>使用量  
 若要使用您的自訂篩選搭配路由服務，您必須將它新增至篩選資料表藉由指定新的篩選器項目類型的 「 自訂 」 訊息篩選器的完整型別名稱和組件的名稱。  就像使用其他 MessageFilters 一般，您可以指定 filterData 字串，這個字串將傳遞至自訂篩選的建構函式。  
