...
|應用程式注意事項：<br /><br /> Web 服務定址端點參考和身分識別|即將發行|  
...
|IssuedTokenOverTransport|聯合|X509|傳輸|  
...
|AnonymousForSslNegotiated|無|X509、TLS\-Nego|Message|  
...
### 1.1 UsernameToken  
 ...
 這些子項目是用來協助進行重新執行偵測。[!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] 會改用訊息簽章。  
...
### 1.3 Kerberos 權杖  
 ...
 R1301 依照 GSS\_API 和 Kerberos 規格定義，Kerberos 權杖必須具有 GSS 包裝的 Kerberos v4 AP\_REQ 值，而且必須具有值為 \#GSS\_Kerberosv5\_AP\_REQ 的 ValueType 屬性。  
...
### 2.1 TimeStamp  
 時間戳記存在是使用 <xref:System.ServiceModel.Channels.SecurityBindingElement> 類別的 <xref:System.ServiceModel.Channels.SecurityBindingElement.IncludeTimestamp%2A> 屬性加以控制。[!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] 永遠使用 wsse:Created 和 wsse:Expires 欄位來序列化 wsse:TimeStamp。使用簽章時，一定會簽署 wsse:TimeStamp。  
  
### 2.2 保護順序  
 [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] 支援訊息保護順序「加密後簽署」和「簽署後加密」\(安全性原則 1.2\)。"基於下列理由，建議使用「簽署後加密」：除非使用 WS\-Security 1.1 SignatureConfirmation 機制，否則使用「加密後簽署」保護的訊息容易遭受簽章替換攻擊，而且加密內容上的簽章會造成稽核困難。  
  
### 2.3 簽章保護  
 使用「加密後簽署」時，建議保護簽章，以防止暴力密碼破解攻擊猜測加密內容或簽署金鑰 \(特別是在自訂權杖與弱式金鑰內容搭配使用時\)。  
  
### 2.4 演算法組合  
 [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] 支援安全性原則 1.2 中列出的所有演算法組合。  
...
### 3.2 使用 X.509 憑證來驗證服務  
 本章節描述下列驗證模式：MutualCertificate WSS1.0、MutualCertificateDuplex、MutualCertificate WSS1.1、AnonymousForCertificate、UserNameForCertificate 和 IssuedTokenForCertificate。  
...
### 安全性標頭範例：SignBeforeEncrypt、EncryptSignature  
 Request  
...
#### 3.2.3 搭配使用 SymmetricBinding 與 X.509 服務驗證  
 "WSS10" 會為使用 X509 權杖的案例提供有限支援。例如，對於只使用服務 X509 權杖的訊息，則無法提供簽章和加密保護。"WSS11" 引入 EncryptedKey 做為對稱式權杖。現在，服務 X.509 憑證的暫時加密金鑰可以做為要求和回應訊息保護。下面 3.4 節中描述的驗證模式 \(Mode\) 會使用這個模式 \(Pattern\)。  
