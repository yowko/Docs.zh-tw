...
|分類|通訊協定|規格和用法|  
|--------------|--------------|-----------------------------|  
|Transport|HTTP 1.1|[HTTP 1。1](https://www.ietf.org/rfc/rfc2616.txt)<br /><br /> `BasicHttpBinding`、`WSHttpBinding` 和 `WS2007HttpBinding` 使用 HTTP 和 HTTPS 傳輸。|  
|訊息|MTOM|[MTOM](https://www.w3.org/TR/soap12-mtom/)<br /><br /> `basicHttpBinding`、`wsHttpBinding` 和 `ws2007HttpBinding` 支援訊息傳輸最佳化機制 (MTOM)。 預設不使用。 若要使用 MTOM，請將 `messageEncoding` 屬性設為 `"Mtom"`。<br /><br /> 範例：<br /><br /> `<wsHttpBinding> <binding messageEncoding="Mtom"/> </wsHttpBinding>`|  
...
|分類|通訊協定|規格和用法|  
|--------------|--------------|-----------------------------|  
|訊息|SOAP 1.1|[SOAP 1。1](https://www.w3.org/TR/2000/NOTE-SOAP-20000508/)<br /><br /> 為了與 Basic Profile 1.1 保持一致，`basicHttpBinding` 項目實作 SOAP 1.1 訊息通訊協定。|  
...
|分類|通訊協定|規格和用法|  
|--------------|--------------|-----------------------------|  
|訊息|SOAP 1.2|[初步](https://www.w3.org/TR/soap12-part0/)<br /><br /> [訊息架構](https://www.w3.org/TR/2007/REC-soap12-part1-20070427/)<br /><br /> [附加（包括 HTTP 系結）](https://www.w3.org/TR/soap12-part2/)|  
|訊息|WS-ADDRESSING 2005/08|[Web 服務定址 1.0-核心](https://www.w3.org/TR/ws-addr-core/)<br /><br /> [Web 服務定址 1.0-SOAP](https://www.w3.org/TR/ws-addr-soap/)<br /><br /> `wsHttpBinding`、`ws2007HttpBinding` 和 `wsDualHttpBinding` 會實作全球資訊網協會 (W3C) WS-Addressing 建議，以啟用非同步訊息、訊息相互關聯和傳輸中性的定址機制。<br /><br /> 雖然 WS-* 規格允許 WS-Addressing 標頭加密，但是 WCF 並不提供這項支援。|  
|訊息|WS-Addressing 1.0 - 中繼資料|[Ws-addressing 1.0 中繼資料](https://www.w3.org/2007/05/addressing/metadata/)若要啟用此通訊協定的支援，請在 ServiceMetadata 行為中設定原則版本-將 policyversion 設為1.2 （預設值），wsdl 描述與 WS-ADDRESSING wsdl 相容，其中 policyversion 設定為1.5，wsdl 描述與 ws-addressing 中繼資料相容。<br /><br /> 雖然 WS-* 規格允許 WS-Addressing 標頭加密，但是 WCF 並不提供這項支援。|  
...
|異動|WS-AtomicTransaction|[WS-AtomicTransaction](http://specs.xmlsoap.org/ws/2004/10/wsat/wsat.pdf)<br /><br /> 用於交易管理員之間的通訊。 WCF 用戶端和服務一律使用本機交易管理員。|  
|異動|WS-Coordination|[WS-協調](https://docs.microsoft.com/previous-versions/ms951231(v=msdn.10))<br /><br /> 用於當 `flowTransactions` 屬性設為 "Allowed" 或 "Required" 時流動異動內容。<br /><br /> `<wsHttpBinding>   <binding transactionFlow="true"/> </wsHttpBinding>`|  
...
|訊息|SOAP 1.2|[初步](https://www.w3.org/TR/soap12-part0/)<br /><br /> [訊息架構](https://www.w3.org/TR/2007/REC-soap12-part1-20070427/)<br /><br /> [附加（包括 HTTP 系結）](https://www.w3.org/TR/soap12-part2/)|  
|訊息|WS-ADDRESSING 2005/08|[Web 服務定址 1.0-核心](https://www.w3.org/TR/ws-addr-core/)<br /><br /> [Web 服務定址 1.0-SOAP](https://www.w3.org/TR/ws-addr-soap/)|  
|中繼資料|WS-MetadataExchange|[WS-透過 ws-metadataexchange](http://specs.xmlsoap.org/ws/2004/09/mex/WS-MetadataExchange.pdf)<br /><br /> WCF 會執行透過 ws-metadataexchange，以取得 XML 架構、WSDL 和 WS 原則。|  
...
|分類|通訊協定|規格和用法|  
|--------------|--------------|-----------------------------|  
|Transport|HTTP 1.1|[HTTP 1。1](https://www.ietf.org/rfc/rfc2616.txt)<br /><br /> 已啟用傳輸安全性。|  
|訊息|SOAP 1.2|[初步](https://www.w3.org/TR/soap12-part0/)<br /><br /> [訊息架構](https://www.w3.org/TR/2007/REC-soap12-part1-20070427/)<br /><br /> [附加（包括 HTTP 系結）](https://www.w3.org/TR/soap12-part2/)|  
|訊息|WS-ADDRESSING 2005/08|[Web 服務定址 1.0-核心](https://www.w3.org/TR/ws-addr-core/)<br /><br /> [Web 服務定址 1.0-SOAP](https://www.w3.org/TR/ws-addr-soap/)|  
|中繼資料|WS-MetadataExchange|[WS-透過 ws-metadataexchange](http://specs.xmlsoap.org/ws/2004/09/mex/WS-MetadataExchange.pdf)<br /><br /> WCF 會執行透過 ws-metadataexchange，以取得 XML 架構、WSDL 和 WS 原則。|  
