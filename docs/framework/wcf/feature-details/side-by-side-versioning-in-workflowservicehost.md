...
## <a name="hosting-multiple-versions-in-a-workflow-service"></a>在工作流程服務中裝載多個版本  
 ...
 <xref:System.ServiceModel.Activities.WorkflowServiceHost> 包含兩個可設定為讓多個工作流程版本能夠並存執行的屬性：<xref:System.ServiceModel.Activities.WorkflowServiceHost.SupportedVersions%2A> 和 <xref:System.ServiceModel.Activities.WorkflowService.DefinitionIdentity%2A>。 <xref:System.ServiceModel.Activities.WorkflowServiceHost.SupportedVersions%2A> 包含受支援的工作流程服務版本，而 <xref:System.ServiceModel.Activities.WorkflowService.DefinitionIdentity%2A> 可用來唯一識別每個工作流程服務。 將 <xref:System.Activities.WorkflowIdentity> 與工作流程服務產生關聯，即可達到這個目的。 <xref:System.Activities.WorkflowIdentity> 包含三項識別資訊。 <xref:System.Activities.WorkflowIdentity.Name%2A> 和 <xref:System.Activities.WorkflowIdentity.Version%2A> 包含名稱及 <xref:System.Version>，兩者都是必要項，而 <xref:System.Activities.WorkflowIdentity.Package%2A> 則是選用項，可用來指定包含資訊 (例如組件名稱或其他所需資訊) 的其他字串。 <xref:System.ServiceModel.Activities.WorkflowServiceHost.SupportedVersions%2A> 集合中所包含的每個工作流程服務都必須具有唯一的 <xref:System.Activities.WorkflowIdentity>。 如果 <xref:System.Activities.WorkflowIdentity> 的三個屬性中有任何屬性與另一個 <xref:System.Activities.WorkflowIdentity> 的不同，則其為唯一識別。 `null` <xref:System.Activities.WorkflowIdentity> 是允許的值 <xref:System.ServiceModel.Activities.WorkflowService.DefinitionIdentity%2A> ，但只有一個舊版的工作流程服務可能具有 `null` <xref:System.Activities.WorkflowIdentity> 。  
...
### <a name="configuring-the-definitionidentity"></a>設定 DefinitionIdentity  
 ...
 使用工作流程設計工具建立工作流程服務時， <xref:System.ServiceModel.Activities.WorkflowService.DefinitionIdentity%2A> 會使用 [ **屬性** ] 視窗來設定。 在設計工具中，按一下服務根活動之外的外部，以選取工作流程服務，然後從 [ **View** ] 功能表選擇 [**屬性視窗]** 。 從出現在 [ **DefinitionIdentity** ] 屬性旁邊的下拉式清單中選取 [ **WorkflowIdentity** ]，然後展開並指定所需的 <xref:System.Activities.WorkflowIdentity> 屬性。 在下列範例中， <xref:System.ServiceModel.Activities.WorkflowService.DefinitionIdentity%2A> 是使用的 <xref:System.Activities.WorkflowIdentity.Name%2A> `MortgageWorkflow` 和來設定 <xref:System.Activities.WorkflowIdentity.Version%2A> `1.0.0.0` 。 <xref:System.Activities.WorkflowIdentity.Package%2A> 是選擇項，在這個範例中為 `null`。  
  
 ![顯示 DefinitionIdentity 屬性的螢幕擷取畫面。](./media/side-by-side-versioning-in-workflowservicehost/definitionidentity-property.bmp)  
  
 當工作流程服務是自我裝載的服務時，<xref:System.ServiceModel.Activities.WorkflowService.DefinitionIdentity%2A> 會在工作流程服務建構期間進行設定。 在下列範例中， <xref:System.ServiceModel.Activities.WorkflowService.DefinitionIdentity%2A> 是使用與上述範例相同的值來設定， <xref:System.Activities.WorkflowIdentity.Name%2A> `MortgageWorkflow` 並使用和的來 <xref:System.Activities.WorkflowIdentity.Name%2A> 設定 `1.0.0.0` 。  
...
### <a name="adding-a-new-version-to-a-web-hosted-workflow-service"></a>將新版本加入至 Web 主控工作流程服務  
 ...
 當服務重新啟動時，舊版本會自動加入至 <xref:System.ServiceModel.Activities.WorkflowServiceHost.SupportedVersions%2A> 集合，因為其位於指定的 `App_Code` 子資料夾中。 請注意，如果工作流程服務的主要版本有 `null` <xref:System.ServiceModel.Activities.WorkflowService.DefinitionIdentity%2A> 舊版，將不會加入。 一個版本可以有 `null`<xref:System.ServiceModel.Activities.WorkflowService.DefinitionIdentity%2A>，但如果有多個版本，則主要版本不可以是具有 `null`<xref:System.ServiceModel.Activities.WorkflowService.DefinitionIdentity%2A> 的版本，否則舊版本都不會加入至 <xref:System.ServiceModel.Activities.WorkflowServiceHost.SupportedVersions%2A> 集合。  
