...
## 服務參考  
 服務參考是其他服務的參考。例如，服務可能傳遞服務參考到運作中的用戶端。服務參考也可以搭配「*信任識別驗證器*」\(Trust Identity Verifier\) 使用，它是一種內部元件，可以確保在洩漏應用程式資料或認證等資訊給目標之前識別出目標主體。如果遠端信任識別無法確認或不正確，傳送者應確保不會洩漏可能危害到自己、應用程式或使用者的資料。  
  
 安全防護包括下列：  
  
-   服務參考假設為可以信任。每次傳送服務參考執行個體時，小心確保它們不會遭到竄改。  
  
-   有些應用程式會提供使用者體驗，允許根據服務參考中的資料和經過遠端主機證實的信任資料互相建立信任。[!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] 會提供這類機能的擴充點，但是使用者必須實作它們。  
...
```  
 ...
 如果有指定網域，但使用端點識別功能指定無效的服務主要名稱，則會使用 NTLM。[!INCLUDE[crabout](../../../../includes/crabout-md.md)]如何指定端點識別的詳細資訊，請參閱[服務身分識別和驗證](../../../../docs/framework/wcf/feature-details/service-identity-and-authentication.md)。  
