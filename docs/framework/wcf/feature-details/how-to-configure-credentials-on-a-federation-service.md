...
ms.locfileid: "69912190"
---
# <a name="how-to-configure-credentials-on-a-federation-service"></a>作法：設定同盟服務的認證
在 Windows Communication Foundation (WCF) 中, 建立聯合服務包含下列主要程式:  
  
1. 設定 <xref:System.ServiceModel.WSFederationHttpBinding> 或類似的自訂繫結。 如需建立適當系結的詳細資訊, [請參閱如何:建立 WSFederationHttpBinding](../../../../docs/framework/wcf/feature-details/how-to-create-a-wsfederationhttpbinding.md)。  
  
2. 設定 <xref:System.ServiceModel.Security.IssuedTokenServiceCredential>，此認證可控制如何驗證提供給服務的已發行權杖。  
  
 本主題會提供第二個步驟的詳細資訊。 如需聯合服務運作方式的詳細資訊, 請參閱[同盟](../../../../docs/framework/wcf/feature-details/federation.md)。  
...
2. 如果要驗證自行`true`發行的權杖 (例如 CardSpace 卡), 請將屬性設定為。<xref:System.ServiceModel.Security.IssuedTokenServiceCredential.AllowUntrustedRsaIssuers%2A> 預設為 `false`。  
...
1. 建立元素做為 <`serviceCredentials`> 元素的子系。 `<issuedTokenAuthentication>`  
  
2. 如果驗證`allowUntrustedRsaIssuers`自我發行的`<issuedTokenAuthentication>`權杖 ( `true`例如 CardSpace 卡), 請將元素的屬性設定為。  
...
5. 如有必要, 請`samlSerializer`將 <`issuedTokenAuthentication`> 元素的屬性設定為自訂<xref:System.IdentityModel.Tokens.SamlSerializer>類別的型別名稱。  
