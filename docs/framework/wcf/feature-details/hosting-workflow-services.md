...
ms.locfileid: "75963734"
---
# <a name="hosting-workflow-services"></a>裝載工作流程服務

您必須裝載工作流程服務，才能讓它回應傳入的訊息。 工作流程服務使用了 WCF 訊息基礎結構，因此會以類似的方式裝載。 就像 WCF 服務一樣，工作流程服務可以裝載于任何 managed 應用程式、Internet Information Services （IIS）或 Windows Process Activation Services （WAS）下。 此外，工作流程服務可以裝載于 Windows Server App Fabric 之下。 如需 Windows Server App Fabric 的詳細資訊，請參閱[Windows Server App fabric 檔](https://docs.microsoft.com/previous-versions/appfabric/ff384253(v=azure.10))、 [appfabric 裝載功能](https://docs.microsoft.com/previous-versions/appfabric/ee677189(v=azure.10))和[appfabric 裝載概念](https://docs.microsoft.com/previous-versions/appfabric/ee677371(v=azure.10))。 如需裝載 WCF 服務之各種方式的詳細資訊，請參閱[裝載服務](../../../../docs/framework/wcf/hosting-services.md)。
...
- 將包含活動的組件置入 IIS/WAS 虛擬目錄，該活動會使用多個 WCF 訊息活動。
 ...
 定義工作流程服務的 service1.xamlx 檔案必須包含 <`Service`> 根項目或根項目，其中包含從 <xref:System.Workflow.ComponentModel.Activity>衍生的任何類型。 使用 Visual Studio 活動範本時，會建立 service1.xamlx 檔案。 使用 WCF 工作流程服務範本時，會建立 service1.xamlx 檔案。
...
## <a name="referencing-custom-activities"></a>參考自訂活動
 自訂活動的參考必須加入至 [<`System.Web.Compilation`>] 底下的 [<`Assemblies`>] 區段中，以便將其載入應用程式域中，而 XAML 還原序列化程式可以找到類型。 這些設定可以在應用程式層級進行，如果設定值應該套用到電腦上的所有應用程式，則可以在根 Web.config 進行這些設定。
