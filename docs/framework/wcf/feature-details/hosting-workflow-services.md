...
- 將包含活動的組件置入 IIS/WAS 虛擬目錄，該活動會使用多個 WCF 訊息活動。
 ...
 定義工作流程服務的 service1.xamlx 檔案必須包含 <`Service`> 根項目或根項目，其中包含衍生自的任何類型 <xref:System.Workflow.ComponentModel.Activity> 。 使用 Visual Studio 活動範本時，會建立 service1.xamlx 檔案。 使用 WCF 工作流程服務範本時，會建立 service1.xamlx 檔案。
...
## <a name="referencing-custom-activities"></a>參考自訂活動
 自訂活動的參考必須加入至 [<> 下的 [<>] 區段中， `Assemblies` 以便將 `System.Web.Compilation` 其載入應用程式域中，而且 XAML 還原序列化程式能夠尋找類型。 這些設定可以在應用程式層級進行，如果設定值應該套用到電腦上的所有應用程式，則可以在根 Web.config 進行這些設定。
