...
每個端點都有與其相關聯的位址，以便用來找出並識別端點。 這個位址主要包含一個可指定端點位置的統一資源識別元 (URI)。 端點位址是由類別在 Windows Communication Foundation (WCF) 程式設計模型中表示 <xref:System.ServiceModel.EndpointAddress> ，這個類別包含選擇性 <xref:System.ServiceModel.EndpointAddress.Identity%2A> 的屬性，可讓與它交換訊息的其他端點驗證端點，以及一組選擇性的 <xref:System.ServiceModel.EndpointAddress.Headers%2A> 屬性，這些屬性會定義存取服務所需的任何其他 SOAP 標頭。 選擇性標頭會提供額外與更詳細的定址資訊，以便識別端點或與服務端點互動。 端點位址會在網路上表示為 WS-Addressing 端點參考 (EPR)。  
  
## <a name="uri-structure-of-an-address"></a>位址的 URI 結構  

 大部分傳輸的位址 URI 具有四個部分。 例如，URI 的四個部分可以依照 `http://www.fabrikam.com:322/mathservice.svc/secureEndpoint` 下列方式進行詳細說明：  
...
- 繫結資訊：IP 位址、連接埠、主機標頭  
 ...
 IIS 可以為每個網站指定多個繫結，讓每個配置都有多個基底位址。 在 .NET Framework 3.5 之前，WCF 不支援架構的多個位址，而且如果指定的話，會 <xref:System.ArgumentException> 在啟用期間擲回。  
  
 .NET Framework 3.5 可讓網際網路服務提供者針對相同網站上的相同配置，裝載多個具有不同基底位址的應用程式。  
...
- `http://shipping.myorg.com/Service.svc`
 ...
 使用 .NET Framework 3.5，您可以在設定檔中的 AppDomain 層級指定前置詞篩選準則。 您可以使用包含前置詞清單的元素來完成這 [\<baseAddressPrefixFilters>](../../configure-apps/file-schema/wcf/baseaddressprefixfilters.md) 項作業。 IIS 提供的傳入基底位址會依據選擇性的前置詞清單經過篩選。 根據預設，如果沒有指定前置詞，則所有位址都會通過。 如果指定前置詞，則只會產生相符的基底位址讓該配置通過。  
...
```  
 ...
 在上述範例中， `net.tcp://payroll.myorg.com:8000` 和 `http://shipping.myorg.com:8000` 是其各自配置的唯一基底位址，而這些位址會通過。  
...
## <a name="multiple-iis-binding-support-in-net-framework-4-and-later"></a>.NET Framework 4 及更新版本中的多重 IIS 繫結支援  
 ...
 以下是使用 multipleSiteBindingsEnabled on 的設定程式碼範例 [\<serviceHostingEnvironment>](../../configure-apps/file-schema/wcf/servicehostingenvironment.md) 。  
...
```  
 ...
 如需詳細資訊和範例，請參閱 [支援多重 IIS 網站](supporting-multiple-iis-site-bindings.md) 系結和 <xref:System.ServiceModel.ServiceHostingEnvironment.MultipleSiteBindingsEnabled%2A> 。  
  
## <a name="extending-addressing-in-wcf-services"></a>擴充 WCF 服務中的定址  

 WCF 服務的預設定址模型會基於下列目的使用端點位址 URI：  
...
- 在設定中， [\<headers>](../../configure-apps/file-schema/wcf/headers.md) 會將自訂指定為元素的子系 [\<endpoint>](../../configure-apps/file-schema/wcf/endpoint-of-client.md) 。  
...
- 在 [設定] 中，以服務專案的屬性指定自訂接聽位址 `ListenUri` [\<endpoint>](../../configure-apps/file-schema/wcf/endpoint-element.md) 。  
