...
helpviewer_keywords: 
  - "位址 [WCF]"
  - "WCF [WCF], 位址"
  - "Windows Communication Foundation [WCF], 位址"
...
---
# 端點位址
每個端點都有與其相關聯的位址，以便用來找出並識別端點。這個位址主要包含一個可指定端點位置的統一資源識別元 \(URI\)。端點位址是由 <xref:System.ServiceModel.EndpointAddress> 類別在 [!INCLUDE[indigo1](../../../../includes/indigo1-md.md)] 程式設計模型中表示，這個類別包含選擇性的 <xref:System.ServiceModel.EndpointAddress.Identity%2A> 屬性，而這個屬性可讓與某個端點交換訊息的其他端點驗證該端點；此外，這個類別也包含一組選擇性的 <xref:System.ServiceModel.EndpointAddress.Headers%2A> 屬性，這些屬性則會定義取用服務時所需的其他任何 SOAP 標頭。選擇性標頭會提供額外與更詳細的定址資訊，以便識別端點或與服務端點互動。端點位址會在網路上表示為 WS\-Addressing 端點參考 \(EPR\)。  
...
### 在組態中定義位址  
 若要在組態檔中定義端點，請使用 [\<endpoint\>](../../../../docs/framework/configure-apps/file-schema/wcf/endpoint-element.md) 項目。如需詳細資訊和範例，請參閱[指定端點位址](../../../../docs/framework/wcf/specifying-an-endpoint-address.md)。  
...
## .NET Framework 3.5 中的多重 IIS 繫結支援  
...
-   繫結資訊：IP 位址、連接埠、主機標頭  
 ...
 IIS 可以為每個網站指定多個繫結，讓每個配置都有多個基底位址。在 [!INCLUDE[netfx35_short](../../../../includes/netfx35-short-md.md)] 問世之前，[!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] 並不支援單一配置包含多個位址，而且如果您指定這些位址，就會在啟用期間擲回 <xref:System.ArgumentException>。  
...
-   http:\/\/shipping.myorg.com\/Service.svc  
 ...
 有了 [!INCLUDE[netfx35_short](../../../../includes/netfx35-short-md.md)]，您就可以透過組態檔在 AppDomain 層級指定前置詞篩選條件。使用包含前置詞清單的 [\<baseAddressPrefixFilters\>](../../../../docs/framework/configure-apps/file-schema/wcf/baseaddressprefixfilters.md) 項目即可達到這個目的。IIS 提供的傳入基底位址會依據選擇性的前置詞清單經過篩選。根據預設，如果沒有指定前置詞，則所有位址都會通過。如果指定前置詞，則只會產生相符的基底位址讓該配置通過。  
...
## .NET Framework 4 \(含\) 以後版本中的多重 IIS 繫結支援  
 ...
 下面是在 [\<serviceHostingEnvironment\>](../../../../docs/framework/configure-apps/file-schema/wcf/servicehostingenvironment.md) 上使用 multipleSiteBindingsEnabled 的組態程式碼範例。  
...
-   端點的接聽位址是私用 URI，而且已設為不同於本身 `listenURI` 屬性的值。  
 ...
 `via` 指定的傳輸位址，是指訊息在前往由服務所在位置 `to` 參數指定的某個其他遠端位址途中，一開始要傳送到的位置。在網際網路用途方面，`via` URI 大多與服務最終 `to` 位址的 <xref:System.ServiceModel.EndpointAddress.Uri%2A> 屬性相同。當您必須手動執行路由傳送時，才需要區分這兩個位址。  
  
### 定址標頭  
 端點除了可以由其基本 URI 定址之外，還可由一或多個 SOAP 標頭定址。有些情況適用這種方式，例如 SOAP 媒介，此時端點需要此端點的用戶端加入目標為媒介的 SOAP 標頭。  
...
-   在組態中，您可以將自訂 [\<頁首\>](../../../../docs/framework/configure-apps/file-schema/wcf/headers.md) 指定為 [\<endpoint\>](http://msdn.microsoft.com/zh-tw/13aa23b7-2f08-4add-8dbf-a99f8127c017) 項目的子系。  
...
-   在組態中，可以使用服務 [\<endpoint\>](http://msdn.microsoft.com/zh-tw/13aa23b7-2f08-4add-8dbf-a99f8127c017) 項目的 `ListenUri` 屬性來指定自訂接聽位址。  
