...
3.  透過拒絕使用 NTLM 來要求 SSPI 交涉使用 Kerberos：  
    ...
    2.  或者，您可以將組態檔中的 `allowNtlm` 屬性設定為 `false` 來做到這點。 這個屬性包含在[ \<windows >](../../../../docs/framework/configure-apps/file-schema/wcf/windows-of-clientcredentials-element.md)。  
...
#### <a name="anonymous-ntlm-logon-occurs-but-anonymous-logons-are-disabled-by-default"></a>發生匿名 NTLM 登入，不過匿名登入已依預設地停用。  
 ...
 如需有關模擬的詳細資訊，請參閱[委派和模擬](../../../../docs/framework/wcf/feature-details/delegation-and-impersonation-with-wcf.md)。  
...
#### <a name="sspi-is-not-available"></a>無法使用 SSPI  
 下列作業系統不支援 Windows 驗證時用做為伺服器： [!INCLUDE[wxp](../../../../includes/wxp-md.md)] Home Edition、 [!INCLUDE[wxp](../../../../includes/wxp-md.md)] Media Center Edition，和[!INCLUDE[wv](../../../../includes/wv-md.md)]Home edition。  
