...
## <a name="conditional-http-get-based-caching"></a>條件式 HTTP GET 型快取  
 在 Web HTTP 案例中，服務通常會使用條件式 HTTP GET 來執行智慧型 HTTP 快取，如[HTTP 規格](https://www.w3.org/Protocols/rfc2616/rfc2616.html)中所述。 若要這樣做，服務必須設定 HTTP 回應中之 ETag 標頭的值。 服務也必須檢查 HTTP 請求中的 If-None-Match 標頭，確認是否有任何指定的 ETag 符合目前的 ETag。  
