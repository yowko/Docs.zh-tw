...
```  
 ...
 已啟用的通訊協定清單也可以在 \<applicationDefaults> 儲存于 applicationhost.config 中的網站 XML 設定的元素中設定。  
...
## <a name="building-a-wcf-service-that-uses-was-for-non-http-activation"></a>針對非 HTTP 啟動建置使用 WAS 的 WCF 服務  
 一旦您執行安裝和設定 WAS 的步驟（請參閱[如何：安裝和設定 WCF 啟用元件](how-to-install-and-configure-wcf-activation-components.md)），設定服務以使用 WAS 來進行啟用，類似于設定裝載于 IIS 的服務。  
  
 如需有關建立已啟動之 WCF 服務的詳細指示，請參閱[如何：在 WAS 中裝載 WCF 服務](how-to-host-a-wcf-service-in-was.md)。  
