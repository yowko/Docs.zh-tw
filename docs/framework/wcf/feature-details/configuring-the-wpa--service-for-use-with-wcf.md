...
本主題介紹在 Windows Vista 中設置 Windows 進程啟動服務（也稱為 WAS）以託管不通過 HTTP 網路通訊協定進行通信的 Windows 通信基礎 （WCF） 服務所需的步驟。 下列各節將概述此組態的各項步驟：  
...
## <a name="configuring-a-site-with-non-http-bindings"></a>使用非 HTTP 繫結來設定網站  
 ...
 applicationHost.config 是一個可使用任何標準文字編輯器 (例如 [記事本]) 來開啟的 XML 文字檔。 但是，IIS 7.0 命令列組態工具 （appcmd.exe） 是添加非 HTTP 網站綁定的首選方式。  
...
## <a name="enabling-an-application-to-use-non-http-protocols"></a>啟用應用程式來使用非 HTTP 通訊協定  
 您可以在應用程式級別啟用或禁用單個網路通訊協定。 下列命令說明如何針對在 `Default Web Site` 中執行的應用程式同時啟用 HTTP 和 net.tcp 通訊協定。  
  
```console  
appcmd.exe set app "Default Web Site/appOne" /enabledProtocols:net.tcp  
```  
  
 還可以在\<應用程式中設置已啟用的協定清單，>存儲在應用程式Host.config中的網站 XML 配置的元素。  
...
```  
 ...
 如果您收到這個錯誤，請確認已安裝並正確設定適用於非 HTTP 啟用的 WAS。 有關詳細資訊，請參閱[如何：安裝和配置 WCF 啟動元件](../../../../docs/framework/wcf/feature-details/how-to-install-and-configure-wcf-activation-components.md)。  
  
## <a name="building-a-wcf-service-that-uses-was-for-non-http-activation"></a>針對非 HTTP 啟動建置使用 WAS 的 WCF 服務  
 執行安裝和配置 WAS 的步驟（請參閱[：安裝和配置 WCF 啟動元件](../../../../docs/framework/wcf/feature-details/how-to-install-and-configure-wcf-activation-components.md)），將服務配置為使用 WAS 進行啟動類似于配置 IIS 中託管的服務。  
