...
在 Windows Communication Foundation (WCF) 中, 建立*聯合服務*的用戶端包含三個主要步驟:  
  
1. 設定 wsFederationHttpBinding [ >或類似的自訂系結。\< ](../../../../docs/framework/configure-apps/file-schema/wcf/wsfederationhttpbinding.md) 如需建立適當系結的詳細資訊, [請參閱如何:建立 WSFederationHttpBinding](../../../../docs/framework/wcf/feature-details/how-to-create-a-wsfederationhttpbinding.md)。 或者, 針對同盟服務的中繼資料端點執行[System.servicemodel 中繼資料公用程式工具 (Svcutil)](../../../../docs/framework/wcf/servicemodel-metadata-utility-tool-svcutil-exe.md) , 以產生與同盟服務和一或多個安全性權杖服務進行通訊的設定檔。  
...
> 當用戶端使用模擬認證、<xref:System.Security.Cryptography.CryptographicException> 繫結或自訂發行權杖，以及非對稱金鑰時，可能會擲回 <xref:System.ServiceModel.WSFederationHttpBinding>。 當 <xref:System.ServiceModel.WSFederationHttpBinding> 和 <xref:System.ServiceModel.FederatedMessageSecurityOverHttp.IssuedKeyType%2A> 屬性分別設為 <xref:System.ServiceModel.Security.Tokens.IssuedSecurityTokenParameters.KeyType%2A> 時，非對稱金鑰便能與 <xref:System.IdentityModel.Tokens.SecurityKeyType.AsymmetricKey> 繫結或自訂發行權杖一起使用。 當用戶端嘗試傳送訊息，而用戶端想要模擬的身分識別並沒有使用者設定檔時，就會擲回 <xref:System.Security.Cryptography.CryptographicException>。 若要減少發生這個問題，請在傳送訊息之前登入用戶端電腦，或是呼叫 `LoadUserProfile`。  
 ...
 本主題提供這些程序的詳細資訊。 如需建立適當系結的詳細資訊, [請參閱如何:建立 WSFederationHttpBinding](../../../../docs/framework/wcf/feature-details/how-to-create-a-wsfederationhttpbinding.md)。 如需聯合服務運作方式的詳細資訊, 請參閱[同盟](../../../../docs/framework/wcf/feature-details/federation.md)。  
  
### <a name="to-generate-and-examine-the-configuration-for-a-federated-service"></a>若要產生並檢查聯合服務的組態  
  
1. 以服務的中繼資料 URL 位址做為命令列參數來執行[System.servicemodel 中繼資料公用程式工具 (Svcutil)](../../../../docs/framework/wcf/servicemodel-metadata-utility-tool-svcutil-exe.md) 。  
  
2. 透過適當的編輯器開啟產生的組態檔。  
  
3. 檢查任何產生[ \<之簽發者](../../../../docs/framework/configure-apps/file-schema/wcf/issuer.md)的屬性和內容 > 並[ \<issuerMetadata >](../../../../docs/framework/configure-apps/file-schema/wcf/issuermetadata.md)元素。 這些[位於\<wsFederationHttpBinding >](../../../../docs/framework/configure-apps/file-schema/wcf/wsfederationhttpbinding.md)或自訂系結元素的[ \<安全性 >](../../../../docs/framework/configure-apps/file-schema/wcf/security-of-wsfederationhttpbinding.md)元素中。 請確定位址包含預期的網域名稱或其他位址資訊。 請務必檢查這項資訊，因為用戶端會對這些位址進行驗證，而且可能揭露使用者名稱/密碼組這類的資訊。 如果位址不是預期的位址，可能會導致向不適當的收件者揭露資訊。  
  
4. 檢查已加上批註的 <`alternativeIssuedTokenParameters`> 元素內的任何其他[ \<issuermetadata >](../../../../docs/framework/configure-apps/file-schema/wcf/issuedtokenparameters.md)元素。 使用 Svcutil.exe 工具來產生聯合服務的組態時，如果聯合服務或任何中繼安全性權杖服務未指定發行者位址，而是指定了會公開多個端點之安全性權杖服務的中繼資料位址，則結果組態檔會參照到第一個端點。 其他端點在設定檔中是以批註方式 <`alternativeIssuedTokenParameters`> 元素。  
  
     判斷其中一個 <`issuedTokenParameters`> 是否較適合設定中已存在的其中一個。 例如, 用戶端可能會想要使用 Windows CardSpace 權杖 (而不是使用者名稱/密碼組) 來驗證 Security Token Service。  
  
    > [!NOTE]
    > 如果與服務進行通訊之前，必須周遊多個安全性權杖服務時，中繼安全性權杖服務可能會將用戶端導向不正確的安全性權杖服務。 因此, 請確定[ \<issuermetadata >](../../../../docs/framework/configure-apps/file-schema/wcf/issuedtokenparameters.md)中 Security Token Service 的端點是預期的 Security Token Service, 而不是未知的 Security Token Service。  
...
1. 建立 issuedToken > 元素做為端點行為中[ \<issuedToken >](../../../../docs/framework/configure-apps/file-schema/wcf/issuedtoken.md)元素的子系。 [ \< ](../../../../docs/framework/configure-apps/file-schema/wcf/issuedtoken.md)  
  
2. 如果不需要權杖快取, 請將`cacheIssuedTokens`屬性 (屬於 <`issuedToken`> 元素) 設定為`false`。  
  
3. 如果快取的權杖上需要時間限制, 請將`maxIssuedTokenCachingTime` <`issuedToken`> 元素上的屬性設定為適當的值。 例如：  
    `<issuedToken maxIssuedTokenCachingTime='00:10:00' />`  
  
4. 如果偏好預設值以外的值, 請將 < `issuedTokenRenewalThresholdPercentage` `issuedToken`> 元素上的屬性設定為適當的值, 例如:  
...
6. 選擇性。 藉由建立 <`issuerChannelBehaviors`> 元素做為 <`issuedToken`> 專案的子系, 設定任何簽發者特定的自訂端點行為。 針對每個行為, 建立 <`add`> 元素做為 <`issuerChannelBehaviors`> 元素的子系。 在 < `issuerAddress` `add`> 專案上設定屬性, 以指定行為的簽發者位址。 藉由設定`behaviorConfiguration` <`add`> 元素的屬性, 指定行為本身。  
...
1. 建立 scopedCertificates > 元素做為[ \<serviceCertificate >](../../../../docs/framework/configure-apps/file-schema/wcf/servicecertificate-of-clientcredentials-element.md)元素的子系, 本身就是端點行為中[ \<clientCredentials >](../../../../docs/framework/configure-apps/file-schema/wcf/clientcredentials.md)專案的子系。 [ \< ](../../../../docs/framework/configure-apps/file-schema/wcf/scopedcertificates-element.md)  
...
## <a name="localissuer-required"></a>需要 LocalIssuer  
 ...
 <xref:System.ServiceModel.Security.IssuedTokenClientCredential.LocalIssuerAddress%2A>如需設定類別<xref:System.ServiceModel.Security.IssuedTokenClientCredential>之、 <xref:System.ServiceModel.Security.IssuedTokenClientCredential.LocalIssuerBinding%2A>和<xref:System.ServiceModel.Security.IssuedTokenClientCredential.LocalIssuerChannelBehaviors%2A>屬性的詳細資訊, 請參閱[如何:設定本機簽發者](../../../../docs/framework/wcf/feature-details/how-to-configure-a-local-issuer.md)。  
...
> [!NOTE]
> 用戶端對於規範到特定 URI 範圍的憑證概念，只會套用到對服務 (可在這些 URI 上公開端點) 執行傳出呼叫的應用程式上。 它不會套用到用來簽署已發行權杖的憑證, 例如, 在由<xref:System.ServiceModel.Security.IssuedTokenServiceCredential.KnownCertificates%2A> <xref:System.ServiceModel.Security.IssuedTokenServiceCredential>類別的所傳回的集合中, 伺服器上設定的憑證。 如需詳細資訊，請參閱[如何：在同盟服務](../../../../docs/framework/wcf/feature-details/how-to-configure-credentials-on-a-federation-service.md)上設定認證。  
  
## <a name="see-also"></a>另請參閱

- [同盟範例](../../../../docs/framework/wcf/samples/federation-sample.md)
- [如何：停用 WSFederationHttpBinding 上的安全會話](../../../../docs/framework/wcf/feature-details/how-to-disable-secure-sessions-on-a-wsfederationhttpbinding.md)
...
- [如何：安全中繼資料端點](../../../../docs/framework/wcf/feature-details/how-to-secure-metadata-endpoints.md)
