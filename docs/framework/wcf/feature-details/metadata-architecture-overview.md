...
Windows Communication Foundation (WCF) 提供豐富的基礎結構的匯出、 發行、 擷取和匯入服務中繼資料。 WCF 服務使用中繼資料描述如何與服務端點互動的讓 Svcutil.exe 之類的工具，可以自動產生用戶端程式碼來存取服務。  
 ...
 大部分的 WCF 中繼資料基礎結構所組成的型別位於<xref:System.ServiceModel.Description>命名空間。  
  
 WCF 使用<xref:System.ServiceModel.Description.ServiceEndpoint>類別來描述服務中的端點。 您可以使用 WCF 來產生服務端點的中繼資料，或匯入服務中繼資料來產生<xref:System.ServiceModel.Description.ServiceEndpoint>執行個體。  
...
## <a name="exporting-service-metadata"></a>匯出服務中繼資料  
 ...
 <xref:System.ServiceModel.Description.WsdlExporter>類型是實作<xref:System.ServiceModel.Description.MetadataExporter?displayProperty=nameWithType>抽象 WCF 所包含的類別。 <xref:System.ServiceModel.Description.WsdlExporter> 型別會產生包含附加原則運算式的 WSDL 中繼資料。  
...
## <a name="publishing-service-metadata"></a>發行服務中繼資料  
 WCF 服務發行中繼資料公開一個或多個中繼資料端點。 發行服務中繼資料會使用標準化的通訊協定 (例如 MEX 和 HTTP/GET 要求) 來提供服務中繼資料。 中繼資料端點與其他服務端點相似的地方，在於都有位址、繫結和合約。 您可以透過組態或程式碼，將中繼資料端點新增至服務主機。  
 ...
 若要加入使用 MEX 通訊協定的中繼資料端點，將服務端點加入至您的服務主機使用服務合約會定義具名的 IMetadataExchange.WCF<xref:System.ServiceModel.Description.IMetadataExchange>具有此服務合約名稱的介面。 Ws-metadataexchange 端點或 MEX 端點都可以使用四個預設繫結上公開的靜態 factory 方法之一<xref:System.ServiceModel.Description.MetadataExchangeBindings>類別比對使用 Svcutil.exe 之類的 WCF 工具的預設繫結。 您也可以使用自訂繫結設定 MEX 中繼資料端點。  
...
## <a name="importing-service-metadata"></a>匯入服務中繼資料  
 在 WCF 中，中繼資料匯入是從它的中繼資料產生服務或其元件部分的抽象表示法的程序。 例如，可以匯入 WCF<xref:System.ServiceModel.Description.ServiceEndpoint>執行個體，<xref:System.ServiceModel.Channels.Binding>執行個體或<xref:System.ServiceModel.Description.ContractDescription>服務的執行個體從 WSDL 文件。 若要匯入 WCF 服務中繼資料，請使用 實作<xref:System.ServiceModel.Description.MetadataImporter>抽象類別。 型別衍生自<xref:System.ServiceModel.Description.MetadataImporter?displayProperty=nameWithType>類別會實作支援匯入中繼資料格式以利用 WS 原則匯入 WCF 中的邏輯。  
 ...
 <xref:System.ServiceModel.Description.WsdlImporter?displayProperty=nameWithType>類型是實作<xref:System.ServiceModel.Description.MetadataImporter?displayProperty=nameWithType>抽象 WCF 所包含的類別。 <xref:System.ServiceModel.Description.WsdlImporter?displayProperty=nameWithType> 型別會匯入具有與 <xref:System.ServiceModel.Description.MetadataSet> 物件組合在一起之附加原則的 WSDL 中繼資料。  
