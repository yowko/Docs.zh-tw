---
title: "資料合約中的 XML 與 ADO.NET 型別 | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
...
```  
 ...
 當搭配 `DataContractSerializer` 使用時，可以多型指派 `XmlElement` ，但只有對型別 <xref:System.Object> 的資料成員。  即使實作 <xref:System.Collections.IEnumerable>，`XmlElement` 仍然無法用來做為集合型別，且無法指派給 <xref:System.Collections.IEnumerable> 資料成員。  如同所有的多型指派，`DataContractSerializer` 會在結果 XML 中發出資料合約名稱 – 在本例中，它在 "http:\/\/schemas.datacontract.org\/2004\/07\/System.Xml" 命名空間中是 "XmlElement"。  
...
### XmlNode 的陣列  
 使用 <xref:System.Xml.XmlNode> 的陣列與使用 `XmlElement` 十分類似。  使用 `XmlNode` 的陣列比使用 `XmlElement` 更有彈性。  您可以在資料成員包裝項目內撰寫多個項目。  您也可以將項目以外的內容插入資料成員包裝項目內，例如 XML 註解。  最後，您可以將屬性置入包裝資料成員項目內。  這些都可以藉由將 `XmlNode` 的特定衍生類別 \(例如 <xref:System.Xml.XmlAttribute>、`XmlElement` 或 <xref:System.Xml.XmlComment>\) 填入 `XmlNode` 的陣列來達成。  以下列型別為例。  
...
```  
 ...
 請勿嘗試序列化有一或多個節點設定為 `null` 之 `XmlNode` 的陣列。  允許整個陣列成員為 `null`，但陣列中包含的個別 `XmlNode` 則不允許。  如果整個陣列成員為 null，包裝函式資料成員項目便會包含特別的屬性，指出它為 null。  在還原序列化時，整個陣列成員也會變成 null。  
 ...
 `XmlNode` 的陣列或陣列集合是被允許的。  整個集合會有包裝函式項目，而在外部陣列或集合中 `XmlNode` 之每個陣列則會有不同的包裝函式項目 \(與上例中 `<myDataMember>` 類似\)。  
  
 以 `XmlNode` 執行個體填入 `Object` 之型別 <xref:System.Array> 的資料成員或 `IEnumerable` 的 `Array` 不會造成資料成員被視為 `XmlNode` 執行個體的 `Array`。  每個陣列成員都會分開進行序列化。  
  
 當搭配 `DataContractSerializer` 使用時，可能會多型指派 `XmlNode` 的陣列，但只有對型別 `Object` 的資料成員。  即使實作 `IEnumerable`，`XmlNode` 的陣列仍然無法用來做為集合型別，並指派給 `IEnumerable` 資料成員。  如同所有的多型指派，`DataContractSerializer` 會在結果 XML 中發出資料合約名稱 – 在本例中，它在 "http:\/\/schemas.datacontract.org\/2004\/07\/System.Xml" 命名空間中是 "ArrayOfXmlNode"。  當搭配 Net`DataContractSerializer` 使用時，會支援 `XmlNode` 陣列的有效指派。  
  
### 結構描述的考量  
 如需 XML 型別之結構描述對應的詳細資訊，請參閱[資料合約結構描述參考](../../../../docs/framework/wcf/feature-details/data-contract-schema-reference.md)。  本節將提供重點摘要。  
...
-   當在最上層還原序列化元素型別，但在建構期間沒有指定根名稱和命名空間時，如果可以找到任何元素的起始，<xref:System.Runtime.Serialization.XmlObjectSerializer.IsStartObject%2A> 就會傳回 `true`。  `verifyObjectName` 參數設定為 `true` 的 <xref:System.Runtime.Serialization.XmlObjectSerializer.ReadObject%2A> 在實際讀取物件之前，行為方式會和 `IsStartObject` 相同。  然後 `ReadObject` 就會將控制項傳遞給 `ReadXml` 方法。  
...
### 與 XmlSerializer 的差異  
 <xref:System.Xml.Serialization.XmlSerializer> 也會辨識 `IXmlSerializable` 介面和 `XmlSchemaProviderAttribute` 和 `XmlRootAttribute` 屬性。  然而，在資料合約模型中對於它們的處理方式有一些差異。  重要差異摘要如下：  
...
-   XML 寫入器在撰寫另一份文件期間，通常不會允許 XML 文件宣告 \(例如，\<?xml version\=’1.0’ ?\>\)。  您無法採用完整的 XML 文件並序列化為 `XmlNode` 資料成員的 `Array`。  如果要執行這項操作，您必須刪除文件宣告或使用您自己的編碼配置來表示它。  
  
-   [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] 隨附的所有 XML 寫入器都拒絕 XML 處理指示 \(\<?  … ?\>\) 和文件類型定義 \(\<\!  … \>\)，因為它們在 SOAP 訊息中是不被允許的。  同樣地，您可以使用您自己的編碼機制來解決這個限制。  如果您必須在結果 XML 中包含它們，可以撰寫自訂編碼器，使用支援它們的 XML 寫入器。  
...
-   當實作 `WriteXml` 時，請避免使用在 [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] 隨附的 XML 寫入器上不支援的 <xref:System.Xml.XmlWriter.WriteEntityRef%2A> 和 <xref:System.Xml.XmlWriter.WriteNmToken%2A> 方法。  
  
## 使用 DataSet、Typed DataSet 和 DataTable  
...
-   當匯入 `DataSet` 或 `DataTable` 結構描述時，參照這些型別是很重要的。  使用 Svcutil.exe 命令列工具，這項工作可以藉由將 System.Data.dll 組件名稱傳遞至 `/reference` 參數來完成。  如果匯入具型別資料集結構描述，您必須參照具型別資料集的型別。  使用 Svcutil.exe，將具型別資料集之組件的位置傳遞至 `/reference` 參數。  [!INCLUDE[crabout](../../../../includes/crabout-md.md)]參照型別的詳細資訊，請參閱[匯入結構描述以產生類別](../../../../docs/framework/wcf/feature-details/importing-schema-to-generate-classes.md)。  
