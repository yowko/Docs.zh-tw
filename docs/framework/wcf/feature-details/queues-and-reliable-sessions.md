...
ms.locfileid: "84596743"
---
# <a name="queues-and-reliable-sessions"></a>佇列和可靠的工作階段
佇列和可靠會話是執行可靠訊息的 Windows Communication Foundation （WCF）功能。 本節所包含的主題會討論 WCF 可靠的訊息功能。  
...
- [可靠的會話](reliable-sessions.md)，提供可靠的傳輸，而不會產生高延遲的成本  
  
- [WCF 中的佇列](queues-in-wcf.md)，可提供可靠的傳輸以及來源與目的地之間的分隔。  
...
## <a name="queues"></a>佇列  
 WCF 中的佇列提供可靠的訊息傳輸，以及來源與目的地之間的分隔，代價是高延遲。 WCF 佇列通訊是以訊息佇列（也稱為 MSMQ）為基礎。  
