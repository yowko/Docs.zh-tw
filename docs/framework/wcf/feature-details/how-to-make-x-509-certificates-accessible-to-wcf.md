...
1. 授與執行 WCF 所用的帳戶，其中包含與 x.509 憑證相關聯的私密金鑰之檔案的讀取權限。  
    ...
    3. 使用[FindPrivateKey](../samples/findprivatekey.md)工具，判斷憑證的私密金鑰位於電腦上的哪個位置。  
         ...
         下列程式碼範例會使用[FindPrivateKey](../samples/findprivatekey.md)工具，以的指紋，判斷 `My` 存放區中之憑證的私密金鑰位置 `CurrentUser` `46 dd 0e 7a ed 0b 7a 31 9b 02 a3 a0 43 7a d8 3f 60 40 92 9d` 。  
