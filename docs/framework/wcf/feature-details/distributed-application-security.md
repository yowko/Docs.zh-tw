...
|[模式]|描述|  
|----------|-----------------|  
|無|未在傳輸層或訊息層提供任何安全性。 除了專案或使用程式碼時，任何預先定義的系結預設都不會使用此模式 [\<basicHttpBinding>](../../configure-apps/file-schema/wcf/basichttpbinding.md) <xref:System.ServiceModel.BasicHttpBinding> 。|  
