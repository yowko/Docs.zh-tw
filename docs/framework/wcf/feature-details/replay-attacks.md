...
## 繫結可能受制於反映攻擊  
 「*反映攻擊*」\(Reflection Attack\) 是指將訊息送回至寄件者的重新執行，就好像訊息是來自接收者的回覆一樣。[!INCLUDE[indigo1](../../../../includes/indigo1-md.md)] 機制中的標準「*重新執行偵測*」\(Replay Detection\) 不會自動處理這個情況。  
...
-   使用訊息模式安全性搭配可設定狀態的安全性內容權杖 \(包含或不包含啟用的安全對話\)。[!INCLUDE[crdefault](../../../../includes/crdefault-md.md)][HOW TO：為安全工作階段建立安全性內容權杖](../../../../docs/framework/wcf/feature-details/how-to-create-a-security-context-token-for-a-secure-session.md).  
