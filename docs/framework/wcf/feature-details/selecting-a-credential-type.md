...
ms.locfileid: "96253969"
---
# <a name="selecting-a-credential-type"></a>選取認證類型

*認證* 是 WCF) 用來建立宣告的身分識別或功能的資料 Windows Communication Foundation (。 例如，護照是政府發給的認證，以證明一個國家或地區的公民身分。 在 WCF 中，認證可以採用許多形式，例如使用者名稱權杖和 x.509 憑證。 本主題討論認證、如何在 WCF 中使用它們，以及如何為您的應用程式選取正確的認證。  
  
 在許多國家和地區，駕駛執照是認證的一個例子。 執照包含了代表一個人身分識別和能力的資料。 其中包含以持有人的照片做為所有權的證明。 執照是由受信任的授權單位發出，通常是政府的發照部門。 執照會經過密封，而且可能內含雷射防偽，表示沒有遭到竄改或偽造。  
  
 出示認證包括出示資料以及資料的所有權證明。 WCF 支援傳輸和訊息安全性層級的各種認證類型。 例如，請考慮在 WCF 中支援兩種類型的認證：使用者名稱和 (x.509) 憑證認證。  
  
 對於使用者名稱認證，使用者名稱代表宣告的身分識別，而密碼提供所有權的證明。 在此案例中，受信任的授權單位是驗證使用者名稱和密碼的系統。  
  
 使用 x.509 憑證認證時，憑證中的主體名稱、主體別名或特定欄位可以用來做為身分識別的宣告，而其他欄位（例如 `Valid From` 和 `Valid To` 欄位）則指定憑證的有效性。  
...
|基本|為用戶端指定基本驗證。 如需詳細資訊，請參閱 RFC2617-[HTTP 驗證：基本與摘要式驗證](ftp://ftp.rfc-editor.org/in-notes/rfc2617.txt)。|  
|Digest|為用戶端指定摘要式驗證。 如需詳細資訊，請參閱 RFC2617-[HTTP 驗證：基本與摘要式驗證](ftp://ftp.rfc-editor.org/in-notes/rfc2617.txt)。|  
|Ntlm|指定 NT LAN 管理員 (NTLM) 驗證。 這是用於當您因故無法使用 Kerberos 驗證時。 您也可以將屬性設定為，以停用它做為回復 <xref:System.ServiceModel.Security.WindowsClientCredential.AllowNtlm%2A> `false` ，這會讓 WCF 在使用 NTLM 時能夠盡力擲回例外狀況。 請注意，將此屬性設為 `false`，不一定能夠禁止 NTLM 認證透過網路傳送。|  
...
|使用者名稱|允許服務要求用戶端必須使用使用者名稱認證進行驗證。 請注意，WCF 不允許任何使用使用者名稱的密碼編譯作業，例如產生簽章或加密資料。 WCF 可確保使用使用者名稱認證時的傳輸安全。|  
|憑證|允許服務要求用戶端使用 X.509 憑證進行驗證。|  
|發行的權杖|根據安全性原則設定的自訂權杖類型。 預設的權杖類型為 Security Assertions Markup Language (SAML)。 權杖是由安全的權杖服務所發行。 如需詳細資訊，請參閱 [同盟和已發行的權杖](federation-and-issued-tokens.md)。|  
  
### <a name="negotiation-model-of-service-credentials"></a>服務認證的交涉模型  

 *協調* 程式是指藉由交換認證來建立用戶端與服務之間的信任。 此處理程序會在用戶端和服務之間反覆執行，以便只透露交涉處理程序中下一個步驟所需要的資訊。 實際上，最終結果是將服務的認證傳遞至要用於後續作業中的用戶端。  
  
 有一個例外狀況，在 WCF 中，系統提供的系結預設會在使用訊息層級安全性時自動協商服務認證。  (例外 <xref:System.ServiceModel.BasicHttpBinding> 狀況是，預設不會啟用安全性。 ) 若要停用此行為，請參閱 <xref:System.ServiceModel.MessageSecurityOverHttp.NegotiateServiceCredential%2A> 和 <xref:System.ServiceModel.FederatedMessageSecurityOverHttp.NegotiateServiceCredential%2A> 屬性。  
  
> [!NOTE]
> 使用 SSL 安全性搭配 .NET Framework 3.5 和更新版本時，WCF 用戶端會使用其憑證存放區中的中繼憑證以及在 SSL 協商期間收到的中繼憑證，在服務的憑證上執行憑證鏈驗證。 .NET Framework 3.0 只會使用安裝在本機憑證存放區中的中繼憑證。  
  
#### <a name="out-of-band-negotiation"></a>超出範圍交涉  

 如果停用自動交涉，在傳送任何訊息給服務之前，必須在用戶端提供服務認證。 這也稱為 *頻* 外布建。 例如，如果指定的認證類型是憑證，且停用自動交涉，用戶端就必須連絡服務擁有者，以在執行用戶端應用程式的電腦上接收並安裝憑證。 例如，在商業案例中，當您希望嚴格控制哪些用戶端可以存取服務時，可能會完成這項操作。 這項頻外協商可在電子郵件中完成，而 x.509 憑證會儲存在 Windows 憑證存放區中，並使用 Microsoft Management Console (MMC) 憑證] 嵌入式管理單元之類的工具。  
...
### <a name="setting-service-credentials"></a>設定服務認證  
 ...
 如果您是使用傳輸模式，並使用 HTTP 做為傳輸，您必須使用網際網路資訊服務 (IIS) 或使用憑證設定連接埠。 如需詳細資訊，請參閱 [傳輸安全性概觀](transport-security-overview.md) 和 [HTTP 傳輸安全性](http-transport-security.md)。  
...
- 用戶端認證會在傳送第一個訊息之前使用一次，使用 WCF 用戶端實例來建立安全性內容。 然後所有的應用程式訊息都會透過安全性內容來保護其安全。  
