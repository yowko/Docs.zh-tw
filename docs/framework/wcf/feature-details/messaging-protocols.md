...
-   B1112：[!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] 發出 `mustUnderstand` 值 0 和 1，供 SOAP 封套的 SOAP 1.1 和 SOAP 1.2 版使用。[!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] 接受 `mustUnderstand` 標頭的整個 `xs:boolean` 值空間 \(0、1、`false`、`true`\)  
  
#### SOAP 錯誤  
 以下為 [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] 專屬的 SOAP 錯誤實作清單。  
  
-   B2121：[!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] 會傳回下列 SOAP 1.1 錯誤碼：`s11:mustUnderstand`、`s11:Client` 和 `s11:Server`。  
...
-   R2221：在 SOAP 1.2 要求上呈現時，`application/soap+xml` 動作參數必須符合對應 WSDL 繫結內部之 `wsoap12:operation` 項目的 `soapAction` 屬性。  
...
#### 端點參考和 WS\-Addressing 版本  
 [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] 會實作一些使用 WS\-Addressing 的基礎結構通訊協定，特別是 `EndpointReference` 項目和 `W3C.WsAddressing.EndpointReferenceType` 類別 \(例如，WS\-ReliableMessaging、WS\-SecureConversation 和 WS\-Trust\)。[!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] 支援使用任一使用其他基礎結構通訊協定的 WS\-Addressing 版本。[!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] 端點則支援每個端點一個 WS\-Addressing 版本  
  
 若是 R3111，與 [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] 端點進行訊息交換所使用之 `EndpointReference` 項目的命名空間或型別，必須符合由此端點所實作的 WS\-Addressing 版本。  
  
 例如，若 [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] 端點實作了 WS\-ReliableMessaging，由 `CreateSequenceResponse` 內部的此類端點所傳回的 `AcksTo` 標頭，便會使用由 `EncodingBinding` 項目為此端點所指定的 WS\-Addressing 版本。  
...
#### 訊息標頭  
 對於 WS\-Addressing 的兩個版本，[!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] 會使用下列由 `wsa:To`、 `wsa:ReplyTo`、 `wsa:Action`、 `wsa:MessageID`和 `wsa:RelatesTo` 規格所規定的訊息標頭。  
...
### 訊息交換模式  
 包含在 Web 服務作業叫用內的訊息順序稱為「*訊息交換模式*」\(Message Exchange Pattern\)。[!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] 支援單向、要求\-回覆和雙工訊息交換模式。本章節將根據所使用的訊息交換模式，釐清訊息處理上的 WS\-Addressing 需求。  
...
-   R3321：要求者必須包含在 `wsa:To`、`wsa:Action`, `wsa:MessageID` 要求中，以及包含在所有由端點參考所指定的 \[reference parameters\] 或 \[reference properties\] \(或兩者\) 標頭中。  
...
-   R3324：要求者必須在回覆訊息中加入 `wsa:To`、`wsa:Action` 和 `wsa:RelatesTo` 標頭，以及在要求中所有由 `ReplyTo` 端點參考指定之 \[reference parameters\] 或 \[reference properties\] \(或兩者\) 的標頭。  
...
|程式碼|原因|  
|---------|--------|  
|wsa10:InvalidAddressingHeader|重複的 `wsa:To`、`wsa:ReplyTo`、`wsa:From` 或 `wsa:MessageID`。重複的 `wsa:RelatesTo` ，具有相同的 `RelationshipType`。|  
...
-   R3514：如果端點使用 WSDL 1.1 SOAP 1.x HTTP 繫結，並且具有以 `wsap10:UsingAddressing` 或 `wsap:UsingAddressing` 判斷提示 \(與附加的 `cdp:CompositeDuplex` 緊密結合\) 替代的原則，則傳送至端點的要求訊息必須要有其 `[address]` 屬性不等於 "http:\/\/www.w3.org\/2005\/08\/addressing\/anonymous" 的 `ReplyTo` 標頭。  
  
-   R3515：如果端點使用 WSDL 1.1 SOAP 1.x HTTP 繫結，並具有以 `wsap10:UsingAddressing` 判斷提示 \(未附加 `cdp:CompositeDuplex` 判斷提示\) 替代的原則，則傳送至端點的要求訊息必須要有其 `[address]` 屬性等於 "http:\/\/www.w3.org\/2005\/08\/addressing\/anonymous" 的 `ReplyTo` 標頭，或是根本沒有 `ReplyTo` 標頭。  
  
-   R3516：如果端點使用 WSDL 1.1 SOAP 1.x HTTP 繫結，並具有以 `wsap:UsingAddressing` 判斷提示 \(未附加 `cdp:CompositeDuplex` 判斷提示\) 替代的原則，則傳送至端點的要求訊息必須要有其 `[address]` 屬性等於 "http:\/\/www.w3.org\/2005\/08\/addressing\/anonymous" 的 `ReplyTo` 標頭。  
  
 WS\-Addressing WSDL 規格會試圖描述類似的通訊協定繫結，藉由引入具有三種文字值 \(必要項、選擇性和禁止\) 的 `<wsaw:Anonymous/>``wsa:ReplyTo 項目，指出`  標頭 \(第 3.2 節\) 上的需求。不巧的是，此類項目定義並無法特別當做 WS\-Policy 內容的判斷提示使用，因為它需要網域專屬的延伸，以支援其他使用此類項目做為判斷提示的交集。此類項目定義也指出了 `ReplyTo` 標頭的值，而非網路上的端點行為，如此將使其成為 HTTP 傳輸專屬的定義。  
  
#### 動作定義  
 WS\-Addressing 2004\/08 為 `wsdl:portType/wsdl:operation/[wsdl:input | wsdl:output | wsdl:fault]` 項目定義了 `wsa:Action` 屬性。WS\-Addressing 1.0 WSDL 繫結 \(WS\-ADDR10\-WSDL\) 定義了一個類似的屬性，即 `wsaw10:Action`。  
 ...
 B3521：[!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] 在 `wsdl:portType/wsdl:operation/[wsdl:input | wsdl:output | wsdl:fault]` 項目上使用 `wsaw10:Action` 屬性 \(如 WS\-ADDR10\-WSDL 所定義\) 來判斷對應訊息的 `Action` URI，此 URI 是由端點使用且與 WS\-Addressing 版本無關。  
  
#### 使用 WSDL 連接埠內部的端點參考  
 WS\-ADDR10\-WSDL 第 4.1 節將 `wsdl:port` 項目延伸為包含 `<wsa10:EndpointReference…/>` 子項目，以便使用 WS\-Addressing 的詞彙描述端點。[!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] 在 WS\-Addressing 2004\/08 上則擴充此公用程式，允許 `<wsa:EndpointReference…/>` 顯示為 `wsdl:port` 的子項目。  
  
-   R3531：如果端點含有以 `<wsaw10:UsingAddressing/>`原則判斷提示取代之附加原則，則對應的 `wsdl:port` 項目可以包含一個子項目 `<wsa10:EndpointReference …/>`。  
  
-   R3532：如果 `wsdl:port` 包含子項目 `<wsa10:EndpointReference …/>`，則 `wsa10:EndpointReference/wsa10:Address` 子項目值必須符合同層級 `wsdl:port`\/`wsdl:location` 項目的 `@address` 屬性值。  
  
-   R3533：如果端點含有以 `<wsap:UsingAddressing/>` 原則判斷提示替代的附加原則，則對應的 `wsdl:port` 項目可以包含子項目 `<wsa:EndpointReference …/>`。  
  
-   R3534：如果 `wsdl:port` 包含子項目 `<wsa:EndpointReference …/>`，`wsa:EndpointReference/wsa:Address` 子項目值必須符合同層級 `wsdl:port`\/`wsdl:location` 項目的 `@address` 屬性值。  
...
#### 單向訊息  
 ...
 HTTP 標頭：POST 的目的地符合 `wsa10:To` 項目內的 URI。  
...
1.  確定 SOAP 封套已編碼，並且未包含具有 "http:\/\/www.w3.org\/2004\/08\/xop\/include" 之 `[namespace name]` 和 `Include` 之 `[local name]` 的項目資訊項目。  
...
4.  建立 XOP SOAP 封套 \(為原始 SOAP 封套的複本\)，但具有在先前步驟中以 `xop:Include` 項目資訊項目建構取代而識別的每個項目資訊項目，如下所示：  
    ...
    6.  將 `href` 屬性加入至 `xop:Include` 項目，該項目具有 cid: uri 值，衍生自步驟 4b 中產生的 Content\-ID 標頭值。移除封閉的 “\<” 和 “\>” 字元、URL 逸出的剩餘字串，並加入前置詞 `cid:`。下列最小字元集為 RFC1738 和 RFC2396 逸出所需。其他可以逸出的字元。  
...
3.  對於已建構 SOAP 封套內的每個項目資訊項目，其中具有一個 `xop:Include` 項目資訊項目，做為其 \[children\] 屬性的唯一成員：  
    ...
    1.  移除 `cid:` 前置詞，並以 `xop:Include` 項目的 `@href` 屬性值解除所有 URI 逸出順序的逸出 \(RFC 2396\)。以 “\<”、“\>” 括住結果字串。  
...
-   R4132：HTTP Content\-Type 標頭必須具有含 `application/xop+xml` 值的型別參數，以雙引號括住。  
 ...
 儘管 RFC 2387 內並未明確要求使用雙引號，遵守多重\/相關媒體型別參數的文字較可能會包含如 “@” 或 “\/” 的保留字元，因此需要使用雙引號。  
...
```  
 ...
 而它實際上是一個電子郵件地址，以 “\<” 和 “\>” 括住。RFC 2822 內加入了 `[CFWS]` 前置和後置字元，用以帶出註解，且不應用於保留互通性。  
  
 R4143：Infoset MIME 部分的 Content\-ID 標頭值必須遵守 RFC 2822 實際執行的 `msg-id`，其中省略了 `[CFWS]` 前置和後置字元部分。  
  
 許多 MIME 實作比較不嚴謹的需求，會以 “\<” 和 “\>” 括住值，使其成為電子郵件地址，除了電子郵件地址之外，並以 “\<”、“\>” 括住 `absoluteURI`。這個版本的 [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] 會以下列型式使用 Content\-ID MIME 標頭的值：  
...
-   R4148：SOAP1.1 Infoset 部分必須包含 Content\-Type 標頭，其中具有 application\/xop\+xml 媒體類型，且參數類型等於 ”text\/xml” 和字元集。  
...
-   R4149：SOAP 1.2 Infoset 部分必須包含 Content\-Type 標頭，其中具有 `application/xop+xml` 媒體類型，且參數類型等於 ”`application/soap+xml`” 和 `charset`。  
    ...
    ```  
     ...
     雖然 XOP 將 `application/xop+xml` 的 `charset` 參數定義為選擇性參數，您仍舊需要使用此參數為 `text/xml` 媒體類型的 `charset` 參數提供類似於 BP 1.1 需求的互通性。  
...
#### 使用 MTOM 對 WCF Secure SOAP 1.2 訊息進行編碼  
 在此範例中，訊息是以 MTOM 和 SOAP 1.2 進行編碼，並使用 WS\-Security 予以保護。用於識別編碼的二進位部分是 `BinarySecurityToken`的內容，以及對應於已加密簽章和已加密主體之 `EncryptedData` 的 `CipherValue`。請注意，`EncryptedKey` 的 `CipherValue` 無法識別 [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] 的最佳化，因為其長度小於 1024 位元組。  
