...
### <a name="identity-claims"></a>識別宣告  
 識別的權限是一種特殊權限。 擁有此權限的宣告發出關於實體之識別的陳述式。 例如，類型為「使用者主體名稱」（UPN）的宣告，其值為 " someone@example.com "，而身分識別的許可權表示特定網域中的特定身分識別。  
...
## <a name="wcf-and-identity-model"></a>WCF 和識別模型  
 WCF 使用身分識別模型基礎結構作為執行授權的基礎。 在 WCF 中， <xref:System.ServiceModel.Description.ServiceAuthorizationBehavior> 類別可讓您指定*授權*原則做為服務的一部分。 這類授權原則稱為*外部授權原則*，而且可以根據本機原則或與遠端服務互動來執行宣告處理。 由類別所表示的授權管理員 <xref:System.ServiceModel.ServiceAuthorizationManager> 會評估外部授權原則，並使用可辨識各種認證類型（權杖）的授權原則，並以適用于內送訊息的宣告來填入所謂的*授權*內容。 授權內容會以 <xref:System.IdentityModel.Policy.AuthorizationContext> 類別表示。  
...
|<xref:System.IdentityModel.Policy.IAuthorizationComponent>|提供單一隻讀字串屬性的介面： Id。這個屬性的值在系統中執行此介面的每個實例都是唯一的。|  
|<xref:System.IdentityModel.Policy.AuthorizationContext>|包含*authorization component*一組 `ClaimSet` 具有零或多個屬性之實例的授權元件; 評估一或多個授權原則的結果。|  
...
|<xref:System.IdentityModel.Policy.IAuthorizationPolicy>|衍生自的介面 `IAuthorizationComponent` ，並由授權原則類別所執行。|  
...
- [HOW TO：比較宣告](../extending/how-to-compare-claims.md)
- [如何：建立自訂授權原則](../extending/how-to-create-a-custom-authorization-policy.md)
- [如何：為服務建立自訂授權管理員](../extending/how-to-create-a-custom-authorization-manager-for-a-service.md)
