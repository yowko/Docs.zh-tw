...
---
# <a name="how-to-host-a-wcf-service-in-a-managed-windows-service"></a>作法：在受控的 Windows 服務中裝載 WCF 服務

本主題概述建立 Windows 服務所裝載的 Windows Communication Foundation (WCF) 服務所需的基本步驟。 此案例是由受管理的 Windows 服務裝載選項所啟用，該選項是在非訊息啟動的安全環境中，裝載于 Internet Information Services (IIS) 的長時間執行 WCF 服務。 服務的存留期會改由作業系統來控制。 所有 Windows 版本都提供這個裝載選項。

Windows 服務可以透過 Microsoft Management Console (MMC) 中的 Microsoft.ManagementConsole.SnapIn 嵌入式管理單元進行管理，並設定成系統啟動時自動啟動。 這個裝載選項包含註冊應用程式域 (AppDomain) 將 WCF 服務裝載為 managed Windows 服務，因此服務的進程存留期是由服務控制管理員 (適用于 Windows 服務的 SCM) 所控制。
...
13. 將應用程式組態檔加入至專案。 以下列組態 XML 取代該檔案的內容。
    ...
    ```
     ...
     以滑鼠右鍵按一下 **方案總管** 中的 App.config 檔案，然後選取 [ **屬性**]。 在 [ **複製到輸出目錄** ] 下，選取 [ **如果更新則複製**]。
...
2. 開啟 Visual Studio 的開發人員命令提示字元，然後流覽至專案目錄。 在命令提示字元中輸入 `installutil bin\service.exe` 以安裝 Windows 服務 
     ...
     在命令提示字元中輸入 `services.msc` 以存取服務控制管理員 (SCM)。 Windows 服務應該會在 [服務] 中顯示為 "WCFWindowsServiceSample"。 WCF 服務只有在 Windows 服務正在執行時，才會回應用戶端。 若要啟動服務，請以滑鼠右鍵按一下 SCM 中的該服務，然後選取 [啟動]，或在命令提示字元中輸入 **net Start WCFWindowsServiceSample** 。
