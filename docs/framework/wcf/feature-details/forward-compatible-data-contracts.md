...
Windows Communication Foundation (WCF) 資料合約系統的一項功能，就是合約在一段時間內會以不間斷的方式演進。 也就是說，有舊版資料合約的用戶端可以與有新版相同資料合約的服務通訊，或是有新版資料合約的用戶端可以與有舊版相同資料合約通訊。 如需詳細資訊，請參閱 [最佳做法：資料合約版本控制](../best-practices-data-contract-versioning.md)。  
  
 建立新版的現有資料合約時，您可以根據需要套用大部分的版本控制功能。 不過，必須從第一版的型別內建一個版本控制功能（ *往返*），才能正常運作。  
...
### <a name="example"></a>範例  
 ...
 當 WCF 基礎結構遇到不屬於原始資料合約的資料時，資料會儲存在屬性中，並保留。 這個資料只會以暫時儲存的方式處理。 如果物件傳回到其產生位置，原始 (未知的) 資料也會傳回。 因此，這代表資料往返於原始端點之間，而且沒有遺失。 不過請注意，如果產生物件的端點要求處理資料，上述情況便不成立，端點也就必須偵測與配合變更。  
