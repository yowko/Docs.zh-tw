---
title: "進出工作流程服務的交易流動 | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
...
3.  將稱為 `PrintTransactionInfo` 的新類別加入至 `Common` 專案。此類別衍生自 <xref:System.Activities.NativeActivity>，而且會多載 <xref:System.Activities.NativeActivity.Execute%2A> 方法。  
    ...
    ```  
     ...
     這是一種原生活動，該活動可顯示環境交易的相關資訊，而且可同時用於本主題中所使用的服務和用戶端工作流程。建立方案可讓此活動在 \[**工具箱**\] 的 \[**通用**\] 區段中使用。  
  
### 實作工作流程服務  
  
1.  將稱為 `WorkflowService` 的新 [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] 工作流程服務加入至 `Common` 專案。若要這樣做，請以滑鼠右鍵按一下 `Common` 專案、依序選取 \[**加入**\]、\[**新增項目**\]、\[**已安裝的範本**\] 底下的 \[**工作流程**\] 和 \[**WCF 工作流程服務**\]。  
...
4.  將 <xref:System.ServiceModel.Activities.TransactedReceiveScope> 拖放到 <xref:System.Activities.Statements.WriteLine> 活動後面。<xref:System.ServiceModel.Activities.TransactedReceiveScope> 活動可以在 \[**工具箱**\] 的 \[**傳訊**\] 區段中找到。<xref:System.ServiceModel.Activities.TransactedReceiveScope> 活動是由 \[**要求**\] 和 \[**主體**\] 兩個區段所組成。\[**要求**\] 區段包含 <xref:System.ServiceModel.Activities.Receive> 活動。\[**主體**\] 區段包含訊息接收後，要在交易內執行的活動。  
...
7.  按一下 <xref:System.ServiceModel.Activities.Receive> 活動中的 \[**定義**\] 連結，然後進行下列設定：  
     ...
     ![為 Recieve 活動設定訊息設定](../../../../docs/framework/wcf/feature-details/media/receivemessagesettings.JPG "ReceiveMessageSettings")  
...
9. 將 `PrintTransactionInfo` 活動拖放到 <xref:System.ServiceModel.Activities.TransactedReceiveScope> 活動中 \[**主體**\] 的第二個 <xref:System.Activities.Statements.WriteLine> 活動後面。  
...
11. 將 <xref:System.Activities.Statements.WriteLine> 活動拖放到 <xref:System.Activities.Statements.Assign> 活動後面，並將其 <xref:System.Activities.Statements.WriteLine.Text%2A> 屬性設定為 "Service: Begin reply"。  
     ...
     ![加入 Assign 和 WriteLine 之後](../../../../docs/framework/wcf/feature-details/media/afteraddingsbrwriteline.JPG "AfterAddingSBRWriteLine")  
  
12. 以滑鼠右鍵按一下 <xref:System.ServiceModel.Activities.Receive> 活動並選取 \[**建立 SendReply**\]，然後將其貼在最後一個 <xref:System.Activities.Statements.WriteLine> 活動後面。按一下 `SendReplyToReceive` 活動中的 \[**定義**\] 連結，然後進行下列設定。  
  
     ![回覆訊息設定](../../../../docs/framework/wcf/feature-details/media/replymessagesettings.JPG "ReplyMessageSettings")  
  
13. 將 <xref:System.Activities.Statements.WriteLine> 活動拖放到 `SendReplyToReceive` 活動後面，並將其 <xref:System.Activities.Statements.WriteLine.Text%2A> 屬性設定為 "Service: Reply sent”。  
...
1.  將稱為 `WorkflowClient` 的新 WCF 工作流程應用程式加入至 `Common` 專案。若要這樣做，請以滑鼠右鍵按一下 `Common` 專案、依序選取 \[**加入**\]、\[**新增項目**\]、\[**已安裝的範本**\] 底下的 \[**工作流程**\] 和 \[**活動**\]。  
  
     ![加入 &#91;活動&#93; 專案](../../../../docs/framework/wcf/feature-details/media/addactivity.JPG "AddActivity")  
...
7.  將 <xref:System.Activities.Statements.WriteLine> 活動拖放到 `PrintTransactionInfo` 活動後面，並將其 <xref:System.Activities.Statements.WriteLine.Text%2A> 屬性設定為 “Client: Beginning Send”。工作流程的外觀現在應該如下圖所示：  
  
     ![加入活動](../../../../docs/framework/wcf/feature-details/media/clientaddcbswriteline.JPG "ClientAddCBSWriteLine")  
