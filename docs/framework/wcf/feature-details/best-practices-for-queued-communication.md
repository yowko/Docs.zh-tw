...
ms.locfileid: "84601344"
---
# <a name="best-practices-for-queued-communication"></a>佇列通訊的最佳做法
本主題提供 Windows Communication Foundation （WCF）中佇列通訊的建議做法。 下列各節由案例的觀點來討論建議的做法。  
...
### <a name="use-of-dead-letter-queues"></a>使用寄不出的信件佇列  
 ...
 如需詳細資訊，請參閱使用寄不出[的信件佇列來處理訊息傳輸失敗](using-dead-letter-queues-to-handle-message-transfer-failures.md)。  
...
- 並行。 並行可以增加輸送量，但也會影響對共用資源的爭用。 如需詳細資訊，請參閱[Concurrency](../samples/concurrency.md)。  
  
- 節流。 為了得到最佳效能，控制在發送器管線中的訊息數目。 如需如何執行這項操作的範例，請參閱[節流](../samples/throttling.md)。  
 ...
 如需詳細資訊，請參閱在[交易中批次處理訊息](batching-messages-in-a-transaction.md)和[Windows Vista、windows Server 2003 和 windows XP 中的佇列功能差異](diff-in-queue-in-vista-server-2003-windows-xp.md)。  
...
## <a name="queuing-with-unit-of-work-semantics"></a>有工作單元語意的佇列  
 ...
 如需詳細資訊，請參閱[在會話中分組佇列的訊息](grouping-queued-messages-in-a-session.md)。  
...
- WCF 訊息主體與 MSMQ 訊息內文不同。 使用佇列系結傳送 WCF 訊息時，WCF 訊息本文會放在 MSMQ 訊息內。 MSMQ 基礎結構未注意到此額外資訊，它只看到 MSMQ 訊息。  
...
- 使用 XML 序列化時，您可以使用專案上的屬性來指定已知型別， `KnownTypes` [\<behavior>](../../configure-apps/file-schema/wcf/behavior-of-servicebehaviors.md) 然後用來決定如何還原序列化 XML 訊息。  
