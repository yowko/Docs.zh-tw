...
本主題涵蓋 Windows Communication Foundation (WCF) WSDL 1.1、 Ws-policy 和 Ws-policyattachment 實作詳細資料，以及其他 Ws-policy 判斷提示和 WSDL 1.1 延伸 WCF 所引進。  
 ...
 WCF 實作條件約束及說明本文中所述提交至 W3C 的 Ws-policy 和 Ws-policyattachment 規格。  
...
## <a name="wcf-wsdl11-extensions"></a>WCF WSDL1.1 延伸項目  
 WCF 會使用下列 WSDL1.1 延伸項目來描述合約工作階段的需求。  
...
### <a name="soap-1x-http-binding-transport-uris"></a>SOAP 1.x HTTP 繫結傳輸 URI  
 WCF 會使用下列 Uri，表示要用於 WSDL 1.1、soap 1.1 和 SOAP 1.2 繫結延伸模組項目傳輸。  
...
## <a name="policy-assertions-implemented-by-wcf"></a>WCF 實作的原則判斷提示  
 除了 Web 服務規格中導入的原則判斷提示 (WS-*) 和本文件的其他章節所述，WCF 會實作下列原則判斷提示。  
