...
> 如需 WCF 安全性的完整指南，請參閱 [Wcf 安全性指引](https://archive.codeplex.com/?p=WCFSecurity)。  
 ...
 如果您建立了具有現有技術（例如 HTTPS、Windows 整合式安全性或使用者名稱和密碼）的安全、分散式應用程式來驗證使用者，則 WCF 會使用熟悉的概念。 WCF 不僅能與現有的安全性基礎結構整合，還可以使用安全的 SOAP 訊息，將分散式安全性延伸至僅限 Windows 的網域。 除了現有的通訊協定之外，請將 WCF 視為採用 SOAP 做為通訊協定的主要優點，來實行現有的安全性機制。 例如，識別用戶端或服務的認證 (如使用者名稱和密碼，或 X.509 憑證) 有互通的 XML 架構 SOAP 設定檔。 使用這些設定檔，訊息可以利用像 XML 數位簽章和 XML 加密的開放規格，來安全交換。 如需規格的清單，請參閱 [System-Provided 互通性系結所支援的 Web 服務通訊協定](web-services-protocols-supported-by-system-provided-interoperability-bindings.md)。  
