...
- 您的 WCF 服務必須在 Windows Server 2003 上執行，並使用 HTTP 以外的傳輸。 在 Windows Server 2003 上 IIS 6.0 裝載環境僅限於 HTTP 通訊用途。 Windows 服務應用程式不會受限於這項限制，而且可以使用任何傳輸 WCF 支援，包括 net.tcp、 net.pipe 和 net.msmq。  
