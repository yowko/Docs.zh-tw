...
除了傳輸層安全性之外，您還可以使用 Windows Communication Foundation (WCF) 中的憑證， (SOAP) 透過 HTTP (HTTPS) 或 TCP。 此主題說明這類憑證的不同驗證方法。  
  
## <a name="validation-of-https-client-certificates"></a>HTTPS 用戶端憑證的驗證  

 使用 HTTPS 在用戶端與服務之間通訊時，用戶端用來驗證服務的憑證必須支援信賴鏈結的原則。 也就是說，它必須鏈結至受信任的根憑證授權單位。 否則，HTTP 層將發出訊息為「遠端伺服器傳回錯誤：(403) 禁止」的 <xref:System.Net.WebException>。 WCF 會將這個例外狀況顯示為 <xref:System.ServiceModel.Security.MessageSecurityException> 。  
