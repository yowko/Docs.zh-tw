...
在安全工作階段中使用可設定狀態的安全性內容權杖時，工作階段可以承受正在回收的服務。 例如，當安全工作階段使用沒有狀態的 SCT 而且重設了網際網路資訊服務 (IIS)，則與該服務相關聯的工作階段資料就會遺失。 這個工作階段資料包含 SCT 權杖快取。 因此，下一次當用戶端傳送的服務是無狀態的 SCT 時，便會傳回錯誤，因為無法擷取與 SCT 相關聯的金鑰。 但是，如果使用可設定狀態的 SCT，則與 SCT 相關聯的金鑰就會包含在 SCT 中。 由於金鑰是包含在 SCT (因此也包含在訊息中)，安全工作階段就不會受到正在回收的服務所影響。 預設情況下，Windows 通信基礎 （WCF） 在安全會話中使用無狀態 ST。 此主題將詳細說明如何在安全工作階段中使用具狀態的 SCT。  
...
- 建立自訂繫結，以指定 SOAP 訊息由使用可設定狀態之 SCT 的安全工作階段來保護。  
    ...
    1. 通過將[\<自訂綁定>](../../../../docs/framework/configure-apps/file-schema/wcf/custombinding.md)添加到服務的設定檔，定義自訂綁定。  
  
        ```xml  
        <customBinding>  
        ```  
  
    2. 將[\<綁定>](../../configure-apps/file-schema/wcf/bindings.md)子項目添加到[\<自訂綁定>](../../../../docs/framework/configure-apps/file-schema/wcf/custombinding.md)。  
    ...
    3. 通過將[\<安全>](../../../../docs/framework/configure-apps/file-schema/wcf/security-of-custombinding.md)子項目添加到[\<自訂綁定>，](../../../../docs/framework/configure-apps/file-schema/wcf/custombinding.md)為發送到和從此服務發送的郵件指定身份驗證模式。  
    ...
    4. 通過將[\<安全會話>](../../../../docs/framework/configure-apps/file-schema/wcf/secureconversationbootstrap.md)子項目添加到[\<安全>，](../../../../docs/framework/configure-apps/file-schema/wcf/security-of-custombinding.md)指定在建立安全會話時用戶端的身份驗證方式。  
    ...
    5. 通過添加編碼元素（如[\<文本Messageencode>）](../../../../docs/framework/configure-apps/file-schema/wcf/textmessageencoding.md)指定消息編碼。  
...
## <a name="example"></a>範例  
 ...
 當 Windows 身份驗證與有狀態 SCT 結合使用時，WCF 不會<xref:System.ServiceModel.ServiceSecurityContext.WindowsIdentity%2A>使用實際調用方的標識填充屬性，而是將屬性設置為匿名。 由於 WCF 安全必須為來自傳入 SCT 的每個請求重新創建服務安全上下文的內容，因此伺服器不會跟蹤記憶體中的安全會話。 由於您不可能將 <xref:System.Security.Principal.WindowsIdentity> 執行個體序列化為 SCT，<xref:System.ServiceModel.ServiceSecurityContext.WindowsIdentity%2A> 屬性將傳回匿名身分識別。  
...
- [\<自訂綁定>](../../../../docs/framework/configure-apps/file-schema/wcf/custombinding.md)
