...
> [!NOTE]
> 當 Windows XP 需要模擬時，請使用不包含具狀態之 SCT 的安全工作階段。 當可設定狀態的 SCT 與模擬一起使用時，就會擲回 <xref:System.InvalidOperationException>。 如需詳細資訊，請參閱 [不支援的案例](unsupported-scenarios.md)。  
  
### <a name="to-use-stateful-scts-in-a-secure-session"></a>若要在安全工作階段中使用具狀態的 SCT  
  
- 建立自訂繫結，以指定 SOAP 訊息由使用可設定狀態之 SCT 的安全工作階段來保護。  
  
    1. 藉由將加入 [\<customBinding>](../../configure-apps/file-schema/wcf/custombinding.md) 至服務的設定檔，來定義自訂系結。  
    ...
    2. 將 [\<binding>](../../configure-apps/file-schema/wcf/bindings.md) 子項目加入至 [\<customBinding>](../../configure-apps/file-schema/wcf/custombinding.md) 。  
    ...
    3. 藉由將子專案加入至，來指定此服務所傳送之訊息的驗證模式 [\<security>](../../configure-apps/file-schema/wcf/security-of-custombinding.md) [\<customBinding>](../../configure-apps/file-schema/wcf/custombinding.md) 。  
    ...
    4. 藉由將子專案新增至，指定在建立安全會話時，用戶端的驗證方式 [\<secureConversationBootstrap>](../../configure-apps/file-schema/wcf/secureconversationbootstrap.md) [\<security>](../../configure-apps/file-schema/wcf/security-of-custombinding.md) 。  
    ...
    5. 藉由新增編碼元素（例如）來指定訊息編碼 [\<textMessageEncoding>](../../configure-apps/file-schema/wcf/textmessageencoding.md) 。  
  
        ```xml  
        <textMessageEncoding />  
        ```  
  
    6. 藉由新增傳輸元素（例如）來指定傳輸 [\<httpTransport>](../../configure-apps/file-schema/wcf/httptransport.md) 。  
...
## <a name="example"></a>範例  
 ...
 當 Windows 驗證搭配可設定狀態的 SCT 一起使用時，WCF 不會將 <xref:System.ServiceModel.ServiceSecurityContext.WindowsIdentity%2A> 實際呼叫端的身分識別填入屬性，而是將屬性設為匿名。 因為 WCF 安全性必須針對來自連入 SCT 的每個要求重新建立服務安全性內容的內容，所以伺服器不會追蹤記憶體中的安全性會話。 由於您不可能將 <xref:System.Security.Principal.WindowsIdentity> 執行個體序列化為 SCT，<xref:System.ServiceModel.ServiceSecurityContext.WindowsIdentity%2A> 屬性將傳回匿名身分識別。  
