...
> [!NOTE]
> 支援認證的規格為 WS-SecurityPolicy 規格的一部分。 如需詳細資訊，請參閱 [Web 服務安全性規格](/previous-versions/dotnet/articles/ms951273(v=msdn.10))。  
  
## <a name="supporting-tokens"></a>支援權杖  

 簡而言之，當您使用訊息安全性時，一律會使用 *主要認證* 來保護訊息 (例如，x.509 憑證或 Kerberos 票證) 。  
  
 如規格所定義，安全性系結會使用 *權杖* 來保護訊息交換。 *權杖* 是安全性認證的標記法。  
  
 安全性繫結會使用安全性繫結原則中識別的主要權杖來建立簽章。 此簽章稱為「 *訊息* 簽章」（signature）。  
...
## <a name="programming-supporting-credentials"></a>程式設計的支援認證  
 ...
 若要建立使用支援權杖的服務，您必須建立 [\<customBinding>](../../configure-apps/file-schema/wcf/custombinding.md) 。  (需詳細資訊，請參閱 how [to：使用 SecurityBindingElement 建立自訂](how-to-create-a-custom-binding-using-the-securitybindingelement.md)系結。 )   
...
- *端點支援權杖* 支援端點的所有作業。 這也就是支援權杖所表示的認證，可以在每次叫用端點作業時使用此認證。  
...
1. 建立安全性繫結項目。 底下的範例會使用 <xref:System.ServiceModel.Channels.SymmetricSecurityBindingElement> 驗證模式建立 `UserNameForCertificate`。 使用 <xref:System.ServiceModel.Channels.SecurityBindingElement.CreateUserNameForCertificateBindingElement%2A> 方法。  
