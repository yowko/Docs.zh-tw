...
當使用中繼資料端點或[System.servicemodel 中繼資料公用程式工具（Svcutil）](../../../../docs/framework/wcf/servicemodel-metadata-utility-tool-svcutil-exe.md)從 WINDOWS COMMUNICATION FOUNDATION （WCF）服務匯出中繼資料時，`DataContractSerializer` 將 CLR 類型對應至 XSD。 如需詳細資訊，請參閱[資料合約序列化程式](../../../../docs/framework/wcf/feature-details/data-contract-serializer.md)。
...
|`blockDefault`|忽略：|
|`elementFormDefault`|必須限定。 所有項目必須限定，以便由 `DataContractSerializer`支援結構描述。 這可以藉由將 xs:schema/@elementFormDefault 設定為「限定」，或在每個個別元素宣告上將 xs:element/@form 設定為「限定」來完成。|
|`finalDefault`|忽略：|
|`Id`|忽略：|
...
\<xs： complexType > 的每個複雜類型都會對應至資料合約。
...
|`attributeGroup`|禁止： 在 WCF v1 版本中，`DataContractSerializer` 會忽略 `xs:complexType`內是否存在 `attributeGroup`。|
|`anyAttribute`|禁止：|
|(空白)|對應至不包含資料成員的資料合約。|

### <a name="xssequence-in-a-complex-type-attributes"></a>在複雜類型中 \<xs： sequence >：屬性
...
### <a name="xssequence-in-a-complex-type-contents"></a>在複雜類型中 \<xs： sequence >：內容
...
### <a name="xselement-with-maxoccurs1-within-an-xssequence-data-members"></a>在 \<xs： sequence > （資料成員）中，maxOccurs = 1 的 \<xs： element >
...
### <a name="xselement-with-maxoccurs1-within-an-xssequence-collections"></a>在 \<xs： sequence > （集合）中，\<xs： element > with maxOccurs > 1
...
當使用匿名型別的 `simpleType` 和 `complexType,` 對應時，\* 與非匿名型別的相同，不同之處在于不會有匿名資料合約，因此會建立名為的資料合約，並產生衍生自專案名稱的名稱。 下表為匿名型別的規則：
...
### <a name="xsrestriction-attributes"></a>\<xs：限制 >：屬性
...
### <a name="xsrestriction-for-all-other-cases-contents"></a>\<xs：限制 > 適用于所有其他情況：內容
...
### <a name="xsrestriction-for-enumerations-attributes"></a>列舉的 \<xs：限制 >：屬性
...
### <a name="xsrestriction-for-enumerations-contents"></a>列舉的 \<xs：限制 >：內容
...
### <a name="xsextension-in-xscomplexcontent-attributes"></a>\<xs： complexContent 中的 \<xs： extension > >：屬性
...
### <a name="xsextension-in-xscomplexcontent-contents"></a>\<xs： complexContent 中的 \<xs： extension > >：內容
...
在資料合約序列化結構描述中宣告的選用屬性 `ser:FactoryType` 會參考可以還原序列化型別的處理站類別。 處理站類別必須是正在使用之 `DataContractSerializer` 執行個體的已知型別集合的一部分。 如需已知類型的詳細資訊，請參閱[資料合約已知類型](../../../../docs/framework/wcf/feature-details/data-contract-known-types.md)。
