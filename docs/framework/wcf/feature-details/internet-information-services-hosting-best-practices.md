...
## <a name="implementing-wcf-services-as-dlls"></a>將 WCF 服務實作為 DLL  
 實作 WCF 服務以部署至 Web 應用程式的 \bin 目錄的 DLL，讓您重複使用 Web 應用程式模型中，外部服務，例如可能沒有網際網路資訊服務 (IIS) 部署在測試環境中。  
...
> [!NOTE]
>  WCF 會使用訊息層可靠性以及安全性所做的通訊協定使用的動態記憶體中狀態。 WCF 可靠工作階段與安全性工作階段，則可能會因為應用程式回收而非預期地終止。 IIS 裝載的應用程式，請使用這些通訊協定應該仰賴相互關聯的應用程式層級狀態 （例如，應用程式層級建構或自訂相互關聯標頭） 或停用的 WCF 提供的工作階段金鑰以外的項目IIS 裝載的應用程式的回收處理序。  
  
## <a name="optimizing-performance-in-middle-tier-scenarios"></a>最佳化中介層案例中的效能  
 為了達到最佳效能，在*中介層案例*，呼叫以回應傳入訊息中的其他服務的服務 — 具現化給遠端服務的 WCF 服務用戶端一次，以及在多個連入重複加以使用要求。 具現化 WCF 服務用戶端是昂貴的作業，相對於進行預先存在的用戶端執行個體上呼叫的服務和中介層案例藉由快取在要求間的遠端用戶端會產生一些獨特的效能。 WCF 服務用戶端是安全執行緒，因此不需要跨多個執行緒同步處理存取具備用戶端。  
...
## <a name="wcf-in-multi-homed-or-multi-named-scenarios"></a>多重主目錄系統或多重具名案例中的 WCF  
 您可以部署在 IIS Web 伺服陣列，其中的一組電腦共用通用外部名稱的 WCF 服務 (例如http://www.contoso.com)但會個別定址，依不同的主機名稱 (例如，http://www.contoso.com可能會將兩個不同的電腦的流量名為http://machine1.internal.contoso.com和http://machine2.internal.contoso.com)。 這種部署案例 WCF，完全支援，但是需要裝載服務的中繼資料 （Web 服務描述語言） 中顯示正確的 （外部） 主機名稱的 WCF 服務的 IIS 網站的特殊組態。  
  
 若要確保正確的主機名稱，會出現在產生 WCF 服務中繼資料，設定的預設識別為裝載 WCF 服務使用明確的主機名稱的 IIS 網站。 例如，駐留在 www.contoso.com 伺服器陣列的電腦應該使用的 IIS 網站繫結 *:80:www.contoso.com http 和\*: 443:www.contoso.com https。  
...
## <a name="see-also"></a>另請參閱  
 [裝載的服務範例](http://msdn.microsoft.com/library/f703a3f6-0fba-418a-a92f-7ce75ccfa47e)  
