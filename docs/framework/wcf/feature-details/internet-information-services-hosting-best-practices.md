...
## <a name="service-hosts-in-iis-hosted-applications"></a>IIS 裝載應用程式中的服務主機  
 請勿使用命令式自我裝載 Api 來建立新的服務主機，以接聽 IIS 裝載環境原本不支援的網路傳輸（例如，IIS 6.0 來主控 TCP 服務，因為 IIS 6.0 上原本不支援 TCP 通訊）。 不建議使用此方法。 IIS 裝載環境無法辨識以命令式語法建立的服務主機。 關鍵在於，當 IIS 判斷裝載應用程式集區是否為閒置時，不會考慮透過命令式語法建立的服務所執行的處理作業。 而且內含此類以命令式語法建立之服務主機的應用程式，會透過 IIS 裝載環境強勢地處理 IIS 裝載處理序。  
...
> [!NOTE]
> WCF 用來進行訊息層可靠性和安全性的通訊協定會使用 volatile 記憶體內部狀態。 WCF 可靠會話和安全性會話可能會因為應用程式回收而非預期地終止。 使用這些通訊協定的 IIS 裝載應用程式，應該取決於 WCF 提供的工作階段金鑰，以關聯應用層狀態（例如應用層結構或自訂相互關聯標頭），或是停用裝載應用程式的 IIS 進程回收。  
...
## <a name="optimizing-performance-in-middle-tier-scenarios"></a>最佳化中介層案例中的效能  
 ...
 中介層案例同時會因為使用 `svcutil /a` 選項所產生的非同步 API 而產生一些效能。 `/a`選項會使「 [System.servicemodel 中繼資料公用程式」工具（Svcutil）](../servicemodel-metadata-utility-tool-svcutil-exe.md)為 `BeginXXX/EndXXX` 每項服務作業產生方法，這可讓您在背景執行緒上進行遠端服務的長時間執行呼叫。  
  
## <a name="wcf-in-multi-homed-or-multi-named-scenarios"></a>多重主目錄系統或多重具名案例中的 WCF  
 您可以在 IIS Web 伺服陣列內部部署 WCF 服務，其中一組電腦共用通用的外部名稱（例如）， `http://www.contoso.com` 但由不同的主機名稱個別定址（例如， `http://www.contoso.com` 可能會將流量導向至名為和的兩部不同電腦 `http://machine1.internal.contoso.com` `http://machine2.internal.contoso.com` ）。 WCF 完全支援此部署案例，但需要特殊的 IIS 網站設定，以裝載 WCF 服務，以便在服務的中繼資料（Web 服務描述語言）中顯示正確的（外部）主機名稱。  
  
 為確保 WCF 產生的服務中繼資料中出現正確的主機名稱，請設定裝載 WCF 服務之 IIS 網站的預設識別，以使用明確的主機名稱。 例如，位於伺服陣列內部的電腦 `www.contoso.com` 應使用 *：80： www. .com 的 IIS 網站系結，針對 HTTP 和 \* ：443： www. .com （適用于 HTTPS）。  
...
## <a name="application-pools-running-in-different-user-contexts-overwrite-assemblies-from-other-accounts-in-the-temporary-folder"></a>在不同使用者內容中執行的應用程式集區會覆寫來自暫存資料夾裡其他帳戶的組件  
 ...
 在 web.config 中，您可以使用來設定暫存資料夾 \<system.web/compilation/@tempFolder> 。 針對/Application1，它可以是 "c:\tempForUser1"，而在應用程式2中可以是 "c:\tempForUser2"。 針對這些資料夾賦予兩個身分識別對應的寫入權限。  
