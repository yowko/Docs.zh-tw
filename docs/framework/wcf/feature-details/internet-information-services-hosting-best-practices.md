...
## <a name="optimizing-performance-in-middle-tier-scenarios"></a>最佳化中介層案例中的效能  
 ...
 中介層案例同時會因為使用 `svcutil /a` 選項所產生的非同步 API 而產生一些效能。 `/a` 選項會使[System.servicemodel 中繼資料公用程式工具（Svcutil）](../../../../docs/framework/wcf/servicemodel-metadata-utility-tool-svcutil-exe.md)產生每個服務作業的 `BeginXXX/EndXXX` 方法，這允許在背景執行緒上進行遠端服務的長時間執行呼叫。  
  
## <a name="wcf-in-multi-homed-or-multi-named-scenarios"></a>多重主目錄系統或多重具名案例中的 WCF  
 您可以在 IIS Web 伺服陣列內部部署 WCF 服務，其中一組電腦會共用通用的外部名稱（例如 `http://www.contoso.com`），但會由不同的主機名稱個別定址（例如，`http://www.contoso.com` 可能會將流量導向至名為 `http://machine1.internal.contoso.com` 和 `http://machine2.internal.contoso.com`的兩部不同電腦。 WCF 完全支援此部署案例，但需要特殊的 IIS 網站設定，以裝載 WCF 服務，以便在服務的中繼資料（Web 服務描述語言）中顯示正確的（外部）主機名稱。  
  
 為確保 WCF 產生的服務中繼資料中出現正確的主機名稱，請設定裝載 WCF 服務之 IIS 網站的預設識別，以使用明確的主機名稱。 例如，位於 `www.contoso.com` 伺服器陣列內的電腦應該使用 *：80： www. .com 的 IIS 網站系結，並 \*：443： www. contoso .com for HTTPS。  
...
## <a name="application-pools-running-in-different-user-contexts-overwrite-assemblies-from-other-accounts-in-the-temporary-folder"></a>在不同使用者內容中執行的應用程式集區會覆寫來自暫存資料夾裡其他帳戶的組件  
 ...
 在 web.config 中，您可以使用 \<system.web/compilation/@tempFolder> 來設定暫存資料夾。 針對/Application1，它可以是 "c:\tempForUser1"，而在應用程式2中可以是 "c:\tempForUser2"。 針對這些資料夾賦予兩個身分識別對應的寫入權限。  
