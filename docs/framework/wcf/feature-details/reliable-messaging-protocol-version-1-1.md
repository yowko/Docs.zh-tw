---
title: "Reliable Messaging Protocol 1.1 版 | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
...
本主題涵蓋了 WS\-ReliableMessaging February 2007 \(1.1 版\) 通訊協定的 [!INCLUDE[indigo1](../../../../includes/indigo1-md.md)] 實作詳細資料，這個通訊協定是使用 HTTP 傳輸來進行交互操作時所需的通訊協定。[!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] 遵循本主題所述 WS\-ReliableMessaging 規格的相關限制與說明。請注意，[!INCLUDE[netfx35_long](../../../../includes/netfx35-long-md.md)] 中開始實作 WS\-ReliableMessaging 1.1 版通訊協定。  
  
 WS\-ReliableMessaging February 2007 通訊協定可透過 <xref:System.ServiceModel.Channels.ReliableSessionBindingElement> 在 [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] 中實作。  
...
-   R1102：`CreateSequence` 訊息中的 `AcksTo`、`ReplyTo` 和 `Offer/Endpoint` 端點參照必須具有包含相同字串表示的位址值以符合八位元規格。  
  
    -   [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] 回應程式在建立序列之前，會先驗證 `AcksTo`、`ReplyTo` 和 `Endpoint` 端點參照的 URI 部分是否相同。  
  
-   R1103：`CreateSequence` 訊息中的 `AcksTo`、`ReplyTo` 和 `Offer/Endpoint` 端點參照應該具有相同的參照參數集合。  
  
    -   [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] 不會強制執行，但會假定 `CreateSequence` 上的 `AcksTo`、`ReplyTo` 和 `Offer/Endpoint` 端點參照之參照參數都是一樣，並且使用來自 `ReplyTo` 端點參照的參照參數，以認可並與序列訊息對話。  
  
-   B1104：[!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] 啟動器不會在 `CreateSequence` 訊息中產生選用的 `Expires` 或 `Offer/Expires` 項目。  
  
-   B1105：在存取 `CreateSequence` 訊息時，[!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] 回應程式會使用 `CreateSequence` 項目中的 `Expires` 值做為 `CreateSequenceResponse` 項目中的 `Expires` 值。否則，[!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] 回應程式將讀取並忽略 `Expires` 和 `Offer/Expires` 值。  
  
-   B1106：在存取 `CreateSequenceResponse` 訊息時，[!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] 啟動器會讀取選用的 `Expires` 值但不會加以使用。  
  
-   B1107：[!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] 啟動器和回應程式一律在 `CreateSequence/Offer` 和 `CreateSequenceResponse` 項目中產生選用的 `IncompleteSequenceBehavior` 項目。  
  
-   B1108：[!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] 只會使用 `IncompleteSequenceBehavior` 項目中的 `DiscardFollowingFirstGap` 和 `NoDiscard` 值。  
  
    -   WS\-ReliableMessaging 會使用 `Offer` 機制來建立兩個反向關聯序列以形成工作階段。  
  
-   B1109：如果 `CreateSequence` 包含 `Offer` 項目，單向 [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] 回應程式會透過不含 `Accept` 項目的 `CreateSequenceResponse` 來回應以拒絕提供的序列。  
...
-   R1112：當兩個反向序列都是透過 `Offer` 機制建立時，`CreateSequenceResponse/Accept/AcksTo` 端點參照之 `[address]` 屬性的每個位元組必須完全符合 `CreateSequence` 訊息之目的 URI 的每個位元組。  
  
-   R1113：當兩個反向序列都是透過 `Offer` 機制建立時，從啟動器流向回應程式之雙邊序列上的所有訊息必須傳送至相同的端點參照。  
  
 [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] 使用 WS\-ReliableMessaging 在啟動器與回應程式之間建立可靠工作階段。[!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] WS\-ReliableMessaging 實作會針對單向、要求\-回覆與全雙工訊息模式提供可靠工作階段。`CreateSequence` 和 `CreateSequenceResponse` 上的 WS\-ReliableMessaging `Offer` 機制可讓您建立兩個相互關聯的反向序列，並提供適用於所有訊息端點的工作階段通訊協定。由於 [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] 會針對此類工作階段提供安全性保證，包含工作階段完整性的端對端保護，因此可以實際地確保訊息會抵達預定的相同目的地。這麼做也可以針對應用程式訊息進行 "piggy\-backing" 的序列認可作業。因此，R1102、R1112，和 R1113 的約束條件適用於 [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)]。  
...
### 終止序列  
 [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] 在完成了 `CloseSequence/CloseSequenceResponse` 交握之後，主要使用 `TerminateSequence/TerminateSequenceResponse` 交握。[!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] 可信賴傳訊目的地不會啟始終止作業，而且可信賴傳訊來源不支援從目的地啟始的可信賴傳訊終止作業。以下是適用的條件約束：  
  
-   B1301：[!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] 啟動器只會在成功完成 `CloseSequence/CloseSequenceResponse` 交握之後，才傳送 `TerminateSequence` 訊息。  
  
-   R1302：[!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] 會針對特定序列，驗證所有 `CloseSequence` 和 `TerminateSequence` 訊息上的 `LastMsgNumber` 項目是否一致。也就是說，`LastMsgNumber` 不是不存在於所有 `CloseSequence` 和 `TerminateSequence` 訊息上，就是存在於所有 `CloseSequence` 和 `TerminateSequence` 訊息上而且全部一樣。  
  
-   B1303：在 `CloseSequence/CloseSequenceResponse` 交握之後收到 `TerminateSequence` 訊息時，可信賴傳訊目的地會以 `TerminateSequenceResponse` 訊息來回應。由於可信賴傳訊來源在傳送 `TerminateSequence` 訊息之前已經獲得最後的認可，因此可信賴傳訊目的地毫無疑問地知道序列已結束並立即回收資源。  
  
-   B1304：在 `CloseSequence/CloseSequenceResponse` 交握之前收到 `TerminateSequence` 訊息時，[!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] 可信賴傳訊目的地會以 `TerminateSequenceResponse` 訊息來回應。如果可信賴傳訊目的地判斷序列中沒有任何不一致的項目，則可信賴傳訊目的地會等候應用程式目的地指定的時間一到再回收資源，以便讓用戶端有機會接收最後的認可。否則，可信賴傳訊目的地會立即回收資源並引發 `Faulted` 事件以向應用程式目的地指出序列是在不確定的情況下結束。  
...
-   B1401：[!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] 會產生並存取不超過 `xs:long` 最大含括值 \(9223372036854775807\) 的序號。  
...
-   R1601：當兩個反向序列透過 `Offer` 機制建立時，`SequenceAcknowledgement` 標頭可以包含在任何傳送至目的收件者的應用程式訊息中。遠端端點必須能夠存取以 Piggyback 方式傳送的 `SequenceAcknowledgement` 標頭。  
  
-   B1602：[!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] 不會產生包含 `Nack` 項目的 `SequenceAcknowledgement` 標頭。[!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] 會驗證每個 `Nack` 項目是否都包含序號，但是另一方面忽略 `Nack` 項目與值。  
...
-   B1801：當下列任一情形發生時，[!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] 就會產生並傳送 `Message Addressing Header Required` 錯誤：  
...
-   B1802：[!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] 會產生並傳送 `Endpoint Unavailable` 錯誤以指出沒有任何一個接聽中的端點可依據 `CreateSequence` 訊息中的定址標頭檢查結果來處理序列。  
  
## 通訊協定組合  
  
### 與 WS\-Addressing 組合  
 [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] 支援兩種版本的 WS\-Addressing：WS\-Addressing 2004\/08 \[WS\-ADDR\] 和 W3C WS\-Addressing 1.0 建議使用 \[WS\-ADDR\-CORE\] 和 \[WS\-ADDR\-SOAP\]。  
...
-   R2402：可信賴傳訊啟動器不可以將帶有 `wsrm:UsesSequenceSSL` 標頭的 `CreateSequence` 訊息傳送到 [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] 回應程式中。  
...
-   B3001：[!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] 會將 `wsrmn:RMAssertion` WS\-Policy 判斷提示附加至 `wsdl:binding` 項目。[!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] 同時支援附加至 `wsdl:binding` 和 `wsdl:port` 項目。  
...
## WS\-ReliableMessaging 延伸的流量控制  
 ...
 您可以將 `ReliableSessionBindingElement` 的 `FlowControlEnabled``boolean` 屬性設為 `true` 來啟用流量控制。下列清單列出適用於 [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] 的條件約束：  
  
-   B4001：一旦啟用了可信賴傳訊流量控制，[!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] 會在 `SequenceAcknowledgement` 標頭的項目擴充性中產生 `netrm:BufferRemaining` 項目，如下列範例所示。  
...
-   B4002：即使啟用了可信賴傳訊流量控制，[!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] 也不需要 `SequenceAcknowledgement` 標頭中的 `netrm:BufferRemaining` 項目。  
  
-   B4003：[!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] 可信賴傳訊目的地使用 `netrm:BufferRemaining` 來指出能夠緩衝處理的新訊息數目。  
  
-   B4004：一旦啟用了可信賴傳訊流量控制，[!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] 可信賴傳訊來源就會使用 `netrm:BufferRemaining` 的值來調節訊息傳輸的流量。  
...
#### CreateSequence 交換  
 [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] 啟動器會透過 HTTP 要求將不包含 `Offer` 項目的 `CreateSequence` 訊息傳送出去，並預期透過 HTTP 回應接收 `CreateSequenceResponse` 訊息。[!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] 回應程式會建立序列，並透過 HTTP 回應將不包含 `Accept` 項目的 `CreateSequenceResponse` 訊息傳送出去。  
...
#### CreateSequence 交換  
 [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] 啟動器會透過 HTTP 要求將不包含 `Offer` 項目的 `CreateSequence` 訊息傳送出去。[!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] 回應程式會建立序列，並透過 HTTP 要求將不包含 `Accept` 項目的 `CreateSequenceResponse` 訊息傳送出去。  
...
#### CreateSequence 交換  
 [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] 啟動器會透過 HTTP 要求將包含 `Offer` 項目的 `CreateSequence` 訊息傳送出去。[!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] 回應程式會確保 `CreateSequence` 包含 `Offer` 項目，然後建立序列並將包含 `Accept` 項目的 `CreateSequenceResponse` 訊息傳送出去。  
...
#### CreateSequence 交換  
 [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] 啟動器會透過 HTTP 要求將包含 `Offer` 項目的 `CreateSequence` 訊息傳送出去，並預期透過 HTTP 回應接收 `CreateSequenceResponse` 訊息。[!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] 回應程式會建立序列，並透過 HTTP 回應將包含 `Accept` 項目的 `CreateSequenceResponse` 訊息傳送出去。  
...
#### CloseSequence 交換  
 ...
 隱含地關閉要求序列會一併關閉回覆序列。也就是說，[!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] 啟動器會在 `CloseSequence` 訊息中包含回覆序列的最終 `SequenceAcknowledgement`，而且回覆序列不會有 `CloseSequence` 交換。  
...
#### TerminateSequence 交換  
 ...
 就像 `CloseSequence` 交換一樣，終止要求序列會隱含地終止回覆序列。也就是說，[!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] 啟動器會在 `TerminateSequence` 訊息中包含回覆序列的最終 `SequenceAcknowledgement`，而且回覆序列不會有 `TerminateSequence` 交換。  
...
-   [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] 可確保所有回覆都帶有正確且遵循 `wsa` 要求\/回覆相互關聯規則的 `RelatesTo` 和 `To` 標頭。