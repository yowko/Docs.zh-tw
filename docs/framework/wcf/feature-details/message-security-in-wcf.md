---
title: "WCF 中的訊息安全性 | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
...
## 什麼是訊息安全性？  
 訊息安全性會使用 WS\-Security 規格來保護訊息的安全。WS\-Security 規格描述 SOAP 訊息的加強功能，以確保 SOAP 訊息層級 \(而非傳輸層級\) 的機密性、完整性和驗證。  
...
-   端對端安全性。如 Secure Sockets Layer \(SSL\) 等安全傳輸只有在通訊是點對點時才會保護訊息安全。如果訊息在到達最終接收者之前，傳送至一個或多個 SOAP 媒介 \(例如路由器\)，則只要一個媒介從網路讀取該訊息，該訊息本身便不受保護。此外，用戶端驗證資訊只供第一個媒介使用，如有需要，必須以超出範圍方式重新傳輸至最終接收者。即使整個路由在個別躍點之間使用 SSL 安全性也適用。由於訊息安全性是直接和訊息一起運作，並在其中保護 XML 的安全，因此不論訊息在到達最終接收者之前涉及多少個媒介，安全性都會伴隨訊息。這會啟用真正的端對端安全性情節。  
...
-   支援一整組認證和宣告。訊息安全性是以 WS\-Security 規格為基礎，提供可傳輸 SOAP 訊息內任何類型宣告的可延伸架構。和傳輸安全性不同的是，您可以使用的驗證機制 \(或宣告\) 的集合不受傳輸功能的限制[!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] 訊息安全性包括多種類型的驗證和宣告傳輸，如有需要，還可以延伸以支援其他的類型。例如，基於這些原因，同盟認證情節一定要有訊息安全性。[!INCLUDE[crabout](../../../../includes/crabout-md.md)] WCF 支援之同盟情節的詳細資訊，請參閱[聯合與發行的權杖](../../../../docs/framework/wcf/feature-details/federation-and-issued-tokens.md)。  
...
## 請參閱  
 ...
 [Microsoft 典範與實例，第 3 章：實作傳輸和訊息層安全性](http://go.microsoft.com/fwlink/?LinkId=88897)