 ...
 **服務**節點會顯示所有目前指派的組態檔中的服務。 在樹狀目錄中每一個子節點對應至的子元素 <`services`> 組態檔中的項目。  
  
 當您按一下**服務** 節點，您可以檢視或執行服務上的工作中的摘要頁面**詳細**窗格。  
  
#### <a name="creating-a-new-service-configuration"></a>建立新的服務組態  
 您可以透過下列方式建立新的服務組態：  
  
-   使用精靈： 按一下此連結**建立新服務...** 在工作窗格的 [摘要] 頁面來啟動精靈。 您也可以因此在執行**檔案**功能表-> **加入新項目**。  
  
-   以手動方式建立： 以滑鼠右鍵按一下**服務**節點，然後選擇 **新服務**。  
  
#### <a name="creating-a-new-service-endpoint-configuration"></a>建立新的服務端點組態  
 您可以透過下列方式建立新的服務端點組態：  
  
-   使用精靈來建立： 按一下此連結**建立新的服務端點...** 在工作窗格的 [摘要] 頁面來啟動精靈。 您也可以因此在執行**檔案**功能表-> **加入新項目**。  
  
-   以手動方式建立： 一旦建立服務時，您可以以滑鼠右鍵按一下**端點**節點，然後選擇 "**新的服務端點**"。  
...
> [!NOTE]
>  您無法編輯的值[ \<baseAddressPrefixFilters >](../../../docs/framework/configure-apps/file-schema/wcf/baseaddressprefixfilters.md)這項工具內部。 若要新增或修改這個項目，您應使用文字編輯器或 Visual Studio。  
  
### <a name="client"></a>用戶端  
 **用戶端**節點會顯示所有的用戶端端點組態檔中。 在樹狀目錄中每一個子節點對應至的子元素 <`client`> 組態檔中的項目。  
  
 當您按一下**用戶端** 節點，您可以檢視或用戶端上執行工作**摘要頁面**中**詳細資料 窗格**。  
  
#### <a name="creating-a-new-client-endpoint-configuration"></a>建立新的用戶端端點組態  
 您可以透過下列方式來建立新的用戶端端點組態：  
  
-   使用精靈來建立： 按一下此連結**建立新的用戶端...** 在**工作窗格**上視窗的左下方或**摘要頁面**來啟動精靈。 您也可以因此在執行**檔案**功能表-> **加入新項目**。 精靈會提示您指向服務組態的位置，從這裡產生用戶端組態。 然後您可以選擇要連接的服務端點。  
  
-   以手動方式建立： 以滑鼠右鍵按一下**端點**節點下的**用戶端**，然後選擇 **新增用戶端端點**。  
...
### <a name="standard-endpoint"></a>標準端點  
 ...
 此類組態設定會儲存在**標準端點**節點。 **標準端點**節點會顯示所有的標準端點設定的組態檔中。 在樹狀目錄中每一個子節點對應至中的子元素`<standardEndpoints>`組態檔中的項目。  
  
 當您按一下**標準端點** 節點，您可以檢視或執行工作的標準端點**摘要頁面**中**詳細資料 窗格**。  
  
#### <a name="creating-a-new-standard-endpoint-configuration"></a>建立新的標準端點組態  
 您可以透過下列方式建立新的標準端點組態：  
  
-   以滑鼠右鍵按一下**標準端點**節點，然後選取**新增標準端點設定...** 在對話方塊中選取的繫結類型，然後按一下**確定**。  
  
-   選取**標準端點**節點，然後按一下**新增標準端點設定...** 在**工作窗格**上視窗的左下方。  
  
 **建立新的標準端點**對話方塊會顯示，並且列出所有已註冊的標準端點型別。  
...
-   按一下**標準端點**節點。 在**工作**] 窗格中，按一下 [**刪除標準端點設定**。  
 ...
 如果標準端點正在使用中，當您嘗試刪除時將會顯示警告訊息：「**標準端點正在使用。如果您現在要將它刪除，請確定刪除位於組態其他部分 (例如，服務端點或用戶端端點) 的所有參考。否則組態將無效，且下一次將無法開啟。確定要刪除標準端點嗎？ 」**  
  
### <a name="binding"></a>繫結  
 繫結組態是用來設定端點上的繫結。 此類組態設定會儲存在**繫結**節點。 端點會依據名稱參考繫結組態，而多個端點可參考單一繫結組態。  
  
 **繫結**節點會顯示所有繫結設定的組態檔中。 在樹狀目錄中每一個子節點對應至中的子元素 <`bindings`> 組態檔中的項目。  
  
 當您按一下**繫結** 節點，您可以檢視或執行工作之繫結上**摘要頁面**中**詳細資料 窗格**。  
...
-   在服務或用戶端摘要頁面中，按一下 **按一下以建立**中**繫結組態**欄位，建立對應的端點的繫結組態。  
...
2.  選取有一個繫結延伸項目**繫結項目延伸位置**> 一節。  
  
3.  使用**向上**或**向**清單來變更所選取項目的位置左邊的按鈕。  
...
3.  按一下**記錄層級**連結調整記錄層級。 記錄層級共有三個：格式錯誤、服務和傳輸。  
  
4.  您可以按一下接聽項名稱，指定記錄檔案與選項。  
  
> [!NOTE]
>  如果您想要追蹤與訊息記錄檔，您的應用程式關閉時自動清除，請啟用**自動排清**選項。  
  
 **診斷****摘要頁面**可讓您完成設定診斷最常見的工作。 不過，如果您想要手動編輯接聽程式和來源的設定，您必須展開**診斷**中的節點，然後編輯設定**訊息記錄**，**接聽程式**和**來源**節點。  
...
3.  中的追蹤檔案名稱中的型別**InitData**欄位。 您可以按一下"..."按鈕瀏覽至的路徑。  
  
4.  按一下**TypeName**列會顯示"..."按鈕。 按一下這個按鈕可開啟**追蹤接聽項型別瀏覽器**，可用來尋找已安裝的預先設定的追蹤接聽項。  
  
5.  請注意**來源**> 一節。 按一下**新增**開啟下拉式功能表的對話方塊中，本節會列出可用的追蹤來源。 選取追蹤來源，然後按一下**確定**。  
 ...
 **行為**節點會顯示目前的組態檔中未定義的行為。  
  
 行為組態是用來設定端點與服務的行為。 此類組態設定會儲存在**進階**節點下的**服務行為**和**端點行為**。 服務會使用服務行為，而端點則會使用端點行為。  
...
3.  選取行為延伸項目中的**行為元素延伸位置**> 一節。  
  
4.  使用**向上**或**向**清單來變更所選取項目的位置左邊的按鈕。  
...
4.  在**檔案**功能表上，指向**加入新項目**按一下**服務**。 [新增服務項目精靈] 隨即開啟。  
