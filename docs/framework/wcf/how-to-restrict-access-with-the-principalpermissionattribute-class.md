...
6.  按一下**新增** 按鈕，將新成員加入群組。  
  
7.  如果您將自己加入群組中，而且想要測試以下程式碼，則必須先登出電腦然後再登入，才能包含在該群組中。  
  
### <a name="to-demand-user-membership"></a>要求使用者成員資格  
  
1.  開啟 Windows Communication Foundation (WCF) 的程式碼檔案，其中包含實作的服務合約程式碼。 如需實作合約的詳細資訊，請參閱[實作服務合約](../../../docs/framework/wcf/implementing-service-contracts.md)。  
...
## <a name="using-a-certificate-to-control-access-to-a-method"></a>使用憑證控制方法的存取  
 ...
 若要檢查其屬性的憑證，請參閱[How to： 使用 MMC 嵌入式管理單元檢視憑證](../../../docs/framework/wcf/feature-details/how-to-view-certificates-with-the-mmc-snap-in.md)。 若要尋找的憑證指紋值，請參閱[How to： 擷取憑證的指紋](../../../docs/framework/wcf/feature-details/how-to-retrieve-the-thumbprint-of-a-certificate.md)。  
