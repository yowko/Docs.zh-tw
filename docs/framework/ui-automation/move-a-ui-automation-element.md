...
## <a name="example"></a>範例  
 下列範例會使用 <xref:System.Windows.Automation.WindowPattern> 和 <xref:System.Windows.Automation.TransformPattern> 控制項模式，以程式設計方式將 Win32 目標應用程式移至不同的螢幕位置，並追蹤 <xref:System.Windows.Automation.AutomationElement.BoundingRectangleProperty> 的 <xref:System.Windows.Automation.AutomationElement.AutomationPropertyChangedEvent>。  
