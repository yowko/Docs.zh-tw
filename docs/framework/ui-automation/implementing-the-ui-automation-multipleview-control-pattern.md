...
-   <xref:System.Windows.Automation.Provider.IMultipleViewProvider> 也應該實作管理目前檢視不同於提供目前檢視的控制項是否在容器上。 例如，Windows 檔案總管包含目前資料夾內容的清單控制項，而控制項的檢視則是由 Windows 檔案總管應用程式管理。  
