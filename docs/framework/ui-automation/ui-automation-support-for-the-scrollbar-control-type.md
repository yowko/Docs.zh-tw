...
|控制項檢視|內容檢視|  
|-----------|----------|  
|ScrollBar<br /><br /> -   按鈕 \(2 或 4 個\)<br />-   縮圖 \(0 或 1 個\)|不適用。 捲軸控制項不包含內容。|  
...
|<xref:System.Windows.Automation.AutomationElementIdentifiers.IsContentElementProperty>|False|捲軸控制項不是內容項目。 如果捲軸是獨立的控制項，則必須符合滑桿控制項類型，並傳回 `ControlType` 屬性的 `ControlType.Slider`。|  
...
## 必要的使用者介面自動化事件  
 下表列出所有捲軸控制項都必須支援的 [!INCLUDE[TLA2#tla_uiautomation](../../../includes/tla2sharptla-uiautomation-md.md)] 事件。 如需 [UI Automation Events Overview](../../../docs/framework/ui-automation/ui-automation-events-overview.md) 事件的詳細資訊，請參閱  
