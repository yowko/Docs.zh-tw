...
## <a name="servers-and-clients"></a>伺服器和用戶端  
 ...
 使用者介面自動化提供者可以提供資訊給 Active Accessibility 的用戶端，Active Accessibility 伺服器則可以提供資訊給使用者介面自動化用戶端應用程式。 不過，因為 Active Accessibility 不會公開 [!INCLUDE[TLA2#tla_uiautomation](../../../includes/tla2sharptla-uiautomation-md.md)]的資訊，所以這兩個模型並不完全相容。  
  
<a name="UI_Elements_compare"></a>   
## <a name="ui-elements"></a>UI 項目  
 Active Accessibility 提供 [!INCLUDE[TLA2#tla_ui](../../../includes/tla2sharptla-ui-md.md)] 元素做為 `IAccessible` 介面或子識別碼。 因此我們很難藉由比較兩者的 `IAccessible` 指標，來判斷它們是否參考同一個項目。  
...
|STATE_SYSTEM_UNAVAILABLE|<xref:System.Windows.Automation.AutomationElement.IsEnabledProperty>|Y|  
 ...
 下列狀態不是由大部分的 Active Accessibility 控制伺服器所執行，或在 [!INCLUDE[TLA2#tla_uiautomation](../../../includes/tla2sharptla-uiautomation-md.md)]中沒有對等的。  
