...
## <a name="scaling-in-windows-vista"></a>Windows Vista 中的縮放比例  
 ...
 實際上，當使用者將縮放比例設定為 120 DPI 時，螢幕上的垂直或水準英寸會變大達25%。 所有維度都會因此調整。 來自畫面上方和左邊緣的應用程式視窗位移會增加 25%。 如果應用程式調整已啟用，且應用程式不能感知 DPI，則視窗的大小會以相同比例增加，連同其包含的所有 [!INCLUDE[TLA2#tla_ui](../../../includes/tla2sharptla-ui-md.md)] 元素的位移和大小。  
