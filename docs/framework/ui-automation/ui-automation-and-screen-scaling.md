...
從 Windows Vista 開始，Windows 可讓使用者變更每英寸的時數（DPI）設定，讓畫面上大部分的使用者介面（UI）元素看起來更大。 雖然 Windows 中已提供這項功能，但在舊版中，調整必須由應用程式來執行。 從 Windows Vista 開始，桌面視窗管理員會針對所有不會處理其本身調整的應用程式執行預設調整。 使用者介面自動化用戶端應用程式必須將這項功能納入考量。  
  
<a name="Scaling_in_Windows_Vista"></a>
## <a name="scaling-in-windows-vista"></a>Windows Vista 中的縮放比例  
 預設 DPI 設定為96，這表示96圖元的寬度或高度會占一個名義英寸。 「英吋」的確切測量值取決於監視器的大小和實體解析度。 例如，在 12 英吋寬、1280 像素水平解析度的監視器上，96 像素的水平線長度約一英吋的 9/10。  
 ...
 實際上，當使用者將縮放比例設定為 120 DPI 時，螢幕上的垂直或水準英寸會變大達25%。 所有維度都會因此調整。 來自畫面上方和左邊緣的應用程式視窗位移會增加 25%。 如果應用程式調整已啟用，且應用程式不能感知 DPI，則視窗的大小會以相同比例增加，連同其包含的所有元素的位移和大小 [!INCLUDE[TLA2#tla_ui](../../../includes/tla2sharptla-ui-md.md)] 。  
...
> 根據預設，當使用者將 DPI 設定為120時，DWM 不會針對非 DPI 感知的應用程式執行調整，但會在 DPI 設定為144或更高的自訂值時執行。 不過，使用者可以覆寫此預設行為。  
 ...
 假設您設計的對話方塊在座標 (100, 48) 上有一個按鈕。 以預設 96 DPI 顯示此對話方塊時，按鈕會位於（100，48）的實體座標中。 在 120 DPI 中，它位於（125，60）的實體座標。 但是邏輯座標在任何 DPI 設定上都相同：（100，48）。  
...
1. 首先，將用戶端應用程式設為 DPI 感知。 若要這麼做，請 `SetProcessDPIAware` 在啟動時呼叫 Win32 函數。 以 Managed 程式碼的下列宣告，讓這個函式成為可用的。  
...
> 請勿使用 <xref:System.Windows.Forms.Cursor.Position%2A?displayProperty=nameWithType>。 這個屬性在縮放環境中用戶端視窗之外的行為是未定義的。  
 ...
 如果您的應用程式執行與非 DPI 感知應用程式的直接跨進程通訊，您可能會使用 Win32 函式和，在邏輯和實體座標之間轉換 `PhysicalToLogicalPoint` `LogicalToPhysicalPoint` 。  
