---
title: UI 自動化控制項模式概觀
description: 請參閱消費者介面自動化控制項模式的總覽。 不論類型或外觀為何，控制項模式都可讓您分類和公開控制項的功能。
ms.date: 03/30/2017
helpviewer_keywords:
- control patterns
...
## <a name="ui-automation-providers-and-clients"></a>使用者介面自動化提供者和用戶端  
 ...
 用戶端會使用 <xref:System.Windows.Automation.AutomationElement> (的方法，例如 <xref:System.Windows.Automation.AutomationElement.GetCurrentPropertyValue%2A?displayProperty=nameWithType> 或 <xref:System.Windows.Automation.AutomationElement.GetCachedPropertyValue%2A?displayProperty=nameWithType>) ，或使用 common LANGUAGE runtime (CLR) 存取子來存取 [!INCLUDE[TLA2#tla_uiautomation](../../../includes/tla2sharptla-uiautomation-md.md)] 模式的屬性。 例如，每個控制項模式類別都有一個欄位成員 (例如， <xref:System.Windows.Automation.InvokePattern.Pattern?displayProperty=nameWithType> 或是 <xref:System.Windows.Automation.SelectionPattern.Pattern?displayProperty=nameWithType> 識別該控制項模式的) ，而且可以做為參數傳遞至 <xref:System.Windows.Automation.AutomationElement.GetCachedPattern%2A> 或 <xref:System.Windows.Automation.AutomationElement.GetCurrentPattern%2A> 來取得的模式 <xref:System.Windows.Automation.AutomationElement> 。  
...
|<xref:System.Windows.Automation.GridPattern>|<xref:System.Windows.Automation.Provider.IGridProvider>|用於支援格線功能 (例如調整大小及移動至指定的儲存格) 的控制項。 例如，在 Windows 檔案總管中的大型圖示視圖，或在 Microsoft Word 中沒有標頭的簡單資料表。|  
|<xref:System.Windows.Automation.GridItemPattern>|<xref:System.Windows.Automation.Provider.IGridItemProvider>|用於格線中有儲存格的控制項。 個別儲存格應該支援 GridItem 模式。 例如，Microsoft 中的每個資料格 Windows 檔案總管詳細資料檢視。|  
...
|<xref:System.Windows.Automation.WindowPattern>|<xref:System.Windows.Automation.Provider.IWindowProvider>|公開視窗特定的資訊 (Microsoft Windows 作業系統的基本概念)。 Windows 控制項的範例包括最上層應用程式視窗 (Microsoft Word、Microsoft Windows 檔案總管等等) 、多重文件介面 (MDI) 子視窗和對話方塊。|  
