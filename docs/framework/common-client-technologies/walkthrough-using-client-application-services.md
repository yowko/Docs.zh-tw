...
2.  <span data-ttu-id="7f4bf-129">在 [新增專案] 對話方塊中，展開 [專案類型] 窗格中的 [Visual Basic] 或 [Visual C#] 節點，然後選取 [Windows] 專案類型。</span><span class="sxs-lookup"><span data-stu-id="7f4bf-129">In the **New Project** dialog box, in the **Project types** pane, expand the **Visual Basic** or **Visual C#** node and select the **Windows** project type.</span></span>  
...
5.  <span data-ttu-id="7f4bf-156">在 [方案總管] 中，選取 AppServices 專案，然後在 [專案]  功能表上，選取 [AppServices 屬性] 。</span><span class="sxs-lookup"><span data-stu-id="7f4bf-156">In **Solution Explorer**, select the AppServices project, and then on the **Project** menu, select **AppServices Properties**.</span></span>  
...
11. <span data-ttu-id="7f4bf-168">在 `<system.web>` 開頭標記 (Tag) 之後加入下列標記 (Markup)，將其包含在 `<system.web>` 項目中。</span><span class="sxs-lookup"><span data-stu-id="7f4bf-168">Add the following markup after the `<system.web>` opening tag so that it is within the `<system.web>` element.</span></span>  
...
5.  <span data-ttu-id="7f4bf-182">按 [ **下一步** ] 兩次。</span><span class="sxs-lookup"><span data-stu-id="7f4bf-182">Click **Next** twice.</span></span>  
...
13. <span data-ttu-id="7f4bf-203">按一下 [ **繼續**]。</span><span class="sxs-lookup"><span data-stu-id="7f4bf-203">Click **Continue**.</span></span>  
...
5.  <span data-ttu-id="7f4bf-236">在設計工具中，按兩下表單介面以產生名為 <xref:System.Windows.Forms.Form.Load?displayProperty=nameWithType> 的 `Form1_Load`事件處理常式。</span><span class="sxs-lookup"><span data-stu-id="7f4bf-236">In the designer, double-click the form surface to generate a <xref:System.Windows.Forms.Form.Load?displayProperty=nameWithType> event handler named `Form1_Load`.</span></span>  
...
## <a name="adding-a-login-form-as-a-credentials-provider"></a><span data-ttu-id="7f4bf-251">加入登入表單當做認證提供者</span><span class="sxs-lookup"><span data-stu-id="7f4bf-251">Adding a Login Form as a Credentials Provider</span></span>  
...
## <a name="adding-role-based-functionality"></a><span data-ttu-id="7f4bf-304">加入以角色為基礎的功能</span><span class="sxs-lookup"><span data-stu-id="7f4bf-304">Adding Role-Based Functionality</span></span>  
...
5.  <span data-ttu-id="7f4bf-319">將下列方法加入 Form1 類別的結尾。</span><span class="sxs-lookup"><span data-stu-id="7f4bf-319">Add the following method to the end of the Form1 class.</span></span>  
 ...
 <span data-ttu-id="7f4bf-336">您現在可以執行應用程式並以 employee 身分登入，您會發現按鈕並未顯示；然後再以 manager 身分登入，則會看到按鈕。</span><span class="sxs-lookup"><span data-stu-id="7f4bf-336">You can now run the application and log in as employee to see that the button does not appear, and then log in as manager to see the button.</span></span>  
...
5.  <span data-ttu-id="7f4bf-352">在 [方案總管] 中，選取 ClientAppServicesDemo 專案中的 Form1，然後從 Visual Studio 主功能表選取 [檢視 &#124; 設計工具]。</span><span class="sxs-lookup"><span data-stu-id="7f4bf-352">In **Solution Explorer**, in the ClientAppServicesDemo project, select Form1 and then select **View &#124; Designer** from the Visual Studio main menu.</span></span>  
...
11. <span data-ttu-id="7f4bf-363">選取 <xref:System.Windows.Forms.Form.FormClosing> 事件，然後按 ENTER 鍵產生事件處理常式。</span><span class="sxs-lookup"><span data-stu-id="7f4bf-363">Select the <xref:System.Windows.Forms.Form.FormClosing> event and then press ENTER to generate an event handler.</span></span>  
...
14. <span data-ttu-id="7f4bf-383">如果是使用 C#，請將下列程式碼加入 `Form1_Load` 方法的結尾。</span><span class="sxs-lookup"><span data-stu-id="7f4bf-383">For C#, add the following code to the end of the `Form1_Load` method.</span></span> <span data-ttu-id="7f4bf-384">這段程式碼會將在上一個步驟中加入的方法與 <xref:System.Web.ClientServices.Providers.ClientSettingsProvider.SettingsSaved> 事件產生關聯。</span><span class="sxs-lookup"><span data-stu-id="7f4bf-384">This code associates the method you added in the last step with the <xref:System.Web.ClientServices.Providers.ClientSettingsProvider.SettingsSaved> event.</span></span>  
 ...
 <span data-ttu-id="7f4bf-385">此時若要測試應用程式，請同時使用 employee 和 manager 身分執行數次，並在文字方塊中輸入不同的值。</span><span class="sxs-lookup"><span data-stu-id="7f4bf-385">To test the application at this point, run it multiple times as both employee and manager, and type different values into the text box.</span></span> <span data-ttu-id="7f4bf-386">這些值會保存在每個使用者的工作階段中。</span><span class="sxs-lookup"><span data-stu-id="7f4bf-386">The values will persist across sessions on a per-user basis.</span></span>  
  
## <a name="implementing-logout"></a><span data-ttu-id="7f4bf-387">實作登出</span><span class="sxs-lookup"><span data-stu-id="7f4bf-387">Implementing Logout</span></span>  
 <span data-ttu-id="7f4bf-388">如果使用者在登入時選取 [記住我]  核取方塊，應用程式在後續執行時會自動驗證使用者。</span><span class="sxs-lookup"><span data-stu-id="7f4bf-388">When the user selects the **Remember me** check box when logging in, the application will automatically authenticate the user on subsequent runs.</span></span> <span data-ttu-id="7f4bf-389">當應用程式在離線模式時，自動驗證仍然會繼續執行直到驗證 Cookie 過期為止。</span><span class="sxs-lookup"><span data-stu-id="7f4bf-389">Automatic authentication will then continue while the application is in offline mode or until the authentication cookie expires.</span></span> <span data-ttu-id="7f4bf-390">不過，有時候多個使用者會需要同時存取應用程式，或是單一使用者可能會偶爾使用不同的認證登入。</span><span class="sxs-lookup"><span data-stu-id="7f4bf-390">Sometimes, however, multiple users will need access to the application or a single user might occasionally log in with different credentials.</span></span> <span data-ttu-id="7f4bf-391">若要實現這類案例，您必須實作登出功能，如同下列程序所述。</span><span class="sxs-lookup"><span data-stu-id="7f4bf-391">To enable this scenario, you must implement logout functionality, as described in the following procedure.</span></span>  
  
#### <a name="to-implement-logout-functionality"></a><span data-ttu-id="7f4bf-392">實作登出功能</span><span class="sxs-lookup"><span data-stu-id="7f4bf-392">To implement logout functionality</span></span>  
  
1.  <span data-ttu-id="7f4bf-393">在 Form1 設計工具中，從 [工具箱] 將 <xref:System.Windows.Forms.Button> 控制項新增表單。</span><span class="sxs-lookup"><span data-stu-id="7f4bf-393">In the Form1 designer, add a <xref:System.Windows.Forms.Button> control to the form from the **ToolBox**.</span></span>  
  
2.  <span data-ttu-id="7f4bf-394">在 [屬性] 視窗中，將 [(名稱)] 值指定為 `logoutButton`，並將 [文字] 值指定為 [登出(&L)]。</span><span class="sxs-lookup"><span data-stu-id="7f4bf-394">In the **Properties** window, specify a **(Name)** value of `logoutButton` and a **Text** value of **&Log Out**.</span></span>  
  
3.  <span data-ttu-id="7f4bf-395">按兩下 `logoutButton`，以產生 <xref:System.Windows.Forms.Control.Click> 事件處理常式。</span><span class="sxs-lookup"><span data-stu-id="7f4bf-395">Double-click the `logoutButton` to generate a <xref:System.Windows.Forms.Control.Click> event handler.</span></span>  
  
     <span data-ttu-id="7f4bf-396">程式碼編輯器隨即出現，並將游標置於 `logoutButton_Click` 方法中。</span><span class="sxs-lookup"><span data-stu-id="7f4bf-396">The code editor appears with the cursor in the `logoutButton_Click` method.</span></span>  
  
4.  <span data-ttu-id="7f4bf-397">以下列程式碼取代所產生的 `logoutButton_Click` 方法。</span><span class="sxs-lookup"><span data-stu-id="7f4bf-397">Replace the generated `logoutButton_Click` method with the following code.</span></span>  
  
     <span data-ttu-id="7f4bf-398">這個事件處理常式會先呼叫在上一節中加入的 `SaveSettings` 方法。</span><span class="sxs-lookup"><span data-stu-id="7f4bf-398">This event handler first calls the `SaveSettings` method that you added in the previous section.</span></span> <span data-ttu-id="7f4bf-399">接著，事件處理常式會呼叫 <xref:System.Web.ClientServices.Providers.ClientFormsAuthenticationMembershipProvider.Logout%2A?displayProperty=nameWithType> 方法。</span><span class="sxs-lookup"><span data-stu-id="7f4bf-399">Then, the event handler calls the <xref:System.Web.ClientServices.Providers.ClientFormsAuthenticationMembershipProvider.Logout%2A?displayProperty=nameWithType> method.</span></span> <span data-ttu-id="7f4bf-400">如果驗證服務無法使用， <xref:System.Web.ClientServices.Providers.ClientFormsAuthenticationMembershipProvider.Logout%2A> 方法會擲回 <xref:System.Net.WebException>。</span><span class="sxs-lookup"><span data-stu-id="7f4bf-400">If the authentication service is unavailable, the <xref:System.Web.ClientServices.Providers.ClientFormsAuthenticationMembershipProvider.Logout%2A> method will throw a <xref:System.Net.WebException>.</span></span> <span data-ttu-id="7f4bf-401">在這種情況下， `logoutButton_Click` 方法會顯示警告訊息，並暫時切換至離線模式將使用者登出。下一節將說明離線模式。</span><span class="sxs-lookup"><span data-stu-id="7f4bf-401">In this case, the `logoutButton_Click` method displays a warning message and switches temporarily to offline mode to log the user out. Offline mode is described in the next section.</span></span>  
  
     <span data-ttu-id="7f4bf-402">登出會刪除本機驗證 Cookie，因此當重新啟動應用程式時就需要登入。</span><span class="sxs-lookup"><span data-stu-id="7f4bf-402">Logout deletes the local authentication cookie so that login will be required when the application is restarted.</span></span> <span data-ttu-id="7f4bf-403">在登出後，事件處理常式會重新啟動應用程式。</span><span class="sxs-lookup"><span data-stu-id="7f4bf-403">After logout, the event handler restarts the application.</span></span> <span data-ttu-id="7f4bf-404">應用程式重新啟動時，會在歡迎訊息後顯示 [登入] 對話方塊。</span><span class="sxs-lookup"><span data-stu-id="7f4bf-404">When the application restarts, it displays the welcome message followed by the login dialog box.</span></span> <span data-ttu-id="7f4bf-405">歡迎訊息清楚表示應用程式已重新啟動。</span><span class="sxs-lookup"><span data-stu-id="7f4bf-405">The welcome message makes it clear that the application has restarted.</span></span> <span data-ttu-id="7f4bf-406">如果使用者必須登入才能儲存設定，而且之後由於應用程式重新啟動又必須再登入一次，歡迎訊息可避免可能發生的混淆情況。</span><span class="sxs-lookup"><span data-stu-id="7f4bf-406">This prevents potential confusion if the user must log in to save settings, and then must log in again because the application has restarted.</span></span>  
 ...
 <span data-ttu-id="7f4bf-407">若要測試登出功能，請執行應用程式，然後在 [登入] 對話方塊中選取 [記住我]  。</span><span class="sxs-lookup"><span data-stu-id="7f4bf-407">To test the logout functionality, run the application and select **Remember me** on the Login dialog box.</span></span> <span data-ttu-id="7f4bf-408">接著，關閉再重新啟動應用程式以確認不再需要登入。</span><span class="sxs-lookup"><span data-stu-id="7f4bf-408">Next, close and restart the application to confirm that you no longer have to log in.</span></span> <span data-ttu-id="7f4bf-409">最後，按一下 [登出] 重新啟動應用程式。</span><span class="sxs-lookup"><span data-stu-id="7f4bf-409">Finally, restart the application by clicking Log out.</span></span>  
  
## <a name="enabling-offline-mode"></a><span data-ttu-id="7f4bf-410">啟用離線模式</span><span class="sxs-lookup"><span data-stu-id="7f4bf-410">Enabling Offline Mode</span></span>  
 <span data-ttu-id="7f4bf-411">在下列程序中，您會將核取方塊加入表單，讓使用者能夠進入離線模式。</span><span class="sxs-lookup"><span data-stu-id="7f4bf-411">In the following procedure, you add a check box to the form to enable the user to enter offline mode.</span></span> <span data-ttu-id="7f4bf-412">您的應用程式可透過將 `static` <xref:System.Web.ClientServices.ConnectivityStatus.IsOffline%2A?displayProperty=nameWithType> 屬性設定為 `true`來進入離線模式。</span><span class="sxs-lookup"><span data-stu-id="7f4bf-412">Your application indicates offline mode by setting the `static` <xref:System.Web.ClientServices.ConnectivityStatus.IsOffline%2A?displayProperty=nameWithType> property to `true`.</span></span> <span data-ttu-id="7f4bf-413">離線狀態會儲存在 <xref:System.Windows.Forms.Application.UserAppDataPath%2A?displayProperty=nameWithType> 屬性所表示的本機硬碟位置。</span><span class="sxs-lookup"><span data-stu-id="7f4bf-413">The offline status is stored on the local hard disk at the location indicated by the <xref:System.Windows.Forms.Application.UserAppDataPath%2A?displayProperty=nameWithType> property.</span></span> <span data-ttu-id="7f4bf-414">這表示離線狀態是以個別使用者和個別應用程式為基礎來儲存。</span><span class="sxs-lookup"><span data-stu-id="7f4bf-414">This means that the offline status is stored on a per-user, per-application basis.</span></span>  
  
 <span data-ttu-id="7f4bf-415">在離線模式中，所有用戶端應用程式服務要求會從本機快取擷取資料，而不是嘗試存取服務。</span><span class="sxs-lookup"><span data-stu-id="7f4bf-415">In offline mode, all client application service requests retrieve data from the local cache instead of trying to access the services.</span></span> <span data-ttu-id="7f4bf-416">在預設組態中，本機資料包含以加密格式儲存的使用者密碼。</span><span class="sxs-lookup"><span data-stu-id="7f4bf-416">In the default configuration, the local data includes an encrypted form of the user's password.</span></span> <span data-ttu-id="7f4bf-417">這可讓使用者在應用程式處於離線模式時仍能登入。</span><span class="sxs-lookup"><span data-stu-id="7f4bf-417">This enables the user to log in while the application is in offline mode.</span></span> <span data-ttu-id="7f4bf-418">如需詳細資訊，請參閱 [How to: Configure Client Application Services](../../../docs/framework/common-client-technologies/how-to-configure-client-application-services.md)。</span><span class="sxs-lookup"><span data-stu-id="7f4bf-418">For more information, see [How to: Configure Client Application Services](../../../docs/framework/common-client-technologies/how-to-configure-client-application-services.md).</span></span>  
  
#### <a name="to-enable-offline-mode-in-your-application"></a><span data-ttu-id="7f4bf-419">在應用程式中啟用離線模式</span><span class="sxs-lookup"><span data-stu-id="7f4bf-419">To enable offline mode in your application</span></span>  
  
1.  <span data-ttu-id="7f4bf-420">在 [方案總管] 中，選取 ClientAppServicesDemo 專案中的 Form1，然後從 Visual Studio 主功能表選取 [檢視 &#124; 設計工具]。</span><span class="sxs-lookup"><span data-stu-id="7f4bf-420">In **Solution Explorer**, in the ClientAppServicesDemo project, select Form1 and then select **View &#124; Designer** from the Visual Studio main menu.</span></span>  
  
2.  <span data-ttu-id="7f4bf-421">在設計工具中，將 <xref:System.Windows.Forms.CheckBox> 控制項加入表單。</span><span class="sxs-lookup"><span data-stu-id="7f4bf-421">In the designer, add a <xref:System.Windows.Forms.CheckBox> control to the form.</span></span>  
  
3.  <span data-ttu-id="7f4bf-422">在 [屬性] 視窗中，將 [(名稱)] 值指定為 `workOfflineCheckBox`，並將 [文字] 值指定為 [離線工作(&W)]。</span><span class="sxs-lookup"><span data-stu-id="7f4bf-422">In the **Properties** window, specify a **(Name)** value of `workOfflineCheckBox` and a **Text** value of **&Work offline**.</span></span>  
  
4.  <span data-ttu-id="7f4bf-423">在 [屬性] 視窗中，按一下 [事件] 按鈕。</span><span class="sxs-lookup"><span data-stu-id="7f4bf-423">In the **Properties** window, click the **Events** button.</span></span>  
  
5.  <span data-ttu-id="7f4bf-424">選取 <xref:System.Windows.Forms.CheckBox.CheckedChanged> 事件，然後按 ENTER 鍵產生事件處理常式。</span><span class="sxs-lookup"><span data-stu-id="7f4bf-424">Select the <xref:System.Windows.Forms.CheckBox.CheckedChanged> event and then press ENTER to generate an event handler.</span></span>  
  
6.  <span data-ttu-id="7f4bf-425">以下列程式碼取代所產生的方法。</span><span class="sxs-lookup"><span data-stu-id="7f4bf-425">Replace the generated method with the following code.</span></span>  
  
     <span data-ttu-id="7f4bf-426">這段程式碼會更新 <xref:System.Web.ClientServices.ConnectivityStatus.IsOffline%2A> 值，然後在返回線上模式時以無訊息模式重新驗證使用者。</span><span class="sxs-lookup"><span data-stu-id="7f4bf-426">This code updates the <xref:System.Web.ClientServices.ConnectivityStatus.IsOffline%2A> value and silently revalidates the user when they return to online mode.</span></span> <span data-ttu-id="7f4bf-427"><xref:System.Web.ClientServices.ClientFormsIdentity.RevalidateUser%2A?displayProperty=nameWithType> 方法會使用快取認證，因此使用者不需要明確地登入。</span><span class="sxs-lookup"><span data-stu-id="7f4bf-427">The <xref:System.Web.ClientServices.ClientFormsIdentity.RevalidateUser%2A?displayProperty=nameWithType> method uses the cached credentials so that the user does not have to explicitly log in.</span></span> <span data-ttu-id="7f4bf-428">如果驗證服務無法使用，就會出現警告訊息，而應用程式仍然保持離線狀態。</span><span class="sxs-lookup"><span data-stu-id="7f4bf-428">If the authentication service is unavailable, a warning message appears and the application stays offline.</span></span>  
  
    > [!NOTE]
    >  <span data-ttu-id="7f4bf-429"><xref:System.Web.ClientServices.ClientFormsIdentity.RevalidateUser%2A> 方法只是為了方便而使用。</span><span class="sxs-lookup"><span data-stu-id="7f4bf-429">The <xref:System.Web.ClientServices.ClientFormsIdentity.RevalidateUser%2A> method is for convenience only.</span></span> <span data-ttu-id="7f4bf-430">因為它沒有傳回值，所以無法表示重新驗證是否失敗。</span><span class="sxs-lookup"><span data-stu-id="7f4bf-430">Because it does not have a return value, it cannot indicate whether revalidation has failed.</span></span> <span data-ttu-id="7f4bf-431">例如，如果在伺服器上的使用者認證已變更，重新驗證就可能會失敗。</span><span class="sxs-lookup"><span data-stu-id="7f4bf-431">Revalidation can fail, for example, if the user credentials have changed on the server.</span></span> <span data-ttu-id="7f4bf-432">在這種情況下，您可能想要加入在服務呼叫失敗之後能夠明確驗證使用者的程式碼。</span><span class="sxs-lookup"><span data-stu-id="7f4bf-432">In this case, you might want to include code that explicitly validates users after a service call fails.</span></span> <span data-ttu-id="7f4bf-433">如需詳細資訊，請參閱本逐步解說稍早的＜存取 Web 設定＞一節。</span><span class="sxs-lookup"><span data-stu-id="7f4bf-433">For more information, see the Accessing Web Settings section earlier in this walkthrough.</span></span>  
  
     <span data-ttu-id="7f4bf-434">重新驗證後，這段程式碼會呼叫先前加入的 `SaveSettings` 方法，以儲存對本機 Web 設定所做的任何變更。</span><span class="sxs-lookup"><span data-stu-id="7f4bf-434">After revalidation, this code saves any changes to the local Web settings by calling the `SaveSettings` method that you added previously.</span></span> <span data-ttu-id="7f4bf-435">然後呼叫專案 `Settings` 類別 (在 C# 中當作 `Properties.Settings.Default` 存取，而在 Visual Basic 中則當作 `My.Settings` 存取) 的 <xref:System.Configuration.ApplicationSettingsBase.Reload%2A> 方法，擷取伺服器上任何新增的值。</span><span class="sxs-lookup"><span data-stu-id="7f4bf-435">It then retrieves any new values on the server by calling the <xref:System.Configuration.ApplicationSettingsBase.Reload%2A> method of the project's `Settings` class (accessed as `Properties.Settings.Default` in C# and `My.Settings` in Visual Basic).</span></span>  
...
7.  <span data-ttu-id="7f4bf-436">將下列程式碼加入 `Form1_Load` 方法的結尾，以確定核取方塊會顯示目前的連接狀態。</span><span class="sxs-lookup"><span data-stu-id="7f4bf-436">Add the following code to the end of the `Form1_Load` method to make sure that the check box displays the current connection state.</span></span>  
 ...
 <span data-ttu-id="7f4bf-437">如此即完成範例應用程式。</span><span class="sxs-lookup"><span data-stu-id="7f4bf-437">This completes the example application.</span></span> <span data-ttu-id="7f4bf-438">若要測試離線功能，請執行應用程式，以 employee 或 manager 的身分登入，然後選取 [離線工作] 。</span><span class="sxs-lookup"><span data-stu-id="7f4bf-438">To test the offline capability, run the application, log in as either employee or manager, and then select **Work offline**.</span></span> <span data-ttu-id="7f4bf-439">修改文字方塊中的值，然後關閉應用程式。</span><span class="sxs-lookup"><span data-stu-id="7f4bf-439">Modify the value in the text box, and then close the application.</span></span> <span data-ttu-id="7f4bf-440">重新啟動應用程式。</span><span class="sxs-lookup"><span data-stu-id="7f4bf-440">Restart the application.</span></span> <span data-ttu-id="7f4bf-441">登入前，以滑鼠右鍵按一下工作列上通知區域中的 ASP.NET 程式開發伺服器圖示，然後按一下 [停止] 。</span><span class="sxs-lookup"><span data-stu-id="7f4bf-441">Before you log in, right-click the ASP.NET Development Server icon in the notification area of the taskbar and then click **Stop**.</span></span> <span data-ttu-id="7f4bf-442">接著以正常的方式登入。</span><span class="sxs-lookup"><span data-stu-id="7f4bf-442">Then, log in like normal.</span></span> <span data-ttu-id="7f4bf-443">即使伺服器不在執行中，您仍然可以登入。</span><span class="sxs-lookup"><span data-stu-id="7f4bf-443">Even when the server is not running, you can still log in.</span></span> <span data-ttu-id="7f4bf-444">修改文字方塊的值、結束，然後再重新啟動，以查看修改過的值。</span><span class="sxs-lookup"><span data-stu-id="7f4bf-444">Modify the text box value, exit, and restart to see the modified value.</span></span>  
  
## <a name="summary"></a><span data-ttu-id="7f4bf-445">總結</span><span class="sxs-lookup"><span data-stu-id="7f4bf-445">Summary</span></span>  
 <span data-ttu-id="7f4bf-446">在本逐步解說中，您已了解如何在 Windows Form 應用程式中啟用及使用用戶端應用程式服務。</span><span class="sxs-lookup"><span data-stu-id="7f4bf-446">In this walkthrough, you learned how to enable and use client application services in a Windows Forms application.</span></span> <span data-ttu-id="7f4bf-447">設定測試伺服器之後，您可以將程式碼加入應用程式以驗證使用者，並從伺服器擷取使用者角色和應用程式設定。</span><span class="sxs-lookup"><span data-stu-id="7f4bf-447">After setting up a test server, you added code to your application to authenticate users and retrieve user roles and application settings from the server.</span></span> <span data-ttu-id="7f4bf-448">您也了解如何啟用離線模式，以便讓應用程式在無法連接時，使用本機資料快取而不是遠端服務。</span><span class="sxs-lookup"><span data-stu-id="7f4bf-448">You also learned how to enable offline mode so that your application uses a local data cache instead of the remote services when connectivity is not available.</span></span>  
  
## <a name="next-steps"></a><span data-ttu-id="7f4bf-449">後續步驟</span><span class="sxs-lookup"><span data-stu-id="7f4bf-449">Next Steps</span></span>  
 <span data-ttu-id="7f4bf-450">在真實世界應用程式中，您會從遠端伺服器存取許多使用者的資料，而遠端伺服器並非隨時都能使用，或是可能會在未告知的情況下離線。</span><span class="sxs-lookup"><span data-stu-id="7f4bf-450">In a real-world application, you will access data for many users from a remote server that may not always be available, or may go down without notice.</span></span> <span data-ttu-id="7f4bf-451">為了讓應用程式更強固，您必須能夠適當地回應服務無法使用的各種情況。</span><span class="sxs-lookup"><span data-stu-id="7f4bf-451">To make your application robust, you must respond appropriately to situations where the service is not available.</span></span> <span data-ttu-id="7f4bf-452">本逐步解說包含 try/catch 區塊，可攔截 <xref:System.Net.WebException> ，並在服務無法使用時顯示錯誤訊息。</span><span class="sxs-lookup"><span data-stu-id="7f4bf-452">This walkthrough includes try/catch blocks to catch a <xref:System.Net.WebException> and display an error message when the service is not available.</span></span> <span data-ttu-id="7f4bf-453">在實際執行的程式碼中，您可能想要藉由切換至離線模式、結束應用程式，或拒絕存取特定功能的方式處理這種情況。</span><span class="sxs-lookup"><span data-stu-id="7f4bf-453">In production code, you might want to handle this case by switching to offline mode, exiting the application, or denying access to specific functionality.</span></span>  
  
 <span data-ttu-id="7f4bf-454">為了加強應用程式的安全性，請務必在部署前徹底測試應用程式和伺服器。</span><span class="sxs-lookup"><span data-stu-id="7f4bf-454">To increase the security of your application, make sure to thoroughly test the application and server before deployment.</span></span>  
  
## <a name="see-also"></a><span data-ttu-id="7f4bf-455">請參閱</span><span class="sxs-lookup"><span data-stu-id="7f4bf-455">See Also</span></span>  
 [<span data-ttu-id="7f4bf-456">用戶端應用程式服務</span><span class="sxs-lookup"><span data-stu-id="7f4bf-456">Client Application Services</span></span>](../../../docs/framework/common-client-technologies/client-application-services.md)  
 [<span data-ttu-id="7f4bf-457">用戶端應用程式服務概觀</span><span class="sxs-lookup"><span data-stu-id="7f4bf-457">Client Application Services Overview</span></span>](../../../docs/framework/common-client-technologies/client-application-services-overview.md)  
 [<span data-ttu-id="7f4bf-458">如何：設定用戶端應用程式服務</span><span class="sxs-lookup"><span data-stu-id="7f4bf-458">How to: Configure Client Application Services</span></span>](../../../docs/framework/common-client-technologies/how-to-configure-client-application-services.md)  
 [<span data-ttu-id="7f4bf-459">ASP.NET 網站管理工具</span><span class="sxs-lookup"><span data-stu-id="7f4bf-459">ASP.NET Web Site Administration Tool</span></span>](https://msdn.microsoft.com/library/100ddd8b-7d11-4df9-91ef-0bbbe92e5aec)  
 [<span data-ttu-id="7f4bf-460">建立及設定 SQL Server 的應用程式服務資料庫</span><span class="sxs-lookup"><span data-stu-id="7f4bf-460">Creating and Configuring the Application Services Database for SQL Server</span></span>](https://msdn.microsoft.com/library/ab894e83-7e2f-4af8-a116-b1bff8f815b2)  
 [<span data-ttu-id="7f4bf-461">逐步解說：使用 ASP.NET 應用程式服務</span><span class="sxs-lookup"><span data-stu-id="7f4bf-461">Walkthrough: Using ASP.NET Application Services</span></span>](https://msdn.microsoft.com/library/f3f394f0-20d6-4361-aa8f-4b21bf4933eb)
