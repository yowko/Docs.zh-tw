---
title: "高可用性、嚴重損壞修復的 SqlClient 支援 | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
...
## 連接 MultiSubnetFailover  
 連接至 [!INCLUDE[ssNoVersion](../../../../../includes/ssnoversion-md.md)] 2012 可用性群組接聽程式或 [!INCLUDE[ssNoVersion](../../../../../includes/ssnoversion-md.md)] 2012 容錯移轉叢集執行個體時，一律指定 `MultiSubnetFailover=True`。  `MultiSubnetFailover` 可加速 [!INCLUDE[ssNoVersion](../../../../../includes/ssnoversion-md.md)] 2012 中所有可用性群組和 \(或\) 容錯移轉叢集執行個體的容錯移轉，且將大幅減少單一及多重子網路 AlwaysOn 拓撲的容錯移轉時間。  在多重子網路容錯移轉時，用戶端將嘗試並行連接。  子網路容錯移轉期間，將積極重試 TCP 連接。  
 ...
 若在連接非可用性群組接聽程式或 [!INCLUDE[ssNoVersion](../../../../../includes/ssnoversion-md.md)]2012 容錯移轉叢集執行個體時指定 `MultiSubnetFailover=True`，可能會對效能產生負面影響，因此不受支援。  
...
## 從資料庫鏡像升級以使用多重子網路叢集  
 如果連接字串中出現 `MultiSubnetFailover` 和 `Failover Partner` 連接關鍵字，或者使用了 `MultiSubnetFailover=True` 和 TCP 以外的通訊協定，就會發生連接錯誤 \(<xref:System.ArgumentException>\)。  如果使用 `MultiSubnetFailover`，且 [!INCLUDE[ssNoVersion](../../../../../includes/ssnoversion-md.md)] 傳回的容錯移轉合作夥伴回應指出該合作夥伴是資料庫鏡像組的一部分，也會發生錯誤 \(<xref:System.Data.SqlClient.SqlException>\)。  
...
## 指定應用程式用途  
 ...
 資料庫可以允許或不允許目標 AlwaysOn 資料庫上的讀取工作負載。  \(此作業會透過 `PRIMARY_ROLE` 和 `SECONDARY_ROLE`[!INCLUDE[tsql](../../../../../includes/tsql-md.md)] 陳述式的 `ALLOW_CONNECTIONS` 子句來完成\)。  
...
## 請參閱  
 [SQL Server 功能及 ADO.NET](../../../../../docs/framework/data/adonet/sql/sql-server-features-and-adonet.md)   
