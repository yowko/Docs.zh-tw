...
> [!NOTE]
>  設定`MultiSubnetFailover`至`true`不需要.NET Framework 4.6.1 或更新版本。
...
## <a name="specifying-application-intent"></a>指定應用程式用途  
 ...
 資料庫可以允許或不允許目標 AlwaysOn 資料庫上的讀取工作負載。 (這藉`ALLOW_CONNECTIONS`子句`PRIMARY_ROLE`和`SECONDARY_ROLE`TRANSACT-SQL 陳述式。)  
