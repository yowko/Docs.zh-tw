...
---
# <a name="how-to-map-inheritance-hierarchies"></a>如何：對應繼承階層架構
若要在 LINQ 中執行繼承對應，您必須在繼承階層架構的根類別上指定屬性和屬性屬性，如下列步驟所述。 使用 Visual Studio 的開發人員可以使用物件關聯式設計工具來對應繼承階層架構。 請參閱[如何：使用 O/R 設計工具設定繼承](/visualstudio/data-tools/how-to-configure-inheritance-by-using-the-o-r-designer)。  
...
5. 僅在其中一個 <xref:System.Data.Linq.Mapping.InheritanceMappingAttribute> 屬性 (Attribute) 中，加入 <xref:System.Data.Linq.Mapping.InheritanceMappingAttribute.IsDefault%2A> 屬性 (Property)。  
     ...
     當來自資料庫資料表的鑒別子值不符合繼承對應中的任何 <xref:System.Data.Linq.Mapping.InheritanceMappingAttribute.Code%2A> 值時，這個屬性會指定一個*fallback*對應。  
...
> [!NOTE]
> 如果您使用 Visual Studio，您可以使用物件關聯式設計工具來設定繼承。 請參閱[如何：使用 O/R 設計工具設定繼承](/visualstudio/data-tools/how-to-configure-inheritance-by-using-the-o-r-designer)  
  
 在下列程式碼範例中，`Vehicle` 會定義為根類別，而先前的步驟已實作為描述 LINQ 的階層架構。  
