...
當您使用 SQLMetal 命令列工具或物件關聯式設計工具 (O/R 設計工具) 時，系統就會自動指定 SQL Server 與 Common Language Runtime (CLR) 之間的型別對應。  
 ...
 執行任何自訂的對應時，這些工具指派預設型別對應中所述[SQL CLR 型別對應](../../../../../../docs/framework/data/adonet/sql/linq/sql-clr-type-mapping.md)。 如果您想要建立與這些預設值不同的型別對應，就必須進行一些型別對應的自訂作業。  
...
## <a name="customization-with-sqlmetal-or-or-designer"></a>使用 SQLMetal 或 O/R 設計工具進行自訂  
 ...
 若要使用 SQLMetal 或 O/R 設計工具來自訂型別對應，請先產生 DBML 檔案。 然後，在產生程式碼檔案或對應檔案之前，修改 DBML 檔案，以便識別所需的型別對應。 您必須透過 SQLMetal 來手動變更 DBML 檔案中的 `Type` 和 `DbType` 屬性，以便進行型別對應自訂作業。 使用 O/R 設計工具時，您可以在此設計工具內部進行變更。 如需有關使用 O/R 設計工具的詳細資訊，請參閱[LINQ to SQL 工具，Visual Studio 中](/visualstudio/data-tools/linq-to-sql-tools-in-visual-studio2)。  
  
> [!NOTE]
>  在資料庫之間來回轉譯時，某些型別對應可能會造成溢位或資料遺失的例外狀況 (Exception)。 請仔細檢閱中的型別對應的執行階段行為對照表[SQL CLR 類型對應](../../../../../../docs/framework/data/adonet/sql/linq/sql-clr-type-mapping.md)進行任何自訂項目之前。  
