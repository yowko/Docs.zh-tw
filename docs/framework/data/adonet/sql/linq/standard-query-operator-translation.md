...
[!INCLUDE[vbtecdlinq](../../../../../../includes/vbtecdlinq-md.md)] 會將標準查詢運算子轉譯為 SQL 命令。 資料庫的查詢處理器會判斷 SQL 轉譯的執行語意。  
  
 標準查詢運算子針對定義*序列*。 序列是*排序*，並依賴序列中的每個元素的參考識別。 如需詳細資訊，請參閱[標準查詢運算子概觀](http://msdn.microsoft.com/library/24cda21e-8af8-4632-b519-c404a839b9b2)。  
  
 SQL 主要負責處理*未排序的值組*。 排序通常是一項明確陳述的後置處理作業，會套用至最終查詢結果 (而非中繼結果)。 識別是以值來定義。 基於這個理由，SQL 查詢可了解處理的多重集 (*包*) 而不是*設定*。  
...
### <a name="take-skip"></a>Take、Skip  
 <xref:System.Linq.Enumerable.Take%2A> 和<xref:System.Linq.Enumerable.Skip%2A>方法定義適用於只針對*ordered set*。 未定義適用於未排序集合或多重集的語意 (Semantics)。  
...
>  <xref:System.Linq.Enumerable.Take%2A> 和 <xref:System.Linq.Enumerable.Skip%2A> 在用於對 SQL Server 2000 進行的查詢中時會有一些限制。 如需詳細資訊，請參閱中的"Skip 和 Take 例外狀況在 SQL Server 2000"項目[疑難排解](../../../../../../docs/framework/data/adonet/sql/linq/troubleshooting.md)。  
 ...
 由於在 SQL 中，排序限制[!INCLUDE[vbtecdlinq](../../../../../../includes/vbtecdlinq-md.md)]嘗試方法的結果，這些方法的引數排序移。 例如，請考量下列 [!INCLUDE[vbtecdlinq](../../../../../../includes/vbtecdlinq-md.md)] 查詢：  
...
### <a name="null-semantics"></a>Null 語意  
 [!INCLUDE[vbtecdlinq](../../../../../../includes/vbtecdlinq-md.md)] 不會對 SQL 加上 null 比較語意。 比較運算子會轉譯為語法上的 SQL 對等用法。 基於這個理由，語意會反映伺服器或連接設定所定義的 SQL 語意。 例如，兩個 null 值會視為不相等，在預設 SQL Server 設定，但您可以變更設定來變更語意。 [!INCLUDE[vbtecdlinq](../../../../../../includes/vbtecdlinq-md.md)] 在轉譯查詢時不會考慮伺服器設定。  
...
### <a name="aggregates"></a>彙總  
 標準查詢運算子彙總方法 <xref:System.Linq.Enumerable.Sum%2A> 會將空序列或只包含 null 的序列評估為零。 在[!INCLUDE[vbtecdlinq](../../../../../../includes/vbtecdlinq-md.md)]，SQL 的語意會保留不變，與<xref:System.Linq.Enumerable.Sum%2A>評估為`null`而不是零，將空序列或只包含 null 的序列。  
...
### <a name="entity-arguments"></a>實體引數  
 [!INCLUDE[vbtecdlinq](../../../../../../includes/vbtecdlinq-md.md)] 可將實體類型中使用<xref:System.Linq.Enumerable.GroupBy%2A>和<xref:System.Linq.Enumerable.OrderBy%2A>方法。 在轉譯這些運算子時，使用型別引數會視為指定該型別的所有成員。 例如，下列程式碼為對等用法：  
...
### <a name="equatable--comparable-arguments"></a>可相等 / 可比較的引數  
 ...
 [!INCLUDE[vbtecdlinq](../../../../../../includes/vbtecdlinq-md.md)] 支援和不等比較*一般*引數，但不適用於形式或包含序列的引數。 扁平引數是一種可以對應至 SQL 資料列的型別。 如果一個或多個實體型別的投影可以透過靜態方式判斷為不含序列，則這個投影即為扁平引數。  
...
### <a name="inheritance-mapping-restrictions"></a>繼承對應限制  
 如需詳細資訊，請參閱[如何： 對應繼承階層架構](../../../../../../docs/framework/data/adonet/sql/linq/how-to-map-inheritance-hierarchies.md)。  
...
-   `Sum`  
 ...
 如需對應至這些 SQL Server 日期和時間類型的詳細資訊，請參閱[SQL CLR 型別對應](../../../../../../docs/framework/data/adonet/sql/linq/sql-clr-type-mapping.md)。  
...
### <a name="text--ntext"></a>text / ntext  
 資料型別`text`  /  `ntext`不能用在針對某些查詢作業`varchar(max)`  /  `nvarchar(max)`，支援哪些由[!INCLUDE[sqprsqext](../../../../../../includes/sqprsqext-md.md)]。  
  
 這項限制沒有解決方案。 具體來說，如果結果中含有對應至 `Distinct()` 或 `text` 資料行的成員，就不能對該結果使用 `ntext`。  
  
### <a name="behavior-triggered-by-nested-queries"></a>巢狀查詢觸發的行為  
 [!INCLUDE[ss2k](../../../../../../includes/ss2k-md.md)] （透過 SP4) 繫結器都有由巢狀查詢觸發的某些特性。 觸發這些特性的 SQL 查詢集就無法妥善定義。 基於這個理由，您不能定義一組[!INCLUDE[vbtecdlinq](../../../../../../includes/vbtecdlinq-md.md)]查詢可能會造成 SQL Server 例外狀況。  
...
-   下列方法<xref:System.Linq.Enumerable.AsEnumerable%2A>方法*在本機執行*。 這個方法不會造成立即執行。  
...
-   下列方法會導致*立即 materialization*方法會套用到序列的：  
