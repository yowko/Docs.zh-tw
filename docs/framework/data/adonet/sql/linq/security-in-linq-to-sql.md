...
## <a name="mapping-and-schema-information"></a>對應和結構描述資訊  
 物件模型或外部對應檔案中的 SQL-CLR 型別對應和資料庫結構描述資訊可供所有在檔案系統中擁有這些檔案之存取權的使用者使用。 假設結構描述資訊就是提供給所有可存取物件模型或外部對應檔案的使用者。 若要避免更普遍的存取結構描述資訊，請使用檔案安全性機制來保護原始程式檔和對應檔。  
...
## <a name="connection-strings"></a>連接字串  
 ...
 這類風險降到最低，請將與 SQL Server 的受信任的連接使用整合式的安全性。 使用這種方法，就不需要將密碼儲存在連接字串中。 如需詳細資訊，請參閱 < [SQL Server 安全性](../../../../../../docs/framework/data/adonet/sql/sql-server-security.md)。  
