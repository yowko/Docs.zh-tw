...
## <a name="supported-systemtimespan-member-support"></a>支援的 System.object 成員支援
...
### <a name="addition-and-subtraction"></a>加法和減法  
 ...
 雖然 CLR <xref:System.TimeSpan?displayProperty=nameWithType> 型別支援加法和減法，但是 SQL `TIME` 型別卻不支援。 因此，如果您的 LINQ to SQL 查詢嘗試在對應至 SQL `TIME` 型別時進行加法和減法，它們將會產生錯誤。 您可以在 [SQL CLR 型別對應](sql-clr-type-mapping.md)中找到使用 sql 日期和時間類型的其他考慮。  
