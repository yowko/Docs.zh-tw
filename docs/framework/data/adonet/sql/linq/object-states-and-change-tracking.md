...
|`PossiblyModified`|*附加*至的<xref:System.Data.Linq.DataContext>物件。 如需詳細資訊，請參閱多[層式應用程式中的資料抓取和 CUD 作業（LINQ to SQL）](data-retrieval-and-cud-operations-in-n-tier-applications.md)。|
...
針對繼承階層架構中的類別<xref:System.Data.Linq.Table%601.InsertOnSubmit%2A>，`o`（）也會設定指定為*鑒別*子的成員值，以符合物件`o`的類型。 當型別符合預設鑑別子值時，這個動作會導致鑑別子值遭預設值覆寫。 如需詳細資訊，請參閱[繼承支援](inheritance-support.md)。
