...
不論對物件進行多少的變更，都只會變更記憶體中的複本。 並不會變更到資料庫中的實際資料。 在 <xref:System.Data.Linq.DataContext.SubmitChanges%2A> 上明確呼叫 <xref:System.Data.Linq.DataContext> 之前，變更都不會傳輸至伺服器。  
 ...
 進行這個呼叫時，<xref:System.Data.Linq.DataContext> 會嘗試將變更轉譯為對等的 SQL 命令。 您可以使用自己的自訂邏輯來覆寫這些動作，但提交的順序是由<xref:System.Data.Linq.DataContext>稱為「*變更處理器*」的服務來協調。 事件順序如下：  
...
- [如何：偵測並解決衝突的提交](how-to-detect-and-resolve-conflicting-submissions.md)
