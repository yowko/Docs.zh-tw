...
## <a name="example"></a>範例  
 在下列 [!INCLUDE[vbtecdlinq](../../../../../../includes/vbtecdlinq-md.md)] 查詢中，您可以看到所產生之使用者定義函數方法的內嵌呼叫 `ReverseCustName`。 因為延後查詢執行，所以不會立即執行函式。 針對這個查詢建置的 SQL，會轉譯為對資料庫中使用者定義函式的呼叫 (請參閱查詢後面的 SQL 程式碼)。  
