...
ms.locfileid: "54666515"
---
# <a name="data-binding"></a>資料繫結
[!INCLUDE[vbtecdlinq](../../../../../../includes/vbtecdlinq-md.md)] 繫結至通用控制項，例如方格控制項的支援。 具體來說，[!INCLUDE[vbtecdlinq](../../../../../../includes/vbtecdlinq-md.md)]定義繫結至資料格，以及處理主從式繫結，同時進行顯示和更新的基本模式。  
...
## <a name="operation"></a>運算  
 實作 <xref:System.ComponentModel.IListSource>，就可以隱含地繫結至 Windows Forms 控制項。 資料來源泛型<xref:System.Data.Linq.Table%601>(`Table<T>`在C#或`Table(Of T)`Visual Basic 中) 和泛型`DataQuery`已更新為實作<xref:System.ComponentModel.IListSource>。 使用者介面 (UI) 資料繫結引擎 (Windows Form 和 Windows Presentation Foundation) 兩者都會測試其資料來源是否實作 <xref:System.ComponentModel.IListSource>。 因此，撰寫查詢直接對資料來源控制項的隱含呼叫[!INCLUDE[vbtecdlinq](../../../../../../includes/vbtecdlinq-md.md)]集合產生，如下列範例所示：  
...
-   資料來源<xref:System.Data.Linq.Table%601>:[!INCLUDE[vbtecdlinq](../../../../../../includes/vbtecdlinq-md.md)]瀏覽資料表以填入`DataBindingList`保留在資料表的 參考的集合。  
...
-   <xref:System.ComponentModel.IBindingList.SortDirection%2A> 和 <xref:System.ComponentModel.IBindingList.SortProperty%2A> 屬性會公開 (Expose) 目前儲存在本機成員中的排序定義。  
 ...
 如果您使用 System.Windows.Forms.BindingSource 並設定 BindingSource.DataMember 屬性並將 BindingSource.DataSource 設定為具有一個名為 bindingsource.datamember 之會公開 EntitySet 屬性的類別，\<TEntity >，您不必呼叫 EntitySet\<Tentity >。GetNewBindingList 來更新 BindingSource.List，但您會遺失排序功能。  
  
## <a name="caching"></a>快取  
 [!INCLUDE[vbtecdlinq](../../../../../../includes/vbtecdlinq-md.md)] 查詢實作<xref:System.ComponentModel.IListSource.GetList%2A>。 Windows Forms BindingSource 類別符合這個介面時，會針對單一連接呼叫三次 GetList()。 若要解決此情況下，[!INCLUDE[vbtecdlinq](../../../../../../includes/vbtecdlinq-md.md)]實作每個執行個體存放區，並一律傳回相同的產生集合的快取。  
...
-   對應至主索引鍵的類別成員會有 setter，但是[!INCLUDE[vbtecdlinq](../../../../../../includes/vbtecdlinq-md.md)]不支援物件身分識別變更。 因此，無法更新資料庫中用於對應的主/唯一索引鍵。 在方格中的變更會導致例外狀況，當您呼叫<xref:System.Data.Linq.DataContext.SubmitChanges%2A>。  
