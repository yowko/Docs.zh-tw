...
ms.locfileid: "75634712"
---
# <a name="data-binding"></a>資料繫結

[!INCLUDE[vbtecdlinq](../../../../../../includes/vbtecdlinq-md.md)] 支援與通用控制項（例如方格控制項）的系結。 具體而言，[!INCLUDE[vbtecdlinq](../../../../../../includes/vbtecdlinq-md.md)] 會定義系結至資料方格和處理主版詳細資料系結的基本模式，這兩者都與顯示和更新有關。

## <a name="underlying-principle"></a>基礎準則

[!INCLUDE[vbtecdlinq](../../../../../../includes/vbtecdlinq-md.md)] 會將 LINQ 查詢轉譯為 SQL，以便在資料庫上執行。 而產生的結果會是強型別 (Strongly Typed) `IEnumerable`。 因為這些物件是一般的 common language runtime （CLR）物件，所以可以使用一般物件資料系結來顯示結果。 另一方面，變更作業 (插入、更新和刪除) 則需要額外的步驟。

## <a name="operation"></a>運算

實作 <xref:System.ComponentModel.IListSource>，就可以隱含地繫結至 Windows Forms 控制項。 資料來源一般 <xref:System.Data.Linq.Table%601> （在 Visual Basic C#中的`Table<T>` 或 `Table(Of T)` 中），而泛型 `DataQuery` 已更新為可執行 <xref:System.ComponentModel.IListSource>。 使用者介面 (UI) 資料繫結引擎 (Windows Form 和 Windows Presentation Foundation) 兩者都會測試其資料來源是否實作 <xref:System.ComponentModel.IListSource>。 因此，將查詢的直接對寫入控制項的資料來源，會隱含地呼叫 [!INCLUDE[vbtecdlinq](../../../../../../includes/vbtecdlinq-md.md)] 集合產生，如下列範例所示：
...
[!INCLUDE[vbtecdlinq](../../../../../../includes/vbtecdlinq-md.md)] 會在兩個位置中執行 <xref:System.ComponentModel.IListSource>：

- 資料來源是一種 <xref:System.Data.Linq.Table%601>： [!INCLUDE[vbtecdlinq](../../../../../../includes/vbtecdlinq-md.md)] 流覽資料表以填入保留資料表參考的 `DataBindingList` 集合。

- 資料來源是 <xref:System.Linq.IQueryable%601>。 兩個案例如下：

  - 如果 [!INCLUDE[vbtecdlinq](../../../../../../includes/vbtecdlinq-md.md)] 從 <xref:System.Linq.IQueryable%601>找到基礎 <xref:System.Data.Linq.Table%601>，則來源會允許版本，而這種情況與第一個專案符號點相同。

  - 如果 [!INCLUDE[vbtecdlinq](../../../../../../includes/vbtecdlinq-md.md)] 找不到基礎 <xref:System.Data.Linq.Table%601>，則來源不允許版本（例如，`groupby`）。 [!INCLUDE[vbtecdlinq](../../../../../../includes/vbtecdlinq-md.md)] 流覽查詢以填入泛型 `SortableBindingList`，這是一個簡單的 <xref:System.ComponentModel.BindingList%601>，它會針對指定屬性的 T 實體執行排序功能。
...
當您使用 TEntity > 和系結 EntitySet 時，您必須呼叫 EntitySet\<TEntity >，才會將它系結\<。Entityset<tentity>.getnewbindinglist 以更新 BindingSource. List。

如果您使用 System.object 並設定 BindingSource 屬性，並將 BindingSource. DataSource 設定為在\<TEntity > 公開 EntitySet 的 BindingSource 中具有名為的屬性，您就不需要呼叫 EntitySet\<TEntity >。Entityset<tentity>.getnewbindinglist 以更新 BindingSource. List，但您不會失去排序功能。

## <a name="caching"></a>快取

[!INCLUDE[vbtecdlinq](../../../../../../includes/vbtecdlinq-md.md)] 查詢會執行 <xref:System.ComponentModel.IListSource.GetList%2A>。 Windows Forms BindingSource 類別符合這個介面時，會針對單一連接呼叫三次 GetList()。 為了解決這種情況，[!INCLUDE[vbtecdlinq](../../../../../../includes/vbtecdlinq-md.md)] 會針對每個實例執行快取，以儲存且一律傳回相同的產生集合。
