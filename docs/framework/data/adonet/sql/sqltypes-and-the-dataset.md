...
ADO.NET 2.0 透過 `DataSet` 命名空間 (Namespace) 導入了 <xref:System.Data.SqlTypes> 的增強型別支援。 <xref:System.Data.SqlTypes> 中型別的設計目的是要提供與 SQL Server 資料庫中的資料型別具有相同語意 (Semantics) 及精確度的資料型別。 <xref:System.Data.SqlTypes> 中的每個資料型別，在 SQL Server 中都具有對應的資料型別，並且具有相同的基礎資料表示。  
 ...
 使用 SQL Server 資料型別時，若在 <xref:System.Data.SqlTypes> 中直接使用 <xref:System.Data.DataSet> 會有一些優點。 <xref:System.Data.SqlTypes> 支援 SQL Server 原生資料類型相同的語意。 在 <xref:System.Data.SqlTypes> 的定義中指定其中一個 <xref:System.Data.DataColumn> 可避免將 decimal 或 numeric 資料型別轉換成其中一種 Common Language Runtime (CLR) 資料型別時可能會發生的精確度遺失。  
