...
3. 此外，使用 ASP.NET 2.0 或更新版本建置的 Web 應用程式可以使用 `SqlCacheDependency` Helper 類別 (Class)。  
 ...
 如果您正在開發的應用程式在資料變更時需要可靠的次要秒數通知，請參閱**規劃有效率的查詢通知策略**和[規劃中](https://go.microsoft.com/fwlink/?LinkId=211984)的**查詢通知的替代**章節 SQL Server 線上叢書中的通知主題。 如需查詢通知和 SQL Server Service Broker 的詳細資訊，請參閱下列《SQL Server 線上叢書》主題的連結。  
