...
### <a name="comparing-guid-values"></a>比較 GUID 值  
 比較運算子可以與 `uniqueidentifier` 值搭配使用。 不過，比較這兩個值的位元模式並不會實作排序作業。 唯一允許的作業`uniqueidentifier`值是一種比較 (=、 <>， \<，>， \<=、 > =) 以及檢查 NULL （IS NULL 和 IS NOT NULL）。 而不允許其他算術運算子。  
