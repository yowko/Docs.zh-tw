...
- 針對資料進行廣泛處理，並不需要與資料來源間有開放連接，如此便可將連接釋放給其他用戶端使用。  
 ...
 如果您不需要 `DataSet` 所提供的功能，則可採用 `DataReader` 以順向、唯讀的方式來傳回資料，藉以提升應用程式的效能。 雖然 `DataAdapter` 使用 `DataReader` 來填滿 `DataSet` 的內容（請參閱[從 DataAdapter 填入資料集](populating-a-dataset-from-a-dataadapter.md)），您可以使用 `DataReader`來提升效能，因為您會儲存 `DataSet`所使用的記憶體，並避免建立和填滿 `DataSet`內容所需的處理。  
