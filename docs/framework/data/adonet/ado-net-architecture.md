...
### <a name="the-dataset"></a>DataSet  
 ...
 下圖說明 .NET Framework Data Provider 和之間的關聯性 `DataSet` 。  
  
 ![ADO.Net 圖形](./media/ado-1-bpuedev11.png "ado_1_bpuedev11")  
ADO.NET 架構  
  
### <a name="choosing-a-datareader-or-a-dataset"></a>選擇 DataReader 或 DataSet  
 當您決定應用程式是否應該使用 `DataReader` （請參閱[使用 DataReader 抓取資料](retrieving-data-using-a-datareader.md)）或 `DataSet` （請參閱[資料集、datatable 和 dataview](./dataset-datatable-dataview/index.md)）時，請考慮您的應用程式所需的功能類型。 請使用 `DataSet` 來進行下列作業：  
...
- 針對資料進行廣泛處理，並不需要與資料來源間有開放連接，如此便可將連接釋放給其他用戶端使用。  
 ...
 如果您不需要 `DataSet` 所提供的功能，則可採用 `DataReader` 以順向、唯讀的方式來傳回資料，藉以提升應用程式的效能。 雖然 `DataAdapter` `DataReader` 會使用來填滿的內容 `DataSet` （請參閱[從 DataAdapter 填入資料集](populating-a-dataset-from-a-dataadapter.md)），但您可以藉由使用來 `DataReader` 提升效能，因為您會儲存所使用的記憶體 `DataSet` ，並避免建立和填滿內容所需的處理 `DataSet` 。  
...
## <a name="wcf-data-services"></a>WCF 資料服務  
 WCF Data Services 是用來在 Web 或內部網路上部署資料服務。 系統會根據實體資料模型的規格，讓資料結構化成實體與關聯性。 部署在這個模型上的資料可由標準 HTTP 通訊協定定址。 如需詳細資訊，請參閱 [WCF Data Services 4.5](../wcf/index.md)。  
  
## <a name="xml-and-adonet"></a>XML 和 ADO.NET  
 ADO.NET 利用 XML 的強大功能來提供資料的中斷存取。 ADO.NET 的設計與 .NET Framework 中的 XML 類別搭配使用;兩者都是單一架構的元件。  
  
 ADO.NET 和 .NET Framework 中的 XML 類別會在物件中會合 `DataSet` 。 `DataSet` 可以填入 XML 來源的資料，無論它是檔案或 XML 資料流都一樣。 不管 `DataSet` 的資料來源為何，`DataSet` 都可組合成與全球資訊網協會 (W3C) 相容的 XML，而且包含其結構描述當做 XML 結構描述定義語言 (XSD) 結構描述。 由於 `DataSet` 的原生序列化格式是 XML，所以相當適合當做在各層之間移動資料的媒體，如此當需要在遠端對 XML Web Service 來回傳送資料和結構描述內容時，`DataSet` 將會是最佳選擇。 如需詳細資訊，請參閱 [XML 文件和資料](../../../standard/data/xml/index.md)。  
