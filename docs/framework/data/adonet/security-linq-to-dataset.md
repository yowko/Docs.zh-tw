...
## <a name="external-input"></a>外部輸入  
 應用程式通常採用外部輸入 (從使用者或其他外部代理程式)，並根據該輸入執行動作。  如果是[!INCLUDE[linq_dataset](../../../../includes/linq-dataset-md.md)]，應用程式可能會建構以特定方式，根據外部輸入或使用外部查詢中輸入查詢。 [!INCLUDE[linq_dataset](../../../../includes/linq-dataset-md.md)] 查詢任何位置接受參數接受的常值。 應用程式開發人員應該使用參數化的查詢，而非將常值從外部代理程式直接插入到查詢中。  
