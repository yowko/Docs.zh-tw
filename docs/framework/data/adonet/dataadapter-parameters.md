...
> [!NOTE]
> 針對的`Fill` `DataAdapter` 作業`Get`和的方法，會從.NETFrameworkDataProvider傳回的類型推斷.NETFramework類型。`DataReader` Microsoft SQL Server、OLE DB 和 ODBC 資料類型的推斷 .NET Framework 類型和存取子方法，會在[ADO.NET 的資料類型](data-type-mappings-in-ado-net.md)對應中加以說明。  
...
> [!NOTE]
> 如果沒有為參數提供參數名稱，則會將參數*N*的累加預設名稱指定為*參數，其開頭為*"Parameter1"。 當您提供參數名稱時，建議您避免使用參數*N*命名慣例，因為您所提供的名稱可能會與中`ParameterCollection`現有的預設參數名稱相衝突。 如果提供的名稱已經存在，便會發生例外狀況。  
