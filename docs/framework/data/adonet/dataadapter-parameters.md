...
> 參數預留位置的語法會隨資料來源而有所不同。 此範例將說明 SQL Server 資料來源的保留字元。 若為 <xref:System.Data.OleDb> 和 <xref:System.Data.Odbc> 參數，請使用問號 (?) 保留字元。  
 ...
 在此 Visual Basic 範例中, `CompanyName`會使用`CustomerID`等於`@CustomerID`參數值之資料列`@CompanyName`的參數值來更新欄位。 參數會使用<xref:System.Data.SqlClient.SqlParameter.SourceColumn%2A> <xref:System.Data.SqlClient.SqlParameter>物件的屬性, 從修改過的資料列中取出資訊。 下列是前述範例 UPDATE 陳述式的參數。 程式碼會假設變數 `adapter` 表示有效的 <xref:System.Data.SqlClient.SqlDataAdapter> 物件。  
...
> [!NOTE]
> 針對的`Fill` `DataAdapter` 作業`Get`和的方法,會從.NETFrameworkDataProvider傳回的類型推斷.NETFramework類型。`DataReader` Microsoft SQL Server、OLE DB 和 ODBC 資料類型的推斷 .NET Framework 類型和存取子方法, 會在[ADO.NET 的資料類型](../../../../docs/framework/data/adonet/data-type-mappings-in-ado-net.md)對應中加以說明。  
...
|`Proposed`|這個會參數使用建議值。|  
 ...
 下一區段中的 `SqlClient` 程式碼範例會定義 <xref:System.Data.Common.DbDataAdapter.UpdateCommand%2A> 的參數，其中 `CustomerID` 資料行將做為兩個參數的 `SourceColumn` 使用：`@CustomerID` (`SET CustomerID = @CustomerID`) 和 `@OldCustomerID` (`WHERE CustomerID = @OldCustomerID`)。 參數是用來將**CustomerID**資料行更新為中的`DataRow`目前值。 `@CustomerID` 因此, `CustomerID` `SourceVersion`會使用`Current`具有之的。 `SourceColumn` `@OldCustomerID`參數是用來識別資料來源中的目前資料列。 因為在資料列的 `Original` 版本中找到相符的資料行值，所以會使用 `SourceColumn` 為 `CustomerID` 的同一個 `SourceVersion` (`Original`)。  
...
> [!NOTE]
> 如果沒有為參數提供參數名稱, 則會將參數*N*的累加預設名稱指定為參數, 其開頭為 "Parameter1"。 當您提供參數名稱時, 建議您避免使用參數*N*命名慣例, 因為您所提供的名稱可能會與中`ParameterCollection`現有的預設參數名稱相衝突。 如果提供的名稱已經存在，便會發生例外狀況。  
