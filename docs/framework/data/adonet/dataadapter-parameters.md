...
<xref:System.Data.Common.DbDataAdapter> 具有四個屬性，可用來擷取資料來源的資料，以及將資料更新至資料來源：<xref:System.Data.Common.DbDataAdapter.SelectCommand%2A> 屬性可傳回資料來源的資料，而 <xref:System.Data.Common.DbDataAdapter.InsertCommand%2A>、<xref:System.Data.Common.DbDataAdapter.UpdateCommand%2A> 和 <xref:System.Data.Common.DbDataAdapter.DeleteCommand%2A> 屬性可用來管理在資料來源的變更。 在您呼叫 `SelectCommand` 的 `Fill` 方法前，必須先設定 `DataAdapter` 屬性。 您必須先設定 `InsertCommand`、`UpdateCommand` 或 `DeleteCommand` 屬性，然後再呼叫 `Update` 的 `DataAdapter` 方法，端視針對 <xref:System.Data.DataTable> 中的資料進行哪些變更而定。 例如，如果已經加入資料列，則必須先設定 `InsertCommand`，才能呼叫 `Update`。 `Update` 正在處理已插入、已更新或已刪除的資料列時，`DataAdapter` 會使用個別的 `Command` 屬性來處理這項動作。 已修改資料列的目前資訊會透過 `Command` 集合傳遞給 `Parameters` 物件。  
 ...
 在資料來源更新行時，調用 UPDATE 語句，該語句使用唯一識別碼標識要更新的表中的行。 唯一的識別項一般是主索引鍵欄位的值。 UPDATE 陳述式所使用的參數包含唯一的識別項，以及要更新的資料行和值，如下列 Transact-SQL 陳述式所示。  
...
> 參數預留位置的語法會隨資料來源而有所不同。 此範例將說明 SQL Server 資料來源的保留字元。 若為 <xref:System.Data.OleDb> 和 <xref:System.Data.Odbc> 參數，請使用問號 (?) 保留字元。  
 ...
 在此 Visual Basic 示例中`CompanyName`，該欄位使用`@CompanyName``CustomerID`等於`@CustomerID`參數值的行的參數值進行更新。 參數使用<xref:System.Data.SqlClient.SqlParameter.SourceColumn%2A><xref:System.Data.SqlClient.SqlParameter>物件的屬性從修改後的行中檢索資訊。 下列是前述範例 UPDATE 陳述式的參數。 程式碼會假設變數 `adapter` 表示有效的 <xref:System.Data.SqlClient.SqlDataAdapter> 物件。  
...
> [!NOTE]
> 對於 的操作`Fill``DataAdapter`和`Get``DataReader`方法，從 .NET Framework 資料提供程式返回的類型推斷 .NET Framework 類型。 推斷的 .NET 框架類型和訪問器方法用於 Microsoft SQL 伺服器、OLE DB 和 ODBC 資料類型，ADO.NET[中的資料類型映射](data-type-mappings-in-ado-net.md)仲介紹了這些類型。  
...
|`Proposed`|這個會參數使用建議值。|  
 ...
 下一區段中的 `SqlClient` 程式碼範例會定義 <xref:System.Data.Common.DbDataAdapter.UpdateCommand%2A> 的參數，其中 `CustomerID` 資料行將做為兩個參數的 `SourceColumn` 使用：`@CustomerID` (`SET CustomerID = @CustomerID`) 和 `@OldCustomerID` (`WHERE CustomerID = @OldCustomerID`)。 該`@CustomerID`參數用於將**CustomerID**列更新為 中的`DataRow`當前值。 因此，`CustomerID``SourceColumn``SourceVersion`使用`Current` 該`@OldCustomerID`參數用於標識資料來源中的當前行。 因為在資料列的 `Original` 版本中找到相符的資料行值，所以會使用 `SourceColumn` 為 `CustomerID` 的同一個 `SourceVersion` (`Original`)。  
...
> [!NOTE]
> 如果未為參數提供參數名稱，則為參數指定一個增量預設名稱參數*N，**,* 以"參數 1"開頭。 我們建議您在提供參數名稱時避免參數*N*命名約定，因為提供的名稱可能與 中的`ParameterCollection`現有預設參數名稱衝突。 如果提供的名稱已經存在，便會發生例外狀況。  
