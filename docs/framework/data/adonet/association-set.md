...
## <a name="example"></a>範例  
 ...
 下列範例顯示以前述概念模型為基礎的一個關聯集 (`PublishedBy`) 和兩個實體集 (`Books` 和 `Publishers`)。 `Books` 實體集中的 Bi 表示在執行時間 `Book` 實體類型的實例。 同樣地，Pj 代表 `Publishers` 實體集中的 `Publisher` 實例。 BiPj 代表 `PublishedBy` 關聯集中 `PublishedBy` 關聯的實例。  
  
 ![顯示集合範例的螢幕擷取畫面。](./media/association-set/sets-example-association.gif)  
  
 [ADO.NET Entity Framework](./ef/index.md)會使用稱為概念結構定義語言（[CSDL](/ef/ef6/modeling/designer/advanced/edmx/csdl-spec)）的特定領域語言（DSL）來定義概念模型。 下列 CSDL 定義上圖所示之實體實體容器，每個關聯具有一個關聯集。 請注意，每個關聯集的名稱和關聯都是使用 XML 屬性定義的。  
