...
|<xref:System.Data.DataRowVersion.Default>|指定資料列的預設資料列版本。 `Added`、`Modified` 或 `Unchanged` 資料列的預設資料列版本為 `Current`。 `Deleted` 資料列的預設資料列版本為 `Original`。 `Detached` 資料列的預設資料列版本為 `Proposed`。|  
