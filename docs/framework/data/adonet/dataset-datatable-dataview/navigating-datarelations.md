...
<xref:System.Data.DataRelation> 的其中一個主要功能，是讓您可以從 <xref:System.Data.DataTable> 內的某個 <xref:System.Data.DataSet> 巡覽至另一個。 這可讓您在指定相關**DataTable**的單一**DataRow**時，從一個**datatable**中取出所有相關的 <xref:System.Data.DataRow> 物件。 例如，在客戶資料表與訂單資料表之間建立**DataRelation**之後，您可以使用**GetChildRows**來抓取特定客戶資料列的所有訂單資料列。  
 ...
 請注意，為**Customers**和**Orders**資料表建立**DataRelation**時，不會為**createConstraints**旗標指定任何值（預設為**true**）。 這會假設**Orders**資料表中的所有資料列都具有父系**Customers**資料表中所存在的**CustomerID**值。 如果「**訂單**」資料表中不存在「 **Customers** 」資料表中的**CustomerID** ，則 <xref:System.Data.ForeignKeyConstraint> 會導致擲回例外狀況。  
