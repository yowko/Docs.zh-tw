...
## <a name="case-sensitivity"></a>區分大小寫  
 ...
 <xref:System.Data.DataSet> 中可能有兩個或兩個以上具有相同名稱，但不同大小寫的資料表或關聯。 在這種情況下，按照資料表和關聯名稱進行參考時是區分大小寫的。 例如，如果 <xref:System.Data.DataSet> **資料集** 包含資料表 **Table1** 和 **table1**，您會依名稱將 **table1** 參考為 **資料集。資料表 ["Table1"]** 和 **table1** 做為 **資料集。資料表 ["table1"]**。 嘗試參考其中一個資料表做為 **資料集。資料表 ["TABLE1"]** 會產生例外狀況。  
