...
## <a name="case-sensitivity"></a>區分大小寫  
 <xref:System.Data.DataSet> 中可能有兩個或兩個以上具有相同名稱，但不同大小寫的資料表或關聯。 在這種情況下，按照資料表和關聯名稱進行參考時是區分大小寫的。 例如，如果 <xref:System.Data.DataSet> **資料集**包含資料表**Table1**和**table1**，您會依名稱參考**Table1**做為**資料集。資料表 ["Table1"]** 和**table1**做為**資料集。資料表 ["table1"]**。 嘗試將其中一個資料表當做**資料集來參考。資料表 ["TABLE1"]** 將會產生例外狀況。  
