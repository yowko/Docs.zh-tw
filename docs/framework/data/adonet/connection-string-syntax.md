...
### <a name="sql-server-authentication-with-sqlclient"></a>使用 SqlClient SQL Server 驗證   
...
當您連接到 Azure SQL Database 或 Azure SQL 資料倉儲並以 `user@servername`格式提供登入時，請確定登入中的 `servername` 值符合針對 `Server=`提供的值。
...
### <a name="type-system-version-changes"></a>型別系統版本變更  
 <xref:System.Data.SqlClient.SqlConnection.ConnectionString%2A?displayProperty=nameWithType> 中的 `Type System Version` 關鍵字會指定 SQL Server 類型的用戶端標記法。 如需 <xref:System.Data.SqlClient.SqlConnection.ConnectionString%2A?displayProperty=nameWithType> 關鍵字的詳細資訊，請參閱 `Type System Version`。  
...
## <a name="using-trustservercertificate"></a>使用 TrustServerCertificate  
 只有在使用有效憑證連接到 SQL Server 實例時，`TrustServerCertificate` 關鍵字才有效。 當 `TrustServerCertificate` 設定為 `true` 時，傳輸層 (Transport Layer) 會使用 SSL 來加密通道，而略過逐一檢查憑證鏈結以驗證信任的作業。  
...
### <a name="enabling-encryption"></a>啟用加密  
 ...
 應用程式設定無法降低 SQL Server 中設定的安全性層級，但可以選擇性地進行加強。 應用程式可以藉由將 `TrustServerCertificate` 和 `Encrypt` 關鍵字設定為 `true`來要求加密，以確保即使伺服器憑證尚未布建，以及尚未設定**強制通訊協定加密**，還是會進行加密。適用于用戶端。 不過，如果用戶端組態中未啟用 `TrustServerCertificate`，則仍需要提供伺服器憑證。  
