...
### <a name="type-system-version-changes"></a>型別系統版本變更  
 中的`Type System Version`關鍵字會指定SQLServer類型的用戶端標記法。<xref:System.Data.SqlClient.SqlConnection.ConnectionString%2A?displayProperty=nameWithType> 如需 <xref:System.Data.SqlClient.SqlConnection.ConnectionString%2A?displayProperty=nameWithType> 關鍵字的詳細資訊，請參閱 `Type System Version`。  
