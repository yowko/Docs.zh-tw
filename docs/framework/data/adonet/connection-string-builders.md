...
在舊版的 ADO.NET 中，不會發生具有串連字號串值的連接字串編譯時間檢查，因此在執行時間，不正確的關鍵字會產生 <xref:System.ArgumentException> 。 每個 .NET Framework 資料提供者都支援不同的連接字串關鍵字語法，讓您在手動完成時，難以建立有效的連接字串。 為了解決這個問題，ADO.NET 2.0 為每個 .NET Framework 資料提供者引進了新的連接字串產生器。 每個資料提供者都具有繼承自 <xref:System.Data.Common.DbConnectionStringBuilder> 強型別連接字串產生器類別。 下表列出 .NET Framework 資料提供者及其關聯的連接字串產生器類別。  
