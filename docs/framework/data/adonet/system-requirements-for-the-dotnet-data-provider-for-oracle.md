...
```  
 ...
 以 UTF-16 模式初始化 Oracle 用戶端程式庫。 這會導致 Oracle 用戶端程式庫接受 UTF-16 (與 UCS-2 非常類似)，而不接受多位元組的字串。 這可讓 Oracle 的資料提供者一律使用 Oracle 字碼頁，而無需進行其他轉譯工作。 僅在您使用 Oracle 9i 用戶端與具有 AL16UTF16 替代字元集的 Oracle 9i 資料庫通訊時，此組態才會運作。 當 Oracle 9i 用戶端會與 Oracle 9i 伺服器通訊時，需要額外的資源將 Unicode 轉換**CommandText**為適當的多位元組字元的值集的 Oracle9i 伺服器所使用。 如果您知道將 `Unicode=True` 加入連接字串會得到安全的組態，便可避免此問題。  
  
### <a name="mixing-versions-of-oracle-client-and-oracle-server"></a>Oracle 用戶端及 Oracle 伺服器的混合版本  
 Oracle 8i 用戶端無法存取**NCHAR**， **NVARCHAR2**，或**NCLOB**時伺服器的國家字元集指定為 AL16UTF16 Oracle 9i 資料庫中的資料 (預設值 Oracle 9i）。 因為 Oracle 9i 之前的版本未引入對 UTF-16 字元集的支援，所以 Oracle 8i 用戶端無法讀取它。  
...
### <a name="oraclelob-can-only-update-full-characters"></a>OracleLob 僅能更新完整的字元。  
 基於可用性理由<xref:System.Data.OracleClient.OracleLob>物件會繼承自.NET Framework Stream 類別，並提供**ReadByte**並**WriteByte**方法。 它也會實作方法，例如**CopyTo**並**清除**之區段的 Oracle **LOB**物件。 相反地，Oracle 用戶端軟體會提供數個 Api，以使用字元**LOB**s (**CLOB**並**NCLOB**)。 但是，這些 API 僅適用於完整字元。 由於此差別，Oracle 的資料提供者會實作支援**讀取**並**ReadByte**以位元組方式使用 utf-16 資料。 不過，其他的方法**OracleLob**物件僅允許完整字元作業。  
