...
通用結構描述集合是由每個 .NET Framework Managed 提供者所實作的結構描述集合。 您可以查詢 .NET Framework 託管提供程式，通過調用沒有參數的**GetSchema**方法或使用架構集合名稱"MetaDataCollection"來確定支援的架構集合的清單。 這會傳回 <xref:System.Data.DataTable>，包括支援的結構描述集合清單、每個集合所支援的限制數目，以及集合所使用之識別項部分的數目。 這些集合會描述所有必要資料行。 如果願意，提供者可以隨意加入其他資料行。 例如，`SqlClient` 及 `OracleClient` 會將 ParameterName 加入限制集合。  
  
 如果提供者無法決定必要資料行的值，則會傳回 Null。  
  
 有關使用**獲取架構**方法的詳細資訊，請參閱[獲取架構和架構集合](getschema-and-schema-collections.md)。  
...
|----------------|--------------|-----------------|  
|CollectionName|字串|要傳遞給**GetSchema**方法以返回集合的集合的名稱。|  
|限制數|int|可以為集合指定的限制數目。|  
...
|ColumnName|DataType|描述|  
|----------------|--------------|-----------------|  
|CompositeIdentifierSeparatorPattern|字串|符合複合識別項中複合分隔符號的規則運算式。 例如，"\\. （對於 SQL 伺服器）\@或\\"&#124;。 (適用於 Oracle)。<br /><br /> 複合識別碼通常是用於資料庫物件名稱的內容，例如：pubs.dbo.作者或 pubs\@dbo.作者。<br /><br /> 對於 SQL 伺服器，請使用正則\\運算式"." 。 對於 Oracle 用戶端，\@請使用\\"&#124;"。<br /><br /> 若為 ODBC，請使用 Catalog_name_seperator。<br /><br /> 若為 OLE DB，請使用 DBLITERAL_CATALOG_SEPARATOR 或 DBLITERAL_SCHEMA_SEPARATOR。|  
...
|DataSourceProductVersionNormalized|字串|資料來源的正規化版本，可使用 `String.Compare()` 進行比較。 它的格式對提供者的所有版本來說都一致，這可以防止版本 10 在版本 1 與版本 2 之間排序。<br /><br /> For example, the Oracle provider uses a format of "nn.nn.nn.nn.nn" for its normalized version, which causes an Oracle 8i data source to return "08.01.07.04.01". SQL 伺服器使用典型的 Microsoft"nn.nn.nnnn"格式。<br /><br /> 在某些情況下，DataSourceProductVersion 及 DataSourceProductVersionNormalized 將為同一值。 若為 OLE DB 及 ODBC，這些值將永遠相同，因為它們對應於基礎原生 API 中的同一函式呼叫。|  
...
|ParameterMarkerFormat|字串|表示如何格式化參數的格式字串。<br /><br /> 如果資料來源支援具名參數，則此字串中的第一個替代符號位置應為要格式化參數名稱的位置。<br /><br /> 例如，如果資料來源希望用"："命名和預綴參數，則參數為"："。{0} 使用參數名稱 "p1" 對進行格式化時，產生的字串為 ":p1"。<br /><br /> 如果資料來源\@希望參數預綴于''，但名稱已包含它們，則這將是''，{0}並且格式化名為"p1"\@的參數的結果將只是"p1"。\@<br /><br /> 對於不希望具名引數並期望使用"？"字元的資料來源，格式字串可以指定為簡單的"？"，這將忽略參數名稱。 對於 OLE DB，我們返回"？"|  
|ParameterMarkerPattern|字串|符合參數標記的規則運算式。 它會具有參數名稱的相符值 (如果有的話)。<br /><br /> 例如，如果具名引數支援參數名稱中的'\@引引字元，則為："（[A-Za-z0-9]）"。\@<br /><br /> 但是，如果具名引數支援為"："作為輸入字元，並且它不是參數名稱的一部分，則這將是\*：":([A-Za-z0-9]"。<br /><br /> 當然，如果資料來源不支援具名引數，則這只是"？"|  
...
|QuotedIdentifierPattern|字串|符合引號識別項並具有識別項本身 (不帶有引號) 之相符值的規則運算式。 例如，如果資料來源使用雙引號來標識引用的識別碼，則這將是\\\\："（[&#124;""）"。"\\|  
...
|StringLiteralPattern|字串|符合字串常值並具有常值本身之相符值的規則運算式。 例如，如果資料來源使用單引號來標識字串，則為："（[&#124;'"）""|  
...
|ProviderDbType|int|指定參數類型時應使用的特定于提供程式的類型值。 例如，SqlDbType.Money 或 OracleType.Blob。|  
...
|CreateFormat|字串|表示如何將此資料行加入資料定義陳述式的格式字串，如 CREATE TABLE。 CreateParameter 陣列中的每個項目在格式字串都應使用「參數標記」來表示。<br /><br /> 例如，SQL 資料型別 DECIMAL 需要精確度及小數位數。 在這種情況下，格式字串將為"DECIMAL（{0}，）"。{1}|  
|CreateParameters|字串|建立此資料型別的資料行時，必須指定的建立參數。 每個建立參數會以逗號分隔，並按提供時的順序列在字串中。<br /><br /> 例如，SQL 資料型別 DECIMAL 需要精確度及小數位數。 在此情況下，建立參數應包含字串 "precision, scale"。<br /><br /> 在創建精度為 10 和比例為 2 的文本命令中，"CreateFormat"列的值可能是{0}{1}DECIMAL（），"並且完整的類型規範將為 DECIMAL（10，2）。|  
...
|是受文字支援的|bool|True 表示資料型別可表示為常值。<br /><br /> False 表示資料型別不可表示為常值。|  
