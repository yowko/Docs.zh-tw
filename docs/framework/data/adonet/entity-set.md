...
## <a name="example"></a>範例  
 ...
 下圖顯示以前述概念模型為基礎的兩個實體集 (`Books` 和 `Publishers`)，以及一個關聯集 `PublishedBy`)。 `Books` 實體集中的 Bi 表示在執行時間 `Book` 實體類型的實例。 同樣地，Pj 代表 `Publishers` 實體集中的 `Publisher` 實例。 BiPj 代表 `PublishedBy` 關聯集中 `PublishedBy` 關聯的實例。  
  
 ![顯示集合範例的螢幕擷取畫面。](./media/entity-set/sets-example-association.gif)  
  
 [ADO.NET Entity Framework](./ef/index.md)會使用稱為概念結構定義語言（[CSDL](/ef/ef6/modeling/designer/advanced/edmx/csdl-spec)）的特定領域語言（DSL）來定義概念模型。 下列 CSDL 定義實體容器，上述概念模型中的每個實體類型皆具有一個實體集。 請注意，每個實體集名稱和實體類型都是使用 XML 屬性定義的。  
