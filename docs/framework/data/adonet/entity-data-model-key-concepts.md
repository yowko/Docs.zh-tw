 ...
 *概念模型*是某些資料結構的特定標記法，做為實體和關聯性。 表示概念模型的方法之一就是使用圖表。 下圖代表包含三種實體類型 (`Book`、`Publisher` 和 `Author`) 及兩種關聯 (`PublishedBy` 和 `WrittenBy`) 的概念模型；  
 ...
 不過，若要使用這種表示方法傳達某些與模型相關的詳細資料，則會有些缺點。 例如，圖表中不會顯示屬性和實體集資訊。 以特定領域語言 (DSL) 傳達的概念模型豐富度會更清楚。 [ADO.NET Entity Framework](./ef/index.md)使用名為*概念結構定義語言*（[CSDL](/ef/ef6/modeling/designer/advanced/edmx/csdl-spec)）的 XML 型 DSL 來定義概念模型。 以下是上圖所示之概念模型的 CSDL 定義：  
