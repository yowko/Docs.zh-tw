...
|----------------|--------------|-----------------|  
|CONSTRAINT_CATALOG|字串|條件約束所屬的目錄。|  
|CONSTRAINT_SCHEMA|字串|包含條件約束的結構描述。|  
|CONSTRAINT_NAME|字串|名稱。|  
|TABLE_CATALOG|字串|包含條件約束的資料表名稱。|  
|TABLE_SCHEMA|字串|包含資料表的結構描述。|  
|TABLE_NAME|字串|資料表名稱|  
|CONSTRAINT_TYPE|字串|條件約束的型別。 僅允許 FOREIGN KEY。|  
|IS_DEFERRABLE|字串|指定條件約束是否可以延遲。 傳回 NO。|  
|INITIALLY_DEFERRED|字串|指定條件約束一開始是否可以延遲。 傳回 NO。|  
...
|ColumnName|DataType|說明|  
|----------------|--------------|-----------------|  
|SPECIFIC_CATALOG|字串|目錄的特定名稱。|  
|SPECIFIC_SCHEMA|字串|結構描述的特定名稱。|  
|SPECIFIC_NAME|字串|目錄的特定名稱。|  
|U|字串|預存程序所屬的目錄。|  
|ROUTINE_SCHEMA|字串|包含預存程序的結構描述。|  
|ROUTINE_NAME|字串|預存程序的名稱。|  
|ROUTINE_TYPE|字串|針對預存程序傳回 PROCEDURE，並針對函式傳回 FUNCTION。|  
...
|ColumnName|DataType|說明|  
|----------------|--------------|-----------------|  
|SPECIFIC_CATALOG|字串|此為其參數之程序的目錄名稱。|  
|SPECIFIC_SCHEMA|字串|包含此參數所屬程序的結構描述。|  
|SPECIFIC_NAME|字串|此參數所屬程序的名稱。|  
|ORDINAL_POSITION|Int32|以 1 開始的參數序數位置。 若為程序的傳回值，則此項為 0。|  
|PARAMETER_MODE|字串|若為輸入參數，傳回 IN；若為輸出參數，傳回 OUT；若為輸入/輸出參數，傳回 INOUT。|  
|IS_RESULT|字串|若表示做為函式之程序的結果，傳回 YES。 否則，傳回 NO。|  
|AS_LOCATOR|字串|若宣告為定位器，傳回 YES。 否則，傳回 NO。|  
|PARAMETER_NAME|字串|參數名稱。 若對應至函式的傳回值，則為 NULL。|  
|DATA_TYPE|字串|系統提供的資料型別。|  
...
|COLLATION_CATALOG|字串|參數集合的目錄名稱。 若不是其中一個字元型別，則傳回 NULL。|  
|COLLATION_SCHEMA|字串|永遠傳回 NULL。|  
|COLLATION_NAME|字串|參數集合的名稱。 若不是其中一個字元型別，則傳回 NULL。|  
|CHARACTER_SET_CATALOG|字串|參數字元集的目錄名稱。 若不是其中一個字元型別，則傳回 NULL。|  
|CHARACTER_SET_SCHEMA|字串|永遠傳回 NULL。|  
|CHARACTER_SET_NAME|字串|參數字元集的名稱。 若不是其中一個字元型別，則傳回 NULL。|  
...
|INTERVAL_TYPE|字串|NULL。 保留供 SQL Server 日後使用。|  
...
|ColumnName|DataType|說明|  
|----------------|--------------|-----------------|  
|TABLE_CATALOG|字串|資料表的目錄。|  
|TABLE_SCHEMA|字串|包含資料表的結構描述。|  
|TABLE_NAME|字串|資料表名稱。|  
|TABLE_TYPE|字串|資料表型別。 可為 VIEW 或 BASE TABLE。|  
...
|COLUMN_DEFAULT|字串|資料行的預設值。|  
|IS_NULLABLE|字串|資料行的 Null 屬性。 若此資料行允許 NULL，其會傳回 YES。 否則，傳回 NO。|  
|DATA_TYPE|字串|系統提供的資料型別。|  
...
|CHARACTER_SET_CATALOG|字串|若資料行為字元資料或文字資料型別，則傳回 master，表示字元集所在的資料庫。 否則，傳回 NULL。|  
|CHARACTER_SET_SCHEMA|字串|永遠傳回 NULL。|  
|CHARACTER_SET_NAME|字串|若此資料行為字元資料或文字資料型別，則傳回字元集的唯一名稱。 否則，傳回 NULL。|  
|COLLATION_CATALOG|字串|若資料行為字元資料或文字資料型別，則傳回 master，表示定義定序的資料庫。 否則，此資料行為 UNLL。|  
...
|ORDINAL_POSITION|Int32|資料行識別碼。|  
|COLUMN_DEFAULT|字串|資料行的預設值。|  
|IS_NULLABLE|字串|資料行的 Null 屬性。 若此資料行允許 NULL，其會傳回 YES。 否則，會傳回 NO。|  
|DATA_TYPE|字串|系統提供的資料型別。|  
...
|user_name|字串|使用者名稱或群組名稱，在此資料庫中是唯一的。|  
...
|ColumnName|DataType|說明|  
|----------------|--------------|-----------------|  
|TABLE_CATALOG|字串|檢視表的目錄。|  
|TABLE_SCHEMA|字串|包含檢視表的結構描述。|  
|TABLE_NAME|字串|檢視表名稱。|  
|CHECK_OPTION|字串|WITH CHECK OPTION 的型別。 若原始檢視表是使用 WITH CHECK OPTION 所建立的，則為 CASCADE。 否則，傳回 NONE。|  
|IS_UPDATABLE|字串|指定檢視表是否可以更新。 永遠傳回 NO。|  
  
## <a name="viewcolumns"></a>ViewColumns  
  
|ColumnName|DataType|說明|  
|----------------|--------------|-----------------|  
|VIEW_CATALOG|字串|檢視表的目錄。|  
|VIEW_SCHEMA|字串|包含檢視表的結構描述。|  
|VIEW_NAME|字串|檢視表名稱。|  
|TABLE_CATALOG|字串|與此檢視表相關之資料表的目錄。|  
|TABLE_SCHEMA|字串|包含與此檢視表相關之資料表的結構描述。|  
|TABLE_NAME|字串|與檢視表相關之資料表的名稱。 基底資料表。|  
|COLUMN_NAME|String|資料行名稱。|  
