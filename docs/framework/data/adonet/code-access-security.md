...
> [!NOTE]
> CLR 中執行的程式碼不能授與其本身的使用權限。 例如，程式碼可以要求而且被授與少於安全性原則允許的權限，但是它絕不會被授與更多權限。 授與權限時，系統是以完全沒有權限開始，然後加入執行特定工作的最少權限。 如果一開始便使用所有權限，然後再拒絕個別的權限，則會造成應用程式不安全，因為可能會授與超出必要的權限而導致意外安全性漏洞。 有關詳細資訊,請參閱[設定安全原則](https://docs.microsoft.com/previous-versions/dotnet/netframework-4.0/7c9c2y1w(v=vs.100))[和安全原則管理](https://docs.microsoft.com/previous-versions/dotnet/netframework-4.0/c1k0eed6(v=vs.100))。  
...
### <a name="requesting-permissions"></a>要求權限  
 ...
 在應用程式僅需要從磁碟中讀取資料的情況下，您可以要求絕不授與任何寫入權限給應用程式。 在錯誤或惡意攻擊的事件中，您的程式碼無法破壞它所運作的資料。 有關詳細資訊,請參閱[要求權限](https://docs.microsoft.com/previous-versions/dotnet/netframework-4.0/yd267cce(v=vs.100))。  
...
## <a name="assemblies"></a>組件  
 組件會構成 .NET Framework 應用程式之部署、版本控制、重複使用、啟動範圍和安全性權限的基本單位。 組件會提供針對一起運作而建立而且構成邏輯功能單位之類型和資源的組合。 對 CLR 而言，類型不會存在組件內容外部。 有關建立及部署程式集的詳細資訊,請參閱[使用程式集進行編程](../../../standard/assembly/index.md)。  
...
|`ConnectionString`|識別允許的連接字串。 可識別多個連接字串。 **註:** 請勿在連接字串中包含使用者 ID 或密碼。 在這個發行版本中，您無法使用 .NET Framework 組態工具變更連接字串限制。 <br /><br /> 繼承自 <xref:System.Data.Common.DBDataPermissionAttribute>。|  
|`KeyRestrictions`|識別是否為允許的連接字串參數。 連接字串參數在表單*\<參數名稱>#* 中識別。 也可以指定多個參數，只要以分號 (;) 將其分隔即可。 **註:** 如果不指定`KeyRestrictions`,但將 屬性`KeyRestrictionBehavior``AllowOnly`設置`PreventUsage`為 或 ,則不允許其他連接字串參數。 繼承自 <xref:System.Data.Common.DBDataPermissionAttribute>。|  
...
#### <a name="connectionstring-syntax"></a>ConnectionString 語法  
 下列範例將示範如何使用組態檔的 `connectionStrings` 項目，只允許使用特定的連接字串。 有關從設定檔儲存與檢索連線字串的詳細資訊,請參閱[連接字串](connection-strings.md)。  
...
#### <a name="keyrestrictions-syntax"></a>KeyRestrictions 語法  
 下面的範例啟用相同的連接字串,啟用`Encrypt`和`Packet Size`連接字串選項,但限制使用任何其他連接字串選項。  
...
### <a name="enabling-partial-trust-with-a-custom-permission-set"></a>以自訂的使用權限集啟用部份信任  
 若要啟用特定區域的 <xref:System.Data.SqlClient> 使用權限，系統管理員必須建立自訂的使用權限集合，並將其設定為特定區域的使用權限集合。 不可修改預設的使用權限集合 (例如 `LocalIntranet`)。 例如,<xref:System.Data.SqlClient>要包含<xref:System.Security.Policy.Zone>`LocalIntranet`具有的代碼的許可權,系統管理員可以將`LocalIntranet`許可權集 重新命名為「自定義本地內聯網<xref:System.Data.SqlClient>」,添加許可權,使用[Caspol.exe(代碼存取安全原則工具)](../../tools/caspol-exe-code-access-security-policy-tool.md)匯入自訂本地內聯網`LocalIntranet_Zone`許可權集,並將的許可權集設置為自訂本地內聯網。  
  
### <a name="sample-permission-set"></a>使用權限集合範例  
 下列是部分受信任案例中的「SQL Server 的 .NET Framework 資料提供者」使用權限集合範例。 有關建立自訂權限的資訊,請參閱[使用 Caspol.exe 設定權限集](https://docs.microsoft.com/previous-versions/dotnet/netframework-4.0/4ybs46y6(v=vs.100))。  
...
## <a name="verifying-adonet-code-access-using-security-permissions"></a>使用安全性使用權限驗證 ADO.NET 程式碼存取  
 若為部分信任案例，則可指定 <xref:System.Data.SqlClient.SqlClientPermissionAttribute>，藉以取得程式碼中之特定方法的 CAS 權限。 如果生效的限制安全性原則不允許該權限，則在執行程式碼前會擲回例外狀況。 有關安全原則的詳細資訊,請參閱[安全原則管理和](https://docs.microsoft.com/previous-versions/dotnet/netframework-4.0/c1k0eed6(v=vs.100))[安全策略最佳實踐](https://docs.microsoft.com/previous-versions/dotnet/netframework-4.0/sa4se9bc(v=vs.100))。  
