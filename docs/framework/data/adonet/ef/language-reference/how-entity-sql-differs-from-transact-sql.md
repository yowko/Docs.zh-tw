...
本主題描述之間的差異[!INCLUDE[esql](../../../../../../includes/esql-md.md)]和 Transact SQL。  
...
## <a name="support-for-expressions"></a>運算式的支援  
 -具有子查詢 （資料表） 和運算式 （資料列和資料行）。  
  
 為了支援集合和巢狀的集合，[!INCLUDE[esql](../../../../../../includes/esql-md.md)]將所有東西變成運算式。 [!INCLUDE[esql](../../../../../../includes/esql-md.md)] 比 TRANSACT-SQL 更容易撰寫-每個運算式可用於任何地方。 查詢運算式一定會產生投影的型別集合，而且可在允許集合運算式的任何地方使用。 針對不支援的 TRANSACT-SQL 運算式的詳細資訊[!INCLUDE[esql](../../../../../../includes/esql-md.md)]，請參閱 <<c2> [ 不支援的運算式](../../../../../../docs/framework/data/adonet/ef/language-reference/unsupported-expressions-entity-sql.md)。  
...
## <a name="uniform-treatment-of-subqueries"></a>子查詢的統一處理  
 指定在資料表中的強調，TRANSACT-SQL 執行子查詢的內容解譯。 例如，在子查詢`from`子句數字會被視為多重集 （資料表）。 但是 `select` 子句中使用的相同子查詢會視為純量子查詢。 同樣地，使用左邊的子查詢`in`運算子會被視為純量子查詢，而右側必須是多重集子的查詢。  
...
### <a name="avoiding-implicit-coercions-for-subqueries"></a>避免子查詢的隱含強制型轉  
 統一的子查詢處理有一個相關的副作用，就是會隱含地將子查詢轉換成純量值。 具體來說，在 TRANSACT-SQL 中 （具有單一欄位） 的資料列的多重集是隱含地轉換成純量值資料型別是欄位。  
...
## <a name="select-value-avoiding-the-implicit-row-wrapper"></a>選取值：避免隱含資料列包裝函式  
 TRANSACT-SQL 子查詢中的 select 子句會隱含地建立子句中的項目周圍的資料列包裝函式。 這意味我們無法建立純量或物件的集合。 Transact SQL 可讓具有一個欄位的資料列型別與相同的資料類型的單一值之間的隱含強制型轉。  
...
## <a name="left-correlation-and-aliasing"></a>左邊相互關聯與別名  
 在 TRANSACT-SQL，在給定範圍中的運算式 (例如單一子句`select`或`from`) 不能參考相同的範圍中稍早定義的運算式。 某些 dialect （包括 Transact SQL） 的 SQL 的支援有限的形式的這些功能`from`子句。  
  
 [!INCLUDE[esql](../../../../../../includes/esql-md.md)] 在左邊相互關聯通用化`from`子句，並將它們視為一致的方式。 `from` 子句中的運算式可參考相同子句中的先前定義 (左邊的定義)，而不需要其他語法。  
  
 [!INCLUDE[esql](../../../../../../includes/esql-md.md)] 也會針對與 `group by` 子句有關的查詢做出其他限制。 中的運算式`select`子句和`having`這類查詢子句只能參考`group by`透過其別名的索引鍵。 下列建構是在 Transact SQL，但不是在有效[!INCLUDE[esql](../../../../../../includes/esql-md.md)]:  
...
## <a name="referencing-columns-properties-of-tables-collections"></a>參考資料表 (集合) 的資料行 (屬性)  
 [!INCLUDE[esql](../../../../../../includes/esql-md.md)] 中的所有資料行參考都必須以資料表別名來限定。 下列建構 (假設`a`是有效的資料行的資料表`T`) 是有效的在 TRANSACT-SQL 中使用，但是[!INCLUDE[esql](../../../../../../includes/esql-md.md)]。  
...
## <a name="navigation-through-objects"></a>導覽物件  
 使用 transact SQL"。"標記法來參考資料行 （資料列） 的資料表。 [!INCLUDE[esql](../../../../../../includes/esql-md.md)] 會擴充這個標記法 (從程式語言借用) 來支援物件之屬性的導覽。  
...
## <a name="no-support-for-"></a>* 表示不支援  
 TRANSACT-SQL 支援使用資格不符 * 語法當做整個資料列中，和完整的別名\*語法 (t.\*) 的捷徑，該資料表的欄位。 颾魤 ㄛ TRANSACT-SQL 允許特殊 count (\*) 彙總，其中包含 null。  
  
 [!INCLUDE[esql](../../../../../../includes/esql-md.md)] 不支援 * 建構。 表單的 transact SQL 查詢`select * from T`並`select T1.* from T1, T2...`可以用來表示[!INCLUDE[esql](../../../../../../includes/esql-md.md)]作為`select value t from T as t`和`select value t1 from T1 as t1, T2 as t2...`分別。 此外，這些建構會處理繼承 (值的可替代性)，而 `select *` Variant 則限制為宣告之型別的最上層屬性。  
...
## <a name="order-by-clause-usage"></a>ORDER BY 子句使用方式  
 TRANSACT-SQL 允許 ORDER BY 子句中指定只在最上層的選取... 子句 。 在 [!INCLUDE[esql](../../../../../../includes/esql-md.md)] 中，您可以使用巢狀 ORDER BY 運算式，並將它放在查詢內的任何地方，但是巢狀查詢中的排序並不會保留下來。  
...
## <a name="transact-sql-functionality-not-available-in-entity-sql"></a>Entity SQL 中無法使用 Transact-SQL 功能  
 ...
 命令式程式設計  
 [!INCLUDE[esql](../../../../../../includes/esql-md.md)] 提供命令式程式設計中，不同於 TRANSACT-SQL 中不支援。 請改用程式語言。  
 ...
 內建函式，運算子  
 [!INCLUDE[esql](../../../../../../includes/esql-md.md)] 支援的 TRANSACT-SQL 的子集內建函數和運算子。 主要存放區提供者可能會支援這些運算子和函式。 [!INCLUDE[esql](../../../../../../includes/esql-md.md)] 使用提供者資訊清單中宣告的存放區特有函式。 此外，[!INCLUDE[adonet_ef](../../../../../../includes/adonet-ef-md.md)]可讓您宣告內建和使用者定義的現有存放區函式，如[!INCLUDE[esql](../../../../../../includes/esql-md.md)]使用。  
  
 提示  
 [!INCLUDE[esql](../../../../../../includes/esql-md.md)] 不提供查詢提示的機制。  
  
 批次處理查詢結果  
 [!INCLUDE[esql](../../../../../../includes/esql-md.md)] 不支援批次處理查詢結果。 例如，以下是有效 TRANSACT-SQL （傳送做為批次）：  
