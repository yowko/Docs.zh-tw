...
-   `in` 和`exists`子查詢已通用化，可允許任何集合。  
     ...
     子查詢是一種集合。 `e1 in e2` 並`exists(e)`是[!INCLUDE[esql](../../../../../../includes/esql-md.md)]建構來執行這些作業。  
...
## <a name="support-for-expressions"></a>運算式的支援  
 ...
 為了支援集合和巢狀的集合，[!INCLUDE[esql](../../../../../../includes/esql-md.md)]將所有東西變成運算式。 [!INCLUDE[esql](../../../../../../includes/esql-md.md)] 更容易撰寫比[!INCLUDE[tsql](../../../../../../includes/tsql-md.md)]— 每個運算式可用於任何地方。 查詢運算式一定會產生投影的型別集合，而且可在允許集合運算式的任何地方使用。 如需[!INCLUDE[tsql](../../../../../../includes/tsql-md.md)]中不支援的運算式[!INCLUDE[esql](../../../../../../includes/esql-md.md)]，請參閱[不支援的運算式](../../../../../../docs/framework/data/adonet/ef/language-reference/unsupported-expressions-entity-sql.md)。  
...
## <a name="uniform-treatment-of-subqueries"></a>子查詢的統一處理  
 ...
 [!INCLUDE[esql](../../../../../../includes/esql-md.md)] 消除這些差異。 運算式的統一解譯不依賴使用此運算式的內容。 [!INCLUDE[esql](../../../../../../includes/esql-md.md)] 會考量所有的子查詢視為多重集子的查詢。 如果子查詢，從想要的純量值[!INCLUDE[esql](../../../../../../includes/esql-md.md)]提供`anyelement`作業集合 （在此情況下，子查詢），而且從集合中擷取單一值的運算子。  
...
### <a name="avoiding-implicit-coercions-for-subqueries"></a>避免子查詢的隱含強制型轉  
 ...
 [!INCLUDE[esql](../../../../../../includes/esql-md.md)] 不支援此隱含強制型轉。 [!INCLUDE[esql](../../../../../../includes/esql-md.md)] 提供要從集合中擷取單一值的 ANYELEMENT 運算子和`select value`子句，以避免在查詢運算式期間建立的資料列包裝函式。  
...
## <a name="select-value-avoiding-the-implicit-row-wrapper"></a>選取值：避免隱含資料列包裝函式  
 ...
 [!INCLUDE[esql](../../../../../../includes/esql-md.md)] 也會提供資料列建構函式來建構任意的資料列。 `select` 會擷取投影中的一或多個項目並且產生具有欄位的資料記錄，如下所示：  
...
## <a name="left-correlation-and-aliasing"></a>左邊相互關聯與別名  
 ...
 [!INCLUDE[esql](../../../../../../includes/esql-md.md)] 也會包含查詢的其他限制`group by`子句。 中的運算式`select`子句和`having`這類查詢子句只能參考`group by`透過其別名的索引鍵。 下列建構中是有效[!INCLUDE[tsql](../../../../../../includes/tsql-md.md)]但不是處於[!INCLUDE[esql](../../../../../../includes/esql-md.md)]:  
...
```  
 ...
 資料表別名在 `from` 子句中為選擇性。 資料表名稱會當做隱含別名使用。 [!INCLUDE[esql](../../../../../../includes/esql-md.md)] 可讓下列格式：  
...
## <a name="navigation-through-objects"></a>導覽物件  
 [!INCLUDE[tsql](../../../../../../includes/tsql-md.md)] 使用"。"標記法來參考資料行 （資料列） 的資料表。 [!INCLUDE[esql](../../../../../../includes/esql-md.md)] 會擴充這個標記法 （從程式語言借） 來支援導覽屬性的物件。  
...
## <a name="no-support-for-"></a>* 表示不支援  
 ...
 [!INCLUDE[esql](../../../../../../includes/esql-md.md)] 不支援 * 建構。 [!INCLUDE[tsql](../../../../../../includes/tsql-md.md)] 形式的查詢`select * from T`並`select T1.* from T1, T2...`可以用來表示[!INCLUDE[esql](../../../../../../includes/esql-md.md)]作為`select value t from T as t`和`select value t1 from T1 as t1, T2 as t2...`分別。 此外，這些建構會處理繼承 (值的可替代性)，而 `select *` Variant 則限制為宣告之型別的最上層屬性。  
  
 [!INCLUDE[esql](../../../../../../includes/esql-md.md)] 不支援`count(*)`彙總。 請改用 `count(0)`。  
  
## <a name="changes-to-group-by"></a>變更成 Group By  
 [!INCLUDE[esql](../../../../../../includes/esql-md.md)] 支援的別名`group by`索引鍵。 中的運算式`select`子句和`having`子句必須參考`group by`透過這些別名的索引鍵。 例如，這個 [!INCLUDE[esql](../../../../../../includes/esql-md.md)] 語法：  
...
## <a name="collection-based-aggregates"></a>以集合為基礎的彙總  
 [!INCLUDE[esql](../../../../../../includes/esql-md.md)] 支援兩種類型的彙總。  
...
## <a name="order-by-clause-usage"></a>ORDER BY 子句使用方式  
 [!INCLUDE[tsql](../../../../../../includes/tsql-md.md)] 可讓 ORDER BY 子句中指定只在最上層的選取... 子句 。 在 [!INCLUDE[esql](../../../../../../includes/esql-md.md)] 中，您可以使用巢狀 ORDER BY 運算式，並將它放在查詢內的任何地方，但是巢狀查詢中的排序並不會保留下來。  
...
## <a name="identifiers"></a>識別項  
 在 [!INCLUDE[tsql](../../../../../../includes/tsql-md.md)] 中，識別項比較是以目前資料庫的定序 (Collation) 為基礎。 在 [!INCLUDE[esql](../../../../../../includes/esql-md.md)] 中，識別項一律不會區分大小寫，但是會區分腔調字 (Accent Sensitive) (亦即，[!INCLUDE[esql](../../../../../../includes/esql-md.md)] 會區別有腔調和無腔調字元。例如，'a' 不等於 'ấ')。 [!INCLUDE[esql](../../../../../../includes/esql-md.md)] 字母版本視為看起來一樣卻來自不同字碼頁不同的字元。 如需詳細資訊，請參閱 <<c0> [ 輸入字元集](../../../../../../docs/framework/data/adonet/ef/language-reference/input-character-set-entity-sql.md)。  
...
## <a name="transact-sql-functionality-not-available-in-entity-sql"></a>Entity SQL 中無法使用 Transact-SQL 功能  
 ...
 DDL  
 [!INCLUDE[esql](../../../../../../includes/esql-md.md)] 提供不支援目前的版本中的 DDL。  
  
 命令式程式設計  
 [!INCLUDE[esql](../../../../../../includes/esql-md.md)] 不提供支援命令式程式設計中，不同於[!INCLUDE[tsql](../../../../../../includes/tsql-md.md)]。 請改用程式語言。  
  
 群組函式  
 [!INCLUDE[esql](../../../../../../includes/esql-md.md)] 尚未提供支援群組 （例如 CUBE、 ROLLUP 和 GROUPING_SET） 的函式。  
  
 分析函式  
 [!INCLUDE[esql](../../../../../../includes/esql-md.md)] 並不會分析函式 （還） 提供支援。  
...
```  
 ...
 [!INCLUDE[esql](../../../../../../includes/esql-md.md)] 只支援一個產生結果查詢陳述式每個命令。  
