...
## <a name="example-4"></a>範例 4  
 下列範例會先編譯然後再叫用接受<xref:System.String>輸入參數的查詢，然後傳回其`Contact`電子郵件地址開頭為指定字串的：  
