...
## <a name="example"></a>範例  
 下面的示例編譯並調用接受<xref:System.String>輸入參數的查詢，然後返回`Contact`其電子郵件地址以指定的字串開頭的查詢：  
