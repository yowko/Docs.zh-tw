...
## <a name="simple-identifiers"></a>簡單識別項  
 [!INCLUDE[esql](../../../../../../includes/esql-md.md)] 中的簡單識別碼是一系列的英數位元和底線字元。 識別項的第一個字元必須是字母字元 (a-z 或 A-Z)。  
...
## <a name="aliasing-rules"></a>別名規則  
 建議您在需要時于 [!INCLUDE[esql](../../../../../../includes/esql-md.md)] 查詢中指定別名，包括下列 [!INCLUDE[esql](../../../../../../includes/esql-md.md)] 結構：  
...
### <a name="valid-aliases"></a>有效的別名  
 [!INCLUDE[esql](../../../../../../includes/esql-md.md)] 中的有效別名是任何簡單識別碼或引號識別碼。  
  
### <a name="alias-generation"></a>別名產生  
 如果在 [!INCLUDE[esql](../../../../../../includes/esql-md.md)] 查詢運算式中未指定別名，[!INCLUDE[esql](../../../../../../includes/esql-md.md)] 會嘗試根據下列簡單規則產生別名：  
...
## <a name="scoping-rules"></a>範圍規則  
 ...
 當名稱定義于 [!INCLUDE[esql](../../../../../../includes/esql-md.md)] 查詢中時，就會被視為在範圍內定義。 範圍涵蓋查詢的整個區域。 某個範圍內的所有運算式或名稱參考都可以看到該範圍內定義的名稱。 在範圍開始前及結束後，無法參考此範圍內所定義的名稱。  
  
 範圍可以是巢狀的。 [!INCLUDE[esql](../../../../../../includes/esql-md.md)] 的部分會引進涵蓋整個區域的新範圍，而且這些區域可以包含也會引進範圍的其他 [!INCLUDE[esql](../../../../../../includes/esql-md.md)] 運算式。 當範圍是巢狀時，可以參考最內部範圍 (包含參考) 內定義的名稱。 也可以參考任何外部範圍內定義的任何名稱。 相同範圍內定義的任何兩個範圍都視為同層級 (Sibling) 範圍。 無法參考同層級範圍內定義的名稱。  
...
### <a name="query-expressions"></a>查詢運算式  
 [!INCLUDE[esql](../../../../../../includes/esql-md.md)] 的查詢運算式引進了新的範圍。 FROM 子句中定義的名稱會導入來源範圍內 (根據外觀從左到右)。 在聯結清單中，運算式可以參考之前定義在清單中的名稱。 FROM 子句中識別之項目的公用屬性 (欄位等等) 會加入來源範圍。 這些屬性必須由別名限定的名稱所參考。 一般來說，SELECT 運算式的所有部分都視為在來源範圍中。  
