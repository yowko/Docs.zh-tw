...
> [!NOTE]
>  所指定的運算式 ```Employee``` 必須是所指定資料類型  `Person` 的子類型，或者此資料類型必須是此運算式的子類型。 否則此運算式將會造成編譯時期錯誤。  
...
## 範例  
 下列 [!INCLUDE[esql](../../../../../../includes/esql-md.md)] 查詢使用 TREAT 運算子將 Course 型別的物件轉換成 OnsiteCourse 型別的物件集合。 此查詢是以 [School Model](http://msdn.microsoft.com/zh-tw/859a9587-81ea-4a45-9bc0-f8d330e1adac) 為基礎。  
