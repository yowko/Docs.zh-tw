...
[!INCLUDE[esql](../../../../../../includes/esql-md.md)] 導入了命名空間來避免全域識別碼的名稱衝突，例如型別名稱、實體集、函式等等。 @No__t_0 中的命名空間支援類似于 .NET Framework 中的命名空間支援。  
...
## <a name="name-resolution-rules"></a>名稱解析規則  
 如果無法在本機範圍中解析識別碼，[!INCLUDE[esql](../../../../../../includes/esql-md.md)] 會嘗試在全域範圍（命名空間）中尋找名稱。 [!INCLUDE[esql](../../../../../../includes/esql-md.md)] 會先嘗試將此識別項 (前置詞) 與其中一個限定的命名空間比對。 如果有相符的結果，[!INCLUDE[esql](../../../../../../includes/esql-md.md)] 會嘗試解析指定之命名空間中的其餘識別碼。 如果兩者不相符，則會擲回例外狀況。  
  
 接下來，[!INCLUDE[esql](../../../../../../includes/esql-md.md)] 嘗試搜尋識別碼的所有未限定命名空間（在初構中指定）。 如果可以在剛好一個命名空間內找到此識別項，就會傳回該位置。 如果有一個以上的命名空間符合該識別項，則會擲回例外狀況。 如果找不到識別碼的命名空間，[!INCLUDE[esql](../../../../../../includes/esql-md.md)] 會將此名稱傳遞至下一個向外的範圍（<xref:System.Data.Common.DbCommand> 或 <xref:System.Data.Common.DbConnection> 物件），如下列範例所示：  
