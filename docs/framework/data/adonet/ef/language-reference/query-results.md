...
LINQ to Entities 的查詢轉換成命令樹並執行之後，查詢結果通常會以下列其中一種方式傳回：  
...
- 匿名型別。  
 ...
 如果查詢執行傳回基本概念模型類型，則結果是由獨立且與 Entity Framework 中斷連接的 CLR 類型所組成。 但是，如果查詢傳回以 <xref:System.Data.Objects.ObjectQuery%601> 表示的具型別實體物件集合，這些型別將會由此內容物件追蹤。 所有物件行為 (例如子/父集合、變更追蹤、多型等) 都會如 Entity Framework 中所定義。 這項功能可用於其容量中，如 Entity Framework 中所定義。 如需詳細資訊，請參閱 [使用物件](../working-with-objects.md)。
