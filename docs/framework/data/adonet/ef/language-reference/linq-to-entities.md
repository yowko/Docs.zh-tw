...
## <a name="composing-the-queries"></a>撰寫查詢  
 ...
 如需詳細資訊，請參閱 [LINQ to Entities 中的查詢](queries-in-linq-to-entities.md)。  
  
## <a name="query-conversion"></a>查詢轉換  

 若要針對 Entity Framework 執行 LINQ to Entities 查詢，LINQ 查詢必須轉換成可以針對 Entity Framework 執行的命令樹表示法。  
  
 LINQ to Entities 查詢是由 LINQ 標準查詢運算子所組成 (例如 <xref:System.Linq.Queryable.Select%2A> 、 <xref:System.Linq.Queryable.Where%2A> 和 <xref:System.Linq.Queryable.GroupBy%2A>) 以及 (x > 10、Contact、LastName 等等的運算式。 LINQ 運算子並不是由類別所定義，而是類別上的方法。 在 LINQ 中，運算式可以包含 <xref:System.Linq.Expressions> 命名空間內類型所允許的任何項目，以及能以 Lambda 函式表示的任何項目 (依擴充而定)。 這是 Entity Framework 所允許之運算式的超集，根據定義，這些運算式限制為只有資料庫上允許且 <xref:System.Data.Objects.ObjectQuery%601> 支援的作業。  
 ...
 許多 LINQ 標準查詢運算子沒有 LINQ to Entities 中的有效轉譯。 嘗試使用這些運算子將會在查詢轉譯時產生例外狀況。 如需支援之 LINQ to Entities 運算子的清單，請參閱 [支援和不支援的 LINQ 方法 (LINQ to Entities) ](supported-and-unsupported-linq-methods-linq-to-entities.md)。  
 ...
 一般來說，LINQ to Entities 中的運算式會在伺服器上評估，所以運算式的行為不需要遵循 CLR 語意。 如需詳細資訊，請參閱 [LINQ to Entities 查詢中的運算式](expressions-in-linq-to-entities-queries.md)。  
  
 如需有關 CLR 方法呼叫如何對應到資料來源中標準函式的詳細資訊，請參閱 [Clr 方法與標準](clr-method-to-canonical-function-mapping.md)函式的對應。  
  
 如需如何從 LINQ to Entities 查詢內呼叫標準、資料庫和自訂函數的詳細資訊，請參閱 [LINQ to Entities 查詢中呼叫](calling-functions-in-linq-to-entities-queries.md)函式。  
  
## <a name="query-execution"></a>查詢執行  

 當使用者建立 LINQ 查詢之後，會將它轉換成與 Entity Framework 相容的表示法 (命令樹的形式)，然後針對資料來源執行此查詢。 在執行查詢時，所有的查詢運算式 (或是查詢的元件) 都會在用戶端或伺服器上評估， 這包括用於結果具體化或實體投影的運算式。 如需詳細資訊，請參閱 [查詢執行](query-execution.md)。 如需如何藉由編譯查詢一次，然後以不同的參數執行數次來改善效能的詳細資訊，請參閱 [ (LINQ to Entities) 的編譯查詢 ](compiled-queries-linq-to-entities.md)。  
...
- 匿名型別。  
 ...
 如需詳細資訊，請參閱 [查詢結果](query-results.md)。  
...
## <a name="in-this-section"></a>本節內容  
 ...
 [編譯的查詢 (LINQ to Entities) ](compiled-queries-linq-to-entities.md)  
