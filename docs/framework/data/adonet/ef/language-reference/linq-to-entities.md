...
1.  從 <xref:System.Data.Objects.ObjectContext> 建構 <xref:System.Data.Objects.ObjectQuery%601> 執行個體。  
...
## 撰寫查詢  
 實作泛型 <xref:System.Linq.IQueryable%601> 介面之 <xref:System.Data.Objects.ObjectQuery%601> 泛型類別的執行個體會當做 LINQ to Entities 查詢的資料來源。  在查詢中，您可以精確地指定想要從資料來源中擷取的資訊。  此外，查詢也可以指定該項資訊傳回之前應該如何排序、分組和成形。  在 LINQ 中，查詢會儲存在變數內。  這個查詢變數不會採取任何動作，也不會傳回任何資料。它只會儲存查詢資訊。  在您建立查詢之後，必須執行該查詢以便擷取任何資料。  
...
## 查詢轉換  
 ...
 LINQ to Entities 查詢是由 LINQ 標準查詢運算子 \(如 <xref:System.Linq.Queryable.Select%2A>、<xref:System.Linq.Queryable.Where%2A> 和 <xref:System.Linq.Queryable.GroupBy%2A>\) 和運算式 \(x \> 10、Contact.LastName 等等\) 所組成。  LINQ 運算子並不是由類別所定義，而是類別上的方法。  在 LINQ 中，運算式可以包含 <xref:System.Linq.Expressions> 命名空間內類型所允許的任何項目，以及能以 Lambda 函式表示的任何項目 \(依擴充而定\)。  這是 Entity Framework 所允許之運算式的超集，根據定義，這些運算式限制為只有資料庫上允許且 <xref:System.Data.Objects.ObjectQuery%601> 支援的作業。  
 ...
 許多 LINQ 標準查詢運算子沒有 LINQ to Entities 中的有效轉譯。  嘗試使用這些運算子將會在查詢轉譯時產生例外狀況。  如需支援的 LINQ to Entities 運算子清單，請參閱[支援與不支援的 LINQ 方法 \(LINQ to Entities\)](../../../../../../docs/framework/data/adonet/ef/language-reference/supported-and-unsupported-linq-methods-linq-to-entities.md)。  
  
 如需在 LINQ to Entities 中使用標準查詢運算子的詳細資訊，請參閱 [LINQ to Entities 查詢中的標準查詢運算子](../../../../../../docs/framework/data/adonet/ef/language-reference/standard-query-operators-in-linq-to-entities-queries.md)。  
 ...
 如需 CLR 方法呼叫如何對應到資料來源中之標準函式的詳細資訊，請參閱 [CLR 方法與標準函式的對應](../../../../../../docs/framework/data/adonet/ef/language-reference/clr-method-to-canonical-function-mapping.md)。  
  
 如需如何從 [!INCLUDE[linq_entities](../../../../../../includes/linq-entities-md.md)] 查詢內呼叫標準、資料庫和自訂函式，請參閱[呼叫 LINQ to Entities 查詢中的函式](../../../../../../docs/framework/data/adonet/ef/language-reference/calling-functions-in-linq-to-entities-queries.md)。  
  
## 查詢執行  
 當使用者建立 LINQ 查詢之後，會將它轉換成與 Entity Framework 相容的表示法 \(命令樹的形式\)，然後針對資料來源執行此查詢。  在執行查詢時，所有的查詢運算式 \(或是查詢的元件\) 都會在用戶端或伺服器上評估，  這包括用於結果具體化或實體投影的運算式。  如需詳細資訊，請參閱[查詢執行](../../../../../../docs/framework/data/adonet/ef/language-reference/query-execution.md)。  如需如何將查詢編譯一次然後以不同的參數執行多次，藉此改善效能的詳細資訊，請參閱[已編譯的查詢 \(LINQ to Entities\)](../../../../../../docs/framework/data/adonet/ef/language-reference/compiled-queries-linq-to-entities.md)。  
...
-   匿名型別。  
 ...
 如需詳細資訊，請參閱[查詢結果](../../../../../../docs/framework/data/adonet/ef/language-reference/query-results.md)。  
...
## 本章節內容  
 ...
 [已編譯的查詢 \(LINQ to Entities\)](../../../../../../docs/framework/data/adonet/ef/language-reference/compiled-queries-linq-to-entities.md)  
 ...
 [CLR 方法與標準函式的對應](../../../../../../docs/framework/data/adonet/ef/language-reference/clr-method-to-canonical-function-mapping.md)  
