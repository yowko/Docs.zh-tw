...
以集合為基礎的彙總 (集合函式) 會針對集合運作並傳回一個值。 例如如果 ORDERS 是所有訂單的集合，您可以計算最早出貨日期使用下列運算式：
...
很可能在 SELECT 運算式中有沒有明確的群組依據子句以群組為基礎的彙總。 在此情況下，所有元素都會都視為單一群組。 這是指定群組，根據常數的對等項目。 以下列運算式為例：
