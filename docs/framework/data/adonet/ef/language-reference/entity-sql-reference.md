...
本節包含 Entity SQL 參考文件。 本文摘要說明，並依類別分組的 Entity SQL 運算子。
...
|功能|類型|
|--------------|----------|
|[彙總 Entity SQL 標準函式](aggregate-canonical-functions.md)|討論彙總 Entity SQL 標準函式。|
...
|運算子|使用|
|--------------|---------|
|[FROM](from-entity-sql.md)|指定集合中所使用[選取](select-entity-sql.md)陳述式。|
|[GROUP BY](group-by-entity-sql.md)|指定的物件，查詢所傳回的群組 ([選取](select-entity-sql.md)) 運算式是要放置。|
...
|[ORDER BY](order-by-entity-sql.md)|指定所傳回物件使用的排序次序[選取](select-entity-sql.md)陳述式。|
...
Entity SQL 提供各種功能強大的設定作業。 這包括設定運算子類似 TRANSACT-SQL 運算子，例如 UNION、 INTERSECT、 EXCEPT 和 EXISTS。 Entity SQL 也會支援複本刪除 (SET)、 成員資格測試 (IN) 和聯結 (JOIN) 運算子。 下表列出 Entity SQL 設定運算子：
...
Entity SQL 提供的型別建構、 查詢和操作運算式 （值） 的作業。 下表列出用來與類型搭配使用的運算子：
...
|運算子|使用|
|--------------|---------|
|[+ (字串串連)](string-concatenation-entity-sql.md)|用來串連在 Entity SQL 的字串。|
|[.(成員存取)](member-access-entity-sql.md)|用來存取結構化概念模型型別之執行個體的屬性或欄位值。|
|[-- (註解)](comment-entity-sql.md)|加入實體 SQL 註解。|
