...
1. 在您的資料庫中建立自訂函式。
     ...
     如需在 SQL Server 中建立自訂函數的詳細資訊，請參閱[CREATE FUNCTION （transact-sql）](https://go.microsoft.com/fwlink/?LinkID=139871)。
...
接下來，在 *.edmx*檔案的存放結構定義語言（SSDL）中宣告函式。 下列程式碼會宣告 SSDL 中的 `AvgStudentGrade` 函數：
