...
|Code First|從 Entity Framework 4.1 開始，您可以使用 Code First 開發透過程式設計方式建立模型。 Code First 開發有兩種不同的方案。 在這兩種情況下，開發人員透過編碼 .NET Framework 類別定義建立模型，然後選擇性地使用資料註解或 Fluent 應用程式開發介面指定其他對應或組態。<br /><br /> 請注意，Code First 開發是 Entity Framework 5.0 的一部分。 Entity Framework 5.0 不是 .NET Framework 的一部分，而是以 .NET Framework 4.5 為基礎。 Entity Framework 5.0 是以[Entity Framework](https://www.nuget.org/packages/EntityFramework) NuGet 套件的形式提供。 如需詳細資訊，請參閱[過去的 Entity Framework 版本](/ef/ef6/what-is-new/past-releases)。|  
...
|容器|實體集和關聯集的邏輯群組。<br /><br /> 如需詳細資訊，請參閱[EntityContainer 元素（CSDL）](/ef/ef6/modeling/designer/advanced/edmx/csdl-spec#entitycontainer-element-csdl)和[實體容器](../entity-container.md)。|  
...
|EntityClient|與儲存體無關的 ADO.NET 資料提供者，其中包含 `EntityConnection`、`EntityCommand`和 `EntityDataReader`等類別。 適用于 [!INCLUDE[esql](../../../../../includes/esql-md.md)] 並連接至儲存體特定的 ADO.NET 資料提供者，例如 `SqlClient`。<br /><br /> 如需詳細資訊，請參閱 [適用於 Entity Framework 的 EntityClient 提供者](entityclient-provider-for-the-entity-framework.md)。|  
...
|索引鍵|實體類型的屬性，其中指定要用來識別實體類型之唯一執行個體的屬性或屬性集。 它是由 <xref:System.Data.EntityKey> 類別顯示在物件層。<br /><br /> 如需詳細資訊，請參閱[Key 元素（CSDL）](/ef/ef6/modeling/designer/advanced/edmx/csdl-spec#key-element-csdl)和[實體索引鍵](../entity-key.md)。|  
...
|Multiple Entity Sets Per Type - 每個類型的多重實體|在多個實體集內定義實體類型的能力。<br /><br /> 如需詳細資訊，請參閱[EntitySet 元素（CSDL）](/ef/ef6/modeling/designer/advanced/edmx/csdl-spec#entityset-element-csdl)和[如何：定義具有每個類型之多個實體集的模型](https://docs.microsoft.com/previous-versions/dotnet/netframework-4.0/bb738537(v=vs.100))。|  
...
|物件內容|表示概念模型中定義的實體容器。 它會包含基礎資料來源的連接並提供如變更追蹤和識別解析這類的服務。 物件內容是由 <xref:System.Data.Objects.ObjectContext> 或 `DbContext` 類別的執行個體來表示。<br /><br /> `DbContext` 是 Entity Framework 5.0 的一部分。 Entity Framework 5.0 不是 .NET Framework 的一部分，而是以 .NET Framework 4.5 為基礎。 Entity Framework 5.0 是以[Entity Framework](https://www.nuget.org/packages/EntityFramework) NuGet 套件的形式提供。 如需詳細資訊，請參閱[過去的 Entity Framework 版本](/ef/ef6/what-is-new/past-releases)。|  
...
|POCO entity - POCO 實體|Entity Framework 中的實體，不會繼承自 <xref:System.Data.Objects.DataClasses.EntityObject> 或 <xref:System.Data.Objects.DataClasses.ComplexObject>，而且不會執行 Entity Framework 介面。 POCO 實體通常是您在 Entity Framework 應用程式中使用的現有網域物件。 這些實體支援非持續性。 如需詳細資訊，請參閱[使用 POCO 實體](https://docs.microsoft.com/previous-versions/dotnet/netframework-4.0/dd456853(v=vs.100))。|  
...
|Split Entity - 分割實體|對應至儲存體模型中兩個不同類型的實體類型。<br /><br /> 如需詳細資訊，請參閱[如何：使用對應至兩個數據表的單一實體來定義模型](https://docs.microsoft.com/previous-versions/dotnet/netframework-4.0/bb896233(v=vs.100))。|  
