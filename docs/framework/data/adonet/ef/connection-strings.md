...
`DataDirectory`和 ~ 運算子是用在中<xref:System.Data.EntityClient.EntityConnection.ConnectionString%2A>做為`Metadata`和`Provider Connection String`關鍵字的一部分。 <xref:System.Data.EntityClient.EntityConnection> 會分別將 `DataDirectory` 和 ~ 運算子轉送至 <xref:System.Data.Metadata.Edm.MetadataWorkspace> 和存放區提供者。
