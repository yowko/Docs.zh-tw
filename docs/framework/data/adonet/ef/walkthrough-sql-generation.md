...
控制權會傳回正在處理的 Join4，它是 Join1 的父系。 因為子系已重複使用 Select 陳述式，在使用單一 Join 符號取代 Join1 範圍\<joinSymbol_Join1 >。 此外，新的項目會加入至符號表，以便將 Join1 與產生關聯\<joinSymbol_Join1 >。
...
因為它是巢狀的聯結，我們查詢的屬性"extent4，並"聯結符號的 NameToExtent 字典中，它解析為\<symbol_Extent4 >，並傳回新的 SymbolPair (\<joinSymbol_join2 >， \<symbol_Extent4>)。 因為符號組從傳回執行個體"var(join2 處理。Extent4.OrderID"，"OrderID"屬性是從該符號組 ColumnPart 解析 (\<symbol_Extent4 >)，其中包含它所代表之範圍的資料行清單。 因此，"var(join2。Extent4.OrderID"會解析為 { \<joinSymbol_Join2 >，"。"， \<symbol_OrderID >}。
...
第一個符號可讓您找到的需要重新命名為\<symbol_OrderID >。 它的新名稱會指派為 "OrderID1"，而且 1 會標示為上次用於 "OrderID" 的尾碼，而且此符號會標示為不需要重新命名。 接下來，第一次使用\<symbol_OrderID_2 > 找到。 它會重新命名來使用下一個可用的尾碼 ("OrderID2") 而且會再次標示為不需要重新命名，所以下次使用它時就不會重新命名。 這為了\<symbol_OrderID_3 > 太。
