...
|函數|說明|  
|--------------|-----------------|  
|`ASCII(expression)`|傳回字串運算式最左側字元的 ASCII 字碼值。<br /><br /> **引數**<br /><br /> `expression`：ASCII `String`型別的任何有效運算式。<br /><br /> **傳回值**<br /><br /> `Int32`。<br /><br /> **範例**<br /><br /> `SqlServer.ASCII('A')`|  
...
|`CHARINDEX(expression1, expression2 [, start_location])`|傳回指定運算式在某個字元字串中的起始位置。<br /><br /> **引數**<br /><br /> `expression1`：含有要尋找之字元順序的運算式。 此運算式可以是 String (ASCII 或 Unicode) 型別或 Binary 型別。<br /><br /> `expression2`：通常是搜尋指定順序之資料行的運算式。 此運算式可以是 String (ASCII 或 Unicode) 型別或 Binary 型別。<br /><br /> `start_location`:(選擇性） Int64 （在 SQL Server 2000 中不會傳回）或 Int32，表示開始搜尋運算式2中的運算式類型的字元位置。 如果 start_location 沒有指定、為負數或者為零，則此搜尋就會從 expression2 的起點開始。<br /><br /> **傳回值**<br /><br /> `Int32`。<br /><br /> **範例**<br /><br /> `SqlServer.CHARINDEX('h', 'habcdefgh', 2)`|  
...
|`LEFT(expression, count)`|傳回指定字元數之字元字串的左側部分。<br /><br /> **引數**<br /><br /> `expression`：Unicode 或 ASCII 字串類型。 使用 CAST 函式可明確轉換 character_expression。<br /><br /> `count`：（不會在 SQL Server 2000 中傳回） `Int32`或類型，指定要傳回 character_expression 的字元數。 `Int64`<br /><br /> **傳回值**<br /><br /> Unicode 或 ASCII `String`。<br /><br /> **範例**<br /><br /> `SqlServer.LEFT('SQL Server', 4)`|  
|`LEN(expression)`|傳回指定 String 運算式中的字元數，但尾端空白不算。<br /><br /> **引數**<br /><br /> `expression`：`String` （Unicode 或 ASCII）型別`Binary`或型別的運算式<br /><br /> **傳回值**<br /><br /> `Int32`。<br /><br /> **範例**<br /><br /> `SqlServer.LEN('abcd')`|  
...
|`REPLACE(expression1, expression2, expression3)`|以另一個字元運算式取代字元運算式。<br /><br /> **引數**<br /><br /> `expression1`：要搜尋的字串運算式。 `expression1`可以是 Unicode 或 ASCII 字串類型。<br /><br /> `expression2`：要尋找的子字串。 `expression2`可以是 Unicode 或 ASCII 字串類型。<br /><br /> `expression3`；取代字串。 `expression3`可以是 Unicode 或 ASCII 字串類型。<br /><br /> **範例**<br /><br /> `SqlServer.REPLACE('aabbcc', 'bc', 'zz')`|  
...
|`RIGHT(char_expression, count)`|傳回指定字元數之字元字串的右側部分。<br /><br /> **引數**<br /><br /> `char_expression`： Unicode 或 ASCII 字串類型。 使用 CAST 函式可明確轉換 character_expression。<br /><br /> `count`：（不會在 SQL Server 2000 中傳回） `Int32`或類型，指定要傳回 character_expression 的字元數。 `Int64`<br /><br /> **傳回值**<br /><br /> ASCII `String` 型別。<br /><br /> **範例**<br /><br /> `SqlServer.RIGHT('SQL Server', 6)`|  
...
|`SOUNDEX(expression)`|傳回四個字元的（SOUNDEX）程式碼，以評估兩個字串的相似性。**引數**<br /><br /> `expression`：Unicode 或 ASCII 字串類型。<br /><br /> **傳回值**<br /><br /> ASCII `String`。 四個字元的 (SOUNDEX) 代碼是用來評估兩個字串之相似度的一個字串。<br /><br /> **範例**<br /><br /> `Select SqlServer.SOUNDEX('Smith'), SqlServer.SOUNDEX('Smythe') FROM {1}`<br /><br /> **傳回**<br /><br /> `----- -----  S530  S530`|  
|`SPACE(int_expression)`|傳回重複空格的 ASCII `String`。<br /><br /> **引數**<br /><br /> `int_expression`：（在 SQL Server 2000 中不會傳回） `Int32`或表示空格數目的。 `Int64`<br /><br /> **傳回值**<br /><br /> ASCII `String`。<br /><br /> **範例**<br /><br /> `SqlServer.SPACE(2)`|  
|`STR(float_expression [, length [, decimal]])`|傳回從數值資料轉換而來的 ASCII `String`。<br /><br /> **引數**<br /><br /> `float _expression`：含小數點之近似數值 (`Double`) 資料型別的運算式。<br /><br /> `length`：(選擇項) 代表總長度的 `Int32`。 其中包括小數點、正負號、位數和空格。 預設值為 10。<br /><br /> `decimal`:(選擇性） `Int32` ，代表小數點右邊的位數。 小數位數必須小於或等於 16。 如果小數位數超過 16，結果將會被截斷成小數點右邊的 16 位數。<br /><br /> **傳回值**<br /><br /> ASCII `String`。<br /><br /> **範例**<br /><br /> `SqlServer.STR(212.0)`|  
|`STUFF(str_expression, start, length, str_expression_to_insert)`|刪除指定長度的字元，並在字串運算式中指定的起點插入另一組字元。<br /><br /> **引數**<br /><br /> `str_expression`：Unicode 或 ASCII `String`。<br /><br /> `start:`（在 SQL Server 2000 中不會傳回） `Int32`或值，指定開始刪除和插入的位置。 `Int64`<br /><br /> `length`：`Int64` (SQL Server 2000 中不會傳回) 或指定要刪除之字元數的 `Int32` 值。<br /><br /> `str_expression_to_insert`：Unicode 或 ASCII `String`。<br /><br /> **傳回值**<br /><br /> Unicode 或 ASCII `String`。<br /><br /> **範例**<br /><br /> `SqlServer.STUFF('abcd', 2, 2, 'zz')`|  
|`SUBSTRING(str_expression, start, length)`|傳回 `String` 運算式的一部分。<br /><br /> **引數**<br /><br /> `str_expression`：具有 `String` (ASCII 或 Unicode) 型別或 `Binary` 型別的運算式。<br /><br /> `start`：（在 SQL Server 2000 中不會傳回） `Int32`或指定子字串開始位置的。 `Int64` 1 代表字串中的第一個字元。<br /><br /> `length`：（在 SQL Server 2000 中不會傳回） `Int32` ，或指定將傳回運算式之字元數的。 `Int64`<br /><br /> **傳回值**<br /><br /> `String` (ASCII 或 Unicode) 型別或 `Binary` 型別。<br /><br /> **範例**<br /><br /> `SqlServer.SUBSTRING('abcd', 2, 2)`|  
...
|`UPPER(expression)`|將小寫字元資料轉換成大寫之後，傳回 `String` 運算式。<br /><br /> **引數**<br /><br /> `expression`：ASCII 或 Unicode 字串類型的運算式。<br /><br /> **傳回值**<br /><br /> ASCII 或 Unicode `String` 型別。<br /><br /> **範例**<br /><br /> `SqlServer.UPPER('AbB')`|  
