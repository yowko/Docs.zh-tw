...
因為每個值都會在下一個分號或字串結尾之前讀取，所以後者範例中的值是 `a=b=c`，而最後一個分號則是選擇性的。
...
輸入錯誤可能會導致錯誤。 例如，`Integrated Security=true` 有效，但是 `IntegratedSecurity=true` 會造成錯誤。
...
[連接字串](connection-string-builders.md)產生器\
