...
## <a name="the-net-framework-data-provider-for-odbc"></a>ODBC 的 .NET Framework 資料提供者  
 從1.1 版開始，ODBC （<xref:System.Data.Odbc>）的 .NET Framework Data Provider 會包含為 .NET Framework 的一部分。
  
 如果您的應用程式是針對使用 ODBC 資料提供者連接到資料來源的 .NET Framework 版本1.0 所開發，而您想要在 .NET Framework 版本1.1 或更新版本上執行該應用程式，您必須將 ODBC 資料提供者的命名空間更新為**system.object**。 接著，您必須針對較新版本的 .NET Framework 重新編譯它。  
...
## <a name="the-net-framework-data-provider-for-oracle"></a>Oracle 的 .NET Framework 資料提供者  
 從1.1 版開始，Oracle 的 .NET Framework Data Provider （<xref:System.Data.OracleClient>）會納入 .NET Framework 的一部分。
...
## <a name="code-access-security"></a>程式碼存取安全性  
 .NET Framework 版本1.0 （<xref:System.Data.SqlClient>，<xref:System.Data.OleDb>）中的 .NET Framework 資料提供者，必須以 FullTrust 許可權執行。 嘗試在許可權低於 FullTrust 的區域中，使用來自 .NET Framework 1.0 版的 .NET Framework k 資料提供者，會造成 <xref:System.Security.SecurityException>。  
...
## <a name="sqlcommand-execution"></a>SqlCommand 之執行  
 從 .NET Framework 版本1.1 開始，<xref:System.Data.SqlClient.SqlCommand.ExecuteReader%2A> 在資料來源執行命令的方式已變更。  
  
 在 .NET Framework 版本1.0 中，<xref:System.Data.SqlClient.SqlCommand.ExecuteReader%2A> 在**sp_executesql**預存程式的內容中執行了所有命令。 結果，凡是會影響連接狀態的命令 (例如，SET NOCOUNT ON) 只適用於目前命令的執行。 若連接已開啟，連接狀態就不會因為任何後續命令的執行而被修改。  
  
 在 .NET Framework 1.1 和更新版本中，只有在命令包含參數時，<xref:System.Data.SqlClient.SqlCommand.ExecuteReader%2A> 才會在**sp_executesql**預存程式的內容中執行命令，這可提供效能優勢。 這樣的結果是，若連接已開啟，凡是會影響連接狀態的命令 (它包含於非參數化命令中) 都會修改已執行之所有後續命令的連接狀態。  
...
```  
 ...
 這項變更可能會影響應用程式的向前和回溯相容性（如果您相依于任一 .NET Framework 版本的 <xref:System.Data.SqlClient.SqlCommand.ExecuteReader%2A> 行為）。  
