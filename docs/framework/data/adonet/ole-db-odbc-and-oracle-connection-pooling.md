...
```  
 ...
 如需有關 OLE DB 會話或資源分享, 以及如何藉由覆寫 OLE DB 提供者服務預設值來停用共用的詳細資訊, 請參閱 OLE DB 程式設計[人員指南](https://go.microsoft.com/fwlink/?linkid=45232)。  
...
## <a name="connection-pooling-for-odbc"></a>Odbc 的連接共用  
 ...
 若要啟用或停用連接共用, 請在 [控制台] 的 [系統管理工具] 資料夾中開啟 [ **ODBC 資料來源管理員**]。 [**連接**共用] 索引標籤可讓您指定每個已安裝 ODBC 驅動程式的連接共用參數。 請注意，特定 ODBC 驅動程式的連接共用變更會影響使用該 ODBC 驅動程式的所有應用程式。  
