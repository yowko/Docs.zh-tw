---
title: "OLE DB、ODBC 和 Oracle 連接共用 | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
...
```  
 ...
 如需 OLE DB 工作階段或資源共用，以及如何藉由覆寫 OLE DB 提供者服務預設值以停用共用的詳細資訊，請參閱 MSDN Library 中的 [OLE DB 程式設計人員手冊](http://go.microsoft.com/fwlink/?linkid=45232)。  
  
## Odbc 的連接共用  
 ODBC 的 .NET Framework 資料提供者連接共用由用於連接的 ODBC 驅動程式管理員管理，並且不受 ODBC 的 .NET Framework 資料提供者的影響。  
  
 若要啟用或停用連接共用，請開啟 \[控制台\] 之 \[系統管理工具\] 資料夾中的 \[**ODBC 資料來源管理員**\]。  \[**連接共用**\] 索引標籤可讓您為每個安裝的 ODBC 驅動程式指定連接共用參數。  請注意，特定 ODBC 驅動程式的連接共用變更會影響使用該 ODBC 驅動程式的所有應用程式。  
  
 如需 ODBC 連接共用的詳細資訊，請參閱 MSDN Library 中的 [INFO: 常見關於 ODBC 連接共用區的問題](http://support.microsoft.com/kb/169470)。  
...
### 異動支援  
 從集區中描繪連接，並根據交易內容進行指派。  要求執行緒的內容必須與指派的連接相符。  因此，實際上會將每個連接集區分為無相關聯交易內容的連接，並分為均包含具有特定交易內容之連接的 *N* 個子區塊。  
  
 連接關閉時，會根據其交易內容將其釋放回集區，並置於適當的子區塊中。  因此，即使分散式交易仍處於暫止狀態，您仍可以關閉連接，而不會產生錯誤。  這可讓您稍後認可或中止分散式交易。  
  
### 使用連接字串關鍵字控制連接共用  
 <xref:System.Data.OracleClient.OracleConnection> 物件的 <xref:System.Data.OracleClient.OracleConnection.ConnectionString%2A> 屬性支援連接字串索引鍵\/值配對，這些配對可用於調整連接共用邏輯的行為。  
