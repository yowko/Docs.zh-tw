...
## <a name="example"></a>範例  
 此範例會使用下列預存程序插入至新的分類**Northwind** **類別**資料表。 預存程序使用值**CategoryName**資料行做為輸入的參數，然後使用 scope_identity （） 函式來擷取識別欄位的新值**CategoryID**，並將其傳回在輸出參數。 RETURN 陳述式使用 @@ROWCOUNT函式傳回插入的資料列數。  
...
> [!NOTE]
>  當使用<xref:System.Data.OleDb.OleDbDataAdapter>，您必須指定具有參數<xref:System.Data.ParameterDirection>的**ReturnValue**其他參數前。  
