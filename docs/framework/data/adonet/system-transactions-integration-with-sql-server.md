...
.NET 框架版本 2.0 引入了一個事務框架，可通過<xref:System.Transactions>命名空間進行訪問。 此框架以完全集成在 .NET 框架（包括ADO.NET）的方式公開事務。  
  
 除了可程式設計性增強之外，ADO.NET<xref:System.Transactions>可以在處理事務時協同優化。 可提升的交易是可視需要自動提升為完全分散式交易的輕量型 (本機) 交易。  
  
 從 ADO.NET 2.0<xref:System.Data.SqlClient>開始，在使用 SQL Server 時支援可重現的事務。 除非需要已加入的負荷，否則可提升交易不會叫用分散式交易的已加入負荷。 可促銷事務是自動的，不需要開發人員的干預。  
  
 僅當將 SQL Server 的 .NET 框架資料提供程式 （）`SqlClient`與 SQL Server 一起使用時，可促銷事務才可用。  
  
## <a name="creating-promotable-transactions"></a>建立可提升交易  
 SQL Server 的 .NET 框架提供程式支援可促銷事務，這些事務通過 .NET 框架<xref:System.Transactions>命名空間中的類進行處理。 可提升交易藉由直到需要時才建立分散式交易的方式，來最佳化分散式交易。 如果只需要一個資源管理員，則不會發生分散式交易。  
...
## <a name="promotable-transaction-scenarios"></a>可提升交易案例  
 分散式交易通常要耗用大量的系統資源，並由「Microsoft 分散式交易協調器 (MS DTC)」進行管理，其整合了交易中會存取的所有資源管理者。 可促銷事務是一種特殊形式的<xref:System.Transactions>事務，它有效地將工作委託給簡單的 SQL Server 事務。 <xref:System.Transactions>和<xref:System.Data.SqlClient>SQL Server 協調處理事務所涉及的工作，根據需要將其提升為完全分散式交易。  
