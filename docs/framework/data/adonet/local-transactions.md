...
>  `EnlistDistributedTransaction` 方法不可用於本機異動。  
 ...
 交易的範圍僅限於連接。 下列範例執行由 `try` 區塊中的兩個單獨命令所組成的明確異動。 命令執行針對的 Production.ScrapReason 資料表的 INSERT 陳述式在 AdventureWorks SQL Server 範例資料庫中，也就是已認可，如果擲不回任何例外狀況。 如果擲回例外狀況，則 `catch` 區塊中的程式碼便會復原交易。 如果異動在完成之前遭到中止或連接關閉，則會自動復原該異動。  
