...
- <xref:System.Linq.Enumerable.Except%2A>  
 ...
 這些運算子會針對每個項目集合呼叫 <xref:System.Collections.Generic.IEqualityComparer%601.GetHashCode%2A> 和 <xref:System.Collections.Generic.IEqualityComparer%601.Equals%2A> 方法，藉以比較來源項目。 在 <xref:System.Data.DataRow> 的情況中，這些運算子會執行參考比較，但是這通常不是表格式資料之設定作業的理想行為。 若為設定作業，您通常會想要判斷項目值是否相等，而非項目參考。 因此，<xref:System.Data.DataRowComparer>類別加入 LINQ to DataSet。 這個類別可用來比較資料列值。  
