...
---
# <a name="how-to-implement-copytodatatablet-where-the-generic-type-t-is-not-a-datarow"></a>作法：實作 CopyToDataTable\<T>，其中泛型型別 T 不是 DataRow
...
1. 實作 `ObjectShredder<T>` 類別以從 <xref:System.Data.DataTable> 來源建立 <xref:System.Collections.Generic.IEnumerable%601>：  
    ...
    上述範例假設的屬性和欄位 `DataColumn` 不是可為 null 的實數值型別。 若要使用可為 null 的實值型別來處理屬性和欄位，請使用下列程式碼：
