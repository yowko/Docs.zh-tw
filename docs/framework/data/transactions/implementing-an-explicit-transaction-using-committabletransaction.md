---
title: "使用 CommittableTransaction 實作明確交易  | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
...
## CommittableTransaction 類別的概觀  
 <xref:System.Transactions.CommittableTransaction> 類別係衍生自 <xref:System.Transactions.Transaction> 類別，因此可提供後者所有功能。<xref:System.Transactions.Transaction> 類別上的 <xref:System.Transactions.Transaction.Rollback%2A> 方法特別有用，因為它同時能用來復原 <xref:System.Transactions.CommittableTransaction> 物件。  
...
-   建立 <xref:System.Transactions.CommittableTransaction> 交易不會設定環境交易。您需要特別設定及重設環境交易，以確保資源管理員在適當時機，於正確的交易內容下作業。您可以在全域 <xref:System.Transactions.Transaction> 物件上設定靜態 <xref:System.Transactions.Transaction.Current%2A> 屬性，來設定目前的環境交易。  
...
## 建立 CommittableTransaction  
 ...
 建立 <xref:System.Transactions.CommittableTransaction> 執行個體不會自動設定環境交易內容。因此，資源管理員上的任何作業都不是該交易的一部分。全域 <xref:System.Transactions.Transaction> 物件上的靜態 <xref:System.Transactions.Transaction.Current%2A> 屬性是用來設定或擷取環境交易，應用程式必須手動加以設定以確保資源管理員可以參與交易。儲存舊有的環境交易並在 <xref:System.Transactions.CommittableTransaction> 物件使用完畢時將其還原是很好的使用習慣。  
...
## 非同步認可  
 ...
 進行非同步認可的最簡便方式，就是提供可在完成認可時加以呼叫的回呼方法。但是，您必須在用於叫用呼叫的原始 <xref:System.Transactions.CommittableTransaction> 物件上呼叫 <xref:System.Transactions.CommittableTransaction.EndCommit%2A> 方法。若要取得該物件，由於 <xref:System.Transactions.CommittableTransaction> 類別會實作 <xref:System.IAsyncResult> 類別，因此您可以向下轉換回呼方法的 *IAsyncResult* 參數。  
