...
- 在單一應用程式域中使用交易之應用程式的**AllowPartiallyTrustedCallers** （APTCA）。  
...
### <a name="aptca-partial-trust"></a>APTCA (部分信任)  
 <xref:System.Transactions>元件可由部分信任的程式碼呼叫，因為它已經以**AllowPartiallyTrustedCallers**屬性（APTCA）標記。 這個屬性基本上會移除 <xref:System.Security.Permissions.SecurityAction.LinkDemand> **FullTrust**許可權集合的隱含，否則會自動放在每個型別中每個可公開存取的方法上。 然而，部分型別和成員仍舊需要更強勢的使用權限。  
