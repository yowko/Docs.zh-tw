...
- **AllowPartiallyTrustedCallers** (APTCA) 在單一應用程式網域內使用交易的應用程式。  
...
### <a name="aptca-partial-trust"></a>APTCA (部分信任)  
 <xref:System.Transactions>可由部分信任程式碼呼叫組件，因為標記著**AllowPartiallyTrustedCallers**屬性 (APTCA)。 這個屬性基本上會移除隱含<xref:System.Security.Permissions.SecurityAction.LinkDemand>for **FullTrust**權限，也就是設定自動放置在每個型別中每個可公開存取的方法。 然而，部分型別和成員仍舊需要更強勢的使用權限。  
...
### <a name="fulltrust-link-demands"></a>FullTrust 連結要求  
 ...
 如需有關永久性登記與復原的詳細資訊，請參閱 <<c0> [ 編列的資源，在交易中的參與者](../../../../docs/framework/data/transactions/enlisting-resources-as-participants-in-a-transaction.md)並[執行復原](../../../../docs/framework/data/transactions/performing-recovery.md)主題。  
  
 要擁有完全信任，必須同時具備使用 COM+ 來執行舊版 Interop 工作的應用程式。  
  
 以下是一份類型和成員不是可由呼叫部分信任的程式碼，因為它們會附有**FullTrust**宣告式安全性屬性：  
...
- <xref:System.Transactions.TransactionScope.%23ctor%28System.Transactions.TransactionScopeOption%2CSystem.Transactions.TransactionOptions%2CSystem.Transactions.EnterpriseServicesInteropOption%29>  
 ...
 只有立即呼叫端需要擁有**FullTrust**權限設定為使用上述的型別或方法。
