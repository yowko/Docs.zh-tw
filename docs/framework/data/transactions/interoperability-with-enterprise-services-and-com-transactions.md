...
description: 瞭解如何使用 system.string 命名空間，在 .NET 中與 enterprise services 和 COM + 交易的互通性。
ms.date: 03/30/2017
...
<xref:System.Transactions> 命名空間支援以此命名空間建立的交易物件，以及透過 COM+ 建立的交易兩者之間的互通性。  
 ...
 依預設，當您的應用程式程式碼檢查靜態 <xref:System.Transactions.Transaction.Current%2A> 屬性時， <xref:System.Transactions> 會嘗試尋找其他目前的交易，或指出 <xref:System.Transactions.TransactionScope> <xref:System.Transactions.Transaction.Current%2A> 為 **null**的物件。 如果程式碼找不到當中任何一個，則 <xref:System.Transactions> 會查詢 COM+ 內容，看看是否有交易。 請注意，就算 <xref:System.Transactions> 在 COM+ 內容中找到了交易項目，仍舊偏好 <xref:System.Transactions> 的原生交易。  
...
## <a name="interoperability-levels"></a>互通性等級  
 ...
 <xref:System.Transactions.EnterpriseServicesInteropOption.Automatic> 指定下列需求：  
...
1. <xref:System.Transactions.Transaction.Current%2A> 檢查以查看是否有交易。 檢查結果為：  
...
3. 如果需要建立新的交易，則下列各項 <xref:System.Transactions.EnterpriseServicesInteropOption> 值將產生：  
    ...
    - <xref:System.Transactions.EnterpriseServicesInteropOption.None>： <xref:System.Transactions> 已建立交易。  
  
    - <xref:System.Transactions.EnterpriseServicesInteropOption.Automatic>：如果有 COM + 內容，則會建立交易並將其附加至內容。  
  
 下表說明了 Enterprise Services (ES) 內容，以及需要使用 <xref:System.Transactions.EnterpriseServicesInteropOption> 列舉型別之交易的交易式範圍。  
  
|ES 內容|無|自動|完整|  
|----------------|----------|---------------|----------|  
|預設內容|預設內容|預設內容|新建 <br />交易式內容|  
