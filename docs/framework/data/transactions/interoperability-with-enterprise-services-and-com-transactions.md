...
description: 瞭解使用 System.webserver 命名空間，在 .NET 中與企業服務和 COM + 交易的互通性。
ms.date: 03/30/2017
...
1. <xref:System.Transactions.Transaction.Current%2A>會進行檢查，以查看是否有交易。 檢查結果為：  
...
3. 如果需要建立新的交易，則下列各項 <xref:System.Transactions.EnterpriseServicesInteropOption> 值將產生：  
    ...
    - <xref:System.Transactions.EnterpriseServicesInteropOption.None>： <xref:System.Transactions> 建立交易。  
