...
身為交易式應用程式設計人員，您可以利用 <xref:System.Transactions> 命名空間所提供的兩個程式撰寫模型 (Programming Model) 來建立交易。 您可以使用類別來利用明確的程式設計模型 <xref:System.Transactions.Transaction> ，或透過使用類別，將交易由基礎結構自動管理的隱含程式設計模型運用 <xref:System.Transactions.TransactionScope> 。 我們建議您將隱含的交易模型用於開發。 您可以在[使用交易範圍來執行隱含交易](implementing-an-implicit-transaction-using-transaction-scope.md)主題中找到有關如何使用交易範圍的詳細資訊。  
...
### <a name="concurrency"></a>並行  
 使用[DependentTransaction 管理並行](managing-concurrency-with-dependenttransaction.md)的主題會示範如何使用類別，在非同步工作之間達成平行存取 <xref:System.Transactions.DependentTransaction> 。  
...
### <a name="diagnostics"></a>診斷  
 [診斷追蹤](diagnostic-traces.md)會描述如何使用基礎結構所產生的追蹤程式碼， <xref:System.Transactions> 來疑難排解應用程式中的錯誤。  
  
### <a name="working-within-aspnet"></a>使用 ASP.NET  
 [ASP.NET 主題中的 Using](using-system-transactions-in-aspnet.md) system.string 一節會說明如何在 <xref:System.Transactions> ASP.NET 應用程式內成功使用。
