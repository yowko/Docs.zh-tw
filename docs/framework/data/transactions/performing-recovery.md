...
### <a name="step-2---completing-the-recovery"></a>步驟 2 - 完成復原  
 ...
 在登記新的交易之前，不需要透過資源管理員來解析所有不確定的交易。 在資源管理員與交易管理員建立關聯性之後，您可以隨時執行第一個步驟，但是在被叫用之後 <xref:System.Transactions.TransactionManager.RecoveryComplete%2A> （步驟2），就無法再次執行步驟1。 步驟 2 可以重複執行多次，而不會影響到交易結果。
