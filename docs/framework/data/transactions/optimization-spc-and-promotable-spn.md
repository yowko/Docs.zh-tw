---
title: "使用單一階段交易認可和可提升單一階段告知進行最佳化  | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
...
## 可提升單一階段登記  
 ...
 如果 <xref:System.Transactions> 交易需要擴大規模 \(例如，用以支援多個 RM\)，則 <xref:System.Transactions> 會呼叫 <xref:System.Transactions.IPromotableSinglePhaseNotification> 介面之來源 <xref:System.Transactions.ITransactionPromoter> 介面上的 <xref:System.Transactions.ITransactionPromoter.Promote%2A> 方法，以告知資源管理員。接著，資源管理員會從內部將交易由本機交易 \(不需要記錄\) 轉換為可參與 DTC 交易的交易物件，並將其與已經完成的工作產生關聯。一旦交易被要求認可，交易管理員仍舊會將 <xref:System.Transactions.IPromotableSinglePhaseNotification.SinglePhaseCommit%2A> 告知傳送給資源管理員，使其認可在擴大規模期間所建立的分散式交易。  
  
> [!NOTE]
>  **TransactionCommitted** 追蹤 \(在交易規模擴大期間叫用交易認可時所產生\) 包含 DTC 交易的活動識別碼。  
  
 如需管理擴大規模的詳細資訊，請參閱[交易管理擴大規模案例 ](../../../../docs/framework/data/transactions/transaction-management-escalation.md)。  
...
3.  <xref:System.Transactions> 會呼叫 CN1 於 <xref:System.Transactions.ITransactionPromoter> 介面上所實作的 <xref:System.Transactions.ITransactionPromoter.Promote%2A> 方法。  
