...
```  
 ...
 [!INCLUDE[ssODataShort](../../../../includes/ssodatashort-md.md)] 也可讓您根據查詢運算式的結果定址資源。 這可讓您能夠篩選評估的運算式為基礎的資源集。 例如，下列 URI 會篩選資源，只傳回自 1997 年 9 月 22 日後配送且屬於指定 Customer 的 Orders。  
...
## <a name="system-query-options"></a>系統查詢選項  
 [!INCLUDE[ssODataShort](../../../../includes/ssodatashort-md.md)] 定義一組系統查詢選項可讓您針對資源，例如篩選、 排序和分頁執行傳統查詢運算。 例如，下列 URI 會傳回所有集`Order`實體以及相關`Order_Detail`實體的郵遞區號結尾不`100`:  
