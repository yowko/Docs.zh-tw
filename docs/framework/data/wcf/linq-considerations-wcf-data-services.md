...
> [!IMPORTANT]
> LINQ 語法中可表示的查詢集合，比 OData 資料服務所使用的 URI 語法中所啟用的查詢集更廣。 當查詢無法對應至目標資料服務中的 URI 時，就會引發 <xref:System.NotSupportedException>。 如需詳細資訊，請參閱本主題中的[不支援的 LINQ 方法](linq-considerations-wcf-data-services.md#unsupportedMethods)。  
...
## <a name="executing-linq-queries"></a>執行 LINQ 查詢  
 ...
 當 LINQ 查詢無法在符合 OData 的查詢 URI 中轉譯時，在嘗試執行時，就會引發例外狀況。 如需詳細資訊，請參閱[查詢資料服務](querying-the-data-service-wcf-data-services.md)。  
  
## <a name="linq-query-examples"></a>LINQ 查詢範例  
 下列各節中的範例會示範可針對 OData 服務執行的 LINQ 查詢類型。  
...
### <a name="expand"></a>Expand  
 當您查詢 OData 資料服務時，您可以要求將與查詢做為目標之實體相關的實體包含在傳回的摘要中。 系統會針對 LINQ 查詢做為目標的實體集，在 <xref:System.Data.Services.Client.DataServiceQuery%601.Expand%2A> 上呼叫 <xref:System.Data.Services.Client.DataServiceQuery%601> 方法，並將相關的實體集名稱當做 `path` 參數提供。 如需詳細資訊，請參閱[載入延](loading-deferred-content-wcf-data-services.md)後的內容。  
...
## <a name="unsupported-linq-methods"></a>不支援的 LINQ 方法  
 下表包含不支援的 LINQ 方法類別，而且不能包含在針對 OData 服務執行的查詢中：  
...
|其他運算子|<xref:System.Data.Services.Client.DataServiceQuery%601>也不支援下列運算子：<br /><br /> - <xref:System.Linq.Enumerable.Empty%2A><br />- <xref:System.Linq.Enumerable.Range%2A><br />- <xref:System.Linq.Enumerable.Repeat%2A><br />- <xref:System.Linq.Enumerable.ToDictionary%2A><br />- <xref:System.Linq.Enumerable.ToLookup%2A>|  
  
<a name="supportedExpressions"></a>   
## <a name="supported-expression-functions"></a>支援的運算式函數  
 下列是支援的 common language runtime （CLR）方法和屬性，因為它們可以在查詢運算式中轉譯，以包含在 OData 服務的要求 URI 中：  
...
|<xref:System.DateTime.Year>|`int year(DateTime p0)`|  
 ...
 <sup>1</sup>同時也支援 <xref:Microsoft.VisualBasic.DateAndTime?displayProperty=nameWithType> 的對等日期和時間屬性和 Visual Basic 中的 <xref:Microsoft.VisualBasic.DateAndTime.DatePart%2A> 方法。  
