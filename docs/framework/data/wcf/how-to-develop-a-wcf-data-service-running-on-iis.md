...
### <a name="to-create-the-aspnet-web-application-that-runs-on-iis"></a>若要建立在 IIS 上執行的 ASP.NET Web 應用程式  
  
1.  在 Visual Studio 中，在**檔案**功能表上，選取**新增**，然後選取**專案**。  
  
2.  在**新專案**對話方塊方塊中，選取 Visual Basic 或 Visual C# 當做程式語言。  
  
3.  在**範本**窗格中，選取**ASP.NET Web 應用程式**。 注意：如果您使用 Visual Studio Web Developer，您必須建立新網站，而非建立新的 Web 應用程式。  
  
4.  型別`NorthwindService`做為專案的名稱。  
...
11. 當 ASP.NET 應用程式在 IIS7 上執行時，您也必須執行下列步驟：  
    ...
    4.  在**動作**] 窗格中，按一下 [**編輯**設定安全性主體匿名使用者會連線至站台。  
...
1.  在**方案總管 中**，以滑鼠右鍵按一下 ASP.NET 專案的名稱，然後按一下**加入新項目。**  
  
2.  在**加入新項目**對話方塊中，選取**ADO.NET 實體資料模型**。  
  
3.  資料模型的名稱，輸入`Northwind.edmx`。  
  
4.  在 實體資料模型精靈中，選取 **從資料庫產生**，然後按一下 **下一步**。  
...
1.  在**方案總管 中**，以滑鼠右鍵按一下您的 ASP.NET 專案的名稱，然後按一下**加入新項目**。  
  
2.  在**加入新項目**對話方塊中，選取**ADO.NET Data Services**。  
  
3.  服務的名稱，輸入`Northwind`。  
  
     Visual Studio 會針對新的服務建立 XML 標記和程式碼檔案。 根據預設，程式碼編輯器視窗隨即開啟。 在**方案總管 中**，服務將會具有名稱 Northwind，副檔名。 為.svc.cs 或.svc.vb。  
