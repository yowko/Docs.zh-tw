...
WCF Data Services 會將實體資料公開為數據服務。 反映提供者可讓您定義以任何類別為基礎的資料模型，其會公開會傳回 <xref:System.Linq.IQueryable%601> 執行的成員。 若要可以更新資料來源的資料，這些類別也必須實作 <xref:System.Data.Services.IUpdatable> 介面。 如需詳細資訊，請參閱[資料服務提供者](data-services-providers-wcf-data-services.md)。 本主題說明如何使用反映提供者，建立存取 Northwind 範例資料庫的 LINQ to SQL 類別以及如何根據這些資料類別，建立資料服務。

## <a name="to-add-linq-to-sql-classes-to-a-project"></a>將 LINQ to SQL 類別加入至專案

1. C#從 Visual Basic 或應用程式中，按一下 [**專案**] 功能表上的 [**加入** > **新專案**]。
...
5. 在 **伺服器**/**資料庫總管**的 Northwind 底下，展開 **資料表** ，然後將 `Customers` 資料表拖曳至物件關聯式設計工具（O/R 設計工具）。
