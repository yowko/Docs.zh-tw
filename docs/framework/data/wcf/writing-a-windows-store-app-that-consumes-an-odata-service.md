---
title: "撰寫使用 OData 服務的 Windows 市集應用程式 | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
...
---
# 撰寫使用 OData 服務的 Windows 市集應用程式
Windows 8 推出新的應用程式類型：Windows 市集應用程式。  Windows 市集應用程式具有全新的外觀及操作，可在多種裝置上執行，並透過 Windows 市集提供。  本主題說明如何撰寫使用 OData 服務 \(尤其是 NetFlix Catalog OData 服務\) 的 Windows 市集應用程式。  如需 Windows 市集應用程式的詳細資訊，請參閱 [Windows 市集應用程式入門](http://msdn.microsoft.com/library/windows/apps/br211386.aspx)。  
...
3.  在 App.xaml 檔案的 \<AppName\> 項目中輸入易記名稱。  這會設定應用程式啟動時所顯示的應用程式名稱：  
...
4.  建置並啟動應用程式。  請先參閱應用程式的啟動顯示畫面。  下面的螢幕擷取畫面顯示預設啟動顯示畫面。  使用的影像儲存在專案的 Assets 資料夾中。  
     ...
     ![預設應用程式](../../../../docs/framework/data/wcf/media/defaultapplication.png "DefaultApplication")  
...
2.  在位址列中輸入 Netflix OData 服務的 URL，然後按一下 \[移至\]。  將服務參考的命名空間設為 Netflix，然後按一下 \[確定\]。  
    ...
    > [!NOTE]
    >  如果您尚未安裝[適用於 Windows 市集應用程式的 WCF Data Services 工具](http://go.microsoft.com/fwlink/p/?LinkId=266652)，您會看到以上所示的訊息提示。  您必須下載並安裝連結中所參考的工具，再繼續進行。  
...
1.  將 SampleDataSource.cs 中的現有程式碼替換成 [Gist](https://gist.github.com/3419288) 中的程式碼。  更新的程式碼會加入 LoadMovies 方法 \(至 SampleDataSource 類別\)，以針對 Netflix OData 服務執行查詢，並填入內容類型 \(allGroups\) 清單及每種內容類型的影片清單。  SampleDataGroup 類別用於表示內容類型，而 SampleDataItem 類別用於表示影片。  
    ...
    ```  
     ...
     使用[以工作為基礎的非同步模式](http://go.microsoft.com/fwlink/p/?LinkId=266651) \(TAP\)，以非同步方式從 Netflix 取得 300 部 \(Take\) 最近的 \(OrderByDescending\) PG 分級 \(Where\) 影片。  程式碼的其餘部分會從 OData 摘要傳回的實體，建構 SimpleDataItems 和 SimpleDataGroups。  
...
3.  在 SearchResultsPage.xaml.cs 的第 81 行插入下列兩行程式碼，以擷取搜尋結果。  
 ...
 當使用者叫用 Windows 搜尋時，輸入搜尋字詞，然後觸碰搜尋列中的 Netflix Demo 應用程式圖示，即會執行 SearchResultsPage 的 LoadState 方法。  傳送至 LoadState 的巡覽參數包含查詢文字。  接著呼叫 Filter\_SelectionChanged 方法，再呼叫 SampleDataSource 類別上的 Search 方法。  所傳回的結果會顯示在 SearchResultsPage.xaml 頁面上。  
...
```  
 ...
 如需將搜尋整合至應用程式的詳細資訊，請參閱[搜尋：整合至 Windows 8 搜尋體驗中](http://go.microsoft.com/fwlink/p/?LinkId=266650)。  
  
## 執行應用程式  
 按 F5 鍵啟動應用程式。  請注意，應用程式啟動之後，需要幾秒鐘的時間載入影像。  此外，您的第一個搜尋嘗試可能不會傳回任何結果。  在實際的應用程式中，您會想要處理這兩個問題。  
  
 應用程式會呼叫 Netflix OData 服務，接收所產生之 OData 用戶端類別中的資料，然後將該資料傳送至可繫結的資料類別 \(SampleDataSource、SampleDataGroup 和 SampleDataItem\)。  接著使用這些可繫結的類別，將資料繫結至 GridView。  如果您對 XAML 資料繫結的運作方式不熟悉，請參閱[如何在清單或格線中分組項目 \(使用 C\#\/VB\/C\+\+ 和 XAML 的 Windows 市集應用程式\)](http://msdn.microsoft.com/library/windows/apps/xaml/hh780627)。