...
ms.locfileid: "75346203"
---
# <a name="application-scenarios-wcf-data-services"></a>應用程式案例 (WCF 資料服務)

WCF Data Services 支援一組核心案例，以開放式資料通訊協定（OData）摘要的形式公開和使用資料。 本主題會為您指出與這些案例有關的主題。
...
在以 .NET Framework 為基礎的用戶端應用程式中取用 OData 摘要。
...
在 Silverlight 架構的用戶端應用程式中取用 OData 摘要。
...
- [如何：將資料服務資料系結至控制項（Silverlight 用戶端）](https://docs.microsoft.com/previous-versions/dotnet/wcf-data-services/ee681614(v=vs.103))

在以 AJAX 為基礎的用戶端應用程式中取用 OData 摘要。
...
建立端對端資料解決方案，以使用 OData 在用戶端和伺服器之間傳輸資料。
...
建立以非同步方式取用 OData 摘要的以 .NET Framework 為基礎的用戶端應用程式，以避免用戶端上的延遲問題。
