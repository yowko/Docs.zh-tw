...
## <a name="remarks"></a>備註
 從 .NET Framework 4.6 開始，設定檔中的 `<AppContextSwitchOverrides>` 元素可讓 API 的呼叫者判斷其應用程式是否可以利用新功能，或保留與舊版程式庫的相容性。 例如，如果 API 的行為在兩個版本的程式庫之間已變更，`<AppContextSwitchOverrides>` 元素可讓該 API 的呼叫者選擇不支援新功能之程式庫版本的新行為。 針對在 .NET Framework 中呼叫 Api 的應用程式，如果應用程式是在包含該功能的 .NET Framework 版本上執行，則 `<AppContextSwitchOverrides>` 元素也可以允許其應用程式以舊版 .NET Framework 為目標的呼叫端選擇新功能。
...
|`Switch.System.Windows.Controls.Text.`<br/>`UseAdornerForTextboxSelectionRendering`|判斷非以裝飾項為基礎的選取範圍呈現是否可供 <xref:System.Windows.Controls.TextBox> 和 <xref:System.Windows.Controls.PasswordBox> 控制項使用，以防止 pixels occluded 文字（`false`），或文字是否只在裝飾項層（`true`）中轉譯。|.NET Framework 4.7.2|
|`Switch.System.Windows.Data.Binding.`<br/>`IListIndexerHidesCustomIndexer`|控制 <xref:System.Windows.Data.Binding?displayProperty=nameWithType> 類別是否不正確地使用自訂 IList 索引子（`true`）或正確（`false`）。|.NET Framework 4.8|
