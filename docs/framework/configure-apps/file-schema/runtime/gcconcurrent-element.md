---
title: "&lt;gcConcurrent&gt; 項目 | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
...
> [!WARNING]
>  從 .NET Framework 4 開始，背景記憶體回收就取代了並行記憶體回收。  *「並行」*\(concurrent\) 和*「背景」*\(background\) 等詞彙在 .NET Framework 文件中會交換使用。  若要停用背景記憶體回收，請使用 `<gcConcurrent>` 項目，如本文所述。  
...
```  
 ...
 如需並行和背景記憶體回收的詳細資訊，請參閱[Fundamentals of Garbage Collection](../../../../../docs/standard/garbage-collection/fundamentals.md)主題的＜並行記憶體回收＞一節。  
