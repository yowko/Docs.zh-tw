...
|屬性|描述|  
|---------------|-----------------|  
|type|要移除之 token 處理常式的 CLR 型別名稱。 如需如何指定屬性的詳細資訊 `type` ，請參閱 [自訂型別參考](/previous-versions/windows-identity-foundation/gg638728(v=msdn.10)#custom-type-references)。 必要。|  
