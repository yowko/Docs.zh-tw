...
## <a name="remarks"></a>備註  
 ...
 追蹤設定檔會結構化成追蹤記錄的宣告式訂閱，可讓您查詢特定追蹤記錄的工作流程執行階段。 有少數的查詢類型可讓您訂閱不同類別的 <xref:System.Activities.Tracking.TrackingRecord> 物件。 如需查詢的完整清單，請參閱 [\<participants>](participants.md) 和 [追蹤設定檔](../../../windows-workflow-foundation/tracking-profiles.md)。  
