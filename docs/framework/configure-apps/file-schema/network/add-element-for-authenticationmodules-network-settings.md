...
|**Attribute**|**說明**|  
|-------------------|---------------------|  
|`type`|完整的型別名稱 (由屬性指定 <xref:System.Type.FullName%2A>) 和屬性) 所指示的元件名稱 (<xref:System.Reflection.Assembly.FullName%2A> （以逗號分隔）。|  
...
## <a name="remarks"></a>備註  
 ...
 `add` 項目會將驗證模組加入至已註冊驗證模組清單的尾端。 驗證模組會依新增至清單的順序來呼叫。  
