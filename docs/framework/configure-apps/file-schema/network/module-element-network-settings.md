...
|**屬性**|**描述**|  
|-------------------|---------------------|  
|`type`|完整型別名稱 (由<xref:System.Type.FullName%2A>屬性工作表示) 和元件名稱 ( <xref:System.Reflection.Assembly.FullName%2A>由屬性工作表示), 並以逗號分隔, 以執行 proxy。|  
...
## <a name="remarks"></a>備註  
 元素會註冊用來<xref:System.Net.IWebProxy>執行介面的 proxy 類別。 `module` 註冊 Proxy 類別之後，可以使用 `module` 透過支援的 Proxy 要求資訊。  
