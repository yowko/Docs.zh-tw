...
|[name](module-element-network-settings.md)|將新的 Proxy 模組加入至應用程式。|  
...
## <a name="remarks"></a>備註  
 ...
 如果[module](module-element-network-settings.md)元素指定非公用型別、型別不是衍生自 <xref:System.Net.IWebProxy> 類別、這個物件的無參數的函式發生例外狀況，或在抓取系統指定的預設 proxy 時發生例外狀況，就會擲回例外狀況（exception）。 例外狀況的 <xref:System.Exception.InnerException%2A> 屬性應該會有此錯誤的根本原因之詳細資訊。  
