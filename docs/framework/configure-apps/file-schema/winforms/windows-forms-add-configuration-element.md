...
提供藉由設定選擇加入設定一個預設`DpiAwareness`至`PerMonitorV2`通常適用於新的 Windows Forms 應用程式。 不過，您可以再退出個別的高 DPI 改進應用程式組態檔中加入對應的索引鍵。 比方說，若要利用所有動態 DPI 支援除了新 DPI 功能，您可以加入下列應用程式組態檔來：
