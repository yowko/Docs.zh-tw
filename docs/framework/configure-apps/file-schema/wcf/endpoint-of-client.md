...
|屬性|描述|  
|---------------|-----------------|  
|address|必要的字串屬性。<br /><br /> 指定端點的位址。 預設值是空字串。 位址必須是絕對 URI。|  
|behaviorConfiguration|字串，其中包含要用於產生端點之行為的行為名稱。 行為名稱必須在定義服務之處的範圍內。 預設值是空字串。|  
...
|kind|字串，這個字串會指定所套用之標準端點的型別。 類型必須在區段或 machine.config 中註冊 `<extensions>` 。如果未指定任何內容，則會建立通用通道端點。|  
