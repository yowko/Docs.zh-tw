...
|maxConcurrentInstances|正整數，限制同時在 <xref:System.ServiceModel.ServiceHost> 上執行的 <xref:System.ServiceModel.InstanceContext> 物件數目。  當限制之內的位置可供使用時，建立其他執行個體的要求便會進入佇列並完成。  預設值是 maxConcurrentSessions 和 MaxConcurrentCalls 的總和|  
...
## 範例  
 下列組態範例指定服務將同時呼叫上限限制為 2，且將並行執行個體上限限制為 10。  如需執行這個範例的詳細範例，請參閱[節流](../../../../../docs/framework/wcf/samples/throttling.md)。  
