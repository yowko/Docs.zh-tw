...
定義的訊息層級安全性設定 [\<netHttpBinding>](nethttpbinding.md) 。  
...
|值|描述|  
|-----------|-----------------|  
|UserName|-要求用戶端必須使用 UserName 認證向伺服器進行驗證。 此認證必須使用 <> 元素來指定 `clientCredentials` 。<br />-WCF 不支援傳送密碼摘要或使用密碼衍生金鑰，以及使用這類金鑰來取得訊息安全性。 因此，在使用使用者名稱認證時，WCF 會強制保護傳輸。 對於 `basicHttpBinding`，這需要設定 SSL 通道。|  
|憑證|需要使用憑證對伺服器驗證用戶端。 此案例中的用戶端認證必須使用 <`clientCredentials`> 和 <> 來指定 `clientCertificate` 。 此外，當使用訊息安全性模式時，必須提供服務憑證給用戶端。 此案例中的服務認證必須使用 <xref:System.ServiceModel.Description.ClientCredentials> 類別或 `ClientCredentials` 行為元素來指定，並使用 serviceCredentials 的元素來指定服務憑證 \<serviceCertificate> 。|  
...
|元素|描述|  
|-------------|-----------------|  
|<`security`<的> 元素`netHttpBinding`>|定義 <> 元素的安全性功能 `netHttpBinding` 。|  
