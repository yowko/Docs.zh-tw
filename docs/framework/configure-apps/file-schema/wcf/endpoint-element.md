...
---
# <a name="endpoint-element"></a>\<endpoint> 項目
...
|屬性|描述|  
|---------------|-----------------|  
|address|包含端點位址的字串。 位址可以指定為絕對或相對位址。 如果提供相對位址，主機必須為繫結中使用的傳輸配置提供適當的基底位址。 如果沒有設定位址，會將基底位址假設為該端點的位址。<br /><br /> 預設值是空字串。|  
...
|kind|字串，這個字串會指定所套用之標準端點的型別。 類型必須在區段或 machine.config 中註冊 `<extensions>` 。如果未指定任何內容，則會建立一般服務端點。|  
...
- [HOW TO：在組態中建立服務端點](../../../wcf/feature-details/how-to-create-a-service-endpoint-in-configuration.md)
