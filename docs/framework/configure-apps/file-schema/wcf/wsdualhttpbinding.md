...
|clientBaseAddress|URI，設定用戶端在上面接聽以接收來自服務之回應訊息的基底位址。 如果指定，便會使用這個位址 (再加上每個通道的 GUID) 來接聽。 如果未指定值，則會以傳輸特定的方式來產生用戶端基底位址。 預設值為 `null`。|  
...
|元素|描述|  
|-------------|-----------------|  
|[\<security>](security-of-wsdualhttpbinding.md)|定義繫結的安全性設定。 此項目的型別為 <xref:System.ServiceModel.Configuration.WSDualHttpSecurityElement>。|  
