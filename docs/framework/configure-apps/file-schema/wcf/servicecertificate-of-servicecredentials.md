...
|`storeName`|指定要開啟之 X.509 憑證存放區的名稱。 有效值如下：<br /><br /> -通訊錄：其他使用者的憑證存放區。<br />-AuthRoot：協力廠商憑證授權單位單位（Ca）的憑證存放區。<br />-CertificatAuthority：中繼憑證授權單位單位（Ca）的憑證存放區。<br />-不允許：已撤銷憑證的憑證存放區。<br />-My：個人憑證的憑證存放區。<br />-Root：信任的根憑證授權單位（Ca）的憑證存放區。<br />-TrustedPeople：直接信任之人員和資源的憑證存放區。<br />-TrustedPublisher：直接信任之發行者的憑證存放區。<br /><br /> 預設為 My。|  
