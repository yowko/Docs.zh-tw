...
這個組態區段包含授權原則型別的集合，使用 `add` 關鍵字可加入這些型別。 每個授權原則包含一個必要的 `policyType` 屬性字串。 該屬性會指定授權原則，可讓一組輸入宣告轉換成另一組宣告。 它可以做為授與或拒絕存取控制 (Access Control) 的基礎。 如需授權原則運作方式的詳細資訊, 請<xref:System.IdentityModel.Policy.IAuthorizationPolicy>參閱和[授權原則](../../../wcf/samples/authorization-policy.md)。  
