...
## <a name="remarks"></a>備註  
 ...
 如果傳送至屬性中所列的主要端點 `endpointName` [\<add>](add-of-entries.md) 因為發生通訊例外狀況而失敗，則路由服務會嘗試將訊息傳送至這個設定區段中的第一個端點。 如果這項作業同樣因為通訊例外狀況而失敗，則路由服務將嘗試傳送訊息至這個區段中包含的下一個訊息，直到嘗試成功、傳回通訊例外狀況以外的失敗，或是集合中所有端點都傳回失敗為止。  
