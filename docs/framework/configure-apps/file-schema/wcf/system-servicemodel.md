...
此設定區段包含 WCF) 配置元素 (的所有 Windows Communication Foundation。  
...
|[\<commonBehaviors>](commonbehaviors.md)|這個區段只能定義在 machine.config 檔中。 它會定義兩個名為 `endpointBehaviors` 和 `serviceBehaviors` 的子集合。  每個集合會分別定義電腦上所有 WCF 端點和服務所使用的行為元素。  如果 `<commonBehaviors>` 和 `<behaviors>` 區段中都有定義某個行為，則會優先使用 \<behaviors> 區段中的行為。|  
|[\<diagnostics>](diagnostics.md)|這個區段包含 WCF 之診斷功能的設定。 使用者可以啟用/停用追蹤、效能計數器和 WMI 提供者，並且可以新增自訂訊息篩選條件。|  
|[\<extensions>](extensions-section.md)|這個區段包含延伸的集合，可讓使用者建立使用者定義的繫結程序、行為和其他方面的延伸。|  
|[\<protocolMapping>](protocolmapping.md)|這個區段會定義傳輸通訊協定配置 (例如 http、net.tcp、net.pipe 等) 和 WCF 繫結之間的一組預設通訊協定對應。|  
|[\<routing>](routing.md)|這個區段會定義一組路由篩選準則，這些篩選準則會決定評估內送訊息時所要使用的 Windows Communication Foundation (WCF) 的類型 <xref:System.ServiceModel.Dispatcher.MessageFilter> ，以及定義篩選準則符合時要傳送訊息的目標端點的路由表。|  
...
|[\<tracking>](tracking-of-wcf.md)|此區段會定義工作流程服務的追蹤設定。|
...
## <a name="remarks"></a>備註  
 ...
 WCF 服務是在 `services` 設定檔的區段中定義。 組件可包含任何數目的服務。 各服務都有自己的 `service` 組態區段。 這個區段及其內容會定義特定服務的服務合約、行為和端點。  
