...
|屬性|描述|  
|---------------|-----------------|  
|模式|選擇性. 指定套用的安全性類型。 預設為 `Message`。<br />-這個屬性的類型是 <xref:System.ServiceModel.SecurityMode>。|  
...
|-----------|-----------------|  
|None|停用安全性。|  
|Transport|系統會使用 HTTPS 來提供安全性。 而服務必須使用 SSL 憑證來設定。 用戶端使用 HTTPS 來完全保護訊息，並使用服務的 SSL 憑證來驗證訊息。 用戶端驗證是透過 `ClientCredentials` 屬性 [\<傳輸 >](transport-of-wshttpbinding.md)的。|  
...
|項目|描述|  
|-------------|-----------------|  
|[\<傳輸 >](transport-of-wshttpbinding.md)|定義傳輸安全性設定。 這個項目對應至 <xref:System.ServiceModel.Configuration.HttpTransportSecurityElement> 型別。|  
