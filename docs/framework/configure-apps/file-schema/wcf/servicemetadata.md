...
|httpGetEnabled|布林值，指定是否發行服務中繼資料以使用 HTTP/Get 要求進行擷取。 預設值為 `false`。<br /><br /> 如果未指定 httpGetUrl 屬性，則中繼資料的發行位址會是服務位址加上 "?wsdl"。 例如，如果服務位址為 `http://localhost:8080/CalculatorService` ，則 HTTP/Get 中繼資料位址為 `http://localhost:8080/CalculatorService?wsdl` 。<br /><br /> 如果這個屬性為 `false` ，或服務的位址不是以 HTTP 或 HTTPS 為基礎，則會忽略 "？ wsdl"。|  
...
|httpsGetEnabled|布林值，指定是否發行服務中繼資料以使用 HTTPS/Get 要求進行擷取。 預設值為 `false`。<br /><br /> 如果未指定 httpsGetUrl 屬性，則中繼資料的發行位址會是服務位址加上 "?wsdl"。 例如，如果服務位址是 " https://localhost:8080/CalculatorService "，則 HTTP/Get 中繼資料位址為 " https://localhost:8080/CalculatorService?wsdl "。<br /><br /> 如果這個屬性為 `false` ，或服務的位址不是以 HTTP 或 HTTPS 為基礎，則會忽略 "？ wsdl"。|  
...
## <a name="remarks"></a>備註  
 ...
 這個組態項目可讓您控制服務的中繼資料發行功能。 為了避免意外洩漏潛在的機密服務中繼資料，Windows Communication Foundation (WCF) 服務的預設設定會停用中繼資料發行。 這個行為依預設為安全行為，但也表示您無法使用中繼資料匯入工具 (例如 Svcutil.exe) 來產生呼叫服務所需的用戶端程式碼，除非組態中已明確啟用服務的中繼發行行為。 使用這個組態項目，您就可以為服務啟用此發行行為。  
 ...
 若要降低將服務暴露給惡意使用者的機會，可以使用 SSL over HTTP (HTTPS) 機制來進行安全的傳輸。 若要這麼做，您必須先將適當的 X.509 憑證，繫結至裝載服務之電腦上的特定連接埠   (需詳細資訊，請參閱 [使用憑證](../../../wcf/feature-details/working-with-certificates.md)。 ) 秒，將此專案新增至服務設定，並將 `httpsGetEnabled` 屬性設定 `true` 為。 最後，將 `httpsGetUrl` 屬性設定為服務中繼資料端點的 URL，如下列範例所示。  
...
## <a name="example"></a>範例  
 ...
 下列範例會使用元素設定服務，以公開中繼資料 \<serviceMetadata> 。 它也設定了一個端點，將 `IMetadataExchange` 合約公開做為 WS-MetadataExchange (MEX) 通訊協定的實作。 此範例使用 `mexHttpBinding`，這個方便的標準繫結，相當於將安全性模式設定為 `wsHttpBinding` 的 `None`。 端點中會使用 "mex" 的相對位址，當針對服務基底位址進行解析時，會產生的端點位址 `http://localhost/servicemodelsamples/service.svc/mex` 。  
