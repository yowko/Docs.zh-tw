...
|`leaseTimeout`|<xref:System.TimeSpan>，指定作用中連線關閉前的時間。 預設為 00:05:00。<br /><br /> 連線會在傳回至連線快取時關閉，而不是在傳輸作用期間關閉。 使用 TCP 傳輸的連線快取建立所需的每個端點，最大快取限制所設定的新連線 `maxOutboundConnectionsPerEndpoint.`|  
