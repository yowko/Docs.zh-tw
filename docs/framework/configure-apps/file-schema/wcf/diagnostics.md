...
|performanceCounters|指定是否啟用組件的效能計數器。 有效值為<br /><br /> -Off：效能計數器已停用。<br />-ServiceOnly：僅啟用與此服務相關的效能計數器。<br />-All：效能計數器可以在執行時間查看。<br />-Default：已建立單一效能計數器實例 _WCF_Admin。 這個執行個體用於啟用基礎結構所使用之 SQM 資料的集合。 這個執行個體所有的計數器值都未更新，因此將維持在零。 如果沒有 WCF 的組態，則這是預設值。|  
