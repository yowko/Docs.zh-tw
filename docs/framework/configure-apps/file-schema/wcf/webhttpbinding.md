...
|maxReceivedMessageSize|正整數，指定在使用此繫結設定之通道上可以接收的訊息大小上限 (以位元組為單位，包括標頭)。 超出此限制之訊息的寄件者將會收到錯誤。 收件者會捨棄訊息，然後在追蹤記錄檔中建立此事件的項目。 預設值為 65536。 **注意：** 單獨增加這個值並不足以 ASP.NET 相容模式。 您也應該增加的值 `httpRuntime` （請參閱[HTTPRuntime 元素（ASP.NET 設定架構）](https://docs.microsoft.com/previous-versions/dotnet/netframework-4.0/e1f13641(v=vs.100))）。|  
...
## <a name="remarks"></a>備註  
 WCF Web 程式設計模型可讓開發人員透過使用「單純舊 XML」（POX）樣式訊息的 HTTP 要求來公開 WCF Web 服務，而不是以 SOAP 為基礎的訊息。 若要讓用戶端使用 HTTP 要求與服務進行通訊，必須使用已附加的來設定服務的端點 [\<webHttpBinding>](webhttpbinding.md) \<WebHttpBehavior> 。  
