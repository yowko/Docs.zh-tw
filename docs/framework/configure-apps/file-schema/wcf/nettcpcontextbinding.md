...
|contextProtectionLevel|有效的 <xref:System.Net.Security.ProtectionLevel> 值，此值指定 SOAP 標頭所要的保護層級 (此標頭用來傳播內容資訊)。  預設值為 <xref:System.Net.Security.ProtectionLevel.Sign>。|  
