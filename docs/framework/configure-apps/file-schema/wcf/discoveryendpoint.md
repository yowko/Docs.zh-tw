...
| 屬性        | 說明 |  
| ---------------- | ----------- |  
| discoveryMode    | 字串，這個字串會指定探索通訊協定的模式。 有效的值為「臨機操作」和「受控」。 在 Managed 模式中，通訊協定會依賴探索 Proxy，此 Proxy 的作用是可探索之服務的儲存機制。 Adhoc 模式會要求通訊協定使用 UDP 多點傳送機制尋找可用的服務。 如需屬性的詳細資訊, 請<xref:System.ServiceModel.Discovery.DiscoveryEndpoint.DiscoveryMode%2A>參閱。 |  
