...
|trustedStoreLocation|兩個系統存放位置的其中一個：`LocalMachine` 或 `CurrentUser`。 當與用戶端交涉服務憑證時，會使用這個值。 會針對指定存放區位置中的「**受信任的人**」存放區執行驗證。 預設值為 `CurrentUser`。|  
...
## <a name="example"></a>範例  
 下列範例會執行兩個工作。 它會先指定服務憑證，讓用戶端在與其功能變數名稱是透過 HTTP 通訊協定的端點進行通訊時使用 `www.contoso.com` 。 第二，指定驗證時使用的撤銷模式和存放區位置。  
