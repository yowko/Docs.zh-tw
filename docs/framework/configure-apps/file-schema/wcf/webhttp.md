...
---
# \<webHttp>
此項目透過組態指定端點上的 <xref:System.ServiceModel.Description.WebHttpBehavior>。 這個行為與標準系結搭配使用時 [\<webHttpBinding>](webhttpbinding.md) ，會啟用 Windows Communication Foundation （WCF）服務的 Web 程式設計模型。  
