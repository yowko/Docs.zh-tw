...
|queueTransferProtocol|指定此繫結使用的佇列通訊通道傳輸。 有效值為<br /><br /> 自有使用原生 MSMQ 通訊協定。<br />Srmp使用 Soap Reliable Messaging Protocol (SRMP)。<br />- SrmpSecure:使用 Soap Reliable Messaging Protocol Secure (SRMPS) 傳輸。<br /><br /> 此屬性的型別為 <xref:System.ServiceModel.QueueTransferProtocol>。<br /><br /> 因為 MSMQ 在使用 SOAP 可靠訊息通訊協定時不支援 Active Directory 定址, 所以當設定為`useActiveDirectory` `true`時, 您不應該將此屬性設定為 Srmp 或 Srmps。|  
