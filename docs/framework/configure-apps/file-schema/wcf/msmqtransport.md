...
|deadLetterQueue|指定要使用的寄不出的信件佇列類型。<br /><br /> 有效值包括：<br /><br /> -Custom： 自訂無效信件佇列。<br />-無： 無法傳送信件佇列不是使用。<br />系統： 使用系統無效信件佇列。<br /><br /> 此屬性的型別為 DeadLetterQueue。|  
...
|queueTransferProtocol|指定此繫結使用的佇列通訊通道傳輸。 有效值為<br /><br /> 原生： 使用原生 MSMQ 通訊協定。<br />-Srmp： 使用 Soap Reliable Messaging Protocol (SRMP)。<br />-SrmpSecure： 使用 Soap Reliable Messaging 通訊協定 Secure (SRMPS) 傳輸。<br /><br /> 此屬性的型別為 <xref:System.ServiceModel.QueueTransferProtocol>。<br /><br /> 因為 MSMQ 不支援 Active Directory 定址使用 SOAP Reliable Messaging Protocol 時，您不應將此屬性設為 Srmp 或 Srmps 時`u``seActiveDirectory`設`true`。|  
