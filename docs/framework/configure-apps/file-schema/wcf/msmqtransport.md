...
|queueTransferProtocol|指定此繫結使用的佇列通訊通道傳輸。 有效值為<br /><br /> -Native：使用原生 MSMQ 通訊協定。<br />-Srmp：使用 Soap 可靠訊息通訊協定（SRMP）。<br />-SrmpSecure：使用 Soap 可靠訊息通訊協定安全（SRMPS）傳輸。<br /><br /> 此屬性的型別為 <xref:System.ServiceModel.QueueTransferProtocol>。<br /><br /> 因為 MSMQ 在使用 SOAP 可靠訊息通訊協定時不支援 Active Directory 定址，所以當 `useActiveDirectory` 設定為 `true`時，您不應該將此屬性設定為 Srmp 或 Srmps。|  
...
|項目|描述|  
|-------------|-----------------|  
|[\<Msmqtransportsecurity.msmqauthenticationmode >](msmqtransportsecurity.md)|指定此繫結的傳輸安全性設定。 此項目的型別為 <xref:System.ServiceModel.Configuration.MsmqTransportSecurityElement>。|  
