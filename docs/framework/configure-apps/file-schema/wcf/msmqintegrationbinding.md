...
|receiveContextEnabled|布林值，這個值會指定接收在佇列內處理訊息的內容是否已啟用。 當這個設定為時 `true` ，服務可以「查看」佇列上的訊息來開始處理它，如果發生任何錯誤且擲回例外狀況，則會保留在佇列中。 服務也可以「鎖定」訊息，以便在稍後的時間點重試處理。 ReceiveCoNtext 會提供一種機制，以在處理訊息之後「完成」訊息，以便從佇列中移除訊息。訊息不會再由網路讀取和重新寫入至佇列，而個別訊息在處理期間不會在不同的服務實例間跳動。|  
...
## <a name="remarks"></a>備註  
 ...
 這個繫結項目可用來啟用 Windows Communication Foundation (WCF) 應用程式，以使用 COM、MSMQ 原生 Api 或命名空間中定義的類型，從現有的 MSMQ 應用程式傳送和接收訊息 <xref:System.Messaging?displayProperty=nameWithType> 。您可以使用此設定專案來指定定址佇列的方式、傳輸保證、是否必須永久儲存訊息，以及如何保護和驗證訊息。 如需詳細資訊，請參閱 [如何：與 WCF 端點和訊息佇列應用程式交換訊息](../../../wcf/feature-details/how-to-exchange-messages-with-wcf-endpoints-and-message-queuing-applications.md)。  
