...
ms.locfileid: "73738889"
---
# <a name="msmqintegration"></a>\<Msmqintegration 繫結項目 >
指定自訂繫結的 MSMQ 傳輸。  
...
|serializationFormat|指定用來序列化物件的格式器，這些物件會當做部分 MSMQ 訊息傳送。 有效值為<br /><br /> -ActiveX：序列化 COM 物件時，會使用 ActiveX 格式器。<br />-Binary：將物件序列化為二進位封包。<br />-ByteArray：將物件序列化為位元組陣列。<br />-Stream：將物件序列化為數據流。<br />-Xml：將物件序列化為 XML 封包。 預設為 XML。<br /><br /> 此屬性的型別為 <xref:System.ServiceModel.MsmqIntegration.MsmqMessageSerializationFormat>。|  
