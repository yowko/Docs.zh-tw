...
|serializationFormat|指定用來序列化物件的格式器，這些物件會當做部分 MSMQ 訊息傳送。 有效值為<br /><br /> -ActiveX︰ 序列化 COM 物件時，會使用 ActiveX 格式器。<br />二進位： 將物件序列化為二進位封包。<br />-ByteArray︰ 將物件序列化為位元組陣列。<br />資料流： 序列化資料流的物件。<br />Xml: 將物件序列化為 XML 封包。 預設為 XML。<br /><br /> 此屬性的型別為 <xref:System.ServiceModel.MsmqIntegration.MsmqMessageSerializationFormat>。|  
