...
## <a name="remarks"></a>備註  
 ...
 `<clear>`元素會從集合中移除追蹤來源的所有接聽 `Listeners` 程式，包括 <xref:System.Diagnostics.DefaultTraceListener> 。 您可以使用專案， `<clear>` 然後再使用 `<add>` 元素來確定集合中沒有其他使用中的接聽程式。  
...
## <a name="example"></a>範例  
 ...
 下列範例示範如何使用專案， `<clear>` 再使用專案將接聽 `<add>` `console` `textListener` 程式和追蹤來源的集合加入至 `Listeners` 集合 `TraceSourceApp` 。  
