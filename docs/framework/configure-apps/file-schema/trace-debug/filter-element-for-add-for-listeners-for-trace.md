...
---
# <a name="filter-element-for-add-for-listeners-for-trace"></a>\<filter>\<add>的元素 \<listeners>\<trace>

將篩選加入至追蹤集合中的接聽 `Listeners` 程式。  
...
## <a name="remarks"></a>備註  
 ...
 專案 `<filter>` 必須包含在指定接聽程式 `<add>` 類型之追蹤接聽項的元素中，而不只是在中定義之接聽程式的名稱 [\<sharedListeners>](sharedlisteners-element.md) 。 如果接聽程式是在中定義 [\<sharedListeners>](sharedlisteners-element.md) ，則必須在該元素中定義該接聽程式的篩選準則。  
  
 此元素可用於電腦設定檔 ( # A0) 和應用程式佈建檔。  
  
## <a name="example"></a>範例  

 下列範例示範如何使用專案 `<filter>` ，將篩選準則加入至 `console` 追蹤集合中的接聽 `Listeners` 程式，並將篩選事件層級指定為 `Error` 。  
