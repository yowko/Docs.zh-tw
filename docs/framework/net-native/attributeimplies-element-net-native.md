---
title: "&lt;AttributeImplies&gt; 項目 (.NET Native) | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
...
---
# &lt;AttributeImplies&gt; 項目 (.NET Native)
定義套用包含屬性之程式碼元素的原則。  
...
## <a name="remarks"></a>備註  
 `<AttributeImplies>`如果其包含型別屬性，就會使用項目 (也就是一個類別衍生自<xref:System.Attribute?displayProperty=fullName>)。 如果屬性套用至特定的程式元素，則 `<AttributeImplies>` 元素所定義的原則會套用至該程式元素。  
