...
- <xref:System.Type.MakeArrayType%2A?displayProperty=nameWithType><xref:System.Type.MakeByRefType%2A?displayProperty=nameWithType>當您嘗試建立特定組合（例如，物件的陣列）時，並不會失敗 `byref` 。

- 您不能使用反映來叫用具有指標參數的成員。

- 您不能使用反映來取得或設定指標欄位。

- 當引數計數錯誤，且其中一個引數的類型不正確時，.NET Native 會擲回， <xref:System.ArgumentException> 而不是 <xref:System.Reflection.TargetParameterCountException> 。
...
- [TypeInfo.GUID](xref:System.Type.GUID) <xref:System.PlatformNotSupportedException> 如果屬性未套用至型別，則 TypeInfo 屬性會擲回例外狀況 <xref:System.Runtime.InteropServices.GuidAttribute> 。 GUID 主要用於 COM 支援。
...
在 .NET Native 中， <xref:System.Net.Http.HttpClientHandler> 類別會在內部使用 WinINet （透過 <xref:Windows.Web.Http.Filters.HttpBaseProtocolFilter> 類別），而不是在 <xref:System.Net.WebRequest> 適用于 <xref:System.Net.WebResponse> Windows Store 應用程式的標準 .net 中使用的和類別。  WinINet 並未支援 <xref:System.Net.Http.HttpClientHandler> 類別支援的所有組態選項。  因此：

- 上的一些功能屬性會在 .NET Native 上傳回 <xref:System.Net.Http.HttpClientHandler> `false` ，而它們會 `true` 在適用于 Windows Store 應用程式的標準 .net 中傳回。

- 某些 `get` 設定屬性存取子一律會傳回 .NET Native 上的固定值，這與適用于 Windows Store 應用程式的 .net 中預設的可設定值不同。

下列各小節會說明一些其他的行為差異。

**Proxy**

<xref:Windows.Web.Http.Filters.HttpBaseProtocolFilter>類別不支援根據要求來設定或覆寫 proxy。  這表示 .NET Native 上的所有要求都會使用系統設定的 proxy 伺服器或沒有 proxy 伺服器，端視屬性的值而定 <xref:System.Net.Http.HttpClientHandler.UseProxy%2A?displayProperty=nameWithType> 。  在適用於 Windows 市集應用程式的 .NET 中，是由 <xref:System.Net.Http.HttpClientHandler.Proxy%2A?displayProperty=nameWithType> 屬性來定義 Proxy 伺服器。  在 .NET Native 上，將設定為以外的值，則會擲回 <xref:System.Net.Http.HttpClientHandler.Proxy%2A?displayProperty=nameWithType> 例外狀況 `null` <xref:System.PlatformNotSupportedException> 。  <xref:System.Net.Http.HttpClientHandler.SupportsProxy%2A?displayProperty=nameWithType>屬性 `false` 會在 .NET Native 上傳回，而它會 `true` 在適用于 Windows Store 應用程式的標準 .NET Framework 中返回。

**自動重新導向**

<xref:Windows.Web.Http.Filters.HttpBaseProtocolFilter>類別不允許設定自動重新導向的數目上限。  在適用於 Windows 市集應用程式的標準 .NET 中， <xref:System.Net.Http.HttpClientHandler.MaxAutomaticRedirections%2A?displayProperty=nameWithType> 屬性的值預設為 50，而且可以修改。 在 .NET Native 上，這個屬性的值是10，而嘗試修改它會擲回 <xref:System.PlatformNotSupportedException> 例外狀況。  <xref:System.Net.Http.HttpClientHandler.SupportsRedirectConfiguration%2A?displayProperty=nameWithType>屬性 `false` 會在 .NET Native 上傳回，而 `true` 在適用于 Windows Store 應用程式的 .net 中則會傳回。

**自動解壓縮**

適用於 Windows 市集應用程式的 .NET 可讓您將 <xref:System.Net.Http.HttpClientHandler.AutomaticDecompression%2A?displayProperty=nameWithType> 屬性設為 <xref:System.Net.DecompressionMethods.Deflate>、 <xref:System.Net.DecompressionMethods.GZip>、 <xref:System.Net.DecompressionMethods.Deflate> 和 <xref:System.Net.DecompressionMethods.GZip>同時設定，或是設為 <xref:System.Net.DecompressionMethods.None>。  .NET Native 只支援 <xref:System.Net.DecompressionMethods.Deflate> 搭配 <xref:System.Net.DecompressionMethods.GZip> 、或 <xref:System.Net.DecompressionMethods.None> 。  嘗試以無訊息模式將 <xref:System.Net.Http.HttpClientHandler.AutomaticDecompression%2A> 屬性單獨設為 <xref:System.Net.DecompressionMethods.Deflate> 或 <xref:System.Net.DecompressionMethods.GZip> ，會將其設為同時使用 <xref:System.Net.DecompressionMethods.Deflate> 和 <xref:System.Net.DecompressionMethods.GZip>。
 ...
 **已被取代的 API**
 ...
 有些與 Managed 程式碼交互操作的不常用 API 已被取代。 與 .NET Native 搭配使用時，這些 Api 可能會擲回 <xref:System.NotImplementedException> 或 <xref:System.PlatformNotSupportedException> 例外狀況，或導致編譯器錯誤。 在適用於 Windows 市集應用程式的 .NET 中，這些 API 會標示為已過時，但是呼叫這些 API 會產生編譯器警告，而不是編譯器錯誤。
...
- <xref:System.Runtime.InteropServices.VarEnum?displayProperty=nameWithType>
 ...
 <xref:System.Runtime.InteropServices.UnmanagedType.Struct?displayProperty=nameWithType>支援，但在某些情況下會擲回例外狀況，例如搭配[IDispatch](https://docs.microsoft.com/previous-versions/windows/desktop/api/oaidl/nn-oaidl-idispatch)或 variant 使用時 `byref` 。
...
介面中的已淘汰 Api <xref:System.Runtime.InteropServices.ICustomQueryInterface?displayProperty=nameWithType> （.NET Native 不支援）包含：
...
- 透過 [屬性，在 Managed 類型上實作](https://docs.microsoft.com/previous-versions/windows/desktop/api/oaidl/nn-oaidl-idispatch) IDispatch <xref:System.Runtime.InteropServices.ComDefaultInterfaceAttribute?displayProperty=nameWithType> 介面。 不過，您無法透過呼叫 COM 物件 `IDispatch` ，而且您的 managed 物件無法執行 `IDispatch` 。
...
本節列出 .NET Native 中不支援的其餘 Api。 最大的一組不支援的 Api 是 Windows Communication Foundation （WCF） Api。

**DataAnnotations (System.ComponentModel.DataAnnotations)**

<xref:System.ComponentModel.DataAnnotations> <xref:System.ComponentModel.DataAnnotations.Schema> .NET Native 中不支援和命名空間中的類型。 其中包括下列在適用于 Windows 8 的 Windows Store 應用程式的 .NET 中出現的類型：
...
.NET Native 目前不支援 Visual Basic。 在 <xref:Microsoft.VisualBasic> .NET Native 中，無法使用和命名空間中的下列類型 <xref:Microsoft.VisualBasic.CompilerServices> ：
...
- 在 .NET Native 中， <xref:System.Runtime.Serialization.DataContractSerializer> 和 <xref:System.Runtime.Serialization.Json.DataContractJsonSerializer> 無法序列化或還原序列化具有基類成員（其類型不是根序列化類型）的衍生類別。 例如，在下列程式碼中，嘗試序列化或還原序列化 `Y` 會產生錯誤：
...
- 序列化一般 <xref:System.Exception> 物件無法與和搭配運作 <xref:System.Runtime.Serialization.DataContractSerializer> <xref:System.Runtime.Serialization.Json.DataContractJsonSerializer> 。
