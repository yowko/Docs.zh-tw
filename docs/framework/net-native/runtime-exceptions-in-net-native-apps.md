...
> [!IMPORTANT]
> 有關處理["缺失中繼資料異常](missingmetadataexception-class-net-native.md)"、"[缺少 InteropDataexception](missinginteropdataexception-class-net-native.md)"和["缺少運行時異常"等](missingruntimeartifactexception-class-net-native.md)有關您在測試應用發佈版本時可能遇到的異常的資訊，請參閱"步驟 4：手動解析丟失的中繼資料：[在入門](getting-started-with-net-native.md)主題中，以及[反射和 .NET 本機](reflection-and-net-native.md)和[運行時指令 （rd.xml） 設定檔引用](runtime-directives-rd-xml-configuration-file-reference.md)中。  
...
- 您的程式碼已徹底最佳化。 這代表會盡可能使用內嵌。 （內聯將代碼從外部常式移動到調用常式中。  .NET Native 提供專用運行時並實現主動內聯這一事實會影響調試時顯示的呼叫堆疊。  如需詳細資訊，請參閱 [Runtime call stack](#CallStack) 一節。  
