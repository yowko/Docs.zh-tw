...
- **問題：** 您不知道您的應用程式是使用 .NET Native 編譯。
  ...
  **解決方式：** 如果叫用 .NET Native 編譯器，您會發現組建時間較長，而且 [工作管理員] 會顯示各種 .NET Native 元件進程，例如 ILC.OUT .exe 和 nutc_driver。

  當您使用 .NET Native 成功建立專案之後，您會在 obj\\*config* *\ 架構*\\*專案名稱*ilc\out. 中找到輸出。 您可以在*bin\\的*\\*config*\appx 中找到最終的原生套件內容 如果已部署應用程式，則最終的原生套件內容位於 \bin\\*arch*\\*config*\AppX 下。
