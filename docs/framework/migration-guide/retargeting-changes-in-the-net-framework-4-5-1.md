...
---
# <a name="retargeting-changes-in-the-net-framework-451"></a>.NET Framework 4.5.1 中的重定目標變更
...
<a name="Core"></a>   
## <a name="core-retargeting-changes"></a>核心重定目標變更  
  
|功能|變更|影響|範圍|  
|-------------|------------|------------|-----------|  
|<xref:System.ObsoleteAttribute?displayProperty=fullName> 屬性|當您建立 Windows 中繼資料庫 (.winmd 檔案) 時，<xref:System.ObsoleteAttribute> 屬性會匯出為 <xref:System.ObsoleteAttribute> 和 [Windows.Foundation.DeprecatedAttribute](http://msdn.microsoft.com/library/windows/apps/windows.foundation.metadata.deprecatedattribute.aspx)。|重新編譯使用 <xref:System.ObsoleteAttribute> 屬性的現有原始程式碼，可能會在從 C++/CX 或 JavaScript 使用該程式碼時產生警告。<br /><br /> 不建議您同時將 <xref:System.ObsoleteAttribute> 和 [Windows.Foundation.DeprecatedAttribute](http://msdn.microsoft.com/library/windows/apps/windows.foundation.metadata.deprecatedattribute.aspx) 套用至 Managed 組件中的程式碼；這可能會導致建置警告。<br /><br /> 如需詳細資訊，請參閱 <xref:System.ObsoleteAttribute> 參考主題。|Edge|  
  
<a name="ADO"></a>   
## <a name="adonet-retargeting-changes"></a>ADO.NET 重定目標變更  
...
<a name="MSBuild"></a>   
## <a name="msbuild-retargeting-changes"></a>MSBuild 重定目標變更  
