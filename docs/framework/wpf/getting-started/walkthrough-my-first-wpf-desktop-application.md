...
在逐步解說結束時, 您將會建立獨立的 Windows 應用程式, 讓使用者可以查看所選人員的費用報表。 應用程式是由數個以瀏覽器樣式視窗主控的 WPF 頁面所組成。

> [!TIP]
> 用來建立這個逐步解說的範例程式碼適用于 Visual Basic 和C# [逐步解說 WPF 應用程式範例程式碼](https://github.com/Microsoft/WPF-Samples/tree/master/Getting%20Started/WalkthroughFirstWPFApp)。
>
> 您可以使用本文右上方的C# **\< />** 下拉式按鈕, 切換和 Visual Basic 之間的範例程式碼語言。
...
1. 將和`DesignHeight` `DesignWidth`元素的值設定為300圖元。
...
1. 將名為 [*浮水印 .png* ] 的影像加入至專案。 您可以建立自己的映射、從範例程式碼複製檔案, 或在[這裡](https://raw.githubusercontent.com/microsoft/WPF-Samples/master/Getting%20Started/WalkthroughFirstWPFApp/csharp/watermark.png)取得。
