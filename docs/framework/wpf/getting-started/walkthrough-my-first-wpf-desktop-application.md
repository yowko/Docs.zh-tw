...
> [!TIP]
> 用來建立這個逐步解說的範例程式碼是適用於 Visual Basic 和 C#[建置 WPF 應用程式簡介](http://go.microsoft.com/fwlink/?LinkID=160008)。
...
2. 新增<xref:System.Windows.Controls.Primitives.ButtonBase.Click>事件處理常式來<xref:System.Windows.Controls.Button>項目。 如需詳細資訊，請參閱 <<c0> [ 如何： 建立簡單的事件處理常式](http://msdn.microsoft.com/library/b1456e07-9dec-4354-99cf-18666b64f480)。
...
4. 將範本套用至<xref:System.Windows.Controls.DataGrid>資料行，顯示費用報表資料。
