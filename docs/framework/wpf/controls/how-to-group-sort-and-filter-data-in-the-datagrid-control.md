...
---
# <a name="how-to-group-sort-and-filter-data-in-the-datagrid-control"></a>如何： 分組、 排序和篩選的 DataGrid 控制項中的資料
...
1. 建立資料集合中實作<xref:System.Collections.IEnumerable>介面。
    ...
    > [!NOTE]
    > 集合中的物件必須實作<xref:System.ComponentModel.INotifyPropertyChanged>已變更的介面和<xref:System.ComponentModel.IEditableObject>介面讓<xref:System.Windows.Controls.DataGrid>來正確回應屬性變更及編輯。 如需詳細資訊，請參閱[實作屬性變更通知](../data/how-to-implement-property-change-notification.md)。
...
或者，您可以篩選中的項目<xref:System.Windows.Controls.DataGrid>藉由建立提供篩選的邏輯和設定的方法<xref:System.Windows.Data.CollectionView.Filter%2A?displayProperty=nameWithType>將篩選套用到屬性。 若要查看此方法的範例，請參閱[檢視中的篩選資料](../data/how-to-filter-data-in-a-view.md)。
