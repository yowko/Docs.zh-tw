...
---
# <a name="how-to-sort-a-gridview-column-when-a-header-is-clicked"></a>HOW TO：在按一下標頭時排序 GridView 資料行
...
[!INCLUDE[TLA2#tla_xaml](../../../../includes/tla2sharptla-xaml-md.md)] 標記中的 `s` 和 `p` 識別碼會參考 [!INCLUDE[TLA2#tla_xaml](../../../../includes/tla2sharptla-xaml-md.md)] 頁面之中繼資料中定義的命名空間對應。 下列範例示範中繼資料定義。
...
此範例會定義事件處理常式，讓排序方向在每次您按下資料行標頭按鈕時，在遞增順序與遞減順序之間做變更。 下列範例示範事件處理常式。
...
下列範例示範事件處理常式所呼叫來排序資料的排序演算法。 藉由建立新的執行排序<xref:System.ComponentModel.SortDescription>結構。
