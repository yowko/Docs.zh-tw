...
### <a name="bindings-dynamic-resources-and-event-handlers"></a>繫結、動態資源及事件處理常式  
 請注意，您可以使用 `Setter.Value` 屬性來指定 [Binding 標記延伸](../../../../docs/framework/wpf/advanced/binding-markup-extension.md)或 [DynamicResource 標記延伸](../../../../docs/framework/wpf/advanced/dynamicresource-markup-extension.md)。 如需詳細資訊，請參閱提供的範例<xref:System.Windows.Setter.Value%2A?displayProperty=nameWithType>屬性。  
