---
title: "如何：在實作 GridView 的 ListView 中設定資料列的樣式 | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
ms.prod: ".net-framework"
ms.reviewer: ""
ms.suite: ""
ms.technology: 
  - "dotnet-wpf"
ms.tgt_pltfrm: ""
ms.topic: "article"
helpviewer_keywords: 
  - "GridView 控制項，設定資料列樣式"
  - "在實作 GridView 的 ListView 中設定資料列樣式"
  - "設定使用 Gridview 的資料列樣式的 ListView 控制項"
ms.assetid: 2e406ba2-70a0-4e62-841f-0934859de76e
caps.latest.revision: 12
author: "dotnet-bot"
ms.author: "dotnetcontent"
manager: "wpickett"
caps.handback.revision: 12
---
# 如何：在實作 GridView 的 ListView 中設定資料列的樣式
這個範例示範如何設定樣式中的資料列<xref:System.Windows.Controls.ListView>實作控制項<xref:System.Windows.Controls.GridView><xref:System.Windows.Controls.ListView.View%2A>模式。  
  
## <a name="example"></a>範例  
 您可以設定樣式中的資料列<xref:System.Windows.Controls.ListView>藉由設定控制<xref:System.Windows.Controls.ItemsControl.ItemContainerStyle%2A>上<xref:System.Windows.Controls.ListView>控制項。 將會以表示其項目的樣式設定<xref:System.Windows.Controls.ListViewItem>物件。 <xref:System.Windows.Controls.ItemsControl.ItemContainerStyle%2A>參考<xref:System.Windows.Controls.ControlTemplate>用來顯示資料列內容的物件。  
  
 完整的範例中，下列範例摘錄自、 顯示歌曲資訊儲存在集合[!INCLUDE[TLA2#tla_xml](../../../../includes/tla2sharptla-xml-md.md)]資料庫。 在資料庫中的每首歌曲的評等欄位，而這個欄位的值會指定如何顯示歌曲資訊的資料列。  
  
 下列範例示範如何定義<xref:System.Windows.Controls.ItemsControl.ItemContainerStyle%2A>的<xref:System.Windows.Controls.ListViewItem>代表歌曲集合中的歌曲的物件。 <xref:System.Windows.Controls.ItemsControl.ItemContainerStyle%2A>參考<xref:System.Windows.Controls.ControlTemplate>物件，以指定如何顯示歌曲資訊的資料列。  
  
 [!code-xml[ListViewItemStyleSnippet#ItemContainerStyle](../../../../samples/snippets/csharp/VS_Snippets_Wpf/ListViewItemStyleSnippet/CS/Window1.xaml#itemcontainerstyle)]  
  
 下列範例所示<xref:System.Windows.Controls.ControlTemplate> ，將文字字串`"Strongly Recommended"`資料列。 此範本中參考<xref:System.Windows.Controls.ItemsControl.ItemContainerStyle%2A> ，並顯示當歌曲的評等的值為 5 (5)。 <xref:System.Windows.Controls.ControlTemplate>包含<xref:System.Windows.Controls.GridViewRowPresenter>配置所定義的資料行中的資料列內容的物件<xref:System.Windows.Controls.GridView>檢視模式。  
  
 [!code-xml[ListViewItemStyleSnippet#ControlTemplate](../../../../samples/snippets/csharp/VS_Snippets_Wpf/ListViewItemStyleSnippet/CS/Window1.xaml#controltemplate)]  
  
 下列範例會定義<xref:System.Windows.Controls.GridView>。  
  
 [!code-xml[ListViewItemStyleSnippet#GridView](../../../../samples/snippets/csharp/VS_Snippets_Wpf/ListViewItemStyleSnippet/CS/Window1.xaml#gridview)]  
  
## <a name="see-also"></a>另請參閱  
 <xref:System.Windows.Controls.ListView>   
 <xref:System.Windows.Controls.GridView>   
 [How to 主題](../../../../docs/framework/wpf/controls/listview-how-to-topics.md)   
 [ListView 概觀](../../../../docs/framework/wpf/controls/listview-overview.md)   
 [設定樣式和範本](../../../../docs/framework/wpf/controls/styling-and-templating.md)