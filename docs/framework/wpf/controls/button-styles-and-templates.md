...
本主題描述 <xref:System.Windows.Controls.Button> 控制項的樣式和範本。 您可以修改預設 <xref:System.Windows.Controls.ControlTemplate>，為控制項提供獨特的外觀。 如需詳細資訊，請參閱[透過建立 ControlTemplate 自訂現有控制項的外觀](customizing-the-appearance-of-an-existing-control.md)。  
  
## <a name="button-parts"></a>按鈕部分  
 <xref:System.Windows.Controls.Button> 控制項沒有任何已命名的元件。  
  
## <a name="button-states"></a>按鈕狀態  
 下表列出 <xref:System.Windows.Controls.Button> 控制項的視覺狀態。  
