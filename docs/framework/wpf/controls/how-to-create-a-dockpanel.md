---
title: 如何：建立 DockPanel
ms.date: 03/30/2017
dev_langs:
- csharp
- vb
helpviewer_keywords:
- controls [WPF], DockPanel
- DockPanel control [WPF], creating
ms.assetid: 9194f663-e279-4f1a-86d7-125a57d05c6f
ms.openlocfilehash: 4382314b0fe5385c158af9764ccb0067b741a693
ms.sourcegitcommit: 3d5d33f384eeba41b2dff79d096f47ccc8d8f03d
ms.translationtype: MT
ms.contentlocale: zh-TW
ms.lasthandoff: 05/04/2018
ms.locfileid: "33550146"
---
# <a name="how-to-create-a-dockpanel"></a>如何：建立 DockPanel
## <a name="example"></a>範例  
 下列範例會建立並使用的執行個體<xref:System.Windows.Controls.DockPanel>使用程式碼。 此範例會示範如何藉由建立五個磁碟分割空間<xref:System.Windows.Shapes.Rectangle>項目和位置 （固定） 其父系內<xref:System.Windows.Controls.DockPanel>。 如果您保留預設值，最後的矩形會填滿所有剩餘未配置的空間。  
  
 [!code-csharp[DockPanelCode#1](../../../../samples/snippets/csharp/VS_Snippets_Wpf/DockPanelCode/CSharp/DockPanel_Code.cs#1)]
 [!code-vb[DockPanelCode#1](../../../../samples/snippets/visualbasic/VS_Snippets_Wpf/DockPanelCode/VisualBasic/dockpanel_vb.vb#1)]  
  
## <a name="see-also"></a>另請參閱  
 <xref:System.Windows.Controls.DockPanel>  
 <xref:System.Windows.Controls.Dock>  
 [面板概觀](../../../../docs/framework/wpf/controls/panels-overview.md)
