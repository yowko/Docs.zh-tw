...
### <a name="typeconverter"></a>TypeConverter  
 在先前提供的 <xref:System.Windows.Point> 範例中，已提及類別 <xref:System.Windows.PointConverter>。 針對 XAML 的 .NET 執行，所有用於 XAML 用途的類型轉換器都是衍生自基類 <xref:System.ComponentModel.TypeConverter>的類別。 <xref:System.ComponentModel.TypeConverter> 類別存在於存在 XAML 之前的 .NET Framework 版本中;其中一個原始用法是在視覺化設計工具中提供屬性對話方塊的字串轉換。 對於 XAML，<xref:System.ComponentModel.TypeConverter> 的角色會展開為包含的基類，使其成為字串和字串之間的轉換，以啟用剖析字串屬性值，以及可能將特定物件屬性的運行時間值處理回的字串序列化為屬性。  
...
- <xref:System.ComponentModel.TypeConverter.ConvertFrom%2A>  
 ...
 在這些情況下，最重要的方法是 <xref:System.ComponentModel.TypeConverter.ConvertFrom%2A>。 這個方法會將輸入字串轉換為所需的物件類型。 嚴格來說，<xref:System.ComponentModel.TypeConverter.ConvertFrom%2A> 方法可以實作為將更廣泛的型別轉換成轉換器的預定目的地型別，進而提供延伸超過 XAML 的用途，例如支援執行時間轉換，但 XAML 用途是只有可以處理重要 <xref:System.String> 輸入的程式碼路徑。  
