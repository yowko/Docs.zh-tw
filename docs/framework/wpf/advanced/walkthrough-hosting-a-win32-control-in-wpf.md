...
4. 將裝載的視窗建立為包含 WPF 頁面之視窗的子系。 雖然傳統 WPF 程式設計不需要明確地使用它，但主控頁面是具有控制碼（HWND）的視窗。 您會透過 <xref:System.Windows.Interop.HwndHost.BuildWindowCore%2A> 方法的 `hwndParent` 參數來接收分頁 HWND。 裝載的視窗應該建立為此 HWND 的子系。  
...
6. 處理傳送至主視窗的已選取訊息，例如來自子控制項的通知。 有兩種方式可以達成這個目的，  
    ...
    - 如果您想要讓 WPF 處理訊息，請處理常式代碼後置中 <xref:System.Windows.Interop.HwndHost.MessageHook> 事件的 <xref:System.Windows.Interop.HwndHost> 類別。 針對裝載的視窗接收到的每個訊息，都會發生此事件。 如果您選擇此選項，您仍然必須覆寫 <xref:System.Windows.Interop.HwndHost.WndProc%2A>，但您只需要最少的執行。  
  
7. 覆寫 <xref:System.Windows.Interop.HwndHost>的 <xref:System.Windows.Interop.HwndHost.DestroyWindowCore%2A> 和 <xref:System.Windows.Interop.HwndHost.WndProc%2A> 方法。 您必須覆寫這些方法以滿足 <xref:System.Windows.Interop.HwndHost> 合約，但您可能只需要提供最基本的執行。  
  
8. 在您的程式碼後置檔案中，建立控制項裝載類別的實例，並將它設為要裝載視窗之 <xref:System.Windows.Controls.Border> 專案的子系。  
  
9. 藉由傳送 Microsoft Windows 訊息並處理來自其子視窗的訊息（例如控制項所傳送的通知），與裝載的 windows 通訊。  
...
## <a name="implement-a-class-to-host-the-microsoft-win32-control"></a>實作類別以裝載 Microsoft Win32 控制項  
 此範例的核心是實際裝載 ControlHost.cs 控制項的類別。 它繼承自 <xref:System.Windows.Interop.HwndHost>。 此函式會採用兩個參數： height 和 width，其對應至主控 ListBox 控制項之 <xref:System.Windows.Controls.Border> 專案的高度和寬度。 稍後會使用這些值，以確保控制項的大小符合 <xref:System.Windows.Controls.Border> 元素。  
...
#### <a name="create-the-host-window-and-listbox-control"></a>建立主視窗和 ListBox 控制項  
 ...
 ListBox 控制項會建立為主視窗的子系。 如上面所討論，這兩個視窗的高度和寬度都會設定為傳遞至建構函式的值。 這確保主視窗和控制項的大小與頁面上的保留區域相同。  建立 windows 之後，此範例會傳回包含主機視窗之 HWND 的 <xref:System.Runtime.InteropServices.HandleRef> 物件。  
...
### <a name="implement-destroywindow-and-wndproc"></a>實作 DestroyWindow 和 WndProc  
 除了 <xref:System.Windows.Interop.HwndHost.BuildWindowCore%2A>以外，您還必須覆寫 <xref:System.Windows.Interop.HwndHost>的 <xref:System.Windows.Interop.HwndHost.WndProc%2A> 和 <xref:System.Windows.Interop.HwndHost.DestroyWindowCore%2A> 方法。 在此範例中，控制項的訊息是由 <xref:System.Windows.Interop.HwndHost.MessageHook> 處理常式處理，因此 <xref:System.Windows.Interop.HwndHost.WndProc%2A> 和 <xref:System.Windows.Interop.HwndHost.DestroyWindowCore%2A> 的執行最少。 在 <xref:System.Windows.Interop.HwndHost.WndProc%2A>的情況下，請將 `handled` 設定為 `false`，以指出未處理訊息，並傳回0。 針對 <xref:System.Windows.Interop.HwndHost.DestroyWindowCore%2A>，只要終結視窗即可。  
...
## <a name="host-the-control-on-the-page"></a>在頁面上裝載控制項  
 若要在頁面上裝載控制項，您必須先建立 `ControlHost` 類別的新實例。 將包含控制項（`ControlHostElement`）之 border 元素的高度和寬度傳遞至 `ControlHost` 的「檢查程式」。 這確保 ListBox 的大小正確。 然後藉由將 `ControlHost` 物件指派給主機 <xref:System.Windows.Controls.Border>的 <xref:System.Windows.Controls.Decorator.Child%2A> 屬性，在頁面上裝載控制項。  
  
 這個範例會將處理常式附加至 `ControlHost` 的 <xref:System.Windows.Interop.HwndHost.MessageHook> 事件，以接收來自控制項的訊息。 針對傳送至裝載之視窗的每個訊息，都會引發此事件。 在此情況下，這些是傳送至包裝實際 ListBox 控制項之視窗的訊息，包括來自控制項的通知。 此範例會呼叫 SendMessage 以從控制項取得資訊，並修改其內容。 下節討論頁面如何與控制項進行通訊的詳細資料。  
  
> [!NOTE]
> 請注意，SendMessage 有兩個 PInvoke 宣告。 這是必要的，因為其中一個使用 `wParam` 參數來傳遞字串，另一個使用它來傳遞整數。 每個簽章都必須要有不同的宣告，確保正確地封送處理資料。  
...
- 顯示清單中的項目數。  
 ...
 若要附加專案，請傳送清單方塊[`LB_ADDSTRING` 訊息](/windows/desktop/Controls/lb-addstring)。 若要刪除專案，請傳送[`LB_GETCURSEL`](/windows/desktop/Controls/lb-getcursel)以取得目前選取範圍的索引，然後[`LB_DELETESTRING`](/windows/desktop/Controls/lb-deletestring)以刪除專案。 此範例也會傳送[`LB_GETCOUNT`](/windows/desktop/Controls/lb-getcount)，並使用傳回的值來更新顯示專案數的顯示。 [`SendMessage`](/windows/desktop/api/winuser/nf-winuser-sendmessage)的兩個實例都使用上一節所討論的其中一個 PInvoke 宣告。  
  
 [!code-csharp[WPFHostingWin32Control#AppendDeleteText](~/samples/snippets/csharp/VS_Snippets_Wpf/WPFHostingWin32Control/CSharp/Page1.xaml.cs#appenddeletetext)]
 [!code-vb[WPFHostingWin32Control#AppendDeleteText](~/samples/snippets/visualbasic/VS_Snippets_Wpf/WPFHostingWin32Control/VisualBasic/Page1.xaml.vb#appenddeletetext)]  
  
 當使用者選取專案或變更其選擇時，控制項會傳送[`WM_COMMAND` 訊息](/windows/desktop/menurc/wm-command)來通知主機視窗，這會引發頁面的 <xref:System.Windows.Interop.HwndHost.MessageHook> 事件。 處理常式會接收與主視窗的主要視窗程序相同的資訊。 它也會將參考傳遞至布林值 `handled`。 您將 `handled` 設定為 `true`，以指出您已處理訊息，且不需要進一步處理。  
  
 基於各種原因而傳送[`WM_COMMAND`](/windows/desktop/menurc/wm-command) ，因此您必須檢查通知識別碼，以判斷它是否為您想要處理的事件。 識別碼包含在 `wParam` 參數的最大字組中。 此範例會使用位運算子來將識別碼解壓縮。 如果使用者已做出或變更其選擇，將會[`LBN_SELCHANGE`](/windows/desktop/Controls/lbn-selchange)識別碼。  
  
 收到[`LBN_SELCHANGE`](/windows/desktop/Controls/lbn-selchange)時，範例會藉由傳送[`LB_GETCURSEL` 訊息](/windows/desktop/Controls/lb-getcursel)給控制項，取得所選取專案的索引。 若要取得文字，您必須先建立 <xref:System.Text.StringBuilder>。 接著，您會將[`LB_GETTEXT` 訊息](/windows/desktop/Controls/lb-gettext)傳送給控制項。 傳遞空的 <xref:System.Text.StringBuilder> 物件做為 `wParam` 參數。 當[`SendMessage`](/windows/desktop/api/winuser/nf-winuser-sendmessage)傳回時，<xref:System.Text.StringBuilder> 將包含所選取專案的文字。 這項[`SendMessage`](/windows/desktop/api/winuser/nf-winuser-sendmessage)的用法還需要另一個 PInvoke 宣告。  
