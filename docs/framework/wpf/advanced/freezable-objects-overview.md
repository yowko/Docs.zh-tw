 ...
 **注意**由於凍結的可凍結無法繪製動畫, 因此當您嘗試以動畫顯示<xref:System.Windows.Freezable> <xref:System.Windows.Media.Animation.Storyboard>時, 動畫系統會自動建立凍結物件的可修改複本。 若要消除複製所造成的效能額外負荷, 如果您想要以動畫顯示物件, 請將它保留為未凍結。 如需使用分鏡腳本製作動畫的詳細資訊, 請參閱分鏡腳本[總覽](../graphics-multimedia/storyboards-overview.md)。  
