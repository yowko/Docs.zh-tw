---
title: "如何：將應用程式當地語系化 | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
...
## 需求  
 ...
 此外，也會指引您使用專案檔。  如需有關如何使用 [!INCLUDE[TLA2#tla_msbuild](../../../../includes/tla2sharptla-msbuild-md.md)] 和專案檔的指示，請參閱[建置和部署](../../../../docs/framework/wpf/app-development/building-and-deploying-wpf-applications.md)。  
  
 此討論中的所有範例都是使用 en\-US \(美式英文\) 做為文化特性。  這可讓您逐步執行範例中的所有步驟，而不需要安裝不同的語言。  
  
<a name="create_sample_app"></a>   
## 建立範例應用程式  
 在此步驟中，您將準備要當地語系化的應用程式。  在 [!INCLUDE[TLA#tla_winclient](../../../../includes/tlasharptla-winclient-md.md)] 範例中有提供一個 HelloApp 範例，將用於此討論中的程式碼範例。  如果您想要使用此範例，請從 [LocBaml 工具範例](http://go.microsoft.com/fwlink/?LinkID=160016)下載 [!INCLUDE[TLA#tla_xaml](../../../../includes/tlasharptla-xaml-md.md)] 檔案。  
...
3.  將 UID 加入您的 [!INCLUDE[TLA2#tla_xaml](../../../../includes/tla2sharptla-xaml-md.md)] 檔案。  UID 可用來追蹤對檔案的變更，以及識別必須轉譯的項目。  若要將 UID 加入您的檔案，請在您的專案檔上執行 **updateuid**：  
  
     **msbuild \/t:updateuid helloapp.csproj**  
  
     若要確認您有沒有遺漏或重複的 UID，請執行 **checkuid**：  
...
1.  建置 LocBaml 時所需的所有檔案都位在 [!INCLUDE[TLA2#tla_winclient](../../../../includes/tla2sharptla-winclient-md.md)] 範例中。  從 [LocBaml 工具範例](http://go.microsoft.com/fwlink/?LinkID=160016)下載 [!INCLUDE[TLA#tla_lhcshrp](../../../../includes/tlasharptla-lhcshrp-md.md)] 檔案。  
...
4.  當您執行 LocBaml 時，可指定的選項如下：  
    ...
    -   **parse** 或 **\-p:**，可剖析 Baml、資源或 [!INCLUDE[TLA2#tla_dll](../../../../includes/tla2sharptla-dll-md.md)] 檔案，以產生 .csv 或 .txt 檔案。  
  
    -   **generate** 或 **\-g:**，可利用已轉譯的檔案來產生當地語系化的二進位檔案。  
  
    -   **out** 或 **\-o** \[*檔案目錄*\]**:**，輸出檔案名稱。  
  
    -   **culture** 或 **\-cul** \[*文化特性*\]**:**，輸出組件的地區設定。  
  
    -   **translation** 或 **\-trans** \[*translation.csv*\]**:**，轉譯或當地語系化的檔案。  
  
    -   **asmpath** 或 **\-asmpath:** \[*檔案目錄*\]**:**，如果您的 [!INCLUDE[TLA2#tla_xaml](../../../../includes/tla2sharptla-xaml-md.md)] 程式碼包含自訂控制項，則必須提供 **asmpath** 給自訂控制項組件。  
    ...
    > [!NOTE]
    >  當您執行此工具時，如果需要選項清單，請輸入 **LocBaml.exe**，並按 ENTER 鍵。  
...
3.  當您執行 LocBaml 來剖析檔案時，輸出中會包含以逗號 \(.csv 檔案\) 或定位點 \(.txt 檔\) 分隔的七個欄位。  下面顯示針對 HelloApp.resources.dll 所剖析的 .csv 檔案：  
    ...
    |HelloApp.g.en\-US.resources:window1.baml|Text2:System.Windows.Controls.TextBlock.$Content|無|TRUE|TRUE||Goodbye World|  
     ...
     請注意，\[註解\] 欄位的所有值都不含任何值；如果欄位沒有值，則是空的。  另請注意，第一個資料列中的項目無法讀取或修改，且其 \[分類\] 值為 "Ignore"，這一切都表示該值無法當地語系化。  
...
## 使用 LocBaml 來產生新的 .resources.dll 檔案  
