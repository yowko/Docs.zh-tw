...
## <a name="remarks"></a>備註  
 ...
 `assemblyUri`字串（<xref:System.Windows.ThemeDictionaryExtension.AssemblyName%2A>屬性值）會形成命名慣例的基礎，以識別特定主題所適用的字典。 的<xref:System.Windows.Markup.MarkupExtension.ProvideValue%2A>邏輯會`ThemeDictionary`藉由產生指向特定主題字典變異的統一資源識別項（URI）來完成慣例，如預先編譯的資源元件中所含。 此處將不會完整說明此慣例，或是概念上與一般控制項樣式設定和頁面/應用程式層級樣式設定進行的佈景主題互動。 使用`ThemeDictionary`的基本案例是指定在應用層<xref:System.Windows.ResourceDictionary.Source%2A>級宣告之`ResourceDictionary`的屬性。 當您透過`ThemeDictionary`延伸模組（而非直接 URI）提供元件的 uri 時，延伸模組邏輯將提供當系統主題變更時適用的失效邏輯。  
...
```  
 ...
 [!INCLUDE[TLA2#tla_winclient](../../../../includes/tla2sharptla-winclient-md.md)] [!INCLUDE[TLA2#tla_xaml](../../../../includes/tla2sharptla-xaml-md.md)]在處理器執行中，此標記延伸模組的處理是由<xref:System.Windows.ThemeDictionaryExtension>類別所定義。  
