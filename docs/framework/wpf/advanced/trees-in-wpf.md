...
## <a name="tree-traversal"></a>樹狀周遊  
 ...
 視覺化樹狀結構也支援視覺化樹狀結構周遊的 helper 類別<xref:System.Windows.Media.VisualTreeHelper>。 視覺化樹狀結構不會公開為方便地透過控制項特定屬性，因此<xref:System.Windows.Media.VisualTreeHelper>類別是周遊視覺化樹狀結構，如果是您的程式設計案例所需的建議的方式。 如需詳細資訊，請參閱 [WPF 圖形轉譯概觀](../graphics-multimedia/wpf-graphics-rendering-overview.md)。  
