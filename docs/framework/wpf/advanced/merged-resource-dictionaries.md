...
> [!NOTE]
> 請勿使用內嵌資源建置動作。 [!INCLUDE[TLA2#tla_winclient](../../../../includes/tla2sharptla-winclient-md.md)] 的應用程式支援組建動作本身，但 <xref:System.Windows.ResourceDictionary.Source%2A> 的解決方式不會納入 <xref:System.Resources.ResourceManager>，因此無法將個別資源從資料流程中分離出來。 您仍然可以使用內嵌資源來進行其他目的，只要您也使用 <xref:System.Resources.ResourceManager> 來存取資源。  
