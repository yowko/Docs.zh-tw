...
3. 將產生的程式碼取代為下列程式碼：
     ...
     此程式碼會定義包含兩個子控制項的 <xref:System.Windows.Controls.UserControl?displayProperty=nameWithType>。 第一個子控制項是 <xref:System.Windows.Controls.Label?displayProperty=nameWithType> 控制項;第二個是顯示立體錐形的 <xref:System.Windows.Controls.Viewport3D> 控制項。
