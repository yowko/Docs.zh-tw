...
XAML 瀏覽器應用程式（Xbap）會在部分信任的安全性沙箱中執行，而此沙箱僅限於許可權的網際網路區域集。 這個許可權集合會將 Web 服務通話限制為位於 XBAP 應用程式的來源網站上的 Web 服務。 不過，從 Visual Studio 2005 調試 XBAP 時，不會將其所參考之 Web 服務的來源網站視為相同。 這會導致當 XBAP 嘗試呼叫 Web 服務時，會引發安全性例外狀況。 不過，Visual Studio 2005 [!INCLUDE[TLA#tla_wpfbrowserappproj](../../../../includes/tlasharptla-wpfbrowserappproj-md.md)] 專案可以設定為模擬與在進行調試時所呼叫的 Web 服務相同的來源網站。 這可讓 XBAP 安全地呼叫 Web 服務，而不會導致安全性例外狀況。
