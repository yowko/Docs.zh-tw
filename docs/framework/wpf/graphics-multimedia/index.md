...
[!INCLUDE[TLA#tla_winclient](../../../../includes/tlasharptla-winclient-md.md)] 提供多媒體、向量圖形、動畫和內容撰寫的支援，讓開發人員輕鬆建立有趣的使用者介面和內容。 您可以使用 Visual Studio 建立向量圖形或複雜的動畫，並將媒體整合到您的應用程式中。
...
- 版面配置進位
  ...
  當物件邊緣位於像素裝置中間時，DPI 獨立圖形系統可以建立轉譯成品，例如模糊或半透明的邊緣。 先前的 WPF 版本包含像素貼齊功能來協助處理這種情況。 Silverlight 2 引進版面配置進位，這是另一種移動元素以讓邊緣落在整個像素邊界上的方法。 WPF 現在支援 <xref:System.Windows.FrameworkElement>上的 <xref:System.Windows.FrameworkElement.UseLayoutRounding%2A> 附加屬性進行版面配置進位。
...
- 像素著色器 3 支援
  ...
  WPF 4 是以 WPF 3.5 SP1 中所引進的 <xref:System.Windows.Media.Effects.ShaderEffect> 支援為基礎，藉由允許應用程式使用圖元著色器（PS）3.0 版來寫入效果。 PS 3.0 著色器模型比 PS 2.0 更為複雜，其在支援的硬體上允許使用更多效果。
...
WPF 可支援高品質的 2D 圖形。 這些功能包括筆刷、幾何、影像、圖形及轉換。 如需詳細資訊，請參閱[圖形](graphics.md)。 圖形元素的呈現是以 <xref:System.Windows.Media.Visual> 類別為基礎。 螢幕上視覺物件的結構是由視覺化樹狀結構描繪。 如需詳細資訊，請參閱 [WPF 圖形轉譯概觀](wpf-graphics-rendering-overview.md)。
