 ...
 **最大的多型值**可讓您調整立體內容的最大消除鋸齒量。 使用此層級可停用 Windows Vista 中的3D 消除鋸齒功能。  
