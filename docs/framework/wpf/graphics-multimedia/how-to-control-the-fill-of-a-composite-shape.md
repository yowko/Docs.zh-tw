...
ms.locfileid: "54574673"
---
# <a name="how-to-control-the-fill-of-a-composite-shape"></a>HOW TO：控制複合圖案的填色
 ...
 **EvenOdd:** 此規則會判斷某個點是否在填滿區域中，藉由從該點朝任意方向繪製無限遠的光線，並且計算給定圖案中與光線相交中的路徑區段數目。 如果這個數字是奇數，該點即是在區域內；如為偶數，該點即在區域外。  
 ...
 ![螢幕擷取畫面：EvenOdd Fillrule](../../../../docs/framework/wpf/graphics-multimedia/media/fillruleevenoddfirstone.png "FillRuleEvenOddFirstOne")  
  
 在上圖中，請注意中間和第 3 個環形不會填滿。 這是因為從這兩個環形中的任何點所繪製的光線，會通過偶數數目的線段。 請參閱下圖︰  
  
 ![圖表：為 EvenOdd 的 FillRule 屬性值](../../../../docs/framework/wpf/graphics-multimedia/media/fillruleevenodd2.png "FillRuleEvenOdd2")  
 ...
 **非零值：** 此規則會判斷某個點是否在路徑填滿區域中，從該點朝任意方向繪製無限遠的光線，然後檢查圖案線段與光線交的所在的位置。 從零開始計算，路徑線段每次從左到右與光線交會就加一，每次從右到左與光線交會就減一。 計算交會後，如果結果為零，則點即在路徑外。 否則就在路徑內。  
 ...
 ![螢幕擷取畫面：NonZero fillrule](../../../../docs/framework/wpf/graphics-multimedia/media/fillrulenonzero1.png "FillRuleNonZero1")  
 ...
 ![圖表：屬性值等於 NonZero FillRule](../../../../docs/framework/wpf/graphics-multimedia/media/fillrulenonzero2.png "FillRuleNonZero2")  
 ...
 **注意：** 針對目的<xref:System.Windows.Media.FillRule>，所有圖案都視為封閉式。 如果線段中有間隙，請繪製虛構線段將它封閉。 在上面的範例中，環形中有小型的間隙。 有鑑於此，我們可能會預期通過間隙射出的光線，會提供與另一個方向的光線不同的結果。 以下是其中一個間隙以及 「 虛構線段 」 的放大的圖 (基於套用繪製的線段<xref:System.Windows.Media.FillRule>)，將它關閉。  
 ...
 ![圖表：針對 FillRule，區段保持關閉](../../../../docs/framework/wpf/graphics-multimedia/media/fillruleclosedshapes.png "FillRuleClosedShapes")  
