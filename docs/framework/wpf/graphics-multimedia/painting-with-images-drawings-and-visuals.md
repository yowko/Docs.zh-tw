...
## <a name="paint-an-area-with-an-image"></a>使用影像繪製區域  
 <xref:System.Windows.Media.ImageBrush>繪製區域<xref:System.Windows.Media.ImageSource>。 最常見的類型<xref:System.Windows.Media.ImageSource>以搭配<xref:System.Windows.Media.ImageBrush>是<xref:System.Windows.Media.Imaging.BitmapImage>，其會描述點陣圖形。 您可以使用<xref:System.Windows.Media.DrawingImage>若要使用繪製<xref:System.Windows.Media.Drawing>物件，但它是簡單好用<xref:System.Windows.Media.DrawingBrush>改為。 如需詳細資訊<xref:System.Windows.Media.ImageSource>物件，請參閱[影像處理概觀](imaging-overview.md)。  
...
ImageBrush 所繪製的物件  
 ...
 根據預設，<xref:System.Windows.Media.ImageBrush>兩端之間自動縮放其影像來填滿區域所繪製，可能造成影像扭曲，如果繪製的區域有不同的外觀比例比的映像。 您可以變更此行為變更<xref:System.Windows.Media.TileBrush.Stretch%2A>屬性的預設值<xref:System.Windows.Media.Stretch.Fill>要<xref:System.Windows.Media.Stretch.None>， <xref:System.Windows.Media.Stretch.Uniform>，或<xref:System.Windows.Media.Stretch.UniformToFill>。 因為<xref:System.Windows.Media.ImageBrush>是一種<xref:System.Windows.Media.TileBrush>，您可以指定完全如何影像筆刷填滿輸出區域，並甚至建立圖樣。 如需有關進階<xref:System.Windows.Media.TileBrush>功能，請參閱[TileBrush 概觀](tilebrush-overview.md)。  
