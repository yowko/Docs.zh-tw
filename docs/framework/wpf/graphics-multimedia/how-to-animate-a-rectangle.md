...
## <a name="example"></a>範例  
 下列範例會使用的執行個體<xref:System.Windows.Media.Animation.RectAnimation>類別以動畫方式顯示<xref:System.Windows.Media.RectangleGeometry.Rect%2A>屬性<xref:System.Windows.Media.RectangleGeometry>，其中繪製的矩形的位置和大小的變更。  
