...
使用 SolidColorBrush 繪製的矩形  
 ...
 如需詳細資訊<xref:System.Windows.Media.SolidColorBrush>類別，請參閱[使用純色和漸層概觀繪製](painting-with-solid-colors-and-gradients-overview.md)。  
...
使用 LinearGradientBrush 繪製的矩形  
 ...
 如需詳細資訊<xref:System.Windows.Media.LinearGradientBrush>類別，請參閱[使用純色和漸層概觀繪製](painting-with-solid-colors-and-gradients-overview.md)。  
...
使用 RadialGradientBrush 繪製的矩形  
 ...
 如需詳細資訊<xref:System.Windows.Media.RadialGradientBrush>類別，請參閱[使用純色和漸層概觀繪製](painting-with-solid-colors-and-gradients-overview.md)。  
...
使用 DrawingBrush 繪製的矩形  
 ...
 如需詳細資訊<xref:System.Windows.Media.DrawingBrush>類別，請參閱[使用影像、 繪圖和視覺效果繪製](painting-with-images-drawings-and-visuals.md)。  
...
使用 VisualBrush 繪製的矩形  
 ...
 如需詳細資訊<xref:System.Windows.Media.VisualBrush>類別，請參閱[使用影像、 繪圖和視覺效果繪製](painting-with-images-drawings-and-visuals.md)。  
...
>  會更有效率，若要變更筆刷的不透明度值，變更整個項目使用的不透明度超出其<xref:System.Windows.UIElement.Opacity%2A?displayProperty=nameWithType>屬性。  
 ...
 您可以旋轉、 縮放、 扭曲，和轉譯所使用的筆刷的內容及其<xref:System.Windows.Media.Brush.Transform%2A>或<xref:System.Windows.Media.Brush.RelativeTransform%2A>屬性。 如需詳細資訊，請參閱 <<c0> [ 筆刷轉換概觀](brush-transformation-overview.md)。  
  
 因為它們<xref:System.Windows.Media.Animation.Animatable>物件，<xref:System.Windows.Media.Brush>可以動畫顯示物件。 如需詳細資訊，請參閱 [動畫概觀](animation-overview.md)。  
  
<a name="freezable_features"></a>   
### <a name="freezable-features"></a>Freezable 功能  
 因為它繼承自<xref:System.Windows.Freezable>類別，<xref:System.Windows.Media.Brush>類別會提供數個特殊功能︰<xref:System.Windows.Media.Brush>物件可以宣告為[資源](../advanced/xaml-resources.md)、 在多個物件之間共用，並複製。 此外，所有<xref:System.Windows.Media.Brush>型別除了<xref:System.Windows.Media.VisualBrush>可以成為唯讀，以改善效能，並變更為安全執行緒。  
  
 如需詳細資訊，所提供的不同功能的相關<xref:System.Windows.Freezable>物件，請參閱[Freezable 物件概觀](../advanced/freezable-objects-overview.md)。  
