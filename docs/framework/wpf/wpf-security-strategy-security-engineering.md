...
## <a name="wpf-specifics"></a>WPF 特性  
 [!INCLUDE[TLA2#tla_winclient](../../../includes/tla2sharptla-winclient-md.md)] 工程小組會同時套用和擴充 SDL，其結合包括下列重要層面：  
...
### <a name="source-analysis-and-editing-tools"></a>來源分析和編輯工具  
 除了 SDL 的手動安全性程式碼審查元素之外，[!INCLUDE[TLA2#tla_winclient](../../../includes/tla2sharptla-winclient-md.md)] 小組也會使用數種工具進行來源分析，並進行相關的編輯，以降低安全性弱點。 使用廣泛的來源工具，包含下列項目：  
...
- **禁止的應用程式開發介面**：搜尋原始程式碼，來識別已知會造成安全性問題之函式的意外使用，例如 `strcpy`。 一旦識別出來，這些函式會取代為更安全的替代專案。  
...
- **從其他產品減輕安全性問題**：如果相關，則測試來自相關產品的安全性問題。 例如，已識別 Internet Explorer 大約60安全性問題的適當變種，並嘗試其對 [!INCLUDE[TLA2#tla_winclient](../../../includes/tla2sharptla-winclient-md.md)]的適用性。  
