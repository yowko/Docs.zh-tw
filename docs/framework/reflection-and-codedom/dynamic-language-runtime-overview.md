...
DLR 的目的是讓動態語言系統能在 .NET Framework 上執行，並提供它們 .NET 互通性。 DLR 將動態物件新增至 C# 和 Visual Basic，以支援這些語言中的動態行為，並啟用其與動態語言的互通。
...
- IronRuby。 以開放原始碼軟體的形式提供，可從 [IronRuby](http://ironruby.net/) 網站取得。
...
## <a name="dlr-architecture"></a>DLR 架構
 ...
 DLR 新增一組服務給 CLR，以便更妥善支援動態語言。 這些服務包括：
