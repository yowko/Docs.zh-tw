---
title: 反映發出中的安全性問題
description: 知道反映發出中的安全性問題，這是透過動態元件來完成，或是與現有元件連線或匿名裝載的動態方法。
ms.date: 03/30/2017
helpviewer_keywords:
...
- 如果您指定 JIT 可見度檢查應該要跳過，則在建構方法時提出的要求會包含具有 <xref:System.Security.Permissions.ReflectionPermissionFlag.RestrictedMemberAccess?displayProperty=nameWithType> 旗標的 <xref:System.Security.Permissions.ReflectionPermission>以及包含正在存取非公用成員之組件的授權集。  
 ...
 裝載匿名裝載動態方法的系統組件會使用 <xref:System.Security.SecurityRuleSet.Level1?displayProperty=nameWithType> 透明度模型，這是在 .NET Framework 4 之前的 .NET Framework 中所使用透明度模型。  
