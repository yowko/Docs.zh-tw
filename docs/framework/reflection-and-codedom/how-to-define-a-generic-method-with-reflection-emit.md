...
5.  將包含參數名稱的字串陣列傳遞至 <xref:System.Reflection.Emit.MethodBuilder.DefineGenericParameters%2A?displayProperty=fullName> 方法，以定義 `DemoMethod` 的泛型型別參數。 這會讓方法成為泛型方法。 下列程式碼使 `Factory` 成為具有類型參數 `TInput` 和 `TOutput` 的泛型方法。 為更方便閱讀程式碼，會建立有這些名稱的變數以保留代表兩個類型參數的 <xref:System.Reflection.Emit.GenericTypeParameterBuilder> 物件。  
...
7.  發出程式碼將 `TOutput` 物件推送至堆疊，並從方法傳回。 區域變數 `retVal` 和 `ic` 同時包含新 `TOutput` 的參考；`ic` 僅用於存取 <xref:System.Collections.Generic.ICollection%601.Add%2A?displayProperty=fullName> 方法。  
...
3.  若要使用委派叫用方法，您必須有符合建構泛型方法簽章的委派。 執行此作業的簡單方法是建立泛型委派。 下列程式碼會使用 <xref:System.Delegate.CreateDelegate%28System.Type%2CSystem.Reflection.MethodInfo%29?displayProperty=fullName> 方法多載建立在範例程式碼中定義之泛型委派 `D` 的執行個體，並叫用委派。 委派執行優於晚期繫結呼叫。  
