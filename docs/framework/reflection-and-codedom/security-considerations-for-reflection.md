...
- 組件 A 無法使用反射來存取 .NET Framework 組件的私有成員 (例如 mscorlib.dll)，因為 mscorlib.dll 為完全受信任，擁有尚未授與組件 A 的權限。當程式碼存取安全性查核在執行階段查核堆疊時，會擲回 <xref:System.MemberAccessException>。  
