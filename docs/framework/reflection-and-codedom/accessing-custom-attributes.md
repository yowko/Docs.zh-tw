...
## <a name="the-execution-context"></a>執行內容  
 ...
 <xref:System.Reflection.Assembly.GetCustomAttributes%28System.Boolean%29?displayProperty=fullName> 這類方法會檢查型別引數的可視性和存取性。 只有包含使用者定義型別之組件中的程式碼才能使用 **GetCustomAttributes** 擷取類型的自訂屬性。  
...
4.  執行階段會使用自訂屬性參數來呼叫 **MyDescriptionAttribute** 的建構函式，並將新物件傳回給呼叫者。  
 ...
 自訂屬性反映模型可能會流失在其中定義使用者定義型別之組件外部的類型執行個體。 這與傳回使用者定義型別執行個體的執行階段系統程式庫成員沒有差異，例如 <xref:System.Type.GetMethods%2A?displayProperty=fullName> 傳回 **RuntimeMethodInfo** 物件的陣列。 若要防止用戶端探索使用者定義自訂屬性類型的資訊，請將類型的成員定義為非公用。  
