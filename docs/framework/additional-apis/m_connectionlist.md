...
`ConnectionGroup.m_ConnectionList` 是<xref:System.Collections.ArrayList>連線物件所做的相同 URI 和共用的其他屬性相同的值，如到期日和驗證。
...
> [!WARNING]
> `ConnectionGroup.m_ConnectionList`欄位是私用，而且不會直接在您的程式碼中使用它們。
> 
