...
> [!NOTE]
>  就 .NET Framework 的類型和成員而言，*「過時」*\(Obsolete\) 和*「已被取代」*\(Deprecated\) 具有相同的意義。  
...
1.  建立主要可執行檔的組態檔，並將它命名為 *應用程式名稱*.exe.config，其中 *應用程式名稱* 是應用程式可執行檔的名稱。  對於範例中名為 app1.exe 的應用程式而言，您要建立名為 app1.exe.config 的組態檔。  
