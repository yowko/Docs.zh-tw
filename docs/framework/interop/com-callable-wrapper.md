---
title: "COM 可呼叫包裝函式 | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
...
## 模擬 COM 介面  
 [COM 可呼叫包裝函式](../../../docs/framework/interop/com-callable-wrapper.md) \(CCW\) 會以與 COM 的介面互動強制一致的方式，公開所有的公用、COM 可見介面、資料類型和傳回值給 COM 用戶端。  對於 COM 用戶端而言，在 .NET Framework 物件上叫用方法與在 COM 物件上叫用方法相同。  
...
|**IdispatchEx**|如果類別實作 **IExpando**，則為執行階段提供的介面。  **IDispatchEx** 介面是 **IDispatch** 介面的延伸，它不同於 **IDispatch**，可進行成員的列舉、加入、刪除和區分大小寫的呼叫。|  
...
```  
 ...
 COM 用戶端可以取得名為 `_Mammal` 之類別介面的指標，它描述於[類型程式庫匯出工具 \(Tlbexp.exe\)](../../../docs/framework/tools/tlbexp-exe-type-library-exporter.md) 工具所產生的類型程式庫中。  如果 `Mammal` 類別實作一個或多個介面，介面會出現在 coclass 底下。  
 ...
 **ClassInterfaceType.None** 值可防止在類別中繼資料匯出至類型程式庫時產生類別介面。  在上述範例中，COM 用戶端只能透過 `IExplicit` 介面存取 `LoanApp` 類別。  
...
### 避免快取分派識別項 \(DispId\)。  
 ...
 若要避免在使用類別介面時中斷晚期繫結 COM 用戶端，請套用 **ClassInterfaceAttribute** 與 **ClassInterfaceType.AutoDispatch** 值。  這個值會實作僅分派類別介面，但會略過來自類型程式庫的介面描述。  沒有介面描述，用戶端將無法在編譯時間快取 DispId。  雖然這是類別介面的預設介面類型，您仍可以明確地套用屬性值。  
...
```  
 ...
 若要在執行階段取得介面成員的 DispId，COM 用戶端可以呼叫 **IDispatch.GetIdsOfNames**。  若要在介面上叫用方法，請將傳回的 DispId 做為引數傳遞給 **IDispatch.Invoke**。  
  
### 限制針對類別介面使用雙重介面選項。  
 雙重介面可讓 COM 用戶端進行介面成員的早期和晚期繫結。  在設計階段和測試期間，您可能會發現將類別介面設為雙重很有用。  對於絕對不會修改的 Managed 類別 \(和其基底類別\)，此選項也是可以接受的。  在其他情況下，請避免將類別介面設定為雙重。  
  
 自動產生的雙重介面可能適合於少數情況，不過，通常它會造成與版本相關的複雜性。  例如，使用衍生類別之類別介面的 COM 用戶端，可能會因為對基底類別的變更而輕易地中斷。  當協力廠商提供基底類別時，類別介面的配置會超出您的控制。  此外，不同於僅分派介面，雙重介面 \(**ClassInterface.AutoDual**\) 在匯出的類型程式庫中提供類別介面的描述。  這類描述鼓勵晚期繫結的用戶端在執行階段快取 DispId。  
...
## 請參閱  
 ...
 [Simulating COM Interfaces](http://msdn.microsoft.com/zh-tw/ad2ab959-e2be-411b-aaff-275c3fba606c)   
