...
![Managed 和非受控堆積之間的 Interop 封送處理](./media/interop-marshaling/interop-heaps-managed-and-unmanaged.gif "相同 Apartment 封送處理流程")
...
當 Managed 用戶端和 Unmnaged 伺服器位於相同的 Apartment 中時，Interop 封送處理服務會處理所有資料封送處理。 不過，當用戶端和伺服器在不同的 Apartment 中初始化時，還需要 COM 封送處理。 下圖顯示跨單元呼叫的元素：
