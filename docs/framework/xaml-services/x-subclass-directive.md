...
## <a name="wpf-usage-notes"></a>WPF 使用注意事項  
 ...
 在衍生類別中的事件處理常式必須是`internal override`(`Friend Overrides`中[!INCLUDE[TLA#tla_visualb](../../../includes/tlasharptla-visualb-md.md)]) 才能在編譯期間建立中繼類別中覆寫針對此處理常式的虛設常式。 否則衍生的類別實作會隱藏 （陰影） 的中繼類別實作，而且不會叫用的中繼類別處理常式。  
