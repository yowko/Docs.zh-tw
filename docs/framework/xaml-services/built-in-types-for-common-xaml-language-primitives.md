...
### <a name="xstring"></a>x:String  
 ...
 字串和 char 類型會與 XML 層級之檔案的整體編碼間互動。 如需 XAML 語言規格定義，請參閱[ \[MS-XAML\] 5.2.6](http://go.microsoft.com/fwlink/?LinkId=114525)。  
...
### <a name="xbyte"></a>x:Byte  
 對於 CLR 支援， `x:Byte` 基本類型對應至 <xref:System.Byte>。 A <xref:System.Byte>  /  `x:Byte`會被視為為不帶正負號。  
...
### <a name="xarray"></a>x:Array  
 ...
 如需 XAML 語言規格定義，請參閱[ \[MS-XAML\] 5.2.18](http://go.microsoft.com/fwlink/?LinkId=114525)。  
...
## <a name="wpf-support"></a>WPF 支援  
 ...
 您可以在其中使用 XAML 2009 功能與 WPF 的情況是您編寫鬆散的 XAML，您再將該 XAML 載入 WPF 執行階段和物件圖與<xref:System.Windows.Markup.XamlReader.Load%2A?displayProperty=nameWithType>。 WPF<xref:System.Windows.Markup.XamlReader?displayProperty=nameWithType>及其<xref:System.Windows.Markup.XamlReader.Load%2A>可以 XAML 2009 語言關鍵字和功能處理為有效的物件圖表示法。
