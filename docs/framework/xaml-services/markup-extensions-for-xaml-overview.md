...
ms.locfileid: "71736500"
---
# <a name="markup-extensions-for-xaml-overview"></a>XAML 標記延伸概觀

標記延伸是一種 XAML 技巧，用於取得不是基本類型和特定 XAML 類型的值。 對於屬性使用方式，標記延伸使用左大括號 `{` 的已知字元序列進入標記延伸範圍，並使用右大括號 `}` 結束。 使用 .NET Framework XAML 服務時，您可以使用 System.Xaml 組件中的一些預先定義 XAML 語言標記延伸。 您也可以從 <xref:System.Windows.Markup.MarkupExtension> 類別產生子類別 (定義於 System.Xaml 中)，以及定義您自己的標記延伸。 或者，如果您已經參考該架構，您可以使用特定架構所定義的標記延伸。
...
## <a name="xaml-defined-markup-extensions"></a>XAML 定義的標記延伸

.NET Framework XAML 服務會針對 XAML 語言支援實作數個標記延伸。 這些標記延伸對應至 XAML 規格的各部分，做為語言。 這些通常可透過常見使用方式中所見語法的 `x:` 前置詞予以識別。 這些 XAML 語言專案的 .NET Framework XAML 服務實例全都衍生自 <xref:System.Windows.Markup.MarkupExtension> 基類。
...
### <a name="the-parameterless-constructor"></a>無參數的構造函式
...
## <a name="attributing-for-a-custom-markup-extension"></a>自訂標記延伸的將其做為特性
