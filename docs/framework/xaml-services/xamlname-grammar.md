...
```  
 ...
 XAML 定義的第二個文法，DottedXamlName，用於屬性和事件限定的參考，同時並針對繫結成員。 如需詳細資訊，請參閱 <<c0> <xref:System.Windows.DependencyProperty> 並[XAML 概觀 (WPF)](../wpf/advanced/xaml-overview-wpf.md)。  
