---
title: "在 Windows 8 上安裝 .NET Framework | Microsoft Docs"
description: "了解如何在 Windows 8 上安裝 .NET Framework"
author: rlander
keywords: ".NET Framework, 安裝"
ms.date: 03/28/2017
ms.topic: article
ms.prod: .net-framework-4.6
ms.technology: vs-ide-deployment
ms.devlang: dotnet
ms.assetid: 85cda1d5-c9a4-4eb5-93e6-4f478de07ff7
ms.translationtype: Human Translation
ms.sourcegitcommit: 4437ce5d344cf06d30e31911def6287999fc6ffc
ms.openlocfilehash: 3746f31c316d5c79f9672c4e785fc2bb82411db9
ms.contentlocale: zh-tw
ms.lasthandoff: 06/02/2017

---

<a id="install-the-net-framework-on-windows-8" class="xliff"></a>

# 在 Windows 8 上安裝 .NET Framework

您可能需要 .NET Framework，才能在 Windows 8 上執行應用程式。 您可以使用下列指示來協助您。 這些指示取決於您所需的 .NET Framework 版本。

<a id="net-framework-462" class="xliff"></a>

## .NET Framework 4.6.2

您無法在 Windows 8 上安裝 [.NET Framework 4.6.2](https://go.microsoft.com/fwlink/?linkid=845529&source=dotnetdocs) 或更新版本。

.NET Framework 4.6.2 隨附於 [Windows 10 年度更新](https://www.microsoft.com/software-download/windows10)。

.NET Framework 4.6.2 可用來執行針對 .NET Framework 4.0 或更新版本建置的應用程式。

<a id="net-framework-35" class="xliff"></a>

## .NET Framework 3.5

請依照指示[在 Windows 10 上安裝 .NET Framework 3.5](dotnet-35-windows-10.md)。

.NET Framework 3.5 可用來執行針對 .NET Framework 1.0 到 3.5 建置的應用程式。

<a id="help" class="xliff"></a>

## 說明

如果您無法安裝正確的 .NET Framework 版本，可以[連絡 Microsoft 以取得協助](mailto:dotnet-install-help@microsoft.com?subject=Help installing .NET Framework)。

