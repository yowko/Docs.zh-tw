...
description: 排除您遇到的阻止安裝 .NET 框架的問題。 有關解決問題的資訊，請參閱狀態訊息。
ms.date: 04/18/2019
...
ms.locfileid: "79506953"
---
# <a name="troubleshoot-blocked-net-framework-installations-and-uninstallations"></a>針對 .NET Framework 安裝和解除安裝遭封鎖的問題進行疑難排解

當您執行 .NET Framework 4.5 或更新版本的 [Web 或離線安裝程式](guide-for-developers.md)時，可能會遇到阻止或封鎖安裝 .NET Framework 的問題。 下面表格列出可能造成阻礙的問題並且提供疑難排解資訊。

在 Windows 8 (含) 以上版本中，.NET Framework 是作業系統元件，無法單獨解除安裝。 .NET Framework 的更新會出現在 [控制台] 之 [程式和功能]**** 應用程式的 [已安裝更新]**** 索引標籤中。 若是未預先安裝 .NET Framework 的作業系統，.NET Framework 會出現在 [主控台] 之 [程式和功能]**** 應用程式的 [解除安裝或​​變更程式]**** 索引標籤 (或 [新增/移除程式]**** 索引標籤) 中。 如需預先安裝 .NET Framework 之 Windows 版本的資訊，請參閱[系統需求](../get-started/system-requirements.md)。
...
您可以判斷系統上所安裝之 .NET Framework 的版本。 如需詳細資訊，請參閱[如何：判斷安裝的 .NET Framework 版本](../migration-guide/how-to-determine-which-versions-are-installed.md)。

下表中的 4.5.x 是指 .NET Framework 4.5 和其點發行版本，4.5.1 和 4.5.2、4.6.x 是指 .NET Framework 4.6 和其點發行版本 4.6.1 和 4.6.2，4.7.x 是指 .NET Framework 4.7 和其點發行版本 4.7.1 和 4.7.2，而 4.8 是指 .NET Framework 4.8。

|封鎖訊息|如需詳細資訊或解決這個問題|  
|----------------------|--------------------------------------------------|  
|解除安裝 Microsoft .NET Framework 可能會導致某些應用程式停止運作。|一般而言，您不應該解除安裝電腦上已安裝的任何 .NET Framework 版本，因為您使用的應用程式可能相依於特定的 .NET Framework 版本。 如需詳細資訊，請參閱*使用者入門*指南中的[適用於使用者的 .NET Framework](../get-started/index.md#ForUsers)。|  
|這部電腦已安裝 .NET Framework 4.5.x/4.6.x/4.7.x (繁體中文) 或更新版本。|不需採取任何動作。<br /><br /> 若要判斷系統上所安裝之 .NET Framework 的版本，請參閱[如何：判斷所安裝的 .NET Framework 版本](../migration-guide/how-to-determine-which-versions-are-installed.md)。|  
|.NET Framework 4.5.x/4.6.x/4.7.x/4.8 (「語言」**) 需要 .NET Framework 4.5.x/4.6.x/4.7.x/4.8。 請從下載中心安裝 .NET Framework 4.5.x/4.6.x/4.7.x/4.8，並重新執行安裝程式。|在安裝語言套件之前，必須安裝指定 .NET Framework 版本的英文版本。 如需詳細資訊，請參閱《安裝指南》中的[安裝語言套件](guide-for-developers.md#to-install-language-packs)一節。|  
...
|安裝程式無法執行，因為這部電腦沒有可用的 Windows Installer 服務。|[當您嘗試在 Windows 7 或 Microsoft 支援網站上安裝 Windows Vista 中的程式時，請參閱"無法訪問 Windows 安裝程式服務"錯誤](https://support.microsoft.com/help/2642495/the-windows-installer-service-could-not-be-accessed-error-when-you-try)。|  
|安裝程式無法正確執行，因為這部電腦沒有可用的 Windows Update 服務。|電腦可能是設定為使用 Windows Server Update Services (WSUS)，而不是使用 Microsoft Windows Update。 有關詳細資訊，請參閱 .NET 框架 3.5 安裝錯誤中錯誤代碼 0x800F0906 的部分[：0x800F0906、0x800F081F、0x800F0907](https://support.microsoft.com/help/2734782/net-framework-3-5-installation-error-0x800f0906-0x800f081f-0x800f0907)。<br /><br /> 另請參閱[如何將 Windows 更新代理更新到](https://support.microsoft.com/help/949104/how-to-update-the-windows-update-agent-to-the-latest-version)Microsoft 支援網站上的最新版本。|  
|安裝程式無法正確執行，因為這部電腦沒有可用的背景智慧型傳送服務 (BITS)。|請參閱 可在 Microsoft 支援網站上[修復基於 Windows Vista 的電腦上的後臺智慧傳輸服務 （BITS） 崩潰](https://support.microsoft.com/help/940520/an-update-is-available-to-fix-a-background-intelligent-transfer-servic)的更新。|  
...
|.NET Framework 4.5..x/4.6.x/4.7.x/4.8 已經包含在此作業系統中。 您不需要安裝 .NET Framework 4.5.x/4.6.x/4.7.x/4.8 可轉散發套件。|不進行動作。<br /><br /> 若要判斷系統上所安裝之 .NET Framework 的版本，請參閱[如何：判斷所安裝的 .NET Framework 版本](../migration-guide/how-to-determine-which-versions-are-installed.md)。 如需支援的作業系統，請參閱[系統需求](../get-started/system-requirements.md)。|  
...
|.NET 框架 4.5.x 已是此作業系統的一部分，但當前已關閉（僅限 Windows Server 2012）。| 在 **"控制台**"中使用**打開或關閉 Windows 功能**來打開 .NET 框架 4.5.x。 |  
...
1. 在 [名稱] 資料行中，根據您安裝的版本尋找 .NET Framework 4.5、4.5.1、4.5.2、4.6、4.6.1、4.6.2、4.7、4.7.1 或 4.7.2 下載名稱，然後刪除這些項目。 如需下載名稱，請參閱[安裝適用於開發人員的 .NET Framework](guide-for-developers.md)一文。

1. 請重新執行 4.5、4.5.1、4.5.2、4.6、4.6.1、4.6.2、4.7、4.7.1 或 4.7.2 版的 .NET Framework 安裝程式。

## <a name="see-also"></a>另請參閱

- [安裝適用於開發人員的 .NET Framework](guide-for-developers.md)
- [作法：判斷安裝的 .NET Framework 版本](../migration-guide/how-to-determine-which-versions-are-installed.md)
- [版本和依賴項](../migration-guide/versions-and-dependencies.md)
