---
title: "XML 序列化程式產生器工具 (Sgen.exe) | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
...
|選項|描述|  
|--------|--------|  
|**\/a**\[**ssembly**\]**:***檔案名稱*|為組件中包含的所有型別或 *filename* 所指定的可執行檔產生序列化程式碼。  只能提供一個檔名。  如果重複使用這個引數，則會使用最後一個檔名。|  
|**\/c\[ompiler\]:** *選項*|指定要傳遞至 C\# 編譯器的選項。  所有 csc.exe 選項都受到支援，可以傳遞至編譯器。  這個選項可以用來指定組件必須經過簽署，並指定金鑰檔。|  
|**\/d**\[**ebug**\]|產生可以與偵錯工具搭配使用的影像。|  
|**\/f\[orce\]**|強制覆寫現有的同名組件。  預設為 **false**。|  
|**\/help or \/?**|顯示工具的命令語法和選項。|  
|**\/k**\[**eep**\]|將產生的原始程式檔 \(Source File\) 和其他暫存檔案編譯成序列化組件之後，隱藏刪除這些檔案的動作。  這個選項可以用來判斷工具是否正在為特定的型別產生序列化程式碼。|  
|**\/n**\[**ologo**\]|隱藏顯示 Microsoft 程式啟始資訊。|  
|**\/o**\[**ut**\]**:***路徑*|指定要在其中儲存所產生之組件的目錄。 **Note:**  產生的組件名稱是由輸入組件的名稱加上 "xmlSerializers.dll" 所組成。|  
|**\/p**\[**roxytypes**\]|只為 XML Web 服務 Proxy 型別產生序列化程式碼。|  
|**\/r**\[**eference**\]**:***組件檔案*|指定要求 XML 序列化的型別所參考的組件。  這個選項接受以逗號分隔多個組件檔案。|  
|**\/s**\[**ilent**\]|隱藏顯示成功訊息。|  
|**\/t**\[**ype**\]**:***類型*|只為指定的型別產生序列化程式碼。|  
...
## 請參閱  
 ...
 [XML Web Services Overview](http://msdn.microsoft.com/zh-tw/9db0c7b8-bca6-462b-9be5-f5f9a7f05a4d)   
