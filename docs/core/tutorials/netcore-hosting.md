...
由於主機是本機應用程式，本教程介紹構建C++應用程式來承載 .NET Core。 您將需要 C++ 開發環境 (例如 [Visual Studio](https://aka.ms/vsdownload?utm_source=mscom&utm_campaign=msdocs) 所提供的環境)。
...
有三個不同的 API 可用來裝載 .NET Core。 本文（及其相關[示例](https://github.com/dotnet/samples/tree/master/core/hosting)）涵蓋了所有選項。
...
`nethost` 程式庫會提供用來找出 `hostfxr` 程式庫的 `get_hostfxr_path` 函式。 `hostfxr` 程式庫會公開用來裝載 .NET Core 執行階段的函式。 函數的完整清單可以在[`hostfxr.h`](https://github.com/dotnet/core-setup/blob/master/src/corehost/cli/hostfxr.h)和[本機宿主設計文檔中](https://github.com/dotnet/core-setup/blob/master/Documentation/design-docs/native-hosting.md)找到。 範例和本教課程會使用下列項目：

* `hostfxr_initialize_for_runtime_config`：初始化主機上下文，並準備使用指定的運行時配置初始化 .NET Core 運行時。
* `hostfxr_get_runtime_delegate`：獲取運行時功能的委託。
* `hostfxr_close`：關閉主機上下文。
...
找到後，庫將載入（`LoadLibraryEx`在 Windows 上）`dlopen`或（在 Linux/macOS 上）。
...
* `coreclr_initialize`：啟動 .NET 核心運行時並設置預設（和僅）AppDomain。
* `coreclr_execute_assembly`：執行託管程式集。
* `coreclr_create_delegate`： 創建指向託管方法的函數指標。
* `coreclr_shutdown`：關閉 .NET 核心運行時。
* `coreclr_shutdown_2`：喜歡`coreclr_shutdown`，但也檢索託管代碼的結束代碼。

載入 CoreCLR 庫後，下一步是使用`GetProcAddress`（在 Windows 上）或`dlsym`（在 Linux/macOS 上）獲取對這些函數的引用。
...
* `PLATFORM_RESOURCE_ROOTS`此清單包括要用於資源附屬程式集的探查路徑（在特定于區域性的子目錄中）。
...
`ICLRRuntimeHost4` .NET Core 裝載介面定義於 [MSCOREE.IDL](https://github.com/dotnet/runtime/blob/master/src/coreclr/src/inc/MSCOREE.IDL)。 您的主機必須參考此檔案的標頭版本 (mscoree.h)，該檔案會在建置 [.NET Core 執行階段](https://github.com/dotnet/runtime/)時透過 MIDL 產生。 如果不想生成 .NET Core 運行時，mscoree.h 也可以作為[預構建的標頭](https://github.com/dotnet/runtime/blob/master/src/coreclr/src/pal/prebuilt/inc/)在 dotnet/運行時存儲庫中使用。

### <a name="step-1---identify-the-managed-entry-point"></a>步驟 1 - 識別 Managed 進入點
參考必要的標頭 (例如 [mscoree.h](https://github.com/dotnet/runtime/blob/master/src/coreclr/src/pal/prebuilt/inc/mscoree.h) 和 stdio.h) 之後，.NET Core 主機必須先找出所要使用的 Managed 進入點。 在我們的示例主機中，只需將第一個命令列參數作為將執行其方法的`main`託管二進位檔案的路徑來實現。
...
在Linux或macOS上，核心運行時庫分別是*libcoreclr.so*或*libcoreclr.dylib。*

我們的範例主機會在一些常見的位置中探查 *CoreCLR.dll*。 一旦找到它，它必須通過`LoadLibrary`（或`dlopen`Linux/macOS）載入。
...
託管`ICLRRuntimeHost4`介面通過調用`GetProcAddress`（或`dlsym`在 Linux/macOS 上）檢索`GetCLRRuntimeHost`，然後調用該函數。
...
* `TRUSTED_PLATFORM_ASSEMBLIES`這是程式集路徑的清單（在`;`Windows 和`:`Linux/macOS 上劃定），AppDomain 應優先載入並給予完全信任（即使在部分受信任的域中）。 此清單可用來包含 'Framework' 組件及其他信任的模組，類似於 .NET Framework 案例中的 GAC。 某些主機會將任何程式庫放在此清單中的 *coreclr.dll* 旁，其他主機則會有針對其用途列出信任組件的硬式編碼資訊清單。
* `APP_PATHS`：這是在信賴平台組件 (TPA) 清單中找不到組件時，要在其中探查組件的路徑清單。 因為主機可以進一步控制使用 TPA 清單載入哪些組件，所以主機最好確定它們預期載入哪些組件，並明確列出這些組件。 不過，如果需要在執行階段進行探查，此屬性可用於這種情況。
...
如果最初操作不工作，請仔細檢查*coreclr.dll*是否在主機預期的位置可用，是否在所有必需的框架庫都位於 TPA 清單中，以及 CoreCLR 的位數（32 位或 64 位）是否與主機的構建方式相匹配。
