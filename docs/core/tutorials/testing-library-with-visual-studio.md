...
---
# <a name="tutorial-test-a-net-class-library-with-net-using-visual-studio"></a>教學課程：使用 Visual Studio 以 .NET 測試 .NET 類別庫

本教學課程示範如何將測試專案加入至方案，以自動化單元測試。

## <a name="prerequisites"></a>先決條件

- 本教學課程適用于 [使用 Visual Studio 建立 .net 類別庫](library-with-visual-studio.md)中所建立的方案。
...
1. 開啟 `ClassLibraryProjects` 您在 [使用 Visual Studio 建立 .net 類別庫](library-with-visual-studio.md)中建立的方案。
...
1. 將名為 "StringLibraryTest" 的新單元測試專案加入至方案。
   ...
   1. 選擇 [ **單元測試專案** ] 範本，然後選擇 [ **下一步]**。

   1. 在 [**設定您的新專案**] 頁面的 [**專案名稱**] 方塊中，輸入 **StringLibraryTest** 。 然後選擇 [下一步]  。
...
如果您使用 Visual Studio 作為 IDE，您可以使用教學課程：使用您的單元測試專案來進行程式碼 [的偵錯工具](debugging-with-visual-studio.md) 中所示的相同程式：使用 Visual Studio 來對程式碼進行程式碼處理。 請以滑鼠右鍵按一下 **StringLibraryTests** 專案，然後從內容功能表中選取 [ **Debug 測試**]，而不是啟動 *展示* 應用程式專案。
