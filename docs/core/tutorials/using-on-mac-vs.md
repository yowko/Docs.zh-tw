...
Visual Studio for Mac 針對開發 .NET Core 應用程式，提供功能完整的整合式開發環境 (IDE)。 本文會逐步引導您使用 Visual Studio for Mac 和 .NET Core 來建立簡單的主控台應用程式。
...
請參閱[.Net Core 相依性和需求](../install/dependencies.md?pivots=os-macos)一文。
