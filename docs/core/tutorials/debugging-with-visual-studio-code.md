...
中斷點位於 `Console.ReadLine` 方法呼叫之後。 **偵錯主控台**不接受執行程式的終端機輸入。 若要在進行偵錯工具時處理終端機輸入，您可以使用整合式終端 (其中一個 Visual Studio Code windows) 或外部終端機。 在本教學課程中，您將使用整合式終端機。

1. 開啟 *vscode/launch.js開啟*]。
