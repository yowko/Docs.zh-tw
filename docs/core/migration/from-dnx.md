---
title: "從 DNX 移轉到 .NET Core CLI | Microsoft Docs"
...
# 從 DNX 移轉到 .NET Core CLI (project.json)
...
一旦您決定要使用何種可攜性類型時，即需要變更目標架構。 如果您撰寫過 .NET Core 的應用程式，您很可能會使用 `dnxcore50` 做為目標架構。 由新的 [.NET Standard](https://github.com/dotnet/corefx/blob/master/Documentation/architecture/net-platform-standard.md) \(英文\) 所帶來的 CLI 和變更，使得架構必須為下列其中一項：
