...
> [!IMPORTANT]
> 如果您收到類似 [**找不到封裝 dotnet-sdk-3.1**] 的錯誤訊息，請參閱[APT 疑難排解](#apt-troubleshooting)一節。
...
> [!IMPORTANT]
> 如果您收到類似 [**找不到套件 aspnetcore-執行時間-3.1**] 的錯誤訊息，請參閱[APT 疑難排解](#apt-troubleshooting)一節。

除了 ASP.NET Core 執行時間之外，您還可以安裝不包含 ASP.NET Core 支援的 .NET Core 執行時間：在 `aspnetcore-runtime-3.1` 上述命令中以取代 `dotnet-runtime-3.1` 。
