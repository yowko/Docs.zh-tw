...
- **開發︰**逐一查看變更的速度以及偵錯變更的能力。 映像大小不那麼重要，而是變更及查看程式碼的速度。 我們有些工具，像是用於 Visual Studio Code 的 [yo docker](https://aka.ms/yodocker)，會在開發期間使用此映像。 
...
在 Visual Studio Code 左側的 [檢視] 列中選取偵錯圖示。
