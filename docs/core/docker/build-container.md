...
> [!WARNING]
> **本教學課程不適用於 ASP.NET Core 應用程式。** 如果您使用 ASP.NET Core，請閱讀[瞭解如何容器化 ASP.NET Core 應用程式](/aspnet/core/host-and-deploy/docker/building-net-docker-images)教學課程。
...
如果您使用的`ls`是 Linux 或 macOS，請使用命令來取得目錄清單，並確認已建立*myapp .dll*檔案。
...
在您的終端機中，流覽至您在一開始建立的工作資料夾所在的一個目錄。 在工作資料夾中建立名為 *Dockerfile* 的檔案，然後在文字編輯器中開啟它。 視您要容器化的應用程式類型而定，您將選擇 ASP.NET Core 執行時間或 .NET Core 執行時間。 若不確定，請選擇包含 .NET Core 執行時間的 ASP.NET Core 執行時間。 本教學課程將使用 ASP.NET Core 執行時間映射，但在上一節中建立的應用程式是 .NET Core 應用程式。
...
Docker 將會處理 *Dockerfile* 中的每一行。 命令中的`.`會指示 Docker 使用目前的資料夾來尋找 Dockerfile。 *Dockerfile* `docker build` 此命令會建置映像，並建立名為 **myimage** 的本機存放庫以指向該映像。 當此命令完成之後，執行 `docker images` 以查看已安裝的映像清單：
...
在此範例中，`ENTRYPOINT` 會變更為 `cmd.exe`。 <kbd>CTRL</kbd> +已按下 CTRL<kbd>C</kbd>來結束處理常式並停止容器。
