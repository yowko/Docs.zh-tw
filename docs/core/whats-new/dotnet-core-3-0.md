...
- [<span data-ttu-id="a31a1-113">.NET Core 3.0 Preview 1 公告</span><span class="sxs-lookup"><span data-stu-id="a31a1-113">.NET Core 3.0 Preview 1 announcement</span></span>](https://devblogs.microsoft.com/dotnet/announcing-net-core-3-preview-1-and-open-sourcing-windows-desktop-frameworks/)
...
### <a name="async-streams"></a><span data-ttu-id="a31a1-126">非同步資料流</span><span class="sxs-lookup"><span data-stu-id="a31a1-126">Async streams</span></span>
...
<span data-ttu-id="a31a1-150">仍有一些情況，*switch 陳述式*比起 *switch 運算式*將是更好的選擇，而且模式可與這兩種語法樣式搭配使用。</span><span class="sxs-lookup"><span data-stu-id="a31a1-150">There will still be cases where *switch statements* will be a better choice than *switch expressions* and patterns can be used with both syntax styles.</span></span>
...
* [<span data-ttu-id="a31a1-180">實作簡單 SSE2 硬體內建</span><span class="sxs-lookup"><span data-stu-id="a31a1-180">Implement simple SSE2 hardware intrinsics</span></span>](https://github.com/dotnet/coreclr/pull/15585)
...
<span data-ttu-id="a31a1-194">`dotnet build` 現在會從 NuGet 快取將您應用程式的 NuGet 相依性複製到組建輸出資料夾。</span><span class="sxs-lookup"><span data-stu-id="a31a1-194">`dotnet build` now copies NuGet dependencies for your application from the NuGet cache to the build output folder.</span></span> <span data-ttu-id="a31a1-195">先前，只有在進行 `dotnet publish` 的過程中，才會複製相依性。</span><span class="sxs-lookup"><span data-stu-id="a31a1-195">Previously, dependencies were only copied as part of `dotnet publish`.</span></span>
...
<span data-ttu-id="a31a1-212">若要建立 `dotnet-tools.json` 資訊清單，請使用：</span><span class="sxs-lookup"><span data-stu-id="a31a1-212">To create a `dotnet-tools.json` manifest file, use:</span></span>
...
<span data-ttu-id="a31a1-222">工具資訊清單檔的設計目的是要允許手動編輯 – 當您要更新與存放庫搭配運作所需的版本時，可能會這麼做。</span><span class="sxs-lookup"><span data-stu-id="a31a1-222">The tool manifest file is designed to allow hand editing – which you might do to update the required version for working with the repository.</span></span> <span data-ttu-id="a31a1-223">以下是一個範例 `dotnet-tools.json` 檔案：</span><span class="sxs-lookup"><span data-stu-id="a31a1-223">Here is an example `dotnet-tools.json` file:</span></span>
...
<span data-ttu-id="a31a1-241">如果應用程式同時使用這兩種架構 (例如當 Windows Forms 對話方塊裝載 WPF 控制項時)，則可以將 `UseWPF` 和 `UseWindowsForms` 都設定為 `true`。</span><span class="sxs-lookup"><span data-stu-id="a31a1-241">Both `UseWPF` and `UseWindowsForms` can be set to `true` if the app uses both frameworks, for example when a Windows Forms dialog is hosting a WPF control.</span></span>
...
<span data-ttu-id="a31a1-251">新的內建 JSON 支援是高效能、低配置，並以 `Span<byte>` 為基礎。</span><span class="sxs-lookup"><span data-stu-id="a31a1-251">The new built-in JSON support is high-performance, low allocation, and based on `Span<byte>`.</span></span> <span data-ttu-id="a31a1-252">三個新的主要 JSON 相關類型已新增到 .NET Core 3.0 `System.Text.Json` 命名空間。</span><span class="sxs-lookup"><span data-stu-id="a31a1-252">Three new main JSON-related types have been added to .NET Core 3.0 the `System.Text.Json` namespace.</span></span>
...
<span data-ttu-id="a31a1-290">.NET Framework 透過應用程式定義域 (AppDomain) 提供了組件卸載功能，但不支援其與 .NET Core 搭配。</span><span class="sxs-lookup"><span data-stu-id="a31a1-290">Assembly unloadability was provided in the .NET Framework by Application Domains (AppDomains), which are not supported with .NET Core.</span></span> <span data-ttu-id="a31a1-291">相較於這個新模型，AppDomain 既有優點也有限制。</span><span class="sxs-lookup"><span data-stu-id="a31a1-291">AppDomains had both benefits and limitations compared to this new model.</span></span> <span data-ttu-id="a31a1-292">相較於 AppDomain 時，若需要更多彈性和更高效能，請考慮這個新的載入器模型。</span><span class="sxs-lookup"><span data-stu-id="a31a1-292">Consider this new loader model to be more flexible and higher performant when compared to AppDomains.</span></span>
...
* <span data-ttu-id="a31a1-323">因為用戶端與伺服器之間的來回行程次數減少，所以改善了連線時間。</span><span class="sxs-lookup"><span data-stu-id="a31a1-323">Improved connection times due to a reduction in the number of round trips required between the client and server.</span></span>
...
* <span data-ttu-id="a31a1-359">IPEndPoint 剖析</span><span class="sxs-lookup"><span data-stu-id="a31a1-359">IPEndPoint parsing</span></span>
* <span data-ttu-id="a31a1-360">RandomNumberGenerator.GetInt32</span><span class="sxs-lookup"><span data-stu-id="a31a1-360">RandomNumberGenerator.GetInt32</span></span>

## <a name="tiered-compilation"></a><span data-ttu-id="a31a1-361">階層式編譯</span><span class="sxs-lookup"><span data-stu-id="a31a1-361">Tiered compilation</span></span>

<span data-ttu-id="a31a1-362">使用 .NET Core 3.0 時，預設會開啟[階層式編譯](https://devblogs.microsoft.com/dotnet/tiered-compilation-preview-in-net-core-2-1/)。</span><span class="sxs-lookup"><span data-stu-id="a31a1-362">[Tiered compilation](https://devblogs.microsoft.com/dotnet/tiered-compilation-preview-in-net-core-2-1/) is on by default with .NET Core 3.0.</span></span> <span data-ttu-id="a31a1-363">這是一項可讓執行階段更彈性使用 Just-In-Time (JIT) 編譯器的功能，可在啟動時及將輸送量最大化時獲得更佳的效能。</span><span class="sxs-lookup"><span data-stu-id="a31a1-363">It is a feature that enables the runtime to more adaptively use the Just-In-Time (JIT) compiler to get better performance, both at startup and to maximize throughput.</span></span>

<span data-ttu-id="a31a1-364">此功能在 [.NET Core 2.1](https://devblogs.microsoft.com/dotnet/announcing-net-core-2-1/) 中是新增為選用功能，然後在 [.NET Core 2.2 Preview 2](https://devblogs.microsoft.com/dotnet/announcing-net-core-2-2-preview-2/) 中為預設啟用的功能。</span><span class="sxs-lookup"><span data-stu-id="a31a1-364">This feature was added as an opt-in feature in [.NET Core 2.1](https://devblogs.microsoft.com/dotnet/announcing-net-core-2-1/) and then was enabled by default in [.NET Core 2.2 Preview 2](https://devblogs.microsoft.com/dotnet/announcing-net-core-2-2-preview-2/).</span></span> <span data-ttu-id="a31a1-365">之後，在 .NET Core 2.2 版本中則又還原回選用功能。</span><span class="sxs-lookup"><span data-stu-id="a31a1-365">Subsequently, it has been reverted back to opt in with the .NET Core 2.2 release.</span></span>

## <a name="arm64-linux-support"></a><span data-ttu-id="a31a1-366">ARM64 Linux 支援</span><span class="sxs-lookup"><span data-stu-id="a31a1-366">ARM64 Linux support</span></span>

<span data-ttu-id="a31a1-367">已新增 ARM64 for Linux 的支援。</span><span class="sxs-lookup"><span data-stu-id="a31a1-367">Support has been added for ARM64 for Linux.</span></span> <span data-ttu-id="a31a1-368">ARM64 的主要使用案例目前是搭配 IoT 案例。</span><span class="sxs-lookup"><span data-stu-id="a31a1-368">The primary use case for ARM64 is currently with IoT scenarios.</span></span>

<span data-ttu-id="a31a1-369">[針對適用於 ARM64 的 .NET Core 有提供 Docker 映像](https://hub.docker.com/r/microsoft/dotnet/) (Alpine、Debian 及 Ubuntu Docker 映像)。</span><span class="sxs-lookup"><span data-stu-id="a31a1-369">Alpine, Debian and Ubuntu [Docker images are available for .NET Core for ARM64](https://hub.docker.com/r/microsoft/dotnet/).</span></span>

<span data-ttu-id="a31a1-370">如需詳細資訊，請參閱 [.NET Core ARM64 狀態](https://github.com/dotnet/announcements/issues/82) \(英文\)。</span><span class="sxs-lookup"><span data-stu-id="a31a1-370">Please check [.NET Core ARM64 Status](https://github.com/dotnet/announcements/issues/82) for more information.</span></span>

>[!NOTE]
> <span data-ttu-id="a31a1-371">目前尚未提供 **ARM64** Windows 支援。</span><span class="sxs-lookup"><span data-stu-id="a31a1-371">**ARM64** Windows support isn't yet available.</span></span>

## <a name="install-net-core-30-previews-on-linux-with-snap"></a><span data-ttu-id="a31a1-372">在 Linux 上使用嵌入式管理單元安裝.NET Core 3.0 Preview</span><span class="sxs-lookup"><span data-stu-id="a31a1-372">Install .NET Core 3.0 Previews on Linux with Snap</span></span>

<span data-ttu-id="a31a1-373">嵌入式管理單元是慣用方法，用來在 [支援嵌入式管理單元的 Linux 散發套件](https://docs.snapcraft.io/installing-snapd/6735)上安裝和嘗試 .NET Core Preview。</span><span class="sxs-lookup"><span data-stu-id="a31a1-373">Snap is the preferred way to install and try .NET Core previews on [Linux distributions that support Snap](https://docs.snapcraft.io/installing-snapd/6735).</span></span>

<span data-ttu-id="a31a1-374">在您的系統上設定嵌入式管理單元之後，請執行下列命令來安裝 [.NET Core SDK 3.0 Preview SDK](https://snapcraft.io/dotnet-sdk)。</span><span class="sxs-lookup"><span data-stu-id="a31a1-374">After configuring Snap on your system, run the following command to install the [.NET Core SDK 3.0 Preview SDK](https://snapcraft.io/dotnet-sdk).</span></span>

```console
sudo snap install dotnet-sdk --beta --classic
```

<span data-ttu-id="a31a1-375">使用嵌入式管理單元套件安裝 .NET Core 時，預設 .NET Core 命令是 `dotnet-sdk.dotnet`，而非只是 `dotnet`。</span><span class="sxs-lookup"><span data-stu-id="a31a1-375">When .NET Core in installed using the Snap package, the default .NET Core command is `dotnet-sdk.dotnet`, as opposed to just `dotnet`.</span></span> <span data-ttu-id="a31a1-376">命名空間命令的優點是，不會與您可能已全域安裝的 .NET Core 版本發生衝突。</span><span class="sxs-lookup"><span data-stu-id="a31a1-376">The benefit of the namespaced command is that it will not conflict with a globally installed .NET Core version you may have.</span></span> <span data-ttu-id="a31a1-377">可以使用下列方式，將這個命令的別名設為 `dotnet`：</span><span class="sxs-lookup"><span data-stu-id="a31a1-377">This command can be aliased to `dotnet` with:</span></span>

```console
sudo snap alias dotnet-sdk.dotnet dotnet
```

<span data-ttu-id="a31a1-378">某些散發套件需要額外的步驟來啟用 SSL 憑證的存取。</span><span class="sxs-lookup"><span data-stu-id="a31a1-378">Some distros require an additional step to enable access to the SSL certificate.</span></span> <span data-ttu-id="a31a1-379">如需詳細資料，請參閱我們的 [Linux 安裝程式](https://github.com/dotnet/core/blob/master/Documentation/linux-setup.md)。</span><span class="sxs-lookup"><span data-stu-id="a31a1-379">See our [Linux Setup](https://github.com/dotnet/core/blob/master/Documentation/linux-setup.md) for details.</span></span>

## <a name="gpio-support-for-raspberry-pi"></a><span data-ttu-id="a31a1-380">Raspberry Pi 的 GPIO 支援</span><span class="sxs-lookup"><span data-stu-id="a31a1-380">GPIO Support for Raspberry Pi</span></span>

<span data-ttu-id="a31a1-381">兩個新的套件已發佈至您可以用於 GPIO 程式設計的 NuGet。</span><span class="sxs-lookup"><span data-stu-id="a31a1-381">Two new packages have been released to NuGet that you can use for GPIO programming.</span></span>

* [<span data-ttu-id="a31a1-382">System.Device.Gpio</span><span class="sxs-lookup"><span data-stu-id="a31a1-382">System.Device.Gpio</span></span>](https://www.nuget.org/packages/System.Device.Gpio/0.1.0-prerelease.19078.2)
* [<span data-ttu-id="a31a1-383">Iot.Device.Bindings</span><span class="sxs-lookup"><span data-stu-id="a31a1-383">Iot.Device.Bindings</span></span>](https://www.nuget.org/packages/Iot.Device.Bindings/0.1.0-prerelease.19078.2)

<span data-ttu-id="a31a1-384">GPIO 套件包含 GPIO、SPI、I2C 和 PWM 裝置的 API。</span><span class="sxs-lookup"><span data-stu-id="a31a1-384">The GPIO Packages includes APIs for GPIO, SPI, I2C and PWM devices.</span></span> <span data-ttu-id="a31a1-385">IoT 繫結套件包含各種晶片和感應器的[裝置繫結](https://github.com/dotnet/iot/blob/master/src/devices/README.md)，與 [dotnet/iot - src/devices](https://github.com/dotnet/iot/tree/master/src/devices) 中提供的裝置繫結相同。</span><span class="sxs-lookup"><span data-stu-id="a31a1-385">The IoT bindings package includes [device bindings](https://github.com/dotnet/iot/blob/master/src/devices/README.md) for various chips and sensors, the same ones available at [dotnet/iot - src/devices](https://github.com/dotnet/iot/tree/master/src/devices).</span></span>

<span data-ttu-id="a31a1-386">已宣佈為.NET Core 3.0 Preview 1 一部分的更新序列連接埠，不是這些套件的一部分，但可以作為 .NET Core 平台的一部分。</span><span class="sxs-lookup"><span data-stu-id="a31a1-386">The updated serial port APIs that were announced as part of .NET Core 3.0 Preview 1 are not part of these packages but are available as part of the .NET Core platform.</span></span>

## <a name="platform-support"></a><span data-ttu-id="a31a1-387">平台支援</span><span class="sxs-lookup"><span data-stu-id="a31a1-387">Platform Support</span></span>

<span data-ttu-id="a31a1-388">以下作業系統將支援 .NET Core 3：</span><span class="sxs-lookup"><span data-stu-id="a31a1-388">.NET Core 3 will be supported on the following operating systems:</span></span>

* <span data-ttu-id="a31a1-389">Windows 用戶端：7、8.1、10 (1607+)</span><span class="sxs-lookup"><span data-stu-id="a31a1-389">Windows Client: 7, 8.1, 10 (1607+)</span></span>
* <span data-ttu-id="a31a1-390">Windows Server：2012 R2 SP1+</span><span class="sxs-lookup"><span data-stu-id="a31a1-390">Windows Server: 2012 R2 SP1+</span></span>
* <span data-ttu-id="a31a1-391">macOS：10.12+</span><span class="sxs-lookup"><span data-stu-id="a31a1-391">macOS: 10.12+</span></span>
* <span data-ttu-id="a31a1-392">RHEL：6+</span><span class="sxs-lookup"><span data-stu-id="a31a1-392">RHEL: 6+</span></span>
* <span data-ttu-id="a31a1-393">Fedora：26+</span><span class="sxs-lookup"><span data-stu-id="a31a1-393">Fedora: 26+</span></span>
* <span data-ttu-id="a31a1-394">Ubuntu：16.04+</span><span class="sxs-lookup"><span data-stu-id="a31a1-394">Ubuntu: 16.04+</span></span>
* <span data-ttu-id="a31a1-395">Debian：9+</span><span class="sxs-lookup"><span data-stu-id="a31a1-395">Debian: 9+</span></span>
* <span data-ttu-id="a31a1-396">SLES：12+</span><span class="sxs-lookup"><span data-stu-id="a31a1-396">SLES: 12+</span></span>
* <span data-ttu-id="a31a1-397">openSUSE：42.3+</span><span class="sxs-lookup"><span data-stu-id="a31a1-397">openSUSE: 42.3+</span></span>
* <span data-ttu-id="a31a1-398">Alpine：3.8+</span><span class="sxs-lookup"><span data-stu-id="a31a1-398">Alpine: 3.8+</span></span>

<span data-ttu-id="a31a1-399">晶片支援如下：</span><span class="sxs-lookup"><span data-stu-id="a31a1-399">Chip support follows:</span></span>

* <span data-ttu-id="a31a1-400">Windows、macOS 及 Linux 上的 x64</span><span class="sxs-lookup"><span data-stu-id="a31a1-400">x64 on Windows, macOS, and Linux</span></span>
* <span data-ttu-id="a31a1-401">Windows 上的 x86</span><span class="sxs-lookup"><span data-stu-id="a31a1-401">x86 on Windows</span></span>
* <span data-ttu-id="a31a1-402">Windows 和 Linux 上的 ARM32</span><span class="sxs-lookup"><span data-stu-id="a31a1-402">ARM32 on Windows and Linux</span></span>
* <span data-ttu-id="a31a1-403">Linux 上的 ARM64</span><span class="sxs-lookup"><span data-stu-id="a31a1-403">ARM64 on Linux</span></span>

<span data-ttu-id="a31a1-404">若為 Linux，Debian 9+ 和 Ubuntu 16.04+ 支援 ARM32。</span><span class="sxs-lookup"><span data-stu-id="a31a1-404">For Linux, ARM32 is supported on Debian 9+ and Ubuntu 16.04+.</span></span> <span data-ttu-id="a31a1-405">若為 ARM64，與 ARM32 相同，但加上 Alpine 3.8。</span><span class="sxs-lookup"><span data-stu-id="a31a1-405">For ARM64, it is the same as ARM32 with the addition of Alpine 3.8.</span></span> <span data-ttu-id="a31a1-406">這些與 X64 支援的散發套件版本相同。</span><span class="sxs-lookup"><span data-stu-id="a31a1-406">These are the same versions of those distros as is supported for X64.</span></span>

<span data-ttu-id="a31a1-407">.NET Core 3.0 的 Docker 映像位於 [Docker Hub 上的 microsoft/dotnet](https://hub.docker.com/r/microsoft/dotnet/)。</span><span class="sxs-lookup"><span data-stu-id="a31a1-407">Docker images for .NET Core 3.0 are available at [microsoft/dotnet on Docker Hub](https://hub.docker.com/r/microsoft/dotnet/).</span></span> <span data-ttu-id="a31a1-408">Microsoft 目前正在採用 [Microsoft Container Registry (MCR)](https://cloudblogs.microsoft.com/opensource/2019/01/17/improved-discovery-experience-microsoft-containers-docker-hub/)，而且預期最終 .NET Core 3.0 映像只會發佈至 MCR。</span><span class="sxs-lookup"><span data-stu-id="a31a1-408">Microsoft is currently in the process of adopting [Microsoft Container Registry (MCR)](https://cloudblogs.microsoft.com/opensource/2019/01/17/improved-discovery-experience-microsoft-containers-docker-hub/) and it is expected that the final .NET Core 3.0 images will only be published to MCR.</span></span>
