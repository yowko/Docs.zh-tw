...
從 .NET Core 2.1 開始，.NET CLI 可讓您選擇使用電腦上安裝的 SDK 和執行階段版本清單。  使用[`dotnet --list-sdks`](../tools/dotnet.md#options)查看電腦上安裝的 sdk 清單。 使用[`dotnet --list-runtimes`](../tools/dotnet.md#options)查看電腦上安裝的執行時間清單。 下列文字顯示 Windows、macOS 或 Linux 的一般輸出：
...
在 Linux 上，您必須移除已建立版本的目錄，分別移除 Sdk 和執行時間。 為清楚明瞭，這會從磁片刪除 SDK 和執行時間。 例如，若要移除 1.0.1 SDK 和執行階段，您可以使用下列 bash 命令：
...
在 Mac 上，您必須移除已建立版本的目錄，分別移除 Sdk 和執行時間。 為清楚明瞭，這會從磁片刪除 SDK 和執行時間。 例如，若要移除 1.0.1 SDK 和執行階段，您可以使用下列 bash 命令：
...
在 .NET Core 3.0 SDK 之前，.NET Core SDK 安裝程式會使用*NuGetFallbackFolder*來儲存 NuGet 套件的快取。 此快取是在作業期間使用`dotnet restore` ， `dotnet build /t:Restore`例如或。 位於`NuGetFallbackFolder` Windows 上的*C:\Program Files\dotnet\sdk*和 macOS 上的 */usr/local/share/dotnet/sdk* 。
