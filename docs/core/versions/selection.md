...
指定的 SDK 支援一組固定的架構，限制為其隨附執行階段的目標 Framework。 例如，.NET Core 3.0 SDK 包括 .NET Core 3.0 運行時，這是目標`netcoreapp3.0`框架的實現。 .NET Core 3.0 `netcoreapp2.1`SDK `netcoreapp2.2` `netcoreapp3.0`支援 、、不支援`netcoreapp3.1`（或更高）。 安裝 .NET 核心 3.1 SDK`netcoreapp3.1`來構建 。

.NET Standard 目標 Framework 也限制為 SDK 隨附執行階段的目標 Framework。 .NET 核心 3.1 SDK 被`netstandard2.1`限制為 。 如需詳細資訊，請參閱 [.NET Standard](../../standard/net-standard.md)。
...
如果您針對 3.0，一些使用示例演示了該行為：

- ✔️ 3.0 指定。 3.0.5 是安裝的最高修補程式版本。 使用 3.0.5。
- ❌指定 3.0。 未安裝 3.0.* 版本。 2.1.1 是安裝的最高運行時。 顯示錯誤訊息。
- ✔️ 3.0 指定。 未安裝 3.0.* 版本。 3.1.0 是安裝的最高執行階段版本。 使用 3.1.0。
- ❌指定 2.0。 未安裝 2.x 版本。 3.0.0 是安裝的最高運行時。 顯示錯誤訊息。

次要版本向前復原有一個可能會影響終端使用者的副作用。 請考慮下列案例：

1. 應用程式指定 3.0 是必需的。
2. 運行時，未安裝版本 3.0.*，但是，3.1.0 是。 將使用版本 3.1.0。
3. 稍後，使用者安裝 3.0.5 並再次運行應用程式，現在將使用 3.0.5。

3.0.5 和 3.1.0 的行為可能不同，尤其是對於序列化二進位資料等方案。
...
發佈過程會選取指定執行階段系列的最新修補程式版本。 例如，`dotnet publish`如果 .NET Core 3.0 運行時系列中的最新修補程式版本是 .NET Core 3.0.4，則選擇 .NET Core 3.0.4。 目標 Framework (包括最新安裝的安全性修補程式) 會封裝於應用程式。
...
- [下載並安裝 .NET 核心](../install/index.md)。
- [如何刪除 .NET 核心運行時和 SDK](remove-runtime-sdk-versions.md)。
