...
.NET core 會套用一組原則，以決定在各種案例中使用何種 .NET Core 執行階段和 SDK 的版本。 您可在[版本選取項目](selection.md)上的文章中完整探索這些案例及原則。

您可以將這些原則視為執行下列角色：

* 可簡單且有效率地部署 .NET Core，包括安全性和可靠性更新。
* 可讓開發人員使用最新的工具和命令，且不受目標執行階段影響。
