...
---
# <a name="most-code-access-security-apis-are-obsolete"></a><span data-ttu-id="2090e-103">大部分的代碼啟用安全性 Api 已淘汰</span><span class="sxs-lookup"><span data-stu-id="2090e-103">Most code access security APIs are obsolete</span></span>

<span data-ttu-id="2090e-104">在 .NET 中，大部分的代碼啟用安全性 (CAS) 相關類型現在已淘汰為警告。</span><span class="sxs-lookup"><span data-stu-id="2090e-104">Most code access security (CAS)-related types in .NET are now obsolete as warning.</span></span> <span data-ttu-id="2090e-105">這包括 CAS 屬性（例如）、 <xref:System.Security.Permissions.SecurityPermissionAttribute> <xref:System.Net.SocketPermission> <xref:System.Security.Policy.EvidenceBase> 衍生型別和其他支援的 api 等 cas 權限物件。</span><span class="sxs-lookup"><span data-stu-id="2090e-105">This includes CAS attributes, such as <xref:System.Security.Permissions.SecurityPermissionAttribute>, CAS permission objects, such as <xref:System.Net.SocketPermission>, <xref:System.Security.Policy.EvidenceBase>-derived types, and other supporting APIs.</span></span>

## <a name="change-description"></a><span data-ttu-id="2090e-106">變更描述</span><span class="sxs-lookup"><span data-stu-id="2090e-106">Change description</span></span>

<span data-ttu-id="2090e-107">在 .NET Framework 2.x-4.x 中，CAS 屬性和 Api 可能會影響程式碼執行的過程，包括確保 CAS 需求堆疊會成功或失敗。</span><span class="sxs-lookup"><span data-stu-id="2090e-107">In .NET Framework 2.x - 4.x, CAS attributes and APIs can influence the course of code execution, including ensuring that CAS-demand stack walks succeed or fail.</span></span>
...
<span data-ttu-id="2090e-108">在 .NET Core 2.x 中，執行時間不會接受 CAS 屬性或 CAS Api。</span><span class="sxs-lookup"><span data-stu-id="2090e-108">In .NET Core 2.x - 3.x, the runtime does not honor CAS attributes or CAS APIs.</span></span> <span data-ttu-id="2090e-109">執行時間會忽略方法專案上的屬性，而大部分的程式設計 Api 都沒有任何作用。</span><span class="sxs-lookup"><span data-stu-id="2090e-109">The runtime ignores attributes on method entry, and most programmatic APIs have no effect.</span></span>
...
<span data-ttu-id="2090e-110">此外，以程式設計方式呼叫廣泛的 Api (`Assert`) 一律會成功，而以程式設計方式呼叫受限制的 api (`Deny` ， `PermitOnly`) 一律會在執行時間擲回例外狀況。</span><span class="sxs-lookup"><span data-stu-id="2090e-110">Additionally, programmatic calls to expansive APIs (`Assert`) always succeed, while programmatic calls to restrictive APIs (`Deny`, `PermitOnly`) always throw an exception at run time.</span></span> <span data-ttu-id="2090e-111"> (<xref:System.Security.Permissions.PrincipalPermission> 是此規則的例外狀況。</span><span class="sxs-lookup"><span data-stu-id="2090e-111">(<xref:System.Security.Permissions.PrincipalPermission> is an exception to this rule.</span></span> <span data-ttu-id="2090e-112">請參閱下面的 [建議動作](#cas-action) 一節。 ) </span><span class="sxs-lookup"><span data-stu-id="2090e-112">See the [Recommended action](#cas-action) section below.)</span></span>
...
<span data-ttu-id="2090e-113">在 .NET 5.0 和更新版本中，大部分 CAS 相關的 Api 都會淘汰，並產生編譯時期警告 `SYSLIB0003` 。</span><span class="sxs-lookup"><span data-stu-id="2090e-113">In .NET 5.0 and later versions, most CAS-related APIs are obsolete and produce compile-time warning `SYSLIB0003`.</span></span>
...
<span data-ttu-id="2090e-114">這是僅限編譯時期的變更。</span><span class="sxs-lookup"><span data-stu-id="2090e-114">This is a compile-time only change.</span></span> <span data-ttu-id="2090e-115">舊版 .NET Core 沒有任何執行時間變更。</span><span class="sxs-lookup"><span data-stu-id="2090e-115">There is no run-time change from previous versions of .NET Core.</span></span> <span data-ttu-id="2090e-116">在 .NET Core 2.x-3.x 中執行沒有作業的方法，在 .NET 5.0 和更新版本中的執行時間不會繼續執行任何操作。</span><span class="sxs-lookup"><span data-stu-id="2090e-116">Methods that perform no operation in .NET Core 2.x - 3.x will continue to perform no operation at run time in .NET 5.0 and later.</span></span> <span data-ttu-id="2090e-117">在 .net Core 2.x-3.x 中擲回的方法， <xref:System.PlatformNotSupportedException> 將會在 <xref:System.PlatformNotSupportedException> 執行時間繼續于 .net 5.0 和更新版本中擲回。</span><span class="sxs-lookup"><span data-stu-id="2090e-117">Methods that throw <xref:System.PlatformNotSupportedException> in .NET Core 2.x - 3.x will continue to throw a <xref:System.PlatformNotSupportedException> at run time in .NET 5.0 and later.</span></span>

## <a name="reason-for-change"></a><span data-ttu-id="2090e-118">變更的原因</span><span class="sxs-lookup"><span data-stu-id="2090e-118">Reason for change</span></span>

<span data-ttu-id="2090e-119">[ (CAS) 的代碼啟用安全性 ](../../../../framework/misc/code-access-security.md) 是一種不受支援的舊版技術。</span><span class="sxs-lookup"><span data-stu-id="2090e-119">[Code access security (CAS)](../../../../framework/misc/code-access-security.md) is an unsupported legacy technology.</span></span> <span data-ttu-id="2090e-120">啟用 CA 的基礎結構只存在於 .NET Framework 2.x-4.x 中，但已被取代且未收到服務或安全性修正程式。</span><span class="sxs-lookup"><span data-stu-id="2090e-120">The infrastructure to enable CAS exists only in .NET Framework 2.x - 4.x, but is deprecated and not receiving servicing or security fixes.</span></span>

<span data-ttu-id="2090e-121">由於 CAS 的淘汰，支援的 [基礎結構尚未升級至 .Net Core](../../../porting/net-framework-tech-unavailable.md) 或 .net 5.0 +。</span><span class="sxs-lookup"><span data-stu-id="2090e-121">Due to CAS's deprecation, the [supporting infrastructure was not brought forward to .NET Core](../../../porting/net-framework-tech-unavailable.md) or .NET 5.0+.</span></span> <span data-ttu-id="2090e-122">不過，Api 已向前推進，讓應用程式可以針對 .NET Framework 和 .NET Core 進行交叉編譯。</span><span class="sxs-lookup"><span data-stu-id="2090e-122">However, the APIs were brought forward so that apps could cross-compile against .NET Framework and .NET Core.</span></span> <span data-ttu-id="2090e-123">這會導致「開啟開啟」案例，其中某些 CAS 相關的 Api 存在且可供呼叫，但在執行時間不會執行任何動作。</span><span class="sxs-lookup"><span data-stu-id="2090e-123">This led to "fail open" scenarios, where some CAS-related APIs exist and are callable but perform no action at run time.</span></span> <span data-ttu-id="2090e-124">這可能會對預期執行時間接受 CAS 相關屬性或程式設計 API 呼叫的元件產生安全性問題。</span><span class="sxs-lookup"><span data-stu-id="2090e-124">This can lead to security issues for components that expect the runtime to honor CAS-related attributes or programmatic API calls.</span></span> <span data-ttu-id="2090e-125">為了更妥善地溝通執行時間不遵守這些屬性或 Api，我們已在 .NET 5.0 中淘汰大部分的屬性或 Api。</span><span class="sxs-lookup"><span data-stu-id="2090e-125">To better communicate that the runtime doesn't respect these attributes or APIs, we have obsoleted the majority of them in .NET 5.0.</span></span>

## <a name="version-introduced"></a><span data-ttu-id="2090e-126">引進的版本</span><span class="sxs-lookup"><span data-stu-id="2090e-126">Version introduced</span></span>

<span data-ttu-id="2090e-127">5.0</span><span class="sxs-lookup"><span data-stu-id="2090e-127">5.0</span></span>

## <a name=""></a><span data-ttu-id="2090e-128"><a id="cas-action">建議的動作</a></span><span class="sxs-lookup"><span data-stu-id="2090e-128"><a id="cas-action">Recommended action</a></span></span>

- <span data-ttu-id="2090e-129">如果您要判斷提示任何安全性許可權，請移除會判斷提示許可權的屬性或呼叫。</span><span class="sxs-lookup"><span data-stu-id="2090e-129">If you're asserting any security permission, remove the attribute or call that asserts the permission.</span></span>
...
- <span data-ttu-id="2090e-130">如果您要透過) 任何許可權來拒絕或限制 (`PermitOnly` ，請洽詢您的安全性顧問。</span><span class="sxs-lookup"><span data-stu-id="2090e-130">If you're denying or restricting (via `PermitOnly`) any permission, contact your security advisor.</span></span> <span data-ttu-id="2090e-131">由於 CAS 屬性不是由 .NET 5.0 + 執行時間所採用，如果您的應用程式不正確依賴 CAS 基礎結構來限制這些方法的存取權，則可能會有安全性漏洞。</span><span class="sxs-lookup"><span data-stu-id="2090e-131">Because CAS attributes are not honored by the .NET 5.0+ runtime, your application could have a security hole if it incorrectly relies on the CAS infrastructure to restrict access to these methods.</span></span>
...
- <span data-ttu-id="2090e-132">如果您要求) 以外的任何許可權 (<xref:System.Security.Permissions.PrincipalPermission> ，請移除需求。</span><span class="sxs-lookup"><span data-stu-id="2090e-132">If you're demanding any permission (except <xref:System.Security.Permissions.PrincipalPermission>), remove the demand.</span></span> <span data-ttu-id="2090e-133">所有要求都會在執行時間成功。</span><span class="sxs-lookup"><span data-stu-id="2090e-133">All demands will succeed at run time.</span></span>
...
- <span data-ttu-id="2090e-134">如果您的需求 <xref:System.Security.Permissions.PrincipalPermission> 很嚴苛，請參閱 PrincipalPermissionAttribute 的指導方針 [已淘汰為錯誤](principalpermissionattribute-obsolete.md)。</span><span class="sxs-lookup"><span data-stu-id="2090e-134">If you're demanding <xref:System.Security.Permissions.PrincipalPermission>, consult the guidance for [PrincipalPermissionAttribute is obsolete as error](principalpermissionattribute-obsolete.md).</span></span> <span data-ttu-id="2090e-135">本指南適用于 <xref:System.Security.Permissions.PrincipalPermission> 和 <xref:System.Security.Permissions.PrincipalPermissionAttribute> 。</span><span class="sxs-lookup"><span data-stu-id="2090e-135">That guidance applies for both <xref:System.Security.Permissions.PrincipalPermission> and <xref:System.Security.Permissions.PrincipalPermissionAttribute>.</span></span>

- <span data-ttu-id="2090e-136">如果您絕對必須停用這些警告 (但) 不建議這麼做，您可以 `SYSLIB0003` 在程式碼中隱藏警告。</span><span class="sxs-lookup"><span data-stu-id="2090e-136">If you absolutely must disable these warnings (which is not recommended), you can suppress the `SYSLIB0003` warning in code.</span></span>
  ...
  <span data-ttu-id="2090e-137">您也可以隱藏專案檔中的警告。</span><span class="sxs-lookup"><span data-stu-id="2090e-137">You can also suppress the warning in your project file.</span></span> <span data-ttu-id="2090e-138">這麼做會停用專案內所有原始程式檔的警告。</span><span class="sxs-lookup"><span data-stu-id="2090e-138">Doing so disables the warning for all source files within the project.</span></span>
  ...
  > [!NOTE]
  > <span data-ttu-id="2090e-139">隱藏 `SYSLIB0003` 只會停用 CAS 相關的 obsoletion 警告。</span><span class="sxs-lookup"><span data-stu-id="2090e-139">Suppressing `SYSLIB0003` disables only the CAS-related obsoletion warnings.</span></span> <span data-ttu-id="2090e-140">它不會停用任何其他警告或變更 .NET 5.0 + 執行時間的行為。</span><span class="sxs-lookup"><span data-stu-id="2090e-140">It does not disable any other warnings or change the behavior of the .NET 5.0+ runtime.</span></span>
- <span data-ttu-id="2090e-141">安全性</span><span class="sxs-lookup"><span data-stu-id="2090e-141">Security</span></span>

## <a name="affected-apis"></a><span data-ttu-id="2090e-142">受影響的 API</span><span class="sxs-lookup"><span data-stu-id="2090e-142">Affected APIs</span></span>
