...
1. 建立專案。
   ...
   選擇 **"檔** > **新專案** > **"。** 在 [新增專案]**** 對話方塊中，展開 [已安裝]**** 專案類型窗格中的語言 (C# 或 Visual Basic) 專案類別，選擇 [.NET Core]****，然後選擇中央窗格中的 [主控台應用程式 (.NET Core)]**** 範本。 在 [名稱]**** 文字方塊中輸入專案名稱，例如 "FDD"。 選擇"**確定"** 按鈕。

1. 新增應用程式的原始程式碼。

   打開編輯器中的*Program.cs*或*程式.vb*檔，並將自動生成的代碼替換為以下代碼。 它會提示使用者輸入文字，並顯示使用者輸入的個別文字。 它會使用規則運算式 `\w+` 分隔輸入文字中的字詞。
...
1. 建立應用程式的偵錯組建。
   ...
   選擇**生成** > **生成解決方案**。 還可以通過選擇**調試** > **啟動調試**來編譯和運行應用程式的調試生成。
...
1. 部署您的應用程式。
   ...
   在您偵錯並測試程式之後，請建立要隨應用程式一起部署的檔案。 若要從 Visual Studio 發行，請執行下列作業：
      ...
      1. 以滑鼠右鍵按一下方案總管**** 中的專案 (而非方案)，然後選取 [發行]****。
...
1. 建立專案。
   ...
   選擇 **"檔** > **新專案** > **"。** 在 [新增專案]**** 對話方塊中，展開 [已安裝]**** 專案類型窗格中的語言 (C# 或 Visual Basic) 專案類別，選擇 [.NET Core]****，然後選擇中央窗格中的 [主控台應用程式 (.NET Core)]**** 範本。 在 [名稱]**** 文字方塊中輸入專案名稱 (例如 "SCD")，然後選取 [確定]**** 按鈕。
...
1. 建立應用程式的偵錯組建。
   ...
   選擇**生成** > **生成解決方案**。 還可以通過選擇**調試** > **啟動調試**來編譯和運行應用程式的調試生成。 透過此偵錯步驟，您可以識別應用程式在主機平台上執行時的問題。 您仍然必須在每個目標平台上進行測試。
...
1. 定義您應用程式目標的平台。
   ...
   該`<RuntimeIdentifiers>`元素可以進入`<PropertyGroup>`*csproj*檔中的任何內容。 本節稍後會提供完整的 *csproj* 檔案範例。
...
1. 發行您的應用程式。
   ...
   若要從 Visual Studio 發行您的應用程式，請執行下列作業：
      ...
      1. [發行]**** 索引標籤現在會顯示單一設定檔 **FolderProfile**。 設定檔的配置設置顯示在選項卡的 **"摘要"** 部分。**目標運行時**標識已發佈的運行時，**目標位置**標識自包含部署的檔寫入的位置。
      ...
      每個目標位置都包含啟動應用所需的完整檔集（包括應用檔和所有 .NET Core 檔）。
...
1. 選取 Visual Studio 發行您應用程式的位置。
   ...
   如果您要發行到多個平台，請附加識別目標平台的字串。 例如，如果將字串"linux"追加到檔路徑中，Visual Studio 會將應用程式與框架相關的部署發佈到*\<專案目錄>\bin_Release_netcoreapp2.1_publish_linux*目錄。
...
您已完成設定您的設定檔，並已準備好發行應用程式。 作法：
   ...
   每個目標位置（在我們的示例中，bin_release_netcoreapp2.1]發佈\\*設定檔名稱*包含啟動應用所需的完整檔集（包括應用檔和所有 .NET Core 檔）。
...
您只能將具有協力廠商庫的自包含部署部署到該庫支援的平臺。 這類似於在與 Framework 相依的部署中擁有仰賴原生相依性的協力廠商相依性；在其中，原生相依性不會存在於目標平台上，除非先前已在該處安裝這些相依性。
