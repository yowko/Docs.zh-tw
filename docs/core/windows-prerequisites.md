...
---
# <a name="prerequisites-for-net-core-on-windows"></a>Windows 上 .NET Core 的先決條件

此文章說明支援的 OS 版本，以便在 Windows 上執行 .NET Core 應用程式。 支援的作業系統版本和跟隨的相依性，適用於在 Windows 開發 .NET Core 應用程式的三種方式：
...
此外，如果您正在使用 Visual Studio 2017 在 Windows 上進行開發，則[使用 Visual Studio 2017 的先決條件](#prerequisites-with-visual-studio-2017)一節將詳細介紹 .NET Core 開發支援的最低版本。
...
>
> 請確定您的 Windows 安裝處於最新狀態，且包含 [KB2999226](https://support.microsoft.com/en-us/help/2999226/update-for-universal-c-runtime-in-windows) \(機器翻譯\) (可透過 Windows Update 安裝)。 如果未安裝此更新，當您啟動 .NET Core 應用程式時，將會看到如下的錯誤： `The program can't start because api-ms-win-crt-runtime-1-1-0.dll is missing from your computer. Try reinstalling the program to fix this problem.`
>
...
## <a name="prerequisites-with-visual-studio-2017"></a>使用 Visual Studio 2017 的先決條件
...
>   * 針對 .NET Core 2.2 應用程式，需要 Visual Studio 2017 15.9 版或更新版本。
>   * 針對 .NET Core 2.1 應用程式，需要 Visual Studio 2017 15.7 版或更新版本。
>   * 針對 .NET Core 1.x 應用程式，需要 Visual Studio 2017 版本 15.0 或更高版本。