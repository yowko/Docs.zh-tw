...
該`dotnet new`命令基於範本創建 .NET Core 專案或其他專案。
...
- **`TEMPLATE`**
  ...
  您可以運行`dotnet new --list`以查看所有已安裝範本的清單。 如果`TEMPLATE`該值與返回表中的 **"範本**"或 **"短名稱"** 列中的文本不匹配，則對這兩列執行子字串匹配。

  從 .NET Core 3.0 SDK 開始，當您在以下條件下調用`dotnet new`命令時，CLI 會搜索NuGet.org中的範本：

  - 如果 CLI 在調用`dotnet new`時找不到範本匹配，甚至不是部分範本。
  - 如果有較新版本的範本可用。 在這種情況下，將創建專案或專案，但 CLI 會警告您有關範本的更新版本。

  此命令包含預設的範本清單。 使用 `dotnet new -l` 以取得可用範本的清單。 下表顯示了預先安裝的 .NET Core SDK 的範本。 範本的預設語言會顯示在方括號內。 按一下短名稱連結以查看特定的範本選項。

| 範本                                    | 簡短名稱                      | Language     | Tags                                  | 介紹 |
|----------------------------------------------|---------------------------------|--------------|---------------------------------------|------------|
| 主控台應用程式                          | [console](#console)             | [C#], F#, VB | 通用/主控台                        | 1.0        |
| 類別庫                                | [classlib](#classlib)           | [C#], F#, VB | 通用/程式庫                        | 1.0        |
| WPF 應用程式                              | [Wpf](#wpf)                     | [C#]         | 公共/WPF                            | 3.0        |
| WPF 類庫                            | [wpflib](#wpf)                  | [C#]         | 公共/WPF                            | 3.0        |
| WPF 自訂控制項程式庫                   | [wpf自訂控制lib](#wpf)     | [C#]         | 公共/WPF                            | 3.0        |
| WPF 使用者控制項程式庫                     | [wpfuser控制lib](#wpf)       | [C#]         | 公共/WPF                            | 3.0        |
| 視窗表單（贏表單）應用程式         | [溫表單](#winforms)           | [C#]         | 通用/雙贏                       | 3.0        |
| 視窗表單（獲勝表單）類庫       | [溫多利布](#winforms)        | [C#]         | 通用/雙贏                       | 3.0        |
...
| 協定緩衝區檔                         | [原](#namespace)             |              | 網路/gRPC                              | 3.0        |
| 點網 gitignore 檔                        | `gitignore`                     |              | Config                                | 3.0        |
| global.json 檔案                             | [globaljson](#globaljson)       |              | Config                                | 2.0        |
| NuGet 組態                                 | `nugetconfig`                   |              | Config                                | 1.0        |
| 點網本地工具清單檔              | `tool-manifest`                 |              | Config                                | 3.0        |
...
- **`-i|--install <PATH|NUGET_ID>`**
  ...
  從`PATH`或`NUGET_ID`安裝 提供的範本包。 若您想要安裝預先發行版本的範本套件，就必須以 `<package-name>::<package-version>` 的格式指定版本。 預設情況下，`dotnet new`將傳遞\*版本，該版本表示最新的穩定包版本。 請參閱["示例](#examples)"部分中的示例。
...
- **`-u|--uninstall [PATH|NUGET_ID]`**
  ...
  在`PATH`或`NUGET_ID`上卸載範本包。 未指定`<PATH|NUGET_ID>`該值時，將顯示所有當前安裝的範本包及其關聯的範本。 指定`NUGET_ID`時，不包括版本號。
...
- **`--update-apply`**
  ...
  檢查當前安裝的範本包是否有可用的更新並安裝它們。 自 .NET Core 3.0 SDK 起提供。
...
- **`--langVersion <VERSION_NUMBER>`**
  ...
  在`LangVersion`創建的專案檔案中設置屬性。 例如，使用 `--langVersion 7.3` 可使用 C# 7.3。 不支援 F#。 自 .NET 核心 2.2 SDK 以來可用。

  有關預設 C# 版本的清單，請參閱[預設值](../../csharp/language-reference/configure-language-version.md#defaults)。

- **`--no-restore`**

  如果指定，則不會在專案創建期間執行隱式還原。 自 .NET 核心 2.2 SDK 以來可用。
...
### <a name="wpf"></a>wpf， wpflib， wpf自訂控制lib， wpfusercontrollib
...
- **`--langVersion <VERSION_NUMBER>`**
  ...
  在`LangVersion`創建的專案檔案中設置屬性。 例如，使用 `--langVersion 7.3` 可使用 C# 7.3。
...
- **`-p|--enable-pack`**
  ...
  使用[dotnet 包](dotnet-pack.md)為專案啟用打包。
...
- **`--aad-b2c-instance <INSTANCE>`**
  ...
  要連接到的 Azure 活動目錄 B2C 實例。 搭配 `IndividualB2C` 驗證使用。 預設值是 `https://login.microsoftonline.com/tfp/`。

- **`-ssp|--susi-policy-id <ID>`**

  此專案的登錄和註冊策略 ID。 搭配 `IndividualB2C` 驗證使用。

- **`-rp|--reset-password-policy-id <ID>`**

  此專案的重置密碼原則 ID。 搭配 `IndividualB2C` 驗證使用。

- **`-ep|--edit-profile-policy-id <ID>`**

  此專案的編輯設定檔策略 ID。 搭配 `IndividualB2C` 驗證使用。
...
- **`--callback-path <PATH>`**
  ...
  重定向 URI 的應用程式基本路徑中的請求路徑。 搭配 `SingleOrg` 或 `IndividualB2C` 驗證使用。 預設值是 `/signin-oidc`。

- **`-r|--org-read-access`**

  允許此應用程式讀取存取目錄。 僅適用於 `SingleOrg` 或 `MultiOrg` 驗證。
...
- **`--no-https`**
  ...
  關閉 HTTPS。 僅當 、或`Individual`未`IndividualB2C`用於`SingleOrg``--auth`時`MultiOrg`，此選項才適用。

- **`-uld|--use-local-db`**

  指定本機資料庫應使用而不是 SQLite。 僅適用於 `Individual` 或 `IndividualB2C` 驗證。
...
- **`-uld|--use-local-db`**
  ...
  指定本機資料庫應使用而不是 SQLite。 僅適用於 `Individual` 或 `IndividualB2C` 驗證。 自 .NET Core 3.0 SDK 起提供。
...
- **`-s|--support-pages-and-views`**
  ...
  除了元件添加到此庫之外，還支援添加傳統的 Razor 頁面和視圖。 自 .NET Core 3.0 SDK 起提供。
...
- **`-r|--org-read-access`**
  ...
  允許此應用程式讀取存取目錄。 僅適用于`SingleOrg`身份驗證。
...
- **`--no-https`**
  ...
  關閉 HTTPS。 `app.UseHsts` 和 `app.UseHttpsRedirection` 並未新增至 `Startup.Configure`。 此選項僅適用于或不`SingleOrg`用於身份驗證`IndividualB2C`時。

- **`-uld|--use-local-db`**

  指定本機資料庫應使用而不是 SQLite。 僅適用于`IndividualB2C`身份驗證。
...
- 在目前的目錄中創建*一個全域.json*將 SDK 版本設置為 3.1.101：
