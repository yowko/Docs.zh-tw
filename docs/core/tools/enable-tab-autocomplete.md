...
設定後，只要在殼層中鍵入 `dotnet ` 命令，然後按 TAB 鍵，即會觸發 .NET Core CLI 的 tab 鍵自動完成。 目前的命令列會傳送至 `dotnet complete` 命令，並由您的殼層處理結果。 您可以直接傳送某些內容至 `dotnet complete` 命令來測試結果，而不啟用 tab 鍵自動完成。 例如：
