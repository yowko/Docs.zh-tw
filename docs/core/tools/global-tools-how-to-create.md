...
1. 新增名為的新方法 `ShowBot` ，其接受字串參數。 方法會使用文字行來印出訊息和機器人的影像。
...
1. 開啟*botsay .csproj*檔案，然後將三個新的 XML 節點加入至節點的結尾 `<PropertyGroup>` ：
   ...
   `<ToolCommandName>`是選擇性的專案，指定在安裝工具之後叫用它的命令。 如果未提供此元素，則工具的命令名稱是不含 *.csproj*副檔名的專案檔案名稱。
...
1. 執行[dotnet pack](dotnet-pack.md)命令來建立 NuGet 套件：
   ...
   *Nupkg*檔案是在 botsay .csproj 檔案的值所識別的資料夾中建立的 `<PackageOutputPath>` ，在此範例*microsoft.botsay.csproj*中，是 */nupkg*資料夾。
