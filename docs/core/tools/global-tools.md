...
* 作為全域工具。
  ...
  工具二進位檔會安裝在已新增至 PATH 環境變數的預設目錄中。 您可以從電腦上的任何目錄中叫用此工具，而不需指定其位置。 電腦上的所有目錄都會使用一個版本的工具。

* 做為自訂位置中的全域工具 (也稱為工具路徑工具) 。

  工具二進位檔會安裝在您指定的位置。 您可以從安裝目錄中叫用此工具，或提供具有命令名稱的目錄，或將目錄新增至 PATH 環境變數。 電腦上的所有目錄都會使用一個版本的工具。
...
若要將工具安裝為通用工具，請使用 `-g` `--global` [dotnet 工具安裝](dotnet-tool-install.md)的或選項，如下列範例所示：

```dotnetcli
dotnet tool install -g dotnetsay
```

輸出會顯示用來叫用工具和安裝版本的命令，類似于下列範例：
...
第一次執行 SDK 時，此位置會新增至使用者的路徑，因此可以從任何目錄叫用通用工具，而不需指定工具位置。

工具存取是使用者特定的，而不是電腦全域。 通用工具僅供已安裝此工具的使用者使用。

### <a name="install-a-global-tool-in-a-custom-location"></a>在自訂位置安裝通用工具

若要在自訂位置將工具安裝為通用工具，請使用 `--tool-path` [dotnet 工具安裝](dotnet-tool-install.md)的選項，如下列範例所示。
...
若要安裝僅限本機存取的工具 (針對目前的目錄和子目錄) ，必須將其新增至工具資訊清單檔。 若要建立工具資訊清單檔，請執行 `dotnet new tool-manifest` 下列命令：
...
您通常會將本機工具新增至存放庫的根目錄。 將資訊清單檔案簽入存放庫之後，從儲存機制簽出程式碼的開發人員會取得最新的資訊清單檔案。 若要安裝資訊清單檔案中列出的所有工具，請執行 `dotnet tool restore` 下列命令：
...
您用來叫用工具的命令可能與您安裝的封裝名稱不同。 若要顯示目前使用者目前安裝在電腦上的所有工具，請使用 [dotnet 工具清單](dotnet-tool-list.md) 命令：
...
如這個範例所示，此清單會顯示本機工具。 若要查看通用工具，請使用 `--global` 選項，若要查看工具路徑工具，請使用 `--tool-path` 選項。

### <a name="invoke-a-global-tool"></a>叫用通用工具

若為通用工具，請單獨使用工具命令。 例如，如果命令是 `dotnetsay` 或，就會 `dotnet-doc` 用來叫用命令：
...
如果命令的開頭是前置詞 `dotnet-` ，則叫用工具的替代方式是使用 `dotnet` 命令並省略工具命令前置詞。 例如，如果命令為 `dotnet-doc` ，則下列命令會叫用工具：

```dotnetcli
dotnet doc
```

不過，在下列案例中，您無法使用 `dotnet` 命令叫用通用工具：

* 全域工具和本機工具的前面都有相同的命令 `dotnet-` 。
* 您想要從本機工具範圍中的目錄叫用全域工具。

在此案例中，會叫用 `dotnet doc` `dotnet dotnet-doc` 本機工具。 若要叫用全域工具，請單獨使用命令：
...
若要叫用使用選項所安裝的通用工具 `tool-path` ，請確定命令可供使用，如本文稍 [早](#install-a-global-tool-in-a-custom-location)所述。

### <a name="invoke-a-local-tool"></a>叫用本機工具

若要叫用本機工具，您必須 `dotnet` 從安裝目錄中使用命令。 您可以使用 [完整格式] (`dotnet tool run <COMMAND_NAME>`) 或 () 的簡短格式 `dotnet <COMMAND_NAME>` ，如下列範例所示：
...
如果命令的前置詞為 `dotnet-` ，當您叫用工具時，可以包含或省略前置詞。 例如，如果命令為 `dotnet-doc` ，下列任何範例都會叫用本機工具：
...
更新工具需要以最新的穩定版本卸載並重新安裝。 若要更新工具，請使用 [dotnet tool update](dotnet-tool-update.md) 命令與您用來安裝此工具的相同選項：
...
針對本機工具，SDK 會藉由查看目前目錄和上層目錄，尋找包含封裝識別碼的第一個資訊清單檔案。 如果任何資訊清單檔中沒有這類套件識別碼，SDK 會將新專案新增至最接近的資訊清單檔案。

## <a name="uninstall-a-tool"></a>卸載工具

使用您用來安裝此工具的相同選項，以 [dotnet 工具 uninstall](dotnet-tool-uninstall.md) 命令移除工具：
...
針對本機工具，SDK 會藉由查看目前目錄和上層目錄，尋找包含封裝識別碼的第一個資訊清單檔案。

## <a name="get-help-and-troubleshoot"></a>取得協助及進行疑難排解
