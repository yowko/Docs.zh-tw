...
代表來源通道上的組建版本 (請參閱 `-Channel` 選項)。 可能值為：

- `latest` - 通道上的最新組建
- `coherent` - 通道上的最新一致性組建，使用最新的穩定套件組合
- X.Y.Z 格式的三部分版本代表特定組建版本 (例如 `1.0.x`，其中 `x` 代表更新程式版本，或是特定組建，例如 `2.0.0-preview2-006120`)

如果省略，`-Version` 預設為包含 `version` 成員的第一個 [global.json](global-json.md)。 如果不存在，`-Version` 預設為 `latest`。
