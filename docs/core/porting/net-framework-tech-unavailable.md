...
應用程式定義域 (AppDomain) 可將應用程式互相隔離。 AppDomains 需要運行時支援,而且通常成本高昂。 不支援創建其他應用域,並且將來沒有添加此功能的計劃。 對於代碼隔離,請使用單獨的進程或容器作為替代方法。 要動態載入程式集,請使用類別<xref:System.Runtime.Loader.AssemblyLoadContext>。
