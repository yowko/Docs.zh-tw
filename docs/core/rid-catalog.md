...
RID 圖表或執行階段後援圖形是與彼此相容的 RID 清單。 RID 是在 [Microsoft.NETCore.Platforms](https://www.nuget.org/packages/Microsoft.NETCore.Platforms/) 套件中定義。 您可以在位於 `dotnet/runtime` 存放庫的[*執行時間. json*](https://github.com/dotnet/runtime/blob/master/src/libraries/pkg/Microsoft.NETCore.Platforms/runtime.json)檔案中，看到支援的 RID 和 rid 圖表清單。 在此檔案中，您可以看到所有 RID (基底項目除外) 都包含 `"#import"` 陳述式。 這些陳述式指出相容的 RID。
...
如需最新且完整的版本，請參閱 `dotnet/runtime` 儲存機制上的[執行時間. json](https://github.com/dotnet/runtime/blob/master/src/libraries/pkg/Microsoft.NETCore.Platforms/runtime.json)檔案。
...
僅列出常見值。 如需最新且完整的版本，請參閱 `dotnet/runtime` 儲存機制上的[執行時間. json](https://github.com/dotnet/runtime/blob/master/src/libraries/pkg/Microsoft.NETCore.Platforms/runtime.json)檔案。
...
macOS RID 使用較舊的 "OSX" 商標。 僅列出常見值。 如需最新且完整的版本，請參閱 `dotnet/runtime` 儲存機制上的[執行時間. json](https://github.com/dotnet/runtime/blob/master/src/libraries/pkg/Microsoft.NETCore.Platforms/runtime.json)檔案。
