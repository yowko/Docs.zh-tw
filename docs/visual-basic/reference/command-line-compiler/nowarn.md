...
## <a name="remarks"></a>備註  
 `-nowarn`選項會使編譯器不會產生警告。 若要隱藏個別的警告，請將警告識別碼提供`-nowarn`給冒號後面的選項。 以逗號分隔多個警告編號。  
  
 您只需要指定警告識別碼的數位部分。 例如，如果您想要隱藏 BC42024，針對未使用之區域變數的警告，請`-nowarn:42024`指定。  
...
## <a name="example"></a>範例  
 下列程式碼會`T2.vb`編譯並不會顯示未使用之區域變數（42024）的警告。  
