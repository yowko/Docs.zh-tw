...
## <a name="remarks"></a>備註  
 `fileList`參數所列出的檔案必須使用`-target:module`選項來建立，或與另一個編譯器的相當於`-target:module`。  
 ...
 如果您指定 [（隱含或明確）] 以外`-target:module`的任何[目標（Visual Basic）](../../../visual-basic/reference/command-line-compiler/target.md)選項`-addmodule`，則您傳遞給`-addmodule`的檔案會成為專案元件的一部分。 需要元件才能執行包含一或多個已加入之檔案的輸出檔`-addmodule`。  
  
 使用[-reference （Visual Basic）](../../../visual-basic/reference/command-line-compiler/reference.md)從包含元件的檔案匯入中繼資料。  
  
> [!NOTE]
> 此`-addmodule`選項無法在 Visual Studio 開發環境中使用;只有在從命令列編譯時，才可以使用它。  
