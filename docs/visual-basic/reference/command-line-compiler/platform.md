...
|`anycpu32bitpreferred`|編譯要在任何平台上執行的組件。 應用程式將在 32 位元和 64 位元版本的 Windows 上當做 32 位元應用程式執行。 This flag is valid only for executables (.EXE) and requires .NET Framework 4.5.|  
...
- 使用其元件不存在於所有平台的平台叫用或 COM Interop。  
 ...
 The **-platform** option will mitigate some issues if you know you have made assumptions about the architecture your code will run on. 尤其是：  
