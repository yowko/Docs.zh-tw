...
|`anycpu32bitpreferred`|編譯要在任何平台上執行的組件。 應用程式將在 32 位元和 64 位元版本的 Windows 上當做 32 位元應用程式執行。 此旗標僅適用于可執行檔（。EXE），而且需要 .NET Framework 4.5。|  
...
2. 在 [**編譯**] 索引標籤上，選取或清除 [**慣用 32-bit** ] 核取方塊，或在 [**目標 CPU** ] 清單中選擇一個值。  
  
     如需詳細資訊，請參閱[專案設計工具、編譯頁面（Visual Basic）](/visualstudio/ide/reference/compile-page-project-designer-visual-basic)。  
