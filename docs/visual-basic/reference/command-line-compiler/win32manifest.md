...
> [!NOTE]
> 此選項和[-win32resource](win32resource.md)選項是互斥的。 如果您嘗試在相同的命令列中使用這兩個選項，您將會收到組建錯誤。  
...
1. 您可以使用 `-nowin32manifest` 選項，而且您不會在稍後的組建步驟中提供資訊清單，或使用選項，在 Windows 資源（.res）檔案的一部分 `-win32resource` 。  
...
2. 您可以提供未指定所要求執行層級的自訂資訊清單。  
 ...
 您可以使用選項，將應用程式資訊清單提供為自訂的後置組建步驟，或做為 Win32 資源檔的一部分 `-nowin32manifest` 。 如果您想要應用程式受制於 Windows Vista 上的檔案或登錄虛擬化，請使用這個相同的選項。 這會防止編譯器在 PE 檔案中建立和內嵌預設資訊清單。  
