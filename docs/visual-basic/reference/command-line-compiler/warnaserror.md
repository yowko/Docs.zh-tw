...
|詞彙|定義|  
|---|---|  
|+ &#124; -|選擇性。 根據預設，`-warnaserror-` 會生效;警告不會防止編譯器產生輸出檔。 `-warnaserror` 選項與 `-warnaserror+`相同，會導致警告視為錯誤。|  
|`numberList`|選擇性。 以逗號分隔的清單，其中列出 `-warnaserror` 選項套用的警告識別碼編號。 如果未指定警告識別碼，`-warnaserror` 選項會套用至所有警告。|  
  
## <a name="remarks"></a>備註  
 `-warnaserror` 選項會將所有警告視為錯誤。 通常會回報為警告的任何訊息會改為回報為錯誤。 編譯器會報告與警告相同的後續出現的警告。  
  
 根據預設，`-warnaserror-` 會生效，這會導致警告僅做為參考之用。 `-warnaserror` 選項與 `-warnaserror+`相同，會導致警告視為錯誤。  
...
## <a name="example"></a>範例  
 下列程式碼會編譯 `In.vb`，並指示編譯器在第一次出現其發現的每個警告時顯示錯誤。  
...
## <a name="example"></a>範例  
 下列程式碼會編譯 `T2.vb`，而且只會將未使用的區域變數（42024）的警告視為錯誤。  
