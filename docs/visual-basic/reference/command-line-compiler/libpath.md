...
## <a name="remarks"></a>備註  
 `-libpath` 選項會指定[-reference](../../../visual-basic/reference/command-line-compiler/reference.md)選項所參考之元件的位置。  
...
4. LIB 環境變數所指定的目錄。  
 ...
 `-libpath` 選項為加法;指定超過一次以上會附加到任何先前的值。  
  
 使用 `-reference` 來指定元件參考。  
  
|在 Visual Studio 的整合式開發環境中設定-libpath|  
|---|  
|1. 在**方案總管**中選取專案。 在 [專案] 功能表上，按一下 [屬性]。 <br />2. 按一下 [**參考**] 索引標籤。<br />3. 按一下 [**參考路徑 ...** ] 按鈕。<br />4. 在 [**參考路徑**] 對話方塊的 [**資料夾：** ] 方塊中，輸入目錄名稱。<br />5. 按一下 [**新增資料夾**]。|  
