...
- 所叫用的欄位、屬性、事件或方法具有組件 B 的傳回型別或參數類型。  
 ...
 如同 [參考](reference.md) 編譯器選項， `-link` 編譯器選項會使用 Vbc 回應檔，其會參考常用的 .NET Framework 元件。 如果您不想要編譯器使用 Vbc 檔案，請使用 [-noconfig](noconfig.md) 編譯器選項。  
...
## <a name="example"></a>範例  
 ...
 下列命令列將 `OfficeApp.vb` 從和產生來源檔案和參考元件 `COMData1.dll` `COMData2.dll` `OfficeApp.exe` 。  
