...
|詞彙|定義|  
|---|---|  
|`filename`|必要項。 要內嵌至輸出檔的資源檔名稱。 根據預設，`filename`是公用的組件中。 將檔案名稱括在引號 ("") 如果包含空格。|  
|`identifier`|選擇項。 邏輯名稱的資源;用來將其載入的名稱。 預設值是檔案的名稱。 您可以選擇性地指定的資源如同下列是公用或私用組件資訊清單中： `/res:``filename.res`，`myname.res`，`public`|  
