---
title: 沒有方法 '<methodname>'可接受類型的引數'<typename>'的參數'<parametername>'
ms.date: 07/20/2015
ms.assetid: 175c01dc-279c-4996-8501-d19a6671fdeb
ms.openlocfilehash: 4f782e612a66fd7e387166347e5c2478b7a816cc
ms.sourcegitcommit: 558d78d2a68acd4c95ef23231c8b4e4c7bac3902
ms.translationtype: MT
ms.contentlocale: zh-TW
ms.lasthandoff: 04/09/2019
ms.locfileid: "59294947"
---
# <a name="no-method-methodname-can-accept-an-argument-of-type-typename-for-parameter-parametername"></a>沒有方法 '\<方法名稱 >' 可接受類型的引數'\<類型名稱 >' 參數 '\<參數名稱 >'
方法呼叫中包含無效類型的引數。  
  
## <a name="to-correct-this-error"></a>更正這個錯誤  
  
1. 請檢查引數的類型。  
  
2. 比對引數類型與方法的參數類型，確定它們相符。 視需要變更類型。  
  
## <a name="see-also"></a>另請參閱

- [以傳值和傳址方式傳遞引數](../../visual-basic/programming-guide/language-features/procedures/passing-arguments-by-value-and-by-reference.md)
