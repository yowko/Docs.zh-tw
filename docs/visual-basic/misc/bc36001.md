...
`Global` 關鍵字使用在不允許的陳述式。  
 ...
 `Global` 關鍵字可讓您存取在您的程式碼要編譯所在命名空間階層架構之外定義的命名空間。 `Global` 的限定性條件路徑從 .NET Framework 類別庫的最外層命名空間層級開始。 如需圖例，請參閱 [Global - delete](http://msdn.microsoft.com/en-us/18c8ba14-40f6-4978-8096-6a5852324635)。  
