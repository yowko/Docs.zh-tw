...
---
# <a name="type-argument-inference-failed-for-type-parameter-typeparametername1-of-genericproceduresignature"></a>\<typeparametername1>' ' 的類型參數 ' ' 的類型引數推斷失敗 \<genericproceduresignature>
' ' 的類型參數 ' ' 的類型引數推斷失敗 \<typeparametername1> \<genericproceduresignature> 。 從傳遞給參數 ' ' 的引數推斷的型別引數 \<parametername1> ，與從傳遞給參數 ' ' 的引數推斷而來的型別引數衝突 \<parametername2> 。  
