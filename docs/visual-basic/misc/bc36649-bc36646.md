...
---
# <a name="data-types-of-the-type-parameters-in-extension-method-methodname-defined-in-typename-cannot-be-inferred-from-these-arguments"></a>無法從這些引數推斷在 '\< \<typename > ' 中定義之擴充方法 ' 方法名稱 > ' 中類型參數的資料類型
無法從這些引數推斷在 '\< \<typename > ' 中定義之擴充方法 ' 方法名稱 > ' 中類型參數的資料類型。 明確指定資料類型或許可以更正這個錯誤。  
