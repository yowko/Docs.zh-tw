...
1.  判斷是否有從 `<type1>` 到 `<type2>` 的任何類型轉換。 如果兩者都是 [!INCLUDE[vbprvb](../../csharp/programming-guide/concepts/linq/includes/vbprvb-md.md)] 基礎類型，或兩者都是類別的執行個體，則您通常可以參閱[Widening and Narrowing Conversions](../../visual-basic/programming-guide/language-features/data-types/widening-and-narrowing-conversions.md) 中的表格來進行這項判斷。  
