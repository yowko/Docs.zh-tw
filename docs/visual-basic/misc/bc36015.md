...
---
# <a name="typename-has-the-same-name-as-another-type-exposed-in-a-my-group"></a>' \<typename> ' 與在 ' My ' 群組中公開的另一個類型有相同的名稱

' \<typename> ' 與在 ' My ' 群組中公開的另一個類型有相同的名稱。 請重新命名表單或它的封入命名空間。  
