...
`Option Strict On` 時，不可進行 '`<type1>`' 至 '`<type2>`' 的隱含轉換。Visual Basic 6.0 集合類型與 [!INCLUDE[dnprdnshort](../../csharp/getting-started/includes/dnprdnshort-md.md)] 集合類型不相容。  
  
 Visual Basic 6.0 中使用的集合物件與 [!INCLUDE[vs_current_long](../../csharp/misc/includes/vs-current-long-md.md)] 中使用的集合物件不同。  
