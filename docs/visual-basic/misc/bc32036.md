...
衍生類別不會宣告函式，而且 Visual Basic 無法產生一個，因為它無法判斷要呼叫哪一個基類的函式。  
  
 當衍生類別未宣告函式時，Visual Basic 會嘗試產生呼叫的隱含無參數的函式 `MyBase.New()` 。 如果沒有引數即可呼叫的基類中沒有可存取的函式，或如果有多個，Visual Basic 無法產生隱含的函式。  
