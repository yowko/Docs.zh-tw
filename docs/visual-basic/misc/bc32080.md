...
泛型 `Sub` 程序在其宣告中包含 [Handles](../../visual-basic/language-reference/statements/handles-clause.md) 子句。  
 ...
 `Handles` 子句指定 `Sub` 程序所處理的事件清單。 若要成為事件處理常式， `Sub` 程序所擁有的簽章必須與其處理的每個事件相同。 泛型程序可由一次以上，使用在編譯時期無法預測的 Visual Basic 的簽章。 因此，Visual Basic 無法保證的簽章相符的項目中的事件，`Handles`子句。  
