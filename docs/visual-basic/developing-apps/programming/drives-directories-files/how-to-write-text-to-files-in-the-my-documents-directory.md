...
## <a name="net-framework-security"></a>.NET Framework 安全性  
 ...
 這個範例會建立新的檔案。 如果應用程式需要建立檔案，該應用程式就需要資料夾的 [建立] 權限。 您可以使用存取控制清單來設定權限。 如果檔案已經存在，則應用程式只需要 [寫入] 權限，這是較小的權限。 若有可能，更為安全的做法是在部署期間建立檔案，並且只授與單一檔案的 [讀取] 權限，而不授與資料夾的 [建立] 權限。 此外，將資料寫入使用者資料夾，比根資料夾或 **Program Files** 資料夾更安全。 如需詳細資訊，請參閱 [ACL 技術概觀](/previous-versions/dotnet/netframework-4.0/ms229742(v=vs.100))。  
