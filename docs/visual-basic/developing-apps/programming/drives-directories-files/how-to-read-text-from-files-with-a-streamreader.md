...
`My.Computer.FileSystem` 物件提供方法來開啟 <xref:System.IO.TextReader> 和 <xref:System.IO.TextWriter>。 除非您選取 [全部]`OpenTextFileWriter``OpenTextFileReader` 索引標籤，否則 **和** 這兩種方法是未出現在 IntelliSense 中的進階方法。  
