...
在 Windows Forms 中，表單是顯示資訊給使用者的視覺介面。 您通常會將控制項放在表單上，並開發使用者動作的回應，例如滑鼠點按或按鍵，以建立 Windows Forms 應用程式。 「控制項」是獨立的使用者介面 (UI) 項目，可顯示資料或接受資料輸入。
...
利用 Windows Form 智慧型用戶端，透過網路連接到資料來源是一項簡單的工作。 @No__t_0 元件是 Visual Studio 2005 和 .NET Framework 2.0 中 Windows Forms 的新功能，代表與資料來源的連接，並公開將資料系結至控制項、流覽至上一筆記錄、編輯記錄和儲存的方法變更回原始來源。 <xref:System.Windows.Forms.BindingNavigator> 控制項透過 <xref:System.Windows.Forms.BindingSource> 元件提供一個簡單的介面，可讓使用者在記錄之間巡覽。
...
您可以在 Windows Forms 中管理的另一種資料系結類型是 [設定]。 大部分的智慧型用戶端應用程式都必須保留一些有關執行時間狀態的資訊，例如最後已知的表單大小，並保留使用者喜好設定資料，例如已儲存檔案的預設位置。 應用程式設定功能藉由提供簡單的方法，在用戶端電腦上儲存這兩種類型的設定，來解決這些需求。 一旦使用 Visual Studio 或程式碼編輯器來定義之後，這些設定就會保存為 XML，並在執行時間自動讀回記憶體。
...
|使用 ADO.NET 資料來源|[操作說明：使用 Windows Forms BindingSource 元件排序和篩選 ADO.NET 資料](../../../framework/winforms/controls/sort-and-filter-ado-net-data-with-wf-bindingsource-component.md)|
...
|若要|請參閱|
|--------|---------|
|列印表單的內容|[操作說明：列印 Windows Forms 中的圖形](../../../framework/winforms/advanced/how-to-print-graphics-in-windows-forms.md)<br /><br /> [操作說明：在 Windows Forms 中列印多頁文字檔](../../../framework/winforms/advanced/how-to-print-a-multi-page-text-file-in-windows-forms.md)|
