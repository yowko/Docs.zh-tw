...
5. 將 `ClassId` 的空字串取代為 GUID，移除開頭和尾端的大括弧。 例如，如果 Guidgen 所提供的 GUID 是 `"{2C8B0AEE-02C9-486e-B809-C780A11530FE}"` 則您的程式碼應該會如下所示。  
...
8. COM 類別必須有無參數的 `Public Sub New()` 的函式，否則類別將不會正確註冊。 將無參數的構造函式新增至類別：  
