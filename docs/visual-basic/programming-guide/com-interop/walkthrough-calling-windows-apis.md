...
Windows 應用程式開發介面是屬於 Windows 作業系統的動態連結程式庫 (Dll)。 您可以使用它們來執行工作時很難撰寫您自己的對等的程序。 例如，Windows 提供函式，名為`FlashWindowEx`，可讓您的應用程式的標題列切換淺色及深色陰影。  
 ...
 Windows Api 就是一個特殊種類的互通性。 Windows 應用程式開發介面不會使用 managed 程式碼，並沒有內建型別程式庫，並使用不同於 Visual Studio 搭配使用的資料類型。 因為這些差異，以及因為 Windows 應用程式開發介面不是 COM 物件，與 Windows Api 的互通性和[!INCLUDE[dnprdnshort](~/includes/dnprdnshort-md.md)]必須利用平台叫用時，或 PInvoke。 平台叫用是一項服務，可讓 managed 程式碼呼叫 unmanaged 函式實作在 Dll 中。 如需詳細資訊，請參閱[使用 Unmanaged DLL 函式](../../../framework/interop/consuming-unmanaged-dll-functions.md)。 您可以使用中的 PInvoke[!INCLUDE[vbprvb](~/includes/vbprvb-md.md)]使用`Declare`陳述式或套用`DllImport`屬性設定為空的程序。  
  
 Windows API 呼叫是很重要的一部分[!INCLUDE[vbprvb](~/includes/vbprvb-md.md)]程式設計在過去，但這是很少需要使用 Visual Basic.NET。 可能的話，您應該使用 managed 程式碼從[!INCLUDE[dnprdnshort](~/includes/dnprdnshort-md.md)]來執行工作，而不是 Windows API 呼叫。 本逐步解說提供針對這些情況中使用的資訊是必要的 Windows 應用程式開發介面。  
...
3.  選取**Windows 應用程式**從清單中[!INCLUDE[vbprvb](~/includes/vbprvb-md.md)]專案範本。 新的專案隨即顯示。  
...
#### <a name="lib-and-alias-keywords"></a>Lib 和別名的關鍵字  
 ...
 使用`Alias`關鍵字，如果您要呼叫的函式名稱不是有效[!INCLUDE[vbprvb](~/includes/vbprvb-md.md)]程序名稱或與您的應用程式中其他項目的名稱相衝突。 `Alias`表示正在呼叫函式，則為 true 的名稱。  
  
#### <a name="argument-and-data-type-declarations"></a>引數和資料型別宣告  
 宣告引數和其資料類型。 此組件會有點困難，因為 Windows 使用的資料型別沒有對應至 Visual Studio 資料類型。 [!INCLUDE[vbprvb](~/includes/vbprvb-md.md)]將引數轉換成相容的資料類型，稱為程序會為您完成許多工作*封送處理*。 您可以明確地控制如何引數封送處理使用<xref:System.Runtime.InteropServices.MarshalAsAttribute>中定義屬性<xref:System.Runtime.InteropServices>命名空間。  
  
> [!NOTE]
>  舊版的[!INCLUDE[vbprvb](~/includes/vbprvb-md.md)]可讓您將參數宣告`As Any`，這表示該資料的任何資料類型無法使用。 [!INCLUDE[vbprvb](~/includes/vbprvb-md.md)]需要您使用特定資料類型，所有`Declare`陳述式。  
...
#### <a name="data-marshaling"></a>資料封送處理  
 [!INCLUDE[vbprvb](~/includes/vbprvb-md.md)]自動轉換資料類型的參數和傳回值，Windows 應用程式開發介面呼叫，但是您可以使用`MarshalAs`屬性來明確地指定應用程式開發介面所預期的 unmanaged 的資料類型。 如需 interop 封送處理的詳細資訊，請參閱[Interop 封送處理](../../../framework/interop/interop-marshaling.md)。  
...
2.  選取**Windows 應用程式**從清單中[!INCLUDE[vbprvb](~/includes/vbprvb-md.md)]專案範本。 新的專案隨即顯示。  
