...
Visual Basic 中的 [Async](../../../language-reference/modifiers/async.md) 和 [Await](../../../language-reference/operators/await-operator.md) 關鍵字都是非同步程式設計的核心。 您可以使用這兩個關鍵字，使用 .NET Framework 或 Windows 執行階段中的資源來建立非同步方法，幾乎就像建立同步方法一樣容易。 使用 `Async` 和 `Await` 定義的非同步方法就稱為非同步方法。

下列範例將示範非同步方法。 程式碼中的一切對您而言應該幾乎完全熟悉。 註解會標註您加入以建立非同步的功能。

您可以在本主題結尾找到完整的 Windows Presentation Foundation (WPF) 範例檔案，並且可從[非同步範例：＜使用 Async 和 Await 進行非同步程式設計＞中的範例 (英文)](/samples/dotnet/samples/async-and-await-vb/) 下載範例。
...
- 傳回型別是下列其中一種類型：
  ...
  - 如果您的方法有 return 語句，其中運算元的類型為 TResult，則[ (TResult) 的](xref:System.Threading.Tasks.Task%601)工作。
  - 如果方法沒有 return 陳述式或是 return 陳述式沒有運算元，則為 <xref:System.Threading.Tasks.Task>。
  - 如果您撰寫的是非同步事件處理常式，則為 [Sub](../../language-features/procedures/sub-procedures.md)。
...
## <a name="what-happens-in-an-async-method"></a><a name="BKMK_WhatHappensUnderstandinganAsyncMethod"></a> 非同步方法中發生的情況
...
1. 事件處理常式會呼叫並等候 `AccessTheWebAsync` async 方法。
...
3. `GetStringAsync` 中發生了導致進度暫停的一些狀況。 可能必須等待網站下載或其他封鎖活動。 為了避免封鎖資源，`GetStringAsync` 會將控制權遞交 (Yield) 給它的呼叫端 `AccessTheWebAsync`。
     ...
     `GetStringAsync` 傳回 [tresult) ](xref:System.Threading.Tasks.Task%601) 的工作 (，其中 tresult 是字串，並將 `AccessTheWebAsync` 工作指派給 `getStringTask` 變數。 工作代表對 `GetStringAsync` 之呼叫的進行中程序，並承諾會在工作完成時產生實際字串值。
...
您可能會想知道哪裡可以找到支援非同步程式設計的方法，例如 `GetStringAsync`。 .NET Framework 4.5 或更高版本包含許多可搭配和使用的成員 `Async` `Await` 。 您可以透過附加至成員名稱的 "Async" 尾碼，以及 <xref:System.Threading.Tasks.Task> [TResult) ](xref:System.Threading.Tasks.Task%601)的工作 (的傳回類型，來辨識這些成員。 例如，相對於同步方法 <xref:System.IO.Stream.CopyTo%2A>、<xref:System.IO.Stream.Read%2A> 和 <xref:System.IO.Stream.Write%2A>，`System.IO.Stream` 類別也包含一些方法，例如 <xref:System.IO.Stream.CopyToAsync%2A>、<xref:System.IO.Stream.ReadAsync%2A> 和 <xref:System.IO.Stream.WriteAsync%2A>。

Windows 執行階段也包含許多您可以在 Windows 應用程式中與 `Async` 和 `Await` 搭配使用的方法。 如需詳細資訊和範例方法，請參閱 [以 c # 或 Visual Basic 呼叫非同步 api](/windows/uwp/threading-async/call-asynchronous-apis-in-csharp-or-visual-basic)、 [非同步程式設計 (Windows 執行階段應用程式) ](/previous-versions/windows/apps/hh464924(v=win.10))和 [system.threading.tasks.task.whenany：在 .NET Framework 和 Windows 執行階段之間銜接](/previous-versions/visualstudio/visual-studio-2013/jj635140(v=vs.120))。
...
`Async` 和 `Await` 關鍵字不會導致建立其他執行緒。 非同步方法不需要多執行緒處理，因為非同步方法不會在自己的執行緒上執行。 方法會在目前的同步處理內容執行，而且只有在方法為作用中時才會在執行緒上花費時間。 您可以使用 <xref:System.Threading.Tasks.Task.Run%2A?displayProperty=nameWithType> 將受限於 CPU 的工作移到背景執行緒，但是背景執行緒無法協助處理正在等待結果產生的處理序。

非同步程式設計的非同步方法幾乎是所有案例的現有方法當中較好的方法。 這種方法特別適用于 i/o 系結 <xref:System.ComponentModel.BackgroundWorker> 作業，因為程式碼較簡單，而且您不需要防範競爭情況。 與 <xref:System.Threading.Tasks.Task.Run%2A?displayProperty=nameWithType> 結合時，非同步程式設計會比受限於 CPU 之作業的 <xref:System.ComponentModel.BackgroundWorker> 還要好，因為非同步程式設計會將執行程式碼的協調工作細節，從 `Task.Run` 傳輸至執行緒集區的工作中分出來。

## <a name="async-and-await"></a><a name="BKMK_AsyncandAwait"></a> Async 和 Await

如果您使用 [async](../../../language-reference/modifiers/async.md) 修飾詞將方法指定為非同步方法，就會啟用下列兩項功能。

- 標記的非同步方法可以使用 [Await](../../../language-reference/operators/await-operator.md) 來指定暫停點。 await 運算子會告知編譯器，非同步方法只有在等候的非同步處理序完成後，才能繼續通過該點。 同時，控制權會返回非同步方法的呼叫端。
...
在 .NET Framework 程式設計中，非同步方法通常會傳回 <xref:System.Threading.Tasks.Task> [TResult) ](xref:System.Threading.Tasks.Task%601)的或工作 (。 在非同步方法內，會將 `Await` 運算子套用到呼叫另一個非同步方法所傳回的工作。

如果方法包含指定類型運算元的[return](../../../language-reference/statements/return-statement.md)語句，則您可以將[TResult) ](xref:System.Threading.Tasks.Task%601)的工作 (指定為傳回型別 `TResult` 。

如果方法沒有 return 陳述式，或者方法的 return 陳述式不會傳回運算元，請使用 `Task` 做為傳回類型。

下列範例將示範如何宣告和呼叫方法，以傳回 [TResult) ](xref:System.Threading.Tasks.Task%601) 或的工作 (<xref:System.Threading.Tasks.Task> ：
...
無法等候程式的非同步方法 `Sub` ，呼叫端無法攔截方法擲回的任何例外狀況。

非同步方法無法宣告 [ByRef](../../../language-reference/modifiers/byref.md) 參數，但是此方法可以呼叫具有這類參數的方法。

如需詳細資訊和範例，請參閱[非同步傳回型別 (Visual Basic)](async-return-types.md)。 如需如何在非同步方法中攔截例外狀況的詳細資訊，請參閱 [Try...Catch...Finally 陳述式](../../../language-reference/statements/try-catch-finally-statement.md)。

Windows 執行階段程式設計中的非同步 API 具有下列其中一種傳回型別 (類似於工作)：

- [Iasyncoperation<tresult>HTTP) 的 tresult (](xref:Windows.Foundation.IAsyncOperation%601)，對應至 [tresult 的工作 () ](xref:System.Threading.Tasks.Task%601)
- <xref:Windows.Foundation.IAsyncAction>，對應至 <xref:System.Threading.Tasks.Task>
- [TProgress) 的 Iasyncactionwithprogress<tprogress> iasyncoperation<tresult> (](xref:Windows.Foundation.IAsyncActionWithProgress%601)
- [IAsyncOperationWithProgress (的 TResult、TProgress) ](xref:Windows.Foundation.IAsyncOperationWithProgress%602)

如需詳細資訊和範例，請參閱使用 [c # 或 Visual Basic 呼叫非同步 api](/windows/uwp/threading-async/call-asynchronous-apis-in-csharp-or-visual-basic)。

## <a name="naming-convention"></a><a name="BKMK_NamingConvention"></a> 命名慣例

依照慣例，您會將 "Async" 附加至具有 `Async` 修飾詞的方法名稱。

當事件、基底類別或介面合約採用不同的名稱時，您可以忽略慣例。 例如，您不應該重新命名常見的事件處理常式，例如 `Button1_Click` 。
...
|[微調非同步應用程式 (Visual Basic)](fine-tuning-your-async-application.md)|顯示如何將下列功能加入至您的非同步方案：<br /><br /> - [取消一項非同步工作或工作清單 (Visual Basic)](cancel-an-async-task-or-a-list-of-tasks.md)<br />- [在一段時間後取消非同步工作 (Visual Basic)](cancel-async-tasks-after-a-period-of-time.md)<br />- [當其中一項工作完成時，取消剩餘的非同步工作 (Visual Basic)](cancel-remaining-async-tasks-after-one-is-complete.md)<br />- [啟動多項非同步工作並在它們完成時進行處理 (Visual Basic)](start-multiple-async-tasks-and-process-them-as-they-complete.md)|[Async Sample: Fine Tuning Your Application](https://code.msdn.microsoft.com/Async-Fine-Tuning-Your-a676abea) (非同步範例：微調應用程式)|
|[處理非同步應用程式中的重新進入 (Visual Basic)](handling-reentrancy-in-async-apps.md)|示範如何處理使用中的非同步作業在執行時重新開機的案例。||
|[WhenAny：銜接 .NET Framework 和 Windows 執行階段](/previous-versions/visualstudio/visual-studio-2013/jj635140(v=vs.120))|顯示如何在 Windows 執行階段中進行 .NET Framework 與 IAsyncOperations 之工作型別之間的橋接，讓您可以搭配使用 <xref:System.Threading.Tasks.Task.WhenAny%2A> 與 Windows 執行階段方法。|[非同步範例：銜接 .NET 和 Windows 執行階段 (AsTask 和 WhenAny) (英文)](/previous-versions/visualstudio/visual-studio-2013/jj635140(v=vs.120))|
...
|[以工作為基礎的非同步模式 (TAP)](../../../../standard/asynchronous-programming-patterns/task-based-asynchronous-pattern-tap.md)|描述 .NET Framework 中非同步的新模式。 此模式是以 <xref:System.Threading.Tasks.Task> [TResult) ](xref:System.Threading.Tasks.Task%601) 類型的和工作 (為基礎。||
...
下列程式碼是本主題討論之 Windows Presentation Foundation (WPF) 應用程式中的 MainWindow.xaml.vb 檔案。 您可以從[非同步範例：＜使用 Async 和 Await 進行非同步程式設計＞中的範例 (英文)](/samples/dotnet/samples/async-and-await-vb/) 下載範例。
