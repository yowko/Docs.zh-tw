...
您可以在本主題結尾找到完整的 Windows Presentation Foundation (WPF) 範例檔案，並且可從[非同步範例：＜使用 Async 和 Await 進行非同步程式設計＞中的範例](https://docs.microsoft.com/samples/dotnet/samples/async-and-await-vb/) \(英文\) 下載範例。
...
## <a name="BKMK_WhatHappensUnderstandinganAsyncMethod"></a>非同步方法中發生的情況
...
非同步程式設計的非同步方法幾乎是所有案例的現有方法當中較好的方法。 特別是，此方法優於 i/o 系結作業的 <xref:System.ComponentModel.BackgroundWorker>，因為程式碼較簡單，而且您不需要防範競爭條件。 與 <xref:System.Threading.Tasks.Task.Run%2A?displayProperty=nameWithType> 結合時，非同步程式設計會比受限於 CPU 之作業的 <xref:System.ComponentModel.BackgroundWorker> 還要好，因為非同步程式設計會將執行程式碼的協調工作細節，從 `Task.Run` 傳輸至執行緒集區的工作中分出來。
...
- 標記的非同步方法可以使用 [Await](../../../../visual-basic/language-reference/operators/await-operator.md) 來指定暫停點。 await 運算子會告知編譯器，非同步方法只有在等候的非同步處理序完成後，才能繼續通過該點。 同時，控制權會返回非同步方法的呼叫端。
  ...
  非同步方法在 @no__t 0 的運算式暫止不會構成從方法結束，而且不會執行 @no__t 1 區塊。

- 標記的非同步方法本身可以做為其呼叫方法的等候目標。

非同步方法通常包含一或多個 `Await` 運算子的出現，但缺少 @no__t 1 運算式不會造成編譯器錯誤。 如果非同步方法未使用 `Await` 運算子來標記暫停點，則即使 `Async` 修飾詞，方法還是會當做同步方法來執行。 編譯器將對這類方法發出警告。
...
下列範例顯示如何宣告和呼叫傳回 <xref:System.Threading.Tasks.Task%601> 或 <xref:System.Threading.Tasks.Task> 的方法：
...
不能等候為 @no__t 0 程式的非同步方法，而且呼叫端無法攔截方法擲回的任何例外狀況。
...
如需詳細資訊和範例，請參閱[在或 Visual Basic C#中呼叫非同步 api](/windows/uwp/threading-async/call-asynchronous-apis-in-csharp-or-visual-basic)。
...
|標題|說明|範例|
|-----------|-----------------|------------|
|[逐步解說：使用 Async 和 Await 存取 Web （Visual Basic） ](walkthrough-accessing-the-web-by-using-async-and-await.md)|顯示如何將同步 WPF 方案轉換為非同步 WPF 方案。 應用程式會下載一系列的網站。|[非同步範例：存取 Web 逐步解說](https://code.msdn.microsoft.com/Async-Sample-Accessing-the-9c10497f) \(英文\)|
|[如何：使用 System.threading.tasks.task.whenall （Visual Basic） ](how-to-extend-the-async-walkthrough-by-using-task-whenall.md) 擴充非同步逐步解說|將 <xref:System.Threading.Tasks.Task.WhenAll%2A?displayProperty=nameWithType> 加入至前一個逐步解說。 使用 `WhenAll` 會同時開始進行所有的下載。||
|[如何：使用 Async 和 Await （Visual Basic），同時發出多個 Web 要求 ](how-to-make-multiple-web-requests-in-parallel-by-using-async-and-await.md)|示範如何同時啟動數個工作。|[非同步範例：以平行方式提出多個 Web 要求](https://code.msdn.microsoft.com/Async-Make-Multiple-Web-49adb82e)|
|[非同步方法的傳回型別 (Visual Basic)](async-return-types.md)|說明非同步方法可以傳回的類型，並解釋每種類型的適用時機。||
|[非同步程式中的控制流程 (Visual Basic)](control-flow-in-async-programs.md)|在非同步程式中詳細追蹤一連串 await 運算式的控制流程。|[非同步範例：非同步程式中的控制流程](https://code.msdn.microsoft.com/Async-Sample-Control-Flow-5c804fc0)|
...
|非同步取消：銜接 .NET Framework 和 Windows 執行階段|顯示如何在 Windows 執行階段中進行 .NET Framework 與 IAsyncOperations 之工作型別之間的橋接，讓您可以搭配使用 <xref:System.Threading.CancellationTokenSource> 與 Windows 執行階段方法。|[非同步範例：銜接 .NET 和 Windows 執行階段 (AsTask & Cancellation) (英文)](https://code.msdn.microsoft.com/Async-Sample-Bridging-9479eca3)|
...
下列程式碼是本主題討論之 Windows Presentation Foundation (WPF) 應用程式中的 MainWindow.xaml.vb 檔案。 您可以從[非同步範例：＜使用 Async 和 Await 進行非同步程式設計＞中的範例](https://docs.microsoft.com/samples/dotnet/samples/async-and-await-vb/) \(英文\) 下載範例。
