...
- 傳回型別是下列其中一種類型：
  ...
  - 如果您的方法具有傳回語句，其中運算元的類型為 TResult，則工作（也就[是 tresult）](xref:System.Threading.Tasks.Task%601) 。
...
3. `GetStringAsync` 中發生了導致進度暫停的一些狀況。 可能必須等待網站下載或其他封鎖活動。 為了避免封鎖資源，`GetStringAsync` 會將控制權遞交 (Yield) 給它的呼叫端 `AccessTheWebAsync`。
     ...
     `GetStringAsync`傳回工作[（](xref:System.Threading.Tasks.Task%601)也就是 tresult），其中 tresult 是字串，並將 `AccessTheWebAsync` 工作指派給 `getStringTask` 變數。 工作代表對 `GetStringAsync` 之呼叫的進行中程序，並承諾會在工作完成時產生實際字串值。
...
您可能會想知道哪裡可以找到支援非同步程式設計的方法，例如 `GetStringAsync`。 .NET Framework 4.5 或更高版本包含許多可與和搭配使用的成員 `Async` `Await` 。 您可以透過附加至成員名稱的 "Async" 尾碼，以及或 Task 的傳回類型 <xref:System.Threading.Tasks.Task> [（TResult）](xref:System.Threading.Tasks.Task%601)來辨識這些成員。 例如，相對於同步方法 <xref:System.IO.Stream.CopyTo%2A>、<xref:System.IO.Stream.Read%2A> 和 <xref:System.IO.Stream.Write%2A>，`System.IO.Stream` 類別也包含一些方法，例如 <xref:System.IO.Stream.CopyToAsync%2A>、<xref:System.IO.Stream.ReadAsync%2A> 和 <xref:System.IO.Stream.WriteAsync%2A>。

Windows 執行階段也包含許多您可以在 Windows 應用程式中與 `Async` 和 `Await` 搭配使用的方法。 如需詳細資訊和範例方法，請參閱[以 c # 呼叫非同步 api 或 Visual Basic](/windows/uwp/threading-async/call-asynchronous-apis-in-csharp-or-visual-basic)、[非同步程式設計（Windows 執行階段應用程式）](https://docs.microsoft.com/previous-versions/windows/apps/hh464924(v=win.10))和[system.threading.tasks.task.whenany：橋接 .NET Framework 和 Windows 執行階段](https://docs.microsoft.com/previous-versions/visualstudio/visual-studio-2013/jj635140(v=vs.120))。
...
非同步程式設計的非同步方法幾乎是所有案例的現有方法當中較好的方法。 特別的是，這種方法比 i/o 系結作業來得好， <xref:System.ComponentModel.BackgroundWorker> 因為程式碼較簡單，而且您不需要防範競爭條件。 與 <xref:System.Threading.Tasks.Task.Run%2A?displayProperty=nameWithType> 結合時，非同步程式設計會比受限於 CPU 之作業的 <xref:System.ComponentModel.BackgroundWorker> 還要好，因為非同步程式設計會將執行程式碼的協調工作細節，從 `Task.Run` 傳輸至執行緒集區的工作中分出來。
...
在 .NET Framework 程式設計中，非同步方法通常會傳回 <xref:System.Threading.Tasks.Task> 或工作（也就是[TResult）](xref:System.Threading.Tasks.Task%601)。 在非同步方法內，會將 `Await` 運算子套用到呼叫另一個非同步方法所傳回的工作。

如果方法包含指定型別運算元的[return](../../../language-reference/statements/return-statement.md)語句，您可以指定[Task （TResult）](xref:System.Threading.Tasks.Task%601)做為傳回型別 `TResult` 。

如果方法沒有 return 陳述式，或者方法的 return 陳述式不會傳回運算元，請使用 `Task` 做為傳回類型。

下列範例示範如何宣告和呼叫方法，以傳回工作（也就是[TResult）](xref:System.Threading.Tasks.Task%601)或 <xref:System.Threading.Tasks.Task> ：
...
無法等候進程的非同步方法 `Sub` ，而且呼叫端無法攔截方法擲回的任何例外狀況。
...
- [Iasyncoperation<tresult> （Of tresult）](xref:Windows.Foundation.IAsyncOperation%601)，對應至[Task （of tresult）](xref:System.Threading.Tasks.Task%601)
...
如需詳細資訊和範例，請參閱[以 c # 呼叫非同步 api 或 Visual Basic](/windows/uwp/threading-async/call-asynchronous-apis-in-csharp-or-visual-basic)。
...
|[處理非同步應用程式中的重新進入 (Visual Basic)](handling-reentrancy-in-async-apps.md)|示範如何處理使用中的非同步作業在執行時重新開機的情況。||
...
|[工作式非同步模式 (TAP)](../../../../standard/asynchronous-programming-patterns/task-based-asynchronous-pattern-tap.md)|描述 .NET Framework 中非同步的新模式。 模式是以 <xref:System.Threading.Tasks.Task> 和工作[（TResult）](xref:System.Threading.Tasks.Task%601)類型為基礎。||
