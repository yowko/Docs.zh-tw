...
## <a name="BKMK_TaskTReturnType"></a> Task(T) 傳回型別  
 <xref:System.Threading.Tasks.Task%601> 傳回型別用於非同步方法，其中包含運算元具有類型 `TResult` 的 [return](../../../../visual-basic/language-reference/statements/return-statement.md) 語句。  
...
```  
 ...
 如先前<xref:System.Threading.Tasks.Task%601>範例所示，您可以`Task_MethodAsync` `Await`從運算子的應用程式中分隔的呼叫，如下列程式碼所示。 不過，請記住，`Task` 沒有 `Result` 屬性，且將 await 運算子套用至 `Task` 時不會產生任何值。  
