...
`GetEnumerator` 方法會使用 `Yield` 語句，一次傳回一個字串，而 `Iterator` 修飾詞則是在函式宣告中。
...
參考類別執行個體 (`For Each`) 的 `theZoo` 陳述式會隱含呼叫 `GetEnumerator` 方法。 參考 `For Each` 和 `Birds` 屬性的 `Mammals` 陳述式會使用 `AnimalsForType` 具名迭代器方法。
