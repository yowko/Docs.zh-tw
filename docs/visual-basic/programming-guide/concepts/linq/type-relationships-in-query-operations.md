...
## 從所選取項目傳回一個欄位的查詢  
 下列範例顯示的 [!INCLUDE[vbtecdlinq](../../../../csharp/language-reference/keywords/includes/vbtecdlinq_md.md)] 查詢作業，會傳回只包含從資料來源中選取之每個項目的某一部分的序列。  這個查詢會採用 `Customer` 物件的集合做為它的資料來源，而且只在結果中規劃 `Name` 屬性。  因為客戶名稱是字串，所以查詢會產生字串的序列做為輸出。  
