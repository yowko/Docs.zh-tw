...
本主題簡介 Visual Basic 中的 [!INCLUDE[vbteclinqext](../../../../csharp/getting-started/includes/vbteclinqext-md.md)] 運算式，以及一些可以在查詢中執行的一般作業。  如需詳細資訊，請參閱下列主題：  
...
## 指定資料來源 \(From\)  
 在 [!INCLUDE[vbteclinq](../../../../csharp/includes/vbteclinq-md.md)] 查詢中，第一步是指定想要查詢的資料來源。  因此，查詢中的 `From` 子句一定排在最前面的位置。查詢運算子來選取和塑造會根據來源類型的結果。  
