...
---
# <a name="finding-text-in-word-documents-visual-basic"></a>Word 文件 (Visual Basic) 中尋找文字
...
```  
 ...
 請注意，這個範例的執行方式與撰寫為單一查詢的執行方式約略相同。 由於每個查詢都會使用延遲、延後的方式實作，因此每個查詢在反覆運算前不會產生其結果。 如需有關執行與延遲評估的詳細資訊，請參閱[延後執行與 LINQ to XML (Visual Basic) 中的延遲評估](../../../../visual-basic/programming-guide/concepts/linq/deferred-execution-and-lazy-evaluation-in-linq-to-xml.md)。  
