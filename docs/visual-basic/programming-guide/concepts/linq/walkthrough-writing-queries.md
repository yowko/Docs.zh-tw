...
2. 將滑鼠指標停留在迴圈控制變數 `studentRecord`，以查看其資料類型。 `studentRecord` 的類型會推斷為 `Student`，因為 `studentQuery` 會傳回 `Student` 實例的集合。
...
1. 在查詢的 `Where` 語句和 `Select` 語句之間加入下列 `Order By` 子句。 `Order By` 子句會根據每個學生的姓氏，從 A 到 Z 依字母順序排序結果。
