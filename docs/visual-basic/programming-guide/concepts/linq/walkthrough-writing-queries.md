...
本逐步解說示範如何使用 Visual Basic 語言功能來撰寫 [!INCLUDE[vbteclinqext](../../../../csharp/programming-guide/concepts/linq/includes/vbteclinqext_md.md)] 查詢運算式。  其中會示範如何在 Student 物件清單上建立查詢、如何執行這些查詢，以及如何加以修改。  這些查詢加入了 Visual Basic 2008 的幾項新功能，包括物件初始設定式、區域型別推斷和匿名型別。  
