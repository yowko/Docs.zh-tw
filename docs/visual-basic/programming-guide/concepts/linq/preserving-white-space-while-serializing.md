...
## <a name="white-space-behavior-of-methods-that-serialize-xml-trees"></a>序列化 XML 樹狀結構之方法的空白字元行為  
 下列方法<xref:System.Xml.Linq.XElement>和<xref:System.Xml.Linq.XDocument>類別序列化 XML 樹狀結構。</xref:System.Xml.Linq.XDocument> </xref:System.Xml.Linq.XElement> 您可以將序列化 XML 樹狀結構至檔案， <xref:System.IO.TextReader>，或<xref:System.Xml.XmlReader>.</xref:System.Xml.XmlReader> </xref:System.IO.TextReader> `ToString` 方法會序列化至字串。  
...
-   <xref:System.Xml.Linq.XDocument.ToString%2A?displayProperty=fullName></xref:System.Xml.Linq.XDocument.ToString%2A?displayProperty=fullName>  
 ...
 如果此方法不採用<xref:System.Xml.Linq.SaveOptions>做為引數，該方法將會格式化 （縮排） 序列化的 XML。</xref:System.Xml.Linq.SaveOptions> 在此情況下，會宣告 XML 樹狀結構中的所有有效空白字元。  
  
 如果此方法採用<xref:System.Xml.Linq.SaveOptions>做為引數，然後您可以指定該方法不格式化 （縮排） 序列化的 XML。</xref:System.Xml.Linq.SaveOptions> 在此情況下，會保留 XML 樹狀中的所有空白字元。  
