...
|[如何：在命名空間中尋找元素（XPath-LINQ to XML）（Visual Basic）](../../../../visual-basic/programming-guide/concepts/linq/how-to-find-elements-in-a-namespace.md)|比較 XPath <xref:System.Xml.XmlNamespaceManager> 類別的用法與 [!INCLUDE[sqltecxlinq](~/includes/sqltecxlinq-md.md)] 類別的 <xref:System.Xml.Linq.XName.Namespace%2A> <xref:System.Xml.Linq.XName> 屬性，以便與 XML 命名空間搭配使用。<br /><br /> 相關聯的 XPath 運算式為：`"./aw:*"`|  
