...
`#Region`指示詞可讓您可以摺疊和隱藏程式碼中的區段[!INCLUDE[vbprvb](~/includes/vbprvb-md.md)]檔案。 `#Region`指示詞可讓您使用時，指定一段程式碼，您可以展開或摺疊[!INCLUDE[vsprvs](~/includes/vsprvs-md.md)]程式碼編輯器。 選擇性地隱藏程式碼的功能可讓您的檔案，更容易管理而且更容易閱讀。 如需詳細資訊，請參閱[大綱](/visualstudio/ide/outlining)。  
