...
-   不同於允許多個繼承的語言[!INCLUDE[vbprvb](~/includes/vbprvb-md.md)]允許只能有單一繼承類別; 也就是說，衍生的類別可以有只有一個基底類別。 雖然在類別中不允許多重繼承，類別可以實作多個介面，可以達到相同的端點。  
...
## <a name="inheritance-modifiers"></a>繼承修飾詞  
 [!INCLUDE[vbprvb](~/includes/vbprvb-md.md)]導入了下列類別層級的陳述式和修飾詞，以支援繼承：  
