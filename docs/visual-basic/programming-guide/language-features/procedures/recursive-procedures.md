 ...
 **記憶體使用量**。 您的應用程式對於本機變數的空間數量有限。 每次程式呼叫本身時，它會使用更多該空間來取得其區域變數的其他複本。 如果此程式無限期地繼續，最後會導致 <xref:System.StackOverflowException> 錯誤。
 ...
 **使用括弧呼叫**。 當 `Function` 程式以遞迴方式呼叫本身時，即使沒有引數清單，您也必須在程式名稱後面加上括弧。 否則，會採用函式名稱來表示函數的傳回值。
