...
Visual Basic 中的 XML 常值語法支援大部分的可延伸標記語言 (XML)  (XML) 1.0 規格。 如需 XML 1.0 規格的詳細資訊，請參閱 W3C 網站上的 [可延伸標記語言 (XML)  (xml) 1.0](https://www.w3.org/TR/xml) 。  
  
## <a name="what-visual-basic-does-not-support"></a>Visual Basic 不支援的內容  
  
- XML 常值不能包含 (DTD) 的檔案類型定義。  
  
- XML 檔常值的開頭必須是 XML 檔宣告。  
  
- XML 常值在同一行不能包含超過65535個字元。  
  
- XML 命名空間前置詞、元素名稱和屬性名稱不能超過1024個字元。  
