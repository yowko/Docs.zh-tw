...
<xref:System> 命名空間中 <xref:System.Convert> 類別的 <xref:System.Convert.ToInt32%2A> 方法會將 `True` 轉換為 + 1。
...
`String` 資料類型不會擴展到[Char 資料型別](../../../../visual-basic/language-reference/data-types/char-data-type.md)。 這表示如果您想要將常值指派給 `Char` 變數，則必須進行縮小轉換，或強制將常值設為 `Char` 類型。
