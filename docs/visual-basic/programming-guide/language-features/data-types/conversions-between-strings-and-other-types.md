...
## <a name="conversion-of-strings-to-numbers"></a>字串轉換成數字  
 ...
 當[!INCLUDE[vbprvb](~/includes/vbprvb-md.md)]轉換為數值的字串時，它會使用**地區選項**指定在 Windows 中設定**控制台**解譯千分位分隔符號、 小數點，和貨幣符號。 這表示，轉換可能會成功的其中一個下但沒有其他設定。 例如，`"$14.20"`是可接受的英文 （美國） 地區設定，但不是在法文地區設定。  
