...
<xref:System.TupleExtensions> 類別中的擴充方法可讓您輕鬆地在 Visual Basic 元組和 .NET**元組**物件之間進行轉換。 **ToTuple**方法會將 Visual Basic 元組轉換成 .Net**元組**物件，而**ToValueTuple**方法會將 .net**元組**物件轉換成 Visual Basic 元組。
