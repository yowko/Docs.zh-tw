 ...
 *「縮小轉換」* (narrowing conversion) 會將值變更為資料類型，而可能無法保留一些可能的值。 For example, a fractional value is rounded when it is converted to an integral type, and a numeric type being converted to `Boolean` is reduced to either `True` or `False`.  
  
## <a name="widening-conversions"></a>擴展轉換  
 The following table shows the standard widening conversions.  
  
|資料類型|Widens to data types <sup>1</sup>|  
|---|---|  
...
|任何類型|Any interface it implements.|  
