...
## 位元移位作業  
 位元移位作業會在位元模式上執行算術移位 \(Arithmetic Shift\)。  模式是在左方的運算元內，右方的運算元則指定要將模式移位的位置數目。  您可以使用 [\>\> Operator](../../../../visual-basic/language-reference/operators/right-shift-operator.md) 將模式移位到右方，或使用 [\<\< Operator](../Topic/%3C%3C%20Operator%20\(Visual%20Basic\).md) 將模式移位到左方。  
