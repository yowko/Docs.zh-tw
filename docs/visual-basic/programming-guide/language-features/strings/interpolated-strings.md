...
- *format-string* 是一個格式字串，適用於將格式化的物件類型。 例如，如果 <xref:System.DateTime> 值，它可能是[標準日期和時間格式字串](../../../../standard/base-types/standard-date-and-time-format-strings.md)，例如 "d" 或 "d"。
...
3. 將插入字串轉換為代表複合格式字串的 <xref:System.FormattableString> 變數。 檢查複合格式字串及其如何轉譯為結果字串有助於在建立查詢時，防止插入式攻擊。 <xref:System.FormattableString> 也包含：

      - 產生 <xref:System.FormattableString.ToString> 的結果字串的 <xref:System.Globalization.CultureInfo.CurrentCulture> 多載。

      - <xref:System.FormattableString.Invariant%2A> 的方法，會產生 <xref:System.Globalization.CultureInfo.InvariantCulture>的字串。
