...
|Provider|描述|  
|---|---|  
|LINQ to Objects|LINQ to Objects 提供者讓您查詢記憶體內部的集合和陣列。 如果物件支援 <xref:System.Collections.IEnumerable> 或 <xref:System.Collections.Generic.IEnumerable%601> 介面，LINQ to Objects 提供者就會讓您查詢。<br /><br /> 您可以藉由匯入 <xref:System.Linq> 命名空間（預設會針對所有 Visual Basic 專案匯入）來啟用 LINQ to Objects 提供者。<br /><br /> 如需有關 LINQ to Objects 提供者的詳細資訊，請參閱[LINQ to Objects](../../concepts/linq/linq-to-objects.md)。|  
...
## <a name="visual-basic-features-that-support-linq"></a>支援 LINQ 的 Visual Basic 功能  
 Visual Basic 包含其他顯著的功能，可讓您使用 LINQ simple，並減少您必須撰寫來執行 LINQ 查詢的程式碼數量。 包含以下各項：  
