...
當兩個程式項目共用相同名稱時，其中之一可以隱藏，也就是「*遮蔽*」\(Shadow\) 另一個。  在這種情況下，無法參考受遮蔽項目 \(Shadowed Element\)，而在程式碼使用項目名稱時，[!INCLUDE[vbprvb](../../../../csharp/programming-guide/concepts/linq/includes/vbprvb-md.md)] 編譯器會將它解析為主導遮蔽項目 \(Shadowing Element\)。  
