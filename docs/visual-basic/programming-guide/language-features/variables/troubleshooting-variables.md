...
此頁面列出在 Visual Basic 中使用變數時可能發生的一些常見問題。  
...
## <a name="other-components-cannot-access-your-variable"></a>其他元件無法存取您的變數  
 ...
 Visual Basic 名稱不區分 *大小寫*。 如果兩個名稱只有字母大小寫不同，則編譯器會將它們解譯成相同的名稱。 例如，它會將 `ABC` 和 `abc` 視為相同的宣告元素。  
...
## <a name="wrong-variable-being-used"></a>使用錯誤的變數  
 ...
 當您有多個具有相同名稱的變數時，Visual Basic 編譯器會嘗試解析該名稱的每個參考。 如果變數具有不同的範圍，則編譯器會用最小的範圍來解析宣告的參考。 如果範圍相同，則解析會失敗，且編譯器會發出錯誤信號。 如需詳細資訊，請參閱 [References to Declared Elements](../declared-elements/references-to-declared-elements.md)。  
