...
- 介面類別型，例如 <xref:System.IComparable> 或 <xref:System.IDisposable>  
 ...
 您可以在一個語句中宣告數個變數，而不需要重複資料類型。 在下列語句中，`i`、`j`和 `k` 的變數會宣告為類型 `Integer`、`l` 和 `m` 做為 `Long`，並 `x` 和 `y` 為 `Single`：  
 ...
 *型別推斷*用來判斷未使用 `As` 子句宣告之區域變數的資料類型。 編譯器會從初始化運算式的類型推斷變數的類型。 這可讓您宣告變數，而不需要明確陳述型別。 在下列範例中，`num1` 和 `num2` 都是強型別做為整數。  
...
## <a name="characteristics-of-declared-variables"></a>已宣告變數的特性  
 變數的*存留*期是可供使用的時間週期。 一般而言，只要宣告它的元素（例如程式或類別）仍存在，變數就會存在。 如果變數不需要繼續存在於其包含專案的存留期以外，您就不需要在宣告中執行任何特殊動作。 如果變數需要繼續存在的時間超過其包含的元素，您可以在其 `Dim` 語句中包含 `Static` 或 `Shared` 關鍵字。 如需詳細資訊，請參閱[Visual Basic 中的存留期](../../../../visual-basic/programming-guide/language-features/declared-elements/lifetime.md)。  
 ...
 變數的*存取層級*是有權存取它的程式碼範圍。 這是由您在 `Dim` 語句中使用的存取修飾詞（例如[公用](../../../../visual-basic/language-reference/modifiers/public.md)或[私](../../../../visual-basic/language-reference/modifiers/private.md)用）所決定。 如需詳細資訊，請參閱[Visual Basic 中的存取層級](../../../../visual-basic/programming-guide/language-features/declared-elements/access-levels.md)。  
