...
5. 遵循 `As` 子句加上等號（`=`），並在等號後面加上變數的初始值。

    Visual Basic 會在每次執行 `Dim` 語句時，將指定的值指派給變數。 如果您未指定初始值，Visual Basic 會在第一次輸入包含 `Dim` 語句的程式碼時，為變數的資料類型指派預設初始值。

    如果變數是參考型別，您可以在 `As` 子句中包含[New Operator](../../../../visual-basic/language-reference/operators/new-operator.md)關鍵字，藉以建立其類別的實例。 如果您未使用 `New`，變數的初始值就是不是[任何](../../../../visual-basic/language-reference/nothing.md)值。
