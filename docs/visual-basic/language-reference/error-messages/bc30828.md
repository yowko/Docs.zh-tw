...
@No__t 0 資料類型與 Visual Basic 6.0 和更早版本中的 @no__t 1 語句搭配使用，以允許使用可包含任何資料類型的引數。 但是 Visual Basic 支援多載，因此 @no__t 0 資料類型會過時。  
...
2. 當所呼叫的程式預期 `Void*` 時，請使用 <xref:System.Runtime.InteropServices.MarshalAsAttribute> 屬性來指定 `As Any`。  
