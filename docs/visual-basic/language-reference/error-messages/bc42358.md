...
---
# <a name="bc42358-because-this-call-is-not-awaited-execution-of-the-current-method-continues-before-the-call-is-completed"></a>BC42358：因為未等候此呼叫，所以在呼叫完成之前會繼續執行目前的方法

因為未等候此呼叫，所以在呼叫完成之前會繼續執行目前的方法。 請考慮將 `Await` 運算子套用至呼叫的結果。

目前的方法會呼叫傳回或的非同步方法， <xref:System.Threading.Tasks.Task> <xref:System.Threading.Tasks.Task%601> 而不會將 [Await](../operators/await-operator.md) 運算子套用至結果。 非同步方法的呼叫會啟動非同步工作。 不過，由於不會套用任何 `Await` 運算子，因此程式會繼續執行，而不等候工作完成。 在大部分情況下，不應該有此行為。 通常呼叫方法的其他方面取決於呼叫的結果，或至少被呼叫的方法必須完成，才能從包含呼叫的方法傳回。
...
下列 Windows Presentation Foundation (WPF) 應用程式包含了前述範例的方法。 下列步驟會設定應用程式：

1. 建立 WPF 應用程式，並將其命名為 `AsyncWarning`。

2. 在 Visual Studio 程式碼編輯器中，選擇 [ **MainWindow.xaml** ] 索引標籤。

     如果看不到索引標籤，請在 **方案總管**中開啟 MainWindow 的快捷方式功能表，然後選擇 [ **View Code**]。

3. 以下列程式碼取代 MainWindow 的 **xaml** 視圖中的程式碼：
    ...
    ```
     ...
     如需 XAML 設計工具的詳細資訊，請參閱 [Creating a UI by using XAML Designer](/visualstudio/xaml-tools/creating-a-ui-by-using-xaml-designer-in-visual-studio)。 如需如何建置您自己的簡單 UI 的資訊，請參閱[逐步解說：使用 Async 和 Await 存取 Web](../../programming-guide/concepts/async/walkthrough-accessing-the-web-by-using-async-and-await.md)的＜建立 WPF 應用程式＞和＜設計簡單的 WPF MainWindow＞章節。
