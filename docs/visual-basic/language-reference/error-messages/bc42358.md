...
目前的方法會呼叫非同步方法，以傳回 <xref:System.Threading.Tasks.Task> 或 <xref:System.Threading.Tasks.Task%601>，而不會將[Await](../operators/await-operator.md)運算子套用至結果。 非同步方法的呼叫會啟動非同步工作。 不過，由於不會套用任何 `Await` 運算子，因此程式會繼續執行，而不等候工作完成。 在大部分情況下，不應該有此行為。 通常呼叫方法的其他方面取決於呼叫的結果，或至少被呼叫的方法必須完成，才能從包含呼叫的方法傳回。
...
3. 使用下列程式碼取代 Mainwindow.xaml 之**xaml**視圖中的程式碼：
    ...
    ```
     ...
     如需 XAML 設計工具的詳細資訊，請參閱[使用 XAML 設計工具建立 UI](/visualstudio/xaml-tools/creating-a-ui-by-using-xaml-designer-in-visual-studio)。 如需如何建置您自己的簡單 UI 的資訊，請參閱[逐步解說：使用 Async 和 Await 存取 Web](../../../visual-basic/programming-guide/concepts/async/walkthrough-accessing-the-web-by-using-async-and-await.md)的＜建立 WPF 應用程式＞和＜設計簡單的 WPF MainWindow＞章節。
