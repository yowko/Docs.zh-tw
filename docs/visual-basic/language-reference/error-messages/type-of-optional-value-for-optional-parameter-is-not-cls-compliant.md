...
---
# <a name="type-of-optional-value-for-optional-parameter-parametername-is-not-cls-compliant"></a>選擇性參數 \<parametername> 的選擇性值類型不符合 CLS 標準
程序已標記為 `<CLSCompliant(True)>`，但所宣告的 [Optional](../modifiers/optional.md) 參數卻具有不符合標準之型別的預設值。  
...
- 如果選擇性參數必須有此特定類型的預設值，請移除 <xref:System.CLSCompliantAttribute> 。 程序不符合 CLS 標準。  
