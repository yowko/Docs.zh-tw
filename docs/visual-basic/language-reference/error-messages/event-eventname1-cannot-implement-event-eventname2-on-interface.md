...
[!INCLUDE[vbprvb](../../../csharp/programming-guide/concepts/linq/includes/vbprvb-md.md)] 無法實作事件，因為事件的委派型別 \(Delegate Type\) 和介面中事件的委派型別不相符。  當您在介面中定義多個事件，然後嘗試以相同的事件同時實作這些事件時，便可能發生這個錯誤。  只有當所有實作的事件都是使用 `As` 語法宣告，而且指定相同的委派型別時，一個事件才可以實作兩個或多個事件。  
