...
## <a name="run-time-errors"></a>執行階段錯誤  
 ...
 如果 Visual Basic 應用程式嘗試執行系統無法執行的動作，就會發生執行階段錯誤，而 Visual Basic 會擲回 `Exception` 物件。 Visual Basic 可以使用語句來產生任何資料類型（包括物件）的自訂錯誤 `Exception` `Throw` 。 應用程式可以透過顯示已攔截例外狀況的錯誤碼和訊息來識別錯誤。 如果未攔截到錯誤，應用程式便會結束。  
