...
|詞彙|定義|  
|---|---|  
|`object`|必要項。 A queryable collection. That is, a collection that implements <xref:System.Collections.Generic.IEnumerable%601> or <xref:System.Linq.IQueryable%601>.|  
|(|必要項。 Denotes the start of the indexer property.|  
|`index`|必要項。 An integer expression that specifies the zero-based position of an element of the collection.|  
|)|必要項。 Denotes the end of the indexer property.|  
