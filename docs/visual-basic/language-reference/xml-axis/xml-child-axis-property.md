...
|. <|必要。 代表子軸屬性的開頭。|  
|`child`|必要。 要存取的子節點名稱，格式為 `[prefix:]name` 。<br /><br /> -   `Prefix`選擇性. 子節點的 XML 命名空間前置詞。 必須是以 `Imports` 陳述式定義的全域 XML 命名空間。<br />-   `Name`具備. 本機子節點名稱。 請參閱宣告[的 XML 元素和屬性的名稱](../../programming-guide/language-features/xml/names-of-declared-xml-elements-and-attributes.md)。|  
|>|必要。 代表子軸屬性的結尾。|  
...
## <a name="remarks"></a>備註  
 您可以使用 XML 子軸屬性，從 <xref:System.Xml.Linq.XElement> 或 <xref:System.Xml.Linq.XDocument> 物件，或從 <xref:System.Xml.Linq.XElement> 或 <xref:System.Xml.Linq.XDocument> 物件集合依據名稱存取子節點。 使用 XML `Value` 屬性來存取傳回的集合中第一個子節點的值。 如需詳細資訊，請參閱[XML Value 屬性](xml-value-property.md)。  
  
 Visual Basic 編譯器會將子軸屬性轉換成方法的呼叫 <xref:System.Xml.Linq.XContainer.Elements%2A> 。  
  
## <a name="xml-namespaces"></a>XML 命名空間  
 子軸屬性中的名稱只可以使用以 `Imports` 陳述式全域宣告的 XML 命名空間前置詞。 它不能使用在 XML 項目常值內本機宣告的 XML 命名空間前置詞。 如需詳細資訊，請參閱[Imports 語句（XML 命名空間）](../statements/imports-statement-xml-namespace.md)。  
