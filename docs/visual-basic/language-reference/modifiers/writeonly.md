...
- **覆寫.** 如果屬性是類別的成員，則允許它預設為[NotOverridable](../../../visual-basic/language-reference/modifiers/notoverridable.md)，而且不會將它宣告 `Overridable` 或 `MustOverride`。 這可防止衍生類別透過覆寫進行不想要的存取。  
