...
- **Declaration Context.** 您只能在模組層級使用 `Shared`。 This means the declaration context for a `Shared` element must be a class or structure, and cannot be a source file, namespace, or procedure.  
