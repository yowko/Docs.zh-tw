...
## <a name="result"></a>結果  
 結果是除以的完整商 `expression1` `expression2` ，包括任何餘數。  
...
|一個運算式是[Decimal](../data-types/decimal-data-type.md)資料型別，另一個則不是[單一](../data-types/single-data-type.md)或[雙精度浮點數](../data-types/double-data-type.md)|`Decimal`|  
|任一運算式是[Double](../data-types/double-data-type.md)資料類型|`Double`|  
  
 在執行除法之前，任何整數數值運算式會擴展為 `Double` 。 如果您將結果指派給整數資料類型，Visual Basic 會嘗試將的結果從轉換成 `Double` 該類型。 如果結果不符合該類型，這可能會擲回例外狀況。 特別是，請參閱此說明頁面上的「嘗試除數為零」。  
...
|運算元資料類型|當 `expression2` 為零時的行為|  
|------------------------|---------------------------------------|  
|浮點（ `Single` 或 `Double` ）|<xref:System.Double.PositiveInfinity> <xref:System.Double.NegativeInfinity> <xref:System.Double.NaN> 如果 `expression1` 同時為零，則傳回無限大（或），或（不是數位）|  
|`Decimal`|引發<xref:System.DivideByZeroException>|  
|整數（帶正負號或不帶正負號）|<xref:System.OverflowException>因為整數類型無法接受 <xref:System.Double.PositiveInfinity> 、或，所以嘗試轉換回整數類資料類型 <xref:System.Double.NegativeInfinity><xref:System.Double.NaN>|  
...
## <a name="example"></a>範例  
 ...
 上述範例中的運算式會傳回2.5 和3.333333 的值。 請注意， `Double` 即使兩個運算元都是整數常數，結果一律是浮點（）。  
