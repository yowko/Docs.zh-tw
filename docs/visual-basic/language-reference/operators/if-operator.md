...
|`argument2`|必要。 `Object` 如果 `argument1` 評估為 `True`，則評估並傳回。|
|`argument3`|必要。 `Object` 如果 `argument1` 評估為 `False`，或如果 `argument1` 是評估為[無](../../../visual-basic/language-reference/nothing.md)的[可為 null](../../../visual-basic/programming-guide/language-features/data-types/nullable-value-types.md)`Boolean` 變數，則評估並傳回。|
...
|詞彙|定義|
|---|---|
|`argument2`|必要。 `Object` 必須是參考或可為 null 的類型。 評估並傳回（當它評估為 `Nothing`以外的任何專案時）。|
|`argument3`|必要。 `Object` 如果 `argument2` 評估為 `Nothing`，則評估並傳回。|
