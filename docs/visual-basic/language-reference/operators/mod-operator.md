...
運算子可以多載 *，這*表示類別或結構可以重新定義其行為。 `Mod` 如果您的程式`Mod`代碼適用于包含這類多載的類別或結構實例，請務必瞭解其已重新定義的行為。 如需詳細資訊，請參閱 [Operator Procedures](../../../visual-basic/programming-guide/language-features/procedures/operator-procedures.md)。
