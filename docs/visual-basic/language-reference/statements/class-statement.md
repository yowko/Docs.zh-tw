...
## 備註  
 `Class` 陳述式會定義新的資料型別。  「*類別*」是物件導向程式設計 \(OOP\) 的基礎建置組塊 \(Building Block\)。  如需詳細資訊，請參閱 [Objects and Classes](../../../visual-basic/reference/command-line-compiler/index.md)。  
...
-   **物件導向** ：類別為物件導向，但模組不是。  您可以對類別建立一或多個執行個體。  如需詳細資訊，請參閱 [Objects and Classes](../../../visual-basic/reference/command-line-compiler/index.md)。  
