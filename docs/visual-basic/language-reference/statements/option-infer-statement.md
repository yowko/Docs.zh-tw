...
> [!NOTE]
> 當變數被宣告為 `Object` 時，執行階段類型可以在程式執行時變更。 Visual Basic 會執行稱為「*裝箱*」和「*取消裝箱*」的作業，以便在 `Object` 和實數值型別之間進行轉換，使執行速度變慢。 如需有關裝箱和取消裝箱的詳細資訊，請參閱[Visual Basic 語言規格](~/_vblang/spec/conversions.md#value-type-conversions)。
...
當您建立新的專案時，[**編譯**] 索引標籤上的 [推斷設定]**選項**會設定為 [ **VB 預設值**] 對話方塊中的**選項 [推斷**設定]。 若要存取 [ **VB 預設值**] 對話方塊，請在 [**工具**] 功能表上按一下 [**選項**]。 在 [選項] 對話方塊中，展開 [專案和方案]，然後按一下 [VB 預設值]。 **VB 預設值**中的初始預設設定是 `On`。
