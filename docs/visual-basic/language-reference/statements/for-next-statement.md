...
|`datatype`|選擇項。 Data type of `counter`. For more information, see [Counter Argument](#BKMK_Counter) later in this topic.|
|`start`|必要項。 Numeric expression. `counter` 的初始值。|
|`end`|必要項。 Numeric expression. The final value of `counter`.|
|`step`|選擇項。 Numeric expression. The amount by which `counter` is incremented each time through the loop.|
|`statements`|選擇項。 One or more statements between `For` and `Next` that run the specified number of times.|
|`Continue For`|選擇項。 Transfers control to the next loop iteration.|
|`Exit For`|選擇項。 Transfers control out of the `For` loop.|
|`Next`|必要項。 Terminates the definition of the `For` loop.|
...
|----------------------------|-----------------------------------|-------------------------------------------------------------------------------------------------------------|
|否|[是]|No, because `counter` is already defined. If the scope of `counter` isn't local to the procedure, a compile-time warning occurs.|
