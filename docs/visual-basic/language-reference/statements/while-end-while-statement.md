...
|詞彙|定義|  
|---|---|  
|`condition`|必要。 `Boolean` 運算式。 如果`condition`是`Nothing`，Visual Basic 會將它視為`False`。|  
|`statements`|選擇性。 一或多個陳述式下列`While`，每次執行的`condition`是`True`。|  
|`Continue While`|選擇性。 將控制項傳送至下一個反覆運算`While`區塊。|  
|`Exit While`|選擇性。 控制權轉移出`While`區塊。|  
