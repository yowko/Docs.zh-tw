...
|詞彙|定義|  
|---|---|  
|`condition`|必要項。 `Boolean` expression. If `condition` is `Nothing`, Visual Basic treats it as `False`.|  
|`statements`|選擇項。 One or more statements following `While`, which run every time `condition` is `True`.|  
|`Continue While`|選擇項。 Transfers control to the next iteration of the `While` block.|  
|`Exit While`|選擇項。 Transfers control out of the `While` block.|  
