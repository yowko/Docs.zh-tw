...
## 備註  
 ...
 類別或結構會為介面所定義的每個成員提供程式碼，以便實作介面。  最後，當應用程式根據該類別或結構建立介面時，此物件會存在並於記憶體中執行。  如需詳細資訊，請參閱 [Objects and Classes](../../../visual-basic/reference/command-line-compiler/index.md)和 [Interfaces](../../../visual-basic/programming-guide/language-features/interfaces/index.md)。  
