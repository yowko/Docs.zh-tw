...
`attributelist`  
選擇性。 適用于此參數的屬性清單。 您必須將[屬性清單](../../../visual-basic/language-reference/statements/attribute-list.md)放在角括弧中（「`<`」和「`>`」）。
...
`defaultvalue`  
`Optional` 參數所需。 評估為參數資料類型的任何常數或常數運算式。 如果類型是 `Object`，或是類別、介面、陣列或結構，則預設值只能 `Nothing`。
