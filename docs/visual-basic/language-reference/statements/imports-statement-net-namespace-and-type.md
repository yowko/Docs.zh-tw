...
|詞彙|定義|  
|---|---|  
|`aliasname`|選擇項。 匯*入別名*或名稱，由程式碼用來參考 `namespace`，而不是完整限定字串。 請參閱 [Declared Element Names](../../../visual-basic/programming-guide/language-features/declared-elements/declared-element-names.md)。|  
|`namespace`|必要項。 要匯入之命名空間的完整名稱。 可以是嵌套于任何層級的命名空間字串。|  
|`element`|選擇項。 命名空間中宣告的程式設計項目名稱。 可以是任何容器元素。|  
  
## <a name="remarks"></a>備註  
 @No__t_0 語句可讓您直接參考指定命名空間中包含的類型。  
  
 您可以提供單一命名空間名稱或一個嵌套命名空間的字串。 每個嵌套的命名空間會與下一個較高層級的命名空間分開（以句點（`.`）分隔），如下列範例所示。  
...
### <a name="element-names"></a>項目名稱  
 ...
 @No__t_0 語句所提供的元素範圍，視您是否指定 `element` 而定。 如果您只指定 `namespace`，就可以使用該命名空間的所有唯一名稱成員，以及該命名空間內的容器元素成員，而無需限定。 如果您同時指定 `namespace` 和 `element`，則只有該元素的成員可以使用，而不需要限定。  
  
## <a name="example"></a>範例  
 下列範例會傳回 C：\ 中的所有資料夾目錄，方法是使用 <xref:System.IO.DirectoryInfo> 類別。  
  
 此程式碼在檔案的頂端沒有 `Imports` 的語句。 因此，`DirectoryInfo`、<xref:System.Text.StringBuilder> 和 <xref:Microsoft.VisualBasic.ControlChars.CrLf> 參考都是以命名空間完整限定。  
