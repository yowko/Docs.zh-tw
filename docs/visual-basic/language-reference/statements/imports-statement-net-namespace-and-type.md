...
|詞彙|定義|
|---|---|
|`aliasname`|選擇性。 匯*入別名*或名稱，由程式碼用來參考 `namespace`，而不是完整限定字串。 請參閱 [Declared Element Names](../../programming-guide/language-features/declared-elements/declared-element-names.md)。|
|`namespace`|必要。 要匯入之命名空間的完整名稱。 可以是嵌套于任何層級的命名空間字串。|
