...
-   **實作。** 如果這個屬性會使用`Implements`關鍵字、 包含類別或結構必須`Implements`陳述式之後立即其`Class`或`Structure`陳述式。 `Implements`陳述式必須包含在指定的每個介面`implementslist`。 不過，介面定義的名稱`Property`(在`definedname`) 並沒有這個屬性的名稱相同 (在`name`)。  
