...
## <a name="attributes-and-modifiers"></a>屬性和修飾詞  
 ...
 您可以指定`WithEvents`只對成員變數，而非程序內的區域變數。 如果您指定`WithEvents`，變數的資料型別不可以是特定類別類型時， `Object`。 您無法宣告陣列`WithEvents`。 如需事件的詳細資訊，請參閱[事件](../../../visual-basic/programming-guide/language-features/events/index.md)。  
...
## <a name="releasing-managed-resources"></a>釋放 Managed 的資源  
 ...
 如果類別是由特別有用，且很少資源 （例如資料庫連接或檔案控制代碼） 所佔用，您可能不想等到下一個記憶體回收清除不再使用的類別執行個體。 類別可以實作<xref:System.IDisposable>介面，以提供方法來釋放記憶體回收之前的資源。</xref:System.IDisposable> 實作該介面的類別會公開`Dispose`強制立即釋出寶貴的資源，可以呼叫的方法。  
  
 `Using`陳述式會自動取得資源、 執行一組陳述式，然後處置資源的程序。 不過，資源必須實作<xref:System.IDisposable>介面。</xref:System.IDisposable> 如需詳細資訊，請參閱[Using 陳述式](../../../visual-basic/language-reference/statements/using-statement.md)。  
  
## <a name="example"></a>範例  
 下列範例會使用宣告變數`Dim`陳述式搭配各種選項。  
  
 [!code-vb[VbVbalrStatements #&141; 毫秒](../../../visual-basic/language-reference/error-messages/codesnippet/VisualBasic/dim-statement_1.vb)]  
