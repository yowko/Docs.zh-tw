...
## <a name="remarks"></a>備註  
 ...
 `eventhandler` 程式的簽章必須符合事件 `event`的簽章。  
...
> [!NOTE]
> 若為自訂事件，`AddHandler` 語句會叫用事件的 `AddHandler` 存取子。 如需自訂事件的詳細資訊，請參閱[Event 語句](../../../visual-basic/language-reference/statements/event-statement.md)。  
