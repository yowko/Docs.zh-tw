...
|組件|描述|  
|---|---|  
|`attrlist`|選擇項。 套用至此事件的屬性清單。 以逗號分隔多個屬性。 您必須將[屬性清單](../../../visual-basic/language-reference/statements/attribute-list.md)放在角括弧中（「`<`」和「`>`」）。|  
...
|`parameterlist`|選擇項。 本機變數清單，表示此事件的參數。 您必須將[參數清單](../../../visual-basic/language-reference/statements/parameter-list.md)括在括弧中。|  
