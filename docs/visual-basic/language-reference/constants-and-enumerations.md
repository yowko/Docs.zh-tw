...
[!INCLUDE[vbprvb](../../csharp/programming-guide/concepts/linq/includes/vbprvb-md.md)] 提供一些預先定義的常數和列舉型別供開發人員使用。  常數會儲存在應用程式的執行過程中仍為常數的值。  列舉型別提供使用相關常數組和建立常數值與名稱間關聯的便利方法。  
...
|`TARGET`|字串，表示專案的輸出型別或命令列 **\/target** 選項。  `TARGET` 的可能值為：<br /><br /> -   用於 Windows 應用程式 "winexe"。<br />-   用於主控台應用程式的 "exe"。<br />-   用於類別庫的 "library"。<br />-   用於模組 "module"。<br />-   **\/target** 選項可於 [!INCLUDE[vsprvs](../../csharp/includes/vsprvs-md.md)] 整合式開發環境中設定。  如需詳細資訊，請參閱 [\/target](../../visual-basic/reference/command-line-compiler/target.md)。|  
...
|`VBC_VER`|此號碼代表 [!INCLUDE[vbprvb](../../csharp/programming-guide/concepts/linq/includes/vbprvb-md.md)] 版本，其格式為 *major*.*minor*。  [!INCLUDE[vbprvblong](../../visual-basic/developing-apps/customizing-extending-my/includes/vbprvblong-md.md)] 的版本號碼為 8.0。|  
...
## 列舉  
 下表列出並描述 [!INCLUDE[vbprvb](../../csharp/programming-guide/concepts/linq/includes/vbprvb-md.md)] 所提供的列舉型別。  
