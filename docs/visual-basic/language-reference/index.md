---
title: "Visual Basic 語言參考"
ms.date: 07/20/2015
ms.prod: .net
ms.technology: devlang-visual-basic
ms.topic: article
helpviewer_keywords:
- Visual Basic language reference
- run-time libraries, Visual Basic
- libraries, Visual Basic run-time
- Visual Basic code, run-time libraries
- Visual Basic code, language reference
- language reference, Visual Basic
- Visual Basic, language reference
ms.assetid: 5c6bd31a-9542-4f3e-bfd5-e951af58f0f2
caps.latest.revision: "17"
author: dotnet-bot
ms.author: dotnetcontent
ms.openlocfilehash: 2524df987bfea25b294c6e24abe5a6d8ff548df1
ms.sourcegitcommit: bd1ef61f4bb794b25383d3d72e71041a5ced172e
ms.translationtype: MT
ms.contentlocale: zh-TW
ms.lasthandoff: 10/18/2017
---
# <a name="visual-basic-language-reference"></a>Visual Basic 語言參考
本節提供 [!INCLUDE[vbprvb](~/includes/vbprvb-md.md)] 語言各種層面的參考資訊。  
  
## <a name="in-this-section"></a>本章節內容  
 [印刷樣式與程式碼慣例](../../visual-basic/language-reference/typographic-and-code-conventions.md)  
 摘要說明用來格式化 [!INCLUDE[vbprvb](~/includes/vbprvb-md.md)] 中語言的關鍵字、預留位置和其他項目的方式。  
  
 [Visual Basic 執行階段程式庫成員](../../visual-basic/language-reference/runtime-library-members.md)  
 列出 <xref:Microsoft.VisualBasic?displayProperty=nameWithType> 命名空間的類別和模組，以及其成員函式、方法、屬性、常數和列舉的連結。  
  
 [關鍵字](../../visual-basic/language-reference/keywords/index.md)  
 列出所有 Visual Basic 關鍵字，並提供更多資訊的連結。  
  
 [屬性 (Visual Basic)](../../visual-basic/language-reference/attributes.md)  
 記錄 [!INCLUDE[vbprvb](~/includes/vbprvb-md.md)] 中可用的屬性。  
  
 [常數和列舉](../../visual-basic/language-reference/constants-and-enumerations.md)  
 記錄 [!INCLUDE[vbprvb](~/includes/vbprvb-md.md)] 中可用的常數和列舉。  
  
 [資料類型](../../visual-basic/language-reference/data-types/data-type-summary.md)  
 記錄 [!INCLUDE[vbprvb](~/includes/vbprvb-md.md)] 中可用的資料類型。  
  
 [指示詞](../../visual-basic/language-reference/directives/directives.md)  
 記錄 [!INCLUDE[vbprvb](~/includes/vbprvb-md.md)] 中可用的編譯器指示詞。  
  
 [函式](../../visual-basic/language-reference/functions/index.md)  
 記錄 [!INCLUDE[vbprvb](~/includes/vbprvb-md.md)] 中可用的執行階段函式。  
  
 [修飾詞](../../visual-basic/language-reference/modifiers/index.md)  
 列出 Visual Basic 執行階段修飾詞，並提供更多資訊的連結。  
  
 [模組](../../visual-basic/language-reference/modules.md)  
 記錄 [!INCLUDE[vbprvb](~/includes/vbprvb-md.md)] 中可用的模組和其成員。  
  
 [Nothing](../../visual-basic/language-reference/nothing.md)  
 描述任何資料類型的預設值。  
  
 [物件](../../visual-basic/language-reference/objects/index.md)  
 記錄 [!INCLUDE[vbprvb](~/includes/vbprvb-md.md)] 中可用的物件和其成員。  
  
 [運算子](../../visual-basic/language-reference/operators/index.md)  
 記錄 [!INCLUDE[vbprvb](~/includes/vbprvb-md.md)] 中可用的運算子。  
  
 [屬性](../../visual-basic/language-reference/properties.md)  
 記錄 [!INCLUDE[vbprvb](~/includes/vbprvb-md.md)] 中可用的屬性。  
  
 [查詢](../../visual-basic/language-reference/queries/queries.md)  
 提供在程式碼中使用 [!INCLUDE[vbteclinqext](~/includes/vbteclinqext-md.md)] 運算式的參考資訊。  
  
 [陳述式](../../visual-basic/language-reference/statements/index.md)  
 記錄 [!INCLUDE[vbprvb](~/includes/vbprvb-md.md)] 中可用的宣告和可執行陳述式。  
  
 [XML 註解標記](../../visual-basic/language-reference/xmldoc/recommended-xml-tags-for-documentation-comments.md)  
 描述 [!INCLUDE[vbprvb](~/includes/vbprvb-md.md)] 程式碼編輯器中所提供 IntelliSense 的文件註解。  
  
 [XML 軸屬性](../../visual-basic/language-reference/xml-axis/xml-axis-properties.md)  
 提供使用 XML 軸屬性直接在程式碼中存取 XML 之資訊的連結。  
  
 [XML 常值](../../visual-basic/language-reference/xml-literals/index.md)  
 提供使用 XML 常值直接在程式碼中包含 XML 之資訊的連結。  
  
 [錯誤訊息](../../visual-basic/language-reference/error-messages/index.md)  
 提供 [!INCLUDE[vbprvb](~/includes/vbprvb-md.md)] 編譯器和執行階段錯誤訊息的清單，以及其處理方式的說明。  
  
## <a name="related-sections"></a>相關章節  
 [Visual Basic](../../visual-basic/index.md)  
 提供 [!INCLUDE[vbprvb](~/includes/vbprvb-md.md)] 語言所有區域的完整說明。  
  
 [Visual Basic 命令列編譯器](../../visual-basic/reference/command-line-compiler/index.md)  
 描述如何使用命令列編譯器作為從 [!INCLUDE[vsprvs](~/includes/vsprvs-md.md)] 整合式開發環境 (IDE) 編譯程式的替代方法。
