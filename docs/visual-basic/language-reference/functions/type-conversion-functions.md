...
從 Visual Basic 15.8 開始，當您將下列方法所傳回的 <xref:System.Single> 或 <xref:System.Double> 值傳遞給其中一個整數轉換函數（`CByte`、`CShort`、`CInt`、`CLng`、`CSByte`、`CUShort`、`CUInt`、`CULng`）時，就會優化浮點對整數轉換的效能：
...
`CChar` 的輸入引數必須是 `Char` 或 `String`的資料類型。 您無法使用 `CChar` 將數位轉換成字元，因為 `CChar` 無法接受數值資料類型。 下列範例會取得代表程式碼點（字元碼）的數位，並將它轉換成對應的字元。 它會使用 <xref:Microsoft.VisualBasic.Interaction.InputBox%2A> 函式來取得數位的字串，`CInt` 將字串轉換成類型 `Integer`，以及 `ChrW` 將數位轉換成類型 `Char`。
...
下列範例會使用 `CObj` 函數將數值轉換成 `Object`。 `Object` 變數本身只包含四個位元組的指標，指向指派給它的 `Double` 值。
