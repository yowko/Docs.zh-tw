...
## <a name="remarks"></a>備註  
 ...
 使用 `String` 資料類型來保存多個字元，而不會有 `Char()`的陣列管理額外負荷，也就是 `Char` 元素的陣列。  
  
 `String` 的預設值為 `Nothing` （null 參考）。 請注意，這與空字串（值 `""`）不同。  
...
## <a name="unicode-characters"></a>Unicode 字元  
 ...
 您可以使用 <xref:System.Char.IsDigit%2A> 的方法，並 <xref:System.Char.IsPunctuation%2A> `String` 變數中的個別字元來判斷其 Unicode 分類。  
  
## <a name="format-requirements"></a>格式需求  

 您必須將 `String` 常值括在引號內（`" "`）。 如果您必須在字串中包含引號做為其中一個字元，您可以使用兩個連續的引號（`""`）。 說明如下例。  
...
```  
 ...
 請注意，在字串中代表引號的連續引號，與開始和結束 `String` 常值的引號無關。  
  
## <a name="string-manipulations"></a>字串操作  

 當您將字串指派給 `String` 變數之後，該字串是*不可變*的，這表示您無法變更其長度或內容。 當您以任何方式改變字串時，Visual Basic 會建立新的字串，並放棄上一個字串。 然後 `String` 變數會指向新的字串。  
  
 您可以使用各種字串函數來操作 `String` 變數的內容。 下列範例說明 <xref:Microsoft.VisualBasic.Strings.Left%2A> 函式  
...
- **負數。** 請記住，`String` 所保留的字元不帶正負號，而且不能代表負值。 在任何情況下，您不應該使用 `String` 來保存數值。  
  
- **Interop 考慮。** 如果您要使用的元件不是針對 .NET Framework 所撰寫（例如 Automation 或 COM 物件），請記住，在其他環境中，字串字元具有不同的資料寬度（8位）。 如果您要將8位字元的字串引數傳遞給這類元件，請將它宣告為 `Byte()`、`Byte` 元素的陣列，而不是新的 Visual Basic 程式碼中的 `String`。  
  
- **輸入字元。** 將識別項型別字元 `$` 附加至任何識別碼，會強制其成為 `String` 的資料類型。 `String` 沒有常數值型別字元。 不過，編譯器會將括在引號中的常值（`" "`）視為 `String`。  
