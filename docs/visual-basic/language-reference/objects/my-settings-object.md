...
## <a name="remarks"></a>備註  
 ...
 `My.Settings`物件可讓您存取應用程式的設定，並可讓您以動態方式儲存和取出應用程式的屬性設定和其他資訊。 如需詳細資訊，請參閱[管理應用程式設定 (.NET)](/visualstudio/ide/managing-application-settings-dotnet)。  
...
- **範圍** 指出屬性是否為唯讀。 如果值為 **Application**，則屬性是唯讀的;如果值為 **User**，則屬性為讀寫。  
...
|方法|說明|  
|---|---|  
|`Reload`|重載上次儲存值的使用者設定。|  
|`Save`|儲存目前的使用者設定。|  
  
 `My.Settings`物件也提供繼承自類別的 advanced 屬性和方法 <xref:System.Configuration.ApplicationSettingsBase> 。  
