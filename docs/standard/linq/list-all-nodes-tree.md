...
---
# <a name="how-to-list-all-nodes-in-a-tree-linq-to-xml"></a>如何列出樹狀結構中的所有節點 (LINQ to XML) 

有時候，列出樹狀結構中的所有節點會很有説明，例如，要瞭解方法或屬性如何影響樹狀結構的確切方式。 下列 c # 和 Visual Basic 範例會列出樹狀結構的節點。 每個節點都會以 XPath 運算式來表示，該運算式會指定樹狀結構中節點的位置。

> [!NOTE]
> 使用 LINQ to XML 執行 XPath 運算式並不特別有用。 LINQ to XML 查詢的執行效果優於 XPath 運算式，而且功能更強大。 不過，若是要識別 XML 樹狀中的節點，XPath 就非常適合。

## <a name="example-use-xpath-to-list-all-nodes-in-a-tree"></a>範例：使用 XPath 列出樹狀結構中的所有節點

這個範例有一個名為 `GetXPath` 的函式，它會針對 XML 樹狀結構中的任何節點產生特定的 XPath 運算式。 即使節點位於命名空間中，它也會執行這項工作。 所產生之 XPath 運算式中的元素有必要的命名空間前置詞。

此範例會先建立包含數種節點類型的小型 XML 樹狀結構。 然後，它會逐一查看節點，並列印每個節點的 XPath 運算式。
...
以下是 XML 樹狀結構中的節點清單，如 XPath 運算式所定義：
