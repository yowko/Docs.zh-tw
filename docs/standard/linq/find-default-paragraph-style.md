...
---
# <a name="find-the-default-paragraph-style-linq-to-xml"></a>尋找預設段落樣式 (LINQ to XML) 

教學課程中的第一個工作 [：管理 WordprocessingML 檔中的內容](xml-shape-wordprocessingml-documents.md) ，是尋找檔中的預設段落樣式。

## <a name="example-find-the-default-style-name"></a>範例：尋找預設樣式名稱

下列範例會開啟 Office Open XML WordprocessingML 文件、尋找文件和封裝的樣式部分，然後執行尋找預設樣式名稱的查詢。 如需 Office Open XML 檔封裝及其組成元件的詳細資訊，請參閱 [Office OPEN Xml WordprocessingML 檔的詳細資料](wordprocessingml-document-styles.md)。
...
在本教學課程的下一篇文章中，您將建立類似的查詢，以尋找檔中的所有段落及其樣式：

- [取出段落及其樣式](retrieve-paragraphs-styles.md)
