...
---
# <a name="how-to-write-a-linq-to-xml-axis-method-linq-to-xml"></a>如何 (LINQ to XML 撰寫 LINQ to XML 的座標軸方法) 

XML [軸方法](linq-xml-axes-overview.md) 會從 xml 檔或上階元素抓取 xml 元素的集合。 您可以撰寫自己的座標軸方法來擷取 XML 樹狀結構中的集合。 其中一個最好的方式是，撰寫可傳回項目或屬性集合的擴充方法。 您可以根據您應用程式的需求，撰寫您的擴充方法來傳回項目或屬性的特定子集。 本文提供 c # 和 Visual Basic 的範例。
