...
description: 這個 XML 檔案（用於範例）包含客戶和訂單的相關資料。
ms.date: 07/20/2015
...
ms.locfileid: "89552677"
---
# <a name="sample-xml-file-customers-and-orders-linq-to-xml"></a><span data-ttu-id="ed436-103">範例 XML 檔：客戶和訂單 (LINQ to XML) </span><span class="sxs-lookup"><span data-stu-id="ed436-103">Sample XML file: Customers and orders (LINQ to XML)</span></span>

<span data-ttu-id="ed436-104">下列 XML 檔案用於 LINQ to XML 檔的各種範例中。</span><span class="sxs-lookup"><span data-stu-id="ed436-104">The following XML file is used in various examples in the LINQ to XML documentation.</span></span> <span data-ttu-id="ed436-105">這個檔案包含客戶和訂單。</span><span class="sxs-lookup"><span data-stu-id="ed436-105">This file contains customers and orders.</span></span>

<span data-ttu-id="ed436-106">[檔範例 xsd 檔案：客戶和訂單](sample-xsd-file-customers-orders.md)包含可用於驗證此檔的 xsd。</span><span class="sxs-lookup"><span data-stu-id="ed436-106">The article [Sample XSD file: Customers and orders](sample-xsd-file-customers-orders.md) contains an XSD that can be used to validate this document.</span></span> <span data-ttu-id="ed436-107">它會使用 XSD 的 `xs:key` 和 `xs:keyref` 功能來建立 `CustomerID` 項目的 `Customer` 屬性為索引鍵，並在每個 `CustomerID` 項目中的 `Order` 項目和每個 `CustomerID` 項目中的 `Customer` 屬性之間建立關聯性。</span><span class="sxs-lookup"><span data-stu-id="ed436-107">It uses the `xs:key` and `xs:keyref` features of XSD to establish that the `CustomerID` attribute of the `Customer` element is a key, and to establish a relationship between the `CustomerID` element in each `Order` element and the `CustomerID` attribute in each `Customer` element.</span></span>

<span data-ttu-id="ed436-108">如需使用子句來撰寫利用此關聯性之 LINQ 查詢的範例 `Join` ，請參閱 [如何聯結兩個集合](join-two-collections.md)。</span><span class="sxs-lookup"><span data-stu-id="ed436-108">For an example of writing LINQ queries that take advantage of this relationship using the `Join` clause, see [How to join two collections](join-two-collections.md).</span></span>

## <a name="customersordersxml"></a><span data-ttu-id="ed436-109">CustomersOrders.xml</span><span class="sxs-lookup"><span data-stu-id="ed436-109">CustomersOrders.xml</span></span>
