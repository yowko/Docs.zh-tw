...
下列 XML 檔案用於 LINQ to XML 檔的各種範例中。 這個檔案包含客戶和訂單。

[檔範例 xsd 檔案：客戶和訂單](sample-xsd-file-customers-orders.md)包含可用於驗證此檔的 xsd。 它會使用 XSD 的 `xs:key` 和 `xs:keyref` 功能來建立 `CustomerID` 項目的 `Customer` 屬性為索引鍵，並在每個 `CustomerID` 項目中的 `Order` 項目和每個 `CustomerID` 項目中的 `Customer` 屬性之間建立關聯性。
