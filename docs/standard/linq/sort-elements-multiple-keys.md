---
title: 如何排序多個索引鍵上的元素-LINQ to XML
description: 瞭解如何排序多個索引鍵上的 XML 元素。
ms.date: 07/20/2015
dev_langs:
- csharp
- vb
...
---
# <a name="how-to-sort-elements-on-multiple-keys-linq-to-xml"></a>如何排序多個索引鍵上的元素 (LINQ to XML) 

本文說明如何在 c # 和 Visual Basic 中排序多個索引鍵。

## <a name="example-sort-xml-elements-on-multiple-keys"></a>範例：排序多個索引鍵上的 XML 元素

此範例會根據兩個索引鍵來排序：寄送郵遞區號 (主要) 和訂購日期。 它會使用 XML [檔範例 xml 檔：客戶和訂單](sample-xml-file-customers-orders.md)。
...
## <a name="example-sort-xml-thats-in-a-namespace"></a>範例：排序命名空間中的 XML

這個範例會執行與第一個相同的排序，但是針對命名空間中的 XML 執行。 它會使用 XML [檔範例 xml 檔：命名空間中的客戶和訂單](sample-xml-file-customers-orders-namespace.md)。
