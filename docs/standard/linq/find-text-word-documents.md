...
---
# <a name="find-text-in-word-documents-linq-to-xml"></a>尋找 Word 檔中的文字 (LINQ to XML) 

本文將擴充先前的查詢，以在 WordProcessingML 檔中尋找所有出現的字串。

## <a name="example-find-all-occurrences-of-a-string-in-a-wordprocessingml-document"></a>範例：在 WordProcessingML 檔中尋找所有出現的字串

此範例會在字串 "Hello" )  (的特定文字片段的 WordprocessingML 檔中尋找所有出現的專案。 此範例是以本教學課程中先前的範例為基礎。 程式碼中的批註會指出新增的查詢。
...
## <a name="example-find-all-lines-in-a-wordprocessingml-document-that-have-a-specific-style"></a>範例：尋找 WordProcessingML 檔中具有特定樣式的所有行

在此範例中，查詢會經過修改，以搜尋特定樣式的行。 它會尋找具有樣式的所有空白行 `Code` 。
...
您可以透過數種方式來增強這個範例。 例如，我們可以使用規則運算式 (Regular Expression) 來搜尋文字；我們可以在特定的字典中逐一查看所有 Word 檔案等等。

這個範例幾乎會執行，而且它是以單一查詢的形式來撰寫。 因為每個查詢都是以延遲的延遲方式來執行，所以在反復查詢查詢之前不會產生其結果。 如需詳細資訊，請參閱 [延後執行和延遲評估](deferred-execution-lazy-evaluation.md)。

下一節會提供 WordprocessingML 檔的詳細資料：

- [Office Open XML WordprocessingML 檔的詳細資料](wordprocessingml-document-styles.md)
