...
撰寫 I/O-bound 和 CPU-bound 非同步程式碼的直接做法，就是使用以 .NET 工作為基礎的非同步模型。 在 C# 和 Visual Basic 中，此模型是由 `Task` 和 `Task<T>` 類型以及 `async` 和 `await` 關鍵字所公開。 （有關特定于語言的資源，請參閱["另請參閱"](#see-also)部分。本文介紹如何使用 .NET 非同步，並提供對封面下使用非同步框架的見解。
