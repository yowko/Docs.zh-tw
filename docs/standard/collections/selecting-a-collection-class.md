...
ms.topic: "article"
helpviewer_keywords: 
  - "集合 [.NET Framework], selecting collection 類別"
  - "集合類別"
  - "先進先出集合"
  - "群組集合中的資料, selecting collection 類別"
  - "具索引的集合"
  - "後進先出集合"
...
## LINQ to Objects 和 PLINQ  
 只要物件類型實作 <xref:System.Collections.IEnumerable> 或 <xref:System.Collections.Generic.IEnumerable%601>，LINQ to Objects 就可讓開發人員使用 LINQ 查詢以存取記憶體內的物件。  LINQ 查詢提供一般模式以存取資料，比標準的 `foreach` 迴圈更精簡、可讀性更高，並提供篩選、排序和群組功能。  如需詳細資訊，請參閱 [LINQ to Objects](../../../ocs/visual-basic/programming-guide/concepts/linq/linq-to-objects.md)。  
