 ...
 **✓**會針對事件使用「引發」一詞，而非「引發」或「觸發程式」。  
  
 **✓**會使用 <xref:System.EventHandler%601?displayProperty=nameWithType>，而不是手動建立新的委派，以當做事件處理常式使用。  
  
 **✓請考慮**使用 <xref:System.EventArgs> 的子類別做為事件引數，除非您確實確定事件永遠不需要將任何資料傳送至事件處理方法，在此情況下，您可以直接使用 `EventArgs` 類型。  
  
 如果您使用 `EventArgs` 直接寄送 API，您永遠都無法在不中斷相容性的情況下，新增任何要攜帶事件的資料。 如果您使用子類別，即使一開始完全空白，您也可以在需要時將屬性新增至子類別。  
  
 **✓**會使用受保護的虛擬方法來引發每個事件。 這只適用于未密封類別上的非靜態事件，而不適用於結構、密封類別或靜態事件。  
 ...
 **✓**會對引發事件的受保護方法採取一個參數。  
 ...
 當引發非靜態事件時， **X 不會**傳遞 null 做為傳送者。  
  
 當引發靜態事件時， **✓ DO**傳遞 null 做為寄件者。  
  
 當引發事件時， **X 不會**傳遞 null 做為事件資料參數。  
  
 如果您不想要將任何資料傳遞至事件處理方法，則應該傳遞 `EventArgs.Empty`。 開發人員預期此參數不是 null。  
  
 **✓請考慮**引發終端使用者可以取消的事件。 這只適用于前置事件。  
 ...
 **✓ DO**會針對事件處理常式使用 void 的傳回型別。  
  
 事件處理常式可以叫用多個事件處理方法，可能會在多個物件上叫用。 如果允許事件處理方法傳回值，每個事件調用都會有多個傳回值。  
  
 **✓**會使用 `object` 做為事件處理常式第一個參數的型別，並 `sender`呼叫它。  
  
 **✓**會使用 <xref:System.EventArgs?displayProperty=nameWithType> 或其子類別做為事件處理常式之第二個參數的型別，並 `e`呼叫它。  
  
 **X**在事件處理常式上不能有兩個以上的參數。  
