 ...
 **X 避免**frivolously 定義擴充方法，特別是在您不擁有的類型上。  
 ...
 **X 避免**在 <xref:System.Object?displayProperty=nameWithType>上定義擴充方法。  
 ...
 **X**不會將擴充方法放在與延伸類型相同的命名空間中，除非它是用來將方法加入介面或用於相依性管理。  
  
 **X 避免**定義兩個或多個具有相同簽章的擴充方法，即使它們位於不同的命名空間也一樣。  
  
 如果類型為介面，而且如果擴充方法要用於大部分或所有案例， **✓請考慮**在與延伸類型相同的命名空間中定義擴充方法。  
  
 **X 不會**定義在命名空間中執行功能的擴充方法，通常與其他功能相關聯。 相反地，請在與它們所屬的功能相關聯的命名空間中定義它們。  
  
 **X 避免**針對擴充方法（例如「延伸模組」）專用的命名空間進行一般命名。 請改用描述性名稱（例如「路由」）。  
