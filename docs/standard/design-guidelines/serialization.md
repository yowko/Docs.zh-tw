...
## <a name="choosing-the-right-serialization-technology-to-support"></a>選擇要支援的正確序列化技術  
 如果您的型別實例可能需要保存或用於 Web 服務， **✓請考慮**支援資料合約序列化。  
  
 如果您需要更充分掌控在序列化類型時所產生的 XML 格式， **✓可考慮**支援 xml 序列化，而不是資料合約序列化。  
  
 當您需要使用資料合約序列化不支援的 XML 結構，例如產生 XML 屬性時，在某些互通性案例中可能會需要此功能。  
  
 如果您的型別實例需要跨 .NET 遠端處理界限進行移動， **✓請考慮**支援執行時間序列化。  
  
 **X**基於一般持續性原因，請避免支援執行時間序列化或 XML 序列化。 建議改為使用資料合約序列化。  
...
## <a name="supporting-data-contract-serialization"></a>支援資料合約序列化  
 ...
 如果類型可以在部分信任中使用， **✓可考慮**將類型的資料成員標示為公用。  
  
 在完全信任的情況下，資料合約序列化程式可以將非公用類型和成員序列化和還原序列化，但只有公用成員可以在部分信任中序列化和還原序列化。  
  
 **✓**會在所有具有 <xref:System.Runtime.Serialization.DataMemberAttribute>的屬性上執行 getter 和 setter。 資料合約序列化程式需要 getter 和 setter，才能將類型視為可序列化。 （在 .NET Framework 3.5 SP1 中，某些集合屬性可以是僅限取得）。如果類型不會在部分信任中使用，則其中一個或兩個屬性存取子可以是非公用的。  
  
 **✓請考慮**使用序列化回呼來初始化已還原序列化的實例。  
 ...
 **✓請考慮**使用 <xref:System.Runtime.Serialization.KnownTypeAttribute> 來指示還原序列化複雜物件圖形時，應該使用的具體類型。  
  
 建立或變更可序列化型別時， **✓**會考慮回溯和回溯相容性。  
 ...
 **✓請考慮**執行 <xref:System.Runtime.Serialization.IExtensibleDataObject>，以允許在類型的不同版本之間進行往返。  
...
## <a name="supporting-runtime-serialization"></a>支援執行階段序列化  
 ...
 如果您的型別將搭配 .NET 遠端處理使用， **✓會考慮**支援執行時間序列化。 例如，<xref:System.AddIn?displayProperty=nameWithType> 命名空間會使用 .NET 遠端處理，因此 `System.AddIn` 增益集之間交換的所有類型都必須支援執行時間序列化。  
  
 如果您想要完整控制序列化程式， **✓請考慮**執行執行時間 Serializable 模式。 例如，如果您想要在資料序列化或還原序列化時加以轉換。  
  
 此模式非常簡單。 您只需要實作 <xref:System.Runtime.Serialization.ISerializable> 介面，並提供還原序列化物件時使用的特殊建構函式。  
  
 **✓確實**會保護序列化的函式，並提供兩個輸入和命名的參數，如這裡的範例所示。  
 ...
 **✓**會明確地執行 <xref:System.Runtime.Serialization.ISerializable> 成員。  
  
 **✓確實**會將連結需求套用至 <xref:System.Runtime.Serialization.ISerializable.GetObjectData%2A?displayProperty=nameWithType> 的執行。 這可確保只有完全信任的核心和執行時間序列化程式可以存取該成員。  
