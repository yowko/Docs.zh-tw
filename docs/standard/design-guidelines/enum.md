 ...
 **✓ 不要**列舉使用強型別參數、 屬性和傳回值，表示一組值。  
  
 **✓ 不要**偏好使用列舉，而不靜態的常數。  
  
 **X 不**列舉用於開啟設定 （例如作業系統版本、 您的朋友、 等等的名稱。）。  
 ...
 **✓ 考慮**使用<xref:System.Int32>（大部分的程式設計語言中的預設值） 列舉的基礎類型為除非有下列情況：  
 ...
 **✓ 不要**命名為旗標列舉的複數名詞或名詞片語，並使用單數名詞或名詞片語的簡單列舉。  
 ...
 **✓ 不要**套用<xref:System.FlagsAttribute?displayProperty=nameWithType>用於旗標列舉。 並不適用此屬性至簡單列舉。  
  
 **✓ 不要**使用的 2 乘冪的旗標列舉值，因此也可以自由地結合使用位元 OR 運算。  
