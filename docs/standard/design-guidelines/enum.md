...
**✓確實**會使用列舉來代表值集合的強型別參數、屬性和傳回值。

**✓**會偏好使用列舉，而不是靜態常數。

**X 不會**針對開啟的集合（例如作業系統版本、您的朋友名稱等）使用列舉。
...
**✓請考慮**使用 <xref:System.Int32> （大部分程式設計語言中的預設值）做為列舉的基礎類型，除非下列任何一項為 true：
...
具有複數名詞或名詞片語的**✓ DO** name 旗標列舉，以及具有單數名詞或名詞片語的簡單列舉。

**X 不會**直接擴充 <xref:System.Enum?displayProperty=nameWithType>。
...
**✓ DO**會將 <xref:System.FlagsAttribute?displayProperty=nameWithType> 套用至旗標列舉。 請勿將此屬性套用至簡單列舉。

**✓**會針對旗標列舉值使用二的乘冪，讓它們可以使用位 or 運算自由地結合。

**✓請考慮**為常用的旗標組合提供特殊的列舉值。

位運算是一種先進的概念，而且不應該為簡單的工作所需。 <xref:System.IO.FileAccess.ReadWrite> 是這種特殊值的範例。

**X 避免**建立旗標列舉，其中某些值的組合無效。

**X 避免**使用旗標列舉值零，除非此值表示「已清除所有旗標」並適當地命名，如同下一個指導方針所規定。

**✓ DO** name：旗標列舉 `None`的零值。 針對旗標列舉，此值必須一律表示「已清除所有旗標」。
