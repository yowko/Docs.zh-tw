...
> [!NOTE]
> 處理常式必須將元件載入至載入來源內容、載入內容或沒有內容。 如果處理常式使用或方法，將元件載入至僅限反映的內容 <xref:System.Reflection.Assembly.ReflectionOnlyLoad%2A?displayProperty=nameWithType> <xref:System.Reflection.Assembly.ReflectionOnlyLoadFrom%2A?displayProperty=nameWithType> ，則引發事件的載入嘗試 <xref:System.AppDomain.AssemblyResolve> 會失敗。
...
> [!IMPORTANT]
> 從 .NET Framework 4 開始，會針對附屬組件引發 <xref:System.AppDomain.AssemblyResolve> 事件。 如果處理常式嘗試解析所有組件載入要求，則這項變更會影響針對舊版 .NET Framework 所撰寫的事件處理常式。 忽略無法辨識之元件的事件處理常式不會受到這項變更的影響：它們會傳回 `null` ，而且會遵循一般的恢復機制。
...
### <a name="the-correct-way-to-handle-assemblyresolve"></a>處理 System.appdomain.assemblyresolve 的正確方式

從 <xref:System.AppDomain.AssemblyResolve> 事件處理常式解析元件時， <xref:System.StackOverflowException> 如果處理常式使用 <xref:System.Reflection.Assembly.Load%2A?displayProperty=nameWithType> 或方法呼叫，最終將會擲回 <xref:System.AppDomain.Load%2A?displayProperty=nameWithType> 。 請改用 <xref:System.Reflection.Assembly.LoadFile%2A> 或 <xref:System.Reflection.Assembly.LoadFrom%2A> 方法，因為它們不會引發 `AssemblyResolve` 事件。

想像一下 `MyAssembly.dll` 位於執行中元件附近的已知位置中，可以使用 `Assembly.LoadFile` 指定的元件路徑來解決此問題。
