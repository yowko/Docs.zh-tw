...
如需支援的`func`參數和傳回類型的引數清單，請參閱[資料類型](types.md)。

指定`state`引數會將該值傳遞至函式的每個調用。 使用此來避免結束。

指定`isDeterministic`您的函數是否具決定性，以允許 SQLite 在編譯查詢時使用額外的優化。
...
彙總函式會針對查詢中的所有資料列傳回單一匯總的值。 使用來<xref:Microsoft.Data.Sqlite.SqliteConnection.CreateAggregate%2A>定義和覆寫彙總函式。

`seed`引數會指定內容的初始狀態。 請使用此來避免同時結束。

`func`引數會針對每個資料列叫用一次。 使用內容來累積最終的結果。 傳回內容。 此模式可讓內容是實值型別或不可變的。

如果未`resultSelector`指定，則會使用內容的最終狀態做為結果。 這可簡化函式的定義，例如 sum 和 count，這只需要每個資料列遞增一個數位，然後傳回它。

指定`resultSelector`在逐一查看所有資料列之後，計算內容的最終結果。

如需`func`引數和傳回類型的支援參數類型清單，請參閱[資料類型。](types.md) `resultSelector`

如果您的函式具決定性`isDeterministic` ，請指定，以允許 SQLite 在編譯查詢時使用額外的優化。
...
根據預設，錯誤 SQLite 錯誤碼將會 SQLITE_ERROR （或1）。 不過，您可以使用所需<xref:Microsoft.Data.Sqlite.SqliteException> <xref:Microsoft.Data.Sqlite.SqliteException.SqliteErrorCode>的指定，在函式中擲回來變更它。
