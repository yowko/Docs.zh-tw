---
title: "使用 XPathNavigator 插入 XML 資料 | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
ms.prod: ".net"
ms.reviewer: ""
...
<xref:System.Xml.XPath.XPathNavigator> 類別提供一組方式，可在 XML 文件中插入同層級節點、子節點及屬性節點。  為了使用這些方法，<xref:System.Xml.XPath.XPathNavigator> 物件必須是可編輯的，也就是說，它的 <xref:System.Xml.XPath.XPathNavigator.CanEdit%2A> 屬性必須為 `true`。  
 ...
 可編輯 XML 文件的 <xref:System.Xml.XPath.XPathNavigator> 物件是由 <xref:System.Xml.XmlDocument> 類別的 <xref:System.Xml.XmlDocument.CreateNavigator%2A> 方法所建立。  由 <xref:System.Xml.XPath.XPathDocument> 類別建立的 <xref:System.Xml.XPath.XPathNavigator> 物件是唯讀的，而且如果嘗試使用由 <xref:System.Xml.XPath.XPathDocument> 物件建立之 <xref:System.Xml.XPath.XPathNavigator> 物件的編輯方法，則會導致 <xref:System.NotSupportedException>。  
  
 如需建立可編輯 <xref:System.Xml.XPath.XPathNavigator> 物件的詳細資訊，請參閱 [使用 XPathDocument 及 XmlDocument 讀取 XML 資料](../../../../docs/standard/data/xml/reading-xml-data-using-xpathdocument-and-xmldocument.md)。  
...
-   <xref:System.Xml.XPath.XPathNavigator.CreateAttributes%2A>  
 ...
 在下列範例中，會使用從 <xref:System.Xml.XPath.XPathNavigator.CreateAttributes%2A> 方法傳回的 <xref:System.Xml.XmlWriter> 物件，在 `contosoBooks.xml` 檔案中第一個 `book` 項目的 `price` 項目子系上，建立新的 `discount` 及 `currency` 屬性。  
...
## 複製節點  
 在某些情況下，您可能要使用其他 XML 文件的內容，填入 XML 文件。  <xref:System.Xml.XPath.XPathNavigator> 類別及 <xref:System.Xml.XmlWriter> 類別可以從現有的 <xref:System.Xml.XmlReader> 物件或 <xref:System.Xml.XPath.XPathNavigator> 物件，將節點複製到 <xref:System.Xml.XmlDocument> 物件。  
  
 <xref:System.Xml.XPath.XPathNavigator> 類別的 <xref:System.Xml.XPath.XPathNavigator.AppendChild%2A>、<xref:System.Xml.XPath.XPathNavigator.PrependChild%2A>、<xref:System.Xml.XPath.XPathNavigator.InsertBefore%2A> 及 <xref:System.Xml.XPath.XPathNavigator.InsertAfter%2A> 方法都包含多載，它們可以接受 <xref:System.Xml.XPath.XPathNavigator> 物件或 <xref:System.Xml.XmlReader> 物件做為參數。  
  
 <xref:System.Xml.XmlWriter> 類別的 <xref:System.Xml.XmlWriter.WriteNode%2A> 方法包含可接受 <xref:System.Xml.XmlNode>、<xref:System.Xml.XmlReader> 或 <xref:System.Xml.XPath.XPathNavigator> 物件的多載。  
...
### 插入不具型別值  
 ...
 在下列範例中，<xref:System.Xml.XPath.XPathNavigator.SetValue%2A> 方法用於更新 `contosoBooks.xml` 檔案中的所有 `price` 項目。  
...
### 插入具型別值  
 當節點的型別為 W3C XML 結構描述簡單型別時，會根據簡單型別的 Facet，對 <xref:System.Xml.XPath.XPathNavigator.SetTypedValue%2A> 方法插入的新值執行設定前的檢查。  如果根據節點的型別，新值無效 \(例如，在型別為 `xs:positiveInteger` 的項目上設定 `-1` 值\)，則會導致例外狀況。  
  
 下列範例會嘗試將 `contosoBooks.xml` 檔案中第一個 `book` 項目之 `price` 項目的值，變更為 <xref:System.DateTime> 值。  因為在 `contosoBooks.xsd` 檔案中將 `price` 項目的 XML 結構描述型別定義為 `xs:decimal`，所以這個動作會擲回例外狀況。  
...
## InnerXml 及 OuterXml 屬性  
 <xref:System.Xml.XPath.XPathNavigator> 類別的 <xref:System.Xml.XPath.XPathNavigator.InnerXml%2A> 及 <xref:System.Xml.XPath.XPathNavigator.OuterXml%2A> 屬性會變更 <xref:System.Xml.XPath.XPathNavigator> 物件目前所在之節點的 XML 標記。  
 ...
 除了本主題中所說明的方法之外，還可以使用 <xref:System.Xml.XPath.XPathNavigator.InnerXml%2A> 及 <xref:System.Xml.XPath.XPathNavigator.OuterXml%2A> 屬性，將節點及值插入 XML 文件。  如需使用 <xref:System.Xml.XPath.XPathNavigator.InnerXml%2A> 及 <xref:System.Xml.XPath.XPathNavigator.OuterXml%2A> 屬性插入節點及值的詳細資訊，請參閱 [使用 XPathNavigator 修改 XML 資料](../../../../docs/standard/data/xml/modify-xml-data-using-xpathnavigator.md) 主題。  
  
## 命名空間及 xml:lang 衝突  
 當使用 <xref:System.Xml.XPath.XPathNavigator> 類別的 <xref:System.Xml.XPath.XPathNavigator.InsertBefore%2A>、<xref:System.Xml.XPath.XPathNavigator.InsertAfter%2A>、<xref:System.Xml.XPath.XPathNavigator.AppendChild%2A> 及 <xref:System.Xml.XPath.XPathNavigator.PrependChild%2A> 方法 \(其採用 <xref:System.Xml.XmlReader> 物件做為參數\) 來插入 XML 資料時，可能會發生某些與命名空間的範圍及 `xml:lang` 宣告相關的衝突。  
...
-   如果在 <xref:System.Xml.XmlReader> 物件的內容中包含範圍中的 `xml:lang` 屬性，但 <xref:System.Xml.XPath.XPathNavigator> 物件的內容中沒有，則會將從 <xref:System.Xml.XmlReader> 物件取得值的 `xml:lang` 屬性加入至新插入的節點。  
  
-   如果 <xref:System.Xml.XmlReader> 物件的內容及 <xref:System.Xml.XPath.XPathNavigator> 物件的內容都包含範圍中的 `xml:lang` 屬性，但是屬性的值不同，則會將從 <xref:System.Xml.XmlReader> 物件取得值的 `xml:lang` 屬性加入至新插入的節點。  
  
-   如果 <xref:System.Xml.XmlReader> 物件的內容及 <xref:System.Xml.XPath.XPathNavigator> 物件的內容都包含範圍中的 `xml:lang` 屬性，但是屬性的值相同，則不會將新的 `xml:lang` 屬性加入新插入的節點。  
  
-   如果 <xref:System.Xml.XPath.XPathNavigator> 物件的內容包含範圍中的 `xml:lang` 屬性，但是在 <xref:System.Xml.XmlReader> 物件的內容中沒有，則不會將 `xml:lang` 屬性加入至新插入的節點。  
  
## 使用 XmlWriter 插入節點  
 多載＜插入節點及值＞一節中說明之用於插入同層級節點、子節點及屬性節點的方法。  <xref:System.Xml.XPath.XPathNavigator> 類別的 <xref:System.Xml.XPath.XPathNavigator.InsertAfter%2A>、<xref:System.Xml.XPath.XPathNavigator.InsertBefore%2A>、<xref:System.Xml.XPath.XPathNavigator.AppendChild%2A>、<xref:System.Xml.XPath.XPathNavigator.PrependChild%2A> 及 <xref:System.Xml.XPath.XPathNavigator.CreateAttributes%2A> 方法會傳回用於插入節點的 <xref:System.Xml.XmlWriter> 物件。  
  
### 不支援的 XmlWriter 方法  
 由於 XPath 資料模型與文件物件模型 \(DOM\) 之間的差異，<xref:System.Xml.XPath.XPathNavigator> 類別並不支援所有使用 <xref:System.Xml.XmlWriter> 類別將資訊寫入 XML 文件的方法。  
  
 下表說明 <xref:System.Xml.XPath.XPathNavigator> 類別不支援的 <xref:System.Xml.XmlWriter> 類別方法。  
...
-   當呼叫每個 <xref:System.Xml.XmlWriter> 物件的 <xref:System.Xml.XmlWriter.Close%2A> 方法時，會將由 <xref:System.Xml.XmlWriter> 物件寫入的 XML 片段加入至 XML 文件。  直到那時，<xref:System.Xml.XmlWriter> 物件會寫入中斷連接的片段。  如果在 XML 文件上執行作業，則不會影響在呼叫 <xref:System.Xml.XmlWriter.Close%2A> 之前，由 <xref:System.Xml.XmlWriter> 物件寫入的任何片段。  
...
-   如果在 XML 文件中同時開啟多個 <xref:System.Xml.XmlWriter> 物件，則會依照關閉 <xref:System.Xml.XmlWriter> 物件的順序 \(而非開啟它們的順序\)，將它們加入至 XML 文件。  
 ...
 下列範例會建立 <xref:System.Xml.XmlDocument> 物件及 <xref:System.Xml.XPath.XPathNavigator> 物件，然後使用由 <xref:System.Xml.XPath.XPathNavigator.PrependChild%2A> 方法傳回的 <xref:System.Xml.XmlWriter> 物件，在 `books.xml` 檔案建立第一本書的結構。  之後，範例會將其儲存為 `book.xml` 檔案。  
