...
```  
 ...
 如需安全性層級和辨識項，以及它如何影響指令碼的詳細資訊，請參閱[XSLT 樣式表指令碼使用\<msxsl: script >](../../../../docs/standard/data/xml/xslt-stylesheet-scripting-using-msxsl-script.md)。 安全性層級和辨識項和它如何影響擴充物件的相關資訊，請參閱[樣式表參數和擴充物件的 XsltArgumentList](../../../../docs/standard/data/xml/xsltargumentlist-for-style-sheet-parameters-and-extension-objects.md)。  
  
 如需安全性層級和辨識項，以及它如何影響`document()`函式，請參閱[解析外部的 XSLT 樣式表和文件](../../../../docs/standard/data/xml/resolving-external-xslt-style-sheets-and-documents.md)。  
...
## <a name="transforming-a-section-of-an-xml-document"></a>轉換 XML 文件的段落  
 轉換是指套用到整個文件。 換言之，如果您要傳入的節點不是文件的根節點，則不會阻止轉換程序取得已載入文件中的所有節點。 若要轉換結果樹狀結構片段，則必須建立僅包含結果樹狀結構片段的 <xref:System.Xml.XmlDocument>，並將 <xref:System.Xml.XmlDocument> 傳遞至 <xref:System.Xml.Xsl.XslTransform.Transform%2A> 方法中。 以下範例將執行結果樹狀結構片段的轉換。  
...
|.NET Framework 1.0 版過時負載方法|取代.NET Framework 1.1 版的載入方法|  
|------------------------------------------------------|---------------------------------------------------------|  
...
|.NET Framework 1.0 版過時轉換方法|.NET Framework 1.1 版替代轉換方法|  
|-----------------------------------------------------------|--------------------------------------------------------------|  
...
|Void Transform(String input, String output);|Void Transform(String input, String output, XmlResolver resolver);|  
 ...
 <xref:System.Xml.Xsl.XslTransform.XmlResolver%2A?displayProperty=nameWithType> 屬性在 [!INCLUDE[dnprdnshort](../../../../includes/dnprdnshort-md.md)] 1.1 版中已過時。 相反地，使用 新<xref:System.Xml.Xsl.XslTransform.Transform%2A>多載，它採用<xref:System.Xml.XmlResolver>物件。  
