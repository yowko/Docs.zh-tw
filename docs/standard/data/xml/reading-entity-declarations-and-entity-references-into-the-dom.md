...
```  
 ...
 Microsoft.NET Framework 實作 XML 文件物件模型 (DOM) 的預設值是保留實體參考，並且在 XML 載入時不會擴充實體。 這個併入會在載入 DOM 中的文件**XmlEntityReference**節點包含參考變數`&publisher;`建立，代表內容實體中的子節點，DTD 中宣告。  
  
 使用`<!ENTITY publisher "Microsoft Press">`實體宣告時下, 圖顯示**XmlEntity**和**XmlText**從這個宣告建立的節點。  
  
 ![從實體宣告建立的節點](../../../../docs/standard/data/xml/media/xml-entitydeclaration-node2.png "xml_entitydeclaration_node2")  
  
 實體參考是否會擴充的差異在於記憶體中 DOM 樹狀結構中產生了哪些節點。 主題中會說明所產生之節點的差異[保留實體參考](../../../../docs/standard/data/xml/entity-references-are-preserved.md)和[實體參考已擴充且沒有保留](../../../../docs/standard/data/xml/entity-references-are-expanded-and-not-preserved.md)。  
