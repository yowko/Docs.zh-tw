...
|--------------------|-------------|-----------------------------------|  
|文字節點同時符合 `xsl:strip-space` 及 `xsl:preserve-space`。|3.4|Recover|  
|來源節點符合一個以上的範本 (Template) 規則。|5.5|Recover|  
|命名空間 URI 會宣告成多個命名空間 URI 的別名，且全都具有相同的匯入優先順序。|7.1.1|Recover|  
|從屬性值產生之 `name` 及 `xsl:attribute` 中的 `xsl:element` 屬性不是 QName。|7.1.2, 7.1.3|錯誤*|  
|具有相同匯入及展開名稱的兩個屬性集具有通用的屬性，而且其他任何屬性集都不會包含具有相同名稱、更高重要性的通用屬性。|7.1.4|Recover|  
...
|具現化 `xsl:processing-instruction` 內容的結果包含字串「?>」|7.3|Recover|  
|具現化 `xsl:processing-instruction` 內容的結果包含字串 --，或以 - 為結尾。|7.4|Recover|  
...
|有多個具有相同名稱、不同值的屬性，它們不是具有相同匯入優先順序之 `xsl:output` 中的具名 cdata-section 項目。|16|Recover|  
|處理器不支援 `xsl:output` 編碼屬性中的編碼。|16.1|Recover|  
...
|`value` 的 `xsl:number` 屬性為 NAN、無限或小於 0.5|勘誤表 24|Recover|  
|文件函式的第二個引數節點集是空的，且 URI 參考是相對的。|勘誤表 14|Recover|  
