...
|DOM 節點型別|Object|描述|  
|-------------------|------------|-----------------|  
|Document|<xref:System.Xml.XmlDocument>|樹狀中所有節點的容器。 它也稱為文件根，不一定總是與根項目相同。|  
...
|單位|<xref:System.Xml.XmlEntity>|表示 XML 文件中的 `<!ENTITY…>` 宣告，可能來自內部文件類型定義 (DTD) 子集，或來自外部 DTD 和參數實體。|  
