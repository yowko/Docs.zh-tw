---
title: "使用 XPathNavigator 修改 XML 資料 | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
ms.prod: ".net"
ms.reviewer: ""
...
## 修改節點  
 一種變更節點值的簡單技術是使用 <xref:System.Xml.XPath.XPathNavigator> 類別的 <xref:System.Xml.XPath.XPathNavigator.SetValue%2A> 及 <xref:System.Xml.XPath.XPathNavigator.SetTypedValue%2A> 方法。  
  
 下表列出了這些方法對不同節點型別的影響。  
  
|<xref:System.Xml.XPath.XPathNodeType>|資料變更|  
|---------------------------------------------------------------------------------------------------------------------------------------------|----------|  
|<xref:System.Xml.XPath.XPathNodeType>|不支援。|  
|<xref:System.Xml.XPath.XPathNodeType>|項目的內容。|  
|<xref:System.Xml.XPath.XPathNodeType>|屬性的值。|  
|<xref:System.Xml.XPath.XPathNodeType>|文字內容。|  
|<xref:System.Xml.XPath.XPathNodeType>|內容 \(目標除外\)。|  
|<xref:System.Xml.XPath.XPathNodeType>|註解的內容。|  
|<xref:System.Xml.XPath.XPathNodeType>|不支援。|  
  
> [!NOTE]
>  不支援編輯 <xref:System.Xml.XPath.XPathNodeType> 節點或 <xref:System.Xml.XPath.XPathNodeType> 節點。  
  
 <xref:System.Xml.XPath.XPathNavigator> 類別還提供一組可用於插入及移除節點的方法。  如需在 XML 文件中插入及移除節點的詳細資訊，請參閱 [使用 XPathNavigator 插入 XML 資料](../../../../docs/standard/data/xml/insert-xml-data-using-xpathnavigator.md) 及 [使用 XPathNavigator 移除 XML 資料](../../../../docs/standard/data/xml/remove-xml-data-using-xpathnavigator.md) 主題。  
...
>  屬性僅可具有簡單型別。  
 ...
 如果項目的有效性取決於其項目子系及屬性的有效性，則修改任何一項都會導致變更項目的有效性 \(如果先前已為有效\)。  特別是當插入、更新或刪除項目的項目子系或屬性時，項目的有效性會變為不明。  此由項目 <xref:System.Xml.XPath.XPathNavigator.SchemaInfo%2A> 屬性的 <xref:System.Xml.Schema.IXmlSchemaInfo.Validity%2A> 屬性設為 <xref:System.Xml.Schema.XmlSchemaValidity> 來表示。  而且，此影響還會按階層向上遞迴處理整個 XML 文件，因為項目的父項目 \(及其父項目等\) 有效性也會變為不明。  
...
## InnerXml 及 OuterXml 屬性  
 ...
 下列範例使用 <xref:System.Xml.XPath.XPathNavigator.OuterXml%2A> 屬性來修改 `price` 項目的值，並在 `contosoBooks.xml` 檔案中的第一個 `book` 項目上插入新的 `discount` 屬性。  
...
```  
 ...
 在上述 XML 範例中，屬性 `a:parent-id` 是插入在 `http://www.contoso.com/parent-id` 命名空間中的 `parent` 項目上。  <xref:System.Xml.XPath.XPathNavigator.CreateAttribute%2A> 方法用於在定位於 `parent` 項目上時插入屬性。  <xref:System.Xml.XPath.XPathNavigator> 類別會自動插入 `http://www.contoso.com` 命名空間宣告，以保持 XML 文件其餘內容的一致性。  
