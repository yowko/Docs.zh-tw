---
title: "XSLT 安全性考量 | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
ms.prod: ".net"
ms.reviewer: ""
...
### 樣式表指令碼  
 指令碼可透過 `msxsl:script` 擴充項目內嵌在樣式表中。  指令碼支援是 <xref:System.Xml.Xsl.XslCompiledTransform> 類別上的選擇性功能 \(預設為停用\)。  可透過將 <xref:System.Xml.Xsl.XsltSettings.EnableScript%2A?displayProperty=fullName> 屬性設為 `true`，並將 <xref:System.Xml.Xsl.XsltSettings> 物件傳遞至 <xref:System.Xml.Xsl.XslCompiledTransform.Load%2A> 方法，即可啟用指令碼。  
...
-   執行轉換時，會使用 <xref:System.Xml.XmlResolver> 解析 `document()` 函式。  
    ...
    > [!NOTE]
    >  `document()` 函式在 <xref:System.Xml.Xsl.XslCompiledTransform> 類別上預設為停用。  可透過將 <xref:System.Xml.Xsl.XsltSettings.EnableDocumentFunction%2A?displayProperty=fullName> 屬性設為 `true`，並將 <xref:System.Xml.Xsl.XsltSettings> 物件傳遞至 <xref:System.Xml.Xsl.XslCompiledTransform.Load%2A> 方法，即可啟用此功能。  
