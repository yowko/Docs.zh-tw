---
title: "非同步檔案 I-O | Microsoft Docs"
ms.custom: ""
ms.date: "03/03/2017"
...
-   `Await` \(Visual Basic\) 或 `await` \(C\#\) 運算子，會套用至非同步方法的結果。  
 ...
 若要實作非同步 I\/O 作業，請搭配非同步方法使用這些關鍵字，如下列範例所示。 如需詳細資訊，請參閱[使用 Async 和 Await 設計非同步程式](../Topic/Asynchronous%20Programming%20with%20Async%20and%20Await%20\(C%23%20and%20Visual%20Basic\).md)。  
  
 下列範例示範如何使用兩個 <xref:System.IO.FileStream> 物件，以非同步方式將檔案從一個目錄複製到另一個目錄。 請注意，<xref:System.Windows.Controls.Button> 控制項的 <xref:System.Web.UI.WebControls.Button.Click> 事件處理常式由於會呼叫非同步方法，因此會以 `async` 修飾詞標記。  
 ...
 下一個範例顯示程式碼後置檔案和 XAML 檔案，可用來開啟檔案作為 [!INCLUDE[win8_appname_long](../../../includes/win8-appname-long-md.md)] 應用程式中的 <xref:System.IO.Stream>，並使用 <xref:System.IO.StreamReader> 類別的執行個體來讀取其內容。 它會使用非同步方法開啟檔案作為資料流並讀取其內容。  
