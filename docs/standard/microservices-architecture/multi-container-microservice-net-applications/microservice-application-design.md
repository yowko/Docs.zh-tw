...
- **eShopOnContainers GitHub 存放庫：Source code for the reference application** (參考應用程式的原始程式碼)  
...
![外部架構 (微服務模式、API 閘道、彈性通訊、發佈/訂閱等) 與內部架構 (資料驅動/CRUD、DDD 模式、相依性插入、多個程式庫等) 之間的差異。](./media/image2.png)
