...
資料持續性元件可讓您存取裝載於微服務界限 (也就是微服務的資料庫) 內的資料。 它們包含存放庫和[工作單位](https://martinfowler.com/eaaCatalog/unitOfWork.html)類別 (例如自訂 Entity Framework (EF) <xref:Microsoft.EntityFrameworkCore.DbContext> 物件) 等元件的實際實作。

## <a name="the-repository-pattern"></a>儲存機制模式

儲存機制是封裝存取資料來源所需邏輯的類別或元件。 它們會集中管理常見的資料存取功能，以提供更佳的可維護性，並將用來存取資料庫與用來存取領域模型層的基礎結構或技術分離。 如果您使用類似 Entity Framework 的物件關聯式對應 (ORM)，多虧有 LINQ 和強型別，必須實作的程式碼會經過簡化。 這可讓您專注於資料持續性邏輯，而不是資料存取連接。
...
如果使用者進行變更，資料會從用戶端應用程式或展示層更新至應用程式層 (例如 Web API 服務)。 當您在命令處理常式中收到命令 (含資料) 時，您可以使用存放庫從資料庫取得您要更新的資料。 您在記憶體中將它更新為使用命令所傳遞的資訊，然後透過交易在資料庫中新增或更新資料 (領域實體)。
...
如稍早一節所述，建議您定義存放庫介面並將它放在領域模型層中，讓應用程式層 (例如您的 Web API 微服務) 不會直接相依於您實作實際存放庫類別的基礎結構層。 藉由這樣做，以及在您的 Web API 的控制器中使用相依性插入，您就可以實作模擬儲存機制來傳回假的資料，而不是資料庫中的資料。 此低耦合方法可讓您建立並執行單元測試，只測試您應用程式的邏輯，而不需要連接到資料庫。
...
資料存取物件會直接對儲存體執行資料存取和持續性作業。 存放庫會以您要在工作單位物件的記憶體中執行的作業來標示資料 (如同使用 <xref:Microsoft.EntityFrameworkCore.DbContext> 類別時的 EF)，但不會立即執行這些更新。
...
存放庫雖然有用，但它對您的 DDD 並不如彙總模式和豐富領域模型同樣重要。 因此，是否使用儲存機制模式完全取決於您。

## <a name="the-specification-pattern"></a>規格模式

規格模式 (其全名會是查詢規格模式) 是 DDD 模式，設計成您可以放置查詢定義以及選擇性排序和分頁邏輯的位置。

規格模式會定義物件中的查詢。 例如，若要封裝搜尋一些產品的分頁查詢，您可以建立 `PagedProduct` 規格來接受必要的輸入參數，例如 `pageNumber`、`pageSize`、`filter` 等。然後，在任何存放庫方法中 (通常是 List() 多載)，它會接受 `ISpecification` 並根據規格執行預期的查詢。
...
- 規格也可用來描述要傳回之資料的圖形，讓查詢只傳回所需的資料。 這樣做可避免必須在 Web 應用程式中進行消極式載入 (通常並不建議)，也可協助防止存放庫實作因為這些詳細資料而變得雜亂無章。
...
接下來的章節將會說明如何使用 EF Core 2.x 來實作規格模式，以及如何從任何存放庫類別來使用它。

> [!IMPORTANT]
> 規格模式是可透過許多不同方式實作的舊模式，如下列其他資源所示。 作為模式/概念，較舊的方法比較容易了解，但請注意，較舊的實作不會利用 Linq 和運算式等現代語言功能。
...
- **Eric Evans：Domain-Driven Design: Tackling Complexity in the Heart of Software.** (書籍，包括存放庫模式的討論) [_https://www.amazon.com/Domain-Driven-Design-Tackling-Complexity-Software/dp/0321125215/_](https://www.amazon.com/Domain-Driven-Design-Tackling-Complexity-Software/dp/0321125215/)
