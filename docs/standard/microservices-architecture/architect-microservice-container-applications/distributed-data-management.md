...
常用的方法是實作以 HTTP (REST) 為基礎的微服務，因為它們簡便易行。 HTTP 型的方法完全可行，問題是您如何使用它。 如果您使用 HTTP 要求和回應只是為了與用戶端應用程式或 API 閘道的微服務進行互動，那沒關係。 但如果您建立跨微服務的同步 HTTP 呼叫長鏈結，將微服務當作整合型應用程式的物件進行跨界限通訊，您的應用程式最後一定會發生問題。
