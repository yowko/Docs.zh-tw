...
|選項|描述|  
|------------|-----------------|  
|**/a @ no__t-1ssembly @ no__t-2：** _filename_|為組件中包含的所有類型或 *filename* 所指定的可執行檔產生序列化程式碼。 只能提供一個檔名。 如果重複使用這個引數，則會使用最後一個檔名。|  
|**/c @ no__t-1ompiler @ no__t-2：** _options_|指定要傳遞至 C# 編譯器的選項。 所有 csc.exe 選項都受到支援，可以傳遞至編譯器。 這個選項可以用來指定組件必須經過簽署，並指定金鑰檔。|  
...
|**/o @ no__t-1ut @ no__t-2：** _path_|指定要在其中儲存所產生之組件的目錄。 **注意：** 產生的組件名稱是由輸入組件的名稱加上 "xmlSerializers.dll" 所組成。|  
|**/p\[roxytypes\]**|只為 XML Web 服務 Proxy 型別產生序列化程式碼。|  
|**/r @ no__t-1eference @ no__t-2：** _assemblyfiles_|指定要求 XML 序列化的型別所參考的組件。 這個選項接受以逗號分隔多個組件檔案。|  
|**/s @ no__t-1ilent @ no__t-2**|隱藏顯示成功訊息。|  
