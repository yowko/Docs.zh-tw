...
XML 架構定義工具 ([Xml 架構定義工具 ( # A0) ](xml-schema-definition-tool-xsd-exe.md)) 與 .NET Framework 工具一起安裝，作為 Windows &reg; 軟體發展工具組 (SDK) 的一部分。 該工具的設計主要有兩個目的：  
...
如需使用此工具的詳細資訊，請參閱 [XML 架構定義工具 ( # A0) ](xml-schema-definition-tool-xsd-exe.md)。  
...
- [XML 結構描述繫結支援](/previous-versions/dotnet/netframework-4.0/sh1e66zd(v=vs.100))
