...
`System.Text.Json`命名空間提供從 JAVASCRIPT 物件標記法（JSON）序列化及還原序列化的功能。 連結`System.Text.Json`庫包含在[.net Core 3.0](https://aka.ms/netcore3download)共用架構中。 若是其他目標 framework，請安裝[System.web](https://www.nuget.org/packages/System.Text.Json) NuGet 封裝。 封裝支援：

* .NET Standard 2.0 和更新版本
* .NET Framework 4.7.2 和更新版本
* .NET Core 2.0、2.1 和2。2

`System.Text.Json`主要著重于效能、安全性和標準合規性。 它在預設行為上有一些主要的差異，並不是要與`Newtonsoft.Json`的功能同位。 在某些情況下`System.Text.Json` ，沒有內建功能，但有建議的因應措施。 針對其他案例，因應措施並不實用。 如果您的應用程式相依于遺失的功能，請考慮提出[問題](https://github.com/dotnet/runtime/issues/new)，以瞭解是否可以新增您案例的支援。

<!-- For information about which features might be added in future releases, see the [Roadmap](https://github.com/dotnet/runtime/tree/81bf79fd9aa75305e55abe2f7e9ef3f60624a3a1/src/libraries/System.Text.Json/roadmap/README.md). [Restore this when the roadmap is updated.]-->

本文大部分是<xref:System.Text.Json.JsonSerializer>關於如何使用 API，但它也包含如何使用（代表檔物件模型或 DOM）、 <xref:System.Text.Json.JsonDocument> <xref:System.Text.Json.Utf8JsonReader>和<xref:System.Text.Json.Utf8JsonWriter>類型的指引。

## <a name="table-of-differences-between-newtonsoftjson-and-systemtextjson"></a>Newtonsoft 與 system.string 之間的差異表

下表列出`Newtonsoft.Json`功能和`System.Text.Json`對應項。 對等專案屬於下列類別：

* 受到內建功能的支援。 從`System.Text.Json`取得類似的行為，可能需要使用屬性或全域選項。
* 不支援，可能的解決方法。 因應措施是[自訂轉換器](system-text-json-converters-how-to.md)，這可能不會提供`Newtonsoft.Json`完整的同位檢查功能。 其中一些範例程式碼會以範例的形式提供。 如果您依賴這些`Newtonsoft.Json`功能，則需要修改您的 .net 物件模型或其他程式碼變更。
* 不支援，因應措施不可行或無法解決。 如果您依賴這些`Newtonsoft.Json`功能，則不會有重大變更，因此無法進行遷移。

| Newtonsoft Json 功能                               | System.object 對等 |
|-------------------------------------------------------|-----------------------------|
...
| 將推斷的類型`object`還原序列化為屬性      | ⚠️[不支援、](#deserialization-of-object-properties)因應措施、範例 |
...
| `Required`在屬性`[JsonProperty]`上設定        | ⚠️[不支援、](#required-properties)因應措施、範例 |
| `NullValueHandling`在屬性`[JsonProperty]`上設定 | ⚠️[不支援、](#conditionally-ignore-a-property)因應措施、範例  |
| `DefaultValueHandling`在屬性`[JsonProperty]`上設定 | ⚠️[不支援、](#conditionally-ignore-a-property)因應措施、範例  |
...
| 屬性的`System.Runtime.Serialization`支援 | ❌ [不受支援](#systemruntimeserialization-attributes) |
...
在還原序列化`Newtonsoft.Json`期間，預設會執行不區分大小寫的屬性名稱比對。 <xref:System.Text.Json>預設值會區分大小寫，這可提供較佳的效能，因為它會執行完全相符的作業。 如需如何執行不區分大小寫比對的相關資訊，請參閱不[區分大小寫的屬性](system-text-json-how-to.md#case-insensitive-property-matching)比對。

如果您使用 ASP.NET Core `System.Text.Json`間接地使用，則不需要執行任何動作來取得之類`Newtonsoft.Json`的行為。 ASP.NET Core 指定使用`System.Text.Json`時， [camel 大小寫屬性名稱](system-text-json-how-to.md#use-camel-case-for-all-json-property-names)和不區分大小寫比對的設定。

### <a name="minimal-character-escaping"></a>最少字元的轉義

在序列化期間`Newtonsoft.Json` ，相對於讓字元通過，而不將它們進行轉義，相當寬鬆。 也就是說，它不會以`\uxxxx`字元的`xxxx`程式碼點取代它們。 當它將其轉義時，會在字元`\`前面發出（例如， `"`變成`\"`）。 <xref:System.Text.Json>根據預設，會將更多字元轉義以提供跨網站腳本（XSS）或資訊洩漏攻擊的深層防禦保護，並藉由使用六個字元的順序來執行此動作。 `System.Text.Json`預設會將所有非 ASCII 字元加上轉義，因此如果您在中`StringEscapeHandling.EscapeNonAscii` `Newtonsoft.Json`使用，就不需要執行任何動作。 `System.Text.Json`根據預設，也會對 HTML 敏感字元進行轉義。 如需如何覆寫預設`System.Text.Json`行為的相關資訊，請參閱[自訂字元編碼](system-text-json-how-to.md#customize-character-encoding)。

### <a name="comments"></a>評價

在還原序列化`Newtonsoft.Json`期間，預設會忽略 JSON 中的批註。 <xref:System.Text.Json>預設為會擲回批註的例外狀況，因為[RFC 8259](https://tools.ietf.org/html/rfc8259)規格不會包含它們。 如需如何允許批註的相關資訊，請參閱[允許批註和尾端逗號](system-text-json-how-to.md#allow-comments-and-trailing-commas)。

### <a name="trailing-commas"></a>尾端逗號

在還原序列化`Newtonsoft.Json`期間，預設會忽略尾端逗號。 它也會忽略多個尾端逗號（例如`[{"Color":"Red"},{"Color":"Green"},,]`）。 <xref:System.Text.Json>預設值是擲回尾端逗號的例外狀況，因為[RFC 8259](https://tools.ietf.org/html/rfc8259)規格不允許它們。 如需`System.Text.Json`如何接受這些專案的相關資訊，請參閱[允許批註和尾端逗號](system-text-json-how-to.md#allow-comments-and-trailing-commas)。 沒有任何方法可以允許多個尾端逗號。

### <a name="converter-registration-precedence"></a>轉換器註冊優先順序

自`Newtonsoft.Json`定義轉換器的註冊優先順序如下所示：
...
自<xref:System.Text.Json>定義轉換器的註冊優先順序不同：
...
`Newtonsoft.Json`預設不具有最大深度限制。 預設<xref:System.Text.Json>限制為64，且可透過設定來進行設定<xref:System.Text.Json.JsonSerializerOptions.MaxDepth?displayProperty=nameWithType>。

### <a name="json-strings-property-names-and-string-values"></a>JSON 字串（屬性名稱和字串值）

在還原序列化`Newtonsoft.Json`期間，接受以雙引號、單引號或不含引號括住的屬性名稱。 它接受以雙引號或單引號括住的字串值。 例如， `Newtonsoft.Json`接受下列 JSON：
...
`Newtonsoft.Json`接受非字串值（例如數位或常`true`值和`false`），以還原序列化為 string 類型的屬性。 以下是可`Newtonsoft.Json`成功還原序列化為下列類別的 JSON 範例：
...
內建功能不支援下列案例，但可能的因應措施。 因應措施是[自訂轉換器](system-text-json-converters-how-to.md)，這可能不會提供`Newtonsoft.Json`完整的同位檢查功能。 其中一些範例程式碼會以範例的形式提供。 如果您依賴這些`Newtonsoft.Json`功能，則需要修改您的 .net 物件模型或其他程式碼變更。
...
`Newtonsoft.Json`可以序列化或還原序列化 JSON 字串所代表的數位（以引號括住）。 例如，它可以接受： `{"DegreesCelsius":"23"}`而不是`{"DegreesCelsius":23}`。 若要在中<xref:System.Text.Json>啟用該行為，請執行如下列範例所示的自訂轉換子。 轉換器會處理定義為的`long`屬性：
...
`Newtonsoft.Json`支援類型`Dictionary<TKey, TValue>`的集合。 內<xref:System.Text.Json>建的字典集合支援僅限於`Dictionary<string, TValue>`。 也就是，索引鍵必須是字串。
...
`Newtonsoft.Json`自動執行多型序列化。 如需的有限多型序列化功能的<xref:System.Text.Json>相關資訊，請參閱[序列化衍生類別的屬性](system-text-json-how-to.md#serialize-properties-of-derived-classes)。

此處所述的因應措施，是定義可能包含衍生類別作為`object`類型的屬性。 如果無法這樣做，另一個選項是使用整個繼承類型階層`Write`的方法來建立轉換器，如[如何撰寫自訂轉換器](system-text-json-converters-how-to.md#support-polymorphic-deserialization)中的範例所示。

### <a name="polymorphic-deserialization"></a>多型還原序列化

`Newtonsoft.Json`具有在`TypeNameHandling`序列化時將類型名稱中繼資料新增至 JSON 的設定。 它會在還原序列化時使用中繼資料來執行多型還原序列化。 <xref:System.Text.Json>可以執行有限範圍的多型序列化，但不能進行多型的還原[序列化](system-text-json-how-to.md#serialize-properties-of-derived-classes)。
...
當`Newtonsoft.Json`還原序列化<xref:System.Object>為時，它會：

* 推斷 JSON 裝載中基本值的類型（不是`null`），並以已裝箱的`string`物件`long`傳回`double`預`boolean`存、 `DateTime` 、、或。 *基本值*是單一 json 值，例如 json 數位、字串、 `true` `false`、或。 `null`
* 針對 JSON `JObject`承載`JArray`中的複雜值，傳回或。 *複雜值*是以括弧（`{}`）括住的 JSON 索引鍵/值組集合，或括弧內`[]`的值清單（）。 大括弧或括弧內的屬性和值可以有額外的屬性或值。
* 當裝載具有`null` JSON 常值時，會傳回 null 參考。

<xref:System.Text.Json>會在還原`JsonElement`序列化為<xref:System.Object>時，儲存基本和複雜值的已裝箱，例如：
...
不過， `System.Text.Json`會`null`將相同的`Newtonsoft.Json`視為，並在裝載具有`null` JSON 常值時傳回 null 參考。

若要執行屬性的`object`型別推斷，請建立如[如何撰寫自訂轉換器](system-text-json-converters-how-to.md#deserialize-inferred-types-to-object-properties)中的範例所示的轉換器。
...
**注意：** 前面的轉換器**處理 null 值**的方式`Newtonsoft.Json`與指定預設值的 poco 不同。 例如，假設下列程式碼代表您的目標物件：
...
在還原序列化之後`Date` ，屬性具有 1/1/0001`default(DateTimeOffset)`（），也就是會覆寫在此函式中設定的值。 假設有相同的 POCO 和 JSON `Newtonsoft.Json` ，還原序列化會在`Date`屬性中留下1/1/2001。
...
藉由[將轉換器加入](system-text-json-converters-how-to.md#registration-sample---converters-collection) <xref:System.Text.Json.JsonSerializerOptions.Converters>集合中，來註冊此自訂的轉換器。
...
`Newtonsoft.Json`屬性可讓您指定在還原序列化為 POCO 時要呼叫的函`[JsonConstructor]`式。 <xref:System.Text.Json>僅支援無參數的函式。 因應措施是，您可以在自訂轉換器中呼叫所需的任何一個方法。 請參閱還原序列化[為不可變類別和結構](#deserialize-to-immutable-classes-and-structs)的範例。

### <a name="required-properties"></a>必要屬性

在`Newtonsoft.Json`中，您可以透過在`Required` `[JsonProperty]`屬性上設定來指定屬性是必要的。 `Newtonsoft.Json`如果在 JSON 中未收到標示為必要的屬性值，則會擲回例外狀況。
...
若要讓還原序列化失敗`Date` ，如果 JSON 中沒有屬性，請執行自訂轉換子。 如果在還原序列化完成後未設定此`Date`屬性，下列範例轉換器程式碼會擲回例外狀況：

[!code-csharp[](~/samples/snippets/core/system-text-json/csharp/WeatherForecastRequiredPropertyConverter.cs)]

[使用 POCO 類別上的屬性](system-text-json-converters-how-to.md#registration-sample---jsonconverter-on-a-type)，或[將轉換器加入](system-text-json-converters-how-to.md#registration-sample---converters-collection)至<xref:System.Text.Json.JsonSerializerOptions.Converters>集合，以註冊此自訂轉換器。

如果您遵循此模式，請不要在遞迴呼叫<xref:System.Text.Json.JsonSerializer.Serialize%2A>或<xref:System.Text.Json.JsonSerializer.Deserialize%2A>時傳入 options 物件。 Options 物件包含<xref:System.Text.Json.JsonSerializerOptions.Converters%2A>集合。 如果您將它傳遞給`Serialize`或`Deserialize`，自訂轉換器會呼叫自己的，使無限迴圈產生堆疊溢位例外狀況。 如果預設選項不可行，請使用您所需的設定來建立選項的新實例。 這個方法將會變慢，因為每個新的實例會獨立快取。
...
* 上`NullValueHandling` `JsonSerializerSettings`的`DefaultValueHandling`和設定可讓您指定應該忽略所有的 null 值或預設值屬性。
* 屬性的`NullValueHandling`和`DefaultValueHandling`設定可讓您指定在設定為 null 或預設值時，應該忽略的個別屬性。 `[JsonProperty]`
...
如果參數的值`Summary`為 null、空字串或 "N/A"，則轉換器會使屬性從序列化中省略。
...
`Newtonsoft.Json`提供數種方式來控制和`DateTime` `DateTimeOffset`類型的屬性如何序列化和還原序列化：
...
在<xref:System.Text.Json>中，您可以藉由撰寫自訂的轉換器來模擬回呼。 下列範例顯示 POCO 的自訂轉換子。 轉換器包含的程式碼會在每個對應至`Newtonsoft.Json`回呼的點上顯示訊息。
...
* 當遞迴呼叫`Serialize`或`Deserialize`時，請勿傳入 options 物件。 Options 物件包含`Converters`集合。 如果您將它傳遞給`Serialize`或`Deserialize`，則會使用轉換器，並產生無限迴圈，導致堆疊溢位例外狀況。
...
`Newtonsoft.Json`可以透過`JsonProperty`屬性使用私用和內部屬性 setter 和 getter。 <xref:System.Text.Json>僅支援公用 setter。 自訂轉換器可以提供這種功能。

### <a name="populate-existing-objects"></a>填入現有的物件

在`JsonConvert.PopulateObject`中`Newtonsoft.Json` ，方法會將 JSON 檔案還原序列化為類別的現有實例，而不是建立新的實例。 <xref:System.Text.Json>一律使用預設的公用無參數函式來建立目標型別的新實例。 自訂轉換器可以還原序列化為現有的實例。
...
在還原序列化`Newtonsoft.Json`期間，即使屬性沒有 setter，也會將物件加入至集合。 <xref:System.Text.Json>忽略沒有 setter 的屬性。 自訂轉換器可以提供這種功能。
...
根據預設， `Newtonsoft.Json`會以傳值方式序列化。 例如，如果物件包含兩個屬性，其中包含相同`Person`物件的參考，則該`Person`物件屬性的值會在 JSON 中重複。

`Newtonsoft.Json`具有上`PreserveReferencesHandling` `JsonSerializerSettings`的設定，可讓您以傳址方式序列化：

* 識別碼中繼資料會加入至為第一個`Person`物件建立的 JSON。
...
<xref:System.Text.Json>不支援命名空間中`System.Runtime.Serialization`的屬性，例如`DataMemberAttribute`和`IgnoreDataMemberAttribute`。
...
<xref:System.Text.Json.JsonDocument?displayProperty=fullName>提供從現有 JSON 承載剖析和建立**唯讀**檔物件模型（DOM）的功能。 DOM 提供 JSON 承載中資料的隨機存取。 組成裝載的 JSON 元素可以透過<xref:System.Text.Json.JsonElement>類型來存取。 `JsonElement`類型會提供 api，將 JSON 文字轉換成常見的 .net 類型。 `JsonDocument`公開<xref:System.Text.Json.JsonDocument.RootElement>屬性。

### <a name="jsondocument-is-idisposable"></a>JsonDocument 是 IDisposable

`JsonDocument`將資料的記憶體中視圖建立成集區緩衝區。 因此，與`JObject`或`JArray`不同`Newtonsoft.Json`， `JsonDocument`型別會`IDisposable`執行，而且必須在 using 區塊內使用。

如果您想`JsonDocument`要將存留期擁有權和處置責任轉移給呼叫者，則只會從您的 API 傳回。 在大部分的情況下，這並不是必要的。 如果呼叫端需要使用整個 JSON 檔， <xref:System.Text.Json.JsonElement.Clone%2A>則會傳回的<xref:System.Text.Json.JsonDocument.RootElement%2A>，其為。 <xref:System.Text.Json.JsonElement> 如果呼叫端需要使用 JSON 檔中的特定元素， <xref:System.Text.Json.JsonElement.Clone%2A>會<xref:System.Text.Json.JsonElement>傳回的。 如果您直接傳回`RootElement`或子專案`Clone`，但未建立，則呼叫端將無法存取擁有它的所傳回`JsonElement`的（ `JsonDocument`在它被處置之後）。
...
上述`JsonElement`程式碼需要包含`fileName`屬性的。 它會開啟 JSON 檔案，並建立`JsonDocument`。 方法假設呼叫端想要使用整份檔，因此它會`Clone`傳回的。 `RootElement`

如果您收到`JsonElement` ，且傳回子項目，則不需要傳回子項目`Clone`的。 呼叫端負責讓傳入的`JsonDocument` `JsonElement`屬於保持運作狀態。 例如：
...
* 若要從頭`JsonDocument`開始建立（也就是不會將現有的 JSON 承載傳遞至`Parse`方法），請使用寫入 JSON 文字， `Utf8JsonWriter`並剖析該輸出，使其成為新`JsonDocument`的。
* 若要修改現有`JsonDocument`的，請使用它來寫入 JSON 文字，並在您撰寫時進行變更，並剖析該輸出以建立`JsonDocument`新的。
* 若要合併現有的 JSON 檔，相當`JObject.Merge`于`JContainer.Merge`來自`Newtonsoft.Json`的或 api，請參閱[此 GitHub 問題](https://github.com/dotnet/corefx/issues/42466#issuecomment-570475853)。

### <a name="jsonelement-is-a-union-struct"></a>JsonElement 是聯集結構

`JsonDocument`將公開`RootElement`為類型<xref:System.Text.Json.JsonElement>的屬性，也就是包含任何 JSON 專案的聯集、結構類型。 `Newtonsoft.Json`會使用專用的階層`JObject`類型`JArray`， `JToken`例如、、等。 `JsonElement`是您可以搜尋和列舉的專案，您可以使用`JsonElement`將 JSON 元素具體化成 .net 類型。

### <a name="how-to-search-a-jsondocument-and-jsonelement-for-sub-elements"></a>如何搜尋子項目的 JsonDocument 和 JsonElement

使用`JObject`或`JArray`從`Newtonsoft.Json`搜尋 JSON 權杖通常會相對快速，因為它們是在某個字典中的查閱。 相較之下，搜尋`JsonElement`需要順序搜尋屬性，因此速度相對緩慢（例如使用`TryGetProperty`時）。 <xref:System.Text.Json>的設計目的是要將初始剖析時間減至最少，而不是查閱時間。 因此，在搜尋`JsonDocument`物件時，請使用下列方法來優化效能：
...
* 請不要`JsonDocument`透過每個屬性，使用`RootElement`來執行順序搜尋。 相反地，請根據 JSON 資料的已知結構來搜尋嵌套的 JSON 物件。 例如，如果`Grade`您要尋找物件中`Student`的屬性，請對`Student`物件執行迴圈，並取得每一個的`Grade`值，而不是搜尋所有`JsonElement`尋找`Grade`屬性的物件。 執行後者會導致相同資料的不必要傳遞。
...
<xref:System.Text.Json.Utf8JsonReader?displayProperty=fullName>為 UTF-8 編碼 JSON 文字的高效能、低配置、順向讀取器、從[\<ReadOnlySpan 的位元組讀取>](xref:System.ReadOnlySpan%601)或 ReadOnlySequence [ \<的位元組>](xref:System.Buffers.ReadOnlySequence%601)。 `Utf8JsonReader`是可以用來建立自訂剖析器和還原序列化程式的低層級類型。

下列各節說明使用`Utf8JsonReader`的建議程式設計模式。

### <a name="utf8jsonreader-is-a-ref-struct"></a>Utf8JsonReader 是 ref 結構

因為`Utf8JsonReader`型別是*ref 結構*，所以有[一些限制](../../csharp/language-reference/builtin-types/struct.md#ref-struct)。 例如，它無法在 ref 結構以外的類別或結構上儲存為欄位。 為了達到高效能，此型別必須是`ref struct` ，因為它需要快取輸入[ReadOnlySpan\<byte>](xref:System.ReadOnlySpan%601)，這本身就是 ref 結構。 此外，這種類型是可變動的，因為它會保存狀態。 因此，請以 ref 而非傳值方式來**傳遞它**。 以傳值方式傳遞會導致結構複製，而且呼叫端看不到狀態變更。 這與不同`Newtonsoft.Json` `Newtonsoft.Json` `JsonTextReader` ，因為是類別。 如需如何使用 ref 結構的詳細資訊，請參閱[撰寫安全且有效率的 c # 程式碼](../../csharp/write-safe-efficient-code.md)。
...
支援從 UTF-8 編碼的[\<ReadOnlySpan 位元組讀取>](xref:System.ReadOnlySpan%601)或[\<ReadOnlySequence byte>](xref:System.Buffers.ReadOnlySequence%601) （這是從讀取的結果<xref:System.IO.Pipelines.PipeReader>）。 `Utf8JsonReader`
...
因為會`Utf8JsonReader`將輸入視為 JSON 文字，所以會將 utf-8 位元組順序標記（BOM）視為不正確 json。 呼叫端必須先篩選掉，再將資料傳遞給讀取器。
...
如果您的 JSON 輸入是[ReadOnlySpan\<的位元組>](xref:System.ReadOnlySpan%601)，則當您執行讀取迴圈時`ValueSpan` ，可以從讀取器上的屬性存取每個 json 元素。 不過，如果您的輸入是[ReadOnlySequence\<的位元組>](xref:System.Buffers.ReadOnlySequence%601) （這是從讀取的結果<xref:System.IO.Pipelines.PipeReader>），某些 JSON 元素可能會跨`ReadOnlySequence<byte>`物件的多個區段。 <xref:System.Text.Json.Utf8JsonReader.ValueSpan%2A>在連續記憶體區塊中無法存取這些元素。 相反地，每當您有多個區段`ReadOnlySequence<byte>`做為輸入時， <xref:System.Text.Json.Utf8JsonReader.HasValueSequence%2A>請輪詢讀取器上的屬性，以瞭解如何存取目前的 JSON 元素。 以下是建議的模式：
...
請勿使用<xref:System.Text.Json.Utf8JsonReader.ValueSpan%2A> ，藉由呼叫<xref:System.MemoryExtensions.SequenceEqual%2A>屬性名稱查閱來進行逐位元組比較。 請<xref:System.Text.Json.Utf8JsonReader.ValueTextEquals%2A>改為呼叫，因為該方法會將任何在 JSON 中進行轉義的字元。 以下範例顯示如何搜尋名為 "name" 的屬性：
...
`Newtonsoft.Json`提供傳回的 api <xref:System.Nullable%601>，例如`ReadAsBoolean`，其會藉由`Null` `TokenType`傳回來為您處理`bool?`。 內`System.Text.Json`建 api 只會傳回不可為 null 的實數值型別。 例如， <xref:System.Text.Json.Utf8JsonReader.GetBoolean%2A?displayProperty=nameWithType>會傳回`bool`。 如果它在 JSON 中找到`Null` ，它會擲回例外狀況。 下列範例示範兩種處理 null 的方式：一個是藉由傳回可為 null 的實值型別，另一個則是傳回預設值。
...
如果您需要繼續針對特定目標`Newtonsoft.Json` framework 使用，您可以多個目標，而且有兩個執行。 不過，這並不簡單，而且需要進行`#ifdefs`一些和來源的重複。 盡可能共用程式碼的方法之一， `ref struct`就是在`Utf8JsonReader`和`Newtonsoft.Json` `JsonTextReader`之間建立包裝函式。 此包裝函式會統一公用介面區，同時隔離行為差異。 這可讓您隔離主要變更與型別的結構，並以傳址方式傳遞新的型別。 這是[DependencyModel](https://www.nuget.org/packages/Microsoft.Extensions.DependencyModel/3.1.0/)程式庫所遵循的模式：
...
<xref:System.Text.Json.Utf8JsonWriter?displayProperty=fullName>是從常見的 .net 類型（例如`String`、 `Int32`和`DateTime`）撰寫 utf-8 編碼 JSON 文字的高效能方式。 寫入器是可以用來建立自訂序列化程式的低層級類型。

下列各節說明使用`Utf8JsonWriter`的建議程式設計模式。

### <a name="write-with-utf-8-text"></a>以 UTF-8 文字撰寫

若要在使用時達到最佳效能`Utf8JsonWriter`，請撰寫已編碼為 utf-8 文字，而不是 utf-16 字串的 JSON 承載。 使用<xref:System.Text.Json.JsonEncodedText>快取和預先編碼已知的字串屬性名稱和值做為靜態，並將其傳遞給寫入器，而不是使用 utf-16 字串常值。 這比快取和使用 UTF-8 位元組陣列更快速。
...
`Newtonsoft.Json`方法會寫入原始 JSON，其中應為`WriteRawValue`值。 <xref:System.Text.Json>沒有直接的對等用法，但以下是可確保只會寫入有效 JSON 的因應措施：
...
的[StringEscapeHandling](https://www.newtonsoft.com/json/help/html/T_Newtonsoft_Json_StringEscapeHandling.htm)設定會`JsonTextWriter`提供選項來將所有非 ASCII 字元**或**HTML 字元全部轉義。 根據預設， `Utf8JsonWriter`會將所有非 ASCII**和**HTML 字元全部轉義。 這項轉義是針對深層防禦的安全性原因而完成。 若要指定不同的轉義原則，請<xref:System.Text.Encodings.Web.JavaScriptEncoder>建立， <xref:System.Text.Json.JsonWriterOptions.Encoder?displayProperty=nameWithType>並設定。 如需詳細資訊，請參閱[自訂字元編碼](system-text-json-how-to.md#customize-character-encoding)。
...
若要使用`Utf8JsonWriter`寫入 null 值，請呼叫：
...
若為字串屬性，則為，如果字串為<xref:System.Text.Json.Utf8JsonWriter.WriteString%2A> null <xref:System.Text.Json.Utf8JsonWriter.WriteStringValue%2A> ，而且相當`WriteNull`于`WriteNullValue`和。

### <a name="write-timespan-uri-or-char-values"></a>寫入 Timespan、Uri 或 char 值

`JsonTextWriter`提供`WriteValue` [TimeSpan](https://www.newtonsoft.com/json/help/html/M_Newtonsoft_Json_JsonTextWriter_WriteValue_18.htm)、 [Uri](https://www.newtonsoft.com/json/help/html/M_Newtonsoft_Json_JsonTextWriter_WriteValue_22.htm)和[char](https://www.newtonsoft.com/json/help/html/M_Newtonsoft_Json_JsonTextWriter_WriteValue_3.htm)值的方法。 `Utf8JsonWriter`沒有對等的方法。 相反地，請將這些值格式化為字串`ToString()`（例如，藉由呼叫<xref:System.Text.Json.Utf8JsonWriter.WriteStringValue%2A>）並呼叫。

### <a name="multi-targeting"></a>多目標

如果您需要繼續針對特定目標`Newtonsoft.Json` framework 使用，您可以多個目標，而且有兩個執行。 不過，這並不簡單，而且需要進行`#ifdefs`一些和來源的重複。 盡可能共用程式碼的方法之一，就是在`Utf8JsonWriter`和`Newtonsoft` `JsonTextWriter`之間建立包裝函式。 此包裝函式會統一公用介面區，同時隔離行為差異。 這可讓您將變更主要隔離到類型的結構。 [DependencyModel](https://www.nuget.org/packages/Microsoft.Extensions.DependencyModel/3.1.0/)程式庫如下：
