...
這不是 `Newtonsoft.Json` 功能的詳盡清單。 此清單包含[GitHub 問題](https://github.com/dotnet/runtime/issues?q=is%3Aopen+is%3Aissue+label%3Aarea-System.Text.Json)或[StackOverflow](https://stackoverflow.com/questions/tagged/system.text.json)文章中所要求的許多案例。 如果您針對此處所列的其中一個案例執行因應措施，但目前沒有範例程式碼，而且您想要共用您的方案，請在此頁面的 [意見反應][區段](/dotnet/standard/serialization/system-text-json-migrate-from-newtonsoft-how-to#feedback)中選取**此頁面**。 這會建立 GitHub 問題，並將其列在此頁面底部。
...
當 `Newtonsoft.Json` 還原序列化為 <xref:System.Object>時，它會：
...
<xref:System.Text.Json> 會在還原序列化為 <xref:System.Object>時，儲存基本和複雜值的已裝箱 `JsonElement`，例如：
...
不過，`System.Text.Json` 會將 `null` 視為與 `Newtonsoft.Json` 相同，並在裝載具有 `null` JSON 常值時傳回 null 參考。
