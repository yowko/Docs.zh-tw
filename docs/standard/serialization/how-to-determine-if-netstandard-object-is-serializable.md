...
---
# <a name="how-to-determine-if-a-net-standard-object-is-serializable"></a>如何： 判斷是否可序列化的.NET Standard 物件

.NET Standard 是定義的類型和成員必須要有特定的.NET 實作符合標準的該版本上的規格。 不過，.NET Standard 並未定義是否可序列化類型。 .NET 的標準程式庫中定義的類型未標示有<xref:System.SerializableAttribute>屬性。 相反地，.NET 特定的實作，例如.NET Framework 和.NET Core，可用來判斷特定的型別是否可序列化。  
