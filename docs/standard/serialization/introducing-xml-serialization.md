...
下列專案可以使用 **XmlSerializer** 類別進行序列化：
...
當您將物件序列化為 XML 時， **XmlSerializer** 類別可讓您完整且彈性地控制。 若您建立 XML Web 服務，可套用控制序列化的屬性至類別與成員，以確保 XML 輸出符合特定結構描述。
