...
二進位序列化允許修改物件內的私用成員，因此變更它的狀態。 因此，建議您在公用 API 介面上運作的其他序列化架構（例如 <xref:System.Text.Json?displayProperty=fullName>）。
...
> | <xref:System.Data.DataTable?displayProperty=nameWithType> | 如果您將 `RemotingFormat` 設定為 `SerializationFormat.Binary`，則只能與 .NET Core 2.1 和更新版本交換。 |
