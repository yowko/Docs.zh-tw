...
有時候，結構的預設封送處理規則並不是您所需要的樣子。 .NET 執行階段提供一些延伸點，讓您自訂您結構的配置以及封送處理欄位的方式。
...
## <a name="customizing-boolean-field-marshalling"></a>自訂布林值欄位封送處理
...
布林值預設為封送處理為原生 4 位元 Win32 [`BOOL`](/windows/desktop/winprog/windows-data-types#BOOL) 值，如下列範例所示：
...
## <a name="customizing-array-field-marshalling"></a>自訂陣列欄位封送處理
...
若需要就地封送處理陣列，您可以使用 <xref:System.Runtime.InteropServices.UnmanagedType.ByValArray?displayProperty=nameWithType> 值告訴封送處理器就地封送處理陣列。 當您此用此封送處理時，您也必須提供值給 <xref:System.Runtime.InteropServices.MarshalAsAttribute.SizeConst?displayProperty=nameWithType> 欄位做為陣列中的元素數目，以便執行階段可以正確地為結構配置空間。
...
## <a name="customizing-string-field-marshalling"></a>自訂字串欄位封送處理
...
若您的 API 要求您在結構中就地傳遞字串，您可以使用 <xref:System.Runtime.InteropServices.UnmanagedType.ByValTStr?displayProperty=nameWithType> 值。 請注意，由 `ByValTStr` 封送處理之字串的編碼是由 `CharSet` 屬性決定的。 此外，它要求字串長度必須由 <xref:System.Runtime.InteropServices.MarshalAsAttribute.SizeConst?displayProperty=nameWithType> 欄位傳遞。
...
## <a name="customizing-decimal-field-marshalling"></a>自訂十進位欄位封送處理
...
## <a name="marshalling-systemobjects"></a>封送處理 `System.Object`
