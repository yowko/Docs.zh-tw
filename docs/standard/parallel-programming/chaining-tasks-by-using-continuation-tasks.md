...
-   您可以使用第二個接續來觀察第一個接續的 <xref:System.Threading.Tasks.Task.Exception%2A> 屬性。 在下列範例中，工作會嘗試讀取不存在的檔案。 然後接續會顯示有關前項工作中例外狀況的資訊。  
     ...
     如需詳細資訊，請參閱[例外狀況處理](../../../docs/standard/parallel-programming/exception-handling-task-parallel-library.md)和 [NIB：如何：處理工作擲回的例外狀況](http://msdn.microsoft.com/library/d6c47ec8-9de9-4880-beb3-ff19ae51565d)。  
