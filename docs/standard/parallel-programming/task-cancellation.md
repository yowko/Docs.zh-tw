...
<xref:System.Threading.Tasks.Task?displayProperty=nameWithType> 和 <xref:System.Threading.Tasks.Task%601?displayProperty=nameWithType> 類別可使用 .NET Framework 中的取消語彙基元來支援取消作業。 如需詳細資訊，請參閱[Managed 執行緒中的取消](../../../docs/standard/threading/cancellation-in-managed-threads.md)。 在 Task 類別中，取消作業包括使用者委派之間的合作，這是指可取消的作業和要求取消的程式碼。  成功的取消包括要求的程式碼呼叫<xref:System.Threading.CancellationTokenSource.Cancel%2A?displayProperty=nameWithType>方法，以及適時終止作業的使用者委派。 您可以使用下列選項之一來終止作業：  
...
-   擲回 <xref:System.OperationCanceledException> ，並將其傳遞至據以要求取消的語彙基元。 執行此作業的常見方式是使用 <xref:System.Threading.CancellationToken.ThrowIfCancellationRequested%2A> 方法。 以此方式取消的工作會切換至 Canceled 狀態，可供呼叫端節點用以驗證工作已回應其取消要求。  
 ...
 如果您正在等候轉換至 Canceled 狀態的工作上<xref:System.Threading.Tasks.TaskCanceledException?displayProperty=nameWithType>例外狀況 (包裝在<xref:System.AggregateException>例外狀況) 就會擲回。 請注意，此例外狀況表示取消成功，而非錯誤情況。 因此，工作的 <xref:System.Threading.Tasks.Task.Exception%2A> 屬性會傳回 `null`。  
