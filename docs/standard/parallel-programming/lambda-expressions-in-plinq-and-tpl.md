...
## <a name="action-delegate"></a>Action 委派  
 <xref:System.Action?displayProperty=nameWithType> 委派所封裝的方法 (在 [!INCLUDE[vbprvb](../../../includes/vbprvb-md.md)] 中為 Sub) 不會傳回值，或是會傳回 [void](~/docs/csharp/language-reference/keywords/void.md)。 在 Action 型別簽章中，型別參數只代表輸入參數。 就像 Func，Framework Class Library 會定義 17 個版本的 Action，從不具型別參數的版本，一直到具有 16 個型別參數的版本。  
