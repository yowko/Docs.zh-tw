...
請注意，區分文化特性的字串作業並不一定是所要的行為。 若結果應該與文化特性無關，但使用了區分文化特性的作業，可能會造成應用程式程式碼在具有自訂大小寫對應和排序規則的文化特性上失敗。 例如，請參閱"[使用字串的最佳做法](../../../docs/standard/base-types/best-practices-strings.md)"一文中的["使用目前範圍性的字串比較"](../../../docs/standard/base-types/best-practices-strings.md#string-comparisons-that-use-the-current-culture)部分。
