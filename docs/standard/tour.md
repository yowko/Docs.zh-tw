...
有一些物件會參考 *Unmanaged 資源*。 Unmanaged 的資源是指 .NET 執行階段不會自動維護的資源。  例如檔案控制程式碼就是 Unmanaged 資源。  @System.IO.FileStream 物件是Managed 物件，但會 Unmanaged 檔案控制代碼。  當您結束使用 FileStream 之後，必須釋放檔案控制代碼。
...
.NET 語言是物件導向，具有基底和衍生類別的階層架構。 .NET 執行階段只允許符合物件階層架構的的物件轉換和呼叫。 請記住，以任何 .NET 語言所定義的每種類型都是衍生自基底 `object` 型別。
...
C#、Visual Basic 與 F# 支援本機**型別推斷**。 型別推斷表示編譯器會從右邊的運算式推算左邊的運算式型別。 這並不會破壞或規避型別安全。 產生的類型**具有**強類型，其中包含其所指的所有項目。 讓我們重寫上一個範例中的前兩行，以引入型別推斷。 請注意，此範例的其餘部分完全相同。
