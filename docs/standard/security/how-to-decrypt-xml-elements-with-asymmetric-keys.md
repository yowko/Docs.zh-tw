...
---
# <a name="how-to-decrypt-xml-elements-with-asymmetric-keys"></a>HOW TO：使用非對稱金鑰解密 XML 項目
您可以使用 <xref:System.Security.Cryptography.Xml> 命名空間中的類別來加密和解密 XML 文件內的項目。  XML 加密是交換或儲存加密 XML 資料的標準方法，不必擔心資料被輕易讀取。  如需 XML 加密標準的詳細資訊，請參閱全球資訊網協會（W3C）建議[xml 簽章語法和處理](https://www.w3.org/TR/xmldsig-core/)。  
 ...
 此程式中的範例會使用 how [to：使用非對稱金鑰加密 XML](../../../docs/standard/security/how-to-encrypt-xml-elements-with-asymmetric-keys.md)專案中所述的方法，解密已加密的 XML 元素。  它會尋找 <`EncryptedData`> 專案、將專案解密，然後使用原始純文字 XML 專案來取代元素。  
  
 此範例會使用兩個金鑰來解密 XML 項目。  它會從金鑰容器中抓取先前產生的 RSA 私密金鑰，然後使用 RSA 金鑰來解密儲存在 <`EncryptedData`> 元素的 <`EncryptedKey`> 元素中的工作階段金鑰。  範例接著會使用工作階段金鑰解密 XML 項目。  
...
5. 呼叫 <xref:System.Security.Cryptography.Xml.EncryptedXml.DecryptDocument%2A> 方法，以將 <`EncryptedData`> 元素解密。  這個方法會使用 RSA 金鑰來解密工作階段金鑰，並會自動使用工作階段金鑰解密 XML 項目。  它也會自動以原始純文字取代 <`EncryptedData`> 元素。  
...
## <a name="example"></a>範例  
 這個範例假設名為 `test.xml` 的檔案已存在於和編譯程式相同的目錄中。  它也假設 `test.xml` 包含使用[如何：使用非對稱金鑰加密 XML](../../../docs/standard/security/how-to-encrypt-xml-elements-with-asymmetric-keys.md)專案中所述的技術來加密的 XML 元素。  
...
## <a name="net-framework-security"></a>.NET Framework 安全性  
 ...
 絕對不要直接將金鑰內嵌在您的原始程式碼。  您可以使用[Ildasm （IL](../../../docs/framework/tools/ildasm-exe-il-disassembler.md)解譯器）或在文字編輯器（例如 [記事本]）中開啟元件，輕鬆地從元件中讀取內嵌的索引鍵。  
