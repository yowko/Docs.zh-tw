...
<xref:System.Text.RegularExpressions.Regex>類別本身是具備執行緒安全，且不可變 （唯讀）。 也就是說，您可以在任何執行緒上建立 **Regex** 物件，並在執行緒之間共用；您也可以從任何執行緒呼叫比對方法，而不會變更任何全域狀態。  
  
 不過，結果物件 (**相符**和**MatchCollection**) 所傳回**Regex**應該使用單一執行緒上。 雖然這當中有許多物件在邏輯上是不可變的，但它們的實作可能會延遲某些結果的計算來提升效能，因此，呼叫端必須進行序列化存取。  
