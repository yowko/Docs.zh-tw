...
## <a name="comparing-strings-of-mixed-case"></a>比較混合大小寫的字串  
 若要比較混合大小寫的字串以決定其順序，請使用 <xref:System.String.CompareTo%2A?displayProperty=nameWithType> 參數呼叫 `comparisonType` 方法的其中一個多載，並為 <xref:System.StringComparison.CurrentCultureIgnoreCase?displayProperty=nameWithType> 引數提供 <xref:System.StringComparison.InvariantCultureIgnoreCase?displayProperty=nameWithType>、<xref:System.StringComparison.OrdinalIgnoreCase?displayProperty=nameWithType> 或 `comparisonType` 的值。 若要使用目前文化特性以外的特定文化特性進行比較，請使用 <xref:System.String.CompareTo%2A?displayProperty=nameWithType> 和 `culture` 參數呼叫 `options` 方法的多載，並提供 <xref:System.Globalization.CompareOptions.IgnoreCase?displayProperty=nameWithType> 的值做為 `options` 引數。  
  
 若要比較混合大小寫的字串以決定字串是否相等，請使用 <xref:System.String.Equals%2A?displayProperty=nameWithType> 參數呼叫 `comparisonType` 方法的其中一個多載，並為 <xref:System.StringComparison.CurrentCultureIgnoreCase?displayProperty=nameWithType> 引數提供 <xref:System.StringComparison.InvariantCultureIgnoreCase?displayProperty=nameWithType>、<xref:System.StringComparison.OrdinalIgnoreCase?displayProperty=nameWithType> 或 `comparisonType` 的值。  
...
## <a name="totitlecase"></a>ToTitleCase  
 ...
 <xref:System.Globalization.TextInfo.ToTitleCase%2A?displayProperty=nameWithType> 方法區分文化特性；也就是說，它會使用特定文化特性的大小寫慣例。 若要呼叫方法，請先從特定文化特性的 <xref:System.Globalization.TextInfo> 屬性，擷取代表特定文化特性之大小寫慣例的 <xref:System.Globalization.CultureInfo.TextInfo%2A?displayProperty=nameWithType> 物件。  
