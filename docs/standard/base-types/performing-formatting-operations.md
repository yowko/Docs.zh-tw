---
title: "執行格式化作業"
description: "執行格式化作業"
keywords: ".NET、.NET Core"
author: stevehoag
ms.author: shoag
ms.date: 07/25/2016
ms.topic: article
ms.prod: .net
ms.technology: dotnet-standard
ms.devlang: dotnet
ms.assetid: 982f0b64-7e69-4bbe-a688-0117f86a603c
translationtype: Human Translation
ms.sourcegitcommit: fb00da6505c9edb6a49d2003ae9bcb8e74c11d6c
ms.openlocfilehash: 7cc5f4e68cb944a2684c58af866afe1edd66d43b

---

# <a name="performing-formatting-operations"></a>執行格式化作業

下列主題提供執行特定格式設定作業的逐步指示。

* [如何：使用前置零填補數字](pad-number.md)

* [如何：定義及使用自訂數值格式提供者](define-custom.md)

* [如何：從特定日期擷取星期幾](extract-day.md)

* [如何：反覆存取日期與時間值](roundtrip.md)

* [如何：在日期與時間值中顯示毫秒](display-milliseconds.md)

* [如何：在非西曆中顯示日期](display-dates.md)

## <a name="see-also"></a>另請參閱

[格式化類型](formatting-types.md)



<!--HONumber=Nov16_HO3-->


