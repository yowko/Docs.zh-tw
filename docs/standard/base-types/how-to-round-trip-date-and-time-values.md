...
在許多應用程式中，日期和時間值的用途都是要明確地識別單一時間點。 本文說明如何儲存和還原 <xref:System.DateTime> 值、 <xref:System.DateTimeOffset> 值，以及具有時區資訊的日期和時間值，讓還原的值能夠識別與儲存值相同的時間。

## <a name="round-trip-a-datetime-value"></a>來回行程的日期時間值
...
## <a name="round-trip-a-date-and-time-value-with-its-time-zone"></a>使用時區的日期和時間值進行來回行程
...
- 下列命名空間會使用 c # 指示詞 `using` 或 Visual Basic 語句匯入 `Imports` ：
...
- 除了類別以外的每個程式碼範例 `DateInTimeZone` 都包含在類別或 Visual Basic 模組中、包裝在方法中，並從 `Main` 方法呼叫。
