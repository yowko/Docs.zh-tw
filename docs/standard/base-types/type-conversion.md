...
- `Explicit` 運算子，這個運算子定義類型之間可用的縮小轉換。 如需詳細資訊，請參閱 [使用明確運算子的明確轉換](#explicit-conversion-with-the-explicit-operator) 一節。  
  
- <xref:System.IConvertible>介面，定義每個基底 .net 資料類型的轉換。 如需詳細資訊，請參閱 [IConvertible 介面](#the-iconvertible-interface)一節。  
...
## <a name="explicit-conversion-with-the-explicit-operator"></a>使用明確運算子進行明確轉換
...
|<xref:System.UInt64>|<xref:System.UInt64.MaxValue?displayProperty=nameWithType> 大於 <xref:System.Int32.MaxValue?displayProperty=nameWithType>。|  
 ...
 為了處理縮小轉換，.NET 允許類型定義 `Explicit` 運算子。 然後，個別語言編譯器就可以使用自己的語法來實作這個運算子，也可以呼叫 <xref:System.Convert> 類別的成員來執行轉換。  (如需類別的詳細資訊 <xref:System.Convert> ，請參閱本主題稍後 [的 Convert 類別](#the-convert-class) 。 ) 下列範例說明如何使用語言功能來處理將這些可能超出範圍的整數值明確轉換成 <xref:System.Int32> 值。  
  
 [!code-csharp[Conceptual.Conversion#4](../../../samples/snippets/csharp/VS_Snippets_CLR/conceptual.conversion/cs/explicit1.cs#4)]
 [!code-vb[Conceptual.Conversion#4](../../../samples/snippets/visualbasic/VS_Snippets_CLR/conceptual.conversion/vb/explicit1.vb#4)]  
  
 明確轉換在不同的語言中可能會產生不同的結果，這些結果可能與對應的 <xref:System.Convert> 方法所傳回的值不同。 例如，如果將 <xref:System.Double> 值12.63251 轉換成 <xref:System.Int32> ，則 Visual Basic `CInt` 方法和 .net <xref:System.Convert.ToInt32%28System.Double%29?displayProperty=nameWithType> 方法都會舍入，以傳回13的 <xref:System.Double> 值，但 c # `(int)` 運算子會截斷以傳回 <xref:System.Double> 值12。 同樣地，C# `(int)` 運算子不支援布林值對整數的轉換，但 Visual Basic `CInt` 方法會將 `true` 的值轉換為 -1。 另一方面，<xref:System.Convert.ToInt32%28System.Boolean%29?displayProperty=nameWithType> 方法會將 `true` 的值轉換為 1。  
...
為了支援將任何類型轉換為通用語言執行時間基底類型，.NET 提供了 <xref:System.IConvertible> 介面。 實作類型需要提供下列項目：  
...
> [!NOTE]
> 除了 <xref:System.IConvertible> .net 提供的介面和 <xref:System.Convert> 類別之外，個別語言也可以提供執行轉換的方式。 例如，C# 使用轉型 (Casting) 運算子、Visual Basic 使用編譯器實作的轉換函式，例如 `CType`、`CInt` 和 `DirectCast`。  
  
 在大部分的情況下， <xref:System.IConvertible> 介面的設計是為了支援在 .net 中的基底類型之間進行轉換。 不過，也可以使用自訂類型來實作介面，以支援從該類型轉換至其他自訂類型。 如需詳細資訊，請參閱本主題稍後的 [使用 ChangeType 方法進行自訂轉換](#custom-conversions-with-the-changetype-method) 一節。
...
> 因為 <xref:System.Convert> 類別包含在每一個基底類型之間來回轉換的方法，所以不需要呼叫每一個基底類型的 <xref:System.IConvertible> 明確介面實作。  
 ...
 下列範例說明 <xref:System.Convert?displayProperty=nameWithType> 如何使用類別，在 .net 基底類型之間執行數個擴展和縮小轉換。  
...
.NET 也可讓您藉由擴充類別來定義自訂類型的類型轉換器 <xref:System.ComponentModel.TypeConverter?displayProperty=nameWithType> ，並透過屬性將類型轉換器與型別產生關聯 <xref:System.ComponentModel.TypeConverterAttribute?displayProperty=nameWithType> 。 下表特別強調這種作法與實作自訂類型之 <xref:System.IConvertible> 介面的差異。  
