---
title: "如何：以前置字元零來填補數字 | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
ms.prod: ".net"
ms.reviewer: ""
...
2.  決定要將整數顯示為十進位值或十六進位值。  
    ...
    -   若要將整數顯示為十進位值，請呼叫其 `ToString(String)` 方法，然後傳遞字串 "D*n*" 以做為 `format` 參數的值，其中 *n* 代表字串的長度下限。  
  
    -   若要將整數顯示為十六進位值，請呼叫其 `ToString(String)` 方法，然後傳遞字串 "X*n*" 以做為 `format` 參數的值，其中 *n* 代表字串的長度下限。  
  
     您也可以在使用[複合格式的方法](../../../docs/standard/base-types/composite-formatting.md) \(例如 <xref:System.String.Format%2A> 或 <xref:System.Console.WriteLine%2A>\) 中使用格式字串。  
