---
title: "規則運算式中的字元類別 | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
ms.prod: ".net"
ms.reviewer: ""
...
---
# 規則運算式中的字元類別
字元類別會定義一組字元，其中任何字元都可在輸入字串中出現，以便讓比對成功。  .NET Framework 中的規則運算式語言支援下列字元類別：  
...
-   非十進位數字。  輸入字串中的字元可以是 Unicode 十進位數字以外的任何字元。  如需詳細資訊，請參閱[十進位數字字元](#NonDigitCharacter)。  
 ...
 .NET Framework 支援字元類別減法運算式，可讓您將一組字元定義為從某個字元類別中排除另一個字元類別的結果。  如需詳細資訊，請參閱[字元類別減法](#CharacterClassSubtraction)。  
...
## 正字元群組：\[ \]  
 ...
 其中 *character\_group* 是為了讓比對成功而可以出現在輸入字串中的個別字元清單。  *character\_group* 可以由一個或多個常值字元、[逸出字元](../../../docs/standard/base-types/character-escapes-in-regular-expressions.md)或字元類別的任何組合所構成。  
...
## 負字元群組：\[^\]  
 ...
 其中 *character\_group* 是為了讓比對成功而不可出現在輸入字串中的個別字元清單。  *character\_group* 可以由一個或多個常值字元、[逸出字元](../../../docs/standard/base-types/character-escapes-in-regular-expressions.md)或字元類別的任何組合所構成。  
...
-   如果 <xref:System.Text.RegularExpressions.RegexOptions?displayProperty=fullName> 選項修改了規則運算式模式，或是 `.` 選項修改了模式中包含 `s` 字元類別的部分，`.` 就會符合任何字元。  如需詳細資訊，請參閱[規則運算式選項](../../../docs/standard/base-types/regular-expression-options.md)。  
  
     下列範例將示範 `.` 字元類別的預設行為與使用 <xref:System.Text.RegularExpressions.RegexOptions?displayProperty=fullName> 選項的行為有何不同。  規則運算式 `^.+` 會從字串開頭開始，比對每一個字元。  根據預設，比對會在第一行結尾結束。規則運算式模式會比對歸位字元 `\r` 或 \\u000D，但不會比對 `\n`。  由於 <xref:System.Text.RegularExpressions.RegexOptions?displayProperty=fullName> 選項會將整個輸入字串解譯為單行，因此它會比對輸入字串中的每個字元，包括 `\n`。  
...
## Unicode 類別或 Unicode 區塊：\\p{}  
 ...
 比對屬於 Unicode 一般分類或具名區塊的任何字元，其中*名稱*是分類縮寫或是具名區塊名稱。  如需分類縮寫的清單，請參閱本主題後段的[支援的 Unicode 一般分類](#SupportedUnicodeGeneralCategories)一節。  如需具名區塊的清單，請參閱本主題後段的[支援的具名區塊](#SupportedNamedBlocks)一節。  
  
 下列範例會使用 `\p{`*名稱*`}` 建構同時比對 Unicode 一般類別 \(在這個案例中是 `Pd`，或稱 Punctuation,Dash 分類\) 以及具名區塊 \(`IsGreek` 和 `IsBasicLatin` 具名區塊\)。  
...
## 負 Unicode 分類或 Unicode 區塊：\\P{}  
 ...
 比對任何不屬於 Unicode 一般分類或具名區塊的字元，其中 *￼名稱* 是分類縮寫或是具名區塊名稱。  如需分類縮寫的清單，請參閱本主題後段的[支援的 Unicode 一般分類](#SupportedUnicodeGeneralCategories)一節。  如需具名區塊的清單，請參閱本主題後段的[支援的具名區塊](#SupportedNamedBlocks)一節。  
  
 下列範例會使用 `\P{`*名稱*`}` 建構從數值字串中移除任何貨幣符號 \(在這個案例中是 `Sc`，或稱 \[符號、貨幣\] 分類\)。  
...
|Pc|標點符號、連接器。  這個分類包含十個字元，其中最常用的是 LOWLINE 字元 \(\_\)，u\+005F。|  
 ...
 如果指定了符合 ECMAScript 的行為，`\w` 就等於 `[a-zA-Z_0-9]`。  如需 ECMAScript 規則運算式的詳細資訊，請參閱[規則運算式選項](../../../docs/standard/base-types/regular-expression-options.md)中的＜ECMAScript 相符行為＞一節。  
...
|\(\\W\){1,2}|比對一次或兩次非文字字元。  這是第二個擷取群組。|  
 ...
 由於第二個擷取群組的 <xref:System.Text.RegularExpressions.Group> 物件只包含單一擷取的非文字字元，因此這個範例會從 <xref:System.Text.RegularExpressions.CaptureCollection> 屬性所傳回之 <xref:System.Text.RegularExpressions.Group.Captures%2A?displayProperty=fullName> 物件擷取所有擷取的非文字字元。  
...
|`\p{Z}`|比對任何分隔符號字元。|  
 ...
 如果指定了符合 ECMAScript 的行為，`\s` 就等於 `[\f\n\r\t\v]`。  如需 ECMAScript 規則運算式的詳細資訊，請參閱[規則運算式選項](../../../docs/standard/base-types/regular-expression-options.md)中的＜ECMAScript 相符行為＞一節。  
...
## 十進位數字字元：\\d  
 ...
 如果指定了符合 ECMAScript 的行為，`\d` 就等於 `[0-9]`。  如需 ECMAScript 規則運算式的詳細資訊，請參閱[規則運算式選項](../../../docs/standard/base-types/regular-expression-options.md)中的＜ECMAScript 相符行為＞一節。  
...
## 支援的 Unicode 一般分類  
 Unicode 定義了下表中所列的一般類別。  如需詳細資訊，請參閱 [Unicode 字元資料庫](http://go.microsoft.com/fwlink/?LinkId=57650)中的＜UCD 檔案格式＞和＜一般分類值＞副主題。  
...
## 字元類別減法：\[base\_group \- \[excluded\_group\]\]  
 ...
 方括號 \(`[]`\) 和連字號 \(`-`\) 為必要。  *base\_group* 是[正字元群組](#PositiveGroup)或[負字元群組](#NegativeGroup)。  *excluded\_group* 元件是另一個正字元群組或負字元群組，或者是另一個字元類別減法運算式 \(也就是說，您可以將字元類別減法運算式設為巢狀\)。  
