...
|逸出的字元|說明|模式|符合|  
|-----------------------|-----------------|-------------|-------------|  
|`\a`|比對警示字元 \u0007。|`\a`|`"\u0007"` (英文) 中的`"Error!" + '\u0007'` (英文)|  
|`\b`|在字元類別中，比對退格鍵 \u0008。|`[\b]{3,}`|`"\b\b\b\b"` (英文) 中的`"\b\b\b\b"` (英文)|  
|`\t`|比對定位點 \u0009。|`(\w+)\t`|`"item1\titem2\t"` 中的 `"item1\t"`、`"item2\t"`|  
|`\r`|比對歸位字元 \u000D (`\r` 不等於新行字元 `\n`。)|`\r\n(\w+)`|`"\r\nThese"` (英文) 中的`"\r\nThese are\ntwo lines."` (英文)|  
|`\v`|比對垂直定位點 \u000B。|`[\v]{2,}`|`"\v\v\v"` (英文) 中的`"\v\v\v"` (英文)|  
|`\f`|比對換頁字元 \u000C。|`[\f]{2,}`|`"\f\f\f"` (英文) 中的`"\f\f\f"` (英文)|  
|`\n`|比對新行字元 \u000A。|`\r\n(\w+)`|`"\r\nThese"` (英文) 中的`"\r\nThese are\ntwo lines."` (英文)|  
|`\e`|比對逸出字元 \u001B。|`\e`|`"\x001B"` (英文) 中的`"\x001B"` (英文)|  
...
|字元類別|說明|模式|符合|  
|---------------------|-----------------|-------------|-------------|  
|`[` *character_group* `]`|比對 *character_group*中的任何單一字元。 根據預設，比對會區分大小寫。|`[ae]`|`"a"` (英文) 中的`"gray"` (英文)<br /><br /> `"lane"` 中的 `"a"`、`"e"`|  
...
|`.`|萬用字元：比對除 \n 以外的任何單一字元。<br /><br /> 若要比對常值句號字元 (. 或 `\u002E`)，您必須在前面加上逸出字元 (`\.`)。|`a.e`|`"ave"` (英文) 中的`"nave"` (英文)<br /><br /> `"ate"` (英文) 中的`"water"` (英文)|  
...
|`\s`|比對任何泛空白字元。|`\w\s`|`"D "` (英文) 中的`"ID A1.3"` (英文)|  
|`\S`|比對任何非泛空白字元。|`\s\S`|`" _"` (英文) 中的`"int __ctr"` (英文)|  
|`\d`|比對任何十進位數字。|`\d`|`"4"` (英文) 中的`"4 = IV"` (英文)|  
...
|判斷提示|說明|模式|符合|  
|---------------|-----------------|-------------|-------------|  
|`^`|根據預設，比對必須在字串的開頭開始；在多行模式中，它必須在一行的開頭開始。|`^\d{3}`|`"901"` (英文) 中的`"901-333-"` (英文)|  
|`$`|根據預設，比對必須發生在字串的結尾或字串結尾的 `\n` 之前；在多行模式中，它必須發生在一行的結尾之前，或一行結尾的 `\n` 之前。|`-\d{3}$`|`"-333"` (英文) 中的`"-901-333"` (英文)|  
|`\A`|比對必須發生在字串開頭。|`\A\d{3}`|`"901"` (英文) 中的`"901-333-"` (英文)|  
|`\Z`|比對必須發生在字串結尾，或發生在字串結尾的 `\n` 之前。|`-\d{3}\Z`|`"-333"` (英文) 中的`"-901-333"` (英文)|  
|`\z`|比對必須發生在字串結尾。|`-\d{3}\z`|`"-333"` (英文) 中的`"-901-333"` (英文)|  
...
|分組建構|說明|模式|符合|  
|------------------------|-----------------|-------------|-------------|  
|`(` *子運算式* `)`|擷取符合的子運算式，並指派以一為起始的序號給它。|`(\w)\1`|`"ee"` (英文) 中的`"deep"` (英文)|  
|`(?<` *名稱* `>` *子運算式* `)`|將符合的子運算式擷取到具名群組中。|`(?<double>\w)\k<double>`|`"ee"` (英文) 中的`"deep"` (英文)|  
|`(?<` *名稱1* `-` *名稱2* `>` *子運算式* `)`|定義對稱群組定義。 如需詳細資訊，請參閱[分組建構](grouping-constructs-in-regular-expressions.md)中的＜對稱群組定義＞一節。|`(((?'Open'\()[^\(\)]*)+((?'Close-Open'\))[^\(\)]*)+)*(?(Open)(?!))$`|`"((1-3)*(3-1))"` (英文) 中的`"3+2^((1-3)*(3-1))"` (英文)|  
|`(?:` *子運算式* `)`|定義非擷取型群組。|`Write(?:Line)?`|`"WriteLine"` (英文) 中的`"Console.WriteLine()"` (英文)<br /><br /> `"Write"` (英文) 中的`"Console.Write(value)"` (英文)|  
|`(?imnsx-imnsx:` *子運算式* `)`|套用或停用 *子運算式*內指定的選項。 如需詳細資訊，請參閱[規則運算式選項](regular-expression-options.md)。|`A\d{2}(?i:\w+)\b`|`"A12xl A12XL a12xl"` 中的 `"A12xl"`、`"A12XL"`|  
...
|`{` *n* `,` *m* `}`|比對上一個項目至少 *n* 次，但不超過 *m* 次。|`"\d{3,5}"`|`"166"`、 `"17668"`<br /><br /> `"19302"` (英文) 中的`"193024"` (英文)|  
...
|反向參考建構|說明|模式|符合|  
|-----------------------------|-----------------|-------------|-------------|  
|`\` *數字*|反向參考。 比對編號子運算式的值。|`(\w)\1`|`"ee"` (英文) 中的`"seek"` (英文)|  
|`\k<` *name* `>`|命名的反向參考。 比對具名運算式的值。|`(?<char>\w)\k<char>`|`"ee"` (英文) 中的`"seek"` (英文)|  
...
|`(?(` *運算式* `)` *是* <code>&#124;</code> *no* `)`|如果 *expression* 所指定的規則運算式模式相符，則比對 *yes* ，否則比對選擇性的 *no* 部分。 *expression* 會解譯為零寬度判斷提示。|<code>(?(A)A\d{2}\b&#124;\b\d{3}\b)</code>|`"A10 C103 910"` 中的 `"A10"`、`"910"`|  
|`(?(` *name* `)` *是* <code>&#124;</code> *no* `)`|如果 *name* (具名或編號擷取群組) 有相符項目，則比對 *yes*，否則比對選擇性的 *no*。|<code>(?&lt;quoted&gt;&quot;)?(?(quoted).+?&quot;&#124;\S+\s)</code>|`"Dogs.jpg \"Yiska playing.jpg\""` 中的 `"Dogs.jpg "`、`"\"Yiska playing.jpg\""`|  
...
-   使用[分組建構](grouping-constructs-in-regular-expressions.md)`(?imnsx-imnsx:`*子運算式*`)`，只針對指定的群組定義選項。  
...
|`m`|使用多行模式。 `^` 和 `$` 符合行的開頭與結尾，而不是字串的開始和結尾。|如需範例，請參閱[規則運算式選項](regular-expression-options.md)中的＜多行模式＞一節。||  
...
|建構|定義|範例|  
|---------------|----------------|-------------|  
|`(?imnsx-imnsx)`|在模式中間設定或停用選項，例如不區分大小寫。如需詳細資訊，請參閱[規則運算式選項](regular-expression-options.md)。|`\bA(?i)b\w+\b` 符合 `"ABA Able Act"` 中的 `"ABA"`、`"Able"`|  
