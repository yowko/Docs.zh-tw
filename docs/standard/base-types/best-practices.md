...
下列範例示範如何建立內含編譯的規則運算式的組件。 範例中會建立名為 `RegexLib.dll` 且內含單一規則運算式類別 `SentencePattern` 的組件，其中包含[解譯與編譯的規則運算式](#interpreted-vs-compiled-regular-expressions)一節中使用的句子比對規則運算式模式。
