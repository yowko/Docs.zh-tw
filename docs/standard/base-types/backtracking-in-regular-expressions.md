...
### <a name="atomic-groups"></a>不可部分完成的群組
 `(?>`*子運算式*`)` language 元素會隱藏回溯至子運算式。 一旦成功符合後，就不會將它的任何部分對應到後續的回溯。 例如，在模式 `(?>\w*\d*)1`中，如果無法比對 `1`，`\d*` 將不會放棄其任何相符項，即使這表示它會允許 `1` 成功比對。 不可部分完成的群組有助於防止與失敗相符的相關效能問題。
