...
.NET Core 可為新的應用程式和應用程式模式提供顯著的優點。 不過，.NET Framework 會持續成為許多現有案例的自然選擇，因此在所有伺服器應用程式中，.NET Framework 不會被 .NET Core 取代。
...
如需 .NET Framework ASP.NET Core 支援的相關資訊，請參閱 [.Net Core 支援原則](https://dotnet.microsoft.com/platform/support/policy/dotnet-core)。

### <a name="third-party-libraries-or-nuget-packages-not-available-for-net-core"></a>適用于 .NET Core 的協力廠商程式庫或 NuGet 套件不適用

程式庫會快速採用 .NET Standard。 .NET Standard 可讓您跨所有 .NET 執行（包括 .NET Core）共用程式碼。 透過 .NET Standard 2.0，甚至會更容易達成：
...
只有當程式庫或 NuGet 套件使用 .NET Standard 或 .NET Core 中無法使用的技術時，才需要使用 .NET Framework。

### <a name="net-technologies-not-available-for-net-core"></a>適用于 .NET Core 的 .NET 技術不適用
...
- ASP.NET Web Forms 的應用程式： ASP.NET Web Forms 僅適用于 .NET Framework。 ASP.NET Core 無法用於 ASP.NET Web Forms。 目前並未規劃將 ASP.NET Web Forms 帶入 .NET Core。

- ASP.NET Web Pages 應用程式：ASP.NET Web Pages 未隨附於 ASP.NET Core 中。

- WCF 服務實作。 即使有 [wcf 用戶端程式庫](https://github.com/dotnet/wcf) 可從 .net CORE 取用 wcf 服務，但 wcf 伺服器的執行目前只在 .NET Framework 中提供。 此案例不是 .NET Core 目前方案的一部分，但會被視為未來的方案。

- 工作流程相關的服務： Windows Workflow Foundation (WF) 、工作流程服務 (單一服務) 中的 WCF + WF，而 WCF Data Services (之前稱為「ADO.NET 資料服務」 ) 僅適用于 .NET Framework。 沒有計劃將這些技術帶入 .NET Core。
