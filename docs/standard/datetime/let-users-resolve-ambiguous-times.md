...
2. 呼叫 <xref:System.TimeZoneInfo.IsAmbiguousTime%2A> 方法，以判斷時間是否不明確。

3. 如果時間不明確，請呼叫 <xref:System.TimeZoneInfo.GetAmbiguousTimeOffsets%2A> 方法，以取出 <xref:System.TimeSpan> 物件的陣列。 陣列中的每個元素都包含不明確的時間可以對應的 UTC 位移。

4. 讓使用者選取想要的位移。

5. 將當地時間減去使用者所選取的位移，以取得 UTC 日期和時間。

6. 呼叫 `static` （Visual Basic .NET 中的`Shared`） <xref:System.DateTime.SpecifyKind%2A> 方法，將 UTC 日期和時間值的 <xref:System.DateTime.Kind%2A> 屬性設定為 [<xref:System.DateTimeKind.Utc?displayProperty=nameWithType>]。
...
範例程式碼的核心會使用 <xref:System.TimeSpan> 物件的陣列，以指出不明確時間與 UTC 的可能位移。 不過，這些位移不可能對使用者具有任何意義。 若要釐清位移的意義，程式碼也會註明位移是否代表當地時區的標準時間或其日光節約時間。 程式碼會藉由將位移與 <xref:System.TimeZoneInfo.BaseUtcOffset%2A> 屬性的值進行比較，來判斷哪一個時間是標準的，哪個時間是日光。 這個屬性指出 UTC 與時區標準時間的差異。
