...
* 若要從.NET 外部來源中擷取日期和時間資訊，通常其中的日期和時間資訊會儲存在簡單資料類型。
...
包含.NET <xref:System.DateTime>， <xref:System.DateTimeOffset>， <xref:System.TimeSpan>，和<xref:System.TimeZoneInfo>全部都是可以用來建置使用日期和時間的應用程式的類型。
...
<xref:System.DateTime> 值會定義特定的日期和時間。 它包含<xref:System.DateTime.Kind%2A>屬性，可提供有限的時區資訊該日期和時間所屬之。 <xref:System.DateTimeKind>所傳回的值<xref:System.DateTime.Kind%2A>屬性會指出是否<xref:System.DateTime>值代表當地時間 (<xref:System.DateTimeKind.Local?displayProperty=nameWithType>)、 國際標準時間 (UTC) (<xref:System.DateTimeKind.Utc?displayProperty=nameWithType>)，或未指定的時間 (<xref:System.DateTimeKind.Unspecified?displayProperty=nameWithType>)。
...
* 從外部.NET 中，例如 SQL 資料庫中的來源中擷取日期和時間資訊。 一般而言，這些來源會將日期和時間資訊以和 <xref:System.DateTime> 結構相容的簡單格式儲存。
...
<xref:System.DateTimeOffset> 類型包含 <xref:System.DateTime> 類型的所有功能並感知時區。 這使其適用於應用程式，執行下列作業：
...
* 測量已耗用時間。 例如，<xref:System.Diagnostics.Stopwatch.Elapsed%2A?displayProperty=nameWithType>屬性會傳回<xref:System.TimeSpan>值，反映呼叫其中一種之後已耗用的時間間隔<xref:System.Diagnostics.Stopwatch>開始測量已耗用時間的方法。

若在未參考一天中之特定時間的情況下反映時間，則 <xref:System.TimeSpan> 值也可用來取代 <xref:System.DateTime> 值。 這種使用方式很類似<xref:System.DateTime.TimeOfDay%2A?displayProperty=nameWithType>和<xref:System.DateTimeOffset.TimeOfDay%2A?displayProperty=nameWithType>屬性，傳回<xref:System.TimeSpan>表示不參考日期時間值。 例如， <xref:System.TimeSpan> 結構可用來反映商店每日開始營業或打烊的時間，或可用來代表任何有規律事件發生的時間。
...
在某些情況下，欲充分利用 <xref:System.TimeZoneInfo> 類別可能需要進一步的開發工作。 如果日期和時間值未緊密結合，它們屬於，進一步的作業的時區是必要項目。 除非您的應用程式提供的日期和時間與相關聯時區連結的機制，很容易特定日期和時間值與其時區解除關聯。 連結此資訊的一種方法是定義類別或結構，其中包含日期和時間值，以及與其相關聯的時區物件。
