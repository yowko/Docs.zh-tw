...
本主題說明如何建立沒有調整規則的時區。 若要建立支援日光節約時間調整規則的時區，請參閱 how [to：建立具有調整規則的時區](../../../docs/standard/datetime/create-time-zones-with-adjustment-rules.md)。
...
4. 具現化可定義時區與 UTC 時差的 <xref:System.TimeSpan> 物件。 時間與 UTC 時間較晚的時區具有正位移。 具有早于 UTC 時間的時區具有負位移。
