...
### <a name="debugging-with-sos"></a><span data-ttu-id="c3f9a-113">以 SOS 偵錯</span><span class="sxs-lookup"><span data-stu-id="c3f9a-113">Debugging with SOS</span></span>
...
- [<span data-ttu-id="c3f9a-145">層代 0 太大</span><span class="sxs-lookup"><span data-stu-id="c3f9a-145">Generation 0 is too big</span></span>](#Issue_Gen0)
...
<span data-ttu-id="c3f9a-164">如果您判斷問題是 Managed 堆積所導致，您必須在一段時間內測量 Managed 堆積，以判斷任何模式。</span><span class="sxs-lookup"><span data-stu-id="c3f9a-164">If you determine that the issue is caused by the managed heap, you must measure the managed heap over time to determine any patterns.</span></span>
...
<span data-ttu-id="c3f9a-185">分散一律發生在大型物件堆積中，因為它不會壓縮。</span><span class="sxs-lookup"><span data-stu-id="c3f9a-185">Fragmentation always occurs in the large object heap because it is not compacted.</span></span> <span data-ttu-id="c3f9a-186">相鄰的可用物件自然而然會聚攏成為單一空間，以滿足大型物件配置要求。</span><span class="sxs-lookup"><span data-stu-id="c3f9a-186">Free objects that are adjacent are naturally collapsed into a single space to satisfy large object allocation requests.</span></span>
...
- [<span data-ttu-id="c3f9a-265">判斷 Managed 堆積保留的記憶體數量。</span><span class="sxs-lookup"><span data-stu-id="c3f9a-265">Determine how much memory the managed heap reserves.</span></span>](#ManagedHeapReserve)
...
- <span data-ttu-id="c3f9a-321">使用 `# Total reserved bytes` 記憶體效能計數器。</span><span class="sxs-lookup"><span data-stu-id="c3f9a-321">Use the `# Total reserved bytes` memory performance counter.</span></span>
...
- <span data-ttu-id="c3f9a-325">在已載入 SOS 偵錯工具擴充功能的 WinDbg 或 Visual Studio 偵錯工具中，輸入下列命令：</span><span class="sxs-lookup"><span data-stu-id="c3f9a-325">In the WinDbg or Visual Studio debugger with the SOS debugger extension loaded, type the following command:</span></span>
  ...
  <span data-ttu-id="c3f9a-328">「區段」所指定的位址是區段的起始位址。</span><span class="sxs-lookup"><span data-stu-id="c3f9a-328">The addresses indicated by "segment" are the starting addresses of the segments.</span></span>
...
2. <span data-ttu-id="c3f9a-363">若要取得執行緒的輸出，請輸入下列命令：</span><span class="sxs-lookup"><span data-stu-id="c3f9a-363">To get an output of threads, type the following command:</span></span>
...
- <span data-ttu-id="c3f9a-373">若要判斷層代 0 中的可用空間，請輸入下列命令以取得依層代的記憶體耗用量資訊：</span><span class="sxs-lookup"><span data-stu-id="c3f9a-373">To determine the free space in generation 0, type the following command for memory consumption information by generation:</span></span>
  ...
  <span data-ttu-id="c3f9a-374">**!eeheap -gc**</span><span class="sxs-lookup"><span data-stu-id="c3f9a-374">**!eeheap -gc**</span></span>
...
- <span data-ttu-id="c3f9a-406">或者，您可以使用[記憶體回收 ETW 事件](../../framework/performance/garbage-collection-etw-events.md)來判斷記憶體回收的長度，並分析資訊來判斷記憶體回收的持續時間。</span><span class="sxs-lookup"><span data-stu-id="c3f9a-406">Alternatively, you can determine the length of a garbage collection by using [garbage collection ETW events](../../framework/performance/garbage-collection-etw-events.md), and analyze the information to determine the duration of garbage collection.</span></span>

  <span data-ttu-id="c3f9a-407">例如，下列資料顯示在非並行記憶體回收期間發生的事件序列。</span><span class="sxs-lookup"><span data-stu-id="c3f9a-407">For example, the following data shows an event sequence that occurred during a non-concurrent garbage collection.</span></span>
  ...
  <span data-ttu-id="c3f9a-408">暫止 Managed 執行緒花了 26 微秒 (`GCSuspendEEEnd` – `GCSuspendEEBegin_V1`)。</span><span class="sxs-lookup"><span data-stu-id="c3f9a-408">Suspending the managed thread took 26us (`GCSuspendEEEnd` – `GCSuspendEEBegin_V1`).</span></span>

  <span data-ttu-id="c3f9a-409">實際的記憶體回收花了 4.8 毫秒 (`GCEnd_V1` – `GCStart_V1`)。</span><span class="sxs-lookup"><span data-stu-id="c3f9a-409">The actual garbage collection took 4.8ms (`GCEnd_V1` – `GCStart_V1`).</span></span>

  <span data-ttu-id="c3f9a-410">繼續 Managed 執行緒花了 21 微秒 (`GCRestartEEEnd` – `GCRestartEEBegin`)。</span><span class="sxs-lookup"><span data-stu-id="c3f9a-410">Resuming the managed threads took 21us (`GCRestartEEEnd` – `GCRestartEEBegin`).</span></span>

  <span data-ttu-id="c3f9a-411">下列輸出提供背景記憶體回收的範例，並包含處理序、執行緒和事件欄位。</span><span class="sxs-lookup"><span data-stu-id="c3f9a-411">The following output provides an example for background garbage collection, and includes the process, thread, and event fields.</span></span> <span data-ttu-id="c3f9a-412">(並非所有資料皆會顯示)。</span><span class="sxs-lookup"><span data-stu-id="c3f9a-412">(Not all data is shown.)</span></span>
  ...
  <span data-ttu-id="c3f9a-413">在 42504816 的 `GCStart_V1` 事件表示這是背景記憶體回收，因為最後一個欄位是 `1`。</span><span class="sxs-lookup"><span data-stu-id="c3f9a-413">The `GCStart_V1` event at 42504816 indicates that this is a background garbage collection, because the last field is `1`.</span></span> <span data-ttu-id="c3f9a-414">這會變成記憶體回收第 </span><span class="sxs-lookup"><span data-stu-id="c3f9a-414">This becomes garbage collection No.</span></span> <span data-ttu-id="c3f9a-415">102019 號。</span><span class="sxs-lookup"><span data-stu-id="c3f9a-415">102019.</span></span>

  <span data-ttu-id="c3f9a-416">因為需要先進行暫時記憶體回收，才能開始背景記憶體回收，因此發生 `GCStart` 事件。</span><span class="sxs-lookup"><span data-stu-id="c3f9a-416">The `GCStart` event occurs because there is a need for an ephemeral garbage collection before you start a background garbage collection.</span></span> <span data-ttu-id="c3f9a-417">這會變成記憶體回收第 </span><span class="sxs-lookup"><span data-stu-id="c3f9a-417">This becomes garbage collection No.</span></span> <span data-ttu-id="c3f9a-418">102020 號。</span><span class="sxs-lookup"><span data-stu-id="c3f9a-418">102020.</span></span>

  <span data-ttu-id="c3f9a-419">在 42514170 處，記憶體回收第</span><span class="sxs-lookup"><span data-stu-id="c3f9a-419">At 42514170, garbage collection No.</span></span> <span data-ttu-id="c3f9a-420">102020 號完成。</span><span class="sxs-lookup"><span data-stu-id="c3f9a-420">102020 finishes.</span></span> <span data-ttu-id="c3f9a-421">Managed 執行緒會在此時重新啟動。</span><span class="sxs-lookup"><span data-stu-id="c3f9a-421">The managed threads are restarted at this point.</span></span> <span data-ttu-id="c3f9a-422">此步驟會在執行緒 4372，觸發此背景記憶體回收完成。</span><span class="sxs-lookup"><span data-stu-id="c3f9a-422">This is completed on thread 4372, which triggered this background garbage collection.</span></span>

  <span data-ttu-id="c3f9a-423">在執行緒 4744 上發生擱置。</span><span class="sxs-lookup"><span data-stu-id="c3f9a-423">On thread 4744, a suspension occurs.</span></span> <span data-ttu-id="c3f9a-424">這是背景記憶體回收唯一必須暫止 Managed 執行緒的時間。</span><span class="sxs-lookup"><span data-stu-id="c3f9a-424">This is the only time at which the background garbage collection has to suspend managed threads.</span></span> <span data-ttu-id="c3f9a-425">這段期間是大約 99 毫秒 ((63784407-63685394)/1000)。</span><span class="sxs-lookup"><span data-stu-id="c3f9a-425">This duration is approximately 99ms ((63784407-63685394)/1000).</span></span>

  <span data-ttu-id="c3f9a-426">背景記憶體回收的 `GCEnd` 事件位於 89931423。</span><span class="sxs-lookup"><span data-stu-id="c3f9a-426">The `GCEnd` event for the background garbage collection is at 89931423.</span></span> <span data-ttu-id="c3f9a-427">這表示背景記憶體回收持續大約 47 秒 ((89931423-42504816)/1000)。</span><span class="sxs-lookup"><span data-stu-id="c3f9a-427">This means that the background garbage collection lasted for about 47seconds ((89931423-42504816)/1000).</span></span>

  <span data-ttu-id="c3f9a-428">在 Managed 執行緒執行時，您可能看到發生任意數目的暫時記憶體回收。</span><span class="sxs-lookup"><span data-stu-id="c3f9a-428">While the managed threads are running, you can see any number of ephemeral garbage collections occurring.</span></span>

<a name="Triggered"></a>

### <a name="to-determine-what-triggered-a-garbage-collection"></a><span data-ttu-id="c3f9a-429">判斷觸發記憶體回收的原因</span><span class="sxs-lookup"><span data-stu-id="c3f9a-429">To determine what triggered a garbage collection</span></span>

- <span data-ttu-id="c3f9a-430">在已載入 SOS 偵錯工具擴充功能的 WinDbg 或 Visual Studio 偵錯工具中，輸入下列命令以顯示所有執行緒及其呼叫堆疊：</span><span class="sxs-lookup"><span data-stu-id="c3f9a-430">In the WinDbg or Visual Studio debugger with the SOS debugger extension loaded, type the following command to show all the threads with their call stacks:</span></span>

  <span data-ttu-id="c3f9a-431">**~\*文庫**</span><span class="sxs-lookup"><span data-stu-id="c3f9a-431">**~\*kb**</span></span>

  <span data-ttu-id="c3f9a-432">這個命令會顯示與下列類似的輸出。</span><span class="sxs-lookup"><span data-stu-id="c3f9a-432">This command displays output similar to the following.</span></span>
  ...
  <span data-ttu-id="c3f9a-433">如果記憶體回收的起因是來自作業系統的記憶體不足通知，則呼叫堆疊會很類似，只除了執行緒是完成項執行緒。</span><span class="sxs-lookup"><span data-stu-id="c3f9a-433">If the garbage collection was caused by a low memory notification from the operating system, the call stack is similar, except that the thread is the finalizer thread.</span></span> <span data-ttu-id="c3f9a-434">完成項執行緒會收到非同步記憶體偏低通知，並引發記憶體回收。</span><span class="sxs-lookup"><span data-stu-id="c3f9a-434">The finalizer thread gets an asynchronous low memory notification and induces the garbage collection.</span></span>

  <span data-ttu-id="c3f9a-435">如果記憶體回收的起因是記憶體配置，則堆疊會如下所示：</span><span class="sxs-lookup"><span data-stu-id="c3f9a-435">If the garbage collection was caused by memory allocation, the stack appears as follows:</span></span>
  ...
  <span data-ttu-id="c3f9a-436">Just-In-Time 協助程式 (`JIT_New*`) 實際上呼叫 `GCHeap::GarbageCollectGeneration`。</span><span class="sxs-lookup"><span data-stu-id="c3f9a-436">A just-in-time helper (`JIT_New*`) eventually calls `GCHeap::GarbageCollectGeneration`.</span></span> <span data-ttu-id="c3f9a-437">如果您判斷層代 2 記憶體回收起因是配置，則必須判斷層代 2 記憶體回收會回收哪些物件及如何加以避免。</span><span class="sxs-lookup"><span data-stu-id="c3f9a-437">If you determine that generation 2 garbage collections are caused by allocations, you must determine which objects are collected by a generation 2 garbage collection and how to avoid them.</span></span> <span data-ttu-id="c3f9a-438">也就是您想要判斷層代 2 記憶體回收開始和結束之間的差異，以及造成層代 2 記憶體回收的物件。</span><span class="sxs-lookup"><span data-stu-id="c3f9a-438">That is, you want to determine the difference between the start and the end of a generation 2 garbage collection, and the objects that caused the generation 2 collection.</span></span>

  <span data-ttu-id="c3f9a-439">例如，在偵錯工具中輸入下列命令以顯示層代 2 記憶體回收的開始：</span><span class="sxs-lookup"><span data-stu-id="c3f9a-439">For example, type the following command in the debugger to show the beginning of a generation 2 collection:</span></span>

  <span data-ttu-id="c3f9a-440">**!dumpheap –stat**</span><span class="sxs-lookup"><span data-stu-id="c3f9a-440">**!dumpheap –stat**</span></span>

  <span data-ttu-id="c3f9a-441">範例輸出 (經過刪減以顯示使用最大空間的物件)：</span><span class="sxs-lookup"><span data-stu-id="c3f9a-441">Example output (abridged to show the objects that use the most space):</span></span>
  ...
  <span data-ttu-id="c3f9a-442">在層代 2 結束時重複執行命令：</span><span class="sxs-lookup"><span data-stu-id="c3f9a-442">Repeat the command at the end of generation 2:</span></span>

  <span data-ttu-id="c3f9a-443">**!dumpheap –stat**</span><span class="sxs-lookup"><span data-stu-id="c3f9a-443">**!dumpheap –stat**</span></span>

  <span data-ttu-id="c3f9a-444">範例輸出 (經過刪減以顯示使用最大空間的物件)：</span><span class="sxs-lookup"><span data-stu-id="c3f9a-444">Example output (abridged to show the objects that use the most space):</span></span>
  ...
  <span data-ttu-id="c3f9a-445">`double[]` 物件從輸出結束消失，這表示它們已被回收。</span><span class="sxs-lookup"><span data-stu-id="c3f9a-445">The `double[]` objects disappeared from the end of the output, which means that they were collected.</span></span> <span data-ttu-id="c3f9a-446">這些物件大約 70 MB。</span><span class="sxs-lookup"><span data-stu-id="c3f9a-446">These objects account for approximately 70 MB.</span></span> <span data-ttu-id="c3f9a-447">剩餘的物件變更不多。</span><span class="sxs-lookup"><span data-stu-id="c3f9a-447">The remaining objects did not change much.</span></span> <span data-ttu-id="c3f9a-448">因此，這些 `double[]` 物件便是這個層代 2 記憶體回收發生的原因。</span><span class="sxs-lookup"><span data-stu-id="c3f9a-448">Therefore, these `double[]` objects were the reason why this generation 2 garbage collection occurred.</span></span> <span data-ttu-id="c3f9a-449">您的下一個步驟是判斷 `double[]` 物件為何出現，以及其終止原因。</span><span class="sxs-lookup"><span data-stu-id="c3f9a-449">Your next step is to determine why the `double[]` objects are there and why they died.</span></span> <span data-ttu-id="c3f9a-450">您可以要求程式碼開發人員這些物件的來源，或者您可以使用 **gcroot** 命令。</span><span class="sxs-lookup"><span data-stu-id="c3f9a-450">You can ask the code developer where these objects came from, or you can use the **gcroot** command.</span></span>

<a name="HighCPU"></a>

### <a name="to-determine-whether-high-cpu-usage-is-caused-by-garbage-collection"></a><span data-ttu-id="c3f9a-451">判斷高 CPU 使用量是否由於記憶體回收所造成</span><span class="sxs-lookup"><span data-stu-id="c3f9a-451">To determine whether high CPU usage is caused by garbage collection</span></span>

- <span data-ttu-id="c3f9a-452">相互關聯 `% Time in GC` 記憶體效能計數器值與處理序時間。</span><span class="sxs-lookup"><span data-stu-id="c3f9a-452">Correlate the `% Time in GC` memory performance counter value with the process time.</span></span>

  <span data-ttu-id="c3f9a-453">如果 `% Time in GC` 值與處理序時間同時升高，則記憶體回收便造成高 CPU 使用量。</span><span class="sxs-lookup"><span data-stu-id="c3f9a-453">If the `% Time in GC` value spikes at the same time as process time, garbage collection is causing a high CPU usage.</span></span> <span data-ttu-id="c3f9a-454">否則，請針對應用程式進行程式碼剖析，尋找發生高使用量的地方。</span><span class="sxs-lookup"><span data-stu-id="c3f9a-454">Otherwise, profile the application to find where the high usage is occurring.</span></span>

## <a name="see-also"></a><span data-ttu-id="c3f9a-455">另請參閱</span><span class="sxs-lookup"><span data-stu-id="c3f9a-455">See also</span></span>

- [<span data-ttu-id="c3f9a-456">記憶體回收</span><span class="sxs-lookup"><span data-stu-id="c3f9a-456">Garbage Collection</span></span>](index.md)
