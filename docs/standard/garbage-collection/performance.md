...
### <a name="debugging-with-sos"></a><span data-ttu-id="d56bb-113">以 SOS 偵錯</span><span class="sxs-lookup"><span data-stu-id="d56bb-113">Debugging with SOS</span></span>
...
- [<span data-ttu-id="d56bb-144">記憶體回收暫停太長</span><span class="sxs-lookup"><span data-stu-id="d56bb-144">Garbage collection pauses are too long</span></span>](#Issue_LongPauses)
...
<span data-ttu-id="d56bb-162">常見的假設是 Windows 工作管理員 [效能] 索引標籤上的記憶體使用量顯示可以指出使用太多記憶體的時刻。</span><span class="sxs-lookup"><span data-stu-id="d56bb-162">A common assumption is that the memory usage display on the **Performance** tab of Windows Task Manager can indicate when too much memory is being used.</span></span> <span data-ttu-id="d56bb-163">不過，該顯示與工作集有關；它不提供虛擬記憶體使用量的相關資訊。</span><span class="sxs-lookup"><span data-stu-id="d56bb-163">However, that display pertains to the working set; it does not provide information about virtual memory usage.</span></span>
...
<span data-ttu-id="d56bb-184">層代 0 中有很多可用空間不成問題，因為這是配置新物件配置所在的層代。</span><span class="sxs-lookup"><span data-stu-id="d56bb-184">Having lots of free space in generation 0 is not a problem because this is the generation where new objects are allocated.</span></span>
...
- [<span data-ttu-id="d56bb-265">判斷 Managed 堆積保留的記憶體數量。</span><span class="sxs-lookup"><span data-stu-id="d56bb-265">Determine how much memory the managed heap reserves.</span></span>](#ManagedHeapReserve)
...
- <span data-ttu-id="d56bb-276">檢查下列兩個記憶體效能計數器：</span><span class="sxs-lookup"><span data-stu-id="d56bb-276">Examine the following two memory performance counters:</span></span>
  ...
  - <span data-ttu-id="d56bb-282">**已認可的位元組總數**。</span><span class="sxs-lookup"><span data-stu-id="d56bb-282">**# Total committed Bytes**.</span></span> <span data-ttu-id="d56bb-283">顯示記憶體回收行程目前已認可的虛擬記憶體數目。</span><span class="sxs-lookup"><span data-stu-id="d56bb-283">Displays the amount of virtual memory currently committed by the garbage collector.</span></span> <span data-ttu-id="d56bb-284">使用此計數器來判斷記憶體回收行程所耗用的記憶體是否佔應用程式所使用記憶體的過多數量。</span><span class="sxs-lookup"><span data-stu-id="d56bb-284">Use this counter to determine whether the memory consumed by the garbage collector is an excessive portion of the memory that your application uses.</span></span>
...
- <span data-ttu-id="d56bb-321">使用 `# Total reserved bytes` 記憶體效能計數器。</span><span class="sxs-lookup"><span data-stu-id="d56bb-321">Use the `# Total reserved bytes` memory performance counter.</span></span>
  ...
  <span data-ttu-id="d56bb-322">記憶體回收行程會以區段來保留記憶體，您可以使用 **eeheap** 命令來判斷區段開始的位置。</span><span class="sxs-lookup"><span data-stu-id="d56bb-322">The garbage collector reserves memory in segments, and you can determine where a segment starts by using the **eeheap** command.</span></span>
...
- <span data-ttu-id="d56bb-325">在已載入 SOS 偵錯工具擴充功能的 WinDbg 或 Visual Studio 偵錯工具中，輸入下列命令：</span><span class="sxs-lookup"><span data-stu-id="d56bb-325">In the WinDbg or Visual Studio debugger with the SOS debugger extension loaded, type the following command:</span></span>
  ...
  <span data-ttu-id="d56bb-328">「區段」所指定的位址是區段的起始位址。</span><span class="sxs-lookup"><span data-stu-id="d56bb-328">The addresses indicated by "segment" are the starting addresses of the segments.</span></span>
...
2. <span data-ttu-id="d56bb-363">若要取得執行緒的輸出，請輸入下列命令：</span><span class="sxs-lookup"><span data-stu-id="d56bb-363">To get an output of threads, type the following command:</span></span>
...
- <span data-ttu-id="d56bb-373">若要判斷層代 0 中的可用空間，請輸入下列命令以取得依層代的記憶體耗用量資訊：</span><span class="sxs-lookup"><span data-stu-id="d56bb-373">To determine the free space in generation 0, type the following command for memory consumption information by generation:</span></span>
  ...
  <span data-ttu-id="d56bb-374">**!eeheap -gc**</span><span class="sxs-lookup"><span data-stu-id="d56bb-374">**!eeheap -gc**</span></span>
...
- <span data-ttu-id="d56bb-406">或者，您可以使用[記憶體回收 ETW 事件](../../../docs/framework/performance/garbage-collection-etw-events.md)來判斷記憶體回收的長度，並分析資訊來判斷記憶體回收的持續時間。</span><span class="sxs-lookup"><span data-stu-id="d56bb-406">Alternatively, you can determine the length of a garbage collection by using [garbage collection ETW events](../../../docs/framework/performance/garbage-collection-etw-events.md), and analyze the information to determine the duration of garbage collection.</span></span>
  ...
  <span data-ttu-id="d56bb-407">例如，下列資料顯示在非並行記憶體回收期間發生的事件序列。</span><span class="sxs-lookup"><span data-stu-id="d56bb-407">For example, the following data shows an event sequence that occurred during a non-concurrent garbage collection.</span></span>
  ...
  <span data-ttu-id="d56bb-413">在 42504816 的 `GCStart_V1` 事件表示這是背景記憶體回收，因為最後一個欄位是 `1`。</span><span class="sxs-lookup"><span data-stu-id="d56bb-413">The `GCStart_V1` event at 42504816 indicates that this is a background garbage collection, because the last field is `1`.</span></span> <span data-ttu-id="d56bb-414">這會變成記憶體回收第</span><span class="sxs-lookup"><span data-stu-id="d56bb-414">This becomes garbage collection No.</span></span> <span data-ttu-id="d56bb-415">102019 號。</span><span class="sxs-lookup"><span data-stu-id="d56bb-415">102019.</span></span>

  <span data-ttu-id="d56bb-416">因為需要先進行暫時記憶體回收，才能開始背景記憶體回收，因此發生 `GCStart` 事件。</span><span class="sxs-lookup"><span data-stu-id="d56bb-416">The `GCStart` event occurs because there is a need for an ephemeral garbage collection before you start a background garbage collection.</span></span> <span data-ttu-id="d56bb-417">這會變成記憶體回收第</span><span class="sxs-lookup"><span data-stu-id="d56bb-417">This becomes garbage collection No.</span></span> <span data-ttu-id="d56bb-418">102020 號。</span><span class="sxs-lookup"><span data-stu-id="d56bb-418">102020.</span></span>

  <span data-ttu-id="d56bb-419">在 42514170 處，記憶體回收第</span><span class="sxs-lookup"><span data-stu-id="d56bb-419">At 42514170, garbage collection No.</span></span> <span data-ttu-id="d56bb-420">102020 號完成。</span><span class="sxs-lookup"><span data-stu-id="d56bb-420">102020 finishes.</span></span> <span data-ttu-id="d56bb-421">Managed 執行緒會在此時重新啟動。</span><span class="sxs-lookup"><span data-stu-id="d56bb-421">The managed threads are restarted at this point.</span></span> <span data-ttu-id="d56bb-422">此步驟會在執行緒 4372，觸發此背景記憶體回收完成。</span><span class="sxs-lookup"><span data-stu-id="d56bb-422">This is completed on thread 4372, which triggered this background garbage collection.</span></span>

  <span data-ttu-id="d56bb-423">在執行緒 4744 上發生擱置。</span><span class="sxs-lookup"><span data-stu-id="d56bb-423">On thread 4744, a suspension occurs.</span></span> <span data-ttu-id="d56bb-424">這是背景記憶體回收唯一必須暫止 Managed 執行緒的時間。</span><span class="sxs-lookup"><span data-stu-id="d56bb-424">This is the only time at which the background garbage collection has to suspend managed threads.</span></span> <span data-ttu-id="d56bb-425">這段期間是大約 99 毫秒 ((63784407-63685394)/1000)。</span><span class="sxs-lookup"><span data-stu-id="d56bb-425">This duration is approximately 99ms ((63784407-63685394)/1000).</span></span>

  <span data-ttu-id="d56bb-426">背景記憶體回收的 `GCEnd` 事件位於 89931423。</span><span class="sxs-lookup"><span data-stu-id="d56bb-426">The `GCEnd` event for the background garbage collection is at 89931423.</span></span> <span data-ttu-id="d56bb-427">這表示背景記憶體回收持續大約 47 秒 ((89931423-42504816)/1000)。</span><span class="sxs-lookup"><span data-stu-id="d56bb-427">This means that the background garbage collection lasted for about 47seconds ((89931423-42504816)/1000).</span></span>

  <span data-ttu-id="d56bb-428">在 Managed 執行緒執行時，您可能看到發生任意數目的暫時記憶體回收。</span><span class="sxs-lookup"><span data-stu-id="d56bb-428">While the managed threads are running, you can see any number of ephemeral garbage collections occurring.</span></span>

<a name="Triggered"></a>

### <a name="to-determine-what-triggered-a-garbage-collection"></a><span data-ttu-id="d56bb-429">判斷觸發記憶體回收的原因</span><span class="sxs-lookup"><span data-stu-id="d56bb-429">To determine what triggered a garbage collection</span></span>

- <span data-ttu-id="d56bb-430">在已載入 SOS 偵錯工具擴充功能的 WinDbg 或 Visual Studio 偵錯工具中，輸入下列命令以顯示所有執行緒及其呼叫堆疊：</span><span class="sxs-lookup"><span data-stu-id="d56bb-430">In the WinDbg or Visual Studio debugger with the SOS debugger extension loaded, type the following command to show all the threads with their call stacks:</span></span>

  <span data-ttu-id="d56bb-431">**~\*kb**</span><span class="sxs-lookup"><span data-stu-id="d56bb-431">**~\*kb**</span></span>

  <span data-ttu-id="d56bb-432">這個命令會顯示與下列類似的輸出。</span><span class="sxs-lookup"><span data-stu-id="d56bb-432">This command displays output similar to the following.</span></span>
  ...
  <span data-ttu-id="d56bb-433">如果記憶體回收的起因是來自作業系統的記憶體不足通知，則呼叫堆疊會很類似，只除了執行緒是完成項執行緒。</span><span class="sxs-lookup"><span data-stu-id="d56bb-433">If the garbage collection was caused by a low memory notification from the operating system, the call stack is similar, except that the thread is the finalizer thread.</span></span> <span data-ttu-id="d56bb-434">完成項執行緒會收到非同步記憶體偏低通知，並引發記憶體回收。</span><span class="sxs-lookup"><span data-stu-id="d56bb-434">The finalizer thread gets an asynchronous low memory notification and induces the garbage collection.</span></span>

  <span data-ttu-id="d56bb-435">如果記憶體回收的起因是記憶體配置，則堆疊會如下所示：</span><span class="sxs-lookup"><span data-stu-id="d56bb-435">If the garbage collection was caused by memory allocation, the stack appears as follows:</span></span>
  ...
  <span data-ttu-id="d56bb-436">Just-In-Time 協助程式 (`JIT_New*`) 實際上呼叫 `GCHeap::GarbageCollectGeneration`。</span><span class="sxs-lookup"><span data-stu-id="d56bb-436">A just-in-time helper (`JIT_New*`) eventually calls `GCHeap::GarbageCollectGeneration`.</span></span> <span data-ttu-id="d56bb-437">如果您判斷層代 2 記憶體回收起因是配置，則必須判斷層代 2 記憶體回收會回收哪些物件及如何加以避免。</span><span class="sxs-lookup"><span data-stu-id="d56bb-437">If you determine that generation 2 garbage collections are caused by allocations, you must determine which objects are collected by a generation 2 garbage collection and how to avoid them.</span></span> <span data-ttu-id="d56bb-438">也就是您想要判斷層代 2 記憶體回收開始和結束之間的差異，以及造成層代 2 記憶體回收的物件。</span><span class="sxs-lookup"><span data-stu-id="d56bb-438">That is, you want to determine the difference between the start and the end of a generation 2 garbage collection, and the objects that caused the generation 2 collection.</span></span>

  <span data-ttu-id="d56bb-439">例如，在偵錯工具中輸入下列命令以顯示層代 2 記憶體回收的開始：</span><span class="sxs-lookup"><span data-stu-id="d56bb-439">For example, type the following command in the debugger to show the beginning of a generation 2 collection:</span></span>

  <span data-ttu-id="d56bb-440">**!dumpheap –stat**</span><span class="sxs-lookup"><span data-stu-id="d56bb-440">**!dumpheap –stat**</span></span>

  <span data-ttu-id="d56bb-441">範例輸出 (經過刪減以顯示使用最大空間的物件)：</span><span class="sxs-lookup"><span data-stu-id="d56bb-441">Example output (abridged to show the objects that use the most space):</span></span>
  ...
  <span data-ttu-id="d56bb-442">在層代 2 結束時重複執行命令：</span><span class="sxs-lookup"><span data-stu-id="d56bb-442">Repeat the command at the end of generation 2:</span></span>

  <span data-ttu-id="d56bb-443">**!dumpheap –stat**</span><span class="sxs-lookup"><span data-stu-id="d56bb-443">**!dumpheap –stat**</span></span>

  <span data-ttu-id="d56bb-444">範例輸出 (經過刪減以顯示使用最大空間的物件)：</span><span class="sxs-lookup"><span data-stu-id="d56bb-444">Example output (abridged to show the objects that use the most space):</span></span>
  ...
  <span data-ttu-id="d56bb-445">`double[]` 物件從輸出結束消失，這表示它們已被回收。</span><span class="sxs-lookup"><span data-stu-id="d56bb-445">The `double[]` objects disappeared from the end of the output, which means that they were collected.</span></span> <span data-ttu-id="d56bb-446">這些物件大約 70 MB。</span><span class="sxs-lookup"><span data-stu-id="d56bb-446">These objects account for approximately 70 MB.</span></span> <span data-ttu-id="d56bb-447">剩餘的物件變更不多。</span><span class="sxs-lookup"><span data-stu-id="d56bb-447">The remaining objects did not change much.</span></span> <span data-ttu-id="d56bb-448">因此，這些 `double[]` 物件便是這個層代 2 記憶體回收發生的原因。</span><span class="sxs-lookup"><span data-stu-id="d56bb-448">Therefore, these `double[]` objects were the reason why this generation 2 garbage collection occurred.</span></span> <span data-ttu-id="d56bb-449">您的下一個步驟是判斷 `double[]` 物件為何出現，以及其終止原因。</span><span class="sxs-lookup"><span data-stu-id="d56bb-449">Your next step is to determine why the `double[]` objects are there and why they died.</span></span> <span data-ttu-id="d56bb-450">您可以要求程式碼開發人員這些物件的來源，或者您可以使用 **gcroot** 命令。</span><span class="sxs-lookup"><span data-stu-id="d56bb-450">You can ask the code developer where these objects came from, or you can use the **gcroot** command.</span></span>

<a name="HighCPU"></a>

### <a name="to-determine-whether-high-cpu-usage-is-caused-by-garbage-collection"></a><span data-ttu-id="d56bb-451">判斷高 CPU 使用量是否由於記憶體回收所造成</span><span class="sxs-lookup"><span data-stu-id="d56bb-451">To determine whether high CPU usage is caused by garbage collection</span></span>

- <span data-ttu-id="d56bb-452">相互關聯 `% Time in GC` 記憶體效能計數器值與處理序時間。</span><span class="sxs-lookup"><span data-stu-id="d56bb-452">Correlate the `% Time in GC` memory performance counter value with the process time.</span></span>

  <span data-ttu-id="d56bb-453">如果 `% Time in GC` 值與處理序時間同時升高，則記憶體回收便造成高 CPU 使用量。</span><span class="sxs-lookup"><span data-stu-id="d56bb-453">If the `% Time in GC` value spikes at the same time as process time, garbage collection is causing a high CPU usage.</span></span> <span data-ttu-id="d56bb-454">否則，請針對應用程式進行程式碼剖析，尋找發生高使用量的地方。</span><span class="sxs-lookup"><span data-stu-id="d56bb-454">Otherwise, profile the application to find where the high usage is occurring.</span></span>

## <a name="see-also"></a><span data-ttu-id="d56bb-455">請參閱</span><span class="sxs-lookup"><span data-stu-id="d56bb-455">See also</span></span>

- [<span data-ttu-id="d56bb-456">記憶體回收</span><span class="sxs-lookup"><span data-stu-id="d56bb-456">Garbage Collection</span></span>](../../../docs/standard/garbage-collection/index.md)
