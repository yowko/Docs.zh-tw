...
|<xref:System.OperationCanceledException>|此例外狀況之建構函式的多載接受 <xref:System.Threading.CancellationToken> 做為參數。 接聽程式可以選擇性地擲回這個例外狀況來驗證取消來源，並通知其他接聽程式它已經回應取消要求。|  
 ...
 新的取消模型已整合至 [!INCLUDE[dnprdnshort](../../../includes/dnprdnshort-md.md)] 的數種類型中。 最重要的部分包括 <xref:System.Threading.Tasks.Parallel?displayProperty=nameWithType>、<xref:System.Threading.Tasks.Task?displayProperty=nameWithType>、<xref:System.Threading.Tasks.Task%601?displayProperty=nameWithType> 和 <xref:System.Linq.ParallelEnumerable?displayProperty=nameWithType>。 我們建議您針對所有新程式庫和應用程式程式碼使用這個新的取消模型。  
