...
不過，即使使用此方法，還是有許多程式碼可以捨棄。 此時就需要 *Lambda 運算式*。 Lambda 運算式（簡稱 "lambda"）是以 c # 3.0 引進，做為語言整合式查詢（LINQ）的其中一個核心建立區塊。 這是更方便使用委派的語法。 它們會宣告簽章和方法主體，但除非指派給委派，否則不會有自己的正式身分識別。 不同於委派，這些運算式可在事件註冊左邊，或在各種 LINQ 子句和方法中直接指派。
