...
-   **排程器** *排程*表示要讓系統管理員啟動叢集中的容器，以便它們也提供使用者介面的功能。 叢集排程器有下列職責：有效率地使用叢集的資源、設定使用者所提供的條件約束、跨節點或主機有效進行容器負載平衡，以及維持容錯性並保障高可用性。
...
不過，Service Fabric 也可能有所謂的「側車」容器 (必須一起部署為邏輯服務一部分的兩個容器)。 重點是商務微服務是圍繞數個緊密結合項目的邏輯界限。 在許多情況下，它可能是一項服務與單一資料模型，但在某些情況下，您可能會有數項實體服務，以及。
