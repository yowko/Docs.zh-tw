...
A[資料磁碟區容器](https://docs.docker.com/v1.8/userguide/dockervolumes/)是一般資料磁碟區。 其本質上是休眠容器具有一個或多個資料磁碟區，在其中建立 （如先前所述）。 從中央的掛接點，資料磁碟區容器會提供存取至容器。 這種存取方法的優點是它擷取原始的資料，讓資料容器的邏輯的掛接點的位置。 它也允許存取資料容器的磁碟區建立和終結同時資料持續性專用的容器中的 「 應用程式 」 容器。
