...
如果您已經使用命令式命令來設定您的叢集，您可以使用`kubectl get svc SERVICENAME -o yaml > service.yaml`來匯出宣告式資訊清單。 此命令會產生類似下面所示的資訊清單：
...
選取此選項時，會使用其根目錄`Dockerfile`中的建立專案，這可用來在 Docker 容器中建立和裝載應用程式。 範例 Dockerfile 如 [圖 3]-6 所示。
