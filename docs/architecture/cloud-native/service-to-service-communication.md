...
在上圖中，發行者會將訊息傳送至主題。 最後，訂閱者會從訂用帳戶接收訊息。 在中間，主題會根據一組*規則*（以暗藍色方塊顯示），將訊息轉送至訂閱。 規則會作為將特定訊息轉寄至訂用帳戶的篩選準則。 在這裡，"CreateOrder" 事件會傳送到訂\#用帳戶1和\#訂用帳戶3，但\#不會傳送到訂用帳戶2。 「OrderCompleted」事件會傳送到訂\#用帳戶2和訂\#用帳戶3。
