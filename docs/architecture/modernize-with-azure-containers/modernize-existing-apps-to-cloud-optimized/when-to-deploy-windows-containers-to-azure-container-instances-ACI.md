...
---
# <a name="when-to-deploy-windows-containers-to-azure-container-instances-aci"></a>何時將 Windows 容器部署到 Azure 容器實例 （ACI）

Azure 容器實例的主要價值主張是，您可以立即向其部署容器，並且不需要維護該環境，無需升級/修補基礎作業系統或 VM，所有這些都是透明的，只需將容器部署到即用型環境中即可。
...
簡而言之，借助 ACI，您可以快速開發應用，而無需管理虛擬機器或學習新工具。 它只是您的應用程式，在容器中，在雲中運行。
