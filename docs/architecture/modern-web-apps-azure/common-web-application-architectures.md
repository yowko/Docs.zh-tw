...
大部分傳統的 .NET 應用程式會部署為單一單位，對應至在單一 IIS appdomain 內執行的可執行檔或單一 Web 應用程式。 這種方法是最簡單的部署模型，而且非常適合許多內部和較小的公用應用程式。 不過，即使指定這種單一部署單位，大部分重要的商務應用程式都能受益於將邏輯稍微分割到數個層級。
...
當應用程式變得越來越複雜時，管理這種複雜性的一種方法，是根據應用程式的責任或關注點來分解應用程式。 這種方法會遵循考慮原則的分隔，並可協助保持不斷成長的程式碼基底，讓開發人員可以輕鬆地找到特定功能的執行位置。 不過，除了程式碼組織之外，分層的架構還提供許多優勢。
...
使用分層式架構，應用程式可以強制限制哪些層級可以與其他層級通訊。 此架構有助於達成封裝。 變更或取代層級時，只有處理該層級的層級會受到影響。 藉由限制哪些層級相依於哪些其他層級，變更的影響可以得到緩解，讓單一變更不會影響整個應用程式。

層級 (和封裝) 使得可以更容易取代應用程式中的功能。 例如，針對持續性，應用程式一開始可能使用自己的 SQL Server 資料庫，但後來可以選擇使用以雲端為基礎的持續性策略，或 Web API 之後的策略。 如果應用程式已在邏輯層內正確封裝其持續性實作為，則該 SQL Server 特定層可能會取代為執行相同公用介面的新層。

除了為了回應未來的需求變更而換掉實作的可能性，應用程式層級也能更輕鬆地基於測試目的而換掉實作。 您不必撰寫測試來操作應用程式的實際資料層級或 UI 層級，這些層級可以在測試階段取代為假的實作，以提供已知的回應給要求。 相較于針對應用程式的實際基礎結構執行測試，這種方法通常可讓您更輕鬆地撰寫測試和執行速度更快。
...
Clean Architecture 會將商務邏輯和應用程式模型放在應用程式的中央位置。 不讓商務邏輯相依於資料存取或其他基礎結構的關注點，而是反轉此相依性：基礎結構和實作詳細資料相依於應用程式核心。 這項功能的達成方式是在應用程式核心中定義抽象概念或介面，然後由基礎結構層中定義的類型來執行。 視覺化這個架構的常見方式是使用一系列的同心圓，類似於洋蔥。 圖 5-7 示範這種架構的表示法。
...
針對整合型應用程式，應用程式核心、基礎結構和 UI 專案都是以單一應用程式的形式執行。 執行階段應用程式架構看起來可能圖 5-12。
...
因為容器在設計上本質上是不可變的，您永遠不需要擔心 Vm 損毀，而更新腳本可能會忘記將某些特定的設定或檔案留在磁片上。

您可以使用 Docker 容器進行更簡單的 web 應用程式整合型部署。 這種方法可改善持續整合和持續部署管線，並協助達成部署到生產環境的成功。 不再有「它可在我的電腦上運作，為什麼它無法在生產環境中運作？」
...
應用程式可能還不需要獨立擴充功能。 許多應用程式若其規模需要超過單一執行個體，則可透過複製整個執行個體之相當簡單的程序來完成。 將應用程式分成不同服務的額外工作，在調整應用程式的完整實例很簡單且符合成本效益時，提供最大的好處。
...
其中一個優點是，容器化部署表示每個應用程式執行個體都會在相同的環境中執行。 這種方法包含早期測試和開發的開發人員環境。 開發小組可以在與生產環境相符的容器化環境中執行應用程式。

此外，容器化應用程式會以較低的成本相應放大。 使用容器環境可共用的資源比傳統 VM 環境更多。
...
`eShopOnWeb`專案會在 .net 上執行。 因此，它可以在 Linux 或 Windows 容器中執行。 請注意，若是 Docker 部署，您想要針對 SQL Server 使用相同的主機類型。 Linux 容器允許較小的使用量，而且是偏好選項。

您可以使用 Visual Studio 2017 或更新版本將 Docker 支援新增到現有的應用程式，方法是以滑鼠右鍵按一下 [方案總管] 中的專案，然後選擇 [新增] > [Docker 支援]。 此步驟會新增所需的檔案，並修改專案以使用這些檔案。 目前的 `eShopOnWeb` 範例已有這些檔案。
...
如果您想要使用 Visual Studio 將 Docker 支援新增到應用程式，請確認這樣做時，Docker Desktop 正在執行。 如果 Docker Desktop 在您啟動精靈時並未執行，精靈將無法正確執行。 此外，精靈會檢查您目前的容器選擇，以新增正確的 Docker 支援。 如果您想要新增 Windows 容器的支援，您必須在具有已設定 Windows 容器的 Docker Desktop 執行時，執行嚮導。 如果您想要新增 Linux 容器的支援，請在您具有已設定 Linux 容器的 Docker 時，執行嚮導。
