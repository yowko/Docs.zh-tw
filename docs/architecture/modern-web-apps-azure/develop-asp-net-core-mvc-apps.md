...
## <a name="working-with-dependencies"></a><span data-ttu-id="aa9a7-190">使用相依性</span><span class="sxs-lookup"><span data-stu-id="aa9a7-190">Working with dependencies</span></span>
...
### <a name="declare-your-dependencies"></a><span data-ttu-id="aa9a7-206">宣告您的相依性</span><span class="sxs-lookup"><span data-stu-id="aa9a7-206">Declare your dependencies</span></span>
...
<span data-ttu-id="aa9a7-234">除了這些專案，還會包含個別測試專案 (第 9 章中會討論測試)。</span><span class="sxs-lookup"><span data-stu-id="aa9a7-234">In addition to these projects, separate test projects are included as well (Testing is discussed in Chapter 9).</span></span>
...
<span data-ttu-id="aa9a7-261">除了 Areas 的內建支援，您也可以使用自己的資料夾結構和慣例，來取代屬性和自訂路由。</span><span class="sxs-lookup"><span data-stu-id="aa9a7-261">In addition to the built-in support for Areas, you can also use your own folder structure, and conventions in place of attributes and custom routes.</span></span> <span data-ttu-id="aa9a7-262">這樣可讓您擁有不包含個別 Views、Controllers 等資料夾的功能資料夾，以保持更平面的階層架構，讓您可以更輕鬆地在單一位置查看每項功能的所有相關檔案。</span><span class="sxs-lookup"><span data-stu-id="aa9a7-262">This would allow you to have feature folders that didn't include separate folders for Views, Controllers, etc., keeping the hierarchy flatter and making it easier to see all related files in a single place for each feature.</span></span>
...
### <a name="cross-cutting-concerns"></a><span data-ttu-id="aa9a7-270">跨領域考量</span><span class="sxs-lookup"><span data-stu-id="aa9a7-270">Cross-cutting concerns</span></span>
...
<span data-ttu-id="aa9a7-293">您可以藉由包含 [Ardalis.ValidateModel](https://www.nuget.org/packages/Ardalis.ValidateModel) 套件，來將 `ValidateModelAttribute` 新增到您的專案作為 NuGet 相依性。</span><span class="sxs-lookup"><span data-stu-id="aa9a7-293">You can add the `ValidateModelAttribute` to your project as a NuGet dependency by including the [Ardalis.ValidateModel](https://www.nuget.org/packages/Ardalis.ValidateModel) package.</span></span> <span data-ttu-id="aa9a7-294">針對 API，您可以使用 `ApiController` 屬性，在不需要個別 `ValidateModel` 篩選的情況下強制執行此行為。</span><span class="sxs-lookup"><span data-stu-id="aa9a7-294">For APIs, you can use the `ApiController` attribute to enforce this behavior without the need for a separate `ValidateModel` filter.</span></span>
...
## <a name="security"></a><span data-ttu-id="aa9a7-303">安全性</span><span class="sxs-lookup"><span data-stu-id="aa9a7-303">Security</span></span>
...
<span data-ttu-id="aa9a7-338">此原則可接著搭配 \[Authorize\] 屬性使用，以保護任何控制器及/或動作，如上所述。</span><span class="sxs-lookup"><span data-stu-id="aa9a7-338">This policy could then be used with the \[Authorize\] attribute to protect any controller and/or action, as described above.</span></span>
...
<span data-ttu-id="aa9a7-383">領域驅動設計 (DDD) 是建置軟體的敏捷式方法，重點強調「商務領域」__。</span><span class="sxs-lookup"><span data-stu-id="aa9a7-383">Domain-Driven Design (DDD) is an agile approach to building software that emphasizes focusing on the _business domain_.</span></span> <span data-ttu-id="aa9a7-384">它相當重視與商務領域專家的溝通和互動，這些專家會向開發人員說明真實世界系統的運作方式。</span><span class="sxs-lookup"><span data-stu-id="aa9a7-384">It places a heavy emphasis on communication and interaction with business domain expert(s) who can relate to the developers how the real-world system works.</span></span> <span data-ttu-id="aa9a7-385">例如，如果您想要建置處理股票交易的系統，您的領域專家可能會是經驗豐富的股票交易員。</span><span class="sxs-lookup"><span data-stu-id="aa9a7-385">For example, if you're building a system that handles stock trades, your domain expert might be an experienced stock broker.</span></span> <span data-ttu-id="aa9a7-386">DDD 是專為處理複雜的大型商務問題所設計，通常不適用於較小、較簡單的應用程式，因為不值得投資了解領域及建立領域模型。</span><span class="sxs-lookup"><span data-stu-id="aa9a7-386">DDD is designed to address large, complex business problems, and is often not appropriate for smaller, simpler applications, as the investment in understanding and modeling the domain is not worth it.</span></span>

<span data-ttu-id="aa9a7-387">建置採用 DDD 方法的軟體時，您的小組 (包括非技術性專案關係人和參與者) 應該針對問題空間開發「通用語言」__。</span><span class="sxs-lookup"><span data-stu-id="aa9a7-387">When building software following a DDD approach, your team (including non-technical stakeholders and contributors) should develop a _ubiquitous language_ for the problem space.</span></span> <span data-ttu-id="aa9a7-388">換句話說，您應該針對要模型化的真實世界概念、對等軟體，以及為了保存概念而可能存在的任何結構 (例如資料庫資料表) 使用相同的用語。</span><span class="sxs-lookup"><span data-stu-id="aa9a7-388">That is, the same terminology should be used for the real-world concept being modeled, the software equivalent, and any structures that might exist to persist the concept (for example, database tables).</span></span> <span data-ttu-id="aa9a7-389">因此，通用語言中所述的概念應該會形成「領域模型」__ 的基礎。</span><span class="sxs-lookup"><span data-stu-id="aa9a7-389">Thus, the concepts described in the ubiquitous language should form the basis for your _domain model_.</span></span>

<span data-ttu-id="aa9a7-390">您的領域模型會包含彼此互動以代表系統行為的物件。</span><span class="sxs-lookup"><span data-stu-id="aa9a7-390">Your domain model is comprised of objects that interact with one another to represent the behavior of the system.</span></span> <span data-ttu-id="aa9a7-391">這些物件的分類如下：</span><span class="sxs-lookup"><span data-stu-id="aa9a7-391">These objects may fall into the following categories:</span></span>

- <span data-ttu-id="aa9a7-392">[實體](https://deviq.com/entity/)，代表具有身分識別執行緒的物件。</span><span class="sxs-lookup"><span data-stu-id="aa9a7-392">[Entities](https://deviq.com/entity/), which represent objects with a thread of identity.</span></span> <span data-ttu-id="aa9a7-393">實體通常會儲存在使用金鑰的持續性儲存區中，稍後可予以擷取。</span><span class="sxs-lookup"><span data-stu-id="aa9a7-393">Entities are typically stored in persistence with a key by which they can later be retrieved.</span></span>

- <span data-ttu-id="aa9a7-394">[彙總](https://deviq.com/aggregate-pattern/)，代表應該當作一個單位保存的物件群組。</span><span class="sxs-lookup"><span data-stu-id="aa9a7-394">[Aggregates](https://deviq.com/aggregate-pattern/), which represent groups of objects that should be persisted as a unit.</span></span>

- <span data-ttu-id="aa9a7-395">[值物件](https://deviq.com/value-object/)，代表可根據其屬性值的總和比較的概念。</span><span class="sxs-lookup"><span data-stu-id="aa9a7-395">[Value objects](https://deviq.com/value-object/), which represent concepts that can be compared on the basis of the sum of their property values.</span></span> <span data-ttu-id="aa9a7-396">例如，由開始和結束日期組成的 DateRange。</span><span class="sxs-lookup"><span data-stu-id="aa9a7-396">For example, DateRange consisting of a start and end date.</span></span>

- <span data-ttu-id="aa9a7-397">[領域事件](https://martinfowler.com/eaaDev/DomainEvent.html)，代表系統內發生的事件，系統的其他組件對這些事件會有興趣。</span><span class="sxs-lookup"><span data-stu-id="aa9a7-397">[Domain events](https://martinfowler.com/eaaDev/DomainEvent.html), which represent things happening within the system that are of interest to other parts of the system.</span></span>

<span data-ttu-id="aa9a7-398">DDD 域模型應封裝模型中的複雜行為。</span><span class="sxs-lookup"><span data-stu-id="aa9a7-398">A DDD domain model should encapsulate complex behavior within the model.</span></span> <span data-ttu-id="aa9a7-399">特別是實體，不應該只是屬性集合。</span><span class="sxs-lookup"><span data-stu-id="aa9a7-399">Entities, in particular, should not merely be collections of properties.</span></span> <span data-ttu-id="aa9a7-400">當領域模型缺少行為且只代表系統的狀態時，即為 [Anemic 模型](https://deviq.com/anemic-model/)，DDD 中並不需要此模型。</span><span class="sxs-lookup"><span data-stu-id="aa9a7-400">When the domain model lacks behavior and merely represents the state of the system, it is said to be an [anemic model](https://deviq.com/anemic-model/), which is undesirable in DDD.</span></span>

<span data-ttu-id="aa9a7-401">除了這些模型類型，DDD 通常還會採用多種模式：</span><span class="sxs-lookup"><span data-stu-id="aa9a7-401">In addition to these model types, DDD typically employs a variety of patterns:</span></span>

- <span data-ttu-id="aa9a7-402">[儲存機制](https://deviq.com/repository-pattern/)，用於抽象化持續性詳細資料。</span><span class="sxs-lookup"><span data-stu-id="aa9a7-402">[Repository](https://deviq.com/repository-pattern/), for abstracting persistence details.</span></span>

- <span data-ttu-id="aa9a7-403">[Factory](https://en.wikipedia.org/wiki/Factory_method_pattern)，用於封裝複雜的物件建立。</span><span class="sxs-lookup"><span data-stu-id="aa9a7-403">[Factory](https://en.wikipedia.org/wiki/Factory_method_pattern), for encapsulating complex object creation.</span></span>

- <span data-ttu-id="aa9a7-404">領域事件，用於分離相依行為與觸發行為。</span><span class="sxs-lookup"><span data-stu-id="aa9a7-404">Domain events, for decoupling dependent behavior from triggering behavior.</span></span>

- <span data-ttu-id="aa9a7-405">[服務](http://gorodinski.com/blog/2012/04/14/services-in-domain-driven-design-ddd/)，用於封裝複雜行為及/或基礎結構實作詳細資料。</span><span class="sxs-lookup"><span data-stu-id="aa9a7-405">[Services](http://gorodinski.com/blog/2012/04/14/services-in-domain-driven-design-ddd/), for encapsulating complex behavior and/or infrastructure implementation details.</span></span>

- <span data-ttu-id="aa9a7-406">[命令](https://en.wikipedia.org/wiki/Command_pattern)，用於分離發出命令與執行命令本身。</span><span class="sxs-lookup"><span data-stu-id="aa9a7-406">[Command](https://en.wikipedia.org/wiki/Command_pattern), for decoupling issuing commands and executing the command itself.</span></span>

- <span data-ttu-id="aa9a7-407">[規格](https://deviq.com/specification-pattern/)，用於封裝查詢詳細資料。</span><span class="sxs-lookup"><span data-stu-id="aa9a7-407">[Specification](https://deviq.com/specification-pattern/), for encapsulating query details.</span></span>

<span data-ttu-id="aa9a7-408">DDD 也建議使用上述的全新架構，允許鬆散結合、封裝，以及可使用單元測試輕鬆驗證的程式碼。</span><span class="sxs-lookup"><span data-stu-id="aa9a7-408">DDD also recommends the use of the Clean Architecture discussed previously, allowing for loose coupling, encapsulation, and code that can easily be verified using unit tests.</span></span>

### <a name="when-should-you-apply-ddd"></a><span data-ttu-id="aa9a7-409">何時應該套用 DDD</span><span class="sxs-lookup"><span data-stu-id="aa9a7-409">When should you apply DDD</span></span>

<span data-ttu-id="aa9a7-410">DDD 非常適合具有重大業務（而不僅僅是技術）複雜性的大型應用程式。</span><span class="sxs-lookup"><span data-stu-id="aa9a7-410">DDD is well suited to large applications with significant business (not just technical) complexity.</span></span> <span data-ttu-id="aa9a7-411">該應用程式需要有領域專家的知識。</span><span class="sxs-lookup"><span data-stu-id="aa9a7-411">The application should require the knowledge of domain experts.</span></span> <span data-ttu-id="aa9a7-412">領域模型本身應該有代表商務規則和互動的明顯行為，而不只是在資料存放區中儲存及擷取各種記錄的目前狀態。</span><span class="sxs-lookup"><span data-stu-id="aa9a7-412">There should be significant behavior in the domain model itself, representing business rules and interactions beyond simply storing and retrieving the current state of various records from data stores.</span></span>

### <a name="when-shouldnt-you-apply-ddd"></a><span data-ttu-id="aa9a7-413">何時不應該套用 DDD</span><span class="sxs-lookup"><span data-stu-id="aa9a7-413">When shouldn't you apply DDD</span></span>

<span data-ttu-id="aa9a7-414">DDD 牽涉到投資模型、架構和通訊，這對較小型的應用程式，或基本上只有 CRUD (建立/讀取/更新/刪除) 的應用程式，可能並不需要。</span><span class="sxs-lookup"><span data-stu-id="aa9a7-414">DDD involves investments in modeling, architecture, and communication that may not be warranted for smaller applications or applications that are essentially just CRUD (create/read/update/delete).</span></span> <span data-ttu-id="aa9a7-415">如果您選擇讓應用程式採用 DDD，但發現您的領域具有不含任何行為的 Anemic 模型，您可能需要重新思考做法。</span><span class="sxs-lookup"><span data-stu-id="aa9a7-415">If you choose to approach your application following DDD, but find that your domain has an anemic model with no behavior, you may need to rethink your approach.</span></span> <span data-ttu-id="aa9a7-416">您的應用程式可能不需要 DDD，或者您可能需要協助來重構應用程式，將商務邏輯封裝在領域模型中，而不是資料庫或使用者介面中。</span><span class="sxs-lookup"><span data-stu-id="aa9a7-416">Either your application may not need DDD, or you may need assistance refactoring your application to encapsulate business logic in the domain model, rather than in your database or user interface.</span></span>

<span data-ttu-id="aa9a7-417">混合式方法只會將 DDD 用於應用程式的異動或較複雜區域，而不會用於較簡單的 CRUD 或應用程式的唯讀部分。</span><span class="sxs-lookup"><span data-stu-id="aa9a7-417">A hybrid approach would be to only use DDD for the transactional or more complex areas of the application, but not for simpler CRUD or read-only portions of the application.</span></span> <span data-ttu-id="aa9a7-418">例如，如果您想要查詢資料來顯示報表或視覺化儀表板的資料，則不需要有彙總的條件約束。</span><span class="sxs-lookup"><span data-stu-id="aa9a7-418">For instance, you needn't have the constraints of an Aggregate if you're querying data to display a report or to visualize data for a dashboard.</span></span> <span data-ttu-id="aa9a7-419">針對這類需求，最好是使用個別且較簡單的讀取模型。</span><span class="sxs-lookup"><span data-stu-id="aa9a7-419">It's perfectly acceptable to have a separate, simpler read model for such requirements.</span></span>

> ### <a name="references--domain-driven-design"></a><span data-ttu-id="aa9a7-420">參考資料 - 領域驅動設計</span><span class="sxs-lookup"><span data-stu-id="aa9a7-420">References – Domain-Driven Design</span></span>
>
> - <span data-ttu-id="aa9a7-421">**DDD 簡單說明 (StackOverflow 解答)**</span><span class="sxs-lookup"><span data-stu-id="aa9a7-421">**DDD in Plain English (StackOverflow Answer)**</span></span>  
>   <https://stackoverflow.com/questions/1222392/can-someone-explain-domain-driven-design-ddd-in-plain-english-please/1222488#1222488>

## <a name="deployment"></a><span data-ttu-id="aa9a7-422">部署</span><span class="sxs-lookup"><span data-stu-id="aa9a7-422">Deployment</span></span>

<span data-ttu-id="aa9a7-423">不論應用程式的裝載位置，部署 ASP.NET Core 應用程式的程序都包含幾個步驟。</span><span class="sxs-lookup"><span data-stu-id="aa9a7-423">There are a few steps involved in the process of deploying your ASP.NET Core application, regardless of where it will be hosted.</span></span> <span data-ttu-id="aa9a7-424">第一步是發佈應用程式，可以使用`dotnet publish`CLI 命令完成。</span><span class="sxs-lookup"><span data-stu-id="aa9a7-424">The first step is to publish the application, which can be done using the `dotnet publish` CLI command.</span></span> <span data-ttu-id="aa9a7-425">這會編譯應用程式，並將執行應用程式所需的所有檔案都放在指定的資料夾中。</span><span class="sxs-lookup"><span data-stu-id="aa9a7-425">This will compile the application and place all of the files needed to run the application into a designated folder.</span></span> <span data-ttu-id="aa9a7-426">當您從 Visual Studio 部署時，則會自動為您執行此步驟。</span><span class="sxs-lookup"><span data-stu-id="aa9a7-426">When you deploy from Visual Studio, this step is performed for you automatically.</span></span> <span data-ttu-id="aa9a7-427">publish 資料夾包含應用程式及其相依性的 .exe 和 .dll 檔案。</span><span class="sxs-lookup"><span data-stu-id="aa9a7-427">The publish folder contains .exe and .dll files for the application and its dependencies.</span></span> <span data-ttu-id="aa9a7-428">獨立應用程式還會包含 .NET 執行階段版本。</span><span class="sxs-lookup"><span data-stu-id="aa9a7-428">A self-contained application will also include a version of the .NET runtime.</span></span> <span data-ttu-id="aa9a7-429">ASP.NET Core 應用程式也會包含組態檔、靜態用戶端資產和 MVC 檢視。</span><span class="sxs-lookup"><span data-stu-id="aa9a7-429">ASP.NET Core applications will also include configuration files, static client assets, and MVC views.</span></span>

<span data-ttu-id="aa9a7-430">ASP.NET Core 應用程式是主控台應用程式，必須在伺服器開機時啟動，並在應用程式 (或伺服器) 損毀時重新啟動。</span><span class="sxs-lookup"><span data-stu-id="aa9a7-430">ASP.NET Core applications are console applications that must be started when the server boots and restarted if the application (or server) crashes.</span></span> <span data-ttu-id="aa9a7-431">您可以使用處理序管理員來自動化此程序。</span><span class="sxs-lookup"><span data-stu-id="aa9a7-431">A process manager can be used to automate this process.</span></span> <span data-ttu-id="aa9a7-432">ASP.NET Core 最常見的處理序管理員是 Linux 上的 Nginx 和 Apache，以及 Windows 上的 IIS 或 Windows 服務。</span><span class="sxs-lookup"><span data-stu-id="aa9a7-432">The most common process managers for ASP.NET Core are Nginx and Apache on Linux and IIS or Windows Service on Windows.</span></span>

<span data-ttu-id="aa9a7-433">除了流程管理器之外，ASP.NET核心應用程式可能使用反向代理伺服器。</span><span class="sxs-lookup"><span data-stu-id="aa9a7-433">In addition to a process manager, ASP.NET Core applications may use a reverse proxy server.</span></span> <span data-ttu-id="aa9a7-434">反向 Proxy 伺服器會從網際網路接收 HTTP 要求，並在進行一些初步處理後，將其轉送至 Kestrel。</span><span class="sxs-lookup"><span data-stu-id="aa9a7-434">A reverse proxy server receives HTTP requests from the Internet and forwards them to Kestrel after some preliminary handling.</span></span> <span data-ttu-id="aa9a7-435">反向代理伺服器為應用程式提供了一層安全性。</span><span class="sxs-lookup"><span data-stu-id="aa9a7-435">Reverse proxy servers provide a layer of security for the application.</span></span> <span data-ttu-id="aa9a7-436">Kestrel 也不支援在相同的連接埠上裝載多個應用程式，因此無法搭配使用主機標頭等技術，以允許在相同的連接埠和 IP 位址上裝載多個應用程式。</span><span class="sxs-lookup"><span data-stu-id="aa9a7-436">Kestrel also doesn't support hosting multiple applications on the same port, so techniques like host headers cannot be used with it to enable hosting multiple applications on the same port and IP address.</span></span>

![Kestrel 到網際網路](./media/image7-5.png)

<span data-ttu-id="aa9a7-438">**圖 7-5**.</span><span class="sxs-lookup"><span data-stu-id="aa9a7-438">**Figure 7-5**.</span></span> <span data-ttu-id="aa9a7-439">ASP.NET在反向代理伺服器後面的 Kestrel 中託管</span><span class="sxs-lookup"><span data-stu-id="aa9a7-439">ASP.NET hosted in Kestrel behind a reverse proxy server</span></span>

<span data-ttu-id="aa9a7-440">在另一個案例中，反向 Proxy 可能有助於使用 SSL/HTTPS 來保護多個應用程式。</span><span class="sxs-lookup"><span data-stu-id="aa9a7-440">Another scenario in which a reverse proxy can be helpful is to secure multiple applications using SSL/HTTPS.</span></span> <span data-ttu-id="aa9a7-441">在此情況下，只有反向 Proxy 需要設定 SSL。</span><span class="sxs-lookup"><span data-stu-id="aa9a7-441">In this case, only the reverse proxy would need to have SSL configured.</span></span> <span data-ttu-id="aa9a7-442">反向 Proxy 伺服器與 Kestrel 之間的通訊可透過 HTTP 進行，如圖 7-6 所示。</span><span class="sxs-lookup"><span data-stu-id="aa9a7-442">Communication between the reverse proxy server and Kestrel could take place over HTTP, as shown in Figure 7-6.</span></span>

![ASP.NET託管在 HTTPS 安全的反向代理伺服器後面](./media/image7-6.png)

<span data-ttu-id="aa9a7-444">**圖 7-6**。</span><span class="sxs-lookup"><span data-stu-id="aa9a7-444">**Figure 7-6**.</span></span> <span data-ttu-id="aa9a7-445">ASP.NET託管在 HTTPS 安全的反向代理伺服器後面</span><span class="sxs-lookup"><span data-stu-id="aa9a7-445">ASP.NET hosted behind an HTTPS-secured reverse proxy server</span></span>

<span data-ttu-id="aa9a7-446">一個越來越普及的方法，是將 ASP.NET Core 應用程式裝載於 Docker 容器，該容器接著可在本機裝載或部署至 Azure 進行雲端式裝載。</span><span class="sxs-lookup"><span data-stu-id="aa9a7-446">An increasingly popular approach is to host your ASP.NET Core application in a Docker container, which then can be hosted locally or deployed to Azure for cloud-based hosting.</span></span> <span data-ttu-id="aa9a7-447">Docker 容器可能會包含您的應用程式程式碼，該程式碼會在 Kestrel 上執行，並部署在反向 Proxy 伺服器後方，如上所示。</span><span class="sxs-lookup"><span data-stu-id="aa9a7-447">The Docker container could contain your application code, running on Kestrel, and would be deployed behind a reverse proxy server, as shown above.</span></span>

<span data-ttu-id="aa9a7-448">如果您在 Azure 上裝載應用程式，您可以使用 Microsoft Azure 應用程式閘道作為專用虛擬設備來提供數項服務。</span><span class="sxs-lookup"><span data-stu-id="aa9a7-448">If you're hosting your application on Azure, you can use Microsoft Azure Application Gateway as a dedicated virtual appliance to provide several services.</span></span> <span data-ttu-id="aa9a7-449">除了作為個別應用程式的反向 Proxy，應用程式閘道也可能提供下列功能：</span><span class="sxs-lookup"><span data-stu-id="aa9a7-449">In addition to acting as a reverse proxy for individual applications, Application Gateway can also offer the following features:</span></span>

- <span data-ttu-id="aa9a7-450">HTTP 負載平衡</span><span class="sxs-lookup"><span data-stu-id="aa9a7-450">HTTP load balancing</span></span>

- <span data-ttu-id="aa9a7-451">SSL 卸載 (SSL 僅限網際網路)</span><span class="sxs-lookup"><span data-stu-id="aa9a7-451">SSL offload (SSL only to Internet)</span></span>

- <span data-ttu-id="aa9a7-452">端對端 SSL</span><span class="sxs-lookup"><span data-stu-id="aa9a7-452">End to End SSL</span></span>

- <span data-ttu-id="aa9a7-453">多網站路由 (單一應用程式閘道上最多可合併 20 個網站)</span><span class="sxs-lookup"><span data-stu-id="aa9a7-453">Multi-site routing (consolidate up to 20 sites on a single Application Gateway)</span></span>

- <span data-ttu-id="aa9a7-454">Web 應用程式防火牆</span><span class="sxs-lookup"><span data-stu-id="aa9a7-454">Web application firewall</span></span>

- <span data-ttu-id="aa9a7-455">WebSocket 支援</span><span class="sxs-lookup"><span data-stu-id="aa9a7-455">Websocket support</span></span>

- <span data-ttu-id="aa9a7-456">進階診斷</span><span class="sxs-lookup"><span data-stu-id="aa9a7-456">Advanced diagnostics</span></span>

<span data-ttu-id="aa9a7-457">_[在第 10 章](development-process-for-azure.md)中瞭解有關 Azure 部署選項的詳細資訊。_</span><span class="sxs-lookup"><span data-stu-id="aa9a7-457">_Learn more about Azure deployment options in [Chapter 10](development-process-for-azure.md)._</span></span>

> ### <a name="references--deployment"></a><span data-ttu-id="aa9a7-458">參考資料 - 部署</span><span class="sxs-lookup"><span data-stu-id="aa9a7-458">References – Deployment</span></span>
>
> - <span data-ttu-id="aa9a7-459">**裝載和部署概觀**</span><span class="sxs-lookup"><span data-stu-id="aa9a7-459">**Hosting and Deployment Overview**</span></span>  
>   <https://docs.microsoft.com/aspnet/core/publishing/>
> - <span data-ttu-id="aa9a7-460">**何時搭配使用 Kestrel 與反向 Proxy**</span><span class="sxs-lookup"><span data-stu-id="aa9a7-460">**When to use Kestrel with a reverse proxy**</span></span>  
>   <https://docs.microsoft.com/aspnet/core/fundamentals/servers/kestrel#when-to-use-kestrel-with-a-reverse-proxy>
> - <span data-ttu-id="aa9a7-461">**在 Docker 中裝載 ASP.NET Core 應用程式**</span><span class="sxs-lookup"><span data-stu-id="aa9a7-461">**Host ASP.NET Core apps in Docker**</span></span>  
>   <https://docs.microsoft.com/aspnet/core/publishing/docker>
> - <span data-ttu-id="aa9a7-462">**Azure 應用程式閘道簡介**</span><span class="sxs-lookup"><span data-stu-id="aa9a7-462">**Introducing Azure Application Gateway**</span></span>  
>   <https://docs.microsoft.com/azure/application-gateway/application-gateway-introduction>

>[!div class="step-by-step"]
><span data-ttu-id="aa9a7-463">[上一個](common-client-side-web-technologies.md)
>[下一個](work-with-data-in-asp-net-core-apps.md)</span><span class="sxs-lookup"><span data-stu-id="aa9a7-463">[Previous](common-client-side-web-technologies.md)
