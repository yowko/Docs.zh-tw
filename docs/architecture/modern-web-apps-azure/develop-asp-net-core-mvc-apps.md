...
<span data-ttu-id="02bdf-294">此外， Blazor 應用程式（無論使用 Blazor 伺服器或 Blazor WebAssembly ）都應該建立為個別的專案。</span><span class="sxs-lookup"><span data-stu-id="02bdf-294">Additionally, Blazor applications, whether using Blazor Server or Blazor WebAssembly, should be built as separate projects.</span></span> <span data-ttu-id="02bdf-295">應用程式具有不同的執行時間特性和安全性模型。</span><span class="sxs-lookup"><span data-stu-id="02bdf-295">The applications have different runtime characteristics as well as security models.</span></span> <span data-ttu-id="02bdf-296">它們可能會與伺服器端 web 應用程式共用一般類型 (或 API 專案) ，而這些類型應定義在一般的共用專案中。</span><span class="sxs-lookup"><span data-stu-id="02bdf-296">They're likely to share common types with the server-side web application (or API project), and these types should be defined in a common shared project.</span></span>
...
### <a name="identity"></a><span data-ttu-id="02bdf-346">身分識別</span><span class="sxs-lookup"><span data-stu-id="02bdf-346">Identity</span></span>
...
<span data-ttu-id="02bdf-376">在 web 應用程式中執行驗證有許多常見的技巧。</span><span class="sxs-lookup"><span data-stu-id="02bdf-376">There are a number of common techniques for performing authentication in web applications.</span></span> <span data-ttu-id="02bdf-377">這些稱為「配置」。</span><span class="sxs-lookup"><span data-stu-id="02bdf-377">These are referred to as schemes.</span></span> <span data-ttu-id="02bdf-378">指定的配置將會定義上述部分或全部選項的動作。</span><span class="sxs-lookup"><span data-stu-id="02bdf-378">A given scheme will define actions for some or all of the above options.</span></span> <span data-ttu-id="02bdf-379">某些配置只支援一部分的動作，而且可能需要個別的配置來執行不支援的動作。</span><span class="sxs-lookup"><span data-stu-id="02bdf-379">Some schemes only support a subset of actions, and may require a separate scheme to perform those it does not support.</span></span> <span data-ttu-id="02bdf-380">例如，OpenId Connect (OIDC) 配置不支援登入或登出，但通常設定為使用 Cookie 驗證來進行這項持續性。</span><span class="sxs-lookup"><span data-stu-id="02bdf-380">For example, the OpenId-Connect (OIDC) scheme doesn't support Sign-in or Sign-out, but is commonly configured to use Cookie authentication for this persistence.</span></span>

<span data-ttu-id="02bdf-381">在您的 ASP.NET Core 應用程式中，您可以 `DefaultAuthenticateScheme` 針對上述的每個動作，設定和選擇性的特定配置。</span><span class="sxs-lookup"><span data-stu-id="02bdf-381">In your ASP.NET Core application, you can configure a `DefaultAuthenticateScheme` as well as optional specific schemes for each of the actions described above.</span></span> <span data-ttu-id="02bdf-382">例如，、 `DefaultChallengeScheme` 等 `DefaultForbidScheme` 。呼叫會設定 [`AddIdentity<TUser,TRole>`](https://github.com/dotnet/aspnetcore/blob/release/3.1/src/Identity/Core/src/IdentityServiceCollectionExtensions.cs#L38-L102) 應用程式的許多層面，並新增許多必要的服務。</span><span class="sxs-lookup"><span data-stu-id="02bdf-382">For example, `DefaultChallengeScheme`, `DefaultForbidScheme`, etc. Calling [`AddIdentity<TUser,TRole>`](https://github.com/dotnet/aspnetcore/blob/release/3.1/src/Identity/Core/src/IdentityServiceCollectionExtensions.cs#L38-L102) configures a number of aspects of the application and adds many required services.</span></span> <span data-ttu-id="02bdf-383">它也包含此呼叫來設定驗證配置：</span><span class="sxs-lookup"><span data-stu-id="02bdf-383">It also includes this call to configure the authentication scheme:</span></span>
...
<span data-ttu-id="02bdf-384">根據預設，這些配置會使用 cookie 來保存及重新導向至登入頁面以進行驗證。</span><span class="sxs-lookup"><span data-stu-id="02bdf-384">These schemes use cookies for persistence and redirection to login pages for authentication by default.</span></span> <span data-ttu-id="02bdf-385">這些配置適用于透過網頁瀏覽器與使用者互動的 web 應用程式，但不建議用於 Api。</span><span class="sxs-lookup"><span data-stu-id="02bdf-385">These schemes are appropriate for web applications that interact with users via web browsers, but not recommended for APIs.</span></span> <span data-ttu-id="02bdf-386">相反地，Api 通常會使用另一種形式的驗證，例如 JWT 持有人權杖。</span><span class="sxs-lookup"><span data-stu-id="02bdf-386">Instead, APIs will typically use another form of authentication, such as JWT bearer tokens.</span></span>

<span data-ttu-id="02bdf-387">Web Api 會由程式碼取用，例如 `HttpClient` 在 .net 應用程式中，以及其他架構中的對等類型。</span><span class="sxs-lookup"><span data-stu-id="02bdf-387">Web APIs are consumed by code, such as `HttpClient` in .NET applications and equivalent types in other frameworks.</span></span> <span data-ttu-id="02bdf-388">這些用戶端預期會有 API 呼叫的可用回應，或是指出發生問題（如果有的話）的狀態碼。</span><span class="sxs-lookup"><span data-stu-id="02bdf-388">These clients expect a usable response from an API call, or a status code indicating what, if any, problem has occurred.</span></span> <span data-ttu-id="02bdf-389">這些用戶端不會透過瀏覽器進行互動，而且不會轉譯或與 API 可能傳回的任何 HTML 互動。</span><span class="sxs-lookup"><span data-stu-id="02bdf-389">These clients are not interacting through a browser and do not render or interact with any HTML that an API might return.</span></span> <span data-ttu-id="02bdf-390">因此，當 API 端點未經過驗證時，不適合將用戶端重新導向至登入頁面。</span><span class="sxs-lookup"><span data-stu-id="02bdf-390">Thus, it is not appropriate for API endpoints to redirect their clients to login pages if they are not authenticated.</span></span> <span data-ttu-id="02bdf-391">另一個配置更適合。</span><span class="sxs-lookup"><span data-stu-id="02bdf-391">Another scheme is more appropriate.</span></span>

<span data-ttu-id="02bdf-392">若要為 Api 設定驗證，您可以 `PublicApi` 在 eShopOnWeb 參考應用程式中設定專案所使用的驗證，如下所示：</span><span class="sxs-lookup"><span data-stu-id="02bdf-392">To configure authentication for APIs, you might set up authentication like the following, used by the `PublicApi` project in the eShopOnWeb reference application:</span></span>
...
<span data-ttu-id="02bdf-393">雖然您可以在單一專案內設定多個不同的驗證配置，但設定單一預設配置更為簡單。</span><span class="sxs-lookup"><span data-stu-id="02bdf-393">While it is possible to configure multiple different authentication schemes within a single project, it is much simpler to configure a single default scheme.</span></span> <span data-ttu-id="02bdf-394">基於這個理由，eShopOnWeb 參考應用程式會將其 Api 區分為自己的專案， `PublicApi` `Web` 並與包含應用程式的 views 和 Razor Pages 的主要專案分開。</span><span class="sxs-lookup"><span data-stu-id="02bdf-394">For this reason, among others, the eShopOnWeb reference application separates its APIs into their own project, `PublicApi`, separate from the main `Web` project that includes the application's views and Razor Pages.</span></span>

#### <a name="authentication-in-no-locblazor-apps"></a><span data-ttu-id="02bdf-395">應用程式中的驗證 Blazor</span><span class="sxs-lookup"><span data-stu-id="02bdf-395">Authentication in Blazor apps</span></span>

<span data-ttu-id="02bdf-396">Blazor 伺服器應用程式可以利用與任何其他 ASP.NET Core 應用程式相同的驗證功能。</span><span class="sxs-lookup"><span data-stu-id="02bdf-396">Blazor Server applications can leverage the same authentication features as any other ASP.NET Core application.</span></span> <span data-ttu-id="02bdf-397">BlazorWebAssembly不過，應用程式無法使用內建的身分識別與驗證提供者，因為它們會在瀏覽器中執行。</span><span class="sxs-lookup"><span data-stu-id="02bdf-397">Blazor WebAssembly applications cannot use the built-in Identity and Authentication providers, however, since they run in the browser.</span></span> <span data-ttu-id="02bdf-398">BlazorWebAssembly應用程式可以在本機儲存使用者驗證狀態，並可存取宣告，以判斷使用者應能執行哪些動作。</span><span class="sxs-lookup"><span data-stu-id="02bdf-398">Blazor WebAssembly applications can store user authentication status locally and can access claims to determine what actions users should be able to perform.</span></span> <span data-ttu-id="02bdf-399">但是，不論應用程式內是否有任何邏輯，都應該在伺服器上執行所有驗證和授權檢查 Blazor WebAssembly ，因為使用者可以輕鬆地略過應用程式，並直接與 api 互動。</span><span class="sxs-lookup"><span data-stu-id="02bdf-399">However, all authentication and authorization checks should be performed on the server regardless of any logic implemented inside the Blazor WebAssembly app, since users can easily bypass the app and interact with the APIs directly.</span></span>

> ### <a name="references--authentication"></a><span data-ttu-id="02bdf-400">參考–驗證</span><span class="sxs-lookup"><span data-stu-id="02bdf-400">References – Authentication</span></span>
>
> - <span data-ttu-id="02bdf-401">**驗證動作和預設值**</span><span class="sxs-lookup"><span data-stu-id="02bdf-401">**Authentication Actions and Defaults**</span></span>  
>   <https://stackoverflow.com/a/52493428>
> - <span data-ttu-id="02bdf-402">**Spa 的驗證和授權**
>   <https://docs.microsoft.com/aspnet/core/security/authentication/identity-api-authorization></span><span class="sxs-lookup"><span data-stu-id="02bdf-402">**Authentication and Authorization for SPAs**
<https://docs.microsoft.com/aspnet/core/security/authentication/identity-api-authorization></span></span>
> - <span data-ttu-id="02bdf-403">**ASP.NET Core Blazor 驗證與授權**
>   <https://docs.microsoft.com/aspnet/core/blazor/security/></span><span class="sxs-lookup"><span data-stu-id="02bdf-403">**ASP.NET Core Blazor Authentication and Authorization**
<https://docs.microsoft.com/aspnet/core/blazor/security/></span></span>
> - <span data-ttu-id="02bdf-404">**安全性： ASP.NET Web Forms 和中的驗證和授權 Blazor**
>   <https://docs.microsoft.com/dotnet/architecture/blazor-for-web-forms-developers/security-authentication-authorization></span><span class="sxs-lookup"><span data-stu-id="02bdf-404">**Security: Authentication and Authorization in ASP.NET Web Forms and Blazor**
...
### <a name="authorization"></a><span data-ttu-id="02bdf-405">授權</span><span class="sxs-lookup"><span data-stu-id="02bdf-405">Authorization</span></span>

<span data-ttu-id="02bdf-406">授權的最簡單形式牽涉到限制匿名使用者的存取。</span><span class="sxs-lookup"><span data-stu-id="02bdf-406">The simplest form of authorization involves restricting access to anonymous users.</span></span> <span data-ttu-id="02bdf-407">只要將 \[Authorize\] 屬性套用至特定控制器或動作，即可達成此目的。</span><span class="sxs-lookup"><span data-stu-id="02bdf-407">This can be achieved by simply applying the \[Authorize\] attribute to certain controllers or actions.</span></span> <span data-ttu-id="02bdf-408">如果角色正在使用中，您可以進一步擴充屬性，限制屬於特定角色之使用者的存取，如下所示：</span><span class="sxs-lookup"><span data-stu-id="02bdf-408">If roles are being used, the attribute can be further extended to restrict access to users who belong to certain roles, as shown:</span></span>
...
<span data-ttu-id="02bdf-409">在本例中，屬於 HRManager 或 Finance 角色 (或兩者) 的使用者可存取 SalaryController。</span><span class="sxs-lookup"><span data-stu-id="02bdf-409">In this case, users belonging to either the HRManager or Finance roles (or both) would have access to the SalaryController.</span></span> <span data-ttu-id="02bdf-410">如果使用者必須屬於多個角色 (而不只是數個角色之一)，您可以套用屬性多次，並每次指定必要的角色。</span><span class="sxs-lookup"><span data-stu-id="02bdf-410">To require that a user belong to multiple roles (not just one of several), you can apply the attribute multiple times, specifying a required role each time.</span></span>

<span data-ttu-id="02bdf-411">將特定角色集指定為許多不同控制器和動作中的字串，可能會導致不想要的重複。</span><span class="sxs-lookup"><span data-stu-id="02bdf-411">Specifying certain sets of roles as strings in many different controllers and actions can lead to undesirable repetition.</span></span> <span data-ttu-id="02bdf-412">至少，定義這些字串常值的常數，並在您需要指定字串的任何地方使用常數。</span><span class="sxs-lookup"><span data-stu-id="02bdf-412">At a minimum, define constants for these string literals and use the constants anywhere you need to specify the string.</span></span> <span data-ttu-id="02bdf-413">您也可以設定授權原則來封裝授權規則，然後在套用授權屬性時指定原則，而不是個別的角色 \[ \] ：</span><span class="sxs-lookup"><span data-stu-id="02bdf-413">You can also configure authorization policies, which encapsulate authorization rules, and then specify the policy instead of individual roles when applying the \[Authorize\] attribute:</span></span>
...
<span data-ttu-id="02bdf-414">以此方式使用原則，您就可以分隔特定角色限制使用的動作類型，或對其套用的規則。</span><span class="sxs-lookup"><span data-stu-id="02bdf-414">Using policies in this way, you can separate the kinds of actions being restricted from the specific roles or rules that apply to it.</span></span> <span data-ttu-id="02bdf-415">稍後，如果您建立需要存取特定資源的新角色，您只要更新原則即可，而不需要在每個 \[Authorize\] 屬性上更新每個角色清單。</span><span class="sxs-lookup"><span data-stu-id="02bdf-415">Later, if you create a new role that needs to have access to certain resources, you can just update a policy, rather than updating every list of roles on every \[Authorize\] attribute.</span></span>

#### <a name="claims"></a><span data-ttu-id="02bdf-416">宣告</span><span class="sxs-lookup"><span data-stu-id="02bdf-416">Claims</span></span>

<span data-ttu-id="02bdf-417">宣告是成對的名稱和數值，代表已驗證使用者的屬性。</span><span class="sxs-lookup"><span data-stu-id="02bdf-417">Claims are name value pairs that represent properties of an authenticated user.</span></span> <span data-ttu-id="02bdf-418">例如，您可以將使用者的員工編號儲存為宣告。</span><span class="sxs-lookup"><span data-stu-id="02bdf-418">For example, you might store users' employee number as a claim.</span></span> <span data-ttu-id="02bdf-419">宣告可接著作為授權原則的一部分使用。</span><span class="sxs-lookup"><span data-stu-id="02bdf-419">Claims can then be used as part of authorization policies.</span></span> <span data-ttu-id="02bdf-420">您可以建立稱為 "EmployeeOnly" 的原則，該原則需要有稱為 "EmployeeNumber" 的宣告，如下列範例所示：</span><span class="sxs-lookup"><span data-stu-id="02bdf-420">You could create a policy called "EmployeeOnly" that requires the existence of a claim called "EmployeeNumber", as shown in this example:</span></span>
...
<span data-ttu-id="02bdf-421">此原則可接著搭配 \[Authorize\] 屬性使用，以保護任何控制器及/或動作，如上所述。</span><span class="sxs-lookup"><span data-stu-id="02bdf-421">This policy could then be used with the \[Authorize\] attribute to protect any controller and/or action, as described above.</span></span>

#### <a name="securing-web-apis"></a><span data-ttu-id="02bdf-422">保護 Web API</span><span class="sxs-lookup"><span data-stu-id="02bdf-422">Securing web APIs</span></span>

<span data-ttu-id="02bdf-423">大多數 Web API 應該實作權杖型驗證系統。</span><span class="sxs-lookup"><span data-stu-id="02bdf-423">Most web APIs should implement a token-based authentication system.</span></span> <span data-ttu-id="02bdf-424">權杖驗證為無狀態並設計成可調整。</span><span class="sxs-lookup"><span data-stu-id="02bdf-424">Token authentication is stateless and designed to be scalable.</span></span> <span data-ttu-id="02bdf-425">在權杖型驗證系統中，用戶端必須先向驗證提供者進行驗證。</span><span class="sxs-lookup"><span data-stu-id="02bdf-425">In a token-based authentication system, the client must first authenticate with the authentication provider.</span></span> <span data-ttu-id="02bdf-426">如果成功，則會向用戶端發出權杖，這只是密碼編譯方面有意義的字元字串。</span><span class="sxs-lookup"><span data-stu-id="02bdf-426">If successful, the client is issued a token, which is simply a cryptographically meaningful string of characters.</span></span> <span data-ttu-id="02bdf-427">最常見的標記格式是 JSON Web 權杖，或 JWT (通常會發音為 "jot" ) 。</span><span class="sxs-lookup"><span data-stu-id="02bdf-427">The most common format for tokens is JSON Web Token, or JWT (often pronounced "jot").</span></span> <span data-ttu-id="02bdf-428">接著，當用戶端需要發出 API 要求時，就會新增此權杖作為要求的標頭。</span><span class="sxs-lookup"><span data-stu-id="02bdf-428">When the client then needs to issue a request to an API, it adds this token as a header on the request.</span></span> <span data-ttu-id="02bdf-429">伺服器接著會驗證要求標頭中找到的權杖，再完成要求。</span><span class="sxs-lookup"><span data-stu-id="02bdf-429">The server then validates the token found in the request header before completing the request.</span></span> <span data-ttu-id="02bdf-430">圖 7-4 示範此程序。</span><span class="sxs-lookup"><span data-stu-id="02bdf-430">Figure 7-4 demonstrates this process.</span></span>

![TokenAuth](./media/image7-4.png)

<span data-ttu-id="02bdf-432">**圖 7-4。**</span><span class="sxs-lookup"><span data-stu-id="02bdf-432">**Figure 7-4.**</span></span> <span data-ttu-id="02bdf-433">Web API 的權杖型驗證</span><span class="sxs-lookup"><span data-stu-id="02bdf-433">Token-based authentication for Web APIs.</span></span>

<span data-ttu-id="02bdf-434">您可以建立自己的驗證服務、與 Azure AD 和 OAuth 整合，或使用 [IdentityServer](https://github.com/IdentityServer)等開放原始碼工具來執行服務。</span><span class="sxs-lookup"><span data-stu-id="02bdf-434">You can create your own authentication service, integrate with Azure AD and OAuth, or implement a service using an open-source tool like [IdentityServer](https://github.com/IdentityServer).</span></span>

<span data-ttu-id="02bdf-435">JWT 權杖可以內嵌使用者的宣告，您可以在用戶端或伺服器上讀取這些宣告。</span><span class="sxs-lookup"><span data-stu-id="02bdf-435">JWT tokens can embed claims about the user, which can be read on the client or server.</span></span> <span data-ttu-id="02bdf-436">您可以使用 [jwt.io](https://jwt.io/) 之類的工具來查看 jwt 權杖的內容。</span><span class="sxs-lookup"><span data-stu-id="02bdf-436">You can use a tool like [jwt.io](https://jwt.io/) to view the contents of a JWT token.</span></span> <span data-ttu-id="02bdf-437">請勿將機密資料（例如密碼或金鑰）儲存在 JTW 權杖中，因為它們的內容很容易讀取。</span><span class="sxs-lookup"><span data-stu-id="02bdf-437">Do not store sensitive data like passwords or keys in JTW tokens, since their contents are easily read.</span></span>

<span data-ttu-id="02bdf-438">使用 JWT 權杖搭配 SPA 或 Blazor WebAssembly 應用程式時，您必須將權杖儲存在用戶端的某個位置，然後將它新增至每個 API 呼叫。</span><span class="sxs-lookup"><span data-stu-id="02bdf-438">When using JWT tokens with SPA or Blazor WebAssembly applications, you must store the token somewhere on the client and then add it to every API call.</span></span> <span data-ttu-id="02bdf-439">這通常是以標頭來完成，如下列程式碼所示：</span><span class="sxs-lookup"><span data-stu-id="02bdf-439">This is typically done as a header, as the following code demonstrates:</span></span>
...
<span data-ttu-id="02bdf-440">呼叫上述方法之後，以提出的要求 `_httpClient` 將會在要求的標頭中內嵌權杖，以允許伺服器端 API 驗證及授權要求。</span><span class="sxs-lookup"><span data-stu-id="02bdf-440">After calling the above method, requests made with the `_httpClient` will have the token embedded in the request's headers, allowing the server-side API to authenticate and authorize the request.</span></span>

#### <a name="custom-security"></a><span data-ttu-id="02bdf-441">自訂安全性</span><span class="sxs-lookup"><span data-stu-id="02bdf-441">Custom Security</span></span>

<span data-ttu-id="02bdf-442">請特別小心加密、使用者成員資格或權杖產生系統的「自行」實作。</span><span class="sxs-lookup"><span data-stu-id="02bdf-442">Be especially careful about "rolling your own" implementation of cryptography, user membership, or token generation system.</span></span> <span data-ttu-id="02bdf-443">可供使用的商業及開放原始碼替代方案非常多，幾乎肯定會比自訂實作更安全。</span><span class="sxs-lookup"><span data-stu-id="02bdf-443">There are many commercial and open-source alternatives available, which will almost certainly have better security than a custom implementation.</span></span>

> ### <a name="references--security"></a><span data-ttu-id="02bdf-444">參考資料 - 安全性</span><span class="sxs-lookup"><span data-stu-id="02bdf-444">References – Security</span></span>
>
> - <span data-ttu-id="02bdf-445">**安全性文件概觀**</span><span class="sxs-lookup"><span data-stu-id="02bdf-445">**Security Docs Overview**</span></span>  
>   <https://docs.microsoft.com/aspnet/core/security/>
> - <span data-ttu-id="02bdf-446">**Enforcing SSL in an ASP.NET Core App** (在 ASP.NET Core 應用程式中強制執行 SSL)</span><span class="sxs-lookup"><span data-stu-id="02bdf-446">**Enforcing SSL in an ASP.NET Core App**</span></span>  
>   <https://docs.microsoft.com/aspnet/core/security/enforcing-ssl>
> - <span data-ttu-id="02bdf-447">**身分識別簡介**</span><span class="sxs-lookup"><span data-stu-id="02bdf-447">**Introduction to Identity**</span></span>  
>   <https://docs.microsoft.com/aspnet/core/security/authentication/identity>
> - <span data-ttu-id="02bdf-448">**授權簡介**</span><span class="sxs-lookup"><span data-stu-id="02bdf-448">**Introduction to Authorization**</span></span>  
>   <https://docs.microsoft.com/aspnet/core/security/authorization/introduction>
> - <span data-ttu-id="02bdf-449">**Azure App Service 中的 API Apps 的驗證與授權**</span><span class="sxs-lookup"><span data-stu-id="02bdf-449">**Authentication and Authorization for API Apps in Azure App Service**</span></span>  
>   <https://docs.microsoft.com/azure/app-service-api/app-service-api-authentication>
> - <span data-ttu-id="02bdf-450">**身分識別伺服器**</span><span class="sxs-lookup"><span data-stu-id="02bdf-450">**Identity Server**</span></span>  
>   <https://github.com/IdentityServer>

## <a name="client-communication"></a><span data-ttu-id="02bdf-451">用戶端通訊</span><span class="sxs-lookup"><span data-stu-id="02bdf-451">Client communication</span></span>

<span data-ttu-id="02bdf-452">除了透過 Web API 提供頁面及回應資料要求，ASP.NET Core 應用程式還可以直接與連線的用戶端通訊。</span><span class="sxs-lookup"><span data-stu-id="02bdf-452">In addition to serving pages and responding to requests for data via web APIs, ASP.NET Core apps can communicate directly with connected clients.</span></span> <span data-ttu-id="02bdf-453">此輸出通訊可以使用各種傳輸技術，最常見的是 WebSockets。</span><span class="sxs-lookup"><span data-stu-id="02bdf-453">This outbound communication can use a variety of transport technologies, the most common being WebSockets.</span></span> <span data-ttu-id="02bdf-454">ASP.NET Core SignalR 是一種程式庫，可讓您簡單地將即時伺服器對用戶端通訊功能新增至應用程式。</span><span class="sxs-lookup"><span data-stu-id="02bdf-454">ASP.NET Core SignalR is a library that makes it simple to add real-time server-to-client communication functionality to your applications.</span></span> <span data-ttu-id="02bdf-455">SignalR 支援各種傳輸技術 (包括 WebSockets)，並從開發人員擷取許多實作詳細資料。</span><span class="sxs-lookup"><span data-stu-id="02bdf-455">SignalR supports a variety of transport technologies, including WebSockets, and abstracts away many of the implementation details from the developer.</span></span>

<span data-ttu-id="02bdf-456">即時用戶端通訊 (不論是直接使用 WebSockets 或其他技術) 在許多不同的應用程式案例中都很有用。</span><span class="sxs-lookup"><span data-stu-id="02bdf-456">Real-time client communication, whether using WebSockets directly or other techniques, are useful in a variety of application scenarios.</span></span> <span data-ttu-id="02bdf-457">部分範例包括：</span><span class="sxs-lookup"><span data-stu-id="02bdf-457">Some examples include:</span></span>

- <span data-ttu-id="02bdf-458">即時聊天室應用程式</span><span class="sxs-lookup"><span data-stu-id="02bdf-458">Live chat room applications</span></span>

- <span data-ttu-id="02bdf-459">監控應用程式</span><span class="sxs-lookup"><span data-stu-id="02bdf-459">Monitoring applications</span></span>

- <span data-ttu-id="02bdf-460">工作進度更新</span><span class="sxs-lookup"><span data-stu-id="02bdf-460">Job progress updates</span></span>

- <span data-ttu-id="02bdf-461">通知</span><span class="sxs-lookup"><span data-stu-id="02bdf-461">Notifications</span></span>

- <span data-ttu-id="02bdf-462">互動式論壇應用程式</span><span class="sxs-lookup"><span data-stu-id="02bdf-462">Interactive forms applications</span></span>

<span data-ttu-id="02bdf-463">在應用程式中內建用戶端通訊時，通常有兩個元件：</span><span class="sxs-lookup"><span data-stu-id="02bdf-463">When building client communication into your applications, there are typically two components:</span></span>

- <span data-ttu-id="02bdf-464">伺服器端連線管理員 (SignalR 中樞、WebSocketManager、WebSocketHandler)</span><span class="sxs-lookup"><span data-stu-id="02bdf-464">Server-side connection manager (SignalR Hub, WebSocketManager WebSocketHandler)</span></span>

- <span data-ttu-id="02bdf-465">用戶端程式庫</span><span class="sxs-lookup"><span data-stu-id="02bdf-465">Client-side library</span></span>

<span data-ttu-id="02bdf-466">用戶端不僅限於瀏覽器，行動裝置應用程式、主控台應用程式和其他原生應用程式也可以使用 SignalR/WebSockets 來通訊。</span><span class="sxs-lookup"><span data-stu-id="02bdf-466">Clients aren't limited to browsers – mobile apps, console apps, and other native apps can also communicate using SignalR/WebSockets.</span></span> <span data-ttu-id="02bdf-467">下列簡單程式會將所有傳送至交談應用程式的內容回應到主控台，以作為 WebSocketManager 範例應用程式的一部分：</span><span class="sxs-lookup"><span data-stu-id="02bdf-467">The following simple program echoes all content sent to a chat application to the console, as part of a WebSocketManager sample application:</span></span>
...
<span data-ttu-id="02bdf-468">請考慮您的應用程式直接與用戶端應用程式通訊的方式，並考慮即時通訊是否會改善應用程式的使用者體驗。</span><span class="sxs-lookup"><span data-stu-id="02bdf-468">Consider ways in which your applications communicate directly with client applications, and consider whether real-time communication would improve your app's user experience.</span></span>

> ### <a name="references--client-communication"></a><span data-ttu-id="02bdf-469">參考資料 - 用戶端通訊</span><span class="sxs-lookup"><span data-stu-id="02bdf-469">References – Client Communication</span></span>
>
> - <span data-ttu-id="02bdf-470">**ASP.NET Core SignalR**</span><span class="sxs-lookup"><span data-stu-id="02bdf-470">**ASP.NET Core SignalR**</span></span>  
>   <https://github.com/dotnet/aspnetcore/tree/master/src/SignalR>
> - <span data-ttu-id="02bdf-471">**WebSocket Manager**</span><span class="sxs-lookup"><span data-stu-id="02bdf-471">**WebSocket Manager**</span></span>  
>   <https://github.com/radu-matei/websocket-manager>

## <a name="domain-driven-design--should-you-apply-it"></a><span data-ttu-id="02bdf-472">領域驅動設計 - 是否應該套用？</span><span class="sxs-lookup"><span data-stu-id="02bdf-472">Domain-driven design – Should you apply it?</span></span>

<span data-ttu-id="02bdf-473">領域驅動設計 (DDD) 是建置軟體的敏捷式方法，重點強調「商務領域」__。</span><span class="sxs-lookup"><span data-stu-id="02bdf-473">Domain-Driven Design (DDD) is an agile approach to building software that emphasizes focusing on the _business domain_.</span></span> <span data-ttu-id="02bdf-474">它相當重視與商務領域專家的溝通和互動，這些專家會向開發人員說明真實世界系統的運作方式。</span><span class="sxs-lookup"><span data-stu-id="02bdf-474">It places a heavy emphasis on communication and interaction with business domain expert(s) who can relate to the developers how the real-world system works.</span></span> <span data-ttu-id="02bdf-475">例如，如果您想要建置處理股票交易的系統，您的領域專家可能會是經驗豐富的股票交易員。</span><span class="sxs-lookup"><span data-stu-id="02bdf-475">For example, if you're building a system that handles stock trades, your domain expert might be an experienced stock broker.</span></span> <span data-ttu-id="02bdf-476">DDD 是專為處理複雜的大型商務問題所設計，通常不適用於較小、較簡單的應用程式，因為不值得投資了解領域及建立領域模型。</span><span class="sxs-lookup"><span data-stu-id="02bdf-476">DDD is designed to address large, complex business problems, and is often not appropriate for smaller, simpler applications, as the investment in understanding and modeling the domain is not worth it.</span></span>

<span data-ttu-id="02bdf-477">建置採用 DDD 方法的軟體時，您的小組 (包括非技術性專案關係人和參與者) 應該針對問題空間開發「通用語言」__。</span><span class="sxs-lookup"><span data-stu-id="02bdf-477">When building software following a DDD approach, your team (including non-technical stakeholders and contributors) should develop a _ubiquitous language_ for the problem space.</span></span> <span data-ttu-id="02bdf-478">換句話說，您應該針對要模型化的真實世界概念、對等軟體，以及為了保存概念而可能存在的任何結構 (例如資料庫資料表) 使用相同的用語。</span><span class="sxs-lookup"><span data-stu-id="02bdf-478">That is, the same terminology should be used for the real-world concept being modeled, the software equivalent, and any structures that might exist to persist the concept (for example, database tables).</span></span> <span data-ttu-id="02bdf-479">因此，通用語言中所述的概念應該會形成「領域模型」__ 的基礎。</span><span class="sxs-lookup"><span data-stu-id="02bdf-479">Thus, the concepts described in the ubiquitous language should form the basis for your _domain model_.</span></span>

<span data-ttu-id="02bdf-480">您的領域模型包含物件，這些物件會彼此互動以表示系統的行為。</span><span class="sxs-lookup"><span data-stu-id="02bdf-480">Your domain model comprises objects that interact with one another to represent the behavior of the system.</span></span> <span data-ttu-id="02bdf-481">這些物件的分類如下：</span><span class="sxs-lookup"><span data-stu-id="02bdf-481">These objects may fall into the following categories:</span></span>

- <span data-ttu-id="02bdf-482">[實體](https://deviq.com/entity/)，代表具有身分識別執行緒的物件。</span><span class="sxs-lookup"><span data-stu-id="02bdf-482">[Entities](https://deviq.com/entity/), which represent objects with a thread of identity.</span></span> <span data-ttu-id="02bdf-483">實體通常會儲存在使用金鑰的持續性儲存區中，稍後可予以擷取。</span><span class="sxs-lookup"><span data-stu-id="02bdf-483">Entities are typically stored in persistence with a key by which they can later be retrieved.</span></span>

- <span data-ttu-id="02bdf-484">[彙總](https://deviq.com/aggregate-pattern/)，代表應該當作一個單位保存的物件群組。</span><span class="sxs-lookup"><span data-stu-id="02bdf-484">[Aggregates](https://deviq.com/aggregate-pattern/), which represent groups of objects that should be persisted as a unit.</span></span>

- <span data-ttu-id="02bdf-485">[值物件](https://deviq.com/value-object/)，代表可根據其屬性值的總和比較的概念。</span><span class="sxs-lookup"><span data-stu-id="02bdf-485">[Value objects](https://deviq.com/value-object/), which represent concepts that can be compared on the basis of the sum of their property values.</span></span> <span data-ttu-id="02bdf-486">例如，由開始和結束日期組成的 DateRange。</span><span class="sxs-lookup"><span data-stu-id="02bdf-486">For example, DateRange consisting of a start and end date.</span></span>

- <span data-ttu-id="02bdf-487">[領域事件](https://martinfowler.com/eaaDev/DomainEvent.html)，代表系統內發生的事件，系統的其他組件對這些事件會有興趣。</span><span class="sxs-lookup"><span data-stu-id="02bdf-487">[Domain events](https://martinfowler.com/eaaDev/DomainEvent.html), which represent things happening within the system that are of interest to other parts of the system.</span></span>

<span data-ttu-id="02bdf-488">DDD 領域模型應該封裝模型內的複雜行為。</span><span class="sxs-lookup"><span data-stu-id="02bdf-488">A DDD domain model should encapsulate complex behavior within the model.</span></span> <span data-ttu-id="02bdf-489">特別是實體，不應該只是屬性集合。</span><span class="sxs-lookup"><span data-stu-id="02bdf-489">Entities, in particular, should not merely be collections of properties.</span></span> <span data-ttu-id="02bdf-490">當領域模型缺少行為且只代表系統的狀態時，即為 [Anemic 模型](https://deviq.com/anemic-model/)，DDD 中並不需要此模型。</span><span class="sxs-lookup"><span data-stu-id="02bdf-490">When the domain model lacks behavior and merely represents the state of the system, it is said to be an [anemic model](https://deviq.com/anemic-model/), which is undesirable in DDD.</span></span>

<span data-ttu-id="02bdf-491">除了這些模型類型，DDD 通常還會採用多種模式：</span><span class="sxs-lookup"><span data-stu-id="02bdf-491">In addition to these model types, DDD typically employs a variety of patterns:</span></span>

- <span data-ttu-id="02bdf-492">[儲存機制](https://deviq.com/repository-pattern/)，用於抽象化持續性詳細資料。</span><span class="sxs-lookup"><span data-stu-id="02bdf-492">[Repository](https://deviq.com/repository-pattern/), for abstracting persistence details.</span></span>

- <span data-ttu-id="02bdf-493">[Factory](https://en.wikipedia.org/wiki/Factory_method_pattern)，用於封裝複雜的物件建立。</span><span class="sxs-lookup"><span data-stu-id="02bdf-493">[Factory](https://en.wikipedia.org/wiki/Factory_method_pattern), for encapsulating complex object creation.</span></span>

- <span data-ttu-id="02bdf-494">[服務](http://gorodinski.com/blog/2012/04/14/services-in-domain-driven-design-ddd/)，用於封裝複雜行為及/或基礎結構實作詳細資料。</span><span class="sxs-lookup"><span data-stu-id="02bdf-494">[Services](http://gorodinski.com/blog/2012/04/14/services-in-domain-driven-design-ddd/), for encapsulating complex behavior and/or infrastructure implementation details.</span></span>

- <span data-ttu-id="02bdf-495">[命令](https://en.wikipedia.org/wiki/Command_pattern)，用於分離發出命令與執行命令本身。</span><span class="sxs-lookup"><span data-stu-id="02bdf-495">[Command](https://en.wikipedia.org/wiki/Command_pattern), for decoupling issuing commands and executing the command itself.</span></span>

- <span data-ttu-id="02bdf-496">[規格](https://deviq.com/specification-pattern/)，用於封裝查詢詳細資料。</span><span class="sxs-lookup"><span data-stu-id="02bdf-496">[Specification](https://deviq.com/specification-pattern/), for encapsulating query details.</span></span>

<span data-ttu-id="02bdf-497">DDD 也建議使用上述的全新架構，允許鬆散結合、封裝，以及可使用單元測試輕鬆驗證的程式碼。</span><span class="sxs-lookup"><span data-stu-id="02bdf-497">DDD also recommends the use of the Clean Architecture discussed previously, allowing for loose coupling, encapsulation, and code that can easily be verified using unit tests.</span></span>

### <a name="when-should-you-apply-ddd"></a><span data-ttu-id="02bdf-498">何時應該套用 DDD</span><span class="sxs-lookup"><span data-stu-id="02bdf-498">When should you apply DDD</span></span>

<span data-ttu-id="02bdf-499">DDD 非常適合大型的應用程式，具有重要的企業 (而不只是技術) 的複雜度。</span><span class="sxs-lookup"><span data-stu-id="02bdf-499">DDD is well suited to large applications with significant business (not just technical) complexity.</span></span> <span data-ttu-id="02bdf-500">該應用程式需要有領域專家的知識。</span><span class="sxs-lookup"><span data-stu-id="02bdf-500">The application should require the knowledge of domain experts.</span></span> <span data-ttu-id="02bdf-501">領域模型本身應該有代表商務規則和互動的明顯行為，而不只是在資料存放區中儲存及擷取各種記錄的目前狀態。</span><span class="sxs-lookup"><span data-stu-id="02bdf-501">There should be significant behavior in the domain model itself, representing business rules and interactions beyond simply storing and retrieving the current state of various records from data stores.</span></span>

### <a name="when-shouldnt-you-apply-ddd"></a><span data-ttu-id="02bdf-502">何時不應該套用 DDD</span><span class="sxs-lookup"><span data-stu-id="02bdf-502">When shouldn't you apply DDD</span></span>

<span data-ttu-id="02bdf-503">DDD 牽涉到投資模型、架構和通訊，這對較小型的應用程式，或基本上只有 CRUD (建立/讀取/更新/刪除) 的應用程式，可能並不需要。</span><span class="sxs-lookup"><span data-stu-id="02bdf-503">DDD involves investments in modeling, architecture, and communication that may not be warranted for smaller applications or applications that are essentially just CRUD (create/read/update/delete).</span></span> <span data-ttu-id="02bdf-504">如果您選擇讓應用程式採用 DDD，但發現您的領域具有不含任何行為的 Anemic 模型，您可能需要重新思考做法。</span><span class="sxs-lookup"><span data-stu-id="02bdf-504">If you choose to approach your application following DDD, but find that your domain has an anemic model with no behavior, you may need to rethink your approach.</span></span> <span data-ttu-id="02bdf-505">您的應用程式可能不需要 DDD，或者您可能需要協助來重構應用程式，將商務邏輯封裝在領域模型中，而不是資料庫或使用者介面中。</span><span class="sxs-lookup"><span data-stu-id="02bdf-505">Either your application may not need DDD, or you may need assistance refactoring your application to encapsulate business logic in the domain model, rather than in your database or user interface.</span></span>

<span data-ttu-id="02bdf-506">混合式方法只會將 DDD 用於應用程式的異動或較複雜區域，而不會用於較簡單的 CRUD 或應用程式的唯讀部分。</span><span class="sxs-lookup"><span data-stu-id="02bdf-506">A hybrid approach would be to only use DDD for the transactional or more complex areas of the application, but not for simpler CRUD or read-only portions of the application.</span></span> <span data-ttu-id="02bdf-507">比方說，如果您要查詢資料來顯示報表或視覺化儀表板的資料，則不需要匯總的條件約束。</span><span class="sxs-lookup"><span data-stu-id="02bdf-507">For instance, you don't need the constraints of an Aggregate if you're querying data to display a report or to visualize data for a dashboard.</span></span> <span data-ttu-id="02bdf-508">針對這類需求，最好是使用個別且較簡單的讀取模型。</span><span class="sxs-lookup"><span data-stu-id="02bdf-508">It's perfectly acceptable to have a separate, simpler read model for such requirements.</span></span>

> ### <a name="references--domain-driven-design"></a><span data-ttu-id="02bdf-509">參考資料 - 領域驅動設計</span><span class="sxs-lookup"><span data-stu-id="02bdf-509">References – Domain-Driven Design</span></span>
>
> - <span data-ttu-id="02bdf-510">**DDD 簡單說明 (StackOverflow 解答)**</span><span class="sxs-lookup"><span data-stu-id="02bdf-510">**DDD in Plain English (StackOverflow Answer)**</span></span>  
>   <https://stackoverflow.com/questions/1222392/can-someone-explain-domain-driven-design-ddd-in-plain-english-please/1222488#1222488>

## <a name="deployment"></a><span data-ttu-id="02bdf-511">部署</span><span class="sxs-lookup"><span data-stu-id="02bdf-511">Deployment</span></span>

<span data-ttu-id="02bdf-512">不論應用程式的裝載位置，部署 ASP.NET Core 應用程式的程序都包含幾個步驟。</span><span class="sxs-lookup"><span data-stu-id="02bdf-512">There are a few steps involved in the process of deploying your ASP.NET Core application, regardless of where it will be hosted.</span></span> <span data-ttu-id="02bdf-513">第一個步驟是發佈應用程式，您可以使用 CLI 命令來完成此操作 `dotnet publish` 。</span><span class="sxs-lookup"><span data-stu-id="02bdf-513">The first step is to publish the application, which can be done using the `dotnet publish` CLI command.</span></span> <span data-ttu-id="02bdf-514">這會編譯應用程式，並將執行應用程式所需的所有檔案都放在指定的資料夾中。</span><span class="sxs-lookup"><span data-stu-id="02bdf-514">This will compile the application and place all of the files needed to run the application into a designated folder.</span></span> <span data-ttu-id="02bdf-515">當您從 Visual Studio 部署時，則會自動為您執行此步驟。</span><span class="sxs-lookup"><span data-stu-id="02bdf-515">When you deploy from Visual Studio, this step is performed for you automatically.</span></span> <span data-ttu-id="02bdf-516">publish 資料夾包含應用程式及其相依性的 .exe 和 .dll 檔案。</span><span class="sxs-lookup"><span data-stu-id="02bdf-516">The publish folder contains .exe and .dll files for the application and its dependencies.</span></span> <span data-ttu-id="02bdf-517">獨立應用程式還會包含 .NET 執行階段版本。</span><span class="sxs-lookup"><span data-stu-id="02bdf-517">A self-contained application will also include a version of the .NET runtime.</span></span> <span data-ttu-id="02bdf-518">ASP.NET Core 應用程式也會包含組態檔、靜態用戶端資產和 MVC 檢視。</span><span class="sxs-lookup"><span data-stu-id="02bdf-518">ASP.NET Core applications will also include configuration files, static client assets, and MVC views.</span></span>

<span data-ttu-id="02bdf-519">ASP.NET Core 應用程式是主控台應用程式，必須在伺服器開機時啟動，並在應用程式 (或伺服器) 損毀時重新啟動。</span><span class="sxs-lookup"><span data-stu-id="02bdf-519">ASP.NET Core applications are console applications that must be started when the server boots and restarted if the application (or server) crashes.</span></span> <span data-ttu-id="02bdf-520">您可以使用處理序管理員來自動化此程序。</span><span class="sxs-lookup"><span data-stu-id="02bdf-520">A process manager can be used to automate this process.</span></span> <span data-ttu-id="02bdf-521">ASP.NET Core 最常見的處理序管理員是 Linux 上的 Nginx 和 Apache，以及 Windows 上的 IIS 或 Windows 服務。</span><span class="sxs-lookup"><span data-stu-id="02bdf-521">The most common process managers for ASP.NET Core are Nginx and Apache on Linux and IIS or Windows Service on Windows.</span></span>

<span data-ttu-id="02bdf-522">除了進程管理員之外，ASP.NET Core 的應用程式也可以使用反向 proxy 伺服器。</span><span class="sxs-lookup"><span data-stu-id="02bdf-522">In addition to a process manager, ASP.NET Core applications may use a reverse proxy server.</span></span> <span data-ttu-id="02bdf-523">反向 Proxy 伺服器會從網際網路接收 HTTP 要求，並在進行一些初步處理後，將其轉送至 Kestrel。</span><span class="sxs-lookup"><span data-stu-id="02bdf-523">A reverse proxy server receives HTTP requests from the Internet and forwards them to Kestrel after some preliminary handling.</span></span> <span data-ttu-id="02bdf-524">反向 proxy 伺服器可為應用程式提供一層安全性。</span><span class="sxs-lookup"><span data-stu-id="02bdf-524">Reverse proxy servers provide a layer of security for the application.</span></span> <span data-ttu-id="02bdf-525">Kestrel 也不支援在相同的連接埠上裝載多個應用程式，因此無法搭配使用主機標頭等技術，以允許在相同的連接埠和 IP 位址上裝載多個應用程式。</span><span class="sxs-lookup"><span data-stu-id="02bdf-525">Kestrel also doesn't support hosting multiple applications on the same port, so techniques like host headers cannot be used with it to enable hosting multiple applications on the same port and IP address.</span></span>

![Kestrel 到網際網路](./media/image7-5.png)

<span data-ttu-id="02bdf-527">**圖 7-5**.</span><span class="sxs-lookup"><span data-stu-id="02bdf-527">**Figure 7-5**.</span></span> <span data-ttu-id="02bdf-528">ASP.NET 裝載在反向 proxy 伺服器後方的 Kestrel 中</span><span class="sxs-lookup"><span data-stu-id="02bdf-528">ASP.NET hosted in Kestrel behind a reverse proxy server</span></span>

<span data-ttu-id="02bdf-529">在另一個案例中，反向 Proxy 可能有助於使用 SSL/HTTPS 來保護多個應用程式。</span><span class="sxs-lookup"><span data-stu-id="02bdf-529">Another scenario in which a reverse proxy can be helpful is to secure multiple applications using SSL/HTTPS.</span></span> <span data-ttu-id="02bdf-530">在此情況下，只有反向 Proxy 需要設定 SSL。</span><span class="sxs-lookup"><span data-stu-id="02bdf-530">In this case, only the reverse proxy would need to have SSL configured.</span></span> <span data-ttu-id="02bdf-531">反向 Proxy 伺服器與 Kestrel 之間的通訊可透過 HTTP 進行，如圖 7-6 所示。</span><span class="sxs-lookup"><span data-stu-id="02bdf-531">Communication between the reverse proxy server and Kestrel could take place over HTTP, as shown in Figure 7-6.</span></span>

![裝載在 HTTPS 安全反向 proxy 伺服器後方的 ASP.NET](./media/image7-6.png)

<span data-ttu-id="02bdf-533">**圖 7-6**。</span><span class="sxs-lookup"><span data-stu-id="02bdf-533">**Figure 7-6**.</span></span> <span data-ttu-id="02bdf-534">裝載在 HTTPS 安全反向 proxy 伺服器後方的 ASP.NET</span><span class="sxs-lookup"><span data-stu-id="02bdf-534">ASP.NET hosted behind an HTTPS-secured reverse proxy server</span></span>

<span data-ttu-id="02bdf-535">一個越來越普及的方法，是將 ASP.NET Core 應用程式裝載於 Docker 容器，該容器接著可在本機裝載或部署至 Azure 進行雲端式裝載。</span><span class="sxs-lookup"><span data-stu-id="02bdf-535">An increasingly popular approach is to host your ASP.NET Core application in a Docker container, which then can be hosted locally or deployed to Azure for cloud-based hosting.</span></span> <span data-ttu-id="02bdf-536">Docker 容器可能會包含您的應用程式程式碼，該程式碼會在 Kestrel 上執行，並部署在反向 Proxy 伺服器後方，如上所示。</span><span class="sxs-lookup"><span data-stu-id="02bdf-536">The Docker container could contain your application code, running on Kestrel, and would be deployed behind a reverse proxy server, as shown above.</span></span>

<span data-ttu-id="02bdf-537">如果您在 Azure 上裝載應用程式，您可以使用 Microsoft Azure 應用程式閘道作為專用虛擬設備來提供數項服務。</span><span class="sxs-lookup"><span data-stu-id="02bdf-537">If you're hosting your application on Azure, you can use Microsoft Azure Application Gateway as a dedicated virtual appliance to provide several services.</span></span> <span data-ttu-id="02bdf-538">除了作為個別應用程式的反向 Proxy，應用程式閘道也可能提供下列功能：</span><span class="sxs-lookup"><span data-stu-id="02bdf-538">In addition to acting as a reverse proxy for individual applications, Application Gateway can also offer the following features:</span></span>

- <span data-ttu-id="02bdf-539">HTTP 負載平衡</span><span class="sxs-lookup"><span data-stu-id="02bdf-539">HTTP load balancing</span></span>

- <span data-ttu-id="02bdf-540">SSL 卸載 (SSL 僅限網際網路)</span><span class="sxs-lookup"><span data-stu-id="02bdf-540">SSL offload (SSL only to Internet)</span></span>

- <span data-ttu-id="02bdf-541">端對端 SSL</span><span class="sxs-lookup"><span data-stu-id="02bdf-541">End to End SSL</span></span>

- <span data-ttu-id="02bdf-542">多網站路由 (單一應用程式閘道上最多可合併 20 個網站)</span><span class="sxs-lookup"><span data-stu-id="02bdf-542">Multi-site routing (consolidate up to 20 sites on a single Application Gateway)</span></span>

- <span data-ttu-id="02bdf-543">Web 應用程式防火牆</span><span class="sxs-lookup"><span data-stu-id="02bdf-543">Web application firewall</span></span>

- <span data-ttu-id="02bdf-544">WebSocket 支援</span><span class="sxs-lookup"><span data-stu-id="02bdf-544">Websocket support</span></span>

- <span data-ttu-id="02bdf-545">進階診斷</span><span class="sxs-lookup"><span data-stu-id="02bdf-545">Advanced diagnostics</span></span>

<span data-ttu-id="02bdf-546">_深入瞭解第 [10 章](development-process-for-azure.md)的 Azure 部署選項。_</span><span class="sxs-lookup"><span data-stu-id="02bdf-546">_Learn more about Azure deployment options in [Chapter 10](development-process-for-azure.md)._</span></span>

> ### <a name="references--deployment"></a><span data-ttu-id="02bdf-547">參考資料 - 部署</span><span class="sxs-lookup"><span data-stu-id="02bdf-547">References – Deployment</span></span>
>
> - <span data-ttu-id="02bdf-548">**裝載和部署概觀**</span><span class="sxs-lookup"><span data-stu-id="02bdf-548">**Hosting and Deployment Overview**</span></span>  
>   <https://docs.microsoft.com/aspnet/core/publishing/>
> - <span data-ttu-id="02bdf-549">**何時搭配使用 Kestrel 與反向 Proxy**</span><span class="sxs-lookup"><span data-stu-id="02bdf-549">**When to use Kestrel with a reverse proxy**</span></span>  
>   <https://docs.microsoft.com/aspnet/core/fundamentals/servers/kestrel#when-to-use-kestrel-with-a-reverse-proxy>
> - <span data-ttu-id="02bdf-550">**在 Docker 中裝載 ASP.NET Core 應用程式**</span><span class="sxs-lookup"><span data-stu-id="02bdf-550">**Host ASP.NET Core apps in Docker**</span></span>  
>   <https://docs.microsoft.com/aspnet/core/publishing/docker>
> - <span data-ttu-id="02bdf-551">**Azure 應用程式閘道簡介**</span><span class="sxs-lookup"><span data-stu-id="02bdf-551">**Introducing Azure Application Gateway**</span></span>  
>   <https://docs.microsoft.com/azure/application-gateway/application-gateway-introduction>

>[!div class="step-by-step"]
><span data-ttu-id="02bdf-552">[上一個](common-client-side-web-technologies.md) 
>[下一步](work-with-data-in-asp-net-core-apps.md)</span><span class="sxs-lookup"><span data-stu-id="02bdf-552">[Previous](common-client-side-web-technologies.md)
