...
在傳統的方法中，所有服務之間會共用單一資料庫，通常使用分層架構。 在微服務方法中，每個微服務都會擁有其模型/資料。 集中式資料庫方法一開始看起來較簡單，而且似乎能重複使用不同子系統中的實體，讓一切一致。 但事實上，您會得到很大的資料表，它們會服務許多不同子系統，且包含在大部分情況下不需要的屬性和資料行。 就像是嘗試使用相同的實體地圖來進行短距離的健走、一整天的汽車旅行，以及學習地理。
...
不過，當您移至微服務架構時，資料存取就會變得更複雜。 即使在微服務或系結內容中使用 ACID 交易時，請務必考慮每個微服務所擁有的資料都是該微服務的私用，而且只能透過其 API 端點同步存取（REST、gRPC、SOAP 等等），或透過訊息（AMQP 或類似）以非同步方式進行。
...
- **聖馬丁 Fowler。BoundedCoNtext** \
  <https://martinfowler.com/bliki/BoundedContext.html>

- **聖馬丁 Fowler。PolyglotPersistence** \
  <https://martinfowler.com/bliki/PolyglotPersistence.html>

- **Alberto Brandolini。具有內容對應的策略性領域導向設計** \
