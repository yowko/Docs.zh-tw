...
實體代表領域物件，而且主要是由其身分識別、連續性及一段時間的持續性所定義，而不只是由包含這些項目的屬性所定義。 正如 Eric Evans 所說,「主要由其標識定義的對象稱為實體。 實體在領域模型中很重要，因為它們是模型的基礎。 因此，您應該仔細識別及設計。

*實體的標識可以跨多個微服務或邊界上下文。*

您可以在多個限定內容或微服務之間，建立相同身分識別 (也就是相同的 `Id` 值) 但可能不同領域實體的模型。 不過，這並不表示會在多個限定內容中實作具有相同屬性和邏輯的相同實體。 相反,每個邊界上下文中的實體將其屬性和行為限制為該邊界上下文域中所需的屬性和行為。

例如,買方實體可能具有在配置檔或身份微服務(包括標識)中的用戶實體中定義的大多數人員屬性。 但訂購微服務中的購買者實體可能有較少的屬性，因為只有特定購買者資料會與訂購流程相關。 每個微服務或限定內容的內容會影響其領域模型。

除了實作資料屬性，領域實體還必須實作行為。**

DDD 中的領域實體必須實作與實體資料 (從記憶體存取的物件) 相關的領域邏輯或行為。 例如，作為訂單實體類別的一部分，您必須將商務邏輯和作業實作為工作的方法，例如新增訂單項目、資料驗證和總計算。 實體的方法負責實體的不變和規則,而不是讓這些規則跨應用程式層分佈。
...
有些人認為 Anemic 領域模型是反模式。 它其實取決於您實作的內容。 如果您要建立的微服務夠簡單 (例如 CRUD 服務)，遵循 Anemic 領域模型就不是反模式。 但是,如果您需要解決具有大量不斷變化的業務規則的微服務域的複雜性,貧血域模型可能是該微服務或邊界上下文的反模式。 在此情況下，將它設計為具有實體的豐富模型可能會相當有利於這類微服務的長期成功，因為這些實體包含資料加上行為，並實作額外的 DDD 模式 (彙總、值物件等)。
...
正如埃裡克·埃文斯所指出的,"許多對象沒有概念認同。 這些物件描述了事物的某些特徵。
...
微服務中的實體不一定會是另一個微服務中的實體，因為在後者中，限定內容可能代表不同的意思。 例如,電子商務應用程式中的位址可能根本不具有標識,因為它可能僅表示個人或公司客戶配置檔的一組屬性。 在此情況下，該地址應該會分類為值物件。 不過，在電力公用事業公司的應用程式中，客戶地址對公司領域可能很重要。 因此，地址必須含有身分識別，帳務系統才能直接連結至該地址。 在此情況下，地址應該會分類為領域實體。
...
彙總根的目的是確保彙總的一致性，它應該是透過彙總根類別中的方法或作業對彙總進行更新的唯一進入點。 您只能透過彙總根變更彙總內的實體。 它是聚合的一致性守護者,考慮到在聚合中可能需要遵守的所有不變和一致性規則。 如果您獨立變更子實體或值物件，彙總根無法確保彙總處於有效狀態。 就像是桌子鬆了桌腳一樣。 維護一致性是彙總根的主要目的。
...
- **沃恩·弗農有效的集合模式 -第 1 部份:對單個聚合建模**(來自<http://dddcommunity.org/>)
  <http://dddcommunity.org/wp-content/uploads/files/pdf_articles/Vernon_2011_1.pdf>

- **沃恩·弗農有效的集合設計 -第二部分:使聚合協同工作**(<http://dddcommunity.org/>來自 )
  <http://dddcommunity.org/wp-content/uploads/files/pdf_articles/Vernon_2011_2.pdf>

- **沃恩·弗農有效的聚合設計 ─第三部分:通過發現獲得洞察**<http://dddcommunity.org/>(來自 )
  <http://dddcommunity.org/wp-content/uploads/files/pdf_articles/Vernon_2011_3.pdf>

- **謝爾蓋·格裡布尼亞克DDD 戰術設計模式** \
  <https://www.codeproject.com/Articles/1164363/Domain-Driven-Design-Tactical-Design-Patterns-Part>

- **克裡斯·理查森使用集合式開發事務性微服務** \
...
>[前一個](ddd-oriented-microservice.md)
