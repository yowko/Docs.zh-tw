...
Entity Framework 是 .NET Framework 的一部分，有著悠久的歷史。 當您使用 .NET 時，您也應該使用在 Windows 或 Linux 上執行的 Entity Framework Core，其方式與 .NET 相同。 EF Core 是完全重寫的 Entity Framework，以較小的使用量和效能的重要改善來實行。
