...
SOLID 與您設計應用程式或微服務的內部層及減少其之間的相依性關係更為密切。 它與域無關,但與應用程式的技術設計無關。 最後一個準則是「相依性反轉」準則，它可允許您將基礎結構層與剩餘的層分離，使其可更進一步地分離 DDD 層的實作。
...
> [前一個](nosql-database-persistence-infrastructure.md)
