...
要在 Web API 專案中設定的一項重要組態，是將 DbContext 類別註冊至服務的 IoC 容器內。 您通常會在 `Startup` 類別中，藉由在 `ConfigureServices()` 方法內呼叫 `services.AddDbContext<DbContext>()` 方法來執行此動作，如下列**簡化**範例所示：
