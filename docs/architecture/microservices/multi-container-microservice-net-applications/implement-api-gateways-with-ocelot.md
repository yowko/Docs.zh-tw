...
在您的本機 Docker 主機中執行目錄微服務。 從 Visual Studio 執行完整的 eShopOnContainers 解決方案（它會執行 docker 撰寫檔案中的所有服務），或使用位於和所在資料夾的 CMD 或 PowerShell 中的下列 docker 撰寫命令來啟動目錄微服務 `docker-compose.yml` `docker-compose.override.yml` 。
...
Ocelot 設計成只能搭配 ASP.NET Core 使用。 套件目標的最新版本 `.NETCoreApp 3.1` ，因此不適用於 .NET Framework 應用程式。
...
例如，讓我們將焦點放在上述 configuration.js的其中一個重做，這是購物籃微服務的設定。
...
由於測試或調試因素的緣故，如果您想要直接存取目錄 Docker 容器（僅限開發環境），而不需要通過 API 閘道，由於「類別目錄-API」是 Docker 主機內部的 DNS 解析（docker 撰寫服務名稱所處理的服務探索），直接存取容器的唯一方式是透過 docker-compose.dev.debug.yml 中發佈的外部埠。 yml，這僅供開發測試之用，如 `http://localhost:5101/api/v1/Catalog/items/1` 下列瀏覽器所示。
...
ValidAudiences （例如「購物籃」）會與每個微服務中定義的物件相互關聯 `AddJwtBearer()` ，其位於啟動類別的 ConfigureServices （），如下列程式碼所示。
...
如果您嘗試存取任何受保護的微服務，例如購物籃微服務，並以 API 閘道（例如）為基礎的重新路由 URL `http://localhost:5202/api/v1/b/basket/1` ，則除非您提供有效的權杖，否則您將會收到401未經授權。 相反地，如果重新路由 URL 經過驗證，Ocelot 會叫用與其相關聯的任何下游配置（內部微服務 URL）。
...
使用 Kubernetes 時（例如在 Azure Kubernetes Service 叢集），您通常會透過以*Nginx*為基礎的[Kubernetes 輸入層](https://kubernetes.io/docs/concepts/services-networking/ingress/)來統一所有 HTTP 要求。
...
部署至 Kubernetes 時，每個 Ocelot API 閘道都會針對每個執行 API 閘道的_pod_使用不同的 "configuration.json" 檔案。 這些「configuration.json」檔案是藉由裝載（原本是使用 deploy.ps1 腳本）所建立，並以名為 ' ocelot ' 的 Kubernetes _config 對應_為基礎。 每個容器會將其相關的設定檔掛接到名為的容器檔案夾中 `/app/configuration` 。

在 eShopOnContainers 的原始程式碼檔案中，可以在資料夾中找到原始的「configuration.json」檔案 `k8s/ocelot/` 。 每個 BFF/APIGateway 都有一個檔案。
...
- **用戶端中的服務探索整合 Ocelot 與 Consul 或 Eureka** \
