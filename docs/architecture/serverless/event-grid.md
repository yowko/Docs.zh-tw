...
除了 `data` 屬性以外，訊息的所有內容都是標準的。 您可以檢查訊息，並使用 `eventType` 和 `dataVersion` 將裝載的自訂部分還原序列化。
