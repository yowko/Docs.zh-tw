...
gRPC services 提供兩種方法來傳回資料集或物件清單。 通訊協定緩衝區訊息規格會使用 `repeated` 關鍵字，來宣告另一個訊息中的清單或訊息陣列。 GRPC 服務規格會使用 `stream` 關鍵字來宣告長時間執行的持續性連線，透過此連接會傳送多個訊息，並可個別處理。 `stream` 功能也可以用於長時間執行的時態性資料（例如通知或記錄訊息），但本章會考慮使用它來傳回單一資料集。
...
如果用戶端在開始處理之前需要所有資料，而且資料集夠小，可在記憶體中建立，則即使在伺服器的記憶體中實際建立資料集的速度較慢，也請考慮使用 `repeated` 欄位。
