...
本章將詳細說明如何將 ASP.NET Core gRPC 服務和使用此服務的網站部署至 Kubernetes 叢集。 使用的範例應用程式可從 GitHub 上的[dotnet-架構/grpc-wcf 開發人員存放庫取得。](https://github.com/dotnet-architecture/grpc-for-wcf-developers/tree/master/KubernetesSample)
...
若要檢查您的叢集是否正在執行且可供存取，請執行 `kubectl version` 命令。
...
使用 `kubectl` 來套用 `namespace.yml` 檔案，並檢查是否已成功建立命名空間。
...
使用 `kubectl` 來套用 `stockdata.yml` 檔案，並檢查是否已建立部署和服務。
...
使用 `kubectl` 來套用 `stockweb.yml` 檔案，並檢查是否已建立部署和服務。
