...
- 根據 Content-type 標頭，將內容串流為文字或位元組。
     > [!注意} 預設會停用內容記錄。 若要啟用它， `Diagnostics.IsLoggingContentEnabled`請`true`將`ClientOptions`設定為中的。
...
如果您要執行追蹤接聽程式，您可以`CreateTraceLogger`使用方法來記錄標準 .net 事件追蹤機制（[`System.Diagnostics.Tracing`](https://docs.microsoft.com/dotnet/api/system.diagnostics.tracing)）。 如需 .NET 中事件追蹤的詳細資訊，請參閱[追蹤](https://docs.microsoft.com/dotnet/framework/debug-trace-profile/trace-listeners)接聽程式。 這個範例會指定詳細資訊的記錄層級：
...
如先前所述，您必須註冊事件接聽程式，以從 Azure SDK for .NET 接收記錄訊息。 如果您不想使用上述簡化的方法來執行完整的記錄，您可以建立`AzureEventSourceListener`類別的實例，並將您所撰寫的回呼函數傳遞給它。 這個方法會接收您可以處理的記錄檔訊息，不過您必須這麼做。 此外，當您建立實例時，可以指定要包含的記錄層級。
...
- [在 Azure App Service 中啟用應用程式的診斷記錄功能](https://docs.microsoft.com/azure/app-service/troubleshoot-diagnostic-logs)
- 查看[Azure 安全性記錄和審核](https://docs.microsoft.com/azure/security/fundamentals/log-audit)選項
- 瞭解如何使用[Azure 平臺記錄](https://docs.microsoft.com/azure/azure-monitor/platform/platform-logs-overview)
- 深入瞭解[.Net Core 記錄和追蹤](https://docs.microsoft.com/dotnet/core/diagnostics/logging-tracing)
