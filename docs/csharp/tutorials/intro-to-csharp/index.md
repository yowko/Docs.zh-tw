...
[字串內插補點](interpolated-strings.yml)教學課程示範如何在字串中插入值。 您將會學到如何使用內嵌 C# 運算式建立插入字串，以及如何控制輸出字串的格式。 此教學課程也可以[在您的本機電腦上執行](interpolated-strings-local.md)。

此教學課程假設您已完成 [Hello World](hello-world.yml)、[C# 中的數字](numbers-in-csharp.yml)與[分支和迴圈](branches-and-loops.yml)課程。
