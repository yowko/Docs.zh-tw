...
您可以在不同的環境中嘗試這些教學課程。 您將瞭解的概念都相同。 其差異在於您偏好的體驗：

- [在您的瀏覽器中，于檔平臺上](hello-world.yml)：此體驗會在檔頁面中內嵌可執行檔 c # 程式碼視窗。 您在瀏覽器中撰寫並執行 c # 程式碼。
- [在 Microsoft Learn 體驗中](https://docs.microsoft.com/learn/paths/csharp-first-steps/)。 此學習路徑包含幾個教學 c # 基本概念的課程模組。
- 在系結程式的[Jupyter 中](https://mybinder.org/v2/gh/dotnet/try-samples/master?filepath=hello-csharp%2Fhello-world.ipynb)。 您可以在系結器上的 Jupyter 筆記本中，試驗 c # 程式碼。
- [在您的本機電腦上](numbers-in-csharp-local.md)。 在線上流覽之後，您可以在電腦上[下載](https://dotnet.microsoft.com/download).NET Core SDK 和組建程式。
...
本教學課程假設您已完成[Hello world](hello-world.yml)和[c # 中的數位](numbers-in-csharp.yml)課程。
