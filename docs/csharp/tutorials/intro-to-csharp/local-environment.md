...
.NET 教程[Hello World 在 10 分鐘內](https://dotnet.microsoft.com/learn/dotnet/hello-world-tutorial/intro)提供了在 Windows、Linux 或 macOS 上設置本地開發環境的說明。
...
您將使用 命令[`dotnet new`](../../../core/tools/dotnet-new.md)創建應用程式。 此命令會產生應用程式所需的檔案和資產。 C# 教學課程簡介都是使用 `console` 應用程式類型。 一旦您掌握基本概念，就可以擴展到其他應用程式類型。

您將使用的其他命令是[`dotnet build`](../../../core/tools/dotnet-build.md)生成可執行檔並[`dotnet run`](../../../core/tools/dotnet-run.md)運行可執行檔。
