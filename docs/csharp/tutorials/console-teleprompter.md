...
請將該類別放在新檔案中，然後如以上所示，將該類別包含在 `TeleprompterConsole` 命名空間中。 您還必須新增 `using static` 陳述式，才能在不使用封入類別或命名空間名稱的情況下參考 `Min` 和 `Max` 方法。 [`using static`](../language-reference/keywords/using-static.md) 陳述式會從一個類別匯入方法。 這與到目前為止所使用的 `using` 陳述式形成對比，該陳述式會從命名空間匯入所有類別。
