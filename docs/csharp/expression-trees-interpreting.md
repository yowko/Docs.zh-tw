...
根節點是 `LambdaExpression`。 為了取得運算子右手邊的有趣程式碼 `=>` ，您需要尋找的其中一個子系 `LambdaExpression` 。 我們將對本節中的所有運算式執行此作業。 父節點無法協助我們找到 `LambdaExpression` 的傳回型別。
...
此演算法是可瀏覽任意 `LambdaExpression` 的演算法基礎。 有許多漏洞，也就是我所建立的程式碼只會尋找可能會遇到的一組可能運算式樹狀結構節點的極小範例。 不過，您仍然可以從其產生的結果獲得相當程度的了解 (`Visitor.CreateFromExpression` 方法中的預設案例會在遇到新的節點類型時，將訊息列印至錯誤主控台。 這樣一來，您就知道要新增運算式類型)。
...
如果您確實透過造訪者執行此運算式，您會看到此輸出，驗證簡單的加法運算式是否為 *左方關聯*。
...
如果運算式已是委派，則可以這樣做，但我無法在其運算式樹狀架構形式中呼叫運算式。 在 [建立運算式樹狀架構](expression-trees-building.md)的一節中，您將瞭解克服這些限制的技巧。
