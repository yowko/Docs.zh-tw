...
根節點是 `LambdaExpression`。 為了在運算子右手邊取得有趣的程式碼 `=>` ，您需要尋找的其中一個子系 `LambdaExpression` 。 我們將對本節中的所有運算式執行此作業。 父節點無法協助我們找到 `LambdaExpression` 的傳回型別。
...
此演算法是可瀏覽任意 `LambdaExpression` 的演算法基礎。 其中有許多漏洞，也就是我所建立的程式碼只會尋找可能會遇到的一組可能的運算式樹狀節點的小型範例。 不過，您仍然可以從其產生的結果獲得相當程度的了解 (`Visitor.CreateFromExpression` 方法中的預設案例會在遇到新的節點類型時，將訊息列印至錯誤主控台。 這樣一來，您就知道要新增運算式類型)。
...
如果您透過訪客執行這個運算式，就會看到這個輸出，確認簡單的加法運算式是*左關聯*的。
...
如果運算式已是委派，則可以這樣做，但我無法在其運算式樹狀架構形式中呼叫運算式。 在[建立運算式樹狀架構](expression-trees-building.md)的一節中，您將學習克服這些限制的技巧。
...
最後，我在本文中使用的程式庫是為了示範和學習所建立。 它不會經過最佳化。 我寫了它，以清楚地使用結構，並強調用來流覽節點並分析該處的技術。 生產環境的實作會比我更注重效能。
