...
**語法視覺化檢視**允許在 Visual Studio IDE 內的當前活動編輯器視窗中檢查 C# 或 Visual Basic 代碼檔的語法樹。 通過按一下 **"查看** > **其他 Windows** > **語法視覺化檢視**"，可以啟動視覺化檢視。  您也可以使用右上角的 [快速啟動]**** 工具列。 輸入 "syntax"，開啟語法視覺化檢視**** 的命令應該會出現。
...
使用 **"檔** > **新專案"** 命令創建新專案。 您可以創建視覺化基本專案或 C# 專案。 當 Visual Studio 開啟這個專案的主要程式碼檔案時，視覺化檢視會顯示其語法樹狀目錄。 您可以在此 Visual Studio 實例中打開任何現有的 C# / Visual Basic 檔，視覺化檢視將顯示該檔的語法樹。 如果您在 Visual Studio 內開啟了多個程式碼檔案，視覺化檢視會顯示目前使用中程式碼檔案 (具有鍵盤焦點的程式碼檔案) 的語法樹狀目錄。
...
![視覺化視覺化基本語法樹](media/syntax-visualizer/visualize-visual-basic.png)
...
* 在程式碼編輯器中，按一下或選取文字。 在前面的視覺化基本示例中，如果在代碼編輯器中選擇包含"模組 1"的行，視覺化檢視將自動導航到樹中的相應 Module語句節點。
...
對與上述視覺基本示例中的方法對應的方法對應`Main()`的**SubBlock**節點嘗試相同的嘗試。 視覺化檢視會顯示看似如下的語法圖：

![查看視覺化基本語法圖](media/syntax-visualizer/visual-basic-syntax-graph.png)
...
前面的示例也可以在視覺化基本版中複製。 鍵入`Dim x As Double = 1 + 1`視覺化基本檔。 在程式碼編輯器視窗中選取運算式 `1 + 1`。 視覺化檢視會在視覺化檢視中反白顯示對應的 **AddExpression** 節點。 對這個 **AddExpression** 重複上述步驟，您應該會看到相同的結果。

在視覺化基礎知識中檢查更多代碼。 使用以下代碼更新主視覺化基本檔：
...
上述視覺化基本示例可以輕鬆地在 C# 中複製。 輸入 `using C = System.Console;` 取代 `Imports C = System.Console`作為別名。 C# 的上述步驟會在視覺化檢視視窗中產生相同的結果。
