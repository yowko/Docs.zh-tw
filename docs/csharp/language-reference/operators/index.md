...
## <a name="primary-operators"></a>主要運算子  
 ...
 [x?.y](../../../csharp/language-reference/operators/null-conditional-operators.md) – null 條件成員存取。  如果左運算元為 `null`，則傳回 `null`。  
 
 [x?[y]](../../../csharp/language-reference/operators/null-conditional-operators.md) - null 條件式索引存取。 如果左運算元為 `null`，則傳回 `null`。
