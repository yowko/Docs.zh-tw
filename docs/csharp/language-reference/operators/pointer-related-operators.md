...
如需固定和可移動變數的詳細資訊，請參閱 [C# 語言規格](~/_csharplang/spec/unsafe-code.md#fixed-and-moveable-variables)的[固定和可移動變數](~/_csharplang/spec/introduction.md)一節。
...
`[]`陣列元素或索引子存取[也可以使用 ](member-access-operators.md#indexer-operator-) 運算子。
...
- `p + n` 和 `n + p` 運算式都會產生因為將 `T*` 新增到 `n * sizeof(T)` 指定位址而得到的 `p` 型別指標。
- `p - n` 運算式會產生因為從 `T*` 指定的位址減去 `n * sizeof(T)` 而得到的 `p` 型別指標。
...
這兩個運算子都支援兩種形式：後置 (`p++` 和 `p--`) 及前置 (`++p` 和 `--p`)。 `p++` 和 `p--` 的結果是作業*之前*`p` 的值。 `++p` 和 `--p` 的結果是作業*之後*`p` 的值。
