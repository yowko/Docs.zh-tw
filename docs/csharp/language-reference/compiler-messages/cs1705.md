...
- 只參考相同 DLL 的 1.0 版，但是會存取 2.0 版之類別的用戶端。  
 ...
 下列程式碼會建立相同 DLL 中的第一個。 有關如何生成金鑰檔的資訊，請參閱[-鍵檔 （C# 編譯器選項）。](../compiler-options/keyfile-compiler-option.md)  
...
- 使用下列命令進行編譯，以便將 DLL 2.0 版的參考加入至 CS1705d.cs：  
     ...
     雖然程式會在您使用此命令時編譯，但是還不會執行。 要使程式能夠運行，可以提供一個[\<](../../../framework/configure-apps/file-schema/runtime/assemblyidentity-element-for-runtime.md)[\<](../../../framework/configure-apps/file-schema/runtime/codebase-element.md)[\<應用程式佈建檔，該檔包含依存性程式集>元素](../../../framework/configure-apps/file-schema/runtime/dependentassembly-element.md)，該元素使用程式集標識>和代碼Base>子項目來指定 DLL 版本 1.0 的位置。 如需設定檔的詳細資訊，請參閱[設定應用程式](../../../framework/configure-apps/index.md)。  
