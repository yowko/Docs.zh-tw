...
- 只參考相同 DLL 的 1.0 版，但是會存取 2.0 版之類別的用戶端。  
 ...
 下列程式碼會建立相同 DLL 中的第一個。 如需如何產生金鑰檔的詳細資訊，請參閱 [-keyfile (c # 編譯器選項) ](../compiler-options/keyfile-compiler-option.md)。  
...
- 使用下列命令進行編譯，以便將 DLL 2.0 版的參考加入至 CS1705d.cs：  
     ...
     雖然程式會在您使用此命令時編譯，但是還不會執行。 若要讓程式執行，您可以提供應用程式佈建檔，其中包含使用和子專案的[ \<dependentAssembly> 元素](../../../framework/configure-apps/file-schema/runtime/dependentassembly-element.md)， [\<assemblyIdentity>](../../../framework/configure-apps/file-schema/runtime/assemblyidentity-element-for-runtime.md) [\<codeBase>](../../../framework/configure-apps/file-schema/runtime/codebase-element.md) 以指定 DLL 1.0 版的位置。 如需設定檔的詳細資訊，請參閱[設定應用程式](../../../framework/configure-apps/index.md)。  
  
## <a name="see-also"></a>另請參閱

- [extern alias](../keywords/extern-alias.md)
- [：：運算子](../operators/namespace-alias-qualifier.md)
- [使用 csc.exe 建置命令列](../compiler-options/command-line-building-with-csc-exe.md)
