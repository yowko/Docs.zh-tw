...
C#編譯器不允許使用未初始化的變數。 如果編譯器偵測到使用可能尚未初始化之變數的情形，就會產生編譯器錯誤 CS0165。 如需詳細資訊，請參閱[欄位](../../programming-guide/classes-and-structs/fields.md)。 當編譯器遇到可能導致使用未指派變數的結構時，即使您的特定程式碼不存在，也會產生此錯誤。 如此可避免明確指派的過度複雜規則。  
