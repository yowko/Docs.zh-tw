...
- [-鍵檔（C# 編譯器選項）](../compiler-options/keyfile-compiler-option.md)而不是屬性<xref:System.Reflection.AssemblyKeyFileAttribute>。  
  
- [-鍵容器（C# 編譯器選項）](../compiler-options/keycontainer-compiler-option.md)而不是<xref:System.Reflection.AssemblyKeyNameAttribute>。  
...
- [簽名頁面，專案設計器](/visualstudio/ide/reference/signing-page-project-designer)
