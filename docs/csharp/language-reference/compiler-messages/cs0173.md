...
若要解決 CS0173，請確認無論朝哪個方向轉換，也無論轉換的類別，`class1` 和 `class2` 之間只有一個隱含轉換。 如需詳細資訊，請參閱[隱含數值轉換表](../keywords/implicit-numeric-conversions-table.md)和[使用者定義轉換運算子](../operators/user-defined-conversion-operators.md)。
