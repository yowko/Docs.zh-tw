...
## <a name="example"></a>範例
 ...
 在下列範例中，因為 C# 中的 [bool](../keywords/bool.md) 無法轉換成 [int](../builtin-types/integral-numeric-types.md)，所以會在兩個位置產生 CS0019。當減法運算子 `-` 套用至字串時，也會產生 CS0019。 加法運算子 `+` 可以與字串運算元一起使用，因為 `String` 類別會多載該運算子來執行字串串連。
