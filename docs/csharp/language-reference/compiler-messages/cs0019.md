...
## <a name="example"></a>範例
 ...
 在下列範例中，CS0019 會在兩個地方產生，因為 c # 中的 [bool](../builtin-types/bool.md) 無法轉換成 [int](../builtin-types/integral-numeric-types.md)。當減法運算子套用至字串時，也會產生 CS0019 `-` 。 加法運算子 `+` 可搭配字串運算元使用，因為該運算子會由類別多載 `String` 以執行字串串連。
