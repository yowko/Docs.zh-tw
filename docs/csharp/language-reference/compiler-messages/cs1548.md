...
簽署組件 'assembly' 時密碼編譯失敗 — 'reason'  
 ...
 有關詳細資訊，請參閱[：使用強式名稱](../../../standard/assembly/sign-strong-name.md)、[鍵檔 （C# 編譯器選項）](../compiler-options/keyfile-compiler-option.md)和[延遲符號 （C# 編譯器選項）](../compiler-options/delaysign-compiler-option.md)對程式集進行簽名。  
