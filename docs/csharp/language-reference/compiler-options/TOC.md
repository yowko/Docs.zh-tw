...
# [/target:appcontainerexe (建立 Windows 可執行檔)](target-appcontainerexe-compiler-option.md)
...
# [/subsystemversion (指定子系統的最低版本)](subsystemversion-compiler-option.md)
...
# [/target:winmdobj (建立可用來轉換成 Windows 執行階段二進位檔案的中繼檔案)](target-winmdobj-compiler-option.md)
