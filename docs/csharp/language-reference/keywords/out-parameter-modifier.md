...
## <a name="declaring-out-arguments"></a>宣告 `out` 引數   
 ...
 [Try 模式](/visualstudio/code-quality/ca1021-avoid-out-parameters#try-pattern-methods.md) 牽涉到傳回 `bool` 指出作業成功或失敗，並傳回作業在 `out` 引數中產生的值。 有許多剖析方法使用此模式，例如 @System.DateTime.TryParse(System.String,@System.DateTime) 方法。
