...
## 範例  
 ...
 `override` 方法提供繼承自基底類別 \(Base Class\) 的新成員實作。  由 `override` 宣告覆寫的方法，就是被覆寫的基底方法。  被覆寫的基底方法必須和 `override` 方法有相同的簽章。  如需繼承 \(Inheritance\) 的詳細資訊，請參閱[繼承](../../../fsharp/language-reference/inheritance.md)。  
  
 您不能覆寫非虛擬或靜態方法。  被覆寫的基底方法必須是 `virtual`、`abstract` 或 `override`。  
  
 `override` 宣告不能變更 `virtual` 方法的存取範圍。  `override` 方法和 `virtual` 方法都必須有相同的[存取層級修飾詞](../../../csharp/programming-guide/classes-and-structs/access-modifiers.md)。  
