...
- 因為兩個修飾詞的意義相反，所以無法使用 [sealed](./sealed.md) 修飾詞來修改抽象類別。 `sealed` 修飾詞可防止繼承類別，而 `abstract` 修飾詞需要繼承類別。  
...
- 因為抽象方法宣告未提供實際實作，所以沒有方法主體；方法宣告的結尾就是分號，而且簽章後面沒有大括號 ({ })。 例如：  
    ...
    ```  
     ...
     方法 [override](./override.md) 提供了實作，而這個方法是非抽象類別的成員。  
  
- 在抽象方法宣告中使用 [static](./static.md) 或 [virtual](./virtual.md) 修飾詞是錯誤的。  
  
 抽象屬性的行為類似抽象方法，但宣告和引動過程語法的差異除外。  
  
- 在靜態屬性上使用 `abstract` 修飾詞是錯誤的。  
  
- 包括使用 [override](./override.md) 修飾詞的屬性宣告，即可覆寫衍生類別中的抽象繼承屬性。  
  
 如需抽象類別的詳細資訊，請參閱[抽象和密封類別以及類別成員](../../programming-guide/classes-and-structs/abstract-and-sealed-classes-and-class-members.md)。  
