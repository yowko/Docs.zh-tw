...
```  
 ...
 `delegate` 是可用來封裝具名方法或匿名方法的參考型別。  委派大致類似 C\+\+ 的函式指標，但是，委派型別安全 \(Type\-Safe\) 又具有安全性。  如需委派的應用程式，請參閱[委派](../../../visual-basic/reference/command-line-compiler/index.md)和[泛型委派](../../../csharp/programming-guide/generics/generic-delegates.md)。  
