...
在下列範例中，以十進位、十六進位和二進位常值表示的 65,034 整數，從 [int](int.md) 隱含轉換成 `ushort` 值。
...
當您呼叫多載方法時，必須使用轉換。 例如，請考慮使用下列使用 `ushort` 和 [int](int.md) 參數的多載方法：
...
有一項從 `ushort` 轉換為 [int](int.md)、[uint](uint.md)、[long](long.md)、[ulong](ulong.md)、[float](float.md)、[double](double.md) 或 [decimal](decimal.md) 之預先定義的隱含轉換。

有一項從 [byte](byte.md) 或 [char](char.md) 轉換為 `ushort` 之預先定義的隱含轉換。 否則必須使用轉換來執行明確轉換。 例如，請考慮使用下列兩個 `ushort` 變數 `x` 和 `y`：
