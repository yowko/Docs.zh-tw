...
本快速入門教導您如何使用 C# [字串插補](../language-reference/tokens/interpolated.md)，在單一結果字串中插入值。 您將會撰寫 C# 程式碼，並查看程式碼編譯和執行的結果。 本快速入門包含一系列的課程，示範如何將值插入至字串，並以不同的方式設定那些值的格式。

本快速入門需要您具備可用於開發的電腦。 .NET 主題[只要 10 分鐘立即上手](https://www.microsoft.com/net/core) \(英文\) 中有關於在 Mac、PC 或 Linux 上設定本機開發環境的指示。 您可以在[本機快速入門簡介](local-environment.md)中快速檢視將會用到的命令，並取得可提供詳細資料的連結。 您也可以在瀏覽器中完成本快速入門的[互動式版本](interpolated-strings.yml)。
...
您可以在自己的開發環境中，繼續完成[清單集合](arrays-and-collections.md)快速入門中的內容。

在＜C# 參考＞的[字串插補](../language-reference/tokens/interpolated.md)主題中，深入了解字串插補。
