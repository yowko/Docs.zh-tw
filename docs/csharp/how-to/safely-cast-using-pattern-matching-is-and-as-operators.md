...
上述範例示範模式比對語法的一些功能。 語句`if (a is Mammal m)`將測試與初始化分配相結合。 只有在測試成功時，才會進行指派。 變數 `m` 僅會在已指派的內嵌 `if` 陳述式範圍內。 您隨後無法使用相同的方法存取 `m`。 前面的示例還演示如何使用[`as`運算子](../language-reference/operators/type-testing-and-cast.md#as-operator)將物件轉換為指定類型。

如果[空數值型別](../language-reference/builtin-types/nullable-value-types.md)具有值，也可以使用相同的語法進行測試，如以下示例所示：
...
如果要測試變數是否為給定類型，但不將其分配給新變數，則可以使用`is`和`as`運算子進行參考型別和空數值型別。 下列程式碼示範在模式比對推出之前，如何使用屬於 C# 語言的 `is` 和 `as` 陳述式測試變數是否屬於特定類型：
