...
<xref:System.String.Split%2A?displayProperty=nameWithType> 方法會根據一或多個分隔符號來分割輸入字串，以建立子字串陣列。 這個方法通常是在文字界限上分隔字串的最簡單方式。 它也可用來分割其他特定字元或字串的字串。
...
下列範例會使用空格、逗號、句號、冒號和索引標籤來分隔字元，並將它們傳遞至 <xref:System.String.Split%2A> 陣列中的。
