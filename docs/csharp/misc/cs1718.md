...
- 有助於避免混淆：C# 2.0 的一項新功能是可為 Null 的實值類型，類似於 T-SQL (SQL Server 所使用的程式設計語言) 中的 `null` 值。 熟悉 T-SQL 的開發人員可能擔心空數值型別對運算式（如`if (a == a)`）的影響，因為在 T-SQL 中使用三元邏輯。 如果您使用 `true` 或 `false`，即可避免可能的混淆。  
