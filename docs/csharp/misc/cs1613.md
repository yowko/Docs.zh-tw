...
- 加入具有 coclass 之組件的參考 (介面和 coclass 大部分的時間都應該在相同的組件中)。 如需詳細資訊，請參閱 [參考](../language-reference/compiler-options/reference-compiler-option.md) 或 [加入參考對話方塊](/visualstudio/ide/how-to-add-or-remove-references-by-using-the-reference-manager) 。  
