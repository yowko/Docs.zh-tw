...
區域函式的明確指派規則與 Lambda 運算式不同。 您可以從所在範圍的任何程式碼位置參考區域函式宣告。 Lambda 運算式必須先指派給委派變數，才可以存取（或透過參考 lambda 運算式的委派呼叫）。請注意，使用 lambda 運算式的版本必須先宣告並初始化 lambda 運算式，`nthFactorial` 之後才加以定義。 如果沒有這麼做的話，系統會在指派 `nthFactorial` 之前就加以參考，而導致編譯時期錯誤。
