...
區域函式的明確指派規則與 Lambda 運算式不同。 您可以從所在範圍的任何程式碼位置參考區域函式宣告。 在訪問 lambda 運算式之前，必須將其分配給委託變數（或通過引用 lambda 運算式的委託調用）。請注意，使用 lambda 運算式的版本必須在定義 lambda 運算式`nthFactorial`之前聲明和初始化該運算式。 如果沒有這麼做的話，系統會在指派 `nthFactorial` 之前就加以參考，而導致編譯時期錯誤。
