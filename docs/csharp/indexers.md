...
您可以透過變數名稱與方括弧來存取索引子。 您會將索引子引數放在方括號內：
...
第一個範例顯示產生 Mandelbrot 集合值的類別。 如需該集合背後數學的詳細資訊，請參閱[這篇文章](https://en.wikipedia.org/wiki/Mandelbrot_set)。
索引子使用兩個雙精度浮點數在 X, Y 平面中定義一個點。
