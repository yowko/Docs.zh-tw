...
|[如何：透過存取標頭資訊來串流 XML 片段 (C#)](../../../../csharp/programming-guide/concepts/linq/how-to-stream-xml-fragments-with-access-to-header-information.md)|描述如何從 <xref:System.Xml.XmlReader> 串流 XML 片段。 您可以使用這個技術來控制您應用程式的記憶體使用量。|  
|[如何：執行大型 XML 文件的串流轉換 (C#)](../../../../csharp/programming-guide/concepts/linq/how-to-perform-streaming-transform-of-large-xml-documents.md)|描述如何從 <xref:System.Xml.XmlReader> 串流 XML、轉換 XML 片段，然後使用 <xref:System.Xml.Linq.XStreamingElement> 串流輸出。|  
