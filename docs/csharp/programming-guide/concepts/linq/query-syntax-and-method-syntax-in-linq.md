...
## <a name="standard-query-operator-extension-methods"></a>標準查詢運算子擴充方法  
 ...
 若要開始使用 LINQ，您真正需要知道的擴充方法，就是如何使用正確的 `using` 指示詞，將它們帶入應用程式的範圍。 從您應用程式的觀點來看，擴充方法和一般執行個體方法都相同。  
  
 如需擴充方法的詳細資訊，請參閱[擴充方法](../../classes-and-structs/extension-methods.md)。 如需標準查詢運算子的詳細資訊，請參閱[標準查詢運算子概觀 (C#)](./standard-query-operators-overview.md)。 某些 LINQ 提供者（例如 [!INCLUDE[vbtecdlinq](~/includes/vbtecdlinq-md.md)] 和 [!INCLUDE[sqltecxlinq](~/includes/sqltecxlinq-md.md)]）會針對除了 <xref:System.Collections.Generic.IEnumerable%601>以外的其他類型，執行自己的標準查詢運算子和其他擴充方法。  
