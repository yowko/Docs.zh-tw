...
## <a name="lambda-expressions"></a>Lambda 運算式  
 ...
 若要開始使用 [!INCLUDE[vbteclinq](~/includes/vbteclinq-md.md)]，您不需要廣泛使用 Lambda。 不過，只能在方法語法中表示特定查詢，而其中有一部分需要 Lambda 運算式。 更熟悉 Lambda 之後，您會發現它們是您 [!INCLUDE[vbteclinq](~/includes/vbteclinq-md.md)] 工具箱中功能強大且靈活的工具。 如需詳細資訊，請參閱 [Lambda 運算式](../../statements-expressions-operators/lambda-expressions.md)。  
