...
description: '瞭解如何擴充 c # 中的 LINQ，以便在 LINQ 模式中查詢任何資料來源，讓用戶端輕鬆地查詢資料來源。'
ms.date: 07/20/2015
...
### <a name="in-memory-data"></a>記憶體中的資料  
 有兩種方式可讓您啟用 LINQ 查詢記憶體中的資料。 如果資料是執行的類型 <xref:System.Collections.Generic.IEnumerable%601> ，您可以使用 LINQ to Objects 來查詢資料。 如果藉由執行介面來啟用類型列舉並不合理 <xref:System.Collections.Generic.IEnumerable%601> ，您可以在該類型中定義 linq 標準查詢運算子方法，或建立擴充類型的 linq 標準查詢運算子方法。 標準查詢運算子的自訂實作 (Implementation) 應該會使用延後執行 (Deferred Execution) 來傳回結果。  
  
### <a name="remote-data"></a>遠端資料  
 啟用遠端資料源的 LINQ 查詢的最佳選項是執行 <xref:System.Linq.IQueryable%601> 介面。 不過，這與擴充資料來源之提供者 (例如 [!INCLUDE[vbtecdlinq](~/includes/vbtecdlinq-md.md)]) 不同。 在 Visual Studio 2008 中，沒有任何提供者模型可用來將現有的 LINQ 技術（例如 [!INCLUDE[vbtecdlinq](~/includes/vbtecdlinq-md.md)] ）擴充至其他資料來源類型。
  
## <a name="iqueryable-linq-providers"></a>IQueryable LINQ 提供者  
 執行的 LINQ 提供者 <xref:System.Linq.IQueryable%601> 在複雜度上會有很大的差異。 本節將討論不同層次的複雜度。  
