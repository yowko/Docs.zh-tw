...
有時候您必須轉換大型 XML 檔案並撰寫您的應用程式，讓應用程式的記憶體使用量可以預測。 如果您嘗試使用非常大的 XML 檔案填入 XML 樹狀結構，您的記憶體使用量將與檔案大小成正比 (也就是，變成過度)。 因此，您應該改用資料流技術。  
 ...
 即使您使用 [HOW TO：透過存取標頭資訊來串流 XML 片段 (C#)](./how-to-stream-xml-fragments-with-access-to-header-information.md) 中所描述的技術，如果您嘗試組合包含已轉換文件的 XML 樹狀結構，記憶體使用量將會太大。  
...
## <a name="example"></a>範例  
 下列範例會以下連結中的範例為基礎建置：[如何：透過存取標頭資訊來串流 XML 片段 (C#)](./how-to-stream-xml-fragments-with-access-to-header-information.md)。  
...
## <a name="example"></a>範例  
 下列範例也會以下連結中的範例為基礎建置：[如何：透過存取標頭資訊來串流 XML 片段 (C#)](./how-to-stream-xml-fragments-with-access-to-header-information.md)。  
