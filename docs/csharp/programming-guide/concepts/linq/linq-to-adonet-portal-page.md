...
LINQ to ADO.NET 可讓您使用語言整合式查詢 (LINQ) 程式設計模型，查詢 ADO.NET 中的任何可列舉物件。  
  
> [!NOTE]
> LINQ to ADO.NET 檔位於 .NET Framework SDK： [LINQ 和 ADO.NET](../../../../framework/data/adonet/linq-and-ado-net.md)的 ADO.NET 一節。  
  
  (LINQ) 技術有三種不同的 ADO.NET 語言整合查詢： LINQ to DataSet、 [!INCLUDE[vbtecdlinq](~/includes/vbtecdlinq-md.md)] 和 LINQ to Entities。 LINQ to DataSet 可提供更豐富且最佳化的 <xref:System.Data.DataSet> 查詢，[!INCLUDE[vbtecdlinq](~/includes/vbtecdlinq-md.md)] 可讓您直接查詢 SQL Server 資料庫結構描述，而 LINQ to Entities 可讓您查詢實體資料模型。  
