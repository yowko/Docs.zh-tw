...
若要有效地撰寫查詢，您應該了解完整查詢作業中的變數類型如何彼此相關。 如果您瞭解這些關係，您將更輕鬆地理解文檔中的 LINQ 示例和代碼示例。 此外，您將了解使用 `var` 讓變數成為隱含類型時的幕後作業。  
  
 LINQ 查詢操作在資料來源、查詢本身和查詢執行中進行強型別。 查詢中的變數類型必須與資料來源中的項目類型以及 `foreach` 陳述式中的反覆運算變數類型相容。 如果類型錯誤可以在使用者遇到它們之前進行更正，則這個強型別可確保在編譯時期攔截到它們。  
...
## <a name="queries-that-do-not-transform-the-source-data"></a>未轉換來源資料的查詢  
 下圖顯示了 LINQ 到物件查詢操作，該操作對資料不執行任何轉換。 來源包含一系列的字串，而且查詢輸出也是一系列的字串。  
...
## <a name="letting-the-compiler-infer-type-information"></a>讓編譯器推斷類型資訊  
 ...
 有關 的詳細資訊`var`，請參閱[隱式鍵入的區域變數](../../classes-and-structs/implicitly-typed-local-variables.md)。  
