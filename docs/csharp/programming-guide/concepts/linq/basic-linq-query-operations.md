...
本主題簡要介紹了 LINQ 查詢運算式以及您在查詢中執行的一些典型操作類型。 下列各主題提供更詳細的資訊：  
...
> [!NOTE]
> 如果您已經熟悉 SQL 或 XQuery 這類查詢語言，則可以略過本主題的大部分內容。 閱讀下一節中`from`的"子句"，瞭解 LINQ 查詢運算式中子句的順序。  
  
## <a name="obtaining-a-data-source"></a>取得資料來源  
 在 LINQ 查詢中，第一步是指定資料來源。 在 C# 中，如同大多數程式設計語言一樣，必須先宣告變數，才能使用變數。 在 LINQ 查詢中`from`，子句排在第一位，以便引入資料來源`customers`（ ）*和範圍變數*（`cust`。  
  
 [!code-csharp[csLINQGettingStarted#23](~/samples/snippets/csharp/VS_Snippets_VBCSharp/CsLINQGettingStarted/CS/Class1.cs#23)]  
  
 範圍變數就像 `foreach` 迴圈中的反覆項目變數，差異在於查詢運算式中沒有實際反覆項目。 執行查詢時，範圍變數將會作為 `customers` 中每個後續項目的參考。 因為編譯器可以推斷 `cust` 的類型，所以您不需要明確予以指定。 `let` 子句可以引進其他範圍變數。 有關詳細資訊，請參閱[讓子句](../../../language-reference/keywords/let-clause.md)。  
  
> [!NOTE]
> 針對 <xref:System.Collections.ArrayList> 這類非泛型資料來源，必須明確範圍變數的類型。 有關詳細資訊，請參閱如何使用[LINQ （C#）](./how-to-query-an-arraylist-with-linq.md)和[從 子句](../../../language-reference/keywords/from-clause.md)查詢 ArrayList。  
...
## <a name="joining"></a>加入  
 聯結作業會建立資料來源中未明確模型化之序列間的關聯。 例如，您可以執行聯結來尋找位置相同的所有客戶和經銷商。 在 LINQ`join`中，子句始終直接針對物件集合而不是資料庫表。  
  
 [!code-csharp[csLINQGettingStarted#36](~/samples/snippets/csharp/VS_Snippets_VBCSharp/CsLINQGettingStarted/CS/Class1.cs#36)]  
  
 在 LINQ 中，不必`join`像 SQL 那樣經常使用，因為 LINQ 中的外鍵在物件模型中表示為包含項集合的屬性。 例如，包含 `Order` 物件集合的 `Customer` 物件。 您可以使用點標記法來存取順序，而不是執行聯結：  
  
```csharp
from order in Customer.Orders...  
```  
  
 有關詳細資訊，請參閱[聯接子句](../../../language-reference/keywords/join-clause.md)。  
  
## <a name="selecting-projections"></a>選取 (投影)  
 `select` 子句會產生查詢的結果，並指定每個所傳回項目的「圖形」或類型。 例如，您可以根據計算或新物件建立指定結果包含完整 `Customer` 物件、僅一個成員、成員子集，還是某個完全不同的結果類型。 `select` 子句不只產生一份來源項目時，作業稱為「投影」**。 使用投影轉換資料是 LINQ 查詢運算式的強大功能。 如需詳細資訊，請參閱[使用 LINQ 轉換資料 (C#)](./data-transformations-with-linq.md) 和 [select 子句](../../../language-reference/keywords/select-clause.md)。  
