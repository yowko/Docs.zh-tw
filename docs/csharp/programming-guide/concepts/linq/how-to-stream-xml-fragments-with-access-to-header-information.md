...
```  
 ...
 下列範例顯示如何實作與使用從 URI 指定之檔案資料流 XML 片段的自訂座標軸方法。 這個自訂座標軸的撰寫方式是，它預期具有 `Customer`、`Name`和 `Item` 元素的檔，而且這些專案將會依照上述 `Source.xml` 檔中的順序排列。 這是簡化的實作方法。 較為複雜的實作方法則用於剖析無效的文件。  
