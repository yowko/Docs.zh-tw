...
## <a name="ienumerablet-variables-in-linq-queries"></a>LINQ 查詢中的 IEnumerable<T\> 變數  
 LINQ 查詢變數會輸入為 <xref:System.Collections.Generic.IEnumerable%601> 或衍生型別，例如 <xref:System.Linq.IQueryable%601>。 當您看到類型為 `IEnumerable<Customer>` 的查詢變數時，只表示查詢在執行時會產生由零個以上的 `Customer` 物件所組成的序列。  
