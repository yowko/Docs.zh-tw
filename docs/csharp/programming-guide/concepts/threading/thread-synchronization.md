...
-   [Managed 執行緒處理的最佳實施方針](http://msdn.microsoft.com/library/e51988e7-7f4b-4646-a06d-1416cee8d557)  
...
## <a name="synchronization-events-and-wait-handles"></a>同步處理事件和等候控制代碼  
 ...
 有兩種類型的同步處理事件︰<xref:System.Threading.AutoResetEvent> 和 <xref:System.Threading.ManualResetEvent>。 它們的差異只在於 <xref:System.Threading.AutoResetEvent> 會在啟動執行緒時即自動從發出訊號變更為未發出訊號。 相反地，<xref:System.Threading.ManualResetEvent> 允許透過其發出訊號狀態啟動任意數目的執行緒，而且只有在呼叫其 <xref:System.Threading.EventWaitHandle.Reset%2A> 方法時才會還原為未發出訊號狀態。  
  
 呼叫一個 wait 方法 (例如 <xref:System.Threading.WaitHandle.WaitOne%2A>、<xref:System.Threading.WaitHandle.WaitAny%2A> 或 <xref:System.Threading.WaitHandle.WaitAll%2A>)，即可產生等候事件的執行緒。 <xref:System.Threading.WaitHandle.WaitOne%2A?displayProperty=fullName> 會讓執行緒等到單一事件變成發出訊號、<xref:System.Threading.WaitHandle.WaitAny%2A?displayProperty=fullName> 會封鎖執行緒直到一或多個指出的事件變成發出訊號，而 <xref:System.Threading.WaitHandle.WaitAll%2A?displayProperty=fullName> 會封鎖執行緒直到所有指出的事件都變成發出訊號。 呼叫事件的 <xref:System.Threading.EventWaitHandle.Set%2A> 方法時，事件就會變成發出訊號。  
  
 在下列範例中，`Main` 函式會建立並啟動執行緒。 使用 <xref:System.Threading.WaitHandle.WaitOne%2A> 方法，新的執行緒就會等候事件。 除非執行 `Main` 函式的主要執行緒讓事件變成發出訊號，否則會暫止執行緒。 一旦事件變成發出訊號，就會傳回輔助執行緒。 在此情況下，因為事件僅用於一個執行緒啟動，所以可以使用 <xref:System.Threading.AutoResetEvent> 或 <xref:System.Threading.ManualResetEvent> 類別。  
 ...
 *Mutex* 與監視器類似，可防止多個執行緒一次同時執行程式碼區塊。 事實上，名稱 "Mutex" 是「互斥」這個詞彙的縮短格式。 不過，與監視器不同，Mutex 可以用來跨處理序同步處理執行緒。 Mutex 是透過 <xref:System.Threading.Mutex> 類別表示。  
  
 用於處理序間同步處理時，Mutex 稱為「具名 Mutex」**，因為它是要用在另一個應用程式中，因此無法透過全域或靜態變數共用。 必須指定它的名稱，以讓兩個應用程式可以存取相同的 Mutex 物件。  
  
 雖然 Mutex 可以用於處理序間執行緒同步處理，但是一般偏好使用 <xref:System.Threading.Monitor>，因為已特別針對 .NET Framework 設計監視器，因此更恰當地使用資源。 相反地，<xref:System.Threading.Mutex> 類別是 Win32 建構的包裝函式。 雖然 Mutex 比監視器的功能更為強大，但是所需的 Interop 轉換比 <xref:System.Threading.Monitor> 類別所需的 Interop 轉換耗費更大量的運算資源。 如需使用 Mutex 的範例，請參閱 [Mutex](http://msdn.microsoft.com/library/9dd06e25-12c0-4a9e-855a-452dc83803e2)。  
