...
JSON 序列化會將物件的公用屬性序列化為符合[RFC 8259 JSON 規格](https://tools.ietf.org/html/rfc8259)的字串、位元組陣列或資料流程。 若要控制 <xref:System.Text.Json.JsonSerializer> 序列化或還原序列化類別實例的方式：

* 使用 <xref:System.Text.Json.JsonSerializerOptions> 物件
* 將 <xref:System.Text.Json.Serialization> 命名空間中的屬性套用至類別或屬性
...
基本序列化使用 .NET Framework 來自動序列化物件。 唯一的要求是類別已套用 <xref:System.SerializableAttribute> 屬性。 可以使用 <xref:System.NonSerializedAttribute> 來防止特定欄位被序列化。
...
[System.web. Text. Json 總覽](../../../../standard/serialization/system-text-json-overview.md)說明如何取得 `System.Text.Json` 程式庫。

[如何在 .net 中序列化和還原序列化 JSON](../../../../standard/serialization/system-text-json-how-to.md)。 示範如何使用 <xref:System.Text.Json.JsonSerializer> 類別，在 JSON 中讀取和寫入物件資料。
