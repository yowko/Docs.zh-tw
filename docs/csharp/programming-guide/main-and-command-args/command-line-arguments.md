...
> [!NOTE]
> 要在 Windows 表單應用程式中`Main`的方法中啟用命令列參數，必須手動修改`Main`*program.cs*中的簽名。 Windows Forms 設計工具所產生的程式碼會建立沒有輸入參數的 `Main`。 您也可以使用 <xref:System.Environment.CommandLine%2A?displayProperty=nameWithType> 或 <xref:System.Environment.GetCommandLineArgs%2A?displayProperty=nameWithType>，以從主控台或 Windows 應用程式的任何點存取命令列引數。
...
1. 將以下代碼粘貼到任何文字編輯器中，然後將該檔另存為名稱*為Factorial.cs*的文字檔。
...
3. 輸入下列命令以編譯應用程式。
     ...
     如果應用程式沒有編譯錯誤，將創建名為*Factorial.exe*的可執行檔。
