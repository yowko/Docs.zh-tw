...
## 範例  
 由於這個範例使用批次檔，因此最好從命令提示字元編譯這段程式碼。  請按照 [How to: Set Environment Variables for the Visual Studio Command Line](../Topic/How%20to:%20Set%20Environment%20Variables%20for%20the%20Visual%20Studio%20Command%20Line.md)中的指示啟用命令列建置，或者是從 \[**開始**\] 功能表中的 \[**Visual Studio Tools**\] 下使用 Visual Studio 命令提示字元。  從命令提示字元，巡覽至您儲存程式的資料夾。  下列命令會編譯 MainReturnValTest.cs，並產生可執行檔 MainReturnValTest.exe。  
