...
-   在配合自己的應用程式調整這個方法時，請記得 LINQ 需要 3.5 或 4 版的 [!INCLUDE[dnprdnshort](../../../csharp/getting-started/includes/dnprdnshort-md.md)]，而且專案必須包含 System.Core.dll 的參考以及 `System.Linq` 的 `using` 指示詞。  LINQ to SQL、LINQ to XML 和 LINQ to DataSet 型別都需要額外的 `using` 指示詞和參考。  如需詳細資訊，請參閱 [How to: Create a LINQ Project](../Topic/How%20to:%20Create%20a%20LINQ%20Project.md)。  
