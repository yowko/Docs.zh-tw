...
|條件約束|描述|
|----------------|-----------------|
|`where T : struct`|類型參數必須是非空數值型別。 有關空數值型別的資訊，請參閱[空數值型別](../../language-reference/builtin-types/nullable-value-types.md)。 由於所有數值型別都有可訪問的無參數建構函式，因此`struct`約束意味著`new()`約束，並且不能與`new()`約束組合。 也不能將`struct`約束與`unmanaged`約束合併。|
|`where T : class`|型別引數必須是參考型別。 此條件約束也適用於任何類別、介面、委派或陣列型別。|
|`where T : notnull`|類型參數必須是非空類型。 參數可以是 C# 8.0 或更高版本中的不可空參考型別，也可以是不可消除的數值型別。 此條件約束也適用於任何類別、介面、委派或陣列型別。|
|`where T : unmanaged`|類型參數必須是非空[的非託管類型](../../language-reference/builtin-types/unmanaged-types.md)。 約束`unmanaged`意味著`struct`約束，不能與 或`struct`約束`new()`組合。|
|`where T : new()`|型別引數必須有公用無參數建構函式。 與其他條件約束搭配使用時，`new()` 條件約束必須是最後一個指定的。 約束`new()`不能與`struct`和`unmanaged`約束結合使用。|
...
編譯器只知道它是`T`編譯時的參考型別，並且必須使用對所有參考型別有效的預設運算子。 如果您必須測試值是否相等，則建議同時套用 `where T : IEquatable<T>` 或 `where T : IComparable<T>` 條件約束，並在任何將用來建構泛型類別的類別中實作介面。
...
- 不能`!=`使用`==`和 運算子，因為不能保證具體類型參數將支援這些運算子。
...
從 C# 8.0 開始，可以使用`notnull`約束指定類型參數必須是不可空的數值型別或不可消除的參考型別。 約束`notnull`只能在`nullable enable`上下文中使用。 如果在可忽略的上下文中添加約束，`notnull`編譯器將生成警告。

與其他約束不同，當類型參數違反`notnull`約束時，編譯器會在`nullable enable`上下文中編譯該代碼時發出警告。 如果代碼是在無虛無的上下文中編譯的，編譯器不會生成任何警告或錯誤。

## <a name="unmanaged-constraint"></a>非受控條件約束

從 C# 7.3 開始，可以使用`unmanaged`約束指定類型參數必須是不可空[的非託管類型](../../language-reference/builtin-types/unmanaged-types.md)。 `unmanaged` 條件約束可讓您撰寫可重複使用的常式來使用型別，而型別可以操作為記憶體區塊，如下列範例所示：

[!code-csharp[using the unmanaged constraint](~/samples/snippets/csharp/keywords/GenericWhereConstraints.cs#15)]

上述方法必須在 `unsafe` 內容中進行編譯，因為它在不知道是內建型別的型別上使用 `sizeof` 運算子。 如果沒有 `unmanaged` 條件約束，則 `sizeof` 運算子無法使用。

約束`unmanaged`意味著`struct`約束，不能與它結合。 由於`struct`約束意味著`new()`約束，`unmanaged`因此也不能將約束與`new()`約束組合。
