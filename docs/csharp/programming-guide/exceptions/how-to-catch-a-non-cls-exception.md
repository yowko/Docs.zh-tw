...
-   在 `catch (Exception e)` 區塊內為 <xref:System.Runtime.CompilerServices.RuntimeWrappedException>。  
     ...
     根據預設，[!INCLUDE[csprcs](~/includes/csprcs-md.md)] 組件會將非 CLS 例外狀況當成包裝例外狀況攔截。 如果您需要存取原始的例外狀況 (它可以透過 <xref:System.Runtime.CompilerServices.RuntimeWrappedException.WrappedException%2A> 屬性存取)，請使用這個方法。 本主題稍後的程序會說明如何以這種方式攔截例外狀況。  
