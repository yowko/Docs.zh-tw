...
```  
 ...
 方法引動過程需要方法的名稱，這個名稱可以是先前範例中的名稱，或是另一個運算式的結果，後面再接著括號和任何方法參數。  如需詳細資訊，請參閱 [方法](../../../csharp/programming-guide/classes-and-structs/methods.md)。  委派 \(Delegate\) 叫用則需要使用委派的名稱，並用括號括住方法參數。  如需詳細資訊，請參閱 [委派](../../../visual-basic/reference/command-line-compiler/index.md)。  如果方法傳回值，則方法叫用和委派叫用會評估為該方法以產生傳回值。  傳回 void 的方法不可以當做運算式中的值。  
  
## 查詢運算式  
 相同的運算式規則通常會套用至查詢運算式。  如需詳細資訊，請參閱 [LINQ 查詢運算式](../../../visual-basic/reference/command-line-compiler/index.md)。  
