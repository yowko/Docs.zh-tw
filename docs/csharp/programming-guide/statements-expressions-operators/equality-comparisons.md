...
### <a name="value-equality-for-floating-point-values"></a>浮點值的實值相等  
 浮點值的相等比較 ([double](../../../csharp/language-reference/keywords/double.md) 和 [float](../../../csharp/language-reference/keywords/float.md)) 有問題，因為二進位電腦上的浮點算術不精確。 如需詳細資訊，請參閱 <xref:System.Double?displayProperty=fullName> 主題中的備註。  
