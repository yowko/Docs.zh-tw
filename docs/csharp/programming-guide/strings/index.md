...
## <a name="string-vs-systemstring"></a>字串與系統.String  
...
## <a name="immutability-of-string-objects"></a>字串物件的不變性  
 ...
 有關如何基於修改（如搜索和替換原始字串上的操作）創建新字串的詳細資訊，請參閱[如何修改字串內容](../../how-to/modify-string-contents.md)。  
...
|\t|水平 Tab 鍵|0x0009|  
|\v|垂直 Tab 鍵|0x000B|  
|\u|Unicode 逸出序列 (UTF-16)|`\uHHHH`（範圍： 0000 - FFFF;示例："*" `\u00E7`|  
|\U|Unicode 逸出序列 (UTF-32)|`\U00HHHHHH`（範圍：000000 - 10FFFF;`\U0001F47D`示例："&_x1F47D;"）|  
|\x|類似 "\u" (除了變數長度之外) 的 Unicode 逸出序列|`\xH[H][H][H]`（範圍： 0 - FFFF;`\x0E7`示例`\xE7`：`\x00E7`或 = "*"|  
...
## <a name="substrings"></a>子字串  
 子字串是包含在字串中的任何字元序列。 使用 <xref:System.String.Substring%2A> 方法，來從原始字串的一部分建立新的字串。 您可以使用 <xref:System.String.IndexOf%2A> 方法，來搜尋子字串的一或多個出現位置。 使用 <xref:System.String.Replace%2A> 方法，以新字串取代所有指定的子字串。 與 <xref:System.String.Substring%2A> 方法類似，<xref:System.String.Replace%2A> 實際上會傳回新字串，並不會修改原始字串。 有關詳細資訊，請參閱[如何搜索字串](../../how-to/search-strings.md)以及如何[修改字串內容](../../how-to/modify-string-contents.md)。
...
## <a name="strings-extension-methods-and-linq"></a>字串、擴充方法和 LINQ  
 因為 <xref:System.String> 類型會實作 <xref:System.Collections.Generic.IEnumerable%601>，所以您可以對字串使用 <xref:System.Linq.Enumerable> 類別中所定義的擴充方法。 為了避免視覺雜亂，這些方法會從 <xref:System.String> 類型的 IntelliSense 中排除，不過您還是可以使用它們。 您還可以在字串上使用 LINQ 查詢運算式。 如需詳細資訊，請參閱 [LINQ 和字串](../concepts/linq/linq-and-strings.md)。  
...
|[如何串聯多個字串](../../how-to/concatenate-multiple-strings.md)|示範如何將多個字串聯結成一個的各種方式。|
|[如何使用 String.Split 解析字串](../../how-to/parse-strings-using-split.md)|包含說明如何使用 `String.Split` 方法剖析字串的程式碼範例。|  
|[如何搜索字串](../../how-to/search-strings.md)|說明如何對字串中的特定文字或模式使用搜尋。|  
...
|[在 .NET 中分析日期和時間字串](../../../standard/base-types/parsing-datetime.md)|示範如何將 "01/24/2008" 這類字串轉換為 <xref:System.DateTime?displayProperty=nameWithType> 物件。|  
