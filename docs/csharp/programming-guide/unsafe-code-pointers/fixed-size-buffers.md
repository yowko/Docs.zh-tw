...
下列的大小 `struct` 不會取決於陣列中的元素數目，因為 `pathName` 是參考：
...
固定大小的緩衝區會使用來編譯 <xref:System.Runtime.CompilerServices.UnsafeValueTypeAttribute?displayProperty=nameWithType> ，這會指示 common language runtime （CLR）類型包含可能溢位的非受控陣列。 這類似于使用[stackalloc](../../language-reference/operators/stackalloc.md)所建立的記憶體，它會自動啟用 CLR 中的緩衝區溢位偵測功能。 上一個範例顯示如何在中存在固定大小的緩衝區 `unsafe struct` 。
