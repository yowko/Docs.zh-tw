...
## 範例  
 下列範例說明必須使用 `var` 關鍵字的常見情節：產生匿名型別序列的查詢運算式。  在此情節中，`foreach` 陳述式中的查詢變數和反覆運算變數必須使用 `var` 設為隱含型別，因為您未具備存取匿名型別之型別名稱的權限。  如需匿名型別的詳細資訊，請參閱[匿名類型](../../../visual-basic/programming-guide/language-features/objects-and-classes/anonymous-types.md)。  
