...
|member|描述|  
|------------|-----------------|  
|[領域](./fields.md)|欄位是在類別範圍中宣告的變數。 欄位可以是內建的數字類型或其他類別的執行個體。 例如，行事曆類別可能包含目前日期的欄位。|  
...
|[運營商](../../language-reference/operators/index.md)|多載運算子視為類型成員。 當您多載運算子時，可將其定義為類型中的公用靜態方法。 如需詳細資訊，請參閱[運算子多載](../../language-reference/operators/operator-overloading.md)。|  
