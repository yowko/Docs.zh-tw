...
## <a name="example"></a>範例  
 ...
 在下列範例中，`ProgrammingGuide` 命名空間中的 `CustomClass` 有三個成員：執行個體建構函式、名稱為 `Number` 的屬性，以及名稱為 `Multiply` 的方法。 `Main`類別中的方法 `Program` 會建立 (物件) 的實例 `CustomClass` ，並使用點標記法來存取物件的方法和屬性。
