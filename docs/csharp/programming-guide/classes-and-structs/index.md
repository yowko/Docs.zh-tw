...
類別 \(Class\) 和結構 \(Struct\) 是 .NET Framework 一般類型系統中的兩個基本建構。  這兩個本質上都是資料結構 \(Structure\)，封裝了一整組資料和行為以組成邏輯單元。  資料和行為是類別或結構的「*成員*」\(Member\)，各自包含自己的方法、屬性和事件等，本主題稍後會列出。  
 ...
 如需詳細資訊，請參閱 [類別](../../../standard/base-types/classes.md)、[物件](../../../csharp/programming-guide/classes-and-structs/objects.md)和[結構](../../../csharp/programming-guide/classes-and-structs/structs.md)。  
...
## 封裝  
 ...
 如需類別的詳細資訊，請參閱[類別](../../../standard/base-types/classes.md) 和[物件](../../../csharp/programming-guide/classes-and-structs/objects.md)。  
...
### 繼承  
 類別 \(非結構\) 支援繼承概念。  繼承自另一個類別 \(「*基底類別*」\(Base Class\)\) 的類別會自動包含基底類別的所有 public、protected 和 internal 成員，但不包括其建構函式和解構函式。  如需詳細資訊，請參閱 [繼承](../../../fsharp/language-reference/inheritance.md)和 [多型](../../../csharp/programming-guide/classes-and-structs/polymorphism.md)。  
...
### 靜態類型  
 類別 \(而不是結構\) 可以宣告為[靜態](../../../visual-basic/language-reference/modifiers/static.md)。  靜態類別只能包含靜態成員，且不可使用 new 關鍵字執行個體化。  程式載入時，會將一個類別副本載入至記憶體中，而且其成員可透過類別名稱存取。  類別和結構都可以包含靜態成員。  如需詳細資訊，請參閱 [靜態類別和靜態類別成員](../../../csharp/programming-guide/classes-and-structs/static-classes-and-static-class-members.md)。  
...
### 匿名類型  
 在不方便或不需要建立具名類別的情況下，例如使用不需要永久保存或傳遞至另一個方法的資料結構填入清單時，可以使用匿名類型。  如需詳細資訊，請參閱[匿名類型](../../../visual-basic/programming-guide/language-features/objects-and-classes/anonymous-types.md)。  
