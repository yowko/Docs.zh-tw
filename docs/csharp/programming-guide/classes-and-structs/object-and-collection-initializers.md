...
## 具有匿名類型的物件初始設定式  
 雖然物件初始設定式可以用於任何內容，但是在 [!INCLUDE[vbteclinq](../../../csharp/includes/vbteclinq_md.md)] 查詢運算式中特別有用。  查詢運算式經常使用[匿名類型](../../../visual-basic/programming-guide/language-features/objects-and-classes/anonymous-types.md)，此類型只能使用物件初始設定式初始化，如下列宣告所示。  
