...
## <a name="struct-instances-vs-class-instances"></a>結構實例與類別實例的比較  
 ...
 `p1` 和 `p2` 的記憶體都配置在執行緒堆疊上。 該記憶體會和其宣告所在的型別或方法一起回收。 這是在指派時複製結構的原因之一。 相較之下，當物件的所有參考都超出範圍時，通用語言執行平台會自動回收為類別執行個體配置的記憶體 (記憶體回收)。 您不可能像在 C++ 中一樣決定性終結類別物件。 如需在 .NET 中進行垃圾收集的詳細資訊，請參閱[垃圾收集](../../../standard/garbage-collection/index.md)。  
