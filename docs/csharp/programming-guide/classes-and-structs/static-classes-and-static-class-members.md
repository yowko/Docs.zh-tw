...
[靜態](../../../visual-basic/language-reference/modifiers/static.md)類別基本上跟非靜態類別相同，但有一個差別：靜態類別不能具現化 \(Instantiated\)。  換句話說，您不能使用 [new](../../../csharp/language-reference/keywords/new.md) 關鍵字建立類別型別的變數。  由於沒有執行個體變數，您要使用類別名稱本身存取靜態類別的成員。  例如，如果您有稱為 `UtilityClass` 的靜態類別，其中有稱為 `MethodA` 的公用方法，您必須如下列範例所示呼叫方法：  
