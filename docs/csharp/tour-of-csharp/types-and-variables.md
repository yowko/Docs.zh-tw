...
`interface` 型別將協定定義為一組具名的公用函式成員。 實作 `class` 的 `struct` 或 `interface` 必須提供介面的函式成員的實作。 `interface` 可以繼承自多個基底介面，`class` 或 `struct`可實作多個介面。
...
`class`、`struct`、`interface`和 `delegate` 類型全都支援泛型，因此它們可以使用其他類型進行參數化。
