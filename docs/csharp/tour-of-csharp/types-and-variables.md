...
`class`、`struct`、`interface` 和 `delegate` 型別全都支援泛型，因此它們可以使用其他型別參數化。
...
當實值型別的值轉換成 `object` 型別時，會配置一個 `object` 執行個體 (亦稱為「Box」) 以包含值，而值會複製到該 Box。 相反地，當 `object` 參考轉換為實值型別時，會檢查參考的 `object` 是否為數值型別正確的 Box，如果檢查成功，則會將 Box 中的值複製出來。

C# 的整合型別系統實際上意味者實值型別可以「依需求」變成物件。 因為整合，使用 `object` 型別的一般用途程式庫就能搭配參考型別和實值型別使用。
