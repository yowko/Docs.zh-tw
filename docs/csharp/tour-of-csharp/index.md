...
C#（「請參閱銳利」）是現代化、物件導向且型別安全的程式設計語言。 C# 源自於是 C 系列語言，使用 C、C++、Java 和 JavaScript 的程式設計人員會立即感到熟悉。

本教學課程提供8和更早版本中C#語言的主要元件總覽。 如果您想要透過互動式範例探索語言，請嘗試教學課程[簡介C# ](../tutorials/intro-to-csharp/index.md) 。

C# 是物件導向的語言，但 C# 更進一步支援「元件導向」程式設計。 現代軟體設計逐漸依賴功能性上獨立與屬於自我描述套件的軟體元件。 這類元件的關鍵在於，它們會呈現具有屬性、方法和事件的程式設計模型。 它們具有可提供元件相關宣告式資訊的屬性。 他們會併入自己的檔。 C#提供語言結構來直接支援這些概念， C#並採用自然語言來建立和使用軟體元件。

有C#數個功能有助於建立強大和持久的應用程式。 ***垃圾收集***會自動回收因為無法使用的物件所佔用的記憶體。 ***例外狀況處理***提供結構化且可延伸的方法來偵測及復原錯誤。 語言的***型別安全***設計使得無法從未初始化的變數讀取、將陣列索引超出其界限，或執行未檢查的型別轉換。
...
為了確保C#程式和程式庫能夠以相容的方式在一段時間內演變， C#我們將著重于設計的***版本***設定。 許多程式設計語言都不太注意此問題。 因此，在引進較新版本的相依程式庫時，以這些其他語言撰寫的程式會有更多的頻率。 C#以版本設定考慮直接影響的設計層麵包括個別的 `virtual` 和 `override` 修飾詞、方法多載解析的規則，以及明確介面成員宣告的支援。

在較新的版本C#中，會採用其他程式設計範例。 C#已包含支援函式程式設計技術（例如 lambda 運算式）的功能。 其他新功能支援分隔資料和演算法，例如模式比對。
...
C# 原始程式檔的副檔名通常是 `.cs`。 若要建立此程式，請先下載並安裝[.NET Core SDK](https://dotnet.microsoft.com/download)。 然後，執行命令 `dotnet new console -o hello` 建立新程式和組建腳本。 程式和組建腳本分別位於 [檔案] `Program.cs` 和 [`hello.csproj`]。 您可以使用 `run` 命令來建立和執行應用程式：
