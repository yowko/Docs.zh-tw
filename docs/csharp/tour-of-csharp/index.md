...
本教學課程提供7和更新版本中C#語言的主要元件總覽。 如果您想要透過互動式範例探索語言，請嘗試教學課程[簡介C# ](../tutorials/intro-to-csharp/index.md) 。
...
C# 原始程式檔的副檔名通常是 `.cs`。 假設 "Hello，World" 程式儲存在檔案*hello.cs*中，程式可能會使用命令列來編譯：

```console
csc hello.cs
```

這會產生名為*hello .exe*的可執行元件。 當此應用程式執行時，它產生的輸出為：
