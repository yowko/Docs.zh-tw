...
C# 建置工具將最新的主要語言版本視為預設語言版本。 主要版本之間可能存在單點發行版本，此節的其他文章對此進行了詳細介紹。 若要使用小數點版本中的最新功能，您需要[設定編譯器語言版本](../language-reference/configure-language-version.md)並選取該版本。 自 c # 7.0 起，已有三個點發行：
...
當您回頭看一下，以 Visual Studio .NET 2002 發行的 c # 版本1.0，看起來很像 JAVA。 在[其聲明的 ECMA 設計目標當中](https://feeldotneteasy.blogspot.com/2011/01/c-design-goals.html)，它試圖成為「簡單、現代化、一般用途的物件導向語言」。  同時，看似 Java 表示它達成了那些早期的設計目標。
...
以 Visual Studio 2010 發行的 c # 版本4.0，會有一個很棘手的時間，也就是版本3.0 的突破性狀態。 3.0 版開始，C# 讓語言穩固地擺脫 Java 的影子，並建立聲望。 語言很快地變優雅。
...
- [內嵌 interop 類型](../../framework/interop/type-equivalence-and-embedded-interop-types.md)
...
以 Visual Studio 2012 發行的 c # 版本5.0 是語言的焦點版本。 幾乎該版本的所有心血都投入了另一個奠基的語言概念：非同步程式設計的 `async` 和 `await` 模型。  以下是主要的功能清單：
...
在 3.0 和 5.0 版本中，C# 在物件導向語言中新增了一些重大的新功能。 隨著版本6.0 的發行，Visual Studio 2015，它會從執行主要的最佳功能開始，改為發行許多較小的功能，讓 c # 程式設計更具生產力。 以下說明其中一部分：
...
C # 版本7.0 已發行，Visual Studio 2017。 此版本擁有 C# 6.0 中的某些進化和酷炫的東西，但是沒有編譯器作為服務。 下列為部分新功能：
...
## <a name="c-version-71"></a>C # 版本7。1

C # 已開始使用 c # 7.1*發行點版本*。 這個版本新增了[語言版本選擇設定專案](../language-reference/configure-language-version.md)、三種新的語言功能和新的編譯器行為。

此版本的新款語言功能包括：

- [`async``Main`方法](./csharp-7-1.md#async-main)
  - 應用程式的進入點允許使用`async`修飾詞。
- [`default`常值運算式](./csharp-7-1.md#default-literal-expressions)
...
- [`private protected`存取修飾詞](./csharp-7-2.md#private-protected-access-modifier)
  - 您可利用 `private protected` 存取修飾詞，存取相同組件中的衍生類別。
- [條件 `ref` 運算式](./csharp-7-2.md#conditional-ref-expressions)
  - 條件運算式 (`?:`) 的結果現在可以是參考。

## <a name="c-version-73"></a>C # 版本7。3
...
- [您可以重新指派 `ref` 本機變數。](csharp-7-3.md#ref-local-variables-may-be-reassigned)
- [您可以在陣列上使用初始化運算式 `stackalloc` 。](csharp-7-3.md#stackalloc-arrays-support-initializers)
- [您可以使用 `fixed` 語句搭配任何支援模式的類型。](csharp-7-3.md#more-types-support-the-fixed-statement)
- [您可以使用其他的泛型限制式。](csharp-7-3.md#enhanced-generic-constraints)

已在現有功能中提供下列增強功能：

- [您可以 `==` `!=` 使用元組類型來測試和。](csharp-7-3.md#tuples-support--and-)
...
- [已改善引數因不同而有所差異的方法解析 `in` 。](csharp-7-3.md#in-method-overload-resolution-tiebreaker)
- [多載解析現在會有較少模稜兩可的情況。](csharp-7-3.md#improved-overload-candidates)

新的編譯器選項：

- [`-publicsign`若要啟用開放原始碼軟體 (OSS) 簽署元件。](csharp-7-3.md#public-or-open-source-signing)
- [`-pathmap`提供來原始目錄的對應。](csharp-7-3.md#pathmap)

## <a name="c-version-80"></a>C # 版本8。0

C # 8.0 是第一個特別以 .NET Core 為目標的主要 c # 版本。 某些功能依賴新的 CLR 功能，有些則是僅在 .NET Core 中新增的程式庫類型。 C # 8.0 新增下列功能，並增強 c # 語言：
...
- [非受控的結構化類型](./csharp-8.md#unmanaged-constructed-types)
- [在嵌套運算式中 Stackalloc](./csharp-8.md#stackalloc-in-nested-expressions)
- [增強內插逐字字串](./csharp-8.md#enhancement-of-interpolated-verbatim-strings)

預設介面成員需要 CLR 中的增強功能。 這些功能已加入 .NET Core 3.0 的 CLR 中。 範圍和索引，以及非同步資料流程需要 .NET Core 3.0 程式庫中的新類型。 當程式庫已標注為提供有關引數和傳回值之 null 狀態的語義資訊時，可為 null 的參考型別（在編譯器中實作為）會更加有用。 這些批註會加入 .NET Core 程式庫中。
