---
title: C# 中的 LINQ
description: LINQ 詳細資訊的主題連結。
ms.date: 11/30/2016
ms.assetid: 8eb3284f-0ab9-4cad-9216-2da58d9761a5
ms.openlocfilehash: 8501156bf2dfeff63a99a56d6f776ee7f2e350dc
ms.sourcegitcommit: 3d5d33f384eeba41b2dff79d096f47ccc8d8f03d
ms.translationtype: HT
ms.contentlocale: zh-TW
ms.lasthandoff: 05/04/2018
ms.locfileid: "33275622"
---
# <a name="linq-in-c"></a>C# 中的 LINQ
本節包含 LINQ 詳細資訊的主題連結。  
  
## <a name="in-this-section"></a>本節內容  
 [LINQ 查詢簡介](../programming-guide/concepts/linq/introduction-to-linq-queries.md)  
 說明所有語言和資料來源通用的基本 LINQ 查詢作業，共分三個部分。  
  
 [LINQ 和泛型型別](../programming-guide/concepts/linq/linq-and-generic-types.md)  
 提供 LINQ 使用的泛型型別簡介。  
  
 [使用 LINQ 轉換資料](../programming-guide/concepts/linq/data-transformations-with-linq.md)  
 說明您可以針對查詢所擷取之資料進行轉換的各種方式。  
  
 [LINQ 查詢作業中的型別關聯性](../programming-guide/concepts/linq/type-relationships-in-linq-query-operations.md)  
 描述這三個部分的 LINQ 查詢作業會如何保留及/或轉換型別。  
  
 [LINQ 中的查詢語法及方法語法](../programming-guide/concepts/linq/query-syntax-and-method-syntax-in-linq.md)  
 針對方法語法和查詢語法這兩種表示 LINQ 查詢的方式進行比較。  
  
 [支援 LINQ 的 C# 功能](../programming-guide/concepts/linq/features-that-support-linq.md)  
 描述支援 LINQ 的 C# 語言建構。  
   
## <a name="related-sections"></a>相關章節  
 [LINQ 查詢運算式](../programming-guide/linq-query-expressions/index.md)  
 包含 LINQ 的查詢概觀，並提供其他資源連結。  
  
 [標準查詢運算子概觀](../programming-guide/concepts/linq/standard-query-operators-overview.md)  
 說明 LINQ 使用的標準方法。  
  
