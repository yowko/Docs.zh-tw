...
在某些情況下，您要到執行階段才知道在 `where` 子句中必須套用多少述詞至來源項目。 以動態方式指定多個述詞篩選方式是使用 <xref:System.Linq.Enumerable.Contains%2A> 方法，如下例所示。 此範例以兩種方式建構。 首先，篩選程式中所提供的值來執行專案。 然後使用執行階段提供的輸入再次執行專案。  
