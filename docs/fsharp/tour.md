...
管線運算子，例如`|>`處理在 F # 中的資料時大量使用。 這些運算子都可讓您以有彈性的方式建立 「 管線 」 的函式的函式。 下列範例逐步解說如何使用這些運算子來建置簡單的功能性管線利用：

[!code-fsharp[Pipelines](../../samples/snippets/fsharp/tour.fs#L227-L282)]

前一個範例所使用的 F # 中，包括清單處理函式，第一級函式，許多功能和[部分應用程式](language-reference/functions/index.md#partial-application-of-arguments)。 雖然深入了解這些概念的每個可以成為稍微進階，應該很清楚如何輕鬆地函式可用來建立管線時，處理資料。
