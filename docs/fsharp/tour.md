...
## <a name="how-to-run-the-code-samples"></a>如何執行的程式碼範例

>[!NOTE]
兩個選項來執行的程式碼範例是[再試一次 F #](http://www.tryfsharp.org/Create) （需要 Silverlight） 和[F # Azure 筆記本的](https://notebooks.azure.com/Microsoft/libraries/fsharp/html/FSharp%20for%20Azure%20Notebooks.ipynb)Microsoft Azure 上。

最快的方式執行這些程式碼範例是使用[F # Interactive](tutorials/fsharp-interactive/index.md)。  只複製/貼上程式碼範例和它們在該處執行。  或者您可以設定要編譯及執行為主控台應用程式的程式碼的專案。  請參閱[開始](./get-started/index.md)區段，以進一步了解。
