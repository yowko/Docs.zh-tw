...
在F#語言中，有兩種不同的型別參數。 第一類是標準的泛型類型參數。 這些會以單引號 （'），如同`'T`和`'U`。 它們是相當於其他.NET Framework 語言中的泛型型別參數。 其他類型是以統計方式解析和是插入號以符號表示，如同`^T`和`^U`。
...
許多F#核心程式庫函式，特別是運算子，有以統計方式解析型別參數。 這些函式和運算子會以內嵌方式，並導致數值計算的高效率的程式碼產生。
...
已解析的型別`(+@)`兩者的使用根據`(+)`和`(*)`，這兩個的這會導致類型推斷來推斷成員以統計方式解析的型別參數條件約束。 解析的型別，如下所示的F#的解譯器，如下所示。
...
開頭為F#4.1，您也可以以統計方式解析的型別參數簽章中指定的具象型別名稱。  在舊版的語言中，實際上無法由編譯器推斷的型別名稱，但無法實際指定簽章中。  從F#4.1，您也可以以統計方式解析的型別參數簽章中指定的具象型別名稱。 以下為範例：
