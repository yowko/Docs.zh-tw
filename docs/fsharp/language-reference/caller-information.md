...
|[CallerMemberName](/dotnet/api/system.runtime.compilerservices.callermembernameattribute)|呼叫端的方法或屬性名稱。 請參閱本主題稍後的成員名稱 > 一節。|`String`|

## <a name="example"></a>範例

下列範例顯示如何使用這些屬性來追蹤呼叫端。
...
呼叫端資訊屬性只能套用至選擇性參數。 您必須提供明確的值為每個選擇性參數。 Caller Info 屬性會導致編譯器寫入使用 Caller Info 屬性裝飾的每個選擇性參數的值。
...
您可以使用[ `CallerMemberName` ](/dotnet/api/system.runtime.compilerservices.callermembernameattribute)屬性避免指定成員名稱做`String`引數呼叫的方法。 您可以藉由使用這項技術，避免重新命名重構並不會變更的問題`String`值。 這項優點對於下列工作特別有用：

* 使用追蹤和診斷常式。
* 實作[INotifyPropertyChanged](/dotnet/api/system.componentmodel.inotifypropertychanged)資料繫結時的介面。 這個介面可讓物件的屬性告知繫結的控制項屬性已變更，所以控制項可以顯示更新的資訊。 不含[ `CallerMemberName` ](/dotnet/api/system.runtime.compilerservices.callermembernameattribute)屬性，您必須為常值指定屬性名稱。

下圖顯示當您使用，CallerMemberName 屬性傳回的名稱的成員。
