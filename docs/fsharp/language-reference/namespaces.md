...
命名空間可以使用 namespace 關鍵字明確宣告，或在宣告模組時以隱含方式宣告。 若要明確宣告命名空間，請使用 namespace 關鍵字並在後面加上命名空間名稱。 下列範例顯示的程式碼檔案會宣告具有類型的命名空間，以及該命名空間中包含的模組 `Widgets`。

[!code-fsharp[Main](~/samples/snippets/fsharp/lang-ref-2/snippet6406.fs)]

如果檔案的整個內容都在一個模組中，您也可以使用 `module` 關鍵字，並在完整的模組名稱中提供新的命名空間名稱，以隱含方式宣告命名空間。 下列範例顯示的程式碼檔案會宣告 `Widgets` 的命名空間，以及包含函式 `WidgetsModule`模組。
...
命名空間也可以宣告為遞迴，讓所有包含的程式碼都可以互相遞迴。  這會透過 `namespace rec`來完成。 使用 `namespace rec` 可以減輕無法在類型和模組之間撰寫相互引用程式碼的一些難題。 以下是這種情況的範例：
...
請注意，例外狀況 `DontSqueezeTheBananaException`，而類別 `Banana` 兩者彼此參考。  此外，模組 `BananaHelpers` 和類別 `Banana` 也會彼此參考。 如果您從 `MutualReferences` 命名空間移除F# `rec` 關鍵字，就無法在中表示。
