...
|相異|||O （1)&AMP;#42;|||傳回序列，其中包含任何重複的項目，根據項目上的泛型雜湊和相等比較。 如果項目序列中出現多次，則會捨棄之後出現。|
|distinctBy|||O （1)&AMP;#42;|||傳回序列，其中包含任何重複的項目，根據指定的索引鍵產生函式會傳回在索引鍵上泛型雜湊和相等比較。 如果項目序列中出現多次，則會捨棄之後出現。|
...
|fold2 所控制|O （N)|O （N)|-|-|-|函式套用至兩個集合，透過計算建立累計值引數的執行緒的對應項目。 這些集合必須完全相同的大小。 如果輸入函式是 f，且項目是在 i0...和 j0...jN，此函式會計算 f （...(f s i0 j0)...)在 jN.|
...
|intersectMany|-|-|-|-|O (N1 &AMP;#42; N2...)|計算集合序列的交集。 序列不得空白。|
...
|unionMany|-|-|-|-|O (N1 &AMP;#42; N2...)|計算集合序列的聯集。|
