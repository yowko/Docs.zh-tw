...
本節是 F # 語言的參考，這是以 .NET 為目標的多架構程式設計語言。 F # 語言支援功能、物件導向和命令式程式設計模型。

## <a name="f-tokens"></a>F# 語彙基元

下表顯示的參考文章提供了在 F # 中當做標記使用的關鍵字、符號和常值資料表。
...
|[模式比對](pattern-matching.md)|描述模式，這是轉換輸入資料的規則，而且會在整個 F # 語言中使用。 您可以比較資料與模式、將資料分解成組成元件，或從資料中以各種方式將資訊解壓縮。|
...
|[XML 檔](xml-documentation.md)|描述針對 XML 文件註解 (也稱為三斜線註解) 產生文件檔案的支援。 您可以從 F # 中的程式碼批註產生檔，就像其他 .NET 語言一樣。|
...
|Title|描述|
|-----|-----------|
|[閾值](./values/index.md)|描述值，這是具有特定類型且不可變的數量；值可以是整數或浮點數、字元或文字、清單、序列、陣列、元組、差別聯集、記錄、類別類型或函式值。|
|[基本類型](basic-types.md)|說明 F # 語言中使用的基本基本類型。 它也會提供對應的 .NET 類型以及每個類型的最小值和最大值。|
...
|[得到](tuples.md)|描述元組，這是不具名但有序之值 (可能是不同的類型) 的群組。|
...
|[序列](sequences.md)|描述序列。 序列是一連串的邏輯項目，而所有項目的類型都相同。 只有在必要時才會計算個別順序元素，因此標記法可能小於常值專案計數。|
|[陣列](arrays.md)|描述陣列。 陣列是一系列固定大小、以零起始、可變的連續資料項目，而所有項目的類型都相同。|
|[記錄](records.md)|描述記錄。 記錄表示具名值的簡單彙總，並選擇性地搭配成員。|
|[差別聯集](discriminated-unions.md)|描述區分等位，其支援可能屬於各種已命名案例之一的值，每個都有可能不同的值和類型。|
...
|[彈性類型](flexible-types.md)|描述彈性類型。 彈性型別注釋表示參數、變數或值的型別與指定的型別相容，其中的相容性取決於類別或介面的物件導向階層架構中的位置。|
...
|[計算運算式](computation-expressions.md)|描述 F# 中的計算運算式提供便利的語法，用於撰寫可以使用控制流程建構和繫結進行排序和合併的計算。 它們可用來為*monad*提供便利的語法，這是一種功能程式設計功能，可用於管理功能程式中的資料、控制項和副作用。 非同步工作流程是一種計算運算式，支援非同步和平行計算。 如需詳細資訊，請參閱[非同步工作流程](asynchronous-workflows.md)。|
...
|[原始碼程式行、檔案與路徑識別項](source-line-file-path-identifiers.md)|描述識別碼 `__LINE__` 、 `__SOURCE_DIRECTORY__` 和 `__SOURCE_FILE__` ，這些都是內建值，可讓您存取程式碼中的原始程式列號、目錄和檔案名。|
