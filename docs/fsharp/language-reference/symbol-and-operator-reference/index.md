...
|`!=`||<ul><li>F# 中不使用。 使用 `<>` 進行不等比較運算。<br /></li></ul>|
...
|`==`||<ul><li>F# 中不使用。 使用 `=` 進行等號比較運算。<br /></li></ul>|
...
|`^`|[以統計方式解析的型別參數](../generics/statically-resolved-type-parameters.md)<br /><br />[字串](../strings.md)|<ul><li>指定必須在編譯階段 (而非執行階段) 解析的型別參數。<br /></li><li>串連字串。<br /></li></ul>|
