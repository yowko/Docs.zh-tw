---
title: 符號和運算子參考
description: 瞭解程式F#設計語言中使用的符號和運算子。
ms.date: 02/11/2019
...
|`.`|[成員](../members/index.md)<br /><br />[基本類型](../basic-types.md)|<ul><li>存取成員，並分隔完整名稱中的個別名稱。<br /></li><li>以浮點數指定小數點。<br /></li></ul>|
...
|`:?`|[比對運算式](../match-expressions.md)|<ul><li>如果值符合指定的類型，則傳回 `true` （包括它是否為子類型）。否則，會傳回 `false` （型別測試運算子）。<br /></li></ul>|
...
|`>=`|[算術運算子](arithmetic-operators.md)|<ul><li>如果左邊大於或等於右邊，則傳回 `true`;否則，會傳回 `false`。<br /></li></ul>|
...
|`&`、 `&&`|左方|
|`:>`、 `:?>`|右方|
|`<`*op*、`>`*op*、`=`、<code>&#124;</code>*op*、`&`*op*、`&`<br /><br />(包括 `<<<`、`>>>`、<code>&#124;&#124;&#124;</code>、`&&&`)|左方|
|`^`*op*<br /><br />(包括 `^^^`)|右方|
|`::`|右方|
|`:?`|未關聯|
|`-`*op*、`+`*op*|適用於這些符號的中置用法|
