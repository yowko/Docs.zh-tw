...
## <a name="remarks"></a>備註
先前的語法中的三種形式顯示定義索引的屬性都有`get`和`set`方法，有`get`方法，或具有`set`只方法。 您也可以合併兩者，僅 get 與集，顯示的語法中所顯示的語法，並產生具有 get 和 set 屬性的屬性。 此第二個表單可讓您將不同的存取範圍修飾詞和屬性放在 get 和 set 方法。

當*PropertyName*是`Item`，編譯器會將屬性視為預設索引屬性。 A*預設索引的屬性*是屬性，您可以存取的物件執行個體上使用類似陣列的語法。 例如，如果`obj`是定義這個屬性，語法的型別物件`obj.[index]`用來存取屬性。
