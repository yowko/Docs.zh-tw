...
## <a name="using-azure-virtual-machines-with-f"></a>搭配 F\# 使用 Azure 虛擬機器
...
## <a name="using-azure-functions-with-f"></a>搭配 F\# 使用 Azure Functions
...
## <a name="using-azure-storage-with-f"></a>搭配 F\# 使用 Azure 儲存體
...
## <a name="using-azure-app-service-with-f"></a>搭配 F\# 使用 Azure App Service
...
Azure Cosmos DB 可以透過兩種F#方式來使用：

1. 建立F# Azure Functions，以回應或導致 Azure Cosmos DB 集合的變更。 請參閱[Azure Functions 的 Azure Cosmos DB](/azure/azure-functions/functions-bindings-cosmosdb)系結，或
2. 使用適用于[SQL API 的 Azure Cosmos DB .NET SDK](/azure/cosmos-db/sql-api-sdk-dotnet)。 相關的範例位於C#。

## <a name="using-azure-event-hubs-with-f"></a>搭配 F\# 使用 Azure 事件中樞
...
## <a name="using-azure-notification-hubs-with-f"></a>搭配 F\# 使用 Azure 通知中樞
...
## <a name="implementing-webhooks-on-azure-with-f"></a>使用 F\# 在 Azure 上執行 Webhook
...
## <a name="using-webjobs-with-f"></a>搭配使用 Webjob 與 F\#
...
## <a name="implementing-timers-on-azure-with-f"></a>使用 F\# 在 Azure 上執行計時器
...
同樣地，其他 Azure 資源也可以使用相同的元件，透過 F# 指令碼進行部署和管理。 例如，您可以建立儲存體帳戶、部署 Azure 雲端服務、建立 Azure Cosmos DB 實例，並從F#腳本以程式設計方式管理 Azure 通知中樞。
