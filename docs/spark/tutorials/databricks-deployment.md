...
本教學課程會指導您如何透過 Azure Databricks （以 Apache Spark 為基礎的分析平臺），將應用程式部署至雲端，並提供單鍵設定、簡化的工作流程，以及啟用共同作業的互動式工作區。
...
2. 在 [Azure Databricks 服務]  底下，提供值以建立 Databricks 工作區。
    ...
    |**資源群組**     | 指定您是要建立新的資源群組，還是使用現有資源群組。 資源群組是存放 Azure 方案相關資源的容器。 如需詳細資訊，請參閱 [Azure 資源群組概觀](/azure/azure-resource-manager/resource-group-overview)。 |
...
1. Databricks CLI 需要 Python 3.6 或更新版本。 如果您已經安裝 Python，您可以略過此步驟。

   **若為 Windows：**

   [下載適用于 Windows 的 Python](https://www.python.org/ftp/python/3.7.4/python-3.7.4.exe)

   **針對 Linux：** Python 已預先安裝在大部分的 Linux 發行版本上。 執行下列命令以查看您已安裝的版本：
...
3. 安裝 Databricks CLI 之後，請開啟新的命令提示字元，然後執行命令 `databricks` 。 如果您收到「 **databricks」無法辨識為內部或外部命令的錯誤**，請確定您已開啟新的命令提示字元。
...
3. 輸入您的主機之後，系統會提示您輸入權杖。 在 [Azure 入口網站中，選取 [ **啟動工作區** ] 以啟動您的 Azure Databricks 工作區。
...
5. 在 [使用者設定] 頁面上，您可以產生新的權杖。 複製產生的權杖，並將其貼回命令提示字元。
...
2. 請執行下列工作來壓縮已發佈的應用程式檔，讓您可以輕鬆地將它們上傳至您的 Databricks Spark 叢集。

   **在 Windows 上：**

   流覽至 mySparkApp/bin/Release/netcoreapp 3.1/ubuntu. 16.04-x64。 然後，在 [ **發行** 資料夾] 上按一下滑鼠右鍵，然後選取 [ **傳送至 > 壓縮 (壓縮) 資料夾**。 將新資料夾命名為 **publish.zip**。
...
1. 流覽至您的作業，然後選取 [ **編輯** ] 以設定作業的叢集。

2. 將您的叢集設定為 **Spark 2.4.1**。 然後，選取 [ **Advanced Options**  >  **Init Scripts**]。 將 Init 腳本路徑設定為 `dbfs:/spark-dotnet/db-init.sh` 。
...
2. 建立作業的叢集需要幾分鐘的時間。 一旦建立之後，您的作業就會提交，而且您可以查看輸出。

3. 從 **左側功能表中選取** [叢集]，然後選取 [名稱] 和 [執行您的作業]。

4. 選取 [ **驅動程式記錄** ] 以查看作業的輸出。 當您的應用程式完成執行時，您會看到寫入至標準輸出主控台的「開始使用」本機回合中有相同的「字數統計」表格。
