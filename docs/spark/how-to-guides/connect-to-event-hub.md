...
1. 讓事件中樞命名空間準備好使用事件中樞，請參閱 [這份檔](https://docs.microsoft.com/azure/event-hubs/event-hubs-create) ，以取得如何執行此作業的逐步指南。 建立事件中樞命名空間時，請務必選取標準定價層。

## <a name="what-is-azure-event-hubs"></a>什麼是 Azure 事件中樞

[Azure 事件中樞](https://docs.microsoft.com/en-us/azure/event-hubs/event-hubs-about) 是大型的資料串流平臺和事件內嵌服務。 它是完全受控的平臺即服務 (PaaS) ，可輕鬆地與 [Apache Kafka](https://kafka.apache.org/) 整合，讓您無需管理、設定或執行自己的叢集，就能為您提供 PaaS Kafka 體驗。

## <a name="connect-your-application-to-azure-event-hubs"></a>將您的應用程式連接到 Azure 事件中樞

1. 請取得事件中樞連接字串和完整網域名稱 (FQDN) 以供稍後使用。 如需相關指示，請參閱[取得事件中樞連接字串](https://docs.microsoft.com/azure/event-hubs/event-hubs-get-connection-string)。
...
若要執行 Apache Spark 應用程式的 .NET，您應該在 `spark-sql-kafka-0-10` Spark 專案中將模組定義為組建定義的一部分，並 `libraryDependency` 在 `build.sbt` sbt 專案中使用。 針對 (或) 這類 Spark 環境 `spark-submit` `spark-shell` ，您應該使用 `--packages` 命令列選項，如下所示：
