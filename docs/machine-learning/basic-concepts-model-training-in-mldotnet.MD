...
本文章說明的基本概念涵蓋 ML.NET 中的*模型定型*情節。 並非所有概念都與*使用現有模型進行預測*的更簡單情節相關。
...
- [*資料讀取器*](#data-reader)，以 `IDataReader<T>` 介面表示。
  ...
  - 例如，*TextLoader* 是 `IDataReader<FileSource>`。 它會取得檔案來源並產生資料。 
- [*評估工具*](#estimator)，以 `IEstimator<T>` 介面表示。
  - 這是從資料學習的物件。 學習的結果是*轉換器*。
  - 您可以將機器學習*演算法*視為評估工具，其會根據資料學習，並產生機器學習*模型*。
- [*預測函數*](#prediction-function)，以 `PredictionFunction<TSrc, TDst>` 類別表示。
...
ML.NET 中大部分的轉換器傾向於一次操作一個*輸入資料行*，以及產生*輸出資料行*。 例如，`new HashTransformer("foo", "bar")` 會從資料行 "foo" 取得值、將其雜湊，然後將它們放入資料行 "bar" 中。 

輸入和輸出資料行名稱相同也是很常見的。 在此情況下，舊資料行會取代為新資料行。 例如，`new HashTransformer("foo")` 會從資料行 "foo" 取得值、將其雜湊，然後將它們放回 "bar"。 

當然，在呼叫 `Transform` 時，任何轉換器都會產生新的資料檢視。 請記住，資料檢視是不可變的。
...
沒有實際計算發生。 只在從 `newData` 取得資料指標，並開始使用值之後，`newData` 才會叫用 `transformer` 轉換邏輯 (甚至只在 `transformer` 有問題時，才需要產生所要求的資料行)。

### <a name="transformer-chains"></a>轉換器鏈結

轉換器的有用屬性是，*您可以將循序應用的轉換器表達為另一個轉換器*：
...
轉換器的另一個類似之處是，因為資料會延遲評估，所以*讀取器會延遲執行*：呼叫 `dataReader.Read()` 時，沒有 (或最少) 實際「讀取」發生：僅在產生的資料要求資料指標時，讀取器才會開始運作。
...
但這樣做很麻煩，且會產生效能成本。 反之，預測函數物件可透過擴充方法 `MakePredictionFunction` 執行相同工作，但更快速且更方便：
