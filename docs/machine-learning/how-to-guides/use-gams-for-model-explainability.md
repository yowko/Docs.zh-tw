...
---
# <a name="use-generalized-additive-models-and-shape-functions-for-model-interpretability-in-mlnet"></a>在 ML.NET 中針對模型 interpretability 使用一般化累加式模型和形狀函數

在建立機器學習模型時，模型通常不足以直接用來進行預測。 一般來說，機器學習開發人員、決策者，以及受該模型影響的人員需要了解機器學習模型如何做出決策，以及哪些特徵會影響其效能。 **一般化的加法模型（gam）** 是在 Microsoft 內部用來進行模型 interpretability，以協助機器學習開發人員建立可輕鬆地由其他人解讀的高容量模型。
