...
ML.NET 能讓您將機器學習新增至 .NET 應用程式。 使用這項功能，您可以使用應用程式可用的資料來建立自動預測。 本文說明 ML.NET 的機器學習基本概念。 
...
在程式碼片段中，`Concatenate` 和 `Sdca` 都是目錄中的方法。 它們會各自建立附加至管線的 [IEstimator](xref:Microsoft.ML.IEstimator`1) 物件。
