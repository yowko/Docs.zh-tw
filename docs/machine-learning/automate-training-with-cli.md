...
> [!NOTE]
> 本主題是指 ML.NET **CLI**和 ML.NET **AutoML**（目前為預覽狀態），而材質可能會有所變更。

## <a name="what-is-the-mlnet-command-line-interface-cli"></a>什麼是 ML.NET 命令列介面（CLI）？

ML.NET CLI 是一種[.Net Core 工具](../core/tools/global-tools.md)。 安裝之後，您會提供機器學習工作和訓練資料集，並產生 ML.NET 模型，以及要在應用程式中執行以使用模型的 c # 程式碼。

如下圖所示，您可以輕鬆地產生高品質的 ML.NET 模型（序列化模型 .zip 檔案），再加上用來執行/評分該模型的範例 c # 程式碼。 此外，也產生建立/定型模型的 C# 程式碼，以便您可以研究並重覆所產生「最佳模型」使用的演算法和設定。
