...
1. 安裝 **Microsoft.ML** NuGet 套件：
    ...
    在**方案總管**中，對專案按一下滑鼠右鍵，然後選取 [管理 NuGet 套件]。 選擇 "nuget.org" 作為 [套件來源]、選取 [瀏覽] 索引標籤、搜尋 **Microsoft.ML**、從清單中選取該套件，然後選取 [安裝] 按鈕。 在 [預覽變更] 對話方塊上，選取 [確定] 按鈕，然後在 [授權接受] 對話方塊上，如果您同意所列套件的授權條款，請選取 [我接受]。 對 **Microsoft.ML.FastTree** NuGet 套件執行相同的動作。
...
將 `Console.WriteLine("Hello World!")` 方法中的 `Main` 程式碼替換成下列程式碼，宣告並初始化 `mlContext` 變數：
...
資料準備工作的最後一個步驟是使用 **轉換類別，將所有特徵資料行合併為**Features`mlContext.Transforms.Concatenate` 資料行。 根據預設，學習演算法只會處理來自 **Features** 資料行的特徵。 新增下列程式碼：
...
請將下列內容新增為 [ 中的下一行程式碼，來將 ](xref:Microsoft.ML.Trainers.FastTree.FastTreeRegressionTrainer)FastTreeRegressionTrainer`Train()` 機器學習服務工作新增到資料轉換定義：
...
在 `dataview` 方法中新增下列程式碼，調整模型使其符合定型的 `Train()`，並傳回已定型的模型：
...
接下來，使用測試資料評估您的模型效能，以進行品質保證和驗證。 在 `Evaluate()` 之後，使用下列程式碼建立 `Train()` 方法：
...
請使用下列程式碼，在緊接著 `Main` 方法呼叫底下，從 `Train` 方法新增對新方法的呼叫：
...
接下來，透過將下列程式碼新增到 `Test` 來轉換 `Evaluate()` 資料：
...
請使用下列程式碼，在緊接著 `TestSinglePrediction` 方法之後，建立 `Evaluate` 方法：
...
將下列程式碼新增到 `PredictionEngine` 來使用 `TestSinglePrediction()` 預測車資：
...
本教學課程會使用此類別內的一個測試行程。 您可以稍後新增其他案例來對此方法進行實驗。 透過建立 `TestSinglePrediction()` 的執行個體，在 `TaxiTrip` 方法中新增車程，以測試定型模型的費用預測：
...
接下來，根據計程車車程資料的單一執行個體預測車資，然後透過將下列內容作為 `PredictionEngine` 方法的下一行程式碼，來將它傳遞至 `TestSinglePrediction()`：
