...
命名規則會考慮 .NET 程式設計語言程式碼元素的命名，例如類別、屬性和方法。 例如，您可以指定公用成員必須為大寫，或私用欄位必須以開頭 `_` 。
...
**\<prefix>** 指定要定義的專案類型為 &mdash; 命名規則、符號群組或命名樣式， &mdash; 而且必須是下列其中一項：
...
1. 只有在開發 Ide （例如 Visual Studio）中才遵守命名規則內的嚴重性規格。 C # 或 VB 編譯器無法理解這項設定，因此不會在組建期間遵守。 相反地，若要在組建上強制執行命名樣式規則，您應該使用以規則識別碼為基礎的嚴重性設定來設定嚴重性，[如本節所述。](#rule-id-ide1006-naming-rule-violation) 如需詳細資訊，請參閱此 [GitHub 問題](https://github.com/dotnet/roslyn/issues/44201)。
...
您可以設定符號群組的下列屬性，以限制要包含在群組中的符號。 若要在單一屬性設定中指定多個值，請以逗號分隔它們。
...
## <a name="rule-id-ide1006-naming-rule-violation"></a><a name="rule-id-ide1006-naming-rule-violation"></a>規則識別碼： "IDE1006" (命名規則違規) 
